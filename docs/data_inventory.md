...
| Operator-maintained live inputs | `operator_inputs/` | Manual/live inputs such as `open_positions.json`. Gate 2 fields like `entry_date` and `target_price` are verified here, not under `data/`. |

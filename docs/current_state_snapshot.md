...
- `exp-20260610-014` `accepted_shared_default_off_revision_allocator_source_extension`: EV `+0.9720`, PnL `$15,197.05`, family `production_visible_default_off_paper_adapter_for_candidate_pool_alpha`, trial `accepted_default_off_helper_source_priority_allocation`.
  Lesson: Revision-surprise low-extension is now a fixed rank-5 source in the shared accepted-helper allocator. It uses replayable earnings/revision snapshots, stays default-off, and beat the exp-20260610-005 comparator in all three canonical windows.
...
- `pilot_or_sleeve` priority `0.6323`, experiments `162`, accept `25.93%`.

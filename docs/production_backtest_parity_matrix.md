...
membership rows, and an identity-only Engine-0 cash/no-signal baseline; it does not feed the legacy production runner,
...
| Research-only Engine-0 cash/no-signal baseline | `v2_engine0_baseline.py` consumes a separately frozen market-clock hash and exact membership rows, separately frozen CandidatePool/Hypothesis identities, and the complete cross-validated research dependency graph | replay and daily are true aliases; the full CandidatePool UniverseEvent graph is projected at the clock cutoff and must exactly equal every market-clock row, including non-candidate states and exact event record hashes, before the immutable policy emits a DecisionRecord | the identical callable and policy hash-seal the verified row lineage into DecisionContext, hold cash, emit no side/risk/size/currency or OrderIntent, and preserve `trade_enabled=false` | `engine0_baseline_scope=validated_candidate_pool` and `membership_lineage_status=verified_exact_rows`; external coverage remains unverified and `pit_tier=research_pit`, `result_ceiling=observed_only`, runtime/production parity remain unwired, so no canonical, promotion, paper/live, execution, or trading parity is claimed |

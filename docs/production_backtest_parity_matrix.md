...
research-only market-decision-clock binder that preserves the validated exact
membership rows, a source-bounded dynamic PIT market-universe snapshot, an
identity-only Engine-0 cash/no-signal baseline, and a sibling research-only
claim-support feature/ranking helper; it does not feed the legacy production runner,
...
| Research-only market decision clock | `v2_market_decision_clock.py` consumes one explicit observation plus a complete evidence-bound `SessionClock` chain | replay and daily are true aliases; observation `as_of` must equal `assignment_cutoff` as a UTC instant, clock freeze/record must be strictly pre-open, and v2 carries the already validated exact membership rows | the identical callable revalidates row schema/order/identity/cutoff plus the semantic membership count/hash while binding adapter/backend/manifest/universe and clock/calendar/session/cutoff identities without process-wall-clock fallback | `market_decision_clock_status=bound_research_only` and `parity_status=contract_only_unwired`; no dynamic market-wide completeness, Engine-0 policy/baseline, scheduler, production/backtest, execution, canonical, or paper/live parity is claimed |
| Research-only dynamic PIT market-universe snapshot | `v2_dynamic_market_universe.py` is a post-CandidatePool, pre-predictive-policy reconciliation boundary over the separately frozen market-clock, CandidatePool/Hypothesis identities, referenced research-graph constraints, and full CandidatePool UniverseEvent population; it does not generate CandidatePool | replay and daily are true aliases; both project every current state at the clock cutoff and require exact equality with the clock rows, including non-candidate memberships, mapping, event semantic/record identity, and effective time | the identical callable freezes exact event, membership, state-count, row-lineage, and candidate-surface identities; no feature, score, rank, signal, sizing, outcome, or order is produced | `dynamic_market_universe_status=verified_exact_rows_research_only` applies only to the source-bounded frozen population; external market-wide coverage remains unverified and `pit_tier=research_pit`, `result_ceiling=observed_only`, scheduler/runtime/production, canonical, promotion, paper/live, execution, and trading parity remain unclaimed |
| Research-only Engine-0 cash/no-signal baseline | `v2_engine0_baseline.py` v3 consumes the separately frozen dynamic market-universe snapshot plus the CandidatePool/Hypothesis identities already validated by that boundary | replay and daily are true aliases; the dynamic snapshot validator owns referenced-graph validation plus population/exact-row reconciliation, after which Engine-0 adds only immutable cash-baseline checks before emitting a DecisionRecord | the identical callable and policy bind the separately frozen dynamic snapshot hash and verified row lineage into DecisionContext, hold cash, emit no side/risk/size/currency or OrderIntent, and preserve `trade_enabled=false` | `engine0_baseline_scope=validated_candidate_pool` and `membership_lineage_status=verified_exact_rows`; external coverage remains unverified and `pit_tier=research_pit`, `result_ceiling=observed_only`, runtime/production parity remain unwired, so no canonical, promotion, paper/live, execution, or trading parity is claimed |
| Research-only shared claim-support feature/ranking | `v2_predictive_policy.py` is a sibling consumer of the same separately frozen dynamic-universe, CandidatePool/Hypothesis, claim/evidence, and clock identities; it rebuilds claim-support metadata internally and accepts no caller feature rows or score callback. Its consumer validator also requires a separately frozen envelope hash, explicit decision identity/times, and the exact record snapshot of hypothesis-referenced claims | replay and daily builders remain true aliases; the identical rule ranks admitted rows by feature availability, maximum relevant claim confidence, and candidate-entry id, while exact feature/rank/no-entry signal snapshots are hash-bound into a treatment DecisionRecord. Validation reconstructs that complete envelope from the same dependencies and requires exact fields, self-hash, and canonical payload | the same pure callable emits the same feature, rank, context, and DecisionRecord for equivalent frozen inputs; the validator returns only its dependency-reconstructed plain payload and rejects record-only claim variants or resealed feature/rank/signal/context/DecisionRecord drift. Every admitted row remains `not_selected`, with no side/risk/size/currency or OrderIntent | claim confidence is research metadata, not calibrated return prediction; predictive efficacy and entry remain unestablished. The validator is a trust boundary, not a new runtime stage: `contract_only_unwired`, external coverage unverified, `research_pit / observed_only`, runtime/production, canonical, promotion, paper/live, execution, and trading parity remain unclaimed |

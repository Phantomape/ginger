...
| Research-only shared claim-support feature/ranking | `v2_predictive_policy.py` is a sibling consumer of the same separately frozen dynamic-universe, CandidatePool/Hypothesis, claim/evidence, and clock identities; it rebuilds claim-support metadata internally and accepts no caller feature rows or score callback | replay and daily are true aliases; the identical rule ranks admitted rows by feature availability, maximum relevant claim confidence, and candidate-entry id, while exact feature/rank/no-entry signal snapshots are hash-bound into a treatment DecisionRecord | the same pure callable emits the same feature, rank, context, and DecisionRecord for equivalent frozen inputs; every admitted row remains `not_selected`, with no side/risk/size/currency or OrderIntent | claim confidence is research metadata, not calibrated return prediction; predictive efficacy and entry remain unestablished. `contract_only_unwired`, external coverage unverified, `research_pit / observed_only`, runtime/production, canonical, promotion, paper/live, execution, and trading parity remain unclaimed |

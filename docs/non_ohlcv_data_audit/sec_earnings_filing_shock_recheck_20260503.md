# SEC / Earnings / Filing Shock Recheck - exp-20260503-019

## Hypothesis

SEC filing shock, financial surprise, 8-K event type, and post-earnings drift may still improve C-strategy grading or A/B event confirmation. This run only checks whether new PIT-safe evidence arrived after `exp-20260503-016`.
...
`new PIT-safe SEC / earnings filing-shock evidence since exp-20260503-016`
...
This run is a duplicate guardrail, not a new static replay. It found no new SEC source archive, no new source diagnostics file, and no new XBRL/companyfacts field coverage after `exp-20260503-016`.
...
| Earnings surprise history | 5198 rows (86.16%) | PIT if snapshot date <= trade date | Some extreme surprise values still need winsor/audit before scoring |
| SEC Atom filings | Existing `data/news_20260502.json` only | Forward archive only | No new archive since prior audit |
...
This run reused the existing `exp-20260503-016` shadow table:
...
- Current production/pilot universe overlap rows: 1 (`TSLA`, `10-K`, usable trade date `2026-05-01`)
- New rows since `exp-20260503-016`: 0

## Shadow Metrics

Forward returns are still not measurable for the current SEC-tagged rows. The only normalized SEC archive is dated `2026-05-02`, and no 5/10/20/60d outcome window has closed.

Candidate overlap and slot value:

- Existing shadow candidate count: 300
- New shadow candidate count since prior audit: 0
- Current production/pilot overlap: 1 row
...
The SEC/earnings filing-shock branch remains worth watching, but another same-archive replay would repeat `exp-20260503-016`. Do not promote to production, default-off replay, C-strategy grading, or slot tie-breaker until new forward SEC archive days or PIT XBRL/companyfacts rows exist.

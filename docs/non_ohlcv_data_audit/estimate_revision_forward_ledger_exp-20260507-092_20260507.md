# Estimate Revision Forward Ledger Harness (exp-20260507-092)
...
- `quant/earnings_snapshot.py` now persists `next_earnings_date`, which `get_earnings_data()` already produced.
...
```bash
python scripts/run_estimate_revision_forward_ledger.py --as-of 2026-05-06 --output-dir data/experiments/exp-20260507-092 --start 2026-05-04
```
...
- current and prior snapshot files were created no later than their respective `as_of_date`;

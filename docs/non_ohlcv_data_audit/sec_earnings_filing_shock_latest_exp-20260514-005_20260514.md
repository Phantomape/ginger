# SEC / earnings / filing shock latest data audit (exp-20260514-005)

- timestamp: `2026-05-14T01:27:44+00:00`
...
SEC filing shock, financial surprise, 8-K/10-Q/10-K event type, and post-earnings drift may improve C-strategy grading or A/B event confirmation, but only if the 2026-05-12 SEC refresh adds PIT directional fields that touch candidates.
...
- `exp-20260512-003`: 2026-05-10 refresh added timestamp/text coverage but no feature file or directional fields.
- `exp-20260513-006`: 2026-05-11 refresh had events/text/features and 1 pilot overlap, but zero same-accession rows, zero directional rows, and no B/C candidate-touch cohort.
...
|---|---:|---:|---:|---|
| SEC events 2026-05-12 | 60 | 60 | n/a | semantic fields absent from raw metadata |
| SEC text 2026-05-12 | 40 | n/a | n/a | unstructured text; no guidance/surprise adapter |
| SEC features 2026-05-12 | 40 | 40 | 0 | same-accession / EPS / revenue / guidance rows still absent |
| Earnings snapshot 2026-05-12 | 58 | replayable snapshot | n/a | no current EPS/revenue surprise vs PIT consensus |
...
- shadow_event_table: `data/non_ohlcv/sec_earnings_filing_shock_shadow_events_exp-20260514-005.json`
- rows: `60`
- PIT-safe rows: `60`
- feature-present rows: `40`
- same-accession rows: `0`
- directional rows: `0`
- tag_counts: `{"D_unclear_or_missing_data": 60}`

## Tagged candidate forward returns

Fresh 2026-05-12 rows are not mature for 5/10/20/60d candidate returns and are not linked to canonical-window candidates. Carried-forward historical candidate tags from `exp-20260511-001`:
...
- open-position event overlap tickers: `AMD, COHR, GOOG, NVDA, TRIP, UNH`
- carried-forward scarce-slot value: avg 20d delta `-9.4314`, win_rate `0.2222`, comparable_count `9`

## Decision

`data_gap`: the 2026-05-12 SEC refresh has timestamp/text/feature coverage, but it still does not produce a PIT-safe directional same-accession shock cohort that touches existing trend/breakout/C candidates. Do not enter a default-off C grading harness yet.

## Next minimal action

Repair same-accession Companyfacts joins for 2026-05-12 10-Q/10-K accessions or ingest PIT consensus/guidance fields, then rerun candidate-touch tagging before any default-off C-strategy grading harness.

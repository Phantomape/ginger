...
| `FINRA_IWM_CONFIRMED_PAPER` | Default-off paper only; live/default orders disabled | `exp-20260530-007` accepted the FINRA short-pressure + IWM-confirmed same-ticker cooldown candidate pool and `exp-20260530-010` promoted it into a shared default-off forward adapter; `exp-20260601-029` accepted cost-liquidity support (`dollar_volume >= $200m` and signal-day `(high-low)/close <= 0.10`, `1.05x` paper notional), adding `+0.0072` EV / `+$314.56` versus the accepted FINRA/IWM overlay with all three windows improving, `34` adjusted rows, and concentration passing | Forward rows are not mature yet; activation is blocked until closed paper replacement-value outcomes and a separate live adapter experiment pass | Small paper sleeve review only after the forward gate clears; do not retune FINRA score, IWM/SPY threshold, cooldown, top-N, hold-day, base notional, or cost-liquidity thresholds/scalar on the frozen sample | FINRA publication timing, stale short-interest data, hidden small-cap beta, execution-cost proxy decay, and ticker concentration |

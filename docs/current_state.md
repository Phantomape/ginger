...
| `FUNDAMENTAL_GROWTH_RS_PAPER` | Default-off paper only; live/default orders disabled | `exp-20260528-008` accepted the Companyfacts operating-profit quality + RS candidate-pool alpha with closed-ledger governor: EV improved in all three canonical windows by `+7.3309` aggregate and PnL by `+$111,268.93`, with 336 target paper trades and concentration passing; `exp-20260528-011` made the same rule production-visible as a shared default-off forward adapter | Forward rows are not mature yet; activation is blocked until 30+ closed paper trades, positive forward PnL, replacement-value/concentration checks, and a separate live adapter experiment pass | Small paper sleeve review only after the forward gate clears; do not retune Companyfacts growth/RS/operating-profit thresholds on the frozen sample | Companyfacts filed-date leakage, RS proxy decay, winner concentration, and core-overlap replacement value |

...
| Default-off external event overlay bundle | `event_sleeve_bundle.py`, `form4_event_sleeve.py`, `sec_negative_event_sleeve.py`, `sec_event_sleeve.py`, `report_generator.py` | default core backtests do not trade it; historical bundle replays must use shared source queues/sleeves before promotion | daily run emits aggregate default-off bundle attribution, normalized candidate schema, source-priority dedupe, forward gate, and kill-switch status only | observe-only until closed forward outcomes pass the shared gate and an explicit trade adapter is enabled |

...
| Default-off broad-market leadership paper sleeve | `broad_market_paper_sleeve.py`, `report_generator.py` | default core backtests do not trade it; historical broad-market replays must use the shared `price_floor_40` feature/filter helper, candidate exclusion semantics, 20-trading-day paper hold, and concentration guard before promotion | daily run emits the default-off `BROAD_MARKET_LEADERSHIP_PAPER` candidate queue, pending/open/closed paper ledger state, source coverage metadata, forward paper gate, and report block only when a candidate universe feed is present | observe-only until closed forward replacement-value outcomes pass a separate gate and an explicit trade adapter is enabled |

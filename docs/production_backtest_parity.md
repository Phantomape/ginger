...
| Default-off macro relief day top-2 leadership paper sleeve | `macro_relief_leadership_paper_sleeve.py`, `run.py`, `default_off_alpha_attribution.py`, `report_generator.py` | default core backtests do not trade it; historical evidence comes from accepted shared adapter `exp-20260606-020`; any replay must use the hardcoded `MACRO_EVENTS_BY_DATE` (CPI/FOMC/NFP), SPY return `>= 0.4%`, QQQ return `>= 0.6%`, both SPY and QQQ close location `>= 0.65` as the broad-market relief pass, candidate gates (price `>= $10`, close location `>= 0.70`, signal return `>= 1.0%`, relative-vs-SPY `>= 0.8%`, relative-vs-QQQ `>= 0.4%`, avg dollar volume `>= $50M`, realized vol `<= 8%`), composite score (2.25×relative-vs-SPY + 1.25×relative-vs-QQQ + 0.80×ret20-excess-SPY + 0.35×ret60-excess-SPY + 0.30×close-location + 0.10×min(volume-ratio,3) + 0.04×liquidity-score − 0.55×realized-vol20 − 0.20×max(ret5,0)), top-2 per day, `$4,000` paper notional, same-ticker 10-day cooldown, next-open paper entry, 10-calendar-day close exit, and concentration guard before promotion | daily run derives candidates from the already-loaded daily OHLCV universe plus `SPY` and `QQQ`, evaluates each `MACRO_EVENTS_BY_DATE` key against the broad-market relief gates, emits candidate score/relative-strength/volume/relief-context metadata, pending/open/closed paper ledger state, is_macro_relief_day flag, macro event context, replacement-value report, forward paper gate, default-off alpha-attribution surface, and human-report block | observe-only; no core universe expansion, no live orders, no core ranking/sizing/exit changes; activation requires ≥ 30 closed forward 10-day paper trades, positive forward paper PnL, replacement value vs. core or cash, kill-switch design, and a separate Gate 1-4 trade adapter (Gate 5) |

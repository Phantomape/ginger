...
- [x] 建立 claim-support envelope reconstruction validator/consumer boundary：要求 separately frozen envelope hash、显式 decision 时钟与 hypothesis-referenced exact claim-record snapshot，按全部依赖重建 feature/rank/no-entry signal/context/DecisionRecord，严格校验字段/自哈希并返回 detached plain payload；不新增 runtime stage、selection、OrderIntent 或权限（2026-09-01）

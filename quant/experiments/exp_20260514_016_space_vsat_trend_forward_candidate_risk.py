...
default-off Space stack: whether VSAT deserves a bounded risk budget only when
the base strategy is trend_long after the closed Golden Dome/defense-budget
forward ledger shows positive 10d cash, same-theme, UFO, and SPY-relative
replacement value.

This deliberately avoids broad satcom breadth, GSAT, LLM soft-ranking, entries,
exits, targets, ranking, or live Space slots. It is a replay-only alpha search,
with non-trend VSAT signals forced to zero risk inside the same candidate gate.
"""
...
EXPERIMENT_ID = "exp-20260514-016"
STEM = "space_vsat_trend_forward_candidate_risk"
...
VSAT_FORWARD_CANDIDATE_TICKERS = ("VSAT",)
VSAT_FORWARD_CANDIDATE_RISK_SCALARS = (0.25, 0.50, 0.75, 1.00)
MIN_SAME_THEME_REPLACEMENT_VALUE = 500.0
...
def _latest_vsat_forward_rows() -> dict[tuple[str, str], dict[str, Any]]:
    ...
    target_tickers = set(VSAT_FORWARD_CANDIDATE_TICKERS)
    if not path.exists():
...
def _vsat_forward_candidate_profile_gate() -> dict[str, Any]:
    rows_by_ticker: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in _latest_vsat_forward_rows().values():
        ticker = str(row.get("ticker") or "").upper()
        horizon = (row.get("horizons") or {}).get(FORWARD_REPLACEMENT_HORIZON) or {}
    ...
    for ticker, rows in sorted(rows_by_ticker.items()):
        ...
        passes = (
            avg_cash > 0.0
            and avg_same >= MIN_SAME_THEME_REPLACEMENT_VALUE
            and avg_spy > 0.0
            and avg_ufo > 0.0
        )
        profiles[ticker] = {
            ...
            "min_same_theme_replacement_value": MIN_SAME_THEME_REPLACEMENT_VALUE,
            "passes_vsat_forward_candidate_gate": passes,
            "event_ids": sorted(
    ...
    return {
        ...
        "profiles": profiles,
        "gate_definition": {
            "ticker_scope": list(VSAT_FORWARD_CANDIDATE_TICKERS),
            "semantic_bucket": "defense_budget_theme",
            "source_type": "official_government_release",
            "cash_relative_pnl": "> 0",
            "same_theme_replacement_value": f">= {MIN_SAME_THEME_REPLACEMENT_VALUE}",
...
def _snapshot_vsat_gate() -> dict[str, Any]:
    ...
    for label, window in source_diversity_exp.WINDOWS.items():
        ...
        if path.exists():
            ...
            present = "VSAT" in augmented or '"VSAT"' in path.read_text(
                encoding="utf-8-sig", errors="replace"
            )
        by_window[label] = {
            "path": window["space_snapshot"],
            "exists": path.exists(),
            "vsat_present": present,
        }
    return {
        "passed": all(row["exists"] and row["vsat_present"] for row in by_window.values()),
        "by_window": by_window,
    }
...
def _run_variant(
    label: str,
    *,
    vsat_candidate_scalar: float,
    include_vsat: bool,
    ten_day_forward_gate: dict[str, Any],
    vsat_gate: dict[str, Any],
    ...
    if include_vsat:
        universe_seed.update(VSAT_FORWARD_CANDIDATE_TICKERS)
    ...
    vsat_tickers = set(vsat_gate["target_tickers"]) if include_vsat else set()
    vsat_profiles = vsat_gate["profiles"]

    adjustments: list[dict[str, Any]] = []
    counts: Counter[str] = Counter()

    def size_with_vsat_candidate_scalar(
        ...
        for signal in sized:
            ...
            if ticker in vsat_tickers and sizing:
                is_trend_candidate = strategy == "trend_long"
                applied_scalar = vsat_candidate_scalar if is_trend_candidate else 0.0
                marker = (
                    "space_vsat_trend_forward_candidate_risk"
                    if is_trend_candidate
                    else "space_vsat_nontrend_forward_candidate_zero_risk"
                )
                ...
                profile = vsat_profiles.get(ticker)
                adjustments.append(
                    {
                        "ticker": ticker,
                        "strategy": strategy,
                        "candidate_scalar": vsat_candidate_scalar,
                        "scalar": applied_scalar,
                        ...
                        "vsat_forward_candidate_profile": profile,
                ...
                signal = {
                    **signal,
                    "space_vsat_trend_forward_candidate_bucket": is_trend_candidate,
                    "space_vsat_nontrend_forward_candidate_zero_risk_bucket": (
                        not is_trend_candidate
                    ),
                    "space_vsat_trend_forward_candidate_scalar": (
                        vsat_candidate_scalar if is_trend_candidate else None
                    ),
                    "space_vsat_nontrend_forward_candidate_scalar": (
                        0.0 if not is_trend_candidate else None
                    ),
                    "space_vsat_forward_candidate_profile": profile,
                }
    ...
    portfolio_engine.size_signals = size_with_vsat_candidate_scalar
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "vsat_trade_attribution": _trade_attribution_for(result, {"VSAT"}),
                "vsat_trend_trade_attribution": _trade_attribution_for(
                    result,
                    {"VSAT"},
                    {"trend_long"},
                ),
                "vsat_nontrend_trade_attribution": _trade_attribution_for(
                    result,
                    {"VSAT"},
                    {"breakout_long"},
                ),
                "space_with_vsat_trade_attribution": _trade_attribution_for(
                    result,
                    set(source_diversity_exp.OFFICIAL_SPACE_TICKERS) | {"VSAT"},
                ),
                "vsat_candidate_adjustment": source_diversity_exp._adjustment_summary(
                    adjustments[before_adjustments:]
                ),
                "vsat_candidate_counts": dict(sorted((counts - before_counts).items())),
            }
        ...
        return {
            ...
            "parameters": {
                ...
                "space_vsat_trend_forward_candidate_scalar": vsat_candidate_scalar,
                "space_vsat_nontrend_forward_candidate_scalar": 0.0,
                "target_vsat_strategy": "trend_long",
                "include_vsat": include_vsat,
                "target_tickers": sorted(vsat_tickers),
            ...
            "vsat_candidate_adjustment_summary": (
                source_diversity_exp._adjustment_summary(adjustments)
            ),
            "vsat_candidate_counts": dict(sorted(counts.items())),
            "vsat_candidate_adjustment_sample": adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_trend_count = int(
        variant["vsat_candidate_counts"].get(
            "space_vsat_trend_forward_candidate_risk_changed_signal",
            0,
        )
    )
    changed_nontrend_zero_count = int(
        variant["vsat_candidate_counts"].get(
            "space_vsat_nontrend_forward_candidate_zero_risk_changed_signal",
            0,
        )
    )
    eligible_count = int(variant["vsat_candidate_counts"].get("eligible_signal", 0))
    eligible_trend_count = int(
        variant["vsat_candidate_counts"].get("eligible_trend_long", 0)
    )
    eligible_nontrend_count = eligible_count - eligible_trend_count
    scalar = float(variant["parameters"]["space_vsat_trend_forward_candidate_scalar"])
    vsat_trade_attr = _aggregate_trade_attr(variant, "vsat_trade_attribution")
    trend_trade_attr = _aggregate_trade_attr(variant, "vsat_trend_trade_attribution")
    nontrend_trade_attr = _aggregate_trade_attr(
        variant,
        "vsat_nontrend_trade_attribution",
    )
    return {
        ...
        "eligible_vsat_candidate_signal_count": eligible_count,
        "eligible_vsat_trend_signal_count": eligible_trend_count,
        "eligible_vsat_nontrend_signal_count": eligible_nontrend_count,
        "changed_vsat_trend_candidate_signal_count": changed_trend_count,
        "zeroed_vsat_nontrend_candidate_signal_count": changed_nontrend_zero_count,
        "vsat_trade_attribution": vsat_trade_attr,
        "vsat_trend_trade_attribution": trend_trade_attr,
        "vsat_nontrend_trade_attribution": nontrend_trade_attr,
        "accepted": bool(
            scalar > 0.0
...
def _gate2_runtime_state(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for variant in variants:
        for window_name, payload in variant.get("by_window", {}).items():
            counts = payload.get("vsat_candidate_counts") or {}
            ...
            changed_trend += int(
                counts.get("space_vsat_trend_forward_candidate_risk_changed_signal", 0)
                or 0
            )
            zeroed_nontrend += int(
                counts.get(
            ...
            summary = payload.get("vsat_candidate_adjustment") or {}
                ...
                samples.append(
                    {
                        ...
                        "profile": row.get("vsat_forward_candidate_profile"),
                    }
                )
    return {
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space VSAT trend-only forward-candidate risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_vsat_trend_forward_candidate_scalar`, where the accepted "
            "exp-009 default-off Space stack is unchanged and only VSAT "
            "`trend_long` signals receive bounded risk after passing the closed "
            "official Golden Dome forward replacement profile. Non-trend VSAT "
            "signals are forced to zero risk; broad satcom, GSAT, LLM/news, "
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_vsat_trend_forward_candidate_scalar']}`"
        ),
        (
        ...
        (
            "- VSAT trend signals changed / non-trend zeroed: "
            f"`{gate['changed_vsat_trend_candidate_signal_count']}` / "
            f"`{gate['zeroed_vsat_nontrend_candidate_signal_count']}` from "
            f"`{gate['eligible_vsat_candidate_signal_count']}` eligible"
        ),
        (
            "- VSAT trend trades / non-trend trades: "
            f"`{gate['vsat_trend_trade_attribution']['trade_count']}` / "
            f"`{gate['vsat_nontrend_trade_attribution']['trade_count']}`, "
            f"PnL `{gate['vsat_trade_attribution']['total_pnl']}`"
        ),
        "",
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["by_window"][name]["vsat_candidate_adjustment"][
            "adjusted_signal_count"
        ]
        vsat_trend_trades = best["by_window"][name]["vsat_trend_trade_attribution"][
            "trade_count"
        ]
        vsat_nontrend_trades = best["by_window"][name][
            "vsat_nontrend_trade_attribution"
        ]["trade_count"]
        lines.append(
            "| {name} | {ev:.6f} | {pnl:.2f} | {dd:.6f} | {trades} | {survival:.6f} | {adjusted} | {vsat_trend_trades} | {vsat_nontrend_trades} |".format(
                name=name,
                ...
                adjusted=adjusted,
                vsat_trend_trades=vsat_trend_trades,
                vsat_nontrend_trades=vsat_nontrend_trades,
            )
        )
    lines.extend(
        [
...
def run() -> dict[str, Any]:
    ...
    vsat_gate = _vsat_forward_candidate_profile_gate()
    snapshot_gate = _snapshot_vsat_gate()

    before = _run_variant(
        "accepted_exp009_no_vsat",
        vsat_candidate_scalar=0.0,
        include_vsat=False,
    ...
    variants = [
        ...
        for scalar in VSAT_FORWARD_CANDIDATE_RISK_SCALARS
    ]
    for variant in variants:
    ...
    gate2 = {
        ...
        "vsat_forward_candidate_profile": vsat_gate,
        "vsat_snapshot_presence": snapshot_gate,
        "vsat_candidate_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested VSAT trend-only forward-candidate scalar improved "
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "VSAT, unlike broad mature satcom or GSAT, now has a closed official "
            "Golden Dome/defense-budget forward profile with positive 10d cash, "
            "same-theme, SPY-relative, and UFO-relative replacement value. "
            "Exp-20260514-015 showed broad VSAT admission was damaged by "
            "breakout losses, so trend-only VSAT risk may keep the replacement "
            "alpha while removing the non-trend tail."
        ),
        "change_type": "candidate_pool_risk_allocation_shadow_sweep",
        "changed_variable": "space_vsat_trend_forward_candidate_risk_scalar",
        "single_causal_variable": (
            "admit only VSAT trend_long signals with a bounded risk scalar after "
            "the closed official defense-budget forward profile passes cash, "
            "same-theme, SPY, and UFO-relative gates; force non-trend VSAT "
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "candidate pool / risk allocation: add only VSAT trend_long risk "
                "after the official Golden Dome defense-budget forward ledger "
                "shows positive 10d cash, same-theme, SPY, and UFO-relative "
                "evidence."
            ),
            ...
            "q3_single_causal_variable": (
                "Only the VSAT trend_long forward-candidate risk scalar changes "
                "and VSAT non-trend sizing is forced to zero; accepted exp-009 "
            ...
            "q4_acceptance_standard": (
                ...
                "trades, actual VSAT trend trades, and zero VSAT non-trend "
                "trades."
            ),
            "q5_reproducibility": (
        ...
        "gate1": {
            ...
            "known_bias": (
                ...
                "The VSAT admission evidence comes from closed 2026 event-state "
        ...
        "parameters": {
            "tested_vsat_trend_forward_candidate_scalars": list(
                VSAT_FORWARD_CANDIDATE_RISK_SCALARS
            ),
            "vsat_nontrend_forward_candidate_scalar": 0.0,
            "target_vsat_strategy": "trend_long",
            "vsat_forward_candidate_tickers": vsat_gate["target_tickers"],
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_vsat_trend_forward_candidate_risk"
            if decision == "accepted"
            else "rejected_space_vsat_trend_forward_candidate_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not promote VSAT trend-only candidate admission from this "
        ...
        "known_risks": [
            ...
            "VSAT evidence is a single closed official Golden Dome event row and can be event-specific.",
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_vsat_trend_candidate_signal_count": result["best_variant_gate"][
            "changed_vsat_trend_candidate_signal_count"
        ],
        "zeroed_vsat_nontrend_candidate_signal_count": result["best_variant_gate"][
            "zeroed_vsat_nontrend_candidate_signal_count"
        ],
        "vsat_trade_attribution": result["best_variant_gate"][
            "vsat_trade_attribution"
        ],
        "vsat_trend_trade_attribution": result["best_variant_gate"][
            "vsat_trend_trade_attribution"
        ],
        "vsat_nontrend_trade_attribution": result["best_variant_gate"][
            "vsat_nontrend_trade_attribution"
        ],

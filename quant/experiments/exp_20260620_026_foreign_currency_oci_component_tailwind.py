"""exp-20260620-026: raw SEC FX-translation OCI component tailwind scout.
...
annual foreign-currency translation OCI component turns positive versus the
prior comparable annual period, while revenue is not contracting and price
already shows liquid SPY-relative leadership, may reflect a recognized global
translation tailwind before a 10-trading-day continuation leg.
...
STEM = "foreign_currency_oci_component_tailwind"
TRIAL_FAMILY = "raw_sec_companyfacts_foreign_currency_oci_component_tailwind"
TRIAL_VARIANT_ID = "fx_translation_oci_tailwind_top1_next_open_10d_v1"
CHANGED_VARIABLE = "raw_sec_companyfacts_foreign_currency_oci_component_tailwind_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260620_026_{STEM}.json"
...
MIN_CURRENT_FX_OCI_USD = 1_000_000.0
MIN_CURRENT_FX_OCI_TO_REVENUE = 0.0005
MIN_FX_OCI_IMPROVEMENT_TO_REVENUE = 0.0010
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "aoci_overhang_family_near_neighbor",
        "field_sign_noise",
    ...
    "confidence_reason": (
        "This uses period OCI foreign-currency translation component flows, not "
        "AOCI balance overhang, cash conversion, capex coverage, accruals, "
        "tax/debt relief, or supplier financing. Coverage scan found hundreds "
        "of CIKs, but raw Companyfacts fields have recently failed many "
        "window/comparator gates."
    ),
    "recorded_at": "2026-06-20T20:11:20+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC annual FX-translation OCI component fact, missing "
            "prior comparable annual OCI fact, missing matched annual revenue, "
    ...
    "parity_note": (
        ...
        "SEC tag mapping, filed-date PIT annual FX-translation OCI component "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts annual foreign-currency "
        "translation OCI component turning positive versus the prior annual "
        "filing, scaled by revenue, with non-contracting revenue and liquid "
        "SPY-relative leadership, may identify global companies where a "
        "reported translation tailwind is being recognized before short-horizon "
        "continuation."
    ),
    "2_history_check": {
        "novelty_gate": (
            "Initial tax-overhang and research-tax-credit candidates were "
            "blocked as near-neighbors of frozen Companyfacts relief/R&D/tax "
            "families. This run used novelty override because the new evidence "
            "axis is a raw SEC period OCI foreign-currency translation component "
            "flow, not AOCI balance overhang, cash conversion, capex coverage, "
            "accruals, tax burden, debt, or supplier-financing relief."
        ),
        "exp-20260616-005": (
            "Rejected tax burden relief. This run does not use tax expense, "
            "pretax income, or effective-tax-rate relief."
        ),
        "exp-20260617-018": (
            "Rejected AOCI overhang relief. This run uses period foreign-currency "
            "OCI component flow and a turn-positive condition, not the aggregate "
            "AOCI balance."
        ),
        "exp-20260619-012": (
            "Rejected reportable-segment simplification. This run is an OCI "
            "component attribution field, not business segment count."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260620_026_foreign_currency_oci_component_tailwind.py"
    ),
}
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        stats["tickers_with_raw_annual_fx_oci_and_revenue"] += 1
...
def _fx_oci_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    del ticker
    current_fx = _latest_period_fact(facts["fx_oci"], asof=asof)
    if current_fx is None:
        return None
    if base._days_between(asof, current_fx["filed"]) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None

    prior_fx = _latest_period_fact(facts["fx_oci"], asof=asof, before_end=current_fx["end"])
    if prior_fx is None:
        return None

    current_revenue = _latest_period_fact(facts["revenue"], asof=asof, end=current_fx["end"])
    prior_revenue = _latest_period_fact(facts["revenue"], asof=asof, end=prior_fx["end"])
    if current_revenue is None or prior_revenue is None:
        return None

    current_fx_value = float(current_fx["value"])
    prior_fx_value = float(prior_fx["value"])
    ...
    current_ratio = current_fx_value / current_revenue_value
    improvement_to_revenue = (current_fx_value - prior_fx_value) / current_revenue_value
    if current_fx_value < MIN_CURRENT_FX_OCI_USD:
        return None
    if current_ratio < MIN_CURRENT_FX_OCI_TO_REVENUE:
        return None
    if improvement_to_revenue < MIN_FX_OCI_IMPROVEMENT_TO_REVENUE:
        return None
    if prior_fx_value > 0.0:
        return None

    return {
        "current_fiscal_year_end": current_fx["end"],
        "prior_fiscal_year_end": prior_fx["end"],
        "current_fx_oci_filed": current_fx["filed"],
        "prior_fx_oci_filed": prior_fx["filed"],
        "current_revenue_filed": current_revenue["filed"],
        "prior_revenue_filed": prior_revenue["filed"],
        "current_fx_oci_tag": current_fx["tag"],
        "current_revenue_tag": current_revenue["tag"],
        "current_fx_oci": _round(current_fx_value, 2),
        "prior_fx_oci": _round(prior_fx_value, 2),
        "fx_oci_improvement": _round(current_fx_value - prior_fx_value, 2),
        "current_fx_oci_to_revenue": _round(current_ratio, 6),
        "fx_oci_improvement_to_revenue": _round(improvement_to_revenue, 6),
        ...
        "fact_age_days": base._days_between(asof, current_fx["filed"]),
    }
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _fx_oci_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_fx_oci_gate"] += 1
                continue
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_FX_TRANSLATION_OCI_TAILWIND_PAPER",
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_foreign_currency_oci_component_tailwind"
        if gate["passed"]
        else "rejected_foreign_currency_oci_component_tailwind_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The raw SEC foreign-currency translation OCI component source "
        ...
        interpretation = (
            "The raw SEC foreign-currency translation OCI component source had "
            ...
            "selects a narrow set of global/AI leadership names and adds too "
            "much drawdown for a fixed top-1/day 10-day sleeve. It is not "
            "retained or promoted."
        )
    else:
        interpretation = (
            "The raw SEC foreign-currency translation OCI component source did "
    ...
    payload.update(
        {
            ...
            "new_evidence_type": (
                "raw_sec_companyfacts_period_foreign_currency_translation_oci_component_flow"
            ),
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual foreign-currency translation OCI component and annual revenue "
        "are read from raw SEC Companyfacts tags and are known only by their "
        "filed date (<= signal date). The current annual FX-translation OCI "
    ...
    payload["next_evidence_needed"] = (
        "A retry needs materially richer PIT global-exposure evidence, such as "
        "segment/currency revenue mix, cash-flow hedge OCI reclassification "
        "detail, FX sensitivity disclosure, or closed forward replacement-value "
        "rows. Do not sweep FX OCI ratio, improvement, freshness, revenue, "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping FX OCI ratio, FX OCI improvement, current "
        ...
        "new_evidence_required": (
            ...
            "global-exposure or risk-control evidence axis, not a parameter "
            "sweep: segment/currency revenue mix, cash-flow hedge OCI "
            "reclassification detail, FX sensitivity disclosure, closed forward "
            "replacement-value rows, or a separately predeclared drawdown-aware "
            "policy bundle."
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Foreign-Currency OCI Component Tailwind",
            "",

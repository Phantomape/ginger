"""exp-20260604-012: lagged consensus prior Companyfacts support scout.
...
variable: selected paper trades whose prior independent confirmation includes
the Companyfacts growth-quality source family receive a 1.05x paper PnL/notional
support scalar.
...
EXPERIMENT_ID = "exp-20260604-012"
STEM = "lagged_consensus_prior_companyfacts_support"
TRIAL_FAMILY = "accepted_free_data_cross_source_consensus_source_timing_quality_support"
TRIAL_VARIANT_ID = "prior_companyfacts_confirmation_support_1p05_v1"
CHANGED_VARIABLE = "prior_companyfacts_growth_quality_confirmation_support_scalar_v1"
RULE_VERSION = "accepted_lagged_consensus_prior_companyfacts_support_v1"

SUPPORT_SOURCE_FAMILY = "companyfacts_growth_quality"
SUPPORT_SCALAR = 1.05
MAX_SUPPORTED_SOURCE_SHARE = 0.80
MIN_SUPPORTED_SOURCE_SHARE = 0.20

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260604_012_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "nearby_support_scalar_overfit",
    ],
    "confidence_reason": (
        "Pre-read of exp008 selected trades shows prior Companyfacts "
        "confirmation has positive PnL in all three windows and lower rough "
        "concentration than the rejected cost/liquidity support, but this is "
        "still a nearby support layer on a strong adapter."
    ),
    "recorded_at": "2026-06-04T12:09:15+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        "This experiment changes no production code. A retained support result "
        "would need the shared free-data consensus adapter to compute the same "
        "prior Companyfacts confirmation support in daily production and "
        "historical replay, with parity tests, before any report queue, paper "
        "ledger, or order surface could change."
    ),
}
...
def _support_context(trade: dict[str, Any]) -> dict[str, Any]:
    ...
    passed = SUPPORT_SOURCE_FAMILY in prior_families
    if passed:
        status = "supported_prior_companyfacts_confirmation"
    elif prior_families:
        status = "prior_confirmation_not_companyfacts"
    else:
        status = "no_prior_confirmation"
    return {
        "prior_companyfacts_support_rule_version": RULE_VERSION,
        "prior_companyfacts_known_at": "current signal date after close, from bounded prior source snapshot history",
        "prior_companyfacts_trade_enabled": False,
        "prior_companyfacts_alters_orders": False,
        "prior_companyfacts_status": status,
        "prior_companyfacts_support_pass_v1": passed,
        "prior_companyfacts_support_scalar": SUPPORT_SCALAR if passed else 1.0,
        "prior_companyfacts_required_source_family": SUPPORT_SOURCE_FAMILY,
        "prior_confirmation_source_families": prior_families,
        "current_source_families": current_families,
        "current_source_family_count": int(trade.get("current_source_family_count") or 0),
        "prior_confirmation_family_count": int(trade.get("prior_confirmation_family_count") or 0),
    }
...
def _support_trades_by_window(
    ...
    diagnostics: dict[str, Any] = {
        ...
        "support_parameters": {
            "required_prior_source_family": SUPPORT_SOURCE_FAMILY,
    ...
    for label, cfg in lagged.same_day.prior.base.WINDOWS.items():
        ...
        for trade in comparator_trades[label]:
            ...
            context = _support_context(trade)
            status_counts[str(context["prior_companyfacts_status"])] += 1
            before_trade = {
                **trade,
                **context,
                "rule_version": RULE_VERSION,
                "strategy": "lagged_free_data_consensus_prior_companyfacts_support",
                "pnl": round(base_pnl, 2),
                "paper_pnl": round(base_pnl, 2),
                "pnl_without_prior_companyfacts_support": round(base_pnl, 2),
                "trade_enabled": False,
                "alters_orders": False,
            }
            scalar = SUPPORT_SCALAR if context["prior_companyfacts_support_pass_v1"] else 1.0
            after_pnl = base_pnl * scalar
            after_trade = {
                **before_trade,
                "pnl": round(after_pnl, 2),
                "paper_pnl": round(after_pnl, 2),
                "paper_pnl_source": "pnl_with_lagged_consensus_prior_companyfacts_support",
            }
            before_rows.append(before_trade)
            after_rows.append(after_trade)
            if context["prior_companyfacts_support_pass_v1"]:
                ...
                incremental_trade = {
                    **after_trade,
                    ...
                    "paper_pnl_source": "lagged_consensus_prior_companyfacts_incremental_support",
                }
        ...
        results.append(
            {
                ...
                "prior_companyfacts_status_counts": dict(sorted(status_counts.items())),
            }
        )
    ...
    diagnostics["supported_source_share"] = round(supported_count / source_trade_count, 6) if source_trade_count else None
...
def _gate4(
    ...
    gates = {
        **base_gate["gates"],
        ...
        "prior_companyfacts_supported_trade_count_positive": int(support_diagnostics["supported_trade_count"]) > 0,
        "prior_companyfacts_selectivity_guard_passed": selectivity_passed,
    }
    passed = bool(base_gate["passed"] and comparator_passed and selectivity_passed)
    if passed:
        decision = "positive_replay_lead_requires_shared_prior_companyfacts_adapter"
        rationale = (
            "Prior Companyfacts confirmation support improved the current accepted "
            "lagged consensus adapter across all three windows. It is not promoted "
            "because shared live/backtest adapter parity is required first."
        )
    elif not comparator_passed:
        decision = "rejected_prior_companyfacts_support_did_not_beat_accepted_adapter"
        rationale = (
        ...
        decision = "rejected_prior_companyfacts_support_failed_selectivity_guard"
        rationale = "The support rule was not selective enough, or too sparse, to qualify as an independent alpha variable."
        ...
        decision = "rejected_prior_companyfacts_support_gate4_failed"
...
def _preflight_payload() -> dict[str, Any]:
    return {
        "alpha_hypothesis": (
            "Accepted lagged free-data consensus candidates with prior Companyfacts "
            "growth-quality confirmation may have stronger source-timing quality "
        ...
        "playbook_alignment": (
            ...
            "This tests a source-timing quality field rather than adding a new "
            "source family, changing the lagged window, changing source count, "
            "or expanding live orders."
        ),
        ...
        "prior_difference": (
            "exp-20260604-009 promoted lagged source timing; exp-20260604-011 "
            "tested signal-day cost/liquidity support and failed concentration. "
            "This run leaves admission and ranking fixed and tests only whether "
            "the prior-confirmation family being Companyfacts carries incremental "
            "support value."
        ),
        ...
        "reproducibility": (
            ...
            "quant\\experiments\\exp_20260604_012_lagged_consensus_prior_companyfacts_support.py"
        ),
    }
...
def _experiment_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "new_evidence_type": "production_visible_source_timing_quality_field_on_accepted_lagged_consensus",
        ...
        "calibration": {
            ...
            "ev_prediction_error": round(comparison["expected_value_score_delta"] - PREDICTION["expected_ev_delta"], 6),
            "expected_pnl_delta": PREDICTION["expected_pnl_delta"],
            "actual_pnl_delta": comparison["strategy_total_pnl_delta"],
            "pnl_prediction_error": round(comparison["strategy_total_pnl_delta"] - PREDICTION["expected_pnl_delta"], 2),
...
def _write_card(payload: dict[str, Any]) -> None:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Lagged Consensus Prior Companyfacts Support",
        "",
        "## Decision",
        "",
...
def main() -> None:
    ...
    payload = {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: accepted lagged consensus candidates with "
                "prior Companyfacts growth-quality confirmation may deserve "
                "modest paper notional support."
            ),
            "2_history_check": {
                ...
                "exp-20260604-011": "Rejected cost/liquidity support due incremental concentration.",
            },
        ...
        "rule": {
            ...
            "required_prior_source_family": SUPPORT_SOURCE_FAMILY,
            "support_scalar": SUPPORT_SCALAR,
            "known_at": "current signal date after close from bounded prior source snapshot history",
            "unchanged": {

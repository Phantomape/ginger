"""exp-20260616-021: receivables DSO/collection improvement scout.
...
annual days-sales-outstanding (DSO) is improving year over year because
receivables are being collected faster versus annual revenue, while revenue is
not contracting. This tests sales-quality / customer-cash-collection evidence
rather than another inventory or SBC refinement.

Why this is a materially new free-data edge: receivables are balance-sheet
INSTANT facts (no FY duration) while revenue is a filed annual flow. This
runner matches receivables to the current, prior, and pre-prior fiscal-year
ends and computes current/prior DSO from average receivables divided by annual
revenue. It is distinct from exp-20260528-019 working-capital support because
that was a support scalar inside the accepted FGRS sleeve with missing selected
coverage; this is a standalone top-1 candidate source using raw Companyfacts
coverage and a fixed OHLCV leadership envelope.
...
private scout because annual revenue + three receivables-point coverage is not
yet proven sufficient across the three canonical windows.
...
EXPERIMENT_ID = "exp-20260616-021"
STEM = "receivables_dso_collection_improvement"
TRIAL_FAMILY = "receivables_dso_collection_improvement_candidate_pool"
TRIAL_VARIANT_ID = "receivables_dso_collection_improvement_top1_next_open_10d_v1"
CHANGED_VARIABLE = "receivables_dso_collection_improvement_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260616_021_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "drawdown_drift",
        "receivables_field_relabels_working_capital_noise",
    ],
    "confidence_reason": (
        "This uses a materially different free SEC Companyfacts balance-sheet "
        "field from the rejected inventory quality scouts and accepted SBC "
        "burden source: faster customer cash collection should indicate cleaner "
        "sales and lower revenue-quality risk. Prior working-capital support "
        "saw missing selected-row coverage, so this is a private data-shape "
        "scout with high failure risk; success requires all canonical windows "
        "and accepted candidate-pool comparators, not just aggregate PnL."
    ),
    "recorded_at": "2026-06-16T18:04:04+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC annual receivables instant fact, missing matching "
            "FY revenue, missing pre-prior receivables point, stale facts, "
    ...
    "parity_note": (
        ...
        "SEC receivables instant mapping, filed-date PIT annual revenue facts, "
        "DSO/collection improvement gate, liquid SPY-relative confirmation, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts annual DSO falling year over year "
        "(average AccountsReceivable / Revenue improving = faster cash "
        "collection and cleaner sales quality), with non-declining revenue and "
        "liquid SPY-relative leadership, may identify demand-confirmed candidates "
    ...
    "2_history_check": {
        "exp-20260528-019": (
            ...
            "standalone candidate-pool source testing receivables DSO/collection "
            "trajectory, not a support scalar or inventory/revenue stability."
        ),
        "exp-20260613-031": (
            "Rejected operating-efficiency (asset turnover = revenue/assets). "
            "This run tests customer-cash collection specifically, not total "
            "asset turnover."
        ),
        "exp-20260615-002": (
            "Rejected low asset-growth quality. This run tests receivables "
            "collection speed rather than broad balance-sheet growth."
        ),
        "exp-20260616-018": (
            ...
            "This run exits the inventory family and tests receivables collection "
            "quality instead."
        ),
        "exp-20260615-016": (
            "Accepted SBC burden improvement is a dilution-quality source. This "
            "run tests a separate sales-quality field and does not change SBC "
            "thresholds, buyback adjustment, allocator rank, or notional."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260616_021_receivables_dso_collection_improvement.py"
    ),
}
...
def _raw_instant_facts(usgaap: dict[str, Any], tags: tuple[str, ...]) -> list[dict[str, Any]]:
    ...
    Receivables are reported as a point-in-time USD value keyed by an `end`
    date with no `start` (or start == end). The FY-duration reader used for
    income-statement flows would drop these facts.
    """
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        stats["tickers_with_raw_receivables_and_revenue"] += 1
...
def _receivables_observation(
    ...
    if current_receivables is None:
        return None
    if base._days_between(asof, current_receivables["filed"]) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None
    ...
    if prior_receivables is None:
        return None
    pre_prior_revenue = rd._latest_period_fact(
        facts["revenue"], asof=asof, before_end=prior_revenue["end"]
    )
    if pre_prior_revenue is None:
        return None
    pre_prior_receivables = rd._latest_period_fact(
        facts["receivables"], asof=asof, end=pre_prior_revenue["end"]
    )
    if pre_prior_receivables is None:
        return None

    current_receivables_value = abs(float(current_receivables["value"]))
    prior_receivables_value = abs(float(prior_receivables["value"]))
    pre_prior_receivables_value = abs(float(pre_prior_receivables["value"]))
    current_revenue_value = float(current_revenue["value"])
    prior_revenue_value = float(prior_revenue["value"])
    if (
        current_receivables_value < MIN_CURRENT_RECEIVABLES
        or current_revenue_value < MIN_CURRENT_REVENUE
        or prior_revenue_value <= 0.0
        or prior_receivables_value <= 0.0
        or pre_prior_receivables_value <= 0.0
    ):
        return None

    current_avg_receivables = (current_receivables_value + prior_receivables_value) / 2.0
    prior_avg_receivables = (prior_receivables_value + pre_prior_receivables_value) / 2.0
    if current_avg_receivables <= 0.0 or prior_avg_receivables <= 0.0:
        return None

    current_dso = 365.0 * current_avg_receivables / current_revenue_value
    prior_dso = 365.0 * prior_avg_receivables / prior_revenue_value
    dso_improvement_days = prior_dso - current_dso
    current_turnover = current_revenue_value / current_avg_receivables
    prior_turnover = prior_revenue_value / prior_avg_receivables
    ...
    if current_dso > MAX_CURRENT_DSO_DAYS:
        return None
    if dso_improvement_days < MIN_DSO_IMPROVEMENT_DAYS:
        return None
    ...
    return {
        "ticker": ticker,
        "current_period_end": current_receivables["end"],
        ...
        "pre_prior_receivables_value": _round(pre_prior_receivables_value, 2),
        "current_revenue_value": _round(current_revenue_value, 2),
        "prior_revenue_value": _round(prior_revenue_value, 2),
        "prior_period_end": prior_receivables["end"],
        "pre_prior_period_end": pre_prior_receivables["end"],
        "current_avg_receivables": _round(current_avg_receivables, 2),
        "prior_avg_receivables": _round(prior_avg_receivables, 2),
        "current_dso_days": _round(current_dso, 6),
        "prior_dso_days": _round(prior_dso, 6),
        "dso_improvement_days": _round(dso_improvement_days, 6),
        ...
        "fact_age_days": base._days_between(asof, current_receivables["filed"]),
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            observation = _receivables_observation(ticker, signal_date, quality_index[ticker])
            if observation is None:
                scan["failed_dso_collection_gate"] += 1
                continue
            ...
            dso_improvement = float(observation["dso_improvement_days"] or 0.0)
            turnover_improvement = float(observation["turnover_improvement"] or 0.0)
            revenue_growth = float(observation["revenue_growth"] or 0.0)
            score = (
                0.025 * min(dso_improvement, 60.0)
                + 0.20 * max(min(turnover_improvement, 2.0), -0.25)
                + 0.25 * max(min(revenue_growth, 0.60), 0.0)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RECEIVABLES_DSO_COLLECTION_PAPER",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    **{f"dso_{k}": v for k, v in observation.items()},
                    **confirm,
                }
            )
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["dso_dso_improvement_days"] or 0.0),
            -float(row["dso_turnover_improvement"] or 0.0),
            -float(row["dso_revenue_growth"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        ...
        "max_current_dso_days": MAX_CURRENT_DSO_DAYS,
        "min_dso_improvement_days": MIN_DSO_IMPROVEMENT_DAYS,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_receivables_dso_collection_improvement"
        if gate["passed"]
        else "rejected_receivables_dso_collection_improvement_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The receivables DSO/collection-improvement source cleared the numeric "
            "three-window replay screen, but remains only a replay lead because "
            "no shared daily/backtest helper was promoted."
        )
    else:
        interpretation = (
            "The receivables DSO/collection-improvement source did not clear Gate 4 "
            f"(failed: {', '.join(gate4['failed_reasons']) or 'none'}). Do not "
            "promote or tune this fixed DSO/collection-quality bundle on the "
            "same frozen windows."
        )
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_companyfacts_receivables_quality_candidate_pool",
            "new_evidence_type": "raw_sec_companyfacts_receivables_dso_collection_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual receivables are read as raw SEC Companyfacts balance-sheet "
        "INSTANT facts (no FY duration) and matched to annual revenue ending "
        "the same fiscal-year date; both are known only by filed date (<= "
        "signal date). Current DSO uses average current/prior receivables over "
        "current revenue, prior DSO uses average prior/pre-prior receivables "
        "over prior revenue, and the rule requires DSO to improve year over year. "
    ...
    payload["next_evidence_needed"] = (
        ...
        "as quarterly DSO where reported, allowance/write-off normalization, "
        "customer concentration context, or closed forward replacement-value rows. "
        "Do not sweep the receivables tag list, DSO threshold, revenue-growth "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping the receivables tag list, DSO "
            "improvement threshold, revenue-growth floor, annual fact freshness, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Receivables DSO Collection Improvement",
            "",

...
`ai_power_energy` / `power_datacenter_infra` / ok-liquidity / full-history
cohort to the core replay universe. This uses the canonical-window
observation-universe OHLCV snapshots from exp-20260519-029, keeps all
signal/ranking/sizing/exit rules fixed, and does not change production
watchlists or live orders.
...
EXPERIMENT_ID = "exp-20260523-009"
STEM = "ai_power_datacenter_core_pool"
TRIAL_FAMILY = "governed_ai_power_candidate_pool"
TARGET_THEME = "ai_power_energy"
TARGET_SEGMENT = "power_datacenter_infra"
SOURCE_UNIVERSE_STATE = REPO_ROOT / "data" / "daily" / "universe" / "universe_state_20260522.json"
...
def _target_universe() -> dict[str, Any]:
    ...
    return {
        ...
        "selection_rule": (
            "records.theme == ai_power_energy and theme_segment == power_datacenter_infra "
            "and liquidity_tier == ok and history_class == full_history and not already in core"
        ),
...
def _target_trade_summary(target_trades_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    positive_hhi = None
    if positive_total > 0 and positive:
        positive_hhi = round(sum((pnl / positive_total) ** 2 for pnl in positive.values()), 6)
    return {
...
def _build_report(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} AI Power Datacenter Infrastructure Core-Pool Scout",
            "",
...
def build_payload() -> dict[str, Any]:
    ...
    with _target_sector_patch(target_tickers):
        for label in WINDOWS:
            ...
            print(f"[{label}] core + {TARGET_THEME}/{TARGET_SEGMENT}")
    ...
    concentration_passed = (
        ...
        and target_summary["positive_pnl_hhi"] <= 0.45
    )
    ...
    decision = (
        ...
        if gate4_passed
        else "rejected_ai_power_datacenter_core_pool"
    )
    rejection_reason = None
    if not gate4_passed:
        rejection_reason = (
            "AI power/datacenter infrastructure cohort did not clear the direct candidate-pool "
            "gate: requires positive aggregate EV/PnL, at least two improved windows, no "
            "EV-regressed window, >=6 target trades across >=2 windows, drawdown drift <=0.5pp, "
            "survival >=5%, and target positive-PnL concentration within guardrails."
        )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Production-governed AI power/datacenter infrastructure names may add cleaner "
            "candidate-pool alpha than broad AI infra baskets because the current market "
            "theme has shifted from compute-only demand toward electrical capacity, cooling, "
            "grid buildout, and contracted power supply."
        ),
        ...
        "parameters": {
            ...
            "acceptance": {
                ...
                "max_target_positive_pnl_hhi": 0.45,
            },
            "anti_js": "No JavaScript was used.",
        },
        "gate_questions": {
            "1_alpha_hypothesis": (
                "entry/candidate_pool: governed AI power/datacenter infrastructure names may "
                "add replacement-value opportunities beyond current core technology leadership."
            ),
            "2_history_check": {
                "exp-20260501-008": "Broad user AI infra expansion was observed-positive/deferred but mixed broad baskets created baseline and governance risk.",
                "exp-20260506-018": "CEG-only was rejected after one old_thin losing trade.",
                "exp-20260515-036": "Second-order electrical-equipment basket was rejected after zero candidate trades and weak-window regression.",
                "exp-20260519-014": "AI_INFRA_AGGRESSIVE segment shadow had positive aggregate variants but baseline alignment, window regression, drawdown, and sample guards failed.",
                "exp-20260523-003": "Optical-connectivity current-universe cohort was rejected after all three windows regressed.",
            },
            "3_single_causal_variable": "membership of one production-visible AI power/datacenter infrastructure cohort in the replay core universe.",
            ...
            "5_reproducibility": ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260523_009_ai_power_datacenter_core_pool.py",
        },
        "gate1": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "universe_state records.theme/theme_segment/liquidity_tier/history_class",
        ...
        "why_not_other_changes": (
            ...
            "state-surface and broad-market scalar/profile retunes due strict gate and "
            "anti-repeat risk. This uses a production-visible candidate-pool cohort "
            "rather than adding arbitrary noisy tickers."
        ),
        "known_risks": [
            ...
            "AI infra candidate-pool experiments have moderate-high multiple-testing risk.",
        ],
        "interpretation": (
            ...
            if gate4_passed
            else "The cohort did not clear the direct core-pool gate; keep AI power/datacenter names in governed pilot/research paths."
        ),
        "rejection_reason": rejection_reason,
        "next_evidence_needed": (
            "Implement shared universe/taxonomy and rerun canonical replay before promotion."
            if gate4_passed
            else "Collect forward AI_INFRA_AGGRESSIVE replacement-value outcomes or a stronger power-specific event/risk field before retrying this cohort."
        ),
        "related_files": [
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "AI power datacenter core-pool scout",

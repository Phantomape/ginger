"""exp-20260703-021: SEC Item 1.01 contract economics attribution.

Observed-only alpha attribution. The fixed SEC 8-K Item 1.01 contract
relation provenance surface from exp-20260703-017 is compressed with the
same issuer-self daily top-1 policy used in exp-20260703-018, then annotated
with machine-checkable contract economics from the existing evidence snippets:
dollar amounts, duration/term language, and normalized counterparty count.
...
EXPERIMENT_ID = "exp-20260703-021"
OWNER = "alpha-explore"
LANE = "alpha_search"
SLUG = "sec_item101_contract_relation_economic_terms"
RUNNER = f"quant/experiments/exp_20260703_021_{SLUG}.py"
...
OUT_JSON = OUT_DIR / f"exp_20260703_021_{SLUG}.json"
...
HYPOTHESIS = (
    "Within the fixed SEC 8-K Item 1.01 contract-relation provenance rows, "
    "machine-extracted contract economics such as dollar amount, duration "
    "language, and normalized counterparty-count may distinguish material "
    "agreements with better 10-session replacement value than boilerplate "
    "relations."
)
CHANGED_VARIABLE = "sec_item101_contract_relation_economic_terms_top1_10d_v1"
TRIAL_FAMILY = "sec_item101_contract_relation_economic_terms_attribution"
TRIAL_VARIANT_ID = "economic_terms_top1_10d_v1"
NEARBY_PRIORS = [
    "exp-20260703-017",
    "exp-20260703-018",
    "exp-20260703-019",
...
NEW_EVIDENCE_AXIS = (
    "New gate shape/field on the already fixed Item 1.01 provenance surface: "
    "machine-checkable contract economic terms (dollar amount, duration/term "
    "language, and normalized counterparty-count) extracted from evidence "
    "snippets. This is not a relation regex, item-code, top-N, hold, notional, "
    "priority, or response-curve retune."
)
PREDICTION = {
    ...
    "main_failure_modes": [
        "economics_regex_too_sparse",
        "public_archive_pit_caveat",
        "no_monotonic_replacement_edge",
        "ticker concentration",
        "already_captured_by_relation_priority",
    ],
    "confidence_reason": (
        "exp-20260703-018 found an observed-only positive Item 1.01 issuer-self "
        "lead and explicitly required richer counterparty/value/duration "
        "economics before retry; nearby SEC text surfaces have weak base rates "
        "and exp-20260703-019 rejected shared promotion, so this is "
        "low-probability attribution only."
    ),
    "recorded_at": "2026-07-03T21:05:25+00:00",
}
...
ACCEPTANCE_RULE = {
    "target_bucket": "amount_or_duration",
    "control_bucket": "no_amount_or_duration",
    "min_target_rows": 12,
    "min_settled_windows": 2,
    "min_target_rows_per_window": 3,
    "min_positive_target_windows_vs_spy_and_qqq": 2,
    "max_top_ticker_share": 0.40,
    "require_target_primary_means_positive": True,
    "require_target_primary_medians_nonnegative": True,
    "require_target_lift_vs_control": True,
}
CHANGED_FILES = [
    RUNNER,
    f"data/experiments/{EXPERIMENT_ID}/exp_20260703_021_{SLUG}.json",
...
DURATION_RE = re.compile(
    ...
    r"|"
    r"\b\d+(?:\.\d+)?\s*(?:year|years|month|months)\s+(?:term|period|agreement|lease|contract)\b"
    r"|"
...
def snippet_text(row: dict[str, Any]) -> str:
    snippets = row.get("evidence_snippets") or []
    parts: list[str] = []
    if isinstance(snippets, list):
        for snippet in snippets:
            if isinstance(snippet, dict):
                parts.append(str(snippet.get("snippet") or ""))
                parts.append(str(snippet.get("matched_text") or ""))
            elif snippet:
...
def extract_contract_economics(row: dict[str, Any]) -> dict[str, Any]:
    ...
    duration_matches = sorted({match.group(0).strip() for match in DURATION_RE.finditer(text)})
    counterparties = [
        str(value).strip()
    ...
    return {
        ...
        "normalized_counterparty_count": counterparty_count,
        "counterparty_examples": sorted(set(counterparties))[:5],
        ...
        "has_named_counterparty": counterparty_count > 0,
    }
...
def dedupe_accessions(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        candidate = {
            ...
            **extract_contract_economics(row),
        }
    ...
    *,
    ...
    for row in selected_rows:
        ...
        base = {
            ...
            "normalized_counterparty_count": row.get("normalized_counterparty_count"),
            "economic_terms_bucket": row.get("economic_terms_bucket"),
            "economic_terms_detail_bucket": row.get("economic_terms_detail_bucket"),
            "has_contract_amount": row.get("has_contract_amount"),
            "has_contract_duration": row.get("has_contract_duration"),
            "has_named_counterparty": row.get("has_named_counterparty"),
            "contract_amount_count": row.get("contract_amount_count"),
            "contract_amount_examples": row.get("contract_amount_examples") or [],
            "contract_duration_count": row.get("contract_duration_count"),
            "contract_duration_examples": row.get("contract_duration_examples") or [],
            "counterparty_examples": row.get("counterparty_examples") or [],
            "accession_number": row.get("accession_number"),
...
def build_result() -> dict[str, Any]:
    ...
    by_economic_bucket = group_summaries(canonical_settled, "economic_terms_bucket")
    by_economic_detail = group_summaries(canonical_settled, "economic_terms_detail_bucket")
    by_ticker = group_summaries(canonical_settled, "ticker")
    counts = count_summary(canonical_settled)
    target_bucket = ACCEPTANCE_RULE["target_bucket"]
    control_bucket = ACCEPTANCE_RULE["control_bucket"]
    target_rows = [
        row for row in canonical_settled if row.get("economic_terms_bucket") == target_bucket
    ]
    control_rows = [
        row for row in canonical_settled if row.get("economic_terms_bucket") == control_bucket
    ]
    target_metrics = metric_summary(target_rows)
    control_metrics = metric_summary(control_rows)
    target_counts = count_summary(target_rows)
    target_by_window = group_summaries(target_rows, "window")
    target_window_rows = {
        item["window"]: item["row_count"]
        for item in target_by_window
        if item.get("window") in WINDOWS
    }
    target_settled_windows = [
        label
        for label, row_count in target_window_rows.items()
        if row_count >= ACCEPTANCE_RULE["min_target_rows_per_window"]
    ]
    positive_target_windows_vs_spy_and_qqq = sum(
        1
        for item in target_by_window
        if item.get("window") in WINDOWS
        and item["row_count"] >= ACCEPTANCE_RULE["min_target_rows_per_window"]
        and item["spy_and_qqq_means_positive"]
    )
    target_lift_vs_control: dict[str, float | None] = {}
    for metric in PRIMARY_METRICS:
        target_mean = target_metrics[metric]["mean"]
        control_mean = control_metrics[metric]["mean"]
        target_lift_vs_control[metric] = (
            round(float(target_mean) - float(control_mean), 4)
    ...
    target_means_positive = all(
        (target_metrics[metric]["mean"] or 0.0) > 0 for metric in PRIMARY_METRICS
    )
    target_medians_nonnegative = all(
        (target_metrics[metric]["median"] or 0.0) >= 0 for metric in PRIMARY_METRICS
    )
    target_lift_positive = all(
        value is not None and value > 0 for value in target_lift_vs_control.values()
    )
    checks = {
        "target_rows_min_passed": len(target_rows)
        >= ACCEPTANCE_RULE["min_target_rows"],
        "target_settled_windows_min_passed": len(target_settled_windows)
        >= ACCEPTANCE_RULE["min_settled_windows"],
        "positive_target_windows_vs_spy_and_qqq_passed": (
            positive_target_windows_vs_spy_and_qqq
            >= ACCEPTANCE_RULE["min_positive_target_windows_vs_spy_and_qqq"]
        ),
        "target_primary_means_positive": target_means_positive,
        "target_primary_medians_nonnegative": target_medians_nonnegative,
        "target_lift_vs_control_positive": target_lift_positive,
        "target_top_ticker_share_passed": (
            target_counts["top_ticker_share"] is not None
            and target_counts["top_ticker_share"] <= ACCEPTANCE_RULE["max_top_ticker_share"]
        ),
    }
    ...
    if directional_support:
        ...
        decision = "observed_only_rejected_no_sec_item101_contract_economic_terms_edge"

    status_counts = Counter(str(row.get("outcome_status") or "unknown") for row in outcomes)
    why = (
        "The amount_or_duration subset of the fixed Item 1.01 issuer-self top-1 "
        "rows had positive cash/SPY/QQQ replacement value and positive lift "
        "versus no_amount_or_duration rows. This remains observed-only because "
        "the SEC text cache is a public-archive PIT proxy and no shared helper "
        "was promoted."
        if directional_support
        else "Machine-extracted Item 1.01 contract economics did not create a "
        "stable positive amount_or_duration edge versus no_amount_or_duration "
        "control rows after daily top-1 compression and ETF opportunity-cost "
        "checks."
    )
    ...
    result: dict[str, Any] = {
        ...
        "causal_components": [
            ...
            "machine-checkable contract economics extraction",
            "daily top-1 compression",
            "amount_or_duration versus no_amount_or_duration attribution",
        ...
        "new_evidence_type": "new_relation_economics_field",
        ...
        "calibration": {
            ...
            "surprise_note": (
                "Moderate surprise: machine-checkable contract economics separated "
                "replacement value despite the dry SEC text base rate."
                if directional_support
                else "Low surprise: the SEC text candidate-pool base rate is very low, "
                "and Item 1.01 amount/duration fields did not add stable lift after "
                "top-1 compression."
            ),
        },
        "source_artifacts": {
        ...
        "policy_bundle": {
            ...
            "attribution_target": "amount_or_duration rows after fixed selection",
            "attribution_control": "no_amount_or_duration rows after fixed selection",
            "economics_fields": [
        ...
        "gate2": {
            ...
            "target_bucket_rows": len(target_rows),
            "control_bucket": control_bucket,
            "control_bucket_rows": len(control_rows),
        ...
        "analysis": {
            ...
            "economic_terms_bucket_summaries": by_economic_bucket,
            "economic_terms_detail_summaries": by_economic_detail,
            "target_bucket_metrics": target_metrics,
            "control_bucket_metrics": control_metrics,
            "target_bucket_counts": target_counts,
            "target_bucket_window_summaries": target_by_window,
            "target_lift_vs_control": target_lift_vs_control,
            "top_ticker_summaries": by_ticker[:12],
            "positive_windows_vs_spy_and_qqq": positive_windows_vs_spy_and_qqq,
            "positive_target_windows_vs_spy_and_qqq": positive_target_windows_vs_spy_and_qqq,
            "settled_windows": settled_windows,
            "target_settled_windows": target_settled_windows,
        ...
        "summary": {
            ...
            "target_mean_cash": target_metrics["replacement_value_vs_cash_usd"]["mean"],
            "target_mean_spy": target_metrics["replacement_value_vs_spy_usd"]["mean"],
            "target_mean_qqq": target_metrics["replacement_value_vs_qqq_usd"]["mean"],
            "target_median_cash": target_metrics["replacement_value_vs_cash_usd"]["median"],
            "target_median_spy": target_metrics["replacement_value_vs_spy_usd"]["median"],
            "target_median_qqq": target_metrics["replacement_value_vs_qqq_usd"]["median"],
            "target_lift_vs_control_cash": target_lift_vs_control[
                "replacement_value_vs_cash_usd"
            ],
            "target_lift_vs_control_spy": target_lift_vs_control[
                "replacement_value_vs_spy_usd"
            ],
            "target_lift_vs_control_qqq": target_lift_vs_control[
                "replacement_value_vs_qqq_usd"
            ],
            "top_ticker_share": counts["top_ticker_share"],
            "target_top_ticker_share": target_counts["top_ticker_share"],
            "decision": decision,
        },
        "production_impact": {
        ...
        "post_run_reflection": {
            ...
            "forbidden_near_neighbor_retry": (
                ...
                "counterparty-count thresholds, relation priority, RS/close/volume "
                "guards, top-N, hold days, cooldown, notional, or response curves "
                "on these rows."
            ),
            "new_evidence_required": (
                "A valid retry needs prospectively accumulated daily rows with "
                "closed replacement value under the unchanged economics extractor, "
                "true exhibit-level normalized customer/supplier identity, contract "
                "revenue exposure by counterparty, or a different non-SEC-text "
        ...
        "pre_run_questions": {
            ...
            "3_single_policy_bundle": (
                ...
                "dedupe, daily top-1, machine-checkable amount/duration/"
                "counterparty-count extraction, next-open 10-session outcome, "
                "and amount_or_duration versus no_amount_or_duration cash/SPY/QQQ "
                "replacement-value verdict."
            ),
            "4_success_failure_standard": (
                "Observed-only lead iff the amount_or_duration bucket has enough "
                "rows, positive cash/SPY/QQQ means and nonnegative medians, "
                "positive lift versus no_amount_or_duration control rows, at "
                "least two target windows are positive versus SPY and QQQ, and "
                "target max ticker share <=40%."
            ),
...
- Target bucket: `{summary["target_bucket"]}`
- Target/control rows: `{summary["target_bucket_rows"]}` / `{summary["control_bucket_rows"]}`
- Target settled windows: `{", ".join(summary["target_settled_windows"]) or "none"}`
- Target positive windows vs SPY and QQQ: `{summary["positive_target_windows_vs_spy_and_qqq"]}`
- Target means cash/SPY/QQQ: `{summary["target_mean_cash"]}` / `{summary["target_mean_spy"]}` / `{summary["target_mean_qqq"]}`
- Target lift vs control cash/SPY/QQQ: `{summary["target_lift_vs_control_cash"]}` / `{summary["target_lift_vs_control_spy"]}` / `{summary["target_lift_vs_control_qqq"]}`
- Top ticker share: `{summary["top_ticker_share"]}`
- Target top ticker share: `{summary["target_top_ticker_share"]}`

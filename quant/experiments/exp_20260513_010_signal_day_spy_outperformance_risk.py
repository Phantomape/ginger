"""exp-20260513-010: signal-day ticker-vs-SPY outperformance risk.

Alpha search. Tests one production-knowable core state variable on the accepted
core stack: whether a candidate's signal-day open-to-close return beat SPY's
signal-day open-to-close return. The experiment changes only a cap-aware
post-sizing risk scalar for that state. It does not change entries, filters,
ranking, exits, targets, universe membership, LLM/news behavior, or portfolio
heat.
"""
...
EXPERIMENT_ID = "exp-20260513-010"
EXPERIMENT_SLUG = "signal_day_spy_outperformance_risk"
MULTIPLIER_KEY = "signal_day_spy_outperformance_risk_multiplier_applied"
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]) -> Callable[..., list[dict[str, Any]]]:
    def wrapped(
        ...
        spy_ret = (features_dict.get("SPY") or {}).get("signal_day_ticker_open_close_return_pct")
        for sig in enriched:
            ...
            sig["signal_day_ticker_outperformed_spy_candle"] = outperformed
        return enriched
...
def _scale_sizing(
    ...
    new_shares = min(max(shares, int(math.floor(shares * scalar))), cap_shares)
    ...
    out = dict(sizing)
    out["signal_day_spy_outperformance_baseline_shares"] = shares
    out["signal_day_spy_outperformance_desired_shares"] = int(math.floor(shares * scalar))
    out["signal_day_spy_outperformance_cap_shares"] = cap_shares
...
def _make_size_wrapper(original: Callable[..., list[dict[str, Any]]]) -> Callable[..., list[dict[str, Any]]]:
    def wrapped(
        ...
        for sig in sized:
            ...
            if (
                sig.get("signal_day_ticker_outperformed_spy_candle") is True
                and sig.get("strategy") in {"trend_long", "breakout_long"}
                and sizing.get("shares_to_buy")
            ):
                adjusted = _scale_sizing(sig, sizing, CURRENT_SCALAR, portfolio_value)
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "ticker_open_close_return_pct": sig.get("signal_day_ticker_open_close_return_pct"),
                            "spy_open_close_return_pct": sig.get("signal_day_spy_open_close_return_pct"),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Signal-Day SPY Outperformance Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    candidates = [
        _candidate_payload(scalar, before_runs)
    ...
    decision = (
        ...
        if passed
        else "rejected_signal_day_spy_outperformance_risk"
    )
    interpretation = (
        "Signal-day ticker-vs-SPY outperformance risk allocation cleared the canonical gate and should be promoted through shared policy before production use."
        if passed
        else "Signal-day ticker-vs-SPY outperformance risk allocation did not clear the canonical three-window gate; do not promote this relative signal-day scalar on the frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Signals whose own signal-day candle outperforms SPY's signal-day candle may have stronger idiosyncratic follow-through than broad green-candle strength alone; allocate slightly more risk rather than changing entry filters."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "signal_day_ticker_vs_spy_outperformance_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing scalar for signals whose signal-day ticker open-to-close return exceeds SPY's same-day open-to-close return"
        ),
        "parameters": {
            "state_definition": "signal_day_ticker_open_close_return_pct > signal_day_spy_open_close_return_pct",
            "risk_multiplier_sweep": SCALARS,
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core state risk allocation using production-knowable signal-day ticker-vs-SPY relative candle strength",
            "2_history_check": {
                "exp-20260513-007": "own green-candle 1.05x was accepted; this tests a materially narrower relative-strength state on top of that accepted baseline, not a nearby own-candle scalar retune.",
                "exp-20260513-009": "upper-quartile close-location top-up was rejected due old_thin regression and drawdown; this run avoids close-location shape and uses benchmark-relative outperformance.",
                "exp-20260512-106/107": "sector-proxy tape scalars failed or were underpowered; this uses own ticker relative to SPY, not sector tape.",
                "exp-20260513-001": "core breakout strong-volume scalar was immaterial; this is strategy-agnostic signal-day relative price action.",
            },
            "3_single_causal_variable": "signal_day_ticker_vs_spy_outperformance_risk_multiplier with the natural zero-excess state boundary fixed",
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_010_signal_day_spy_outperformance_risk.py",
        },
        "backtest_protocol": {
        ...
        "related_files": [
            "quant/experiments/exp_20260513_010_signal_day_spy_outperformance_risk.py",
            "data/experiments/exp-20260513-010/signal_day_spy_outperformance_risk.json",
            "docs/experiments/logs/exp-20260513-010.json",
            "docs/experiments/tickets/exp-20260513-010.json",
            "docs/experiments/artifacts/exp-20260513-010_signal_day_spy_outperformance_risk.md",
            "docs/experiment_log.jsonl",
        ],
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited; SEC semantic fields are missing; Space frozen-sample profile scalars and noisy candidate additions have been over-mined or rejected; own-candle close-location and strong-volume neighbors were just rejected. This tests a fresh benchmark-relative core allocation state without changing entries or expanding the pool."
        ),
    }
    return payload

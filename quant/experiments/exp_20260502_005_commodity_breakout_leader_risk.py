"""exp-20260502-005 commodity breakout leader allocation.

Alpha search. Test one capital-allocation variable: accepted commodity
breakout signals that are otherwise-unmodified risk_on SPY-relative leaders may
deserve more risk. This is sizing only, not a breakout target-width retry and
not a raw ETF universe expansion.
"""
...
VARIANTS = OrderedDict([
    ("commodity_breakout_leader_3_0x", 3.0),
    ("commodity_breakout_leader_4_0x", 4.0),
])
...
def _metrics(result: dict) -> dict:
    ...
    for trade in result.get("trades", []):
        ...
        if multipliers.get("commodity_breakout_leader_risk_multiplier_applied"):
    ...
    return {
        ...
        "commodity_breakout_leader_signals_seen": _state["signals_seen"],
        "commodity_breakout_leader_signals_resized": _state["signals_resized"],
        "commodity_breakout_leader_trade_count": resized_trade_count,
        "commodity_breakout_leader_trade_pnl": round(resized_trade_pnl, 2),
        "commodity_breakout_leader_trades": resized_trade_tickers,
    }
...
def _patch_size_signals(target_multiplier: float | None):
    ...
    def patched(signals, portfolio_value, risk_pct=None):
        ...
        for sig in sized:
            ...
            preserved["commodity_breakout_leader_risk_multiplier_applied"] = target_multiplier
            preserved["commodity_breakout_leader_original_multiplier"] = leader_applied
...
def _aggregate(before: OrderedDict, after: OrderedDict) -> dict:
    ...
    return {
        ...
        "commodity_breakout_leader_signals_resized_delta_sum": sum(
            d["commodity_breakout_leader_signals_resized"] for d in deltas.values()
        ),
    }
...
def build_payload() -> dict:
    ...
    for variant, multiplier in VARIANTS.items():
        ...
        for label, cfg in WINDOWS.items():
            ...
            print(
                ...
                f"PnL={m['total_pnl']} SharpeD={m['sharpe_daily']} "
                f"resized={m['commodity_breakout_leader_signals_resized']}"
            )
    ...
    return {
        ...
        "lane": "alpha_search",
        "change_type": "capital_allocation_commodity_breakout_spy_leader",
        "hypothesis": (
            "Commodity breakout signals that are already risk_on and SPY-relative "
            "leaders may deserve more risk because the current fixed-window cohort "
            "shows positive convexity in late_strong and mid_weak with no old_thin exposure."
        ),
        ...
        "parameters": {
            "single_causal_variable": "commodity breakout SPY-relative leader total risk multiplier",
            "baseline": "accepted 2.0x total risk for otherwise-unmodified risk_on SPY-relative leaders",
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, add a shared portfolio_engine constant and sizing "
                "branch consumed by both backtester and run."
            ),
        },
        "llm_metrics": {
            "used_llm": False,
        ...
        "history_guardrails": {
            ...
            "not_spy_leader_confirmation_retry": True,
            "why_not_simple_repeat": (
                "This tests sizing within already-entered commodity breakout "
                "leaders, not wider exits, raw ETF additions, or another broad "
                "SPY/sector confirmation scalar."
            ),
        },
        "rejection_reason": (
            None if accepted else
            "No commodity breakout leader multiplier passed fixed-window Gate 4."
        ),
        "next_retry_requires": [
            "Do not retry nearby commodity breakout leader multipliers without forward evidence.",
            "A valid retry needs event/news or lifecycle context, not another scalar.",
            "Do not use this as evidence for precious-metals breakout target widening.",
        ],
        "related_files": [
...
def main() -> int:
    ...
    TICKET_JSON.write_text(json.dumps({
        ...
        "title": "Commodity breakout leader sizing",
        "summary": (

"""exp-20260621-021: raw SEC AFS-securities OCI component relief scout.

Replay-only alpha search. The single decision hypothesis is a PIT free SEC
Companyfacts candidate source: broad liquid-universe names whose latest filed
annual available-for-sale securities OCI component turns positive versus the
prior comparable annual period, while revenue is not contracting and price
already shows liquid SPY-relative leadership, may reflect rate-duration /
mark-to-market overhang easing before a 10-trading-day continuation leg.

This is intentionally a private replay scout because the raw Companyfacts field
shape must be verified before any shared historical/daily helper. No production
...
shared daily/backtest parity reproduces the exact PIT field mapping. No
JavaScript is used.
"""
...
EXPERIMENT_ID = "exp-20260621-021"
STEM = "afs_oci_component_relief"
TRIAL_FAMILY = "raw_sec_companyfacts_afs_oci_component_relief_candidate_pool"
TRIAL_VARIANT_ID = "afs_oci_component_relief_top1_next_open_10d_v1"
CHANGED_VARIABLE = "raw_sec_companyfacts_afs_oci_component_relief_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260621_021_{STEM}.json"
...
MIN_CURRENT_AFS_OCI_USD = 1_000_000.0
MIN_CURRENT_AFS_OCI_TO_REVENUE = 0.0005
MIN_AFS_OCI_IMPROVEMENT_TO_REVENUE = 0.0010
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "afs_component_is_bank_rate_noise",
        "old_thin_window_regression",
        "drawdown_drift",
        "accepted_distribution_comparator_not_beaten",
        "companyfacts_oci_family_saturated",
    ],
    "confidence_reason": (
        "AOCI overhang closeout explicitly required a decomposed AFS securities "
        "field. Local Companyfacts coverage is broad, but recent OCI and "
        "Companyfacts scouts failed drawdown/comparator gates."
    ),
    "recorded_at": "2026-06-21T21:10:31+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC annual available-for-sale securities OCI component "
            "fact, missing prior comparable annual AFS OCI fact, missing matched "
            "annual revenue, stale facts, missing CIK mapping, missing OHLCV, "
    ...
    "parity_note": (
        ...
        "SEC tag mapping, filed-date PIT annual AFS securities OCI component "
        "turn-positive gate, revenue context, liquid SPY-relative confirmation, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: PIT raw SEC Companyfacts available-for-sale securities "
        "OCI component relief or turn-positive adjustment, scaled by revenue "
        "and paired with liquid SPY-relative leadership, may identify "
        "rate-duration / mark-to-market overhang easing before 10-trading-day "
        "continuation."
    ),
    "2_history_check": {
        "novelty_gate": (
            "experiment.py new warned on AOCI/OCI near-neighbors. The override "
            "is justified because the new evidence axis is raw SEC "
            "available-for-sale securities OCI adjustment, not aggregate AOCI "
            "balance, FX translation OCI, cash-flow hedge OCI, cash "
            "exchange-rate effect, or an OHLCV risk envelope."
        ),
        "exp-20260617-018": (
            "Rejected aggregate AOCI overhang relief. Its closeout explicitly "
            "required materially different AOCI decomposition such as "
            "available-for-sale securities, FX translation, pension OCI, or "
            "rate-duration exposure."
        ),
        "exp-20260620-026/028/030": (
            "Rejected FX translation OCI, hedge confirmation, and cash-effect "
            "confirmation variants. This run uses AFS securities OCI, not FX "
            "translation, derivative hedge OCI, or cash-flow exchange-rate "
            "effect."
        ),
        "exp-20260621-005": (
            "Rejected FX OCI drawdown-aware notional envelope. This run changes "
            "the PIT source evidence field, not risk allocation or notional "
            "scalars."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260621_021_afs_oci_component_relief.py"
    ),
}
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        afs_oci_facts = _raw_annual_facts(usgaap, AFS_OCI_TAGS, unit="USD")
        revenue_facts = _raw_annual_facts(usgaap, REVENUE_TAGS, unit="USD")
        if not afs_oci_facts:
            stats["tickers_missing_raw_annual_afs_oci"] += 1
            continue
        if not revenue_facts:
            stats["tickers_missing_raw_annual_revenue"] += 1
            continue
        index[ticker] = {
            "afs_oci": afs_oci_facts,
            "revenue": revenue_facts,
        }
        stats["tickers_with_raw_annual_afs_oci_and_revenue"] += 1
        stats["raw_annual_afs_oci_fact_count"] += len(afs_oci_facts)
        stats["raw_annual_revenue_fact_count"] += len(revenue_facts)

    summary = {
        "raw_companyfacts_cache": _repo_rel(RAW_COMPANYFACTS_CACHE),
        "afs_oci_tags": list(AFS_OCI_TAGS),
        "afs_oci_unit": "USD",
...
def _afs_oci_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    del ticker
    current_afs = _latest_period_fact(facts["afs_oci"], asof=asof)
    if current_afs is None:
        return None
    if base._days_between(asof, current_afs["filed"]) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None

    prior_afs = _latest_period_fact(facts["afs_oci"], asof=asof, before_end=current_afs["end"])
    if prior_afs is None:
        return None

    current_revenue = _latest_period_fact(facts["revenue"], asof=asof, end=current_afs["end"])
    prior_revenue = _latest_period_fact(facts["revenue"], asof=asof, end=prior_afs["end"])
    if current_revenue is None or prior_revenue is None:
        return None

    current_afs_value = float(current_afs["value"])
    prior_afs_value = float(prior_afs["value"])
    ...
    current_ratio = current_afs_value / current_revenue_value
    improvement_to_revenue = (current_afs_value - prior_afs_value) / current_revenue_value
    if current_afs_value < MIN_CURRENT_AFS_OCI_USD:
        return None
    if current_ratio < MIN_CURRENT_AFS_OCI_TO_REVENUE:
        return None
    if improvement_to_revenue < MIN_AFS_OCI_IMPROVEMENT_TO_REVENUE:
        return None
    if prior_afs_value > 0.0:
        return None

    return {
        "current_fiscal_year_end": current_afs["end"],
        "prior_fiscal_year_end": prior_afs["end"],
        "current_afs_oci_filed": current_afs["filed"],
        "prior_afs_oci_filed": prior_afs["filed"],
        "current_revenue_filed": current_revenue["filed"],
        "prior_revenue_filed": prior_revenue["filed"],
        "current_afs_oci_tag": current_afs["tag"],
        "current_revenue_tag": current_revenue["tag"],
        "current_afs_oci": _round(current_afs_value, 2),
        "prior_afs_oci": _round(prior_afs_value, 2),
        "afs_oci_improvement": _round(current_afs_value - prior_afs_value, 2),
        "current_afs_oci_to_revenue": _round(current_ratio, 6),
        "afs_oci_improvement_to_revenue": _round(improvement_to_revenue, 6),
        ...
        "fact_age_days": base._days_between(asof, current_afs["filed"]),
    }
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _afs_oci_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_afs_oci_gate"] += 1
                continue
            ...
            score = (
                5.00 * min(float(quality["afs_oci_improvement_to_revenue"] or 0.0), 0.03)
                + 2.00 * min(float(quality["current_afs_oci_to_revenue"] or 0.0), 0.02)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_AFS_OCI_COMPONENT_RELIEF_PAPER",
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["quality_afs_oci_improvement_to_revenue"] or 0.0),
            -float(row["quality_current_afs_oci_to_revenue"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        ...
        "min_current_afs_oci_usd": MIN_CURRENT_AFS_OCI_USD,
        "min_current_afs_oci_to_revenue": MIN_CURRENT_AFS_OCI_TO_REVENUE,
        "min_afs_oci_improvement_to_revenue": MIN_AFS_OCI_IMPROVEMENT_TO_REVENUE,
    }
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_afs_oci_component_relief"
        if gate["passed"]
        else "rejected_afs_oci_component_relief_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    failed_set = set(gate4.get("failed_reasons") or [])
    drawdown_only_failure = (
        not gate4["passed"]
        and failed_set == {"drawdown_drift_too_high"}
    ...
    if gate4["passed"]:
        interpretation = (
            "The raw SEC AFS securities OCI component relief source "
        ...
        interpretation = (
            "The raw SEC AFS securities OCI component relief source had "
            "positive gross replacement value in all three windows and beat the "
            "accepted compression/distribution aggregate comparators, but is "
            "rejected because max drawdown drift exceeded the Gate 4 guardrail "
            f"({float(gate4['max_drawdown_worse'] or 0.0):.4f} > "
            f"{float(gate4['max_drawdown_worse_guardrail'] or 0.0):.4f}). "
            "The readout looks like a real but risk-rough lead: the field "
            "may select rate-sensitive balance-sheet leadership and add too "
            "much drawdown for a fixed top-1/day 10-day sleeve. It is not "
            "retained or promoted."
        )
    else:
        interpretation = (
            "The raw SEC AFS securities OCI component relief source did "
    ...
    payload.update(
        {
            ...
            "new_evidence_type": "raw_sec_available_for_sale_securities_oci_component_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual available-for-sale securities OCI component and annual revenue "
        "are read from raw SEC Companyfacts tags and are known only by their "
        "filed date (<= signal date). The current annual AFS securities OCI "
        "component must be positive, the prior comparable annual component must "
        "be non-positive, and the improvement/current component must clear fixed "
        "revenue-scaled floors while annual revenue is not contracting. Price "
    ...
    payload["gate2"]["runtime_fields"] = [
        "raw SEC companyfacts annual available-for-sale securities OCI component facts",
    ...
    payload["next_evidence_needed"] = (
        "A retry needs materially richer PIT rate-duration evidence, such as "
        "security duration/maturity buckets tied to AFS fair-value marks, "
        "interest-rate sensitivity disclosure, bank/non-bank balance-sheet "
        "classification, or closed forward replacement-value rows. Do not sweep "
        "AFS OCI ratio, improvement, freshness, revenue, RS/close/volume/vol "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping AFS OCI ratio, AFS OCI improvement, current "
            "revenue, revenue growth, annual fact freshness, RS/close/volume/vol "
            "guards, top-N, hold days, cooldown, or notional on these frozen "
            "windows."
        ),
        "new_evidence_required": (
            "Because this failed only on drawdown despite positive three-window "
            "gross deltas, a valid next step needs a materially richer PIT "
            "rate-duration or risk-control evidence axis, not a parameter "
            "sweep: AFS maturity/duration buckets, interest-rate sensitivity "
            "disclosure, bank/non-bank balance-sheet classification, closed "
            "forward replacement-value rows, or a separately predeclared "
            "drawdown-aware policy bundle."
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} AFS OCI Component Relief",
            "",

"""exp-20260514-002: Space forward replacement same-theme strength risk.

Tests one causal variable on top of the accepted exp-20260513-113 default-off
Space stack: an additional risk scalar for official Space tickers whose closed
10d event-state profile has stronger same-theme replacement value.
...
the already accepted forward replacement-positive bucket has a narrower
same-theme strength sub-bucket that deserves incremental default-off risk.
"""
...
EXPERIMENT_ID = "exp-20260514-002"
STEM = "space_forward_replacement_same_theme_strength_risk"
BEFORE_EXPERIMENT_ID = "exp-20260513-113"
...
SAME_THEME_STRENGTH_FLOORS = (250.0, 500.0, 1000.0)
SAME_THEME_STRENGTH_RISK_SCALARS = (
    0.90,
    1.00,
    1.025,
    1.05,
...
def _forward_replacement_profile_gate() -> dict[str, Any]:
    ...
    target_by_floor = {
        str(floor): sorted(
            ...
            if float(profile["avg_10d_same_theme_replacement_value"]) >= floor
        )
        for floor in SAME_THEME_STRENGTH_FLOORS
    }
    return {
        ...
        "same_theme_strength_floors": list(SAME_THEME_STRENGTH_FLOORS),
        "target_tickers_by_floor": target_by_floor,
    }


def _target_tickers_for_floor(
    gate: dict[str, Any],
    same_theme_floor: float,
) -> set[str]:
    return {
        ticker
...
def _run_variant(
    ...
    *,
    ...
    strength_tickers = _target_tickers_for_floor(forward_gate, same_theme_floor)

    adjustments: list[dict[str, Any]] = []
    counts: Counter[str] = Counter()

    def size_with_same_theme_strength_scalar(
        ...
        for signal in sized:
            ...
            if ticker in strength_tickers and sizing:
                ...
                shares_before, shares_after = _scale_and_count(
                    sizing=sizing,
                    scalar=same_theme_scalar,
                    portfolio_value=portfolio_value,
                    marker="space_forward_replacement_same_theme_strength_risk",
                    counts=counts,
                    ticker=ticker,
                )
                ...
                adjustments.append(
                    {
                        ...
                        "scalar": same_theme_scalar,
                        "same_theme_floor": same_theme_floor,
                ...
                signal = {
                    **signal,
                    ...
                    "space_forward_replacement_same_theme_strength_scalar": same_theme_scalar,
                    "space_forward_replacement_same_theme_strength_floor": same_theme_floor,
                }
    ...
    portfolio_engine.size_signals = size_with_same_theme_strength_scalar
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "same_theme_strength_adjustment": source_diversity_exp._adjustment_summary(
                    adjustments[before_adjustments:]
                ),
                "same_theme_strength_counts": dict(sorted((counts - before_counts).items())),
            }
        ...
        return {
            ...
            "same_theme_strength_adjustment_summary": source_diversity_exp._adjustment_summary(
                adjustments
            ),
            "same_theme_strength_counts": dict(sorted(counts.items())),
            "same_theme_strength_adjustment_sample": adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["same_theme_strength_counts"].get(
            "space_forward_replacement_same_theme_strength_risk_changed_signal",
            0,
        )
    )
    eligible_count = int(variant["same_theme_strength_counts"].get("eligible_signal", 0))
    scalar = float(
        variant["parameters"]["space_forward_replacement_same_theme_strength_scalar"]
    )
    ...
    return {
        ...
        "target_set_narrowed_vs_exp113": narrowed,
        "accepted": bool(
            scalar != 1.0
            and narrowed
            and changed_count > 0
...
def _gate2_runtime_state(before: dict[str, Any]) -> dict[str, Any]:
    ...
    for window_name, payload in before["by_window"].items():
        counts = payload.get("same_theme_strength_counts") or {}
        eligible += int(counts.get("eligible_signal", 0) or 0)
        summary = payload.get("same_theme_strength_adjustment") or {}
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space forward replacement same-theme strength risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_forward_replacement_same_theme_strength_scalar` for the "
            "narrower closed-forward profile bucket whose average 10d "
            "same-theme replacement value clears the tested floor. Candidate "
            "pool, event labels, ranking, targets, stops, LLM/news, accepted "
            "exp-113 scalar, and live Space slots stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best floor/scalar: "
            f"`{best['parameters']['space_forward_replacement_same_theme_strength_floor']}` / "
            f"`{best['parameters']['space_forward_replacement_same_theme_strength_scalar']}`"
        ),
        (
            "- Aggregate delta vs exp-113: "
            f"EV `{gate['aggregate_delta_vs_before']['expected_value_score_sum']:.6f}`, "
            f"PnL `{gate['aggregate_delta_vs_before']['total_pnl_sum']:.2f}`"
        ),
        (
            "- Same-theme-strength signals changed: "
        ...
        f"- Target set narrowed vs exp-113: `{gate['target_set_narrowed_vs_exp113']}`",
        "",
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["by_window"][name]["same_theme_strength_adjustment"][
...
def run() -> dict[str, Any]:
    ...
    before = _run_variant(
        "accepted_exp113_same_theme_strength_neutral",
        same_theme_floor=500.0,
        same_theme_scalar=1.0,
    ...
    gate2 = {
        ...
        "same_theme_strength_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    variants = []
    for floor in SAME_THEME_STRENGTH_FLOORS:
        for scalar in SAME_THEME_STRENGTH_RISK_SCALARS:
            variants.append(
                _run_variant(
                    f"{STEM}_{str(floor).replace('.', '_')}_{str(scalar).replace('.', '_')}",
                    same_theme_floor=floor,
                    same_theme_scalar=scalar,
                    forward_gate=forward_gate,
                    source_diversity_gate=source_diversity_gate,
                    attention_gate=attention_gate,
                    single_event_gate=single_event_gate,
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested narrowed same-theme-strength bucket improved aggregate "
            "EV/PnL across the three windows without a window-level EV regression, "
            "drawdown/survival violation, or exp-113 target-set overlap violation."
        )
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "On top of accepted exp-20260513-113, official Space signals whose "
            "closed 10d forward event-state profile has stronger same-theme "
            "replacement value may deserve incremental default-off risk. This "
            "tests the playbook's forward replacement-value direction without "
            "LLM soft-ranking, ticker expansion, or live Space slots."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_forward_replacement_same_theme_strength_risk_scalar",
        "single_causal_variable": (
            "extra risk scalar for the narrowed official Space closed-forward "
            "same-theme replacement-strength profile bucket"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                ...
                "same-theme replacement value is materially stronger than the "
                "accepted merely-positive exp-113 profile."
            ),
            "q2_prior_experiments": [
                ...
                "exp-20260513-037 rejected non-dilutive contract profile risk because the edge was single-window concentrated.",
            ],
            "q3_single_causal_variable": (
                "Only the additional same-theme-strength risk scalar changes; "
                "accepted exp-113 forward scalar and all entries/exits/ranking/targets stay fixed."
            ),
            "q4_acceptance_standard": (
                ...
                "real adjusted signals, and target set narrower than exp-113."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            ...
            "tested_same_theme_strength_floors": list(SAME_THEME_STRENGTH_FLOORS),
            "tested_same_theme_strength_scalars": list(SAME_THEME_STRENGTH_RISK_SCALARS),
            "base_forward_replacement_target_tickers": forward_gate[
                "base_target_tickers"
            ],
            "target_tickers_by_floor": forward_gate["target_tickers_by_floor"],
            "locked_variables": [
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_forward_replacement_same_theme_strength_risk"
            if decision == "accepted"
            else "rejected_space_forward_replacement_same_theme_strength_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not promote same-theme-strength risk from this frozen replay. "
        ...
        "why_not_other_changes": (
            ...
            "are exhausted. This tests forward replacement profile strength instead."
        ),
        "known_risks": [
            ...
            "The floor sweep is a same-sample profile-strength scout; promotion needs conservative sizing and parity tests if accepted.",
        ],
    }
    return payload
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "best_floor": result["best_variant"]["parameters"][

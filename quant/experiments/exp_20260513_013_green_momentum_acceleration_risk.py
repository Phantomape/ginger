"""exp-20260513-013: green-candle momentum-acceleration risk allocation.
...
with both positive, and whose accepted signal-day own candle is green. This is
a cap-aware post-sizing risk top-up scout, not an entry filter or ranking
change.
"""
...
EXPERIMENT_ID = "exp-20260513-013"
EXPERIMENT_SLUG = "green_momentum_acceleration_risk"
MULTIPLIER_KEY = "green_momentum_acceleration_risk_multiplier_applied"
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]) -> Callable[..., list[dict[str, Any]]]:
    def wrapped(
        ...
        for sig in enriched:
            ...
            sig["green_momentum_acceleration_state"] = acceleration_state
        return enriched
...
def _scale_sizing(sizing: dict[str, Any], scalar: float, portfolio_value: float) -> dict[str, Any]:
    ...
    out = dict(sizing)
    out["green_momentum_acceleration_baseline_shares"] = shares
    out["green_momentum_acceleration_desired_shares"] = desired_shares
    out["green_momentum_acceleration_cap_shares"] = cap_shares
    out["green_momentum_acceleration_new_shares"] = new_shares
...
def _make_size_wrapper(original: Callable[..., list[dict[str, Any]]]) -> Callable[..., list[dict[str, Any]]]:
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("green_momentum_acceleration_state") and sizing.get("shares_to_buy"):
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Green Momentum Acceleration Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_green_momentum_acceleration_risk"
    )
    interpretation = (
        "Green-candle momentum-acceleration core risk top-up cleared the canonical three-window gate and requires shared policy implementation before production use."
        if passed
        else "Green-candle momentum-acceleration core risk top-up did not clear the canonical three-window gate; do not promote this state variable without a stronger discriminator."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Core trend/breakout signals with both accepted signal-day green confirmation and short-horizon momentum acceleration should deserve more capital because the newest half of the 20-day path is carrying a constructive signal-day move."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "green_momentum_acceleration_risk_multiplier",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core state risk allocation using production-visible signal-day green confirmation plus short-horizon momentum acceleration",
            "2_history_check": {
                "exp-20260513-007": "accepted own-green 1.05x cap-aware top-up; remains locked as baseline, this tests a narrower acceleration overlay.",
                "exp-20260512-111": "broad momentum acceleration improved late/mid but regressed old_thin; this run adds the already accepted green-candle discriminator instead of retrying a nearby broad scalar.",
                "exp-20260512-950": "RS20-leader acceleration remained underpowered; this run is not restricted to RS20 leaders and uses own signal-day confirmation.",
                ...
                "exp-20260513-011": "rejected ticker-vs-SPY signal-day excess margin; this run does not use benchmark-relative signal-day thresholds.",
            ...
            "3_single_causal_variable": "green_momentum_acceleration_risk_multiplier with fixed production-visible green-plus-acceleration state",
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_013_green_momentum_acceleration_risk.py",
        },
        "backtest_protocol": {
        ...
        "related_files": [
            "quant/experiments/exp_20260513_013_green_momentum_acceleration_risk.py",
            "data/experiments/exp-20260513-013/green_momentum_acceleration_risk.json",
            "docs/experiments/logs/exp-20260513-013.json",
            "docs/experiments/tickets/exp-20260513-013.json",
            "docs/experiments/artifacts/exp-20260513-013_green_momentum_acceleration_risk.md",

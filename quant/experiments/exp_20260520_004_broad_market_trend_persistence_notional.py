"""exp-20260520-004: broad-market trend-persistence paper notional.

Alpha search follow-up to exp-20260520-003. The accepted broad-market
price-floor leadership pool, rank-notional profile, low-extension scalar, and
high-volatility scalar are fixed. This run changes one causal variable: an
extra default-off paper notional scalar for already-selected broad-market
candidates whose last 20 close-to-close returns show high trend persistence.
...
EXPERIMENT_ID = "exp-20260520-004"
EXPERIMENT_SLUG = "broad_market_trend_persistence_notional"
BASELINE_EXPERIMENT_ID = "exp-20260520-003"
...
BASELINE_JSON = (
    ...
    / "broad_market_high_volatility_notional.json"
)
CONTROL_JSON = (
    REPO_ROOT
    / "data"
...
TREND_PERSISTENCE_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict()
TREND_PERSISTENCE_SWEEP["baseline_no_trend_persistence"] = {
    "positive_day_ratio_20_min": 0.60,
    "scalar": 1.00,
}
for threshold in (0.55, 0.60, 0.65, 0.70):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15):
        ...
        TREND_PERSISTENCE_SWEEP[f"posday20_gte_{threshold_text}_scalar_{scalar_text}"] = {
            "positive_day_ratio_20_min": threshold,
            "scalar": scalar,
        }

MIN_ADJUSTED_TRADES = 8
MIN_PREFERRED_ADJUSTED_TRADES = 9
...
def _simulate_window(
    *,
    label: str,
    positive_day_ratio_20_min: float,
    scalar: float,
    ...
    cfg = {
        **PROFILE_CONFIG,
        "trend_persistence_positive_day_ratio_20_min": positive_day_ratio_20_min,
        "trend_persistence_notional_scalar": scalar,
    }
        ...
        rows.append(
            {
                ...
                "high_volatility_notional_multiplier": trade.get("high_volatility_notional_multiplier"),
                ...
                "trend_persistence_notional_multiplier": trade.get("trend_persistence_notional_multiplier"),
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("trend_persistence_support_applied")]
    base["trend_persistence_adjusted_trade_count"] = len(adjusted)
    base["trend_persistence_adjusted_pnl"] = round(
    ...
    *,
    ...
    for label, spec in WINDOWS.items():
        scout = _simulate_window(
            label=label,
            positive_day_ratio_20_min=positive_day_ratio_20_min,
            scalar=scalar,
    ...
    adjusted = [row for row in all_trades if row.get("trend_persistence_support_applied")]
    ...
    gate4_passed = bool(
        variant_name != "baseline_no_trend_persistence"
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    preferred = [
        row
        for row in passing
        if row["adjusted_trade_count"] >= MIN_PREFERRED_ADJUSTED_TRADES
    ]
    pool = preferred or passing or [row for row in variants if row["variant_name"] != "baseline_no_trend_persistence"]
    return sorted(
        pool,
        key=lambda row: (
            bool(row["gate4"]["passed"]),
            row["adjusted_trade_count"] >= MIN_PREFERRED_ADJUSTED_TRADES,
    ...
    return [
        {
            ...
            "positive_day_ratio_20_min": row["positive_day_ratio_20_min"],
            "scalar": row["scalar"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Trend-Persistence Notional",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    if baseline_payload.get("decision") != "accepted_default_off_broad_market_high_volatility_notional":
    ...
    variants = [
        _variant_payload(
            variant_name=name,
            positive_day_ratio_20_min=values["positive_day_ratio_20_min"],
        ...
        for name, values in TREND_PERSISTENCE_SWEEP.items()
    ]
    selected = _choose_selected(variants)
    accepted = selected["gate4"]["passed"]
    decision = (
        "accepted_default_off_broad_market_trend_persistence_notional"
        if accepted
        else "rejected_broad_market_trend_persistence_notional"
    )
    ...
    accepted_high_vol_payload = broad_market_candidate_notional_payload(
        1,
        {
            "ret5": 0.03,
            "realized_volatility_20": DEFAULT_CONFIG["high_volatility_20_min"],
            "positive_day_ratio_20": 0.50,
        },
    )
    accepted_trend_payload = broad_market_candidate_notional_payload(
        1,
        {
            "ret5": 0.03,
            "realized_volatility_20": 0.04,
            "positive_day_ratio_20": selected["positive_day_ratio_20_min"],
        },
    )
    accepted_stacked_payload = broad_market_candidate_notional_payload(
        1,
        {
            "ret5": 0.01,
            "realized_volatility_20": DEFAULT_CONFIG["high_volatility_20_min"],
    ...
    shared_adapter_parity = {
        "passed": selected["positive_day_ratio_20_min"]
        == DEFAULT_CONFIG["trend_persistence_positive_day_ratio_20_min"]
        and selected["scalar"] == DEFAULT_CONFIG["trend_persistence_notional_scalar"]
        and accepted_high_vol_payload["notional"] == 10_350.0
        and accepted_trend_payload["notional"] == 10_350.0
        and accepted_stacked_payload["notional"] == 13_687.87,
        "shared_rule_version": RULE_VERSION,
        "low_extension_rule_version": LOW_EXTENSION_RULE_VERSION,
        "default_config_high_volatility_20_min": DEFAULT_CONFIG["high_volatility_20_min"],
        "default_config_high_volatility_scalar": DEFAULT_CONFIG["high_volatility_notional_scalar"],
        "default_config_trend_persistence_positive_day_ratio_20_min": DEFAULT_CONFIG[
            "trend_persistence_positive_day_ratio_20_min"
        ],
        "default_config_trend_persistence_scalar": DEFAULT_CONFIG["trend_persistence_notional_scalar"],
        "selected_positive_day_ratio_20_min": selected["positive_day_ratio_20_min"],
        "selected_trend_persistence_scalar": selected["scalar"],
        "rank1_high_volatility_notional_from_shared_helper": accepted_high_vol_payload["notional"],
        "rank1_trend_persistence_notional_from_shared_helper": accepted_trend_payload["notional"],
        "rank1_low_extension_high_volatility_trend_persistence_notional_from_shared_helper": accepted_stacked_payload[
            "notional"
        ],
        "trend_persistence_helper_promoted": True,
    }
    production_impact = {
    ...
    payload: dict[str, Any] = {
        ...
        "hypothesis": (
            ...
            "a high 20-day positive close-to-close day ratio may represent "
            "persistent leadership rather than one-day extension; a bounded "
            "support notional scalar can improve allocation after the accepted "
            "rank, low-extension, and high-volatility stack."
        ),
        "alpha_hypothesis": {
            "category": "candidate_pool / capital allocation",
            "playbook_alignment": (
                "Tests a new production-visible quality/risk field on the "
                "default-off broad-market paper sleeve. It avoids LLM "
                "soft-ranking, core expansion, and nearby price/ret20/ret60/"
                "near-high/volume/ret5 threshold retunes."
            ),
        },
        "history_check": {
            "nearby_experiments": [
            ...
            "anti_repeat": (
                "The broad-market candidate definition, rank profile, ret5 "
                "low-extension rule, realized-volatility rule, price floor, "
                "hold, slots, and universe are fixed; only a new trend-"
                "persistence support field changes."
            ),
            "selection_note": (
                "The field is derived from decision-time OHLCV only: the share "
                "of positive close-to-close sessions in the last 20 trading days."
            ),
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "broad_market_trend_persistence_notional_scalar",
        "single_causal_variable": (
            "20-day positive-day-ratio paper notional support for fixed broad-market selected candidates"
        ),
        ...
        "parameters": {
            ...
            "selected_positive_day_ratio_20_min": selected["positive_day_ratio_20_min"],
            "selected_trend_persistence_scalar": selected["scalar"],
            "selection_preferred_min_adjusted_trades": MIN_PREFERRED_ADJUSTED_TRADES,
            "sweep": TREND_PERSISTENCE_SWEEP,
            "profile_config": {
        ...
        "backtest_protocol": (
            ...
            "exp-20260520-003 high-volatility broad-market adapter is the before "
            "state; after state adds the trend-persistence notional helper."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "selected_variant": {
            ...
            "positive_day_ratio_20_min": selected["positive_day_ratio_20_min"],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "candidate_pool/capital allocation: broad-market paper candidates with high 20-day positive-day persistence deserve a bounded support scalar.",
            "2_past_similar_experiments": "Broad-market price-floor, rank-notional, low-extension ret5, and realized-volatility support were accepted; positive-day trend persistence was not previously tested.",
            "3_single_variable": "Only positive_day_ratio_20_min/scalar changes inside one notional-support field; eligibility, rank profile, low-extension, high-volatility, hold, slots, and universe controls stay fixed.",
        ...
        "interpretation": (
            "High 20-day trend persistence is tested as a broad-market paper "
            "allocation field layered after the accepted price-floor, rank, "
            "low-extension, and high-volatility stack. This remains default-off "
            "paper observation until Gate 4 and shared-policy parity are satisfied."
        ),
        "rejection_reason": None if accepted else "Best trend-persistence support variant failed Gate 4.",
        "next_evidence_needed": (
            "Collect forward broad-market paper outcomes with trend-persistence "
            "metadata; do not retry adjacent positive-day-ratio thresholds without new "
            "forward outcomes or a distinct quality/risk field."
        ),
        ...
        "known_risks": [
            ...
            "The adjusted cohort is still small; forward paper evidence remains required before any trade-enabled adapter.",
            "Trend-persistence support can overpay mature leadership if promoted beyond default-off paper without a separate gate.",
        ],
        "related_files": {

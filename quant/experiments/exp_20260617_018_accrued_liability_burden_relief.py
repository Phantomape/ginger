"""exp-20260617-018: accrued liability burden relief scout.

Replay-only alpha search. The single decision hypothesis is a PIT free SEC
Companyfacts candidate source: production-universe names whose accrued current
operating liability burden is falling versus revenue may have easing operating
overhang or cleaner expense recognition. Liquid SPY-relative leadership then
tests whether price is starting to recognize that relief before a next-open,
...
STEM = "accrued_liability_burden_relief"
...
OUT_JSON = OUT_DIR / f"exp_20260617_018_{STEM}.json"
...
MAX_ACCRUED_FACT_AGE_DAYS = 430
MIN_CURRENT_REVENUE = 250_000_000.0
MIN_PRIOR_ACCRUED_LIABILITY = 25_000_000.0
MIN_PRIOR_ACCRUED_TO_REVENUE = 0.02
MAX_CURRENT_ACCRUED_TO_REVENUE = 0.35
MIN_ACCRUED_RATIO_RELIEF = 0.01
MIN_ACCRUED_RELIEF_PCT = 0.08
MIN_REVENUE_GROWTH = 0.0
MIN_PERIOD_GAP_DAYS = 250
MAX_PERIOD_GAP_DAYS = 460

ACCRUED_LIABILITY_TAGS = (
...
PREDICTION = {
    ...
    "confidence_reason": (
        "Official SEC accrued-liability burden relief is a free PIT operating-"
        "overhang field distinct from receivables, inventory, lease, debt, "
        "SBC, AOCI, and warranty tests, but recent Companyfacts burden scouts "
        "usually failed old_thin/drawdown/comparator gates."
    ),
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC accrued-liability or annual revenue facts, missing "
            "prior comparison point, stale facts, missing CIK mapping, missing "
    ...
    "parity_note": (
        ...
        "SEC accrued-liability tag mapping, filed-date PIT annual revenue "
        "normalization, liquid SPY-relative confirmation, cooldown, next-open "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts accrued/current operating "
        "liability burden falling versus revenue may identify companies with "
        "easing operating overhang or cleaner expense recognition; liquid "
        "SPY-relative leadership then tests whether price is recognizing that "
        "relief before a 10-trading-day continuation leg."
    ),
    "2_history_check": {
        "exp-20260616-021": (
            "Rejected receivables DSO collection improvement. This run avoids "
            "customer-collection speed and tests accrued operating liability "
            "burden relief."
        ),
        "exp-20260616-023": (
            "Rejected allowance for doubtful accounts quality. This run is not "
            "credit-loss reserve quality."
        ),
        "exp-20260616-025": (
            "Rejected operating lease burden relief. This run avoids lease "
            "fixed-cost obligations and tests current accrued operating "
            "liability overhang."
        ),
        "exp-20260616-029": (
            "Rejected principal debt burden relief despite high aggregate EV "
            "because old_thin and drawdown failed. This run excludes financial "
            "debt and interest fields."
        ),
        "exp-20260617-001": (
            "Rejected accounts-payable DPO extension. This run excludes "
            "AccountsPayable and AccountsPayableAndAccruedLiabilities tags to "
            "avoid retesting supplier-financing DPO."
        ),
        "history_scan": (
            "No prior committed experiment_log or experiments/logs entry used "
            "AccruedLiabilitiesCurrent / OtherAccruedLiabilitiesCurrent burden "
            "relief as the decision field."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_018_accrued_liability_burden_relief.py"
    ),
}
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        accrued_facts = _raw_instant_facts(usgaap, ACCRUED_LIABILITY_TAGS)
        revenue_facts = rd._raw_annual_facts(usgaap, REVENUE_TAGS)
        if not accrued_facts:
            stats["tickers_missing_raw_accrued_liability"] += 1
            continue
        if not revenue_facts:
            stats["tickers_missing_raw_annual_revenue"] += 1
            continue
        index[ticker] = {"accrued": accrued_facts, "revenue": revenue_facts}
        stats["tickers_with_raw_accrued_revenue"] += 1
        stats["raw_accrued_liability_fact_count"] += len(accrued_facts)
        stats["raw_annual_revenue_fact_count"] += len(revenue_facts)
        for fact in accrued_facts:
            tag_counts[str(fact["tag"])] += 1

    summary = {
        "raw_companyfacts_cache": _repo_rel(RAW_COMPANYFACTS_CACHE),
        "accrued_liability_tags": list(ACCRUED_LIABILITY_TAGS),
        "revenue_tags": list(REVENUE_TAGS),
        "warehouse_source": _repo_rel(base.framework.WAREHOUSE),
        "raw_accrued_liability_tag_counts": dict(tag_counts),
        **dict(stats),
    }
...
def _accrued_relief_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    current = _latest_accrued_fact(facts["accrued"], asof=asof)
    if current is None:
        return None
    if base._days_between(asof, current["filed"]) > MAX_ACCRUED_FACT_AGE_DAYS:
        return None
    prior = _latest_accrued_fact(facts["accrued"], asof=asof, before_end=current["end"])
    if prior is None:
        return None
    ...
    current_accrued = abs(float(current["value"]))
    prior_accrued = abs(float(prior["value"]))
    current_revenue_value = float(current_revenue["value"])
    prior_revenue_value = float(prior_revenue["value"])
    if (
        prior_accrued < MIN_PRIOR_ACCRUED_LIABILITY
        or current_revenue_value < MIN_CURRENT_REVENUE
        or prior_revenue_value <= 0.0
    ):
        return None

    current_ratio = current_accrued / current_revenue_value
    prior_ratio = prior_accrued / prior_revenue_value
    ratio_relief = prior_ratio - current_ratio
    relief_pct = (prior_accrued - current_accrued) / prior_accrued
    revenue_growth = current_revenue_value / prior_revenue_value - 1.0

    if prior_ratio < MIN_PRIOR_ACCRUED_TO_REVENUE:
        return None
    if current_ratio > MAX_CURRENT_ACCRUED_TO_REVENUE:
        return None
    if ratio_relief < MIN_ACCRUED_RATIO_RELIEF:
        return None
    if relief_pct < MIN_ACCRUED_RELIEF_PCT:
        return None
    ...
    return {
        ...
        "current_accrued_filed": current["filed"],
        "prior_accrued_filed": prior["filed"],
        "current_accrued_tag": current["tag"],
        "prior_accrued_tag": prior["tag"],
        "current_accrued_liability": _round(current_accrued, 2),
        "prior_accrued_liability": _round(prior_accrued, 2),
        ...
        "current_accrued_to_revenue": _round(current_ratio, 6),
        "prior_accrued_to_revenue": _round(prior_ratio, 6),
        "accrued_ratio_relief": _round(ratio_relief, 6),
        "accrued_relief_pct": _round(relief_pct, 6),
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _accrued_relief_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_accrued_relief_gate"] += 1
                continue
            ...
            score = (
                1.10 * min(float(quality["accrued_relief_pct"] or 0.0), 1.0)
                + 7.50 * min(float(quality["accrued_ratio_relief"] or 0.0), 0.12)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_ACCRUED_LIABILITY_BURDEN_RELIEF_PAPER",
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["quality_accrued_ratio_relief"] or 0.0),
            -float(row["quality_accrued_relief_pct"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        "rule_version": RULE_VERSION,
        "max_accrued_fact_age_days": MAX_ACCRUED_FACT_AGE_DAYS,
        "min_current_revenue": MIN_CURRENT_REVENUE,
        "min_prior_accrued_liability": MIN_PRIOR_ACCRUED_LIABILITY,
        "min_prior_accrued_to_revenue": MIN_PRIOR_ACCRUED_TO_REVENUE,
        "max_current_accrued_to_revenue": MAX_CURRENT_ACCRUED_TO_REVENUE,
        "min_accrued_ratio_relief": MIN_ACCRUED_RATIO_RELIEF,
        "min_accrued_relief_pct": MIN_ACCRUED_RELIEF_PCT,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_accrued_liability_burden_relief"
        if gate["passed"]
        else "rejected_accrued_liability_burden_relief_candidate_pool"
    )
    return gate
...
def _configure_base() -> None:
    ...
    base.MAX_ANNUAL_FACT_AGE_DAYS = MAX_ACCRUED_FACT_AGE_DAYS
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The accrued liability burden relief source cleared the numeric "
        ...
        interpretation = (
            "The accrued liability burden relief source did not clear Gate 4 "
            f"(failed: {', '.join(gate4['failed_reasons']) or 'none'}). "
            "It regressed "
            f"{', '.join(regressed_windows) or 'no individual window'}, and "
            "the broad liability burden signal appears too close to generic "
            "Companyfacts accrual/liability noise without a stronger event or "
            "contract provenance edge. It is not retained or promoted."
        )
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "AccruedLiabilitiesCurrent / OtherAccruedLiabilitiesCurrent instant "
        "facts are read from raw SEC Companyfacts and known only by filed date "
        "(<= signal date). Current and prior accrued-liability burdens are "
        "normalized by annual revenue filed by the signal date and period-ended "
        "no later than the accrued-liability period. Price confirmation uses "
    ...
    payload["gate2"]["runtime_fields"] = [
        "raw SEC companyfacts AccruedLiabilitiesCurrent instant facts",
        "raw SEC companyfacts OtherAccruedLiabilitiesCurrent instant facts",
    ...
    payload["next_evidence_needed"] = (
        "A retry needs materially different PIT evidence such as accrued-"
        "liability decomposition linked to named cost programs, litigation/"
        "restructuring resolution text, or closed forward replacement-value "
        "rows. Do not sweep accrued-liability tags, burden thresholds, fact "
        "freshness, RS/close/volume guards, top-N, hold, cooldown, or notional "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping accrued-liability tags, accrued/revenue "
            "thresholds, relief percentages, fact freshness, RS/close/volume/"
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Accrued Liability Burden Relief",
            "",

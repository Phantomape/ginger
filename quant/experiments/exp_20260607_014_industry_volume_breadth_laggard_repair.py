"""exp-20260607-014: industry volume-breadth laggard repair candidate pool.

Replay-only alpha search. It tests one free-OHLCV relation source:
when a strong liquid industry group shows broad signal-day volume participation,
select one liquid 20-day laggard that also repairs versus SPY and participates
in the volume thrust. Paper entry is next-open with a fixed 10-trading-day hold.
...
EXPERIMENT_ID = "exp-20260607-014"
STEM = "industry_volume_breadth_laggard_repair"
TRIAL_FAMILY = "industry_volume_breadth_laggard_repair_candidate_pool"
TRIAL_VARIANT_ID = "industry_volume_breadth_laggard_repair_top1_next_open_10d_v1"
CHANGED_VARIABLE = "industry_volume_breadth_laggard_repair_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = previous.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / "exp_20260607_014_industry_volume_breadth_laggard_repair.json"
...
MIN_GROUP_SIGNAL_MEDIAN_RETURN = 0.0025
MIN_GROUP_SIGNAL_POSITIVE_FRACTION = 0.58
MIN_GROUP_SIGNAL_RELATIVE_VS_SPY = 0.002
MIN_GROUP_MEDIAN_RET5_EXCESS_SPY = -0.020
MAX_GROUP_MEDIAN_RET5_EXCESS_SPY = 0.035
MIN_MEMBER_VOLUME_RATIO_FOR_BREADTH = 1.15
MIN_GROUP_VOLUME_BREADTH_FRACTION = 0.55
MIN_GROUP_HIGH_VOLUME_POSITIVE_FRACTION = 0.42
MIN_GROUP_MEDIAN_VOLUME_RATIO_20D = 1.05

MIN_REPAIR_LAG_20D = 0.020
MAX_REPAIR_LAG_20D = 0.180
...
PREDICTION = {
    ...
    "confidence_reason": (
        ...
        "replacement can carry signal, while price-breadth and "
        "dispersion-compression neighbors failed. This tests whether "
        "group-level volume participation is a distinct free-OHLCV flow field "
        "instead of another momentum relabel."
    ),
    "recorded_at": "2026-06-07T12:06:12Z",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "20-day group strength, signal-day volume breadth, high-volume "
        "positive participation, laggard repair fields, same-ticker "
        ...
        "sizing, watchlist, or order surface "
        "could change."
    ),
}
...
def _candidate_from_metrics(
    *,
    ...
    candidate_volume_excess = (
        metrics["volume_ratio_20d"] - group["median_volume_ratio_20d"]
    )
    ...
    score = (
        ...
        + 1.15 * group["volume_breadth_fraction"]
        + 1.05 * group["high_volume_positive_fraction"]
        + 0.70 * group["median_volume_ratio_20d"]
        + 0.80 * group["signal_positive_fraction"]
        + 1.35 * metrics["signal_relative_vs_spy"]
        + 0.72 * repair_lag_20d
        + 0.50 * metrics["close_location"]
        + 0.30 * metrics["volume_ratio_20d"]
        ...
        - 0.08 * max(metrics["volume_ratio_20d"] - 2.2, 0.0)
    )
    return {
        "date": metrics["date"],
        "ticker": metrics["ticker"],
        "source": "INDUSTRY_VOLUME_BREADTH_LAGGARD_REPAIR_PAPER",
        ...
        "candidate_volume_excess_vs_group_median": round(
            candidate_volume_excess,
            6,
        ),
        ...
        "industry_volume_breadth_context": {
            ...
            "median_volume_ratio_20d": round(group["median_volume_ratio_20d"], 6),
            "volume_breadth_fraction": round(group["volume_breadth_fraction"], 6),
            "high_volume_positive_fraction": round(
                group["high_volume_positive_fraction"],
                6,
            ),
            "min_member_volume_ratio_for_breadth": (
                MIN_MEMBER_VOLUME_RATIO_FOR_BREADTH
            ),
...
def _candidate_rows_for_window(
    *,
    ...
    context_scan = {
        ...
        "days_with_volume_breadth_groups": 0,
        "days_with_raw_candidates": 0,
        "volume_breadth_group_rows": 0,
    ...
    for signal_date in dates:
        ...
        for group_key, rows in group_members.items():
            ...
            volume_values = [float(row["volume_ratio_20d"]) for row in rows]
            ...
            volume_breadth_fraction = sum(
                value >= MIN_MEMBER_VOLUME_RATIO_FOR_BREADTH
                for value in volume_values
            ) / len(volume_values)
            high_volume_positive_fraction = sum(
                float(row["volume_ratio_20d"]) >= MIN_MEMBER_VOLUME_RATIO_FOR_BREADTH
                and float(row["signal_return"]) > 0.0
                for row in rows
            ) / len(rows)
            ...
            group_median_volume = median(volume_values)
            ...
            if group_median_signal_rel < MIN_GROUP_SIGNAL_RELATIVE_VS_SPY:
                continue
            if group_median_volume < MIN_GROUP_MEDIAN_VOLUME_RATIO_20D:
                continue
            if volume_breadth_fraction < MIN_GROUP_VOLUME_BREADTH_FRACTION:
                continue
            if high_volume_positive_fraction < MIN_GROUP_HIGH_VOLUME_POSITIVE_FRACTION:
                continue
            group_summaries[group_key] = {
                ...
                "median_volume_ratio_20d": group_median_volume,
                "volume_breadth_fraction": volume_breadth_fraction,
                "high_volume_positive_fraction": high_volume_positive_fraction,
            }

        if not group_summaries:
            continue
        context_scan["days_with_volume_breadth_groups"] += 1
        context_scan["volume_breadth_group_rows"] += len(group_summaries)
        ...
        day_contexts.append(
            {
                ...
                "volume_breadth_group_count": len(group_summaries),
                ...
                "top_volume_ratio_20d": top["candidate_volume_ratio_20d"],
                "top_volume_excess_vs_group_median": top[
                    "candidate_volume_excess_vs_group_median"
                ],
    ...
    context_scan.update(
        {
            ...
            "min_group_volume_breadth_fraction": MIN_GROUP_VOLUME_BREADTH_FRACTION,
            "min_group_high_volume_positive_fraction": (
                MIN_GROUP_HIGH_VOLUME_POSITIVE_FRACTION
            ),
            "min_group_median_volume_ratio_20d": MIN_GROUP_MEDIAN_VOLUME_RATIO_20D,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_industry_volume_breadth_laggard_repair"
        if gate["passed"]
        else "rejected_industry_volume_breadth_laggard_repair_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "In strong liquid industry groups, broad signal-day volume "
                "participation may identify real fund-flow diffusion. The "
                "best replacement candidate should be a 20-day laggard that "
                "also repairs versus SPY and participates in the volume thrust."
            ),
            ...
            "new_evidence_type": "free_ohlcv_industry_volume_breadth_relation",
            ...
            "interpretation": (
                "The industry volume-breadth laggard repair source cleared "
                "Gate 4 as a replay-only/default-off lead, but no production "
                "surface was promoted."
                if gate4["passed"]
                else (
                    "The industry volume-breadth laggard repair source did "
                    "not clear Gate 4. Do not promote it or answer by "
                    "retuning group volume breadth, candidate volume ratio, "
                    "signal-day repair, lag, close-location, hold-day, "
                    "cooldown, top-N, or notional thresholds on these frozen "
                    "windows."
                )
            ),
            "negative_reflection": (
                "If rejected, the likely reason is that group-level volume "
                "breadth still behaves like a crowded momentum chase or a "
                "near-neighbor of accepted laggard repair rather than a "
                "distinct replacement edge."
            ),
            "post_run_reflection": {
                "why_result_happened": (
                ...
                "forbidden_near_neighbor_retry": (
                    "Do not retry by sweeping group volume breadth, member "
                    "volume-ratio threshold, high-volume positive fraction, "
                    "candidate volume ratio, signal-day group repair, lag, "
                    "signal relative strength, close-location, volatility, "
            ...
            "next_evidence_needed": (
                ...
                "required before retrying near-neighbor industry volume "
                "breadth repair variants."
            ),
            "related_files": [
    ...
    if gate4["passed"]:
        payload["post_run_reflection"]["why_result_happened"] = (
            ...
            "concentration guardrails. That would suggest group-level volume "
            "breadth isolated real industry fund-flow diffusion."
        )
    else:
        payload["post_run_reflection"]["why_result_happened"] = (
            ...
            "gates. That means group-level volume breadth did not reliably "
            "separate real fund-flow diffusion from noisy catch-up under "
            "existing costs, slippage, and 10-day exit assumptions."
        )
    ...
    payload["backtest_protocol"]["execution_model"] = (
        ...
        "strength, 5-day extension control, same-day industry repair, "
        "group-level signal-day volume breadth, 60-day trend guard, ADV, "
        "candidate volume ratio, and realized volatility. Paper entry is "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: broad industry volume participation may "
            "identify real fund-flow diffusion before the lagging participant "
            "fully catches up."
        ),
        "2_history_check": {
            "exp-20260606-015": (
                ...
                "and requires group-level volume breadth plus laggard repair."
            ),
            "exp-20260607-007/008": (
                "Industry laggard repair was accepted; this is a scout for a "
                "different relation requiring industry-wide volume "
                "participation, not a retune of that shared helper."
            ),
            "exp-20260607-009": (
            ...
            "exp-20260607-010": (
                ...
                "adds a distinct volume participation field rather than "
                "sweeping the price-breadth threshold."
            ),
            "exp-20260607-012": (
                "Dispersion-compression repair regressed old_thin and "
                "drawdown; this removes the prior-dispersion condition and "
                "tests volume breadth as the independent relation."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260607_014_industry_volume_breadth_laggard_repair.py"
        ),
    }
    payload["pre_run_questions"] = payload["gate_questions"]
    payload["gate3"]["note"] = (
        "No new core filter or entry rule was added. The industry "
        "volume-breadth laggard repair source is additive default-off paper, "
        "so core signals generated/survived are unchanged from baseline."
    )
    payload["context_alias"] = "industry_volume_breadth_contexts"
    payload["industry_volume_breadth_context_samples_by_window"] = {
        label: rows[:20]
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Industry Volume-Breadth Laggard Repair",
            "",

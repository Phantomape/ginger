"""exp-20260528-023: Companyfacts+RS operating-margin durability support.
...
paper candidates whose PIT operating margin is non-declining versus the
...
STEM = "fundamental_growth_rs_operating_margin_durability"
TRIAL_FAMILY = "fundamental_growth_rs_operating_margin_durability_support"
CHANGED_VARIABLE = "fundamental_growth_rs_operating_margin_durability_notional_scalar_v1"
...
OPERATING_MARGIN_AUDIT: "OrderedDict[str, dict[str, Any]]" = OrderedDict()
...
def _configure_modules() -> None:
    ...
    OPERATING_MARGIN_AUDIT.clear()
    prev._configure_modules()
    _base()._select_paper_trades = _select_operating_margin_supported_paper_trades
...
class CompanyfactsOperatingMarginIndex:
    ...
    def current_context(self, ticker: str, asof_date: str) -> dict[str, Any]:
        ticker = ticker.upper()
        operating_income = self.latest_fact(ticker, "operating_income", asof_date)
        if operating_income is None:
            return {
                "operating_margin_status": "missing_current_operating_income",
                "operating_margin_known_at": "SEC Companyfacts filed date <= signal_date",
                "operating_margin_durability_pass_v1": False,
            }
        revenue = self.matching_fact(
            ticker,
            "revenue",
            asof_date,
            end=str(operating_income.get("end") or ""),
        )
        if revenue is None:
            return {
                "operating_margin_status": "missing_current_revenue_same_period",
                "operating_margin_known_at": "SEC Companyfacts filed date <= signal_date",
                "operating_margin_durability_pass_v1": False,
                "operating_income_current_value": _round(operating_income.get("value"), 6),
                "operating_income_current_filed": operating_income.get("filed"),
                "operating_income_current_period_end": operating_income.get("end"),
            }
        operating_income_value = _float(operating_income.get("value"))
        revenue_value = _float(revenue.get("value"))
        if (
            operating_income_value is None
            or revenue_value is None
            or revenue_value <= 0.0
        ):
            return {
                "operating_margin_status": "invalid_current_margin_denominator",
                "operating_margin_known_at": "SEC Companyfacts filed date <= signal_date",
                "operating_margin_durability_pass_v1": False,
                "operating_income_current_value": _round(operating_income_value, 6),
                "operating_income_current_filed": operating_income.get("filed"),
                "operating_income_current_period_end": operating_income.get("end"),
                "operating_margin_revenue_current_value": _round(revenue_value, 6),
                "operating_margin_revenue_current_filed": revenue.get("filed"),
                "operating_margin_revenue_current_period_end": revenue.get("end"),
            }

        fy = operating_income.get("fy_int")
        fp = operating_income.get("fp_norm")
        prior_operating_income = None
        if fy is not None and fp:
            prior_operating_income = self.matching_fact(
                ticker,
                "operating_income",
                asof_date,
                fy=int(fy) - 1,
        ...
        prior_operating_income_value = (
            _float(prior_operating_income.get("value")) if prior_operating_income else None
        )
        if prior_operating_income is None or prior_operating_income_value is None:
            return {
                "operating_margin_status": "missing_prior_operating_income_same_quarter",
                "operating_margin_known_at": "SEC Companyfacts filed date <= signal_date",
                "operating_margin_durability_pass_v1": False,
                "operating_income_current_value": _round(operating_income_value, 6),
                "operating_income_current_filed": operating_income.get("filed"),
        ...
        prior_revenue = self.matching_fact(
            ...
            "revenue",
            asof_date,
            end=str(prior_operating_income.get("end") or ""),
        )
        ...
        if prior_revenue_value is None or prior_revenue_value <= 0.0:
            return {
                "operating_margin_status": "missing_prior_revenue_same_quarter",
                "operating_margin_known_at": "SEC Companyfacts filed date <= signal_date",
                "operating_margin_durability_pass_v1": False,
                "operating_income_current_value": _round(operating_income_value, 6),
                "operating_income_current_filed": operating_income.get("filed"),
                ...
                "operating_margin_revenue_current_filed": revenue.get("filed"),
                "operating_margin_revenue_current_period_end": revenue.get("end"),
                "operating_income_prior_value": _round(prior_operating_income_value, 6),
                "operating_income_prior_filed": prior_operating_income.get("filed"),
                "operating_income_prior_period_end": prior_operating_income.get("end"),
            }

        current_margin = operating_income_value / revenue_value
        prior_margin = prior_operating_income_value / prior_revenue_value
        margin_delta = current_margin - prior_margin
        passed = margin_delta >= MIN_OPERATING_MARGIN_YOY_DELTA
        status = "ok" if passed else "operating_margin_declined"
        return {
            "operating_margin_status": status,
            "operating_margin_known_at": "SEC Companyfacts filed date <= signal_date",
            "operating_margin_durability_pass_v1": passed,
            ...
            "operating_margin_revenue_current_filed": revenue.get("filed"),
            "operating_margin_revenue_current_period_end": revenue.get("end"),
            "operating_margin_revenue_prior_value": _round(prior_revenue_value, 6),
            "operating_margin_revenue_prior_filed": prior_revenue.get("filed"),
            "operating_margin_revenue_prior_period_end": prior_revenue.get("end"),
            "operating_margin_current": _round(current_margin, 6),
            "operating_margin_prior": _round(prior_margin, 6),
            "operating_margin_yoy_delta": _round(margin_delta, 6),
        }


def _operating_margin_index_for_candidates(
    candidates: list[dict[str, Any]],
) -> CompanyfactsOperatingMarginIndex:
    ...
    return CompanyfactsOperatingMarginIndex(rows)


def _operating_margin_bucket(context: dict[str, Any]) -> str:
    if not context.get("operating_margin_durability_pass_v1"):
        return str(context.get("operating_margin_status") or "failed")
    delta = _float(context.get("operating_margin_yoy_delta"))
    if delta is None:
        return "missing_delta"
    if delta <= 0.02:
        return "stable_0_to_2pp"
    if delta <= 0.10:
        return "improved_2_to_10pp"
    return "improved_gt_10pp"


def _operating_margin_scalar(context: dict[str, Any]) -> float:
    if context.get("operating_margin_durability_pass_v1"):
        return OPERATING_MARGIN_NOTIONAL_SCALAR
    return 1.0


def _select_operating_margin_supported_paper_trades(
    ...
    operating_margin_index = _operating_margin_index_for_candidates(candidates)
    ...
    for row in candidates:
        ...
        operating_margin_context = operating_margin_index.current_context(ticker, signal_date)
        operating_margin_scalar = _operating_margin_scalar(operating_margin_context)
        notional_scalar_before_operating_margin = (
            ticker_profit_scalar
            * global_drawdown_scalar
            * low_volume_scalar
        ...
        notional_scalar = notional_scalar_before_operating_margin * operating_margin_scalar
        ...
        operating_margin_bucket = _operating_margin_bucket(operating_margin_context)
        bucket_counts[operating_margin_bucket] += 1
        status_counts[str(operating_margin_context.get("operating_margin_status") or "unknown")] += 1
        ...
        if operating_margin_scalar > 1.0:
            audit["operating_margin_supported"] += 1
        ...
        pre_operating_margin_notional = base_notional * notional_scalar_before_operating_margin
        supported_notional = base_notional * notional_scalar
        pre_operating_margin_pnl = pre_operating_margin_notional * pnl_pct_net
        supported_pnl = supported_notional * pnl_pct_net
        if operating_margin_scalar > 1.0:
            support_pnl_delta_by_ticker[ticker] += supported_pnl - pre_operating_margin_pnl
        ...
        supported_trade = {
            ...
            **balance_context,
            "operating_margin_durability_rule_version": RULE_VERSION,
            "operating_margin_durability_known_at": "SEC Companyfacts operating_income/revenue filed date <= signal_date",
            "operating_margin_durability_trade_enabled": False,
            "operating_margin_durability_alters_orders": False,
            "operating_margin_durability_bucket": operating_margin_bucket,
            "operating_margin_durability_notional_scalar": operating_margin_scalar,
            **operating_margin_context,
            ...
            "pnl_without_operating_margin_durability_support": _round(pre_operating_margin_pnl, 2),
            "operating_margin_durability_support_pnl_delta": _round(
                supported_pnl - pre_operating_margin_pnl,
                2,
            ),
    ...
    OPERATING_MARGIN_AUDIT[label] = {
        "rule_version": RULE_VERSION,
        "min_operating_margin_yoy_delta": MIN_OPERATING_MARGIN_YOY_DELTA,
        "operating_margin_notional_scalar": OPERATING_MARGIN_NOTIONAL_SCALAR,
        "input_candidates": len(candidates),
        "selected_trades": len(selected),
        "filtered_candidates": len(filtered),
        "operating_margin_supported": int(audit.get("operating_margin_supported", 0)),
        ...
        "operating_margin_status_counts": dict(sorted(status_counts.items())),
        "operating_margin_bucket_counts": dict(sorted(bucket_counts.items())),
        "operating_margin_supported_ticker_counts": dict(sorted(supported_ticker_counts.items())),
...
def _update_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = (
        "accepted_candidate_fundamental_growth_rs_operating_margin_durability_support"
        if gate4_passed
        else "rejected_fundamental_growth_rs_operating_margin_durability_support"
    )
    payload["status"] = decision
    payload["decision"] = decision
    payload["hypothesis"] = (
        "candidate_pool / capital allocation: within the accepted Companyfacts "
        "operating-profit + RS paper source, non-declining operating margin "
        "versus the prior-year same quarter is a free SEC quality field. "
        "A small default-off paper notional support should favor growth where "
        "profitability durability is confirmed at the same time as price strength."
    )
    payload["change_type"] = "fundamental_growth_rs_operating_margin_durability_default_off_paper_sleeve"
    ...
    payload["new_evidence_type"] = "production_visible_sec_companyfacts_operating_margin_quality_field"
    payload["parameters"]["operating_margin_durability_support"] = {
        "min_operating_margin_yoy_delta": MIN_OPERATING_MARGIN_YOY_DELTA,
        "paper_notional_scalar": OPERATING_MARGIN_NOTIONAL_SCALAR,
        "applies_to": (
        ...
        "production_visible_field": (
            "latest and prior-year same-quarter operating_income/revenue from SEC "
            "Companyfacts with filed <= signal_date"
        ),
    }
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "candidate_pool / capital allocation alpha: PIT operating-margin "
            "durability is a distinct free SEC quality field for the accepted "
        ...
        "2_history_check": {
            ...
            "exp-20260528-020": "Dual-growth support was rejected versus the accepted exp017 stack.",
            "exp-20260528-021": "Liquidity sweet-spot support was rejected versus the accepted exp017 stack.",
            "current_difference": (
                "This run does not retune growth, RS, cash-flow, volume, "
                "filing-age, liabilities/assets, top-N, hold, or governor thresholds; "
                "it tests operating-margin durability as a same-quarter YoY quality field."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260528_023_fundamental_growth_rs_operating_margin_durability.py"
        ),
    }
    ...
    payload["gate2"]["note"] = (
        "The only new field is operating-margin durability, computed from "
        "Companyfacts operating_income and revenue facts with filed dates "
    ...
    payload["operating_margin_audit"] = OPERATING_MARGIN_AUDIT
    ...
    payload["why_not_other_changes"] = (
        ...
        "This run tests only a new SEC operating-margin durability field on the accepted "
        "Companyfacts+RS paper source."
    )
    payload["interpretation"] = (
        "The operating-margin durability support cleared Gate 4 on top of the "
        ...
        if gate4_passed
            ...
            "The operating-margin durability support did not clear Gate 4. Do not "
            "promote it or retry nearby operating-margin thresholds or scalars "
    ...
    payload["production_impact"].update(
        {
            ...
            "promotion_requirement": (
                ...
                if gate4_passed
                    ...
                    "A retained positive result would require the same operating-margin "
...
def _build_report(payload: dict[str, Any]) -> str:
    lines = [
        ...
        (
            ...
            "PIT operating margin is non-declining versus the prior-year same quarter."
        ),
        "",
        "## Three-Window Result",
        "",
    ...
    for label in _base().WINDOWS:
        ...
        audit = payload["operating_margin_audit"].get(label, {})
        lines.append(
            ...
            "${apnl:,.2f} | ${dpnl:+,.2f} | {dd:+.4f} | {trades} | {supported} |".format(
                ...
                supported=audit.get("operating_margin_supported"),
            )
        )
    ...
    lines.extend(
        [
            ...
            "",
            "```json",
            json.dumps(payload["operating_margin_audit"], indent=2, sort_keys=True),
            "```",
            "",
...
def _persist(payload: dict[str, Any]) -> None:
    ticket = {
        ...
        "title": "Fundamental growth + RS operating-margin durability",
            ...
            _base()._safe(
                {
                    ...
                    "gate4": payload["gate4"],
                    "operating_margin_audit": payload["operating_margin_audit"],

"""exp-20260618-009: SEC 8-K Item 4.02 non-reliance absorption scout.

Replay-only alpha search. The single decision hypothesis is that PIT SEC
8-K Item 4.02 non-reliance/restatement events can become positive candidate-pool
events when signal-day price action absorbs the accounting-risk announcement versus
...
EXPERIMENT_ID = "exp-20260618-009"
STEM = "sec_8k_item402_nonreliance_absorption"
TRIAL_FAMILY = "free_sec_submissions_8k_nonreliance_absorption_candidate_pool"
TRIAL_VARIANT_ID = "sec_8k_item402_nonreliance_absorption_top1_next_open_10d_v1"
CHANGED_VARIABLE = "sec_8k_item_402_nonreliance_absorption_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260618_009_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "8-K Item 4.02 is a free PIT SEC submissions field with clearer "
        "economics than broad 8-K labels and enough cache coverage, but recent "
        "SEC event absorption scouts failed and non-reliance/restatement announcements may "
        "reflect real distress rather than forced-seller exhaustion."
    ),
    "recorded_at": "2026-06-18T07:07:13+00:00",
}
...
PRODUCTION_IMPACT = {
    **runner.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **runner.PRODUCTION_IMPACT["execution_envelope"],
        ...
        "failure_handling": (
            ...
            "8-K Item 4.02 event rows, missing OHLCV, missing next open, or "
    ...
    "parity_note": (
        ...
        "SEC 8-K Item 4.02 events, acceptance-time signal date, price-absorption "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: PIT SEC 8-K Item 4.02 non-reliance/restatement events are "
        "normally negative, but when signal-day price action absorbs the "
        "announcement versus SPY, the accounting-credibility shock absorption information "
    ...
    "2_history_check": {
        "exp-20260610-013": (
            ...
            "uses one structured item-code field with explicit nonreliance "
            "accounting-credibility shock economics, not broad 8-K item-code "
            "or phrase labels."
        ),
        "exp-20260529-014": (
            "Rejected 10-K liquidity relative-strength candidate pool left "
            "restatement/disclosure-quality as future evidence. This run uses "
            "PIT 8-K Item 4.02 event timing rather than annual 10-K liquidity."
        ),
        "exp-20260612-025": (
            "Rejected auditor-change absorption and required richer "
            "restatement linkage for that family. This run observes the "
            "structured non-reliance/restatement item itself."
        ),
        "exp-20260617-023": (
        ...
        "exp-20260617-025": (
            "Rejected NT late-filing notice absorption. This run uses accounting-risk "
            "8-K non-reliance/restatement items, not disclosure-delay notices."
        ),
        "exp-20260617-027": (
            "Rejected non-management proxy-pressure absorption. This run uses "
            "issuer 8-K non-reliance/restatement provenance, not proxy solicitation "
    ...
    "5_reproducibility": (
        ...
        "exp_20260618_009_sec_8k_item402_nonreliance_absorption.py"
    ),
}
...
def _has_item_402(raw: Any) -> bool:
...
def _load_event_index() -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        for i in range(min(len(forms), len(filing_dates))):
            ...
            if not _has_item_402(item_text):
                continue
        ...
        if events:
            ...
            index[ticker] = events
            stats["tickers_with_item402_events"] += 1
            stats["item402_event_count"] += len(events)
...
def _build_quality_index(
    ...
    return index, {
        **summary,
        ...
        "field_source": "sec_submissions_8k_item402_not_companyfacts",
    }
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "SEC_8K_ITEM402_NONRELIANCE_ABSORPTION_PAPER",
                    "candidate_score": runner._round(score, 6),
                    "rule_version": RULE_VERSION,
                    "source_rule_version": RULE_VERSION,
                    "known_at": "sec_8k_item402_and_signal_close_before_next_open_paper_entry",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    "item402_form": event["form"],
                    "item402_items": event["items"],
                    "item402_filing_date": event["filing_date"],
                    "item402_accepted_after_close": event["accepted_after_close"],
                    "item402_acceptance_datetime": event["acceptance_datetime"],
                    "item402_accession_number": event["accession_number"],
                    "item402_primary_document": event["primary_document"],
                    "item402_event_weight": event["event_weight"],
                    **confirm,
                }
            )
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_sec_8k_item402_nonreliance_absorption"
        if gate["passed"]
        else "rejected_sec_8k_item402_nonreliance_absorption_candidate_pool"
    )
    return gate
...
def _interpretation(payload: dict[str, Any]) -> str:
    ...
    if gate4["passed"]:
        return (
            "The SEC 8-K Item 4.02 non-reliance absorption source cleared the "
            "numeric three-window replay screen, but remains only a replay lead "
            "because no shared daily/backtest helper was promoted."
        )
    return (
        "The SEC 8-K Item 4.02 non-reliance absorption source did not clear "
        f"Gate 4 (failed: {', '.join(gate4['failed_reasons']) or 'none'}). "
        "The fixed bundle tested structured Item 4.02 non-reliance/restatement 8-K "
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_submissions_restatement_nonreliance_candidate_pool",
            "new_evidence_type": "sec_submissions_8k_item402_nonreliance_with_price_absorption",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SEC non-reliance/restatement events are read from EDGAR submissions cache recent "
        "8-K filings where recent.items contains Item 4.02. The signal date is "
    ...
    payload["gate2"]["runtime_fields"] = [
        ...
        "SEC submissions recent.items containing 4.02",
    ...
    payload["next_evidence_needed"] = (
        "If this fixed Item 4.02 nonreliance absorption bundle fails, do not "
        ...
        "materially richer PIT nonreliance semantics such as restatement "
        "scope, non-reliance period, auditor involvement, control weakness "
        "language, correction magnitude, or closed forward replacement-value "
        "rows."
    )
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} SEC 8-K Item 4.02 Non-Reliance Absorption",
            "",

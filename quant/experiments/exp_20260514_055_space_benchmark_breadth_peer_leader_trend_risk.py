...
benchmark confirmation deserve another small default-off top-up when the
...
EXPERIMENT_ID = "exp-20260514-055"
STEM = "space_benchmark_breadth_peer_leader_trend_risk"
...
MARKER = "space_benchmark_breadth_peer_leader_trend_risk"
    ...
    *,
    ...
    def patched_extra(
        *,
        ...
        if (
            ticker in benchmark_tickers
            and str(signal.get("space_peer_momentum_state") or "") == TARGET_PEER_STATE
        ):
            _scale_and_record(
                signal=signal,
                sizing=sizing,
                ...
                marker=MARKER,
                counts=counts,
                adjustments=peer_adjustments,
                profile=benchmark_profiles.get(ticker),
            )
            signal["space_benchmark_breadth_peer_leader_trend_bucket"] = True
            signal["space_benchmark_breadth_peer_leader_trend_scalar"] = (
                peer_leader_scalar
            )
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_benchmark_breadth_peer_leader_trend_scalar": peer_leader_scalar,
        "target_strategy": TARGET_STRATEGY,
        "target_peer_momentum_state": TARGET_PEER_STATE,
        "benchmark_breadth_target_tickers": sorted(benchmark_tickers),
    }
    variant["benchmark_breadth_peer_leader_counts"] = peer_counts
    variant["benchmark_breadth_peer_leader_counts_by_window"] = by_window_counts
    variant["benchmark_breadth_peer_leader_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(peer_adjustments)
    )
    variant["benchmark_breadth_peer_leader_adjustment_sample"] = peer_adjustments[:25]
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("benchmark_breadth_peer_leader_counts") or {}
    changed_count = int(counts.get(f"{MARKER}_changed_signal", 0))
    eligible_count = int(counts.get(f"{MARKER}_eligible_signal", 0))
    scalar = float(variant["parameters"]["space_benchmark_breadth_peer_leader_trend_scalar"])
    passed = bool(
        scalar != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "eligible_peer_leader_signal_count": eligible_count,
        "changed_peer_leader_signal_count": changed_count,
        "reasons": {
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            ...
            "may deserve a small extra default-off allocation when the ticker "
            "leads the official Space peer basket."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_benchmark_breadth_peer_leader_trend_scalar",
        "parameters": {
            "scalars_tested": list(SCALARS),
            "selected_scalar": best["parameters"][
                "space_benchmark_breadth_peer_leader_trend_scalar"
            ],
            "target_strategy": TARGET_STRATEGY,
            "target_peer_momentum_state": TARGET_PEER_STATE,
            "accepted_before_experiment": BEFORE_EXPERIMENT_ID,
        ...
        if promoted
            ...
            "benchmark-breadth peer-quality discriminator."
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited, ticker expansion has already "
            "failed, and recent Space timing/source scalars are either accepted "
            "or blocked by sample. This keeps the candidate set fixed and tests "
            "one production-visible peer-state interaction."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space benchmark-breadth peer-leader trend risk",
        "",
        "## Hypothesis",
        (
            "Official Space `trend_long` signals with broad 10d confirmation "
            "versus cash, SPY, QQQ, UFO, and ARKX may deserve a small extra "
            "default-off allocation when the ticker leads the official Space "
            "peer basket."
        ),
        "",
        "## Single Changed Variable",
        (
            "`space_benchmark_breadth_peer_leader_trend_scalar` on top of the "
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_benchmark_breadth_peer_leader_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_peer_leader_signal_count']}`",
            f"- adjusted signals: `{gate['changed_peer_leader_signal_count']}`",
            f"- adjusted counts: `{best['benchmark_breadth_peer_leader_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            f"Benchmark-breadth peer-leader trend scalar "
            f"{best['parameters']['space_benchmark_breadth_peer_leader_trend_scalar']} "
            f"changed {gate['changed_peer_leader_signal_count']} signals with "
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            "Benchmark-breadth Space trend signals with peer momentum leadership "
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate["eligible_peer_leader_signal_count"],
                    "adjusted_signals": gate["changed_peer_leader_signal_count"],

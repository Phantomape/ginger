"""exp-20260607-018: volatility-relief stock-leadership scout.

Replay-only alpha search. This tests one production-visible free-OHLCV
candidate-source variable: on days where VIXY sells off while SPY and QQQ
confirm risk relief, select the strongest liquid stock leaders as next-open,
...
STEM = "volatility_relief_stock_leadership"
TRIAL_FAMILY = "volatility_relief_stock_leadership_candidate_pool"
TRIAL_VARIANT_ID = "vixy_down_spy_qqq_confirmed_stock_leadership_top2_10d_v1"
CHANGED_VARIABLE = "volatility_relief_stock_leadership_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = previous.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260607_018_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "Accepted macro relief leadership shows risk-relief stock leadership "
        "can work; pressure/stress and broad momentum neighbors often failed, "
        "so VIXY relief is plausible but low probability and must pass all "
        "three windows."
    ),
    "recorded_at": "2026-06-07T16:08:33+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "volatility-relief context, sector-known liquid stock universe, "
...
def _candidate_for_ticker(
    *,
    ...
    if row is None:
        return None
    row["source"] = "VOLATILITY_RELIEF_LEADERSHIP_PAPER"
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_volatility_relief_stock_leadership"
        if gate["passed"]
        else "rejected_volatility_relief_stock_leadership_candidate_pool"
    )
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "VIXY volatility-relief days with SPY and QQQ confirmation "
            ...
            "new_evidence_type": "free_ohlcv_vixy_relief_cross_asset_state",
            ...
            "interpretation": (
                "The volatility-relief stock-leadership source cleared Gate 4 "
                "as a replay-only/default-off lead, but no production surface "
                "was promoted."
                if accepted
                else (
                    "The volatility-relief stock-leadership source did not "
                    "clear Gate 4. Do not promote it or answer by retuning "
                    "VIXY/SPY/QQQ thresholds, top-N, hold-day, cooldown, or "
                    "notional on these frozen windows."
                )
            ),
            "rejection_reason": None if accepted else "; ".join(gate4["failed_reasons"]),
            "negative_reflection": (
                "If rejected, the likely reason is that VIXY relief is still "
            ...
            "post_run_reflection": {
                "why_result_happened": (
                    ...
                    "concentration guardrails, suggesting volatility-premium "
                    ...
                    if accepted
                        ...
                        "relief did not reliably separate durable stock "
                ...
                "new_evidence_required": (
                    ...
                    "such as actual VIX term structure, options/realized-vol "
                    "compression, or closed forward replacement-value rows "
                    "from a shared default-off adapter."
                ),
            },
    ...
    payload["backtest_protocol"]["execution_model"] = (
        ...
        "VIXY, SPY, and QQQ daily return/range context plus stock leadership, "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: volatility-premium compression confirmed "
            "by SPY/QQQ relief may identify stock leaders before the next "
            "session fully reprices the risk-on move."
        ),
        "2_history_check": {
            "exp-20260605-033": (
                "Cross-section pressure resilience failed across all windows; "
                "this tests relief after volatility compression, not stress "
            ...
            "exp-20260606-019/020": (
                ...
                "cross-asset volatility state instead of event-calendar days."
            ),
            "exp-20260606-027": (
            ...
            "exp-20260606-004/005": (
                ...
                "requires explicit VIXY/SPY/QQQ relief state."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260607_018_volatility_relief_stock_leadership.py"
        ),
    }
    payload["pre_run_questions"] = payload["gate_questions"]
    payload["gate3"]["note"] = (
        "No new core filter or entry rule was added. The volatility-relief "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Volatility Relief Stock Leadership",
            "",
        ...
        "windows": [
            {
                ...
                "volatility_relief_day_count": payload["context_scan_by_window"][label].get(

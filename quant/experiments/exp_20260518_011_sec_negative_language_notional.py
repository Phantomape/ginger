"""exp-20260518-011: SEC negative-language paper-notional scalar.

Alpha search on one causal variable: a paper-notional scalar for covered
``negative_language`` SEC financial-report T+1 paper-sleeve rows, measured on
...
EXPERIMENT_ID = "exp-20260518-011"
STEM = "exp_20260518_011_sec_negative_language_notional"
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_negative_language_notional.md"
)
EXPERIMENT_LOG_JSONL = REPO_ROOT / "docs" / "experiment_log.jsonl"

ACCEPTED_NEUTRAL_UNDERREACTION_SCALAR = 2.0
ACCEPTED_NEUTRAL_UNDERREACTION_MAX_T1_EXCESS = 0.02
BASELINE_NEGATIVE_LANGUAGE_SCALAR = 1.0
NEGATIVE_LANGUAGE_SCALAR_VARIANTS = (0.0, 0.25, 0.50, 0.75, 1.0, 1.10, 1.25, 1.50)
...
def _t1_excess(candidate: dict[str, Any]) -> float | None:
    return _float_or_none(candidate.get("t1_excess_return_vs_spy"))
...
def _is_negative_language_position(position: dict[str, Any]) -> bool:
    candidate = parent._source_candidate(position)
    return (
...
def _custom_notional_for_position(
    ...
    *,
    negative_language_scalar: float,
    ...
    if _is_negative_language_position(position):
        combined_scalar *= float(negative_language_scalar)
        rules.append("negative_language_scalar")
    ...
    *,
    ...
    def patched_notional(position: dict[str, Any], *, neutral_language_scalar: float) -> tuple[float, float, str]:
        return _custom_notional_for_position(
            position,
            negative_language_scalar=neutral_language_scalar,
        )
    ...
    try:
        row = parent._run_variant(
            ...
            exp100=exp100,
            neutral_language_scalar=negative_language_scalar,
        )
    finally:
        parent._notional_for_position = original_notional
    row["negative_language_notional_scalar"] = negative_language_scalar
    return row
...
def _window_deltas(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    for label in parent.WINDOWS:
        ...
        checks[label] = {
            ...
            "negative_language_closed_trade_count": int(
                (after_sleeve.get("closed_trade_count_by_language_bucket") or {}).get(
                    "negative_language", 0
                )
            ),
            "negative_language_pnl_delta": _round(
                float(
...
def _variant_summary(row: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "negative_language_notional_scalar": row["negative_language_notional_scalar"],
    ...
    *,
    ...
    for label, window in parent.WINDOWS.items():
        ...
        for position in state.get("closed_positions") or []:
            if not _is_negative_language_position(position):
                continue
            candidate = parent._source_candidate(position)
            baseline_pnl = _pnl_for_position(
                position,
                negative_language_scalar=BASELINE_NEGATIVE_LANGUAGE_SCALAR,
                closed=True,
            )
            adjusted_pnl = _pnl_for_position(
                position,
                negative_language_scalar=negative_language_scalar,
                closed=True,
            )
            rows.append(
                {
...
def _gate(summary: dict[str, Any], selection: dict[str, Any]) -> dict[str, Any]:
    ...
    concentration_guard_passed = concentration is None or concentration <= MAX_SINGLE_POSITIVE_PNL_SHARE
    return {
        ...
        "rules": {
            "metric_gate": (
                "aggregate EV/PnL positive, all three windows EV positive, no "
                "window PnL regression, and max drawdown worsening <= 0.5pp"
            ),
            "sample_guard": {
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} SEC Negative-Language Notional",
        "",
        ...
        f"- negative_language_scalar: `{payload['parameters']['best_negative_language_scalar']}`",
    ...
    for label, row in payload["gate"]["by_window"].items():
        lines.append(
            "| {label} | {ev:+.4f} | ${pnl:+,.2f} | {dd:+.4f} | {count} | ${neg_pnl:+,.2f} |".format(
                label=label,
                ev=row.get("expected_value_score", 0.0),
                pnl=row.get("total_pnl", 0.0),
                dd=row.get("max_drawdown_pct", 0.0),
                count=row.get("negative_language_closed_trade_count", 0),
                neg_pnl=row.get("negative_language_pnl_delta", 0.0),
            )
        )
    lines.extend(
        [
...
def build_payload() -> dict[str, Any]:
    ...
    for scalar in NEGATIVE_LANGUAGE_SCALAR_VARIANTS:
        key = f"negative_language_scalar_{scalar:.2f}"
        row = _run_variant(
            core_results=core_results,
            exp100=exp100,
            negative_language_scalar=scalar,
        )
        variants[key] = row

    baseline_key = f"negative_language_scalar_{BASELINE_NEGATIVE_LANGUAGE_SCALAR:.2f}"
        ...
        if _gate(
            ...
            _selection_summary(
                _closed_positions_for_scalar(
                    exp100,
                    negative_language_scalar=summaries[key]["negative_language_notional_scalar"],
                )
            ),
        )["passed"]
    ]
    ...
    best_scalar = float(best_summary["negative_language_notional_scalar"])
    selection = _selection_summary(
        _closed_positions_for_scalar(exp100, negative_language_scalar=best_scalar)
    )
    gate = _gate(best_summary, selection)
    status = "accepted_candidate" if gate["passed"] else "rejected"
    decision = (
        "accepted_candidate_sec_negative_language_notional"
        if gate["passed"]
        else "rejected_sec_negative_language_notional"
    )
    interpretation = (
        "Covered negative-language SEC financial-report rows improved with a "
        ...
        if gate["passed"]
        else "Negative-language paper-notional scalars did not clear the three-window gate on top of exp-20260518-009."
    )
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "negative_language filings may have a distinct continuation profile "
        ...
        "changed_variable": "sec_financial_report_negative_language_notional_scalar",
        "single_causal_variable": "negative-language paper-notional scalar only",
        "parameters": {
            "baseline_negative_language_scalar": BASELINE_NEGATIVE_LANGUAGE_SCALAR,
            "negative_language_scalar_variants": list(NEGATIVE_LANGUAGE_SCALAR_VARIANTS),
            "best_negative_language_scalar": best_scalar,
            ...
            "negative_language_definition": (
                "sec_text_coverage_status == covered and language_bucket == negative_language"
            ),
        ...
        "next_evidence_needed": (
            ...
            if gate["passed"]
            else "Do not retry nearby negative-language notional scalars on this frozen sample without a new semantic field or forward replacement-value evidence."
        ),
        "production_impact": {
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "capital allocation: scale covered negative-language SEC financial-report "
            ...
            "2_history_check": (
                ...
                "exp-20260518-010 rejected capacity priority; guidance, cash-flow "
                "forecast, and operational-fact branches were rejected. No prior "
                "run isolated negative-language notional on top of the accepted "
                "neutral-underreaction stack."
            ),
            "3_single_causal_variable": "negative-language paper-notional scalar",
        ...
        "why_not_other_changes": (
            ...
            "low-deployment ETF threshold/pool work is already rejected; SEC "
            "capacity priority just failed. This tests one fresh production-visible "
            "semantic allocation field instead of adding ticker noise."
        ),
        "related_files": [

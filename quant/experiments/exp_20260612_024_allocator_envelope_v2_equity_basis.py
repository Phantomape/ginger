...
STEM = "allocator_envelope_v2_equity_basis"
...
OUT_JSON = OUT_DIR / f"exp_20260612_024_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "v1 false-triggered on profit giveback because 8pct of fixed bucket sits "
        "inside normal realized variation (healthy windows hit 6.6-10.9pct); the "
        "equity-peak basis at 15pct sits outside every observed healthy giveback. "
        "Caveat: threshold calibrated on the same frozen windows, so the in-sample "
        "no-false-trigger check is partially circular; binding validation is OOS "
        "plus forward rows. "
...
def _gate(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": (
            "accepted_allocator_activation_envelope_gate_v2"
            if passed
            else "rejected_allocator_envelope_v2"
        ),
    }
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Envelope v2 changes one specification versus rejected v1: the kill switch "
            "measures realized drawdown against the realized-equity peak with a 15pct "
            "threshold instead of 8pct of the fixed bucket. "
    ...
    for row in payload["windows"]:
        ...
        rows.append(
            "| {label} | {core:.4f} | {before:.4f} | {after:.4f} | {kept}/{total} | {skips} | {dd:.2%} |".format(
                ...
                dd=float(audit["final_kill_switch_state"]["max_realized_drawdown_pct_of_peak_equity"] or 0),
            )
        )
    gate = payload["gate4"]
    parts = [

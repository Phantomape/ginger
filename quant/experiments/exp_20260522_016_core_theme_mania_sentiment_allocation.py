"""exp-20260522-016: core theme-mania sentiment allocation scout.

Alpha search. Tests whether already-qualified core stock signals in the
theme_mania sentiment surface should receive a risk haircut or small top-up.
The sentiment proxy uses replay-visible regime-exit bucket/score, strategy,
and sector fields; it does not use future prices, news, LLM output, or
post-entry outcomes.
...
from backtest_sentiment_attribution import infer_trade_sentiment  # noqa: E402
from backtester import BacktestEngine  # noqa: E402
from constants import MAX_POSITION_PCT  # noqa: E402
from data_layer import get_universe  # noqa: E402


EXPERIMENT_ID = "exp-20260522-016"
STEM = "core_theme_mania_sentiment_allocation"
MULTIPLIER_KEY = "sentiment_surface_theme_mania_risk_multiplier_applied"
...
VARIANTS = OrderedDict(
    [
        ("theme_mania_haircut_0900", {"multiplier": 0.90}),
        ("theme_mania_haircut_0950", {"multiplier": 0.95}),
        ("theme_mania_topup_1025", {"multiplier": 1.025}),
        ("theme_mania_topup_1050", {"multiplier": 1.05}),
    ]
)

EXCLUDED_SECTORS = {"ETF", "Commodities"}
TARGET_SENTIMENTS = {"theme_mania"}
...
def _sentiment_surface_for_signal(sig: dict[str, Any]) -> dict[str, Any]:
    """Infer a production-visible sentiment surface from signal metadata."""
    proxy_trade = {
        "regime_exit_bucket": sig.get("regime_exit_bucket"),
        "strategy": sig.get("strategy"),
        "sector": sig.get("sector"),
    }
    sentiment = infer_trade_sentiment(proxy_trade)
    return {
        "sentiment_surface": sentiment.get("sentiment"),
...
def _apply_topup_to_sizing(
    ...
    if new_shares <= 0 or new_shares == old_shares:
        return sig, None
    ...
    sizing["sentiment_surface_theme_mania_state"] = True
    sizing["sentiment_surface"] = sentiment.get("sentiment_surface")
    sizing["sentiment_confidence"] = sentiment.get("sentiment_confidence")
    sizing["sentiment_why"] = sentiment.get("sentiment_why")
    sizing["sentiment_surface_theme_mania_baseline_shares"] = old_shares
    sizing["sentiment_surface_theme_mania_new_shares"] = new_shares
    sizing[MULTIPLIER_KEY] = multiplier
    sig = {**sig, **sentiment, "sizing": sizing}
    ...
    record = {
        ...
        "sentiment_surface": sentiment.get("sentiment_surface"),
        "sentiment_confidence": sentiment.get("sentiment_confidence"),
        "sentiment_why": sentiment.get("sentiment_why"),
        "regime_exit_bucket": sig.get("regime_exit_bucket"),
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Core stock signals in the theme_mania sentiment surface may be "
            "crowded enough to deserve a risk haircut, or strong enough to "
            "deserve a small top-up. A single cap-aware multiplier should "
            "improve EV without changing entry filters, ranking, exits, "
            "universe, LLM, or news logic."
        ),
        "change_summary": (
            "Use the existing sentiment-surface classifier as an experiment-only "
            "allocation field and sweep a cap-aware risk multiplier for "
            "non-ETF/non-Commodity core signals classified as theme_mania."
        ),
        "change_type": "capital_allocation",
        "mechanism_family": "core_market_sentiment_surface",
        ...
        "component": "quant/sentiment_surface.py, quant/portfolio_engine.py",
        "parameters": {
            "field": "sentiment_surface",
            "field_definition": (
                "theme_mania market-sentiment proxy inferred from replay-visible "
                "regime_exit_bucket, strategy, and sector metadata"
            ),
            "target_sentiments": sorted(TARGET_SENTIMENTS),
        ...
        "production_impact": {
            ...
            "promotion_required_if_accepted": (
                "Move sentiment-surface theme_mania state and risk multiplier into "
                "shared sentiment_surface/portfolio_engine plumbing with tests, "
                "then rerun the same three-window protocol."
            ),
        },
        "why_not_other_changes": (
            ...
            "the strict same-family gate, and broad default cap relief because "
            "exp-20260522-012 just failed old_thin. Also avoided another core "
            "single-name return-path field after exp-20260522-013 and exp-20260522-014. "
            "The constructive sentiment branch in exp-20260522-015 had zero adjusted "
            "signals, so this tests the nonzero-sample sentiment bucket instead."
        ),
        "known_risks": [
            "Moderate multiple-testing risk because core market-state and allocation fields have many accepted predecessors.",
            "The sentiment proxy is production-visible but coarse until daily market context is persisted per signal.",
            "Cap-aware multipliers can be immaterial if existing core sizing is already at the default position cap.",
        ],
        "rejection_reason": rejection_reason,
        "next_retry_requires": (
            [
                "Do not retry adjacent theme_mania sentiment multipliers on the same frozen windows without forward rows or a richer persisted market-context field.",
                "If sentiment remains promising, first persist full daily sentiment context per signal before another sizing test.",
            ]
            if not passed
            else [
                "Promote the sentiment field and risk policy into shared modules.",
                "Add parity/unit tests for production-visible sentiment metadata and sizing.",
                "Rerun the same three-window protocol after promotion before accepting.",
            ]
        ),
        "related_files": [
            "quant/experiments/exp_20260522_016_core_theme_mania_sentiment_allocation.py",

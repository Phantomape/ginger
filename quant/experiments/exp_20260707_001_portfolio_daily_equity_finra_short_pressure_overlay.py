"""exp-20260707-001: daily-equity replay for the FINRA portfolio lane.

This consumes the observed-only ranking from exp-20260706-022. It deliberately
does not retune the FINRA short-pressure source from exp-20260529-017. The only
tested decision hypothesis is whether the rank-6 FINRA source still looks
...
base.EXPERIMENT_ID = "exp-20260707-001"
base.STEM = "portfolio_daily_equity_finra_short_pressure_overlay"
base.STATUS_POSITIVE = (
    "observed_only_positive_finra_short_pressure_daily_equity_lead_not_activation_ready"
)
base.STATUS_REJECTED = (
    "observed_only_rejected_finra_short_pressure_daily_equity_overlay"
)
base.DECISION_POSITIVE = (
    "observed_only_positive_finra_short_pressure_daily_equity_overlay"
)
base.DECISION_REJECTED = (
    "observed_only_rejected_finra_short_pressure_daily_equity_overlay"
)

base.TRIAL_FAMILY = "portfolio_covariance_daily_equity_overlay"
base.TRIAL_VARIANT_ID = "finra_short_pressure_10pct_daily_equity_overlay_v1"
base.CHANGED_VARIABLE = (
    "finra_short_pressure_ranked_portfolio_daily_equity_overlay_v1"
)
...
base.NEW_EVIDENCE_AXIS = (
    "Materially new ranked candidate family and data source inside the fixed "
    "exp-20260706-022 portfolio ranking: FINRA short-pressure has not been "
    "consumed by the daily-MTM overlay lane; this uses the fixed recorded "
    "ranking and source artifact with no FINRA threshold, top-N, hold-day, "
    "cooldown, notional, overlay-weight, or correlation retune."
)

base.OUT_DIR = base.REPO_ROOT / "data" / "experiments" / base.EXPERIMENT_ID
base.OUT_JSON = base.OUT_DIR / "exp_20260707_001_portfolio_daily_equity_finra_short_pressure_overlay.json"
...
base.TARGET_SOURCE_ARTIFACT = (
    "data/experiments/exp-20260529-017/"
...
base.PREDICTION = {
    ...
    "confidence_reason": (
        "exp-20260706-022 ranked FINRA short pressure sixth with positive "
        "rejected-source PnL and near-zero exit-cashflow correlation to core, "
        "but the prior daily-MTM overlay attempts showed proxy edge can lose "
        "window stability; the original FINRA source also regressed mid_weak."
    ),
    "recorded_at": "2026-07-07T00:04:05+00:00",
}

base.HYPOTHESIS = (
    "Portfolio lane: the exp-20260706-022 rank-6 FINRA short-pressure rejected "
    "candidate family may add value as a fixed 10 percent paper overlay when "
    "replayed with true daily mark-to-market equity, without retuning FINRA "
    "source rules or overlay weight."
)
base.ALPHA_HYPOTHESIS = (
    "capital_allocation / risk_allocation: a rejected official FINRA "
    "short-pressure candidate source can be portfolio-useful if a small fixed "
    "overlay improves aggregate daily-equity EV while drawdown and window "
    "stability remain acceptable."
)
base.CAUSAL_COMPONENTS = [
    "consume exp-20260706-022 ranking",
    "daily mark-to-market equity replay",
    "accepted core equity comparison",
    "no FINRA threshold retune",
    "no strategy behavior change",
]

base.RUNNER = f"quant/experiments/exp_20260707_001_{base.STEM}.py"
base.RUNNER_COMMAND = f".\\.venv\\Scripts\\python.exe -B {base.RUNNER}"
base.RUNNER_WINDOWS = f"quant\\experiments\\exp_20260707_001_{base.STEM}.py"
base.CHANGED_FILES = [
    base.RUNNER,
    "data/experiments/exp-20260707-001/exp_20260707_001_portfolio_daily_equity_finra_short_pressure_overlay.json",
    "experiments/cards/exp-20260707-001.md",
    "experiments/manifests/exp-20260707-001.json",
    "experiments/logs/exp-20260707-001.json",
    "experiments/tickets/exp-20260707-001.json",
...
def source_ranking_audit() -> dict[str, Any]:
    ...
    return {
        ...
        "reason_selected": (
            "Rank-6 official FINRA short-pressure candidate in the "
            "exp-20260706-022 portfolio ranking. The higher-ranked Companyfacts "
            "and sector-breadth candidates were already consumed by "
            "exp-20260706-023..025, and this test does not retune FINRA source "
            "rules."
        ),
    }
...
def patch_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["post_run_reflection"] = {
        "why_result_happened": (
            "The FINRA short-pressure candidate from exp-20260529-017 was "
            "consumed from the exp-20260706-022 portfolio ranking and replayed "
            "as daily mark-to-market equity. The result is judged on aggregate "
            "daily EV, drawdown drift, and window stability, not on FINRA "
            "source thresholds, top-N, hold days, cooldown, or notional."
        ),
        "forbidden_near_neighbor_retry": (
            "Do not rerun this FINRA portfolio overlay by changing overlay "
            "weight, FINRA short-pressure thresholds, breakout confirmation, "
            "hold days, top-N, correlation cutoffs, or source filters. A legal "
            "retry needs materially new ranked candidate families, a different "
            "data source, materially more closed forward rows, or a shared "
            "paper helper / activation-envelope experiment."
        ),
    ...
    payload["next_retry_requires"] = [
        ...
        "no overlay-weight, FINRA threshold, top-N, hold-day, cooldown, or correlation retune",
    ]
    if not positive:

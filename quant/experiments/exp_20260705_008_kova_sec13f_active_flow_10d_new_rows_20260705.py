...
flow surface only because the hot warehouse now supplies materially more
closed 10d replacement-value rows beyond the exp-20260701-009 cohort. It keeps
the active-flow score fixed and does not change ranking, sizing, exits, paper
sleeves, live orders, watchlists, LLM boundaries, or production daily behavior.
"""
...
OWNER = "alpha-explore"
SLUG = "kova_sec13f_active_flow_10d_new_rows_20260705"
RUNNER = f"quant/experiments/exp_20260705_008_{SLUG}.py"
RUNNER_COMMAND = ".\\.venv\\Scripts\\python.exe -B " + RUNNER.replace("/", "\\")

DATA_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = DATA_DIR / f"exp_20260705_008_{SLUG}.json"
...
HYPOTHESIS = (
    "Observed-only alpha hypothesis: the fixed Kova SEC13F active-manager "
    "active-flow field should preserve 10d cash/SPY/QQQ replacement-value "
    "separation on newly settled rows beyond the exp-20260701-009 "
    "2026-06-13..15 cohort."
)
...
TRIAL_VARIANT_ID = "hot_warehouse_new_10d_rows_20260705_v1"
CHANGED_VARIABLE = "kova_sec13f_active_flow_10d_newly_settled_rows_20260705_v1"
SINGLE_CAUSAL_VARIABLE = CHANGED_VARIABLE
NEW_EVIDENCE_TYPE = "materially_more_closed_forward_rows"
NEW_EVIDENCE_AXIS = (
    "Materially more closed 10d Kova SEC13F active-flow forward rows after "
    "exp-20260701-009's 2026-06-13..15 cohort; same fixed score, no threshold "
    "or response retune."
)
...
ALLOWED_WRITE_SCOPE = [
    RUNNER,
    "data/experiments/exp-20260705-008/exp_20260705_008_kova_sec13f_active_flow_10d_new_rows_20260705.json",
    "experiments/cards/exp-20260705-008.md",
    "experiments/manifests/exp-20260705-008.json",
    "experiments/tickets/exp-20260705-008.json",
    "experiments/logs/exp-20260705-008.json",
    "docs/experiment_log.jsonl",
    "docs/experiment_registry.json",
]

HORIZON = 10
PRIOR_COHORT_ASOF_END = "2026-06-15"
...
ACCEPTANCE_RULE = {
    "horizon": HORIZON,
    "new_rows_only": True,
    "prior_cohort_asof_end": PRIOR_COHORT_ASOF_END,
    "min_settled_rows": 1000,
    "min_scored_rows": 700,
    "min_asof_dates": 2,
...
DEFAULT_PREDICTION = {
    ...
    "main_failure_modes": [
        "signal_decay_on_new_rows",
        "cash_edge_not_positive",
        "qqq_beta_only",
        "near_neighbor_kova_sec13f_forward_repeat",
        "concentration_failed",
    ],
    "confidence_reason": (
        "exp-20260701-009 was a positive observed-only 10d lead with a "
        "predeclared reopen on materially more closed rows; preflight showed "
        "2489 newly settled rows after 2026-06-15, while historical promotion "
        "and adjacent Kova/13F attempts remain rejected."
    ),
}
PRODUCTION_IMPACT = {
    "trade_enabled": False,
...
def new_cohort_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        row
        for row in rows
        if str(row.get("asof_date") or "")[:10] > PRIOR_COHORT_ASOF_END
    ]
...
def source_summary(
    ...
    new_settled_rows: list[dict[str, Any]],
    flow: dict[str, Any],
    ...
    new_settled_asof_dates = sorted(
        {
            str(row.get("asof_date") or "")[:10]
            for row in new_settled_rows
    ...
    return {
        ...
        "prior_cohort_asof_end": PRIOR_COHORT_ASOF_END,
        "new_settled_10d_comparator_rows": len(new_settled_rows),
        "new_settled_10d_asof_date_start": (
            new_settled_asof_dates[0] if new_settled_asof_dates else None
        ),
        "new_settled_10d_asof_date_end": (
            new_settled_asof_dates[-1] if new_settled_asof_dates else None
        ),
        "new_settled_10d_asof_date_count": len(new_settled_asof_dates),
        ...
        "active13f_status_counts_new_settled_10d": dict(
            sorted(
                Counter(str(row.get("active13f_status") or "missing") for row in new_settled_rows).items()
            )
        ),
...
def evaluate_gate4(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": (
            "observed_only_positive_new_10d_active_flow_lead_not_promoted_historical_gate4_blocked"
            if observed_only_lead
            else "rejected_no_new_10d_kova_active_flow_forward_edge"
        ),
        "acceptance_checks": checks,
        "failed_reasons": failed,
        "lead_limitations": [
            (
                "Forward-only post-2026-06-15 rows, not canonical fixed-window "
                "PIT coverage."
            ),
...
def build_analysis(source_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    new_settled = new_cohort_rows(settled)
    full_current_summary = active_flow.summarize_horizon(settled, HORIZON)
    primary_summary = active_flow.summarize_horizon(new_settled, HORIZON)
    return {
        ...
        "full_current_summary": full_current_summary,
    }
...
def build_payload() -> dict[str, Any]:
    ...
    why = (
        ...
        f"{'did' if observed_only_lead else 'did not'} preserve a strict "
        f"new-row-only 10d cash/SPY/QQQ replacement-value edge after "
        f"{PRIOR_COHORT_ASOF_END}. This run is attribution only and did not "
    ...
    return {
        ...
        "pre_run_questions": {
            ...
            "2_history_check": {
                ...
                "exp-20260701-009": (
                    "Positive observed-only 10d lead on the 2026-06-13..15 "
                    "cohort; this run tests only later as-of rows."
                ),
            },
            "3_single_policy_bundle": (
                "One observed-only bundle: reuse the fixed active-manager "
                "active-flow score and test new-only 10d cash/SPY/QQQ "
                "replacement-value separation."
            ),
            "4_success_failure_standard": (
                "Observed-only lead only if post-2026-06-15 10d settled/scored "
                "sample floors pass, high active-flow beats low by mean and "
                "median for cash/SPY/QQQ, Spearman correlations are positive, "
        ...
        "parameters": {
            ...
            "primary_sample_definition": (
                f"Only settled 10d comparator rows with asof_date > {PRIOR_COHORT_ASOF_END}; "
                "older rows are reported only as context."
            ),
        ...
        "attribution": {
            ...
            "horizon": HORIZON,
            "primary_sample_definition": (
                f"new settled 10d rows after {PRIOR_COHORT_ASOF_END}"
            ),
            "primary_summary": primary,
            "full_current_summary": analysis["full_current_summary"],
            "sample_rows": [
                {
                ...
                for row in analysis["new_settled_rows"][:25]
            ],
        },
        "primary_summary": {
            "horizon": HORIZON,
            "sample": "new_settled_rows_after_prior_cohort",
            "prior_cohort_asof_end": PRIOR_COHORT_ASOF_END,
            "summary": primary,
        },
        "full_current_summary": {
            "horizon": HORIZON,
            "sample": "all_current_settled_rows",
            "summary": analysis["full_current_summary"],
        },
        ...
        "post_run_reflection": {
            ...
            "forbidden_near_neighbor_retry": (
                ...
                "retunes on this same 2026-06-16..17 10d row set."
            ),
            "new_evidence_required": (
                "A valid retry needs materially more newly closed 10d rows beyond "
                "the 2026-06-16..17 cohort tested here, manager-level flow from "
                "a genuinely new non-quarterly source, borrow/loan-availability "
                "cross-evidence, or canonical fixed-window PIT coverage through "
                "a shared helper that beats accepted comparators."
            ),
        },
        "related_files": [
            RUNNER,
...
def compact_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "primary_summary": {
            "horizon": HORIZON,
            "sample": payload["primary_summary"]["sample"],
            "prior_cohort_asof_end": payload["primary_summary"]["prior_cohort_asof_end"],
        ...
        "full_current_summary": payload["full_current_summary"],
...
def build_card(payload: dict[str, Any]) -> str:
    primary = payload["primary_summary"]["summary"]
    full_current = payload["full_current_summary"]["summary"]
    rows = [
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: Kova SEC13F active-flow 10d new-row value",
            "",
            f"- Status: `{payload['status']}`",
            f"- Decision: `{payload['decision']}`",
            "- Production orders changed: no",
            "- Shared helper promoted: no",
            f"- Primary sample: post-`{PRIOR_COHORT_ASOF_END}` settled 10d rows",
            f"- New settled 10d rows: `{primary['settled_rows']}`",
            f"- New scored 10d rows: `{primary['scored_rows']}`",
            f"- Full current settled 10d rows: `{full_current['settled_rows']}`",
            "",
            "## Hypothesis",
            "",
            payload["hypothesis"],
...
def build_manifest(payload: dict[str, Any]) -> dict[str, Any]:
    files = [
        ...
        REPO_ROOT / "experiments" / "logs" / "exp-20260701-009.json",
    ]
        ...
        json.dumps(
            {
                ...
                "decision": payload["decision"],
                "observed_only_lead": payload["observed_only_lead"],
                "primary_sample": payload["primary_summary"]["sample"],
                "new_settled_10d_rows": primary["settled_rows"],
                "new_scored_10d_rows": primary["scored_rows"],
                "new_scored_asof_dates": primary["scored_asof_date_count"],

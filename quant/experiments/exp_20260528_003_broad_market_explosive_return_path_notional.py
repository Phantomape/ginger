"""exp-20260528-003: broad-market explosive return-path paper notional.

Alpha search follow-up to the accepted exp-20260520-004 broad-market paper
sleeve. The accepted candidate pool, rank profile, low-extension support,
high-volatility support, trend-persistence support, hold period, and entry
slots remain fixed.

This run changes one causal variable: a default-off paper notional scalar for
already-selected broad-market candidates whose 20-day close-to-close gain is
dominated by a single positive day.
...
EXPERIMENT_ID = "exp-20260528-003"
EXPERIMENT_SLUG = "broad_market_explosive_return_path_notional"
...
BASELINE_VARIANT = "baseline_no_explosive_return_path_support"
EXPLOSIVE_PATH_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
                "max_positive_day_share_20_min": 0.55,
                "scalar": 1.0,
            },
        )
    ]
...
for threshold in (0.45, 0.50, 0.55, 0.60, 0.65, 0.70):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15, 1.25, 1.50, 2.00):
        threshold_text = f"{threshold:.2f}".replace(".", "p")
        scalar_text = f"{scalar:.3f}".rstrip("0").rstrip(".").replace(".", "p")
        EXPLOSIVE_PATH_SWEEP[f"maxposshare20_gte_{threshold_text}_scalar_{scalar_text}"] = {
            "max_positive_day_share_20_min": threshold,
            "scalar": scalar,
        }
...
def _return_path_features(rows: list[dict[str, Any]], idx: int) -> dict[str, Any] | None:
    if idx < 20:
        return None
    returns: list[float] = []
    ...
    if close is None or close_20 is None:
        return None
    ...
    max_abs = max(abs(value) for value in returns)
    max_positive_share = max_positive / ret20 if ret20 > 0 else None
    efficiency = abs(ret20) / abs_sum if abs_sum > 0 else None
    return {
        "return_path_ret20": round(ret20, 6),
        "max_positive_day_return_20": round(max_positive, 6),
        "max_positive_day_share_20": (
            round(max_positive_share, 6) if max_positive_share is not None else None
        ),
        "max_abs_daily_return_20": round(max_abs, 6),
        "return_path_efficiency_20": round(efficiency, 6) if efficiency is not None else None,
        "positive_day_count_20": positive_days,
        "negative_day_count_20": negative_days,
    }
    ...
    *,
    ...
    for day in days:
        ...
        for ticker in candidate_tickers:
            ...
            if not feature or not candidate_passes_profile(feature, PROFILE_CONFIG):
                continue
            path_features = _return_path_features(rows, idx)
            if path_features is None:
                continue
            feature.update(path_features)
...
def _variant_trades(
    *,
    ...
    prices: dict[str, list[dict[str, Any]]],
    max_positive_day_share_20_min: float,
    scalar: float,
    label: str,
) -> list[dict[str, Any]]:
    spec = WINDOWS[label]
    ...
    for row in base_rows:
        ...
        if trade is None:
            continue
        share = row["feature"].get("max_positive_day_share_20")
        support_applied = (
            share is not None
            and float(share) >= float(max_positive_day_share_20_min)
            and float(scalar) != 1.0
        )
        if support_applied:
            trade["pre_explosive_return_path_notional"] = trade["notional"]
            trade["pre_explosive_return_path_shares"] = trade["shares"]
            trade["pre_explosive_return_path_pnl"] = trade["pnl"]
        ...
        trade["window"] = label
        trade["max_positive_day_share_20"] = share
        trade["max_positive_day_return_20"] = row["feature"].get(
            "max_positive_day_return_20"
        )
        trade["return_path_efficiency_20"] = row["feature"].get(
            "return_path_efficiency_20"
        )
        trade["max_positive_day_share_20_min"] = max_positive_day_share_20_min
        trade["explosive_return_path_notional_scalar"] = scalar
        trade["explosive_return_path_notional_multiplier"] = scalar if support_applied else 1.0
        trade["explosive_return_path_support_applied"] = support_applied
        trades.append(trade)
    return trades
        ...
        rows.append(
            {
                ...
                "rank": trade["rank"],
                "score": trade["score"],
                "max_positive_day_share_20": trade.get("max_positive_day_share_20"),
                "max_positive_day_return_20": trade.get("max_positive_day_return_20"),
                "return_path_efficiency_20": trade.get("return_path_efficiency_20"),
                "explosive_return_path_support_applied": trade.get(
                    "explosive_return_path_support_applied"
                ),
                "explosive_return_path_notional_multiplier": trade.get(
                    "explosive_return_path_notional_multiplier"
                ),
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("explosive_return_path_support_applied")]
    base["explosive_return_path_adjusted_trade_count"] = len(adjusted)
    base["explosive_return_path_adjusted_pnl"] = round(
    ...
    *,
    ...
    for label, spec in WINDOWS.items():
        trades = _variant_trades(
            ...
            prices=prices,
            max_positive_day_share_20_min=max_positive_day_share_20_min,
            scalar=scalar,
            label=label,
        )
    ...
    adjusted = [row for row in all_trades if row.get("explosive_return_path_support_applied")]
    ...
    relative_ev_improvement = after_ev / before_ev - 1.0 if before_ev != 0.0 else 0.0
    gate4_passed = bool(
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return sorted(
        ...
        key=lambda row: (
            ...
            row["gate4"]["windows_ev_regressed"] == 0,
            row["gate4"]["windows_pnl_regressed"] == 0,
            int(row["gate4"]["windows_ev_improved"]),
    ...
    return [
        {
            ...
            "max_positive_day_share_20_min": row["max_positive_day_share_20_min"],
            "scalar": row["scalar"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Explosive Return-Path Support Notional",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {gate} | {adjusted} | {ev:+.4f} | {rel:+.2%} | ${pnl:+,.2f} | {wi} | {wr} | {dd:+.4%} | {single} | {top5} |".format(
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        _variant_payload(
            variant_name=name,
            max_positive_day_share_20_min=values["max_positive_day_share_20_min"],
        ...
        for name, values in EXPLOSIVE_PATH_SWEEP.items()
    ]
    ...
    decision = (
        "accepted_default_off_broad_market_explosive_return_path_notional"
        if accepted
        else "rejected_broad_market_explosive_return_path_notional"
    )
    ...
    production_impact = {
        ...
        "trade_enabled": False,
        "promotion_policy": (
            "Rejected variants are not promoted. A future positive version would need "
            "the same field and scalar implemented in quant/broad_market_paper_sleeve.py "
            "with parity tests before retention."
        ),
    }
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "Already-selected broad-market leadership paper candidates whose "
            "20-day gain is dominated by one large positive day may represent "
            "high-information acceleration leadership and deserve bounded paper "
            "notional support."
        ),
        "change_type": "default_off_paper_allocation",
        "changed_variable": "broad_market_max_positive_day_share_20_support_notional_scalar",
        "single_causal_variable": (
            "max_positive_day_share_20 threshold/scalar for fixed broad-market selected candidates"
        ),
        "trial_accounting": {
            "trial_family": "broad_market_daily_return_path_support",
            "changed_variable": "max_positive_day_share_20_min_and_scalar",
            "prior_trial_count": 0,
            ...
            "new_evidence_type": "new_return_path_ratio_field_on_frozen_paper_pool",
        },
        ...
        "parameters": {
            ...
            "selected_max_positive_day_share_20_min": selected[
                "max_positive_day_share_20_min"
            ],
            "selected_explosive_return_path_scalar": selected["scalar"],
            "sweep": EXPLOSIVE_PATH_SWEEP,
            "profile_config": {
        ...
        "backtest_protocol": (
            ...
            "state; after state adds an experiment-local max_positive_day_share_20 "
        ...
        "expected_value_score_delta": {
            ...
            **{
                label: selected["delta_metrics"]["by_window"][label][
        ...
        "selected_variant": {
            ...
            "max_positive_day_share_20_min": selected[
                "max_positive_day_share_20_min"
            ],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "capital allocation / candidate-pool quality: selected broad-market "
                "leaders with a high max_positive_day_share_20 may be acceleration "
                "leaders worth extra default-off paper notional."
            ),
            "2_past_similar_experiments": (
                "exp-20260523-006 tested absolute max_daily_return_20 and rejected "
                "smooth/non-spike support; exp-20260524-024 tested coarse path "
                "clusters but not max_positive_day_share_20; exp-20260527-026 and "
                "exp-20260528-001 rejected repeat/freshness support."
            ),
            "3_single_variable": (
                "Only max_positive_day_share_20_min/scalar changes inside one "
                "notional-support field; eligibility, rank, sizing stack before the "
                "new scalar, hold, slots, and frozen universe stay fixed."
            ),
            "4_acceptance": (
                "Gate 4 requires positive aggregate EV/PnL, all 3 windows EV-positive, "
                "no EV/PnL regression windows, >=8 adjusted trades across all 3 "
                "windows, concentration guard, <=0.5pp max drawdown worsening, "
                "identity replay parity, and >10% relative aggregate EV improvement."
            ),
            "5_reproducibility": (
                "Script, frozen candidate universe, JSON artifact, log, tickets, "
                "markdown artifact, and JSONL identify windows, parameters, and metrics."
            ),
        },
        "interpretation": (
            "The field gives a distinct read on acceleration concentration, but the "
            "best no-regression variant is far below the hard EV lift needed for this "
            "crowded broad-market paper-notional family. No shared adapter is changed."
        ),
        "rejection_reason": None
        if accepted
        else (
            "Explosive return-path support did not clear Gate 4: stable variants "
            "improved all three windows only marginally and failed the >10% relative "
            "aggregate EV requirement; aggressive variants introduced window EV "
            "regression."
        ),
        "next_evidence_needed": (
            "Do not retry nearby max-positive-day-share thresholds on the frozen "
            "broad-market sample without new forward paper outcomes or a distinct "
            "free-data candidate-pool source."
        ),
        "related_files": {

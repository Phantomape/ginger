...
an additional small top-up only when the Space peer momentum state is leader.
...
STEM = "space_forward_replacement_peer_leader_trend_risk"
...
PEER_LEADER_TREND_RISK_SCALARS = (1.00, 1.025, 1.05, 1.075, 1.10)
...
def _run_variant(
    label: str,
    *,
    peer_leader_trend_scalar: float,
    ...
    def size_with_peer_leader_trend_scalar(
        ...
        for signal in sized:
            ...
            is_peer_leader_trend = (
                is_strength_trend
                and signal.get("space_peer_momentum_state") == PEER_LEADER_STATE
            )
            if is_peer_leader_trend and sizing:
                counts["eligible_signal"] += 1
                counts[f"eligible_{ticker}"] += 1
                shares_before, shares_after = _scale_and_count(
                    sizing=sizing,
                    scalar=peer_leader_trend_scalar,
                    portfolio_value=portfolio_value,
                    marker="space_forward_replacement_peer_leader_trend_risk",
                    counts=counts,
                    ticker=ticker,
                )
                ...
                signal = {
                    **signal,
                    "space_forward_replacement_peer_leader_trend_bucket": True,
                    "space_forward_replacement_peer_leader_trend_scalar": (
                        peer_leader_trend_scalar
                    ),
                }
    ...
    portfolio_engine.size_signals = size_with_peer_leader_trend_scalar
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "peer_leader_trend_trade_attribution": _space_trade_attribution_for(
                    result,
                    set(str(row.get("ticker") or "").upper() for row in adjustments[before_adjustments:]),
                ),
                "peer_leader_trend_adjustment": source_diversity_exp._adjustment_summary(
                    adjustments[before_adjustments:]
                ),
                "peer_leader_trend_counts": dict(sorted((counts - before_counts).items())),
            }
        ...
        return {
            ...
            "parameters": {
                ...
                "target_peer_state": PEER_LEADER_STATE,
            },
            "by_window": by_window,
            "aggregate": source_diversity_exp._aggregate(metrics_by_window),
            "peer_leader_trend_adjustment_summary": (
                source_diversity_exp._adjustment_summary(adjustments)
            ),
            "peer_leader_trend_counts": dict(sorted(counts.items())),
            "peer_leader_trend_adjustment_sample": adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["peer_leader_trend_counts"].get(
            "space_forward_replacement_peer_leader_trend_risk_changed_signal",
            0,
        )
    )
    eligible_count = int(variant["peer_leader_trend_counts"].get("eligible_signal", 0))
    scalar = float(
        variant["parameters"]["space_forward_replacement_peer_leader_trend_scalar"]
    )
    return {
        ...
        "eligible_peer_leader_trend_signal_count": eligible_count,
        "changed_peer_leader_trend_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _gate2_runtime_state(before: dict[str, Any]) -> dict[str, Any]:
    ...
    for window_name, payload in before.get("by_window", {}).items():
        counts = payload.get("peer_leader_trend_counts") or {}
        eligible += int(counts.get("eligible_signal", 0) or 0)
        summary = payload.get("peer_leader_trend_adjustment") or {}
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space forward replacement peer-leader trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_forward_replacement_peer_leader_trend_scalar` for `trend_long` "
            "signals already in the accepted forward same-theme replacement-strength "
            "bucket and whose Space peer momentum state is `leader`. Candidate pool, "
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_forward_replacement_peer_leader_trend_scalar']}`"
        ),
        (
        ...
        (
            "- Peer-leader trend signals changed: "
            f"`{gate['changed_peer_leader_trend_signal_count']}` of "
            f"`{gate['eligible_peer_leader_trend_signal_count']}` eligible"
        ),
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["by_window"][name]["peer_leader_trend_adjustment"][
...
def run() -> dict[str, Any]:
    ...
    before = _run_variant(
        "accepted_exp009_peer_leader_neutral",
        peer_leader_trend_scalar=1.0,
    ...
    gate2 = {
        ...
        "peer_leader_trend_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    variants = [
        _run_variant(
            ...
            peer_leader_trend_scalar=scalar,
            forward_gate=forward_gate,
            source_diversity_gate=source_diversity_gate,
            attention_gate=attention_gate,
            single_event_gate=single_event_gate,
        ...
        for scalar in PEER_LEADER_TREND_RISK_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested peer-leader trend replacement-strength scalar improved "
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "same-theme cash evidence, trend continuation, and Space peer leadership "
            "agree. On top of accepted exp-20260514-009, a single extra peer-leader "
            "trend scalar tests the playbook's catalyst/source/peer bucket direction "
        ...
        "changed_variable": "space_forward_replacement_peer_leader_trend_risk_scalar",
        "single_causal_variable": (
            "extra risk scalar for trend_long signals in the accepted forward "
            "same-theme replacement-strength bucket when space_peer_momentum_state "
            "is leader"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                ...
                "same-theme replacement-strength bucket and whose peer momentum "
                "state is leader."
            ),
            "q2_prior_experiments": [
                ...
                "No prior record found for same-theme replacement-strength trend plus peer-leader interaction.",
            ],
            "q3_single_causal_variable": (
                "Only the additional peer-leader trend risk scalar changes; accepted "
        ...
        "parameters": {
            ...
            "tested_peer_leader_trend_scalars": list(PEER_LEADER_TREND_RISK_SCALARS),
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_forward_replacement_peer_leader_trend_risk"
            if decision == "accepted"
            else "rejected_space_forward_replacement_peer_leader_trend_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not promote peer-leader trend replacement-strength risk from "
        ...
        "why_not_other_changes": (
            ...
            "and tests a stronger replacement-value plus peer-quality interaction."
        ),
        "known_risks": [
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_peer_leader_trend_signal_count": result["best_variant_gate"][
            "changed_peer_leader_trend_signal_count"
        ],

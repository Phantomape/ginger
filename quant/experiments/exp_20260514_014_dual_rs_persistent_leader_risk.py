"""exp-20260514-014: dual-RS persistent leader risk allocation.

Tests one production-visible allocation variable on the accepted core stack:
already-qualified trend/breakout signals that are both 20-day entry-state
leaders and top-quintile 60-day cross-sectional momentum leaders may deserve a
small additional cap-aware post-sizing top-up.
...
EXPERIMENT_SLUG = "dual_rs_persistent_leader_risk"
MULTIPLIER_KEY = "dual_rs_persistent_leader_risk_multiplier_applied"
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        for sig in enriched:
            sig["dual_rs_persistent_leader_state"] = bool(
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["dual_rs_persistent_leader_baseline_shares"] = shares
    out["dual_rs_persistent_leader_desired_shares"] = desired_shares
    out["dual_rs_persistent_leader_cap_shares"] = cap_shares
    out["dual_rs_persistent_leader_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if (
                sig.get("dual_rs_persistent_leader_state")
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Dual-RS Persistent Leader Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        "accepted_dual_rs_persistent_leader_risk_topup"
        if passed
        else "rejected_dual_rs_persistent_leader_risk_topup"
    )
    interpretation = (
        "The dual-RS persistent-leader interaction cleared the canonical three-window gate and should be promoted only through shared production/backtest sizing policy."
        if passed
        else "The dual-RS persistent-leader interaction did not clear Gate 4 as an additional standalone sizing top-up after the accepted RS20 and RS60 layers."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted RS20 and RS60 allocation states may have interaction value: "
            "signals that are strong across both the entry-state horizon and the "
            "longer cross-sectional horizon could deserve a small additional "
            "allocation top-up, while isolated RS states already have their own "
            "accepted sizing."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "dual_rs_persistent_leader_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for signals where rs20_entry_state_leader and rs60_top_quintile_state are both true"
        ),
        "parameters": {
            "state_definition": {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital/risk allocation: cross-horizon persistent relative strength may identify higher-quality already-qualified core trend/breakout signals."
            ),
            "2_history_check": {
                "exp-20260510-010": (
                    "RS20 entry-state leadership was promoted as a broad cap-aware top-up; this does not retune that scalar and tests only the interaction with RS60."
                ),
                "exp-20260513-030": (
                    "RS60 top-quintile momentum was promoted as a broad cap-aware top-up; this does not retune that scalar and tests only the interaction with RS20."
                ),
                "exp-20260510-014": (
                    "RS60 non-RS20 allocation was rejected; this experiment tests the opposite persistent-leader intersection rather than isolated RS60."
                ),
                "exp-20260513-033": (
                    "Vol-adjusted RS60 was rejected; this uses existing accepted states and no new volatility transform."
                ),
                "llm_soft_ranking": (
                    "Avoided because labeled forward LLM soft-ranking records remain too thin for trustworthy alpha validation."
                ),
            },
            "3_single_causal_variable": (
                "dual_rs_persistent_leader_risk_multiplier sweep only."
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_014_dual_rs_persistent_leader_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        if passed
            ...
            "Use forward attribution for accepted RS20/RS60 overlap or a materially different production-visible discriminator before retrying this interaction family."
        ),
        "related_files": [
            "quant/experiments/exp_20260514_014_dual_rs_persistent_leader_risk.py",
            "data/experiments/exp-20260514-014/dual_rs_persistent_leader_risk.json",
            "docs/experiments/logs/exp-20260514-014.json",
            "docs/experiments/tickets/exp-20260514-014.json",
            "docs/experiments/artifacts/exp-20260514-014_dual_rs_persistent_leader_risk.md",

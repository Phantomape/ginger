"""exp-20260801-004: Massive long-gap dividend-restart research replay.

Private replay scout (research_pit; result ceiling observed_only). The frozen
policy, 89-row endpoint roster, one-use core-or-cash comparator slot map and
falsifier come from panel v5 (cand-ed4c2908c4645ea90319, promotion
data/alpha_search/promotions/massive_dividend_restart_scout_20260801.json).
This runner is the FIRST outcome access for the candidate.

Frozen policy (massive_dividend_restart_1095d_next_open_h10_replacement_value_v1):
- touches: the 89 rows of the hash-bound comparator slot map
  data/alpha_search/massive_dividend_restart_comparator_map_20260731.json
  (39 old_thin / 21 mid_weak / 29 late_strong); entry/exit sessions are
  frozen per row (next open after declaration_date, close of the 10th session)
- $4,000 research notional; 0.35% round-trip cost; 0.70% double-cost stress
- primary comparator: the frozen per-row core slot (exactly one core ticker
  over the identical entry-open to H10-close interval at identical
  notional/cost) or zero-return cash; the slot map is one-use and was frozen
  outcome-blind on 2026-07-31 -- this runner never re-allocates slots
- no dividend cash receipt is credited (frozen execution envelope)
...
- descriptive overlapping-hold cluster block bootstrap (10,000 resamples,
  seed 20260801) on aggregate replacement value
- falsifier: roster exactly 39/21/29; >=2 of 3 windows AND aggregate strictly
  positive replacement value; double-cost aggregate positive; max single
  ticker positive share <= 40%; top-5 <= 60%; contribution HHI <= 0.25
"""
...
SLOT_MAP = (
    ...
    / "data"
    / "alpha_search"
    / "massive_dividend_restart_comparator_map_20260731.json"
)
OUT_DIR = REPO_ROOT / "data" / "experiments" / "exp-20260801-004"
OUT = OUT_DIR / "exp_20260801_004_massive_dividend_restart_scout.json"
...
EXPECTED_WINDOWS = {"old_thin": 39, "mid_weak": 21, "late_strong": 29}
...
def main() -> None:
    ...
    for r in map_rows:
        ...
        if ret is None:
            ...
            continue
        if r["comparator"] == "core_slot":
            core_ticker, core_session, core_price = r["core_slot_id"].split(":")
            core_ret = leg_return(con, core_ticker, entry_s, exit_s)
            core_bar = bar(con, core_ticker, core_session)
            core_slot_price_checks.append(
                {
                    "core_slot_id": r["core_slot_id"],
                    "recorded_entry_open": float(core_price),
                    "warehouse_entry_open": None if core_bar is None else core_bar[0],
                }
            )
            baseline_value = (
                NOTIONAL * (core_ret - COST) if core_ret is not None else 0.0
            )
            baseline_double = (
                NOTIONAL * (core_ret - DOUBLE_COST) if core_ret is not None else 0.0
            )
        else:
            core_ticker, core_ret = None, None
            baseline_value = 0.0
            baseline_double = 0.0
        ...
        rows.append(
            {
                ...
                "replacement_value_double_cost": round(
                    NOTIONAL * (ret - DOUBLE_COST) - baseline_double, 2
                ),
    ...
    falsifier = {
        "roster_exact_39_21_29": roster_exact,
        "all_89_rows_executed": all_executed,
    ...
    passes = all(
        [
            falsifier["roster_exact_39_21_29"],
            falsifier["all_89_rows_executed"],
    ...
    artifact = {
        ...
        "policy_version": "massive_dividend_restart_1095d_next_open_h10_replacement_value_v1",
        "admission_class": "research_replay",
        "evidence_grade": "lead",
        "result_ceiling": "observed_only",
        "trade_enabled": False,
        "price_reader": "data/warehouse/massive_history.sqlite daily_bars (research_pit Massive warehouse; identical reader for treatment, core, SPY and QQQ legs)",
        "cost_pct": COST,

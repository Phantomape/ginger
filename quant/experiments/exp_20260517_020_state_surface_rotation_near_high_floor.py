"""exp-20260517-020: rotation state-surface near-high floor.

Alpha search. Tests one production-visible default-off paper-sleeve variable:
the minimum close-vs-60-day-high ratio required before accepted rotation-only
state-surface paper candidates enter the paper ledger. The accepted
ret20_excess_spy >= 0.0 gate stays locked. Core A/B signals, ranking, sizing,
exits, event bundle logic, LLM/news, and live orders are unchanged.
...
EXPERIMENT_ID = "exp-20260517-020"
EXPERIMENT_SLUG = "state_surface_rotation_near_high_floor"
TARGET_SURFACE = "rotation_breakout_leadership"
BASELINE_FLOOR: float | None = None
FLOOR_VARIANTS: list[float | None] = [None, 0.90, 0.925, 0.95, 0.975, 0.99]
...
def _floor_label(floor: float | None) -> str:
    return "identity_no_near_high_floor" if floor is None else f"{floor:.2%}"
...
def _near_high_gate(row: dict[str, Any], *, floor: float | None) -> dict[str, Any]:
    features = row.get("features") or {}
    near_high = _float_or_none(features.get("near_high_60"))
    enabled = floor is not None
    reasons: list[str] = []
    if enabled:
        if near_high is None:
            reasons.append("near_high_60_unavailable")
        elif near_high < float(floor):
            reasons.append("near_high_60_below_floor")
    allowed = (not enabled) or not reasons
    return {
        "rule_version": "state_surface_near_high_60_floor_replay_v1",
        "enabled": enabled,
        "identity_control": floor is None,
        "allowed": allowed,
        "status": "allowed" if allowed else "blocked",
        "reasons": reasons,
        "near_high_60": _round(near_high),
    ...
    *,
    ...
    for row in candidates:
        if row.get("status") != "price_ready":
            kept.append(row)
            continue
        gate = _near_high_gate(row, floor=floor)
        enriched = {**row, "near_high_60_floor_gate": gate}
        if gate["allowed"]:
            kept.append(enriched)
        else:
            blocked.append({**enriched, "reason": "near_high_60_floor_gate_blocked"})
    return kept, blocked
...
def _selected_trade_rows(trades: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for trade in trades:
        ...
        gate = trade.get("near_high_60_floor_gate") or {}
        rows.append(
            {
                ...
                "near_high_60_floor_gate_allowed": gate.get("allowed"),
                "pnl": trade.get("pnl"),
...
def _variant_payload(
    *,
    ...
    for label, window in WINDOWS.items():
        ...
        filtered, near_high_blocked = _filter_by_floor(spy_filtered, floor=floor)
        gate_blocked = [*spy_blocked, *near_high_blocked]
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rotation Near-High Floor",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    if best_summary["gate4"]["passed"]:
        decision = "passed_replay_requires_shared_near_high_gate"
        status = "pending_promotion"
        interpretation = (
            "A near_high_60 floor improved the accepted rotation-only state-surface paper sleeve. "
            "Promotion requires adding the gate to shared state_surface_sleeve.py and tests "
        ...
        status = "rejected"
        interpretation = (
            "No tested near_high_60 floor improved the accepted rotation-only state-surface paper sleeve "
    ...
    return {
        ...
        "decision": decision,
        "lane": "alpha_search",
        "change_type": "state_surface_rotation_near_high_floor",
        ...
        "hypothesis": (
            ...
            "candidates too far below their 60-day high may be weaker breakout-leadership "
            "replacement bets. A near_high_60 floor may improve replacement value without "
            "expanding the candidate universe or changing core logic."
        ),
        "alpha_hypothesis": {
            ...
            "playbook_alignment": (
                ...
                "candidate-pool quality over broader ticker expansion or scalar retunes."
            ),
        },
        "history_check": {
            ...
            "anti_repeat_boundary": (
                "This is not a ret20_excess_spy threshold retry, not a ret60 retry, "
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                ...
                "already passing ret20_excess_spy >= 0 to stay close enough to their 60-day high"
            ),
            "2_history_check": (
                "Rotation-only, 20-day SPY/IWM relative gates, and ret60 floor were tested; "
                "no current-stack near_high_60 floor experiment was found in experiment_log.jsonl/docs experiments."
            ),
            "3_single_causal_variable": "near_high_60_min",
            "4_acceptance_standard": (
                ...
                f"least two windows, regress zero windows, keep selected trades >= {MIN_SELECTED_TRADES}, "
                f"max drawdown drift <= {MAX_DRAWDOWN_WORSE:.1%}, and single-ticker positive "
                f"share <= {MAX_SINGLE_TICKER_POSITIVE_SHARE:.0%}."
            ),
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\"
                "exp_20260517_020_state_surface_rotation_near_high_floor.py"
            ),
        },
        "parameters": {
            "single_causal_variable": "near_high_60_min",
        ...
        "production_impact": {
            ...
            "production_impact": (
                ...
                "no shared default-off production policy was changed; if a near_high_60 floor "
                "ever passes, it must be added to state_surface_sleeve.py with parity "
                "tests before promotion. Live/default orders remain disabled."
            ),
        },
        "llm_metrics": {
            "used_llm": False,
        ...
        "next_evidence_needed": (
            "Implement the near_high_60 floor in shared state_surface_sleeve.py and rerun parity tests before acceptance."
            if best_summary["gate4"]["passed"]
            else "Do not add a near_high_60 floor on frozen windows; the next state-surface search needs a different production-visible discriminator."
        ),
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rotation near-high floor",

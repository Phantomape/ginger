"""exp-20260520-003: broad-market high-volatility paper notional.

Alpha search follow-up to exp-20260520-002. The accepted broad-market
price-floor leadership pool, rank-notional profile, and low-extension scalar
are fixed. This run changes one causal variable: an extra default-off paper
notional scalar for already-selected broad-market candidates whose 20-day
realized volatility is high enough to identify energetic leadership.
...
EXPERIMENT_ID = "exp-20260520-003"
EXPERIMENT_SLUG = "broad_market_high_volatility_notional"
BASELINE_EXPERIMENT_ID = "exp-20260520-002"
...
BASELINE_JSON = (
    ...
    / "broad_market_low_extension_notional.json"
)
CONTROL_JSON = (
    REPO_ROOT
    / "data"
...
HIGH_VOLATILITY_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict()
HIGH_VOLATILITY_SWEEP["baseline_no_high_volatility"] = {
    "high_volatility_20_min": 0.055,
    "scalar": 1.00,
}
for threshold in (0.040, 0.045, 0.050, 0.055, 0.060):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15):
        ...
        HIGH_VOLATILITY_SWEEP[f"vol20_gte_{threshold_text}_scalar_{scalar_text}"] = {
            "high_volatility_20_min": threshold,
            "scalar": scalar,
        }
...
def _simulate_window(
    *,
    ...
    cfg = {
        **PROFILE_CONFIG,
        "high_volatility_20_min": high_volatility_20_min,
        "high_volatility_notional_scalar": scalar,
    }
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("high_volatility_support_applied")]
    base["high_volatility_adjusted_trade_count"] = len(adjusted)
    base["high_volatility_adjusted_pnl"] = round(
...
def _variant_payload(
    *,
    ...
    adjusted = [row for row in all_trades if row.get("high_volatility_support_applied")]
    ...
    gate4_passed = bool(
        variant_name != "baseline_no_high_volatility"
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pool = preferred or passing or [row for row in variants if row["variant_name"] != "baseline_no_high_volatility"]
    return sorted(
        pool,
        key=lambda row: (
    ...
    return [
        {
            ...
            "high_volatility_20_min": row["high_volatility_20_min"],
            "scalar": row["scalar"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market High-Volatility Notional",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    if baseline_payload.get("decision") != "accepted_default_off_broad_market_low_extension_notional":
    ...
    variants = [
        _variant_payload(
            variant_name=name,
            high_volatility_20_min=values["high_volatility_20_min"],
        ...
        for name, values in HIGH_VOLATILITY_SWEEP.items()
    ]
    selected = _choose_selected(variants)
    accepted = selected["gate4"]["passed"]
    decision = (
        "accepted_default_off_broad_market_high_volatility_notional"
        if accepted
        else "rejected_broad_market_high_volatility_notional"
    )
    ...
    high_vol_payload = broad_market_candidate_notional_payload(
        1,
        {"ret5": 0.03, "realized_volatility_20": selected["high_volatility_20_min"]},
    )
    stacked_payload = broad_market_candidate_notional_payload(
        1,
        {"ret5": 0.01, "realized_volatility_20": selected["high_volatility_20_min"]},
    )
    shared_adapter_parity = {
        "passed": (
            selected["high_volatility_20_min"] == DEFAULT_CONFIG["high_volatility_20_min"]
            and selected["scalar"] == DEFAULT_CONFIG["high_volatility_notional_scalar"]
            and high_vol_payload["notional"] == 10_350.0
            and stacked_payload["notional"] == 11_902.5
        ),
        ...
        "selected_high_volatility_20_min": selected["high_volatility_20_min"],
        "selected_high_volatility_scalar": selected["scalar"],
        "rank1_high_volatility_notional_from_shared_helper": high_vol_payload["notional"],
        "rank1_low_extension_high_volatility_notional_from_shared_helper": stacked_payload["notional"],
    }
    production_impact = {
    ...
    payload: dict[str, Any] = {
        ...
        "hypothesis": (
            ...
            "20-day realized volatility >= 5.5% may represent energetic "
            "leadership rather than noise; a bounded support notional scalar can "
            "improve allocation after the accepted rank and low-extension stack."
        ),
        "alpha_hypothesis": {
        ...
        "history_check": {
            ...
            "anti_repeat": (
                ...
                "low-extension rule, price floor, hold, slots, and universe are "
                "fixed; only a new realized-volatility support field changes."
            ),
            "selection_note": (
                "The raw highest-EV passing variant touched exactly the minimum "
                "eight trades. The selected variant keeps nine adjusted trades "
                "across all windows to reduce sample fragility."
            ),
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "broad_market_high_volatility_notional_scalar",
        "single_causal_variable": (
            "20-day realized-volatility paper notional support for fixed broad-market selected candidates"
        ),
        ...
        "backtest_protocol": (
            ...
            "exp-20260520-002 low-extension broad-market adapter is the before "
            "state; after state uses the shared high-volatility notional helper."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "selected_variant": {
            ...
            "high_volatility_20_min": selected["high_volatility_20_min"],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "candidate_pool/capital allocation: energetic broad-market paper candidates with high realized volatility deserve a bounded support scalar.",
            "2_past_similar_experiments": "Broad-market price-floor, rank-notional, and low-extension ret5 were accepted; broad-market realized-volatility support was not previously tested.",
            "3_single_variable": "Only high_volatility_20_min/scalar changes inside one notional-support field; eligibility, rank profile, low-extension, hold, slots, and universe controls stay fixed.",
        ...
        "interpretation": (
            "High 20-day realized volatility is a useful broad-market paper "
            "allocation field when bounded and layered after the accepted "
            "price-floor, rank, and low-extension stack. This remains default-off "
            "paper observation, not live or core expansion."
        ),
        "rejection_reason": None if accepted else "Best high-volatility support variant failed Gate 4.",
        "next_evidence_needed": (
            "Collect forward broad-market paper outcomes with realized-volatility "
            "metadata; do not retry adjacent volatility thresholds without new "
        ...
        "known_risks": [
            ...
            "High realized volatility can amplify drawdowns if promoted beyond default-off paper without a separate gate.",
        ],
        "related_files": {

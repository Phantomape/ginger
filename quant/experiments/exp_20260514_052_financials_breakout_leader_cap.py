"""exp-20260514-052: Financials breakout sector-leader cap scout.

Tests one production-visible allocation variable on the accepted core stack:
already-qualified Financials breakout signals that are marked as sector leaders
may be capped too tightly by the generic max-position cap. This does not alter
entries, exits, ranking, raw risk multipliers, earnings DTE treatment, LLM/news,
portfolio heat, or slots.
"""
...
EXPERIMENT_SLUG = "financials_breakout_leader_cap"
CAP_KEY = "breakout_financials_sector_leader_max_position_pct_applied"
...
def _resize_with_cap(
    ...
    out = dict(sizing)
    out["breakout_financials_leader_cap_baseline_shares"] = old_shares
    out["breakout_financials_leader_cap_raw_shares"] = raw_shares
    out["breakout_financials_leader_cap_old_cap_shares"] = old_cap_shares
    out["breakout_financials_leader_cap_new_cap_shares"] = new_cap_shares
    out["breakout_financials_leader_cap_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "financials_sector_leader": sig.get(
                            ...
                            "raw_shares": adjusted_sizing.get(
                                "breakout_financials_leader_cap_raw_shares"
                            ),
                            "old_cap_shares": adjusted_sizing.get(
                                "breakout_financials_leader_cap_old_cap_shares"
                            ),
                            "new_cap_shares": adjusted_sizing.get(
                                "breakout_financials_leader_cap_new_cap_shares"
                            ),
                            "breakout_financials_dte_risk_multiplier": sizing.get(
                                "breakout_financials_dte_risk_multiplier_applied"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Financials Breakout Sector-Leader Cap",
            "",
...
def _write_json(path: Path, payload: dict[str, Any]) -> None:
    ...
    path.write_text(
        json.dumps(_safe(payload), ensure_ascii=True, sort_keys=True, indent=2) + "\n",
        encoding="utf-8",
    )
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if selected["passed"]
        else "rejected_financials_breakout_leader_cap"
    )
    interpretation = (
        "Financials breakout sector-leader signals were cap-bound and the selected cap improved the canonical three-window stack without EV regression."
        if selected["passed"]
        else "Financials breakout sector-leader cap expansion did not clear the canonical three-window gate."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Financials sector-leader effects have survived as narrow allocation "
            "rules in trend signals. The separate breakout path may also be "
            "cap-bound when the same production-visible sector-leader field is true."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "max_position_pct_for_financials_breakout_sector_leaders",
        "single_causal_variable": (
            "max_position_pct for breakout_long Financials signals with financials_sector_leader=true"
        ),
        "parameters": {
            "cap_sweep": CAP_SWEEP,
            "baseline_generic_max_position_pct": 0.40,
            ...
            "locked_variables": [
                ...
                "breakout Financials earnings DTE multiplier",
        ...
        "historical_experiment_check": {
            "similar_prior_results": {
                "exp-20260514-023": (
                    "Accepted Financials sector-leader trend cap at 50%; this run tests the separate breakout path."
                ),
                "exp-20260514-030": (
                    "Accepted Financials sector-leader plus mid-dispersion trend cap at 55%; this run leaves that trend sleeve unchanged."
                ),
                "exp-20260514-012": (
                    "Rejected broad non-Financials sector-relative leader top-up; this run stays inside the proven Financials field."
                ),
                "exp-20260511-024": (
                    "Rejected Energy pair-state core risk; this run avoids broad sector pair-state allocation."
                ),
            },
            "why_not_llm_or_space": (
                "LLM soft-ranking, SEC/Form4 semantics, and more Space same-sample refinements are field/outcome limited; this run uses deterministic production-visible fields."
            ),
        },
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital allocation: breakout_long Financials sector leaders may be cap-bound like the accepted trend Financials sector-leader sleeve."
            ),
            "2_history_check": (
                "Trend Financials sector-leader caps were accepted; broad non-Financials sector-relative top-up and Energy pair-state allocation were rejected. No prior Financials breakout sector-leader cap was found."
            ),
            "3_single_causal_variable": (
                "breakout Financials sector-leader max_position_pct only"
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_052_financials_breakout_leader_cap.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "risk_engine financials_sector_leader",
                ...
                "portfolio_engine breakout_financials_dte_risk_multiplier_applied",
            ],
            "passed": gate2["passed"],
        },
        "gate3": {
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, add a shared constant and cap branch in portfolio_engine.size_signals, include the attribution key in backtester.py, and add focused production/backtest parity tests before live orders change."
            ),
        },
        ...
        "next_evidence_needed": (
            ...
            if selected["passed"]
            else "Do not retry nearby Financials breakout leader cap values without forward cap-room evidence or a new production-visible breakout-quality discriminator."
        ),
        "related_files": [

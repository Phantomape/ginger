"""exp-20260519-037: broad-market rank-notional paper profile.

Alpha search follow-up to exp-20260519-036. The accepted broad-market
price-floor leadership pool is fixed. This run changes one causal variable:
the default-off paper notional assigned by same-day source rank.
...
EXPERIMENT_ID = "exp-20260519-037"
EXPERIMENT_SLUG = "broad_market_rank_notional_profile"
BASELINE_EXPERIMENT_ID = "exp-20260519-036"
CONTROL_EXPERIMENT_ID = "exp-20260519-033"
...
RANK_NOTIONAL_SWEEP: OrderedDict[str, list[float]] = OrderedDict(
    [
        ("flat_baseline", [1.00, 1.00, 1.00]),
        ("rank1_105_rank3_95", [1.05, 1.00, 0.95]),
        ("rank1_110_rank3_90", [1.10, 1.00, 0.90]),
        ("rank1_115_rank3_85", [1.15, 1.00, 0.85]),
        ("rank1_120_rank3_80", [1.20, 1.00, 0.80]),
        ("rank12_110_rank3_80", [1.10, 1.10, 0.80]),
        ("rank2_110_rank3_90", [1.00, 1.10, 0.90]),
        ("rank2_115_rank3_85", [1.00, 1.15, 0.85]),
        ("rank2_120_rank3_80", [1.00, 1.20, 0.80]),
    ]
)
...
def _simulate_window(
    *,
    ...
    cfg = {**PROFILE_CONFIG, "rank_notional_multipliers": multipliers}
    ...
    for day in days:
        ...
        for rank, feature in enumerate(selected, start=1):
            ...
            if trade is None:
                continue
            trade["window"] = label
            trade["rank_notional_profile"] = list(multipliers)
            trades.append(trade)
...
def _variant_payload(
    *,
    ...
    gate4_passed = bool(
        ...
        and delta["windows_ev_improved"] >= p35.MIN_EV_IMPROVED_WINDOWS
    ...
    return {
        ...
        "rank_notional_multipliers": list(multipliers),
    ...
    return [
        {
            ...
            "rank_notional_multipliers": row["rank_notional_multipliers"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Rank-Notional Profile",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {gate} | {trades} | {ev:+.4f} | ${pnl:+,.2f} | {wi} | {wr} | {dd:+.4%} | {single} | {top5} |".format(
...
def build_payload() -> dict[str, Any]:
    ...
    if baseline_payload.get("decision") != "accepted_default_off_broad_market_shared_paper_adapter":
        raise RuntimeError(f"Unexpected baseline decision: {baseline_payload.get('decision')}")
    control_metrics = baseline_payload["before_metrics"]
    ...
    variants = [
        ...
        for name, multipliers in RANK_NOTIONAL_SWEEP.items()
    ]
    selected = _choose_best(variants)
    accepted = selected["gate4"]["passed"] and selected["variant_name"] != "flat_baseline"
    decision = (
        "accepted_default_off_broad_market_rank_notional_profile"
        if accepted
        else "rejected_broad_market_rank_notional_profile"
    )
    ...
    shared_adapter_parity = {
        "passed": list(DEFAULT_CONFIG["rank_notional_multipliers"])
        == selected["rank_notional_multipliers"],
        "shared_rule_version": RULE_VERSION,
        "rank_notional_rule_version": RANK_NOTIONAL_RULE_VERSION,
        "default_config_rank_notional_multipliers": list(DEFAULT_CONFIG["rank_notional_multipliers"]),
        "selected_rank_notional_multipliers": selected["rank_notional_multipliers"],
        "rank1_multiplier_from_shared_helper": broad_market_rank_notional_multiplier(1),
        "rank2_multiplier_from_shared_helper": broad_market_rank_notional_multiplier(2),
        "rank3_multiplier_from_shared_helper": broad_market_rank_notional_multiplier(3),
    }
    production_impact = {
    ...
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted broad-market price-floor leadership paper sleeve can "
            "increase replacement value by routing more default-off paper "
            "notional to rank-1 candidates and less to rank-3 candidates while "
            "leaving candidate eligibility unchanged."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / candidate_pool",
            "playbook_alignment": (
                "Matches allocation-over-filtering and all-market sleeve "
                "governance; avoids LLM soft-ranking, core universe expansion, "
                "and nearby price/ret20/ret60/near-high/volume retunes."
            ),
        },
        "history_check": {
            "nearby_experiments": [
                "exp-20260519-034 rejected broad-market leadership without price floor",
                "exp-20260519-035 observed price_floor_40 passing Gate 4",
                "exp-20260519-036 accepted shared default-off broad-market paper adapter",
            ],
            "anti_repeat": (
                "The candidate profile, price floor, hold, slots, and universe "
                "are fixed; only paper notional by selected rank changes."
            ),
        },
        "change_type": "default_off_paper_capital_allocation",
        "changed_variable": "broad_market_rank_notional_profile",
        "single_causal_variable": (
            "rank-based paper notional multipliers for the fixed broad-market "
            "price_floor_40 selected candidates"
        ),
        ...
        "backtest_protocol": (
            ...
            "exp-20260519-036 flat-notional broad-market shared adapter is the "
            "before state; after state uses the shared rank-notional helper."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "selected_variant": {
            ...
            "rank_notional_multipliers": selected["rank_notional_multipliers"],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "capital allocation/candidate_pool: rank-1 broad-market paper candidates deserve more notional than rank-3 candidates.",
            "2_past_similar_experiments": "exp036 accepted the fixed candidate pool with flat notional; no broad-market rank-notional profile has been tested.",
            "3_single_variable": "Only rank_notional_multipliers change; eligibility, ranking, hold, slots, and universe controls stay fixed.",
            ...
            "5_reproducibility": "Script, shared helper, tests, JSON artifact, log, ticket, markdown artifact, and docs JSONL identify windows, parameters, and metrics.",
        },
        "interpretation": (
            "Rank-depth within the broad-market leadership sleeve has usable "
            "allocation value. This remains default-off paper evidence, not a "
            "live order or core universe promotion."
        ),
        "rejection_reason": None
        if accepted
        else "Best rank-notional profile failed Gate 4 or did not beat flat baseline.",
        "next_evidence_needed": (
            "Collect forward broad-market paper outcomes under the rank-notional "
            "profile; do not retune adjacent broad-market thresholds without "
            "new forward evidence or a distinct production-visible field."
        ),
        "why_not_other_changes": [
            ...
            "No state-surface scalar mining.",
        ],
        "known_risks": [
            "Historical replay depends on the large local exp-20260519-030 warehouse.",
            "Forward paper feed maintenance remains required before any live adapter discussion.",
            "The profile increases rank-1 exposure, so forward concentration and tail loss must stay monitored.",
        ],
        "related_files": {

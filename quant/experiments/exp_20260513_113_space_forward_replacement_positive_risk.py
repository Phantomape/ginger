...
Tests one causal variable on top of the accepted exp-20260513-110 default-off
Space stack: an additional risk scalar for official Space signals whose closed
forward event-state ledger profile is both 10d cash-positive and 10d
same-theme replacement-positive.

This avoids LLM soft-ranking, noisy ticker expansion, and another
source-diversity interaction retune. The alpha question is whether already
observed forward replacement value can identify the Space catalyst profiles
that deserve incremental default-off risk.
"""
...
from space_catalyst_sleeve import space_catalyst_forward_replacement_positive_profiles


EXPERIMENT_ID = "exp-20260513-113"
STEM = "space_forward_replacement_positive_risk"
BEFORE_EXPERIMENT_ID = "exp-20260513-110"
...
FORWARD_REPLACEMENT_POSITIVE_RISK_SCALARS = (
    0.50,
    0.75,
    0.90,
    1.00,
...
MIN_AVG_CASH_RELATIVE_PNL = 0.0
MIN_AVG_SAME_THEME_REPLACEMENT_VALUE = 0.0
...
def _field_check_forward_replacement_profile() -> dict[str, Any]:
    ...
    if not path.exists():
        return {
            ...
            "reason": "missing_event_state_ledger",
        }
    ...
    for row_number, line in enumerate(
        ...
        if cash is None or same_theme is None:
            continue
        key = (str(row.get("event_id")), ticker)
        prior = latest_by_event_ticker.get(key)
        if prior is None or str(row.get("asof_date")) >= str(prior.get("asof_date")):
            latest_by_event_ticker[key] = row

    profiles = space_catalyst_forward_replacement_positive_profiles(
        list(latest_by_event_ticker.values()),
        included_tickers=sorted(official_tickers),
    ...
    return {
        ...
        "horizon": FORWARD_REPLACEMENT_HORIZON,
        "min_avg_cash_relative_pnl": MIN_AVG_CASH_RELATIVE_PNL,
        "min_avg_same_theme_replacement_value": (
            MIN_AVG_SAME_THEME_REPLACEMENT_VALUE
        ),
        "closed_official_event_count": len(latest_by_event_ticker),
...
def _run_forward_replacement_variant(
    label: str,
    forward_replacement_scalar: float,
    *,
    forward_replacement_gate: dict[str, Any],
    ...
    forward_tickers = set(forward_replacement_gate["target_tickers"])
    forward_profiles = forward_replacement_gate["profiles"]
    ...
    forward_adjustments: list[dict[str, Any]] = []
    counts: Counter[str] = Counter()

    def size_with_forward_replacement_scalar(
        ...
        for signal in sized:
            ...
            if ticker in source_diverse_tickers and sizing:
                ...
                source_diversity_adjustments.append(
                    {
                        "ticker": ticker,
                ...
                if is_peer_leader and is_iwm_leader:
                    ...
                    shares_before_peer_iwm = int(sizing.get("shares_to_buy") or 0)
                    source_diversity_exp._scale_sizing(
                        sizing,
                    ...
                    peer_iwm_adjustments.append(
                        {
                            ...
                            "shares_before_scalar": shares_before_peer_iwm,
                            "shares_after_scalar": int(sizing.get("shares_to_buy") or 0),
                            "source_diversity_profile": profile,
                            "space_peer_momentum_state": signal.get(
                                "space_peer_momentum_state"
            ...
            if ticker in forward_tickers and sizing:
                counts["forward_replacement_positive_eligible_signal"] += 1
                counts[f"forward_replacement_positive_eligible_{ticker}"] += 1
                shares_before_forward = int(sizing.get("shares_to_buy") or 0)
                source_diversity_exp._scale_sizing(
                    sizing,
                    forward_replacement_scalar,
                    portfolio_value,
                    "space_forward_replacement_positive_risk",
                )
                shares_after_forward = int(sizing.get("shares_to_buy") or 0)
                if shares_after_forward != shares_before_forward:
                    counts["forward_replacement_positive_changed_signal"] += 1
                    counts[f"forward_replacement_positive_changed_{ticker}"] += 1
                forward_adjustments.append(
                    {
                        "ticker": ticker,
                        "strategy": signal.get("strategy"),
                        "marker": "space_forward_replacement_positive_risk",
                        "scalar": forward_replacement_scalar,
                        "shares_before_scalar": shares_before_forward,
                        "shares_after_scalar": shares_after_forward,
                        "forward_replacement_profile": forward_profiles.get(ticker),
                        "space_peer_momentum_state": signal.get(
                            "space_peer_momentum_state"
                        ),
                        "space_peer_excess_momentum_20d_pct": signal.get(
    ...
    portfolio_engine.size_signals = size_with_forward_replacement_scalar
    try:
        by_window: dict[str, Any] = {}
        for name, window in source_diversity_exp.WINDOWS.items():
            before_source = len(source_diversity_adjustments)
            before_peer = len(peer_adjustments)
            before_iwm = len(iwm_adjustments)
            before_peer_iwm = len(peer_iwm_adjustments)
            before_forward = len(forward_adjustments)
            ...
            by_window[name] = {
                ...
                "source_diversity_adjustment": (
                    source_diversity_exp._adjustment_summary(
                        source_diversity_adjustments[before_source:]
                    )
                ),
                "source_diversity_peer_leader_adjustment": (
                    source_diversity_exp._adjustment_summary(
                        peer_adjustments[before_peer:]
                    )
                ),
                ...
                "source_diversity_peer_iwm_leader_adjustment": (
                    source_diversity_exp._adjustment_summary(
                        peer_iwm_adjustments[before_peer_iwm:]
                    )
                ),
                "forward_replacement_positive_adjustment": (
                    source_diversity_exp._adjustment_summary(
                        forward_adjustments[before_forward:]
                    )
                ),
                "forward_replacement_positive_counts": count_delta,
            }
        ...
        return {
            ...
            "forward_replacement_positive_adjustment_summary": (
                source_diversity_exp._adjustment_summary(forward_adjustments)
            ),
            "forward_replacement_positive_counts": dict(sorted(counts.items())),
            "forward_replacement_positive_adjustment_sample": (
                forward_adjustments[:25]
            ),
        }
    finally:
...
def _runtime_state_gate(before: dict[str, Any]) -> dict[str, Any]:
    ...
    for payload in before.get("by_window", {}).values():
        counts = payload.get("forward_replacement_positive_counts") or {}
        eligible += int(counts.get("forward_replacement_positive_eligible_signal", 0) or 0)
        changed += int(counts.get("forward_replacement_positive_changed_signal", 0) or 0)
        for key, value in counts.items():
            prefix = "forward_replacement_positive_eligible_"
            if key.startswith(prefix):
                by_ticker[key.replace(prefix, "")] += int(value or 0)
        summary = payload.get("forward_replacement_positive_adjustment") or {}
        for row in summary.get("sample_adjusted") or []:
            samples.append(
                {
                    "ticker": row.get("ticker"),
                    "strategy": row.get("strategy"),
                    "space_peer_momentum_state": row.get(
                        "space_peer_momentum_state"
                    ),
    ...
    return {
        ...
        "eligible_forward_replacement_positive_signal_count": eligible,
        "changed_forward_replacement_positive_signal_count_at_neutral_before": changed,
        "eligible_by_ticker": dict(sorted(by_ticker.items())),
        "sample_forward_replacement_positive_rows": samples,
    }
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["forward_replacement_positive_counts"].get(
            "forward_replacement_positive_changed_signal",
            0,
        )
    )
    eligible_count = int(
        variant["forward_replacement_positive_counts"].get(
            "forward_replacement_positive_eligible_signal",
            0,
        )
    )
    scalar = float(
        variant["parameters"]["space_forward_replacement_positive_scalar"]
    )
    ...
    return {
        ...
        "eligible_forward_replacement_positive_signal_count": eligible_count,
        "changed_forward_replacement_positive_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space forward replacement-positive risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_forward_replacement_positive_scalar` after the accepted "
            "exp-110 Space stack. The bucket is official Space tickers whose "
            "closed event-state ledger profile is 10d cash-positive and "
            "same-theme replacement-positive. Candidate pool, event labels, "
            "ranking, targets, stops, LLM/news, and live Space slots stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_forward_replacement_positive_scalar']}`"
        ),
        (
            "- Aggregate delta vs exp-110: "
            f"EV `{gate['aggregate_delta_vs_before']['expected_value_score_sum']:.6f}`, "
            f"PnL `{gate['aggregate_delta_vs_before']['total_pnl_sum']:.2f}`"
        ),
        (
            "- Forward replacement-positive signals changed: "
            f"`{gate['changed_forward_replacement_positive_signal_count']}` of "
            f"`{gate['eligible_forward_replacement_positive_signal_count']}` eligible"
        ),
        (
            "- Target tickers: "
            f"`{', '.join(payload['gate2_field_checks']['forward_replacement_profile']['target_tickers'])}`"
        ),
        "",
...
def run() -> dict[str, Any]:
    ...
    forward_replacement_gate = _field_check_forward_replacement_profile()

    before = _run_forward_replacement_variant(
        "accepted_exp110_forward_replacement_positive_neutral",
        1.0,
        forward_replacement_gate=forward_replacement_gate,
    ...
    gate2 = {
        ...
        "forward_replacement_profile": forward_replacement_gate,
        "forward_replacement_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
            ...
            forward_replacement_gate["passed"],
            runtime_state_gate["passed"],
        ]
    )

    variants = [
        _run_forward_replacement_variant(
        ...
        for scalar in FORWARD_REPLACEMENT_POSITIVE_RISK_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested forward replacement-positive scalar improved aggregate "
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "On top of accepted exp-110 Space default-off stack, official Space "
            "signals whose closed event-state ledger profile is both 10d "
            "cash-positive and same-theme replacement-positive may deserve "
            "incremental default-off risk. This tests forward replacement value "
            "as the catalyst-quality allocator requested by the Space playbook "
            "without changing the Space pool, event metadata, ranking, targets, "
            "stops, LLM boundary, or live slots."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_forward_replacement_positive_risk_scalar",
        "single_causal_variable": (
            "extra risk scalar for official Space tickers with closed 10d "
            "cash-positive and same-theme replacement-positive forward profiles"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "risk allocation: scale official Space signals whose closed "
                "event-state ledger shows positive cash and same-theme 10d "
                "replacement value."
            ),
            "q2_prior_experiments": [
                "exp-20260513-038/039/108/110 accepted source-diversity plus peer/IWM interactions.",
                "exp-20260513-111 rejected strong peer-excess source-diversity retuning.",
                "exp-20260512-902 showed cash/SPY/QQQ/same-theme forward attribution coverage is mature enough for slicing, while core replacement remains blocked.",
                "exp-20260513-021/106 rejected single-window satellite/regulatory slices.",
            ],
            "q3_single_causal_variable": (
                "Only the forward replacement-positive scalar changes; accepted "
                "exp-110 risk stack and all entry/exit/ranking/candidate variables "
                "stay fixed."
            ),
            "q4_acceptance_standard": (
                ...
                "and real adjusted forward replacement-positive signals."
            ),
            "q5_reproducibility": (
        ...
        "gate1": {
            ...
            "known_bias": (
                ...
                "from a 2026-05-10 research universe. Forward replacement profiles "
                "come from closed 2026 event-state ledger rows, so the helper remains "
                "default-off metadata until future forward use; live Space slots stay zero."
            ),
        },
        ...
        "parameters": {
            ...
            "target_tickers": forward_replacement_gate["target_tickers"],
            "forward_replacement_horizon": FORWARD_REPLACEMENT_HORIZON,
            "min_avg_cash_relative_pnl": MIN_AVG_CASH_RELATIVE_PNL,
            "min_avg_same_theme_replacement_value": (
                MIN_AVG_SAME_THEME_REPLACEMENT_VALUE
            ),
            ...
            "tested_forward_replacement_positive_scalars": list(
                FORWARD_REPLACEMENT_POSITIVE_RISK_SCALARS
            ),
            "locked_variables": [
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_forward_replacement_positive_risk"
            if decision == "accepted"
            else "rejected_space_forward_replacement_positive_risk"
        ),
        "rejection_reason": rejection_reason,
        "next_evidence_needed": (
            "Promote only as shared default-off Space metadata/helper using the "
            "forward event-state ledger profile, then keep live Space slots at zero "
            "until core replacement value also closes."
            if decision == "accepted"
            else (
                "Do not promote closed-forward replacement-positive scalar from this "
                "frozen replay. Prefer more closed forward rows or core replacement "
                "value before using event-state outcomes as allocation policy."
            )
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains label-thin; noisy ticker expansion and "
            "mature satcom breadth have failed; recent source-diversity and peer "
            "magnitude retunes are exhausted. This moves to the playbook's forward "
            "replacement-value axis instead of another descriptive source slice."
        ),
        "known_risks": [
            "The Space sleeve remains default-off and historical Space snapshots are frozen research copies.",
            "Forward replacement profiles are closed 2026 outcomes and should not be treated as proof for live routing until future decisions confirm the bucket.",
            "Core replacement value remains unavailable in the event-state ledger.",
        ],
    }
    return payload
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_forward_replacement_positive_signal_count": result[
            "best_variant_gate"
        ]["changed_forward_replacement_positive_signal_count"],

"""exp-20260513-024: green-candle gap-cushion risk allocation.

Tests one production-visible state variable on the accepted core stack:
already-qualified trend/breakout signals whose accepted signal-day own candle
is green and whose stop is not in the existing tight-gap warning zone. This is
a cap-aware post-sizing risk top-up scout, not an entry filter, ranking change,
exit change, universe change, or production default change.
"""
...
EXPERIMENT_ID = "exp-20260513-024"
EXPERIMENT_SLUG = "green_gap_cushion_risk"
MULTIPLIER_KEY = "green_gap_cushion_risk_multiplier_applied"
...
def _is_gap_cushion_state(sig: dict[str, Any]) -> bool:
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        for sig in enriched:
            sig["green_gap_cushion_state"] = _is_gap_cushion_state(sig)
            sig["green_gap_cushion_min_pct"] = GAP_WARNING_MIN_CUSHION_PCT
        return enriched
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["green_gap_cushion_baseline_shares"] = shares
    out["green_gap_cushion_desired_shares"] = desired_shares
    out["green_gap_cushion_cap_shares"] = cap_shares
    out["green_gap_cushion_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("green_gap_cushion_state") and sizing.get("shares_to_buy"):
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "green_gap_cushion_min_pct": GAP_WARNING_MIN_CUSHION_PCT,
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Green Gap-Cushion Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_green_gap_cushion_risk"
    )
    interpretation = (
        "Green-candle gap-cushion core risk top-up cleared the canonical three-window gate and requires shared policy implementation before production use."
        if passed
        else "Green-candle gap-cushion core risk top-up did not clear the canonical three-window gate; do not promote this state variable on the frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted own-green signal-day top-up may be safer when the entry stop has at least the existing 2% gap cushion. This should preserve follow-through allocation while avoiding the tight-stop cases that can expand drawdown."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "green_gap_cushion_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for trend/breakout signals where signal_day_ticker_green_candle is true and gap_vulnerability_pct >= 0.02"
        ),
        "parameters": {
            "state_definition": {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "core risk allocation using accepted own-green confirmation plus the existing non-tight-stop gap cushion state"
            ),
            "2_history_check": {
                "exp-20260513-007": (
                    "accepted own-green 1.05x top-up; this tests only a narrower additional overlay."
                ),
                "exp-20260513-009": (
                    "upper-quartile close-location scalar was drawdown-fragile; this uses stop cushion instead of close location."
                ),
                "exp-20260513-013": (
                    "green momentum acceleration regressed old_thin; this avoids momentum acceleration."
                ),
                "exp-20260513-018": (
                    "own-green plus RS20 plus high TQS was underpowered; this uses an execution-risk discriminator instead of another setup-quality stack."
                ),
                "exp-20260513-023": (
                    "green momentum deceleration improved EV but failed drawdown guardrail; this tests a stronger drawdown discriminator rather than another momentum slice."
                ),
                "llm_soft_ranking": (
                    "data remains thin, so this run avoids LLM soft-ranking."
                ),
                "space_profile_scalars": (
                    "recent frozen-sample Space profile slicing is over-mined; this run returns to core allocation."
                ),
            },
            "3_single_causal_variable": (
                "green_gap_cushion_risk_multiplier with fixed production-visible own-green plus existing gap-cushion state"
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_024_green_gap_cushion_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        if passed
            ...
            "Use forward own-green execution-risk attribution or a different production-visible state variable before retrying own-green overlays."
        ),
        "related_files": [
            "quant/experiments/exp_20260513_024_green_gap_cushion_risk.py",
            "data/experiments/exp-20260513-024/green_gap_cushion_risk.json",
            "docs/experiments/logs/exp-20260513-024.json",
            "docs/experiments/tickets/exp-20260513-024.json",
            "docs/experiments/artifacts/exp-20260513-024_green_gap_cushion_risk.md",

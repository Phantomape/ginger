...
Tests one causal variable on top of the accepted exp-20260513-038 default-off
Space source-diversity stack: an additional risk scalar for official Space
signals that are both source-diverse and Space peer momentum leaders.

This avoids LLM soft-ranking, ticker-pool expansion, target/stop changes, and
another broad source-diversity scalar retune. The alpha question is whether the
already useful diversified official catalyst evidence becomes materially more
valuable when it also has in-basket relative strength.
"""
...
EXPERIMENT_ID = "exp-20260513-039"
STEM = "space_source_diversity_peer_leader_risk"
BEFORE_EXPERIMENT_ID = "exp-20260513-038"
ACCEPTED_SOURCE_DIVERSITY_RISK_SCALAR = 1.075
SOURCE_DIVERSITY_PEER_LEADER_RISK_SCALARS = (
    0.75,
    0.90,
    1.00,
    1.025,
...
def _append_jsonl(path: Path, entry: dict[str, Any]) -> None:
    serialized = json.dumps(_safe(entry), separators=(",", ":"), sort_keys=True)
    if path.exists():
...
def _run_peer_leader_variant(
    label: str,
    peer_leader_scalar: float,
    *,
    ...
    def size_with_source_diversity_peer_leader(
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                ...
                shares_after_source = int(sizing.get("shares_to_buy") or 0)
                diversity_adjustments.append(
                    {
                        ...
                        "shares_after_scalar": shares_after_source,
                ...
                if signal.get("space_peer_momentum_state") == "leader":
                    counts["peer_leader_eligible_signal"] += 1
                    counts[f"peer_leader_eligible_{ticker}"] += 1
                    ...
                    shares_after_peer = int(sizing.get("shares_to_buy") or 0)
                    if shares_after_peer != shares_before_peer:
                        counts["peer_leader_changed_signal"] += 1
                        counts[f"peer_leader_changed_{ticker}"] += 1
                    peer_adjustments.append(
                        {
                            "ticker": ticker,
                            "strategy": signal.get("strategy"),
                            "marker": "space_source_diversity_peer_leader_risk",
                            "scalar": peer_leader_scalar,
                            "shares_before_scalar": shares_before_peer,
                            "shares_after_scalar": shares_after_peer,
                            "source_diversity_profile": profile,
                            "space_peer_momentum_state": signal.get(
                                "space_peer_momentum_state"
                            ),
    ...
    portfolio_engine.size_signals = size_with_source_diversity_peer_leader
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "source_diversity_peer_leader_counts": count_delta,
            }
        metrics_by_window = {
            name: row["metrics"] for name, row in by_window.items()
        }
        return {
            "label": label,
            "parameters": {
                ...
                "space_source_diversity_peer_leader_scalar": peer_leader_scalar,
            ...
            "source_diversity_peer_leader_adjustment_counts": dict(
                sorted(counts.items())
            ),
            "source_diversity_peer_leader_adjustment_sample": peer_adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["source_diversity_peer_leader_adjustment_counts"].get(
            "peer_leader_changed_signal", 0
        )
    )
    eligible_count = int(
        variant["source_diversity_peer_leader_adjustment_counts"].get(
            "peer_leader_eligible_signal", 0
        )
    )
    scalar = float(
        variant["parameters"]["space_source_diversity_peer_leader_scalar"]
    )
    ...
    return {
        ...
        "eligible_source_diversity_peer_leader_signal_count": eligible_count,
        "changed_source_diversity_peer_leader_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity peer-leader risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_source_diversity_peer_leader_scalar` after the accepted "
            "source-diversity `1.075x` helper. Candidate pool, source-diversity "
            "definition, rankings, targets, stops, LLM/news, and live Space slots "
            "stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_source_diversity_peer_leader_scalar']}`"
        ),
        (
            "- Aggregate delta vs exp-038: "
            f"EV `{gate['aggregate_delta_vs_before']['expected_value_score_sum']:.6f}`, "
            f"PnL `{gate['aggregate_delta_vs_before']['total_pnl_sum']:.2f}`"
        ),
        (
            "- Peer-leader source-diverse signals changed: "
            f"`{gate['changed_source_diversity_peer_leader_signal_count']}` of "
            f"`{gate['eligible_source_diversity_peer_leader_signal_count']}` eligible"
        ),
        "",
...
def run() -> dict[str, Any]:
    ...
    before = source_diversity_exp._run_source_diversity_variant(
        "accepted_exp038_source_diversity_stack",
        ACCEPTED_SOURCE_DIVERSITY_RISK_SCALAR,
    ...
    variants = [
        ...
        for scalar in SOURCE_DIVERSITY_PEER_LEADER_RISK_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested source-diverse peer-leader scalar improved aggregate EV/PnL "
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "On top of accepted exp-038 Space source-diversity stack, official "
            "Space signals with diversified non-attention official catalyst evidence "
            "may deserve extra risk only when the ticker is also a Space peer "
            "momentum leader. This tests one risk-allocation scalar without changing "
            "the Space pool, event metadata, rankings, targets, stops, LLM boundary, "
            "or live slots."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_source_diversity_peer_leader_risk_scalar",
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                ...
                "when peer leadership confirms relative strength."
            ),
            "q2_prior_experiments": [
                ...
                "exp-20260513-015 accepted government-contract peer-leader risk.",
                "exp-20260513-019 rejected customer-source peer-nonleader risk.",
                "LLM soft-ranking is label-thin and candidate-pool expansion has repeatedly failed.",
            ],
            "q3_single_causal_variable": (
                "Only the extra source-diversity peer-leader scalar changes; the "
                "accepted source-diversity scalar stays 1.075x."
            ),
            "q4_acceptance_standard": (
                ...
                "and real adjusted peer-leader signals."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            ...
            "tested_source_diversity_peer_leader_scalars": list(
                SOURCE_DIVERSITY_PEER_LEADER_RISK_SCALARS
            ),
            "locked_variables": [
                "official Space candidate pool",
                "accepted exp-038 source-diversity definition",
                "accepted exp-038 source-diversity scalar",
                "accepted exp-032 attention-overlay stack",
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_source_diversity_peer_leader_risk"
            if decision == "accepted"
            else "rejected_space_source_diversity_peer_leader_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not keep tuning source-diversity peer-leader scalars on these "
                "frozen snapshots. Prefer forward replacement-value evidence by "
                "source/peer bucket or a genuinely new official catalyst field."
            )
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited, noisy ticker expansion and "
            "mature satcom breadth have failed, and broad Space breakout/profile "
            "retunes are exhausted. This isolates a production-visible catalyst "
            "quality plus peer-leadership interaction."
        ),
        "known_risks": [
            "The Space sleeve remains default-off and historical Space snapshots are frozen research copies.",
            "This is an interaction on a small sample and still requires forward replacement-value validation before live routing.",
        ],
    }
    return payload
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_source_diversity_peer_leader_signal_count": result[
            "best_variant_gate"
        ]["changed_source_diversity_peer_leader_signal_count"],

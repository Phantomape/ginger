"""exp-20260607-015: rates-relief duration-growth laggard candidate pool.

Replay-only alpha search. It tests one free-OHLCV relation source: when TLT
and QQQ both show same-day relief, admit one liquid high-duration growth stock
that has started reacting but still lags QQQ. Paper entry is next-open with a
...
EXPERIMENT_ID = "exp-20260607-015"
STEM = "rates_relief_duration_growth_laggard"
TRIAL_FAMILY = "rates_relief_duration_growth_laggard_candidate_pool"
TRIAL_VARIANT_ID = "tlt_qqq_relief_growth_laggard_top1_next_open_10d_v1"
CHANGED_VARIABLE = "rates_relief_duration_growth_laggard_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

framework = base.framework
REPO_ROOT = base.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260607_015_{STEM}.json"
...
MIN_CANDIDATE_SIGNAL_RETURN = 0.001
MIN_CANDIDATE_RELATIVE_VS_SPY = -0.002
MIN_CANDIDATE_LAG_GAP_VS_QQQ = 0.002
MAX_CANDIDATE_LAG_GAP_VS_QQQ = 0.055
MIN_CANDIDATE_RET20_EXCESS_QQQ = -0.200
MAX_CANDIDATE_RET20_EXCESS_QQQ = -0.015
MIN_CANDIDATE_RET20_EXCESS_SPY = -0.100
MIN_CANDIDATE_RET60_EXCESS_SPY = -0.120
MAX_CANDIDATE_RET5 = 0.120
MIN_CANDIDATE_CLOSE_LOCATION = 0.55
MAX_CANDIDATE_REALIZED_VOL_20 = 0.110
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "window_regression",
        "thin_sample",
        "ETF_relation_relabeling",
        "duration_beta_tail_risk",
        "concentration_failed",
    ],
    "confidence_reason": (
        "Accepted macro relief and relation-aware free-OHLCV adapters show "
        "event/state transfer can work, but rates ETF and sector ETF laggard "
        "neighbors have failed. This tests a different duration-growth stock "
        "relation without tuning accepted adapters."
    ),
    "recorded_at": "2026-06-07T13:05:15+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "require a shared default-off adapter that computes the same TLT/QQQ "
        "rates-relief context, high-duration growth stock universe, lagging "
...
def _candidate_for_ticker(
    *,
    ...
    lag_gap_vs_qqq = qqq_return - signal_return
    ...
    if relative_vs_spy < MIN_CANDIDATE_RELATIVE_VS_SPY:
        return None
    if lag_gap_vs_qqq < MIN_CANDIDATE_LAG_GAP_VS_QQQ:
        return None
    if lag_gap_vs_qqq > MAX_CANDIDATE_LAG_GAP_VS_QQQ:
        return None
    ...
    score = (
        ...
        + 1.10 * lag_gap_vs_qqq
        + 0.75 * relative_vs_spy
        + 0.55 * abs(ret20_excess_qqq)
        + 0.42 * close_location
        + 0.22 * ret60_excess_spy
        + 0.035 * math.log10(max(adv20, 1.0) / 1_000_000.0)
        - 0.62 * realized_vol
        - 0.04 * max(volume_ratio - 2.2, 0.0)
    )
    return {
        "date": signal_date,
        "ticker": ticker,
        "source": "RATES_RELIEF_DURATION_GROWTH_LAGGARD_PAPER",
        "candidate_score": round(score, 6),
        "candidate_signal_day_return": round(signal_return, 6),
        "candidate_relative_vs_spy": round(relative_vs_spy, 6),
        "candidate_lag_gap_vs_qqq": round(lag_gap_vs_qqq, 6),
    ...
    *,
    ...
    candidates.sort(
        key=lambda row: (
            ...
            -float(row["candidate_lag_gap_vs_qqq"]),
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_rates_relief_duration_growth_laggard"
        if gate["passed"]
        else "rejected_rates_relief_duration_growth_laggard_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "Rates-relief days where TLT and QQQ both improve may "
                "identify liquid high-duration Technology/Growth laggards "
                "that start reacting but have not fully caught up."
            ),
            ...
            "new_evidence_type": "production_visible_free_ohlcv_rates_to_growth_relation",
            ...
            "negative_reflection": (
                "If rejected, the likely reason is that TLT/QQQ relief either "
                "arrives too late for next-open single-stock catch-up, or the "
                "selected growth laggards are just duration beta with tail "
                "risk. Do not retry by changing TLT/QQQ thresholds, growth "
                "industry keywords, lag gap, hold-day, cooldown, top-N, or "
    ...
    payload["parameters"] = {
        ...
        "min_candidate_lag_gap_vs_qqq": MIN_CANDIDATE_LAG_GAP_VS_QQQ,
        "max_candidate_lag_gap_vs_qqq": MAX_CANDIDATE_LAG_GAP_VS_QQQ,
    ...
    payload["backtest_protocol"].update(
        {
            "source": (
                ...
                "replay-only rates-relief duration-growth laggard paper overlay"
            ),
            ...
            "execution_model": (
                ...
                "TLT/QQQ context, growth stock lag, close location, liquidity, "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: rates-relief days where TLT and QQQ both "
            "improve should favor high-duration growth stocks that start "
            "reacting but still lag QQQ at the close."
        ),
        "2_history_check": {
            "exp-20260605-027": (
                "Hawkish rates selloff ETF rebalance failed by thin sample and "
                "negative PnL. This tests the opposite relief-to-single-stock "
                "relation, not ETF rotation."
            ),
            "exp-20260606-020": (
                "Macro relief leadership accepted as a shared default-off "
                "adapter; this is not an official macro-day threshold retune "
                "and uses rates ETF context instead."
            ),
            "exp-20260606-029": (
                "Sector ETF laggard failed; this avoids sector ETF labels and "
                "uses a cross-asset TLT/QQQ duration-growth relation."
            ),
            "exp-20260607-011": (
                "Precious metals ETF-to-producer lag failed; this checks a "
                "different macro transmission channel and growth-stock universe."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260607_015_rates_relief_duration_growth_laggard.py"
        ),
    }
    ...
    payload["gate3"]["note"] = (
        ...
        "duration-growth laggard candidate source is additive default-off "
    ...
    payload["interpretation"] = (
        "The rates-relief duration-growth laggard source cleared Gate 4 as a "
        ...
        if payload["gate4"]["passed"]
            ...
            "The rates-relief duration-growth laggard source did not clear "
    ...
    why_result = (
        "The relation was directionally positive in aggregate but too thin "
        "and not robust enough for retention. late_strong and mid_weak "
        "improved, but old_thin regressed by {ev:+.4f} EV and ${pnl:+,.2f}; "
        "only {count} target trades closed across the three windows versus "
        "the 20-trade floor. That pattern suggests TLT/QQQ relief captures "
        "some beta rebound, but not a durable delayed high-duration growth "
        "catch-up edge after next-open entry and costs."
    ).format(
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping TLT return, QQQ return, close-location, "
            "growth industry keywords, QQQ lag gap, ret20 excess, volume, "
            "volatility, hold-day, top-N, cooldown, or paper notional on the "
            "same frozen windows."
        ),
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Rates-Relief Duration-Growth Laggard",
            "",

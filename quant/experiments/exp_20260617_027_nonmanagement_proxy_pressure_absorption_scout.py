"""exp-20260617-027: SEC non-management proxy-pressure absorption scout.

Replay-only alpha search. The single decision hypothesis is that PIT SEC
non-management proxy-pressure filings can become positive candidate-pool events
when signal-day price action absorbs the governance/control-pressure event
versus SPY before the next-open paper entry.

This is distinct from rejected 13D/13G ownership-disclosure entries and generic
8-K/text labels: DFAN14A, PX14A6G, PREC14A, and PRRN14A are proxy solicitation
materials rather than ownership, offering, or business-update filings. The form
type itself is the event source, and no LLM/text semantics are used. No
...
EXPERIMENT_ID = "exp-20260617-027"
STEM = "nonmanagement_proxy_pressure_absorption_scout"
TRIAL_FAMILY = "free_sec_submissions_nonmanagement_proxy_pressure_candidate_pool"
TRIAL_VARIANT_ID = "nonmanagement_proxy_pressure_absorption_top1_next_open_10d_v1"
CHANGED_VARIABLE = "sec_nonmanagement_proxy_pressure_absorption_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_027_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "proxy_pressure_events_are_noisy",
        "governance_pressure_is_priced_before_next_open",
        "old_thin_window_regression",
        "accepted_distribution_comparator_not_beaten",
    ],
    "confidence_reason": (
        "Focused non-management proxy-pressure forms are a real free SEC "
        "submissions event surface with enough three-window coverage and are "
        "distinct from rejected 13D/13G ownership entries, 13F/Form144 delayed "
        "ownership, and generic 8-K/text labels. The likely disconfirmer is "
        "governance-event noise or the event being priced before next-open paper "
        "entry."
    ),
    "recorded_at": "2026-06-17T23:09:25+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        ...
        "failure_handling": (
            "missing SEC submissions cache, missing CIK mapping, missing proxy "
            "pressure event rows, missing OHLCV, missing next open, or missing "
    ...
    "parity_note": (
        ...
        "SEC DFAN14A/PX14A6G/PREC14A/PRRN14A events, acceptance-time signal date, price-absorption "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: PIT SEC non-management proxy-pressure filings "
        "(DFAN14A, PX14A6G, PREC14A, PRRN14A) may identify governance/control-"
        "pressure events that become positive only when signal-day liquid price "
        "action absorbs the event versus SPY before next-open paper entry."
    ),
    "2_history_check": {
        "exp-20260503-017": (
            "Only audited SEC/Form4 data availability and counted DEFA14A rows; "
            "it did not run a proxy-pressure candidate pool over Gate 1-4."
        ),
        "exp-20260612-015": (
            "Rejected direct SC 13D activist-stake initiation. This run does not "
            "use ownership filing-date entries or 13D/A amendments; it tests "
            "proxy solicitation forms plus price absorption."
        ),
        "exp-20260612-016": (
            "Rejected direct 13G passive-stake initiation. This run avoids "
            "passive ownership reports and annual/batch 13G staleness."
        ),
        "exp-20260610-013": (
            "Rejected generic 8-K business-update leadership labels. This run "
            "uses non-management proxy solicitation form provenance, not broad "
            "8-K item-code or phrase labels."
        ),
        "exp-20260617-023": (
            "Rejected SEC offering/prospectus price absorption. This run does "
            "not retry 424B/S-1/S-3/F-3 financing forms or their thresholds; it "
            "tests a governance/proxy-pressure submissions form family."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_027_nonmanagement_proxy_pressure_absorption_scout.py"
    ),
}
...
def _load_event_index() -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        for i in range(min(len(forms), len(filing_dates))):
            ...
            if form not in PROXY_PRESSURE_FORMS:
                continue
            ...
            events.append(
                {
                    ...
                    "form_weight": PROXY_FORM_WEIGHTS.get(form, 0.40),
                }
            )
        ...
        if events:
            ...
            index[ticker] = events
            stats["tickers_with_proxy_pressure_events"] += 1
            stats["proxy_pressure_event_count"] += len(events)
    ...
    summary = {
        ...
        "proxy_pressure_forms": sorted(PROXY_PRESSURE_FORMS),
        **dict(stats),
    }
...
def _build_quality_index(
    ...
    return index, {
        **summary,
        ...
        "field_source": "sec_submissions_proxy_pressure_forms_not_companyfacts",
    }


def _signal_date_for_event(event: dict[str, Any], dates: list[str]) -> str | None:
    filing_date = event["filing_date"]
    pos = bisect.bisect_right(dates, filing_date) if event.get("accepted_after_close") else bisect.bisect_left(dates, filing_date)
    if pos >= len(dates):
        return None
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "SEC_PROXY_PRESSURE_ABSORPTION_PAPER",
                    "candidate_score": _round(score, 6),
                    "rule_version": RULE_VERSION,
                    "source_rule_version": RULE_VERSION,
                    "known_at": "sec_proxy_pressure_form_and_signal_close_before_next_open_paper_entry",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    "proxy_pressure_form": event["form"],
                    "proxy_pressure_filing_date": event["filing_date"],
                    "proxy_pressure_accepted_after_close": event["accepted_after_close"],
                    "proxy_pressure_acceptance_datetime": event["acceptance_datetime"],
                    "proxy_pressure_accession_number": event["accession_number"],
                    "proxy_pressure_primary_document": event["primary_document"],
                    "proxy_pressure_form_weight": event["form_weight"],
                    **confirm,
                }
            )
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_sec_nonmanagement_proxy_pressure_absorption"
        if gate["passed"]
        else "rejected_sec_nonmanagement_proxy_pressure_absorption_candidate_pool"
    )
    return gate
...
def _interpretation(payload: dict[str, Any]) -> str:
    ...
    if gate4["passed"]:
        return (
            "The SEC non-management proxy-pressure absorption source cleared the numeric "
            "three-window replay screen, but remains only a replay lead because "
            "no shared daily/backtest helper was promoted."
        )
    return (
        "The SEC non-management proxy-pressure absorption source did not clear Gate 4 "
        f"(failed: {', '.join(gate4['failed_reasons']) or 'none'}). The fixed "
        "bundle tested DFAN14A/PX14A6G/PREC14A/PRRN14A proxy-pressure forms "
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_submissions_proxy_pressure_candidate_pool",
            "new_evidence_type": "sec_submissions_nonmanagement_proxy_pressure_forms_with_price_absorption",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SEC non-management proxy-pressure events are read from EDGAR submissions cache "
        "recent filings for forms DFAN14A, PX14A6G, PREC14A, and PRRN14A. "
        "The signal date is the filing date unless the SEC acceptance timestamp "
        "is after 16:00, in which case it is the next trading day. Candidates "
        "must show signal-day price absorption before next-open paper entry: "
    ...
    payload["gate2"]["runtime_fields"] = [
        "SEC submissions recent.form (DFAN14A/PX14A6G/PREC14A/PRRN14A)",
    ...
    payload["next_evidence_needed"] = (
        "If this fixed non-management proxy-pressure absorption bundle fails, do not retry "
        "by sweeping DFAN14A/PX14A6G/PREC14A/PRRN14A form lists, signal excess, close-location, "
        ...
        "materially richer PIT proxy-pressure semantics such as parsed filer "
        "identity, campaign objective, board-seat demand, vote outcome, stake/"
        "ownership context, or closed forward replacement-value observations."
    )
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping DFAN14A/PX14A6G/PREC14A/PRRN14A form lists, signal excess, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} SEC Non-Management Proxy Pressure Absorption",
            "",

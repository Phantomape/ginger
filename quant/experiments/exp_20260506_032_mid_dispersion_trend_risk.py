...
meta-allocation state map: `trend_long` trades were repeatedly profitable when
sector 20-day returns showed mid-level dispersion. This runner keeps universe,
entries, ranking, exits, add-ons, LLM/news replay, and all existing sizing rules
fixed, then applies a trend-only risk multiplier on those candidate days.
"""
...
EXPERIMENT_ID = "exp-20260506-032"
STEM = "mid_dispersion_trend_risk"
...
CUSTOM_MULTIPLIER_KEY = "mid_dispersion_trend_risk_multiplier_applied"
...
VARIANTS = OrderedDict(
    [
        ("mid_dispersion_trend_1_25x", {"risk_multiplier": 1.25}),
        ("mid_dispersion_trend_1_50x", {"risk_multiplier": 1.50}),
        ("mid_dispersion_trend_2_00x", {"risk_multiplier": 2.00}),
    ]
)
...
def _markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID}: Mid-Dispersion Trend Risk",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "No trading rule was promoted by this replay script. A passing result must move the mid-dispersion feature and sizing overlay into shared production/backtest policy before live behavior changes.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    if any_pass:
        interpretation = (
            ...
            "sector-dispersion feature and shared sizing policy used by both run.py and "
            "the backtester."
        )
    else:
        interpretation = (
            "Mid-sector-dispersion trend risk expansion did not pass the three-window "
            ...
            "state-aware trend sizing branch."
        )
    ...
    payload: dict[str, Any] = {
        ...
        "timestamp": timestamp,
        "lane": "alpha_search",
        "change_type": "capital_allocation_mid_dispersion_trend_risk",
        "alpha_hypothesis_category": "capital_allocation_meta_routing",
        "hypothesis": (
            "If sector leadership is neither too concentrated nor too uniform, then "
            "existing trend_long candidates may deserve more risk because selective "
            "leadership has enough breadth to persist without the high-dispersion "
            "fragility rejected in exp-20260506-029."
        ),
        ...
        "history_guardrails": {
            ...
            "not_broad_rotation_retry": True,
            "not_universe_expansion_retry": True,
            "why_not_simple_repeat": (
                "This tests the mid-sector-dispersion trend cohort surfaced by "
                "exp-20260506-024. It does not retry high-sector-dispersion de-risking, "
                "broad-rotation multipliers, SPY-relative leader tuning, candidate "
                "ranking, or static universe growth."
            ),
        },
        "parameters": {
            "mid_dispersion_definition": (
                "population stddev of equal-weight sector 20-day returns over the "
                "existing universe, with 0.035 < dispersion < 0.08"
            ),
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, compute sector-dispersion state in shared production/backtest "
                "enrichment and consume it from shared sizing."
            ),
        },
        "ticket": {
            "experiment_id": EXPERIMENT_ID,
            "title": "Mid-dispersion trend risk allocation",
            "decision": decision,
            "best_variant": best_variant_name,
            "next_action": (
        ...
        "related_files": [
            ...
            "quant/experiments/exp_20260506_032_mid_dispersion_trend_risk.py",
        ],
    }
    ...
    payload["experiment_log_entry"] = {
        ...
        "next_retry_requires": [
            "Do not retry nearby mid-dispersion trend-only multipliers on the same bucket if this run fails.",
            "A valid retry needs a new discriminator that materially improves at least two windows without drawdown damage.",

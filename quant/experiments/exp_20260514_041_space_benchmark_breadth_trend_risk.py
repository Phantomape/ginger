...
cash, SPY, QQQ, UFO, and ARKX deserve a conservative extra default-off risk
scalar.

This tests broad benchmark confirmation as a separate closed-forward state. It
avoids LLM soft-ranking, ticker-pool expansion, lifecycle changes, and nearby
source-diversity or delayed-absorption scalar retunes.
"""
...
EXPERIMENT_ID = "exp-20260514-041"
STEM = "space_benchmark_breadth_trend_risk"
...
MIN_10D_CASH = 0.0
MIN_10D_SPY_RELATIVE = 0.0
MIN_10D_QQQ_RELATIVE = 0.0
MIN_10D_UFO_RELATIVE = 0.0
MIN_10D_ARKX_RELATIVE = 0.0
...
def _benchmark_breadth_profile_gate() -> dict[str, Any]:
    ...
    for ticker, rows in sorted(rows_by_ticker.items()):
        ...
        passed = bool(
            ...
            and avg_10d_spy > MIN_10D_SPY_RELATIVE
            and avg_10d_qqq > MIN_10D_QQQ_RELATIVE
            and avg_10d_ufo > MIN_10D_UFO_RELATIVE
            and avg_10d_arkx > MIN_10D_ARKX_RELATIVE
        )
        profiles[ticker] = {
            "passed": passed,
    ...
    return {
        ...
        "thresholds": {
            ...
            "min_avg_10d_spy_relative_value": MIN_10D_SPY_RELATIVE,
            "min_avg_10d_qqq_relative_value": MIN_10D_QQQ_RELATIVE,
            "min_avg_10d_ufo_relative_value": MIN_10D_UFO_RELATIVE,
            "min_avg_10d_arkx_relative_value": MIN_10D_ARKX_RELATIVE,
        },
...
def _run_exp030_stack_variant(
    ...
    *,
    ...
    def patched_scale_and_record(
        *,
        ...
        if ticker in benchmark_tickers:
            ...
            signal["space_benchmark_breadth_trend_scalar"] = benchmark_breadth_scalar
    ...
    benchmark_counts = {
        ...
        if "space_benchmark_breadth_trend_risk" in key
    }
    delayed_counts = {
        key: value
        for key, value in sorted(counts.items())
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_benchmark_breadth_trend_scalar": benchmark_breadth_scalar,
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        counts.get("space_benchmark_breadth_trend_risk_changed_signal", 0)
    )
    eligible_count = int(
        counts.get("space_benchmark_breadth_trend_risk_eligible_signal", 0)
    )
    scalar = float(variant["parameters"]["space_benchmark_breadth_trend_scalar"])
    return {
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space benchmark-breadth trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_benchmark_breadth_trend_scalar` for official Space "
            "`trend_long` signals whose closed 10d event-state profile has "
            "positive cash-relative, SPY-relative, QQQ-relative, UFO-relative, "
            "and ARKX-relative value. Candidate pool, ranking, targets, stops, "
            "LLM/news, and accepted exp030 stack stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        f"- Decision: `{payload['decision']}`",
        f"- Best scalar: `{best['parameters']['space_benchmark_breadth_trend_scalar']}`",
        (
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = counts.get(
            "space_benchmark_breadth_trend_risk_changed_signal",
            0,
        )
        lines.append(
...
def run() -> dict[str, Any]:
    ...
    status = (
        "accepted_default_off_space_benchmark_breadth_trend_risk"
        if decision == "accepted"
        else "rejected_space_benchmark_breadth_trend_risk"
    )
    rejection_reason = ""
    if decision == "rejected":
        rejection_reason = (
            "The benchmark-breadth trend state did not clear Gate 4 versus "
            "the accepted exp030 stack. Do not retry this exact broad-benchmark "
            "confirmation cohort without new closed forward rows."
        )
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Space official catalysts with broad 10d confirmation against cash, "
            "SPY, QQQ, UFO, and ARKX may deserve conservative trend risk "
            "allocation even when the evidence is not purely same-theme. This "
            "tests a production-visible closed-forward breadth state without "
            "adding tickers, LLM authority, or lifecycle changes."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_benchmark_breadth_trend_scalar",
        "single_causal_variable": (
            "extra trend_long risk scalar for the benchmark-breadth forward "
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "risk allocation: broad 10d cash/SPY/QQQ/UFO/ARKX confirmation "
                "may identify Space trend candidates worth incremental "
                "default-off risk."
            ),
            "q2_prior_experiments": [
                ...
                "exp-20260514-033 rejected the below-$500 same-theme benchmark-breadth substitute because it produced no changed signals.",
                "exp-20260514-036 rejected fast 5d confirmation because it produced no runtime signal coverage.",
                "Nearby source-diversity, company-source, and delayed scalars are anti-repeat families.",
            ],
            "q3_single_causal_variable": (
                "Only this benchmark-breadth trend scalar changes; no entry, "
        ...
        "parameters": {
            ...
            "selected_scalar": best["parameters"][
                "space_benchmark_breadth_trend_scalar"
            ],
        ...
        "why_not_other_changes": (
            ...
            "new closed forward breadth state using already logged 10d cash, "
            "SPY, QQQ, UFO, and ARKX replacement values."
        ),
        "known_risks": [

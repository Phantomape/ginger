...
one causal variable: a bounded paper-notional support scalar for already
selected candidates whose score is clearly separated from the next available
same-day broad-market candidate.

The intent is governance/risk allocation, not another trend/extension retune:
score separation is a production-computable same-day crowding/leadership
quality surface. No JavaScript is used.
"""
...
EXPERIMENT_ID = "exp-20260524-022"
EXPERIMENT_SLUG = "broad_market_score_gap_crowding_notional"
...
SCORE_GAP_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict(
    [
        (
            "baseline_no_score_gap_support",
            {"score_gap_to_next_min": 0.10, "scalar": 1.00},
        )
    ]
)
for threshold in (0.075, 0.10, 0.125, 0.15, 0.175, 0.20, 0.25):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15):
        threshold_text = f"{threshold:.3f}".rstrip("0").rstrip(".").replace(".", "p")
        scalar_text = f"{scalar:.3f}".rstrip("0").rstrip(".").replace(".", "p")
        SCORE_GAP_SWEEP[f"score_gap_gte_{threshold_text}_scalar_{scalar_text}"] = {
            "score_gap_to_next_min": threshold,
            "scalar": scalar,
        }
...
def _annotate_score_gap(
    *,
    ...
    score = float(annotated["score"])
    next_score = None
    if rank < len(ranked):
        next_score = float(ranked[rank]["score"])
    top_score = float(ranked[0]["score"]) if ranked else None
    second_score = float(ranked[1]["score"]) if len(ranked) > 1 else None
    third_score = float(ranked[2]["score"]) if len(ranked) > 2 else None
    annotated["day_candidate_count"] = len(ranked)
    annotated["score_gap_to_next"] = (
        round(max(0.0, score - next_score), 6) if next_score is not None else None
    )
    annotated["score_top_to_second_gap"] = (
        round(max(0.0, top_score - second_score), 6)
        if top_score is not None and second_score is not None
        else None
    )
    annotated["score_top3_spread"] = (
        round(max(0.0, top_score - third_score), 6)
        if top_score is not None and third_score is not None
        else None
    )
    return annotated
...
def _base_feature_rows(
    *,
    ...
    for day in days:
        ...
        for rank, raw_feature in enumerate(selected, start=1):
            feature = _annotate_score_gap(feature=raw_feature, ranked=ranked, rank=rank)
...
def _variant_trades(
    *,
    ...
    for row in base_rows:
        ...
        if trade is None:
            continue
        score_gap = row["feature"].get("score_gap_to_next")
        support_applied = (
            score_gap is not None
            and float(score_gap) >= float(score_gap_to_next_min)
        ...
        trade["score_gap_to_next"] = score_gap
        trade["score_top_to_second_gap"] = row["feature"].get("score_top_to_second_gap")
        trade["score_top3_spread"] = row["feature"].get("score_top3_spread")
        trade["score_gap_support_min"] = score_gap_to_next_min
        trade["score_gap_support_notional_scalar"] = scalar
        trade["score_gap_support_notional_multiplier"] = scalar if support_applied else 1.0
        trade["score_gap_support_applied"] = support_applied
        trades.append(trade)
    return trades
        ...
        rows.append(
            {
                ...
                "rank": trade["rank"],
                "score": trade["score"],
                "score_gap_to_next": trade.get("score_gap_to_next"),
                "score_gap_support_applied": trade.get("score_gap_support_applied"),
                "score_gap_support_notional_multiplier": trade.get(
                    "score_gap_support_notional_multiplier"
                ),
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("score_gap_support_applied")]
    base["score_gap_adjusted_trade_count"] = len(adjusted)
    base["score_gap_adjusted_pnl"] = round(
    ...
    *,
    ...
    for label, spec in WINDOWS.items():
        trades = _variant_trades(
            ...
            prices=prices,
            score_gap_to_next_min=score_gap_to_next_min,
            scalar=scalar,
            label=label,
        )
    ...
    adjusted = [row for row in all_trades if row.get("score_gap_support_applied")]
    ...
    gate4_passed = bool(
        variant_name != "baseline_no_score_gap_support"
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pool = passing or [
        row for row in variants if row["variant_name"] != "baseline_no_score_gap_support"
    ]
    return sorted(
        pool,
        key=lambda row: (
    ...
    return [
        {
            ...
            "score_gap_to_next_min": row["score_gap_to_next_min"],
            "scalar": row["scalar"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Score-Gap Crowding Support Notional",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {gate} | {adjusted} | {ev:+.4f} | {rel:+.2%} | ${pnl:+,.2f} | {wi} | {wr} | {dd:+.4%} | {top5} |".format(
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        _variant_payload(
            variant_name=name,
            score_gap_to_next_min=values["score_gap_to_next_min"],
        ...
        for name, values in SCORE_GAP_SWEEP.items()
    ]
    ...
    decision = (
        "accepted_default_off_broad_market_score_gap_crowding_notional"
        if accepted
        else "rejected_broad_market_score_gap_crowding_notional"
    )
    ...
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "Broad-market candidates with clear same-day score separation from "
            "the next candidate may represent cleaner leadership and lower "
            "crowding than clustered beta. A bounded default-off paper notional "
            "support scalar should only be retained if it clears a strict "
            "three-window materiality gate."
        ),
        "change_type": "default_off_paper_allocation",
        "changed_variable": "broad_market_score_gap_crowding_support_notional_scalar",
        "single_causal_variable": "score-gap-to-next support scalar for fixed broad-market selected candidates",
        ...
        "parameters": {
            ...
            "selected_score_gap_to_next_min": selected["score_gap_to_next_min"],
            "selected_score_gap_support_scalar": selected["scalar"],
            "minimum_relative_ev_improvement": MIN_RELATIVE_EV_IMPROVEMENT,
            "sweep": SCORE_GAP_SWEEP,
            "profile_config": {
        ...
        "backtest_protocol": (
            ...
            "state; after state adds a score-gap crowding paper-notional support field."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "selected_variant": {
            ...
            "score_gap_to_next_min": selected["score_gap_to_next_min"],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "capital/risk allocation: score-gap leadership quality may reduce broad-market crowding beta inside the default-off paper sleeve.",
            "2_past_similar_experiments": "Broad-market price-floor, rank-notional, low-extension ret5, high-volatility, trend-persistence, strong-close crowding, recent-repeat, absolute-score, trend-efficiency, red-day pullback, and max-daily-return variants were tested; score-gap-to-next crowding separation was not formalized for this sleeve.",
            "3_single_variable": "Only score_gap_to_next_min/scalar changes inside one notional-support field; eligibility, rank profile, accepted notional scalars, hold, slots, and universe controls stay fixed.",
            "4_acceptance": "Gate 4 requires positive aggregate EV/PnL, all 3 windows EV-positive, no EV/PnL regression windows, >=8 adjusted trades across all 3 windows, concentration guard, <=0.5pp max drawdown worsening, identity control, and >=10% aggregate EV improvement due frozen-sample broad-market crowding retune risk.",
            "5_reproducibility": "Script, JSON artifact, log, ticket, markdown artifact, and JSONL identify windows, parameters, and metrics.",
        },
        "interpretation": (
            "Score-gap separation is a production-computable crowding/leadership "
            "quality surface aligned with the playbook's broad-market governance "
            "direction. Because it is still a frozen-window notional scalar, this "
            "run uses a strict materiality gate before any shared-adapter change."
        ),
        "rejection_reason": None
        if accepted
        else "Score-gap crowding support did not clear the strict broad-market materiality and three-window gate.",
        "next_evidence_needed": (
            "If forward broad-market paper outcomes keep showing score-gap "
            "separation value, retest with production ledger replacement-value "
            "evidence before enabling any shared notional scalar."
        ),
        "related_files": {

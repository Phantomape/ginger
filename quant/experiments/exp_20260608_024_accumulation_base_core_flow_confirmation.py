...
Replay-only alpha search. This tests one production-visible admission variable
on top of exp-20260608-016: admit accumulation-base breakout candidates only
when the same signal date already has core A/B entry flow, while excluding
same-ticker core overlap so the paper row tests independent replacement value.
...
EXPERIMENT_ID = "exp-20260608-024"
STEM = "accumulation_base_core_flow_confirmation"
TRIAL_FAMILY = "accumulation_base_core_flow_confirmation"
TRIAL_VARIANT_ID = "same_day_core_flow_nonoverlap_top1_10d_v1"
CHANGED_VARIABLE = "accumulation_base_core_flow_confirmed_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = previous.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260608_024_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "drawdown_drift",
        "core_flow_overfit",
        "accumulation_beta_relabel",
    ],
    "confidence_reason": (
        "exp-20260608-016 had large positive aggregate EV/PnL but failed "
        "old_thin and drawdown; core-flow confirmation helped industry-stable "
        "relation alpha, but compression core-flow was sample-thin, so this "
        "is a focused admission test rather than a threshold sweep."
    ),
    "recorded_at": "2026-06-08T20:05:05+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "same accumulation-base source, same-day core A/B flow confirmation, "
...
def _candidate_rows_for_window(
    *,
    ...
    for row in candidates:
        same_day_flow = bool(row.get("same_day_ab_overlap"))
    ...
    for context in contexts:
        ...
        if day_rows:
            ...
            context["top_candidate_breakout_pct_after_core_flow"] = top[
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_accumulation_base_core_flow_confirmed"
        if gate["passed"]
        else "rejected_accumulation_base_core_flow_confirmed_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "Ticker-level accumulation-base breakouts may only be useful "
                "when same-day core A/B entry flow confirms real demand, while "
                "excluding same-ticker core overlap to test independent "
                "replacement value."
            ),
            ...
            "interpretation": (
                "The core-flow confirmed accumulation-base breakout source "
                "cleared Gate 4 as a replay-only/default-off lead. It is not "
                "accepted alpha until a shared daily/replay helper reproduces "
                "it without production/backtest divergence."
                if passed
                else (
                    "The core-flow confirmed accumulation-base breakout source "
                    "did not clear Gate 4. Do not promote it or answer with "
                    "nearby accumulation/core-flow threshold tuning on these "
                    "frozen windows."
                )
            ),
            "rejection_reason": None if passed else "; ".join(gate4["failed_reasons"]),
            "negative_reflection": (
                "If rejected, same-day core flow did not isolate the old_thin "
                "and drawdown failure in exp-20260608-016. The likely reason "
                "is that the accumulation-volume proxy still amplifies "
                "pro-cyclical breakout beta, or the core-flow dates already "
                "capture the best opportunity inside the core stack."
            ),
            "post_run_reflection": {
                "why_result_happened": (
                    "Core-flow confirmation retained enough accumulation-base "
                    "edge while filtering the broadest beta tail, but this is "
                    "only replay evidence until daily production uses the same "
                    "helper."
                    if passed
                    else (
                        "Core-flow confirmation did not make the "
                        "accumulation-base source robust enough. It either "
                        "thinned the useful winners, kept the old-window tail, "
                        "or selected dates where the core stack already owned "
                        "the cleaner demand signal."
                    )
                ),
                "forbidden_near_neighbor_retry": (
                    "Do not retry by sweeping same-day core-flow requirements, "
                    "same-ticker overlap handling, accumulation lookback, "
                    "up-volume share, OBV ratio, breakout pct, close-location, "
                    "volume, ret5/ret20, top-N, hold-day, cooldown, or paper "
                    "notional thresholds on these frozen windows."
                ),
                "new_evidence_required": (
                    "A retry requires materially new PIT flow/catalyst "
                    "evidence, closed forward replacement-value rows, or a "
                    "shared default-off helper reproducing this exact fixed "
                    "policy for forward observation."
                ),
            },
            "next_evidence_needed": (
                "A positive replay lead still needs a shared default-off "
                "adapter and parity tests before forward observation. Live "
                "activation would need closed forward replacement-value rows "
                "and a separate activation-envelope Gate 1-4."
            ),
        }
    )
    ...
    payload["backtest_protocol"]["execution_model"] = (
        ...
        "prior close history for the exp-20260608-016 accumulation-base "
        "breakout source and same-day baseline core A/B entry flow from the "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: accumulation-base breakouts should be "
            "cleaner when same-day core A/B flow confirms real demand, while "
            "same-ticker core overlap is excluded to avoid duplicate core "
            "entries."
        ),
        "2_history_check": {
            "exp-20260608-016": (
                "Rejected despite aggregate EV +0.7278 and PnL +$6,710.05 "
                "because old_thin regressed and drawdown drift was +1.72pp."
            ),
            "exp-20260608-007/008": (
                "Core-flow confirmation rescued industry-stable leadership "
                "and was later promoted through a shared default-off adapter."
            ),
            "exp-20260608-020": (
                "Compression core-flow confirmation was too thin and failed, "
                "so sample loss is a live failure mode for this run."
            ),
        },
        ...
        "4_acceptance_standard": (
            ...
            "private replay still requires shared adapter parity before "
            "promotion."
        ),
        "5_reproducibility": (
            ".venv\\Scripts\\python.exe -B quant\\experiments\\"
            "exp_20260608_024_accumulation_base_core_flow_confirmation.py"
        ),
    }
    payload["pre_run_questions"] = payload["gate_questions"]
    payload["gate2"] = {
        "required_runtime_fields": [
            "signal-date OHLCV rows",
            "SPY signal-date and historical OHLCV rows",
            "baseline same-day core A/B entries by signal date",
            "same-day core A/B tickers for overlap exclusion",
            "sector/industry classification cache",
        ],
        "field_reality_check": (
            "The runner reads signal-date OHLCV from the canonical warehouse "
            "snapshot, derives same-day core A/B entries from the before "
            "backtest result through framework.shadow._baseline_entries, and "
            "excludes same-ticker overlap before selecting top-1 paper rows."
        ),
    }
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Accumulation-Base Core-Flow Confirmation",
            "",

"""exp-20260707-008: daily-equity replay for the rank-10 portfolio-lane candidate.

This consumes the observed-only ranking from exp-20260706-022. It does not
retune the gap-hold core-flow source from exp-20260609-016 and does not change
live, paper, ranking, sizing, entry, or exit behavior. The only tested
decision hypothesis is whether this ranked rejected source still looks
...
base.EXPERIMENT_ID = "exp-20260707-008"
base.STEM = "gap_hold_core_flow_daily_equity_overlay"
base.STATUS_POSITIVE = (
    "observed_only_positive_gap_hold_core_flow_daily_equity_lead_not_activation_ready"
)
base.STATUS_REJECTED = "observed_only_rejected_gap_hold_core_flow_daily_equity_overlay"
base.DECISION_POSITIVE = "observed_only_positive_gap_hold_core_flow_daily_equity_overlay"
base.DECISION_REJECTED = "observed_only_rejected_gap_hold_core_flow_daily_equity_overlay"

base.TRIAL_VARIANT_ID = "gap_hold_core_flow_rank10_10pct_daily_equity_overlay_v1"
base.CHANGED_VARIABLE = "gap_hold_core_flow_rank10_portfolio_daily_equity_overlay_v1"
...
base.NEW_EVIDENCE_AXIS = (
    ...
    "consume exp-20260706-022 rank-10 gap_hold_core_flow_confirmed rows from "
    "exp-20260609-016. Prior daily-equity overlays consumed fixed-asset-turnover, "
    "sector-breadth, deferred-revenue, FINRA short-pressure, purchase-obligation, "
    "receivables DSO, industry breadth-repair, and volatility-curve relief; this "
    "does not retune overlay weight, source thresholds, top-N, hold days, cooldown, "
    "notional, or correlation cutoffs."
)

base.OUT_DIR = base.REPO_ROOT / "data" / "experiments" / base.EXPERIMENT_ID
base.OUT_JSON = base.OUT_DIR / f"exp_20260707_008_{base.STEM}.json"
...
base.PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "gap_hold_old_thin_noise",
    ],
    "confidence_reason": (
        "exp-20260706-022 ranked gap-hold core-flow tenth with positive "
        "exit-cashflow proxy delta and low negative core correlation, but the "
        "first seven daily-MTM overlay consumptions all exposed window "
        "instability; this consumes a new ranked source artifact without "
        "changing source thresholds, overlay weight, top-N, hold, cooldown, or "
        "correlation cutoffs."
    ),
    "recorded_at": "2026-07-07T08:03:45+00:00",
}

base.HYPOTHESIS = (
    "Portfolio lane: the exp-20260706-022 rank-10 gap-hold core-flow confirmed "
    "rejected source may add value as a fixed 10 percent paper overlay when "
    "replayed with true daily mark-to-market equity, without retuning the "
    "gap-hold source or overlay weight."
)
...
base.RUNNER = f"quant/experiments/exp_20260707_008_{base.STEM}.py"
base.RUNNER_COMMAND = f".\\.venv\\Scripts\\python.exe -B {base.RUNNER}"
base.RUNNER_WINDOWS = f"quant\\experiments\\exp_20260707_008_{base.STEM}.py"
base.CHANGED_FILES = [
    base.RUNNER,
    f"data/experiments/{base.EXPERIMENT_ID}/exp_20260707_008_{base.STEM}.json",
...
def source_ranking_audit() -> dict[str, Any]:
    ...
    if isinstance(ranking, list):
        for idx, row in enumerate(ranking, start=1):
            if isinstance(row, dict) and row.get("experiment_id") == "exp-20260609-016":
    ...
    return {
        ...
        "reason_selected": (
            ...
            "sector-breadth, deferred-revenue, FINRA short-pressure, purchase-obligation, "
            "receivables DSO, industry breadth-repair, and volatility-curve relief "
            "daily-equity overlay probes."
        ),
    }
...
def patch_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["post_run_reflection"] = {
        "why_result_happened": (
            "The gap-hold core-flow confirmed source from exp-20260609-016 was "
            ...
            "daily EV, drawdown drift, and window stability, not on gap-hold "
            "source thresholds or notional retunes."
        ),
        "forbidden_near_neighbor_retry": (
            "Do not rerun this gap-hold core-flow portfolio overlay by changing "
            "overlay weight, gap/hold confirmation thresholds, core-flow filters, "
        ...
        "new_evidence_required": (
            ...
            "replacement-value rows for the unchanged gap-hold core-flow source."
        ),
    }
    payload["next_retry_requires"] = [
        ...
        "materially more closed gap-hold core-flow forward replacement-value rows",
        "no overlay-weight, gap-hold-threshold, top-N, hold-day, source-filter, or correlation retune",
    ]
    if not positive:

"""exp-20260617-015: pension funded-status relief candidate scout.

Replay-only alpha search. The single decision hypothesis is a PIT free SEC
Companyfacts candidate source: production-universe names whose pension or
postretirement benefit funded-status burden is falling versus revenue may have
a long-duration liability overhang easing. Liquid SPY-relative leadership then
tests whether the market is beginning to price that relief before a next-open,
10-trading-day default-off paper continuation trade.
...
EXPERIMENT_ID = "exp-20260617-015"
STEM = "pension_obligation_relief"
TRIAL_FAMILY = "pension_obligation_relief_candidate_pool"
TRIAL_VARIANT_ID = "pension_obligation_relief_top1_next_open_10d_v1"
CHANGED_VARIABLE = "raw_sec_companyfacts_pension_obligation_relief_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_015_{STEM}.json"
...
MAX_BENEFIT_FACT_AGE_DAYS = 430
MIN_CURRENT_REVENUE = 500_000_000.0
MIN_PRIOR_BENEFIT_BURDEN_USD = 25_000_000.0
MIN_PRIOR_BENEFIT_TO_REVENUE = 0.015
MAX_CURRENT_BENEFIT_TO_REVENUE = 0.75
MIN_BENEFIT_RATIO_RELIEF = 0.005
MIN_BENEFIT_RELIEF_PCT = 0.08
MIN_REVENUE_GROWTH = -0.10
MIN_PERIOD_GAP_DAYS = 250
MAX_PERIOD_GAP_DAYS = 460

BENEFIT_LIABILITY_TAGS = (
    "PensionAndOtherPostretirementDefinedBenefitPlansLiabilitiesNoncurrent",
    "DefinedBenefitPensionPlanLiabilitiesNoncurrent",
    "OtherPostretirementDefinedBenefitPlanLiabilitiesNoncurrent",
)
BENEFIT_OBLIGATION_TAGS = (
    "DefinedBenefitPlanBenefitObligation",
    "DefinedBenefitPlanProjectedBenefitObligation",
    "DefinedBenefitPlanAccumulatedBenefitObligation",
    "PostretirementBenefitPlanAccumulatedPostretirementBenefitObligation",
)
BENEFIT_ASSET_TAGS = (
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "field_coverage_too_old_industrial",
        "window_regression",
        "drawdown_drift",
    ...
    "confidence_reason": (
        "Pension/postretirement obligations are official filed-date SEC "
        "liability-overhang fields with broad raw Companyfacts coverage and no "
        "matching prior alpha trial. Success probability is low because recent "
        "Companyfacts liability/quality scouts often improve aggregate EV but "
        "fail old_thin, drawdown, or accepted comparator gates."
    ),
    "recorded_at": "2026-06-17T13:05:50+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC pension/postretirement liability or obligation "
            "facts, missing plan asset facts when needed, missing annual "
            "revenue facts, missing prior comparison point, stale facts, "
    ...
    "parity_note": (
        ...
        "SEC pension/postretirement funded-status tag mapping, filed-date PIT "
        "annual revenue normalization, liquid SPY-relative confirmation, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts pension/postretirement funded "
        "status burden falling versus revenue may identify companies where "
        "long-duration employee-benefit liability overhang is easing; liquid "
    ...
    "2_history_check": {
        "exp-20260616-029": (
            ...
            "because old_thin and drawdown failed. This run is not debt; it "
            "tests employee-benefit funded-status overhang."
        ),
        "exp-20260616-025": (
            "Rejected operating lease burden relief. This run avoids lease "
            "obligations and tests pension/postretirement benefit obligations."
        ),
        "exp-20260617-013": (
            "Rejected deferred-tax valuation allowance release on sample and "
            "concentration. This run is not tax-asset confidence."
        ),
        "exp-20260617-014": (
            "Rejected product warranty reserve relief on sample/concentration. "
            "This run is not after-sale warranty quality."
        ),
        "history_scan": (
            "No prior committed experiment_log or experiments/logs entry used "
            "pension/postretirement funded-status relief as the decision field."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_015_pension_obligation_relief.py"
    ),
}
...
def _raw_instant_facts(usgaap: dict[str, Any], tags: tuple[str, ...]) -> list[dict[str, Any]]:
    ...
    for tag in tags:
        ...
        for raw in tag_payload.get("units", {}).get("USD", []):
            ...
            if start or not end or not filed or value is None:
                continue
            facts.append(
                {
                    "filed": filed,
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        liability_facts = _raw_instant_facts(usgaap, BENEFIT_LIABILITY_TAGS)
        obligation_facts = _raw_instant_facts(usgaap, BENEFIT_OBLIGATION_TAGS)
        asset_facts = _raw_instant_facts(usgaap, BENEFIT_ASSET_TAGS)
        revenue_facts = rd._raw_annual_facts(usgaap, REVENUE_TAGS)
        if not liability_facts and not obligation_facts:
            stats["tickers_missing_raw_benefit_obligation"] += 1
            continue
        if not revenue_facts:
            stats["tickers_missing_raw_annual_revenue"] += 1
            continue
        index[ticker] = {
            "liability": liability_facts,
            "obligation": obligation_facts,
            "asset": asset_facts,
            "revenue": revenue_facts,
        }
        stats["tickers_with_raw_benefit_revenue"] += 1
        stats["raw_benefit_liability_fact_count"] += len(liability_facts)
        stats["raw_benefit_obligation_fact_count"] += len(obligation_facts)
        stats["raw_benefit_asset_fact_count"] += len(asset_facts)
        stats["raw_annual_revenue_fact_count"] += len(revenue_facts)

    summary = {
        "raw_companyfacts_cache": _repo_rel(RAW_COMPANYFACTS_CACHE),
        "benefit_liability_tags": list(BENEFIT_LIABILITY_TAGS),
        "benefit_obligation_tags": list(BENEFIT_OBLIGATION_TAGS),
        "benefit_asset_tags": list(BENEFIT_ASSET_TAGS),
...
def _latest_annual_fact_at_or_before(
    facts: list[dict[str, Any]],
    *,
    asof: str,
    end_lte: str,
    ...
    candidates = [
        fact for fact in facts if fact["filed"] <= asof and fact["end"] <= end_lte
    ]
    if not candidates:
        return None
    return max(candidates, key=lambda row: (row["end"], row["filed"], float(row.get("value") or 0.0), row["tag"]))
...
def _benefit_state(
    ...
    *,
    ...
    liability = _latest_fact(facts["liability"], asof=asof, before_end=before_end)
    obligation = _latest_fact(facts["obligation"], asof=asof, before_end=before_end)

    if liability is not None and (obligation is None or liability["end"] >= obligation["end"]):
        return {
            "method": "net_liability_tag",
            "period_end": liability["end"],
            "filed": liability["filed"],
            "burden_value": abs(float(liability["value"])),
            "liability_tag": liability["tag"],
    ...
    if obligation is None:
        return None
    asset = _latest_fact(facts["asset"], asof=asof, end=obligation["end"])
    if asset is None:
        return None
    obligation_value = abs(float(obligation["value"]))
    asset_value = abs(float(asset["value"]))
    burden_value = max(obligation_value - asset_value, 0.0)
    return {
        "method": "obligation_minus_plan_assets",
        "period_end": obligation["end"],
        "filed": max(obligation["filed"], asset["filed"]),
        "burden_value": burden_value,
        "liability_tag": None,
        "obligation_value": obligation_value,
        "asset_value": asset_value,
        "obligation_tag": obligation["tag"],
        "asset_tag": asset["tag"],
    }


def _benefit_relief_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    current = _benefit_state(facts, asof=asof)
    if current is None:
        return None
    if base._days_between(asof, current["filed"]) > MAX_BENEFIT_FACT_AGE_DAYS:
        return None
    prior = _benefit_state(facts, asof=asof, before_end=current["period_end"])
    if prior is None:
        return None
    period_gap_days = base._days_between(current["period_end"], prior["period_end"])
    if period_gap_days < MIN_PERIOD_GAP_DAYS or period_gap_days > MAX_PERIOD_GAP_DAYS:
        return None

    current_revenue = _latest_annual_fact_at_or_before(
        facts["revenue"], asof=asof, end_lte=current["period_end"]
    )
    prior_revenue = _latest_annual_fact_at_or_before(
        facts["revenue"], asof=asof, end_lte=prior["period_end"]
    )
    if current_revenue is None or prior_revenue is None:
        return None

    current_burden = float(current["burden_value"])
    prior_burden = float(prior["burden_value"])
    current_revenue_value = float(current_revenue["value"])
    prior_revenue_value = float(prior_revenue["value"])
    if (
        prior_burden < MIN_PRIOR_BENEFIT_BURDEN_USD
        or current_revenue_value < MIN_CURRENT_REVENUE
        or prior_revenue_value <= 0.0
    ):
        return None

    current_ratio = current_burden / current_revenue_value
    prior_ratio = prior_burden / prior_revenue_value
    ratio_relief = prior_ratio - current_ratio
    relief_pct = (prior_burden - current_burden) / prior_burden
    revenue_growth = current_revenue_value / prior_revenue_value - 1.0

    if prior_ratio < MIN_PRIOR_BENEFIT_TO_REVENUE:
        return None
    if current_ratio > MAX_CURRENT_BENEFIT_TO_REVENUE:
        return None
    if ratio_relief < MIN_BENEFIT_RATIO_RELIEF:
        return None
    if relief_pct < MIN_BENEFIT_RELIEF_PCT:
        return None
    ...
    return {
        ...
        "current_liability_tag": current["liability_tag"],
        "current_obligation_tag": current["obligation_tag"],
        "current_asset_tag": current["asset_tag"],
        "current_benefit_burden": _round(current_burden, 2),
        "prior_benefit_burden": _round(prior_burden, 2),
        "current_obligation_value": _round(current["obligation_value"], 2),
        "current_plan_asset_value": _round(current["asset_value"], 2),
        ...
        "current_benefit_to_revenue": _round(current_ratio, 6),
        "prior_benefit_to_revenue": _round(prior_ratio, 6),
        "benefit_ratio_relief": _round(ratio_relief, 6),
        "benefit_relief_pct": _round(relief_pct, 6),
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _benefit_relief_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_pension_relief_gate"] += 1
                continue
            ...
            score = (
                1.20 * min(float(quality["benefit_relief_pct"] or 0.0), 1.0)
                + 9.0 * min(float(quality["benefit_ratio_relief"] or 0.0), 0.10)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_PENSION_OBLIGATION_RELIEF_PAPER",
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["quality_benefit_ratio_relief"] or 0.0),
            -float(row["quality_benefit_relief_pct"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        "rule_version": RULE_VERSION,
        "max_benefit_fact_age_days": MAX_BENEFIT_FACT_AGE_DAYS,
        "min_current_revenue": MIN_CURRENT_REVENUE,
        "min_prior_benefit_burden_usd": MIN_PRIOR_BENEFIT_BURDEN_USD,
        "min_prior_benefit_to_revenue": MIN_PRIOR_BENEFIT_TO_REVENUE,
        "max_current_benefit_to_revenue": MAX_CURRENT_BENEFIT_TO_REVENUE,
        "min_benefit_ratio_relief": MIN_BENEFIT_RATIO_RELIEF,
        "min_benefit_relief_pct": MIN_BENEFIT_RELIEF_PCT,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_pension_obligation_relief"
        if gate["passed"]
        else "rejected_pension_obligation_relief_candidate_pool"
    )
    return gate
...
def _configure_base() -> None:
    ...
    base.MAX_ANNUAL_FACT_AGE_DAYS = MAX_BENEFIT_FACT_AGE_DAYS
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The pension/postretirement funded-status relief source cleared "
        ...
        interpretation = (
            "The pension/postretirement funded-status relief source did not "
            "clear Gate 4 (failed: "
            f"{', '.join(gate4['failed_reasons']) or 'none'}). It is not "
    ...
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_companyfacts_pension_liability_candidate_pool",
            "new_evidence_type": "raw_sec_companyfacts_pension_postretirement_funded_status_relief_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Pension/postretirement net liability tags are read from raw SEC "
        "Companyfacts instant facts and known only by filed date (<= signal "
        "date). When net liability tags are missing, funded-status burden is "
        "computed as benefit obligation minus plan assets for the same period. "
        "Current and prior benefit burden/revenue ratios use annual revenue "
        "filed by the signal date and period-ended no later than the benefit "
        "fact period. Price confirmation uses only signal-date OHLCV. Paper "
    ...
    payload["gate2"]["runtime_fields"] = [
        "raw SEC companyfacts pension/postretirement net liability instant facts",
        "raw SEC companyfacts benefit obligation instant facts",
        "raw SEC companyfacts plan asset instant facts",
    ...
    payload["next_evidence_needed"] = (
        "A retry needs materially different PIT pension evidence such as "
        "service-cost/interest-cost normalization, employer contribution trend, "
        "discount-rate sensitivity, or closed forward replacement-value rows. "
        "Do not sweep pension/postretirement tag lists, funded-status burden "
        "thresholds, relief percentages, annual fact freshness, RS/close/volume "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping pension/postretirement tag lists, "
            "funded-status burden thresholds, relief percentages, annual fact "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Pension Obligation Relief",
            "",

"""exp-20260502-002 risk-on sector-confirmed leader allocation.

Alpha search. Test one capital-allocation variable: the accepted plain risk-on
SPY-relative leader boost may deserve full risk only when the signal's sector
is also leading SPY. This uses sector synchronization, not small-cap breadth,
SPY lookback tuning, lifecycle splitting, or universe expansion.
"""
...
EXPERIMENT_ID = "exp-20260502-002"
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / "risk_on_sector_confirmed_leader.json"
...
VARIANTS = OrderedDict([
    ("sector_confirmed_2_0x_else_1_6x", {
        "confirmed_multiplier": 2.0,
        "unconfirmed_multiplier": 1.6,
    }),
    ("sector_confirmed_2_0x_else_1_5x", {
        "confirmed_multiplier": 2.0,
        "unconfirmed_multiplier": 1.5,
    }),
])
...
def _sector_ret20(
    ...
    if not sector or sector in {"Unknown", "ETF", "Commodities"}:
        return None
    returns = []
...
def _patch_size_signals(variant: dict | None, universe: list[str]):
    ...
    def patched(signals, portfolio_value, risk_pct=None):
        ...
        for sig in sized:
            ...
            if not ticker or ticker == "SPY" or not sector:
                continue
            cache_key = (str(today.date()), ticker)
            if cache_key not in ret_cache:
                ret_cache[cache_key] = _ret20(ohlcv_all.get(ticker), today)
            ...
            sector_key = (str(today.date()), sector)
            if sector_key not in sector_cache:
                sector_cache[sector_key] = _sector_ret20(
                    sector, today, ohlcv_all, universe, ret_cache
                )
            sector_ret = sector_cache[sector_key]
            if sector_ret is None:
                continue
            sector_confirmed = sector_ret > spy_ret

            _state["leader_signals_seen"] += 1
            target_multiplier = (
                variant["confirmed_multiplier"]
                if sector_confirmed else variant["unconfirmed_multiplier"]
            )
            ...
            preserved["sector_confirmed_leader_multiplier_applied"] = target_multiplier
            preserved["sector_confirmed_by_sector_vs_spy"] = sector_confirmed
            preserved["sector_confirmed_original_multiplier"] = leader_applied
            preserved["sector_confirmed_original_shares"] = sizing.get("shares_to_buy")
            preserved["ticker_ret20_pct"] = round(ticker_ret, 6)
            preserved["spy_ret20_pct"] = round(spy_ret, 6)
            preserved["sector_ret20_pct"] = round(sector_ret, 6)
            preserved["sector_minus_spy_ret20_pct"] = round(sector_ret - spy_ret, 6)
...
def _metrics(result: dict) -> dict:
    ...
    for trade in result.get("trades", []):
        ...
        applied = multipliers.get("sector_confirmed_leader_multiplier_applied")
        if not applied:
            continue
        if multipliers.get("sector_confirmed_by_sector_vs_spy") is True:
            confirmed_trades += 1
            confirmed_pnl += float(trade.get("pnl") or 0.0)
        elif multipliers.get("sector_confirmed_by_sector_vs_spy") is False:
...
def _run_window(universe: list[str], cfg: dict, variant: dict | None) -> dict:
    for key in _state:
        _state[key] = 0
    original = _patch_size_signals(variant, universe)
    try:
...
def build_payload() -> dict:
    ...
    return {
        ...
        "lane": "alpha_search",
        "change_type": "capital_allocation_sector_confirmed_risk_on_leader",
        "hypothesis": (
            "Accepted plain risk-on SPY-relative leaders may deserve full risk "
            "only when their sector equal-weight 20-day return also beats SPY, "
            "because sector synchronization can separate durable leadership "
            "from isolated ticker momentum."
        ),
        ...
        "why_not_llm_soft_ranking": (
            ...
            "a deterministic sector-synchronization discriminator instead."
        ),
        "parameters": {
            "single_causal_variable": "sector-vs-SPY confirmation inside accepted risk_on SPY-relative leader sizing",
            ...
            "selector": {
                ...
                "sector_confirmed": "equal-weight same-sector ret20 > SPY ret20",
            },
            "locked_variables": [
                "universe",
                "signal generation",
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, compute sector-vs-SPY 20-day confirmation in shared "
                "enrichment and consume it from shared sizing."
            ),
        },
        "llm_metrics": {
            "used_llm": False,
        ...
        "history_guardrails": {
            ...
            "why_not_simple_repeat": (
                "This tests sector synchronization of already-selected leaders, "
                "not another SPY lookback, IWM breadth scalar, lifecycle split, "
                "raw universe addition, or Financials-only multiplier."
            ),
        },
        "rejection_reason": (
            None if accepted else
            "No sector-confirmed risk-on leader variant passed fixed-window Gate 4."
        ),
        "next_retry_requires": [
            "Do not retry nearby sector-confirmed SPY leader scalars without forward evidence.",
            "A valid retry needs event/news or richer dispersion context, not another scalar.",
            "If promoted, implement the sector confirmation field through shared production/backtest enrichment.",
        ],
        "related_files": [
            "quant/experiments/exp_20260502_002_risk_on_sector_confirmed_leader.py",
            "data/experiments/exp-20260502-002/risk_on_sector_confirmed_leader.json",
            "docs/experiments/logs/exp-20260502-002.json",
            "docs/experiments/tickets/exp-20260502-002.json",
        ],
    }
...
def main() -> int:
    ...
    TICKET_JSON.write_text(json.dumps({
        ...
        "title": "Risk-on sector-confirmed leader allocation",
        "summary": (

"""exp-20260522-013: core max daily return 20d haircut scout.

Alpha search. Tests whether already-qualified core signals with a top-quartile
single-day return over the prior 20 trading days are a crowded / lottery-like
path that should receive a risk haircut.
...
EXPERIMENT_ID = "exp-20260522-013"
STEM = "core_max_daily_return20_haircut"
MULTIPLIER_KEY = "max_daily_return20_top_quartile_risk_multiplier_applied"
...
VARIANTS = OrderedDict(
    [
        ("max20_haircut_000", {"multiplier": 0.0}),
        ("max20_haircut_025", {"multiplier": 0.25}),
        ("max20_haircut_050", {"multiplier": 0.50}),
        ("max20_haircut_075", {"multiplier": 0.75}),
    ]
)
...
def _max_daily_return20_features(data) -> dict[str, Any]:
    if data is None or len(data) < 22:
        return {
            "max_daily_return_20_pct": None,
            "max_daily_return_20_available": False,
        }
    ...
    return {
        "max_daily_return_20_pct": round(float(returns.max()), 6),
        "max_daily_return_20_available": True,
    }
...
def _make_trend_feature_wrapper(original: Callable[..., dict[str, Any] | None]):
    def wrapped(data):
        ...
        if features is None:
            return None
        return {**features, **_max_daily_return20_features(data)}
...
def _top_quartile_cutoff(features_dict: dict[str, Any]) -> float | None:
    ...
    for ticker, features in (features_dict or {}).items():
        ...
        if sector in EXCLUDED_SECTORS:
            continue
        value = features.get("max_daily_return_20_pct")
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]):
    def wrapped(signals, features_dict, atr_target_mult=None):
        ...
        for sig in enriched:
            ...
            sig["max_daily_return_20_pct"] = value
            sig["max_daily_return_20_top_quartile_cutoff"] = (
                round(float(cutoff), 6) if isinstance(cutoff, (int, float)) else None
            )
            sig["max_daily_return_20_top_quartile_state"] = (
...
def _apply_haircut_to_sizing(
    ...
    if sig.get("max_daily_return_20_top_quartile_state") is not True:
        return sig, None
    ...
    sizing["max_daily_return20_top_quartile_state"] = True
    sizing["max_daily_return20_top_quartile_baseline_shares"] = old_shares
    sizing["max_daily_return20_top_quartile_new_shares"] = new_shares
    sizing["max_daily_return20_top_quartile_max_return"] = sig.get(
        "max_daily_return_20_pct"
    )
    sizing["max_daily_return20_top_quartile_cutoff"] = sig.get(
        "max_daily_return_20_top_quartile_cutoff"
    )
    ...
    record = {
        ...
        "max_daily_return_20_pct": sig.get("max_daily_return_20_pct"),
        "cutoff": sig.get("max_daily_return_20_top_quartile_cutoff"),
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Core signals whose prior 20 trading days contain a top-quartile "
            "single-day return may be crowded / lottery-like continuation paths. "
            "A risk haircut should improve EV and tail risk without changing "
            "entry filters, ranking, exits, universe, LLM, or news logic."
        ),
        "change_summary": (
            "Add an experiment-only max_daily_return_20_pct OHLCV field and sweep "
        ...
        "parameters": {
            "field": "max_daily_return_20_pct",
            "field_definition": "max close-to-close daily return over last 20 signal-known trading days",
        ...
        "production_impact": {
            ...
            "promotion_required_if_accepted": (
                "Move max_daily_return_20_pct feature, top-quartile state, and "
        ...
        "known_risks": [
            ...
            "A 0x variant is effectively no-trade alpha and must clear trade-count and concentration guards.",
        ],
        "rejection_reason": rejection_reason,
        "next_retry_requires": (
            [
                "Do not retry adjacent max_daily_return20 cutoffs or scalars on the same frozen windows without forward rows or a distinct daily-return path field.",
                "Consider a richer reversal-vs-continuation path cluster only if it is production-visible and not another threshold sweep.",
            ]
            if not passed
            else [
        ...
        "related_files": [
            "quant/experiments/exp_20260522_013_core_max_daily_return20_haircut.py",

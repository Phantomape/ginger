"""exp-20260630-019: audit 2026-06-30 intraday structured-event deltas.

Measurement repair only. This copies the new 2026-06-30 intraday structured
event forward observations into an experiment-owned artifact, validates the
fixed observer contract, and records duplicate/schema checks without changing
entries, exits, sizing, ranking, prompts, or production trading behavior.
"""
...
EXPERIMENT_ID = "exp-20260630-019"
OWNER = "alpha-explore"
LANE = "measurement_repair"
SLUG = "intraday_structured_event_delta_20260630"
CHANGED_VARIABLE = "intraday_structured_event_observation_delta_20260630_1302_1303_v1"
RUNNER = f"quant/experiments/exp_20260630_019_{SLUG}.py"
...
OUT_JSON = OUT_DIR / f"exp_20260630_019_{SLUG}.json"
...
SOURCE_EXPERIMENT = "exp-20260630-015"
...
HYPOTHESIS = (
    "Intraday structured relation-quality news rows can only become LLM "
    "event-scoring alpha after the fixed observer accumulates replayable pending "
    "and closed replacement-value rows; the new 2026-06-30 captures should be "
    "copied into an experiment-owned delta ledger without changing trading behavior."
)
ALPHA_HYPOTHESIS = (
    "Structured intraday relation-quality events may become LLM event-scoring "
    "alpha after this fixed default-off observer accumulates enough closed "
    "cash/SPY/QQQ replacement-value outcomes."
)
CHANGE_TYPE = "measurement_repair_intraday_forward_observation_refresh"
MECHANISM_FAMILY = "intraday_news_llm_event_scoring_alpha"
TRIAL_FAMILY = "intraday_structured_event_forward_observation_delta"
TRIAL_VARIANT_ID = "20260630_1302_1303_pending_delta_v1"
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_measurement_repair_intraday_structured_event_delta_20260630"
        if accepted
        else "blocked_intraday_structured_event_delta_20260630"
    )
    ...
    return {
        ...
        "new_evidence_type": "new_pending_forward_rows_from_fixed_intraday_observer",
        "new_evidence_axis": "materially more prospective pending rows under the accepted fixed observer contract",
        "pre_run_questions": {
            "money_hypothesis": ALPHA_HYPOTHESIS,
            "decision_class": "LLM event scoring / candidate pool measurement repair",
            "past_nearby_work": {
                "exp-20260630-013": "private intraday structured-news forward observer contract lead",
                "exp-20260630-015": "accepted default-off snapshot/forward-observation wiring",
                "exp-20260630-005": "observed-only daily structured relation-quality forward value lead",
            },
            "single_policy_bundle": CHANGED_VARIABLE,
            "success_criteria": (
                "new 2026-06-30 observation rows are present, pending, schema-valid, "
                "cross-capture observation IDs are unique, and strategy deltas remain zero"
            ),
            "reproducibility": "All source files, hashes, artifact, log, card, manifest, and command are recorded.",
        },
        "parameters": {
        ...
        "gate4": {
            ...
            "semantic_duplicate_note": (
                "Duplicate semantic rows are expected when the same news item is "
                "captured at adjacent intraday times; they are audited separately "
                "from observation_id uniqueness and are not treated as closed edge."
            ),
        },
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "The accepted exp-20260630-015 observer contract already existed, "
                "and the 2026-06-30 1302ET/1303ET source artifacts were schema-valid. "
                "The useful output is a bounded experiment-owned delta plus explicit "
                "semantic duplicate accounting, not a new trading rule."
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retune relation keywords, prompts, horizons, notional, action "
                "response curves, or target-only filters on these pending rows."
            ),
            "new_evidence_required": (
                "Closed replacement-value outcomes from this fixed observer, materially "
                "more prospective captures, or a PIT LLM scorer writing the same "
...
def build_card(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: intraday structured-event delta 20260630",
            "",

"""exp-20260620-030: FX OCI tailwind with cash FX effect confirmation.
...
exp-20260620-026 should only enter the default-off paper queue when the same
filed annual period also shows a positive or improving cash-flow-statement
effect of exchange-rate changes on cash. This is a new operating exposure
confirmation axis, not a sweep of FX OCI thresholds, not a notional/risk
retune, and not the derivative hedge OCI confirmation tested in exp-20260620-028.
...
EXPERIMENT_ID = "exp-20260620-030"
STEM = "fx_oci_cash_effect_confirmation"
TRIAL_FAMILY = "raw_sec_fx_oci_cash_effect_confirmation_candidate_pool"
TRIAL_VARIANT_ID = "fx_oci_tailwind_cash_fx_effect_confirmation_top1_next_open_10d_v1"
CHANGED_VARIABLE = (
    "raw_sec_fx_oci_tailwind_with_exchange_rate_cash_effect_confirmation_candidate_source_v1"
)
...
OUT_JSON = OUT_DIR / f"exp_20260620_030_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "cash_effect_too_sparse",
        "overfilters_mid_weak",
        "drawdown_not_repaired",
        "accepted_distribution_comparator_not_beaten",
        "global_mega_cap_relabeling",
    ],
    "confidence_reason": (
        "exp-20260620-026 showed strong three-window gross FX OCI signal but "
        "drawdown failed; effect-of-exchange-rate-on-cash has broad PIT "
        "Companyfacts coverage and is materially different from exp-20260620-028 "
        "hedge OCI, but raw Companyfacts confirmations have often overfiltered "
        "or failed accepted comparators."
    ),
    "recorded_at": "2026-06-20T22:07:00+00:00",
}
...
PRODUCTION_IMPACT = {
    **prior.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **prior.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            ...
            "effect-of-exchange-rate-on-cash component, missing prior "
            "comparable cash FX effect fact, missing matched annual revenue, "
    ...
    "parity_note": (
        ...
        "SEC FX OCI and cash-flow-statement exchange-rate-on-cash tag mapping, "
        "filed-date PIT annual component gates, revenue context, liquid "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        ...
        "period also shows positive or improving cash-flow-statement "
        "effect-of-exchange-rate-on-cash, because actual cash FX effect is a "
        "PIT operating exposure confirmation rather than accounting "
        "translation noise."
    ),
    "2_history_check": {
        "novelty_gate": (
            "experiment.py new blocked nearby cash-conversion/Companyfacts "
            "quality families. The override is justified because the new "
            "evidence axis is the raw SEC cash-flow-statement "
            "EffectOfExchangeRateOnCash/CashEquivalents annual component "
            "paired with FX translation OCI tailwind; it is not "
            "cash-conversion/FCF quality, not an FX OCI threshold or notional "
            "retune, and not derivative hedge OCI confirmation."
        ),
        "exp-20260620-026": (
            "Rejected FX translation OCI tailwind only on drawdown drift after "
            "positive EV/PnL in all three windows. This run adds actual cash "
            "FX effect provenance instead of changing FX thresholds or sizing."
        ),
        "exp-20260620-028": (
            "Rejected hedge OCI confirmation because it removed mid_weak "
            "coverage and failed accepted comparators. This run uses a broader "
            "cash-flow-statement exchange-rate field, not derivative hedge OCI."
        ),
        "exp-20260617-018": (
            "Rejected AOCI overhang relief. This run uses period OCI component "
            "flows and cash-flow FX effect, not aggregate AOCI balance."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260620_030_fx_oci_cash_effect_confirmation.py"
    ),
}
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        cash_effect_facts = _raw_annual_facts(usgaap, CASH_FX_EFFECT_TAGS, unit="USD")
        if not fx_oci_facts:
        ...
        if not revenue_facts:
            ...
            continue
        if not cash_effect_facts:
            stats["tickers_missing_raw_annual_cash_fx_effect"] += 1
            continue
        index[ticker] = {
            "fx_oci": fx_oci_facts,
            "revenue": revenue_facts,
            "cash_fx_effect": cash_effect_facts,
        }
        stats["tickers_with_fx_revenue_and_cash_fx_effect"] += 1
        stats["raw_annual_fx_oci_fact_count"] += len(fx_oci_facts)
        stats["raw_annual_revenue_fact_count"] += len(revenue_facts)
        stats["raw_annual_cash_fx_effect_fact_count"] += len(cash_effect_facts)

    summary = {
        "raw_companyfacts_cache": _repo_rel(RAW_COMPANYFACTS_CACHE),
        "fx_oci_tags": list(FX_OCI_TAGS),
        "cash_fx_effect_tags": list(CASH_FX_EFFECT_TAGS),
...
def _same_period_cash_fx_confirmation(
    *,
    asof: str,
    ...
    current_cash = prior._latest_period_fact(
        facts["cash_fx_effect"], asof=asof, end=current_period_end
    )
    prior_cash = prior._latest_period_fact(
        facts["cash_fx_effect"], asof=asof, end=prior_period_end
    )
    if current_cash is None or prior_cash is None:
        return None
    if prior.base._days_between(asof, current_cash["filed"]) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None
    current_value = float(current_cash["value"])
    prior_value = float(prior_cash["value"])
    current_to_revenue = current_value / current_revenue if current_revenue else 0.0
    improvement_to_revenue = (current_value - prior_value) / current_revenue if current_revenue else 0.0
    if not (current_value > 0.0 or improvement_to_revenue > 0.0):
        return None
    return {
        "cash_fx_effect_filed": current_cash["filed"],
        "cash_fx_effect_prior_filed": prior_cash["filed"],
        "cash_fx_effect_tag": current_cash["tag"],
        "cash_fx_effect": _round(current_value, 2),
        "prior_cash_fx_effect": _round(prior_value, 2),
        "cash_fx_effect_improvement": _round(current_value - prior_value, 2),
        "cash_fx_effect_to_revenue": _round(current_to_revenue, 6),
        "cash_fx_effect_improvement_to_revenue": _round(improvement_to_revenue, 6),
        "cash_fx_effect_fact_age_days": prior.base._days_between(asof, current_cash["filed"]),
    }


def _fx_oci_cash_effect_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    ...
    if fx is None:
        return None
    cash_fx = _same_period_cash_fx_confirmation(
        asof=asof,
        current_period_end=str(fx["current_fiscal_year_end"]),
        prior_period_end=str(fx["prior_fiscal_year_end"]),
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _fx_oci_cash_effect_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_fx_oci_or_cash_effect_gate"] += 1
                continue
            ...
            score = (
                ...
                + 1.50 * max(min(float(quality["cash_fx_effect_to_revenue"] or 0.0), 0.02), 0.0)
                + 1.00
                * max(min(float(quality["cash_fx_effect_improvement_to_revenue"] or 0.0), 0.02), 0.0)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_FX_OCI_CASH_EFFECT_CONFIRMED_PAPER",
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["quality_cash_fx_effect_improvement_to_revenue"] or 0.0),
            -float(row["quality_cash_fx_effect_to_revenue"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        ...
        "cash_fx_effect_confirmation_gate": "current_value_positive_or_improvement_positive",
    }
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_fx_oci_cash_effect_confirmation"
        if gate["passed"]
        else "rejected_fx_oci_cash_effect_confirmation_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The raw SEC FX OCI tailwind with cash-flow-statement FX effect "
        ...
        interpretation = (
            "The raw SEC FX OCI tailwind with cash-flow-statement FX effect "
            f"confirmation did not clear Gate 4 (failed: {failed_text}). It is "
    ...
    payload.update(
        {
            ...
            "new_evidence_type": "raw_sec_companyfacts_cash_flow_exchange_rate_effect_confirmation",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual FX-translation OCI, annual effect-of-exchange-rate-on-cash "
        "component, and annual revenue are read from raw SEC Companyfacts tags "
        "known only by filed date (<= signal date). The fixed exp-026 FX "
        "tailwind gate is retained, and the same annual period must also show "
        "a positive current cash FX effect or positive improvement versus the "
        "prior comparable annual period. Price confirmation uses only "
        "signal-date OHLCV. Paper entry is next available open and exit is the "
        "close 10 trading days after the signal with standard costs."
    )
    ...
    payload["gate2"]["runtime_fields"] = [
        ...
        "raw SEC companyfacts annual effect-of-exchange-rate-on-cash facts",
    ...
    payload["next_evidence_needed"] = (
        "Do not retry by sweeping FX OCI ratio/improvement, exchange-rate cash "
        "effect tag lists, sign thresholds, revenue, fact freshness, "
        "RS/close/volume/vol guards, top-N, hold, cooldown, or notional on "
        "frozen windows. A valid retry needs richer segment/currency revenue "
        "mix, FX sensitivity disclosure, closed forward replacement-value rows, "
        "or a shared helper promotion if a future lead remains positive."
    )
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping FX OCI ratio/improvement, exchange-rate "
            "cash-effect tag lists, sign thresholds, revenue, fact freshness, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} FX OCI Cash FX Effect Confirmation",
            "",

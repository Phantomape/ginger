...
scalar for already-selected broad-market candidates whose 20-day trend path is
...
EXPERIMENT_ID = "exp-20260520-017"
EXPERIMENT_SLUG = "broad_market_trend_efficiency_notional"
...
TREND_EFFICIENCY_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict(
    [
        (
            "baseline_no_trend_efficiency_support",
            {"trend_efficiency_20_min": 0.45, "scalar": 1.00},
        )
    ]
)
for threshold in (0.45, 0.50, 0.55, 0.60, 0.65, 0.70):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15):
        ...
        TREND_EFFICIENCY_SWEEP[
            f"trend_efficiency20_gte_{threshold_text}_scalar_{scalar_text}"
        ] = {
            "trend_efficiency_20_min": threshold,
            "scalar": scalar,
        }
...
def _trend_efficiency_20(rows: list[dict[str, Any]], idx: int) -> float | None:
    if idx < 20:
        return None
    closes = [_positive_float(row.get("close")) for row in rows[idx - 20 : idx + 1]]
    if any(value is None for value in closes) or len(closes) != 21:
        return None
    close_values = [float(value) for value in closes if value is not None]
    path_length = sum(
        abs(close_values[pos] - close_values[pos - 1]) for pos in range(1, len(close_values))
    )
    if path_length <= 0:
        return None
    net_change = abs(close_values[-1] - close_values[0])
    return round(net_change / path_length, 6)
    ...
    for day in days:
        ...
        for ticker in candidate_tickers:
            ...
            if feature and candidate_passes_profile(feature, PROFILE_CONFIG):
                trend_efficiency_20 = _trend_efficiency_20(rows, idx)
                if trend_efficiency_20 is None:
                    continue
                feature["trend_efficiency_20"] = trend_efficiency_20
...
def _variant_trades(
    *,
    ...
    for row in base_rows:
        ...
        if trade is None:
            continue
        trend_efficiency_20 = float(row["feature"]["trend_efficiency_20"])
        support_applied = (
            trend_efficiency_20 >= float(trend_efficiency_20_min) and float(scalar) != 1.0
        )
        if support_applied:
        ...
        trade["window"] = label
        trade["trend_efficiency_20"] = trend_efficiency_20
        trade["trend_efficiency_support_min"] = trend_efficiency_20_min
        trade["trend_efficiency_support_notional_scalar"] = scalar
        trade["trend_efficiency_support_notional_multiplier"] = scalar if support_applied else 1.0
        trade["trend_efficiency_support_applied"] = support_applied
        trades.append(trade)
    return trades
        ...
        rows.append(
            {
                ...
                "rank": trade["rank"],
                "score": trade["score"],
                "trend_efficiency_20": trade.get("trend_efficiency_20"),
                "trend_efficiency_support_applied": trade.get("trend_efficiency_support_applied"),
                "trend_efficiency_support_notional_multiplier": trade.get(
                    "trend_efficiency_support_notional_multiplier"
                ),
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("trend_efficiency_support_applied")]
    base["trend_efficiency_adjusted_trade_count"] = len(adjusted)
    base["trend_efficiency_adjusted_pnl"] = round(
    ...
    *,
    ...
    for label, spec in WINDOWS.items():
        trades = _variant_trades(
            ...
            prices=prices,
            trend_efficiency_20_min=trend_efficiency_20_min,
            scalar=scalar,
            label=label,
        )
    ...
    adjusted = [row for row in all_trades if row.get("trend_efficiency_support_applied")]
    ...
    gate4_passed = bool(
        variant_name != "baseline_no_trend_efficiency_support"
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pool = passing or [
        row for row in variants if row["variant_name"] != "baseline_no_trend_efficiency_support"
    ]
    return sorted(
        pool,
        key=lambda row: (
    ...
    return [
        {
            ...
            "trend_efficiency_20_min": row["trend_efficiency_20_min"],
            "scalar": row["scalar"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Trend-Efficiency Support Notional",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        _variant_payload(
            variant_name=name,
            trend_efficiency_20_min=values["trend_efficiency_20_min"],
        ...
        for name, values in TREND_EFFICIENCY_SWEEP.items()
    ]
    ...
    decision = (
        "accepted_default_off_broad_market_trend_efficiency_notional"
        if accepted
        else "rejected_broad_market_trend_efficiency_notional"
    )
    ...
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "Already-selected broad-market leadership paper candidates with high "
            "trend efficiency may carry standalone path quality beyond raw rank "
            "and raw persistence; a bounded support notional scalar can improve "
        ...
        "changed_variable": "broad_market_trend_efficiency_support_notional_scalar",
        "single_causal_variable": "trend-efficiency support scalar for fixed broad-market selected candidates",
        ...
        "parameters": {
            ...
            "selected_trend_efficiency_20_min": selected["trend_efficiency_20_min"],
            "selected_trend_efficiency_support_scalar": selected["scalar"],
            "sweep": TREND_EFFICIENCY_SWEEP,
            "profile_config": {
        ...
        "backtest_protocol": (
            ...
            "state; after state adds a trend-efficiency paper-notional support field."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "selected_variant": {
            ...
            "trend_efficiency_20_min": selected["trend_efficiency_20_min"],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "candidate_pool/capital allocation: smooth 20-day trend paths may deserve bounded paper notional support after selection.",
            "2_past_similar_experiments": "Broad-market price-floor, rank-notional, low-extension ret5, high-volatility, trend-persistence, strong-close crowding, recent-repeat, and absolute-score support were tested; trend-efficiency support was not previously formalized for this sleeve.",
            "3_single_variable": "Only trend_efficiency_20_min/scalar changes inside one notional-support field; eligibility, rank profile, low-extension, high-volatility, trend-persistence, hold, slots, and universe controls stay fixed.",
        ...
        "interpretation": (
            "Trend efficiency is a new production-visible quality field aligned with "
            "the playbook's healthy-persistence direction. Promotion still requires "
            "identity control plus the same three-window gate as other broad-market "
            "paper allocation fields."
        ),
        "rejection_reason": None if accepted else "Trend-efficiency support did not clear the broad-market three-window gate and/or identity control.",
        "next_evidence_needed": (
            "Collect forward broad-market paper outcomes with trend-efficiency "
            "metadata; do not retry adjacent trend-efficiency thresholds on the "

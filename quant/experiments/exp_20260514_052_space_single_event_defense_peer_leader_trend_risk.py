...
Space stack: whether single-event defense-only Space trend signals deserve a
small extra default-off top-up when the ticker also leads the official Space
peer basket.
...
EXPERIMENT_ID = "exp-20260514-052"
STEM = "space_single_event_defense_peer_leader_trend_risk"
...
MARKER = "space_single_event_defense_peer_leader_trend_risk"
...
def _run_exp051_stack_variant(label: str, scalar: float, gates: dict[str, Any]) -> dict[str, Any]:
    ...
    def patched_install(
        ...
        def size_with_peer_leader_extra(
            ...
            for signal in sized:
                ...
                if is_target:
                    ...
                    signal = {
                        **signal,
                        "sizing": sizing,
                        "space_single_event_defense_peer_leader_trend_bucket": True,
                        "space_single_event_defense_peer_leader_trend_scalar": scalar,
                        "space_single_event_defense_peer_leader_trend_profile": (
                            profiles.get(ticker)
                        ),
                    }
                out.append(signal)
        ...
        portfolio_engine.size_signals = size_with_peer_leader_extra
        return accepted

    exp032._install_accepted_exp028_policy = patched_install
    try:
        variant = exp051._run_exp047_stack_variant(
            label=label,
            defense_budget_scalar=ACCEPTED_DEFENSE_BUDGET_DELAYED_BENCHMARK_SCALAR,
            gates=gates,
        )
    finally:
        exp032._install_accepted_exp028_policy = original_install

    variant["single_event_defense_peer_leader_trend_scalar"] = scalar
    variant["single_event_defense_peer_leader_trend_counts"] = dict(sorted(counts.items()))
    variant["single_event_defense_peer_leader_trend_adjusted_signal_count"] = counts.get(
        f"{MARKER}_changed_signal",
        0,
    )
    variant["single_event_defense_peer_leader_trend_eligible_signal_count"] = counts.get(
        f"{MARKER}_eligible_signal",
        0,
    )
    variant["single_event_defense_peer_leader_trend_sample_signals"] = adjustments[:25]
    variant["single_event_defense_peer_leader_trend_target_tickers"] = sorted(target_tickers)
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    improved_windows = {
        label: delta["expected_value_score"]
        for label, delta in by_window_delta.items()
    ...
    changed_count = int(
        variant.get("single_event_defense_peer_leader_trend_adjusted_signal_count")
        or 0
    )
    eligible_count = int(
        variant.get("single_event_defense_peer_leader_trend_eligible_signal_count")
        or 0
    )
    passed = (
        variant["single_event_defense_peer_leader_trend_scalar"] != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "reasons": {
            "non_identity_scalar": variant["single_event_defense_peer_leader_trend_scalar"] != 1.0,
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Single-event defense-only official Space trend signals that also lead "
            "the Space peer basket deserve a small extra default-off risk top-up."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_single_event_defense_peer_leader_trend_risk_scalar",
        "parameters": {
            "scalars_tested": list(SCALARS),
            "selected_scalar": best["single_event_defense_peer_leader_trend_scalar"],
            "target_strategy": TARGET_STRATEGY,
            "target_peer_state": TARGET_PEER_STATE,
            "target_tickers": payload["gates"]["single_event_gate"]["target_tickers"],
        },
        "backtest_protocol": (
        ...
        if payload["decision"] == "accept"
            ...
            "Gate 4 failed: the selected scalar did not improve enough windows "
            "without regression."
        ),
        ...
        if payload["decision"] == "accept"
            ...
            "single-event defense discriminator."
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited; broad ticker expansion has "
            "already failed for GSAT/VSAT/IRDM-style tests. This holds the "
            "candidate set fixed and tests one production-visible peer-state "
            "interaction."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    gate = payload["gate_results"]
    lines = [
        f"# {EXPERIMENT_ID} Space single-event defense peer-leader trend risk",
        "",
        "## Hypothesis",
        "Single-event defense-only official Space `trend_long` signals may deserve "
        "an extra default-off risk top-up when the ticker also leads the official "
        "Space peer basket.",
        "",
        "## Single Changed Variable",
        "`space_single_event_defense_peer_leader_trend_risk_scalar` on top of the "
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['single_event_defense_peer_leader_trend_scalar']}`",
            f"- eligible signals: `{best['single_event_defense_peer_leader_trend_eligible_signal_count']}`",
            f"- adjusted signals: `{best['single_event_defense_peer_leader_trend_adjusted_signal_count']}`",
            f"- adjusted counts: `{best['single_event_defense_peer_leader_trend_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            f"Single-event defense peer-leader trend scalar "
            f"{best['single_event_defense_peer_leader_trend_scalar']} changed "
            f"{best['single_event_defense_peer_leader_trend_adjusted_signal_count']} "
            f"signals with aggregate EV delta "
...
def run() -> dict[str, Any]:
    ...
    gate_results_by_scalar = [
        {
            "scalar": variant["single_event_defense_peer_leader_trend_scalar"],
            **variant["gate"],
        }
        for variant in variants
    ]
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            "Single-event defense-only Space trend signals with peer leadership "
            "deserve a small default-off risk top-up."
        ),
        "changed_variable": "space_single_event_defense_peer_leader_trend_risk_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "decision": payload["decision"],
                    "best_scalar": best[
                        "single_event_defense_peer_leader_trend_scalar"
                    ],
                    "eligible_signals": best[
                        "single_event_defense_peer_leader_trend_eligible_signal_count"
                    ],
                    "adjusted_signals": best[
                        "single_event_defense_peer_leader_trend_adjusted_signal_count"
                    ],

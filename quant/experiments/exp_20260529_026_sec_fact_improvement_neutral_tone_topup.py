"""exp-20260529-026: SEC fact-improvement neutral-tone top-up scout.
...
only whether ``fact_improvement_neutral_tone`` deserves a bounded
...
STEM = "exp_20260529_026_sec_fact_improvement_neutral_tone_topup"
TARGET_BUCKET = "fact_improvement_neutral_tone"
SCALAR_FIELD = "sec_fact_improvement_neutral_tone_notional_scalar"
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_fact_improvement_neutral_tone_topup.md"
)


def _variant_key(name: str) -> str:
    return name.replace("negative_reaction", "fact_improvement_neutral_tone")
...
def _notional_for_position(
    ...
    *,
    ...
    if base._is_target_position(position):
        ...
        rule_parts.append("fact_improvement_neutral_tone_scalar")
    return (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} SEC Fact-Improvement Neutral-Tone Top-Up",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    if gate_passed:
        ...
        decision = "promising_sec_fact_improvement_neutral_tone_topup"
        interpretation = (
            "The fact-improvement neutral-tone scalar cleared the paper-sleeve "
        ...
        interpretation = (
            "No fact-improvement neutral-tone scalar cleared the three-window, "
            "tail-aware paper-sleeve gate on top of the accepted SEC stack."
        )
        rejection_reason = gate.get("reason") or interpretation

    best_variant = str(
        payload.get("best_variant") or "fact_improvement_neutral_tone_scalar_1_00"
    )
    ...
    payload.update(
        {
            ...
            "lane": "alpha_search",
            "hypothesis": (
                "SEC financial-report T+1 rows with explicit improving facts but "
                "neutral or mixed tone may be underreacted continuation candidates. "
                "A bounded paper-notional scalar tests that semantic quality bucket "
                "without changing eligibility, exits, live orders, or LLM authority."
            ),
            "change_summary": (
                "Sweep a bounded paper-notional scalar for "
                "fact_tone_gap_bucket=fact_improvement_neutral_tone."
            ),
            "change_type": "alpha_search",
            "component": "offline_sec_financial_report_paper_sleeve_replay",
            "changed_variable": SCALAR_FIELD,
            ...
            "next_evidence_needed": (
                "Do not retry nearby fact-tone bucket scalars on the same frozen "
                "sample unless forward SEC rows add enough evidence or the bucket "
                "is paired with a distinct production-visible replacement-value signal."
            ),
            "production_impact": {
            ...
            "why_not_other_changes": (
                "Skipped LLM soft-ranking due sparse attribution; skipped options "
                "because available chains are not historical PIT for the fixed "
                "windows; skipped 13F/Kova because prior accepted VCP rows lacked "
                "mapped ownership evidence; skipped state-surface retunes because "
                "they require a hard >10% aggregate EV lift."
            ),
            "protocol_answers": {
                "alpha_hypothesis": (
                    "capital allocation: factual improvement with neutral tone may "
                    "identify underreacted SEC financial-report continuation quality."
                ),
                "past_nearby_experiments": (
                    "exp-20260520-034 was blocked before provenance repair; "
                    "exp-20260522-011, exp-20260522-022, exp-20260522-028, and "
                    "exp-20260523-002 tested other fact-tone buckets and rejected "
                    "nearby scalars. This bucket was not previously tested."
                ),
                "single_causal_variable": SCALAR_FIELD,
                "acceptance_standard": (

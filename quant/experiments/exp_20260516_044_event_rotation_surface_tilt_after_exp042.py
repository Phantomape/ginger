"""exp-20260516-044: event rotation-surface tilt after exp042.

Alpha search, replay-only. Revalidates the strongest deterministic event-bundle
paper allocation refinement after the accepted exp-20260516-042 core stack. The
...
import exp_20260516_040_event_rotation_surface_tilt_after_exp039 as exp040


EXPERIMENT_ID = "exp-20260516-044"
EXPERIMENT_SLUG = "event_rotation_surface_tilt_after_exp042"

REPO_ROOT = exp040.REPO_ROOT
...
def _configure_modules() -> None:
    exp040.EXPERIMENT_ID = EXPERIMENT_ID
    exp040.EXPERIMENT_SLUG = EXPERIMENT_SLUG
    exp040.OUT_JSON = OUT_JSON
    exp040.LOG_JSON = LOG_JSON
    exp040.TICKET_JSON = TICKET_JSON
    exp040.ARTIFACT_MD = ARTIFACT_MD
    exp040.EXPERIMENT_LOG = EXPERIMENT_LOG
    exp040._configure_modules()


def _retag_after_exp042(payload: dict[str, Any]) -> dict[str, Any]:
    payload = exp040._retag_after_exp039(payload)
    payload["experiment_id"] = EXPERIMENT_ID
    payload["timestamp"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
    payload["change_type"] = "post_exp042_event_rotation_surface_tilt_revalidation"
    payload["changed_variable"] = "rotation_breakout_leadership_paper_notional_tilt"
    payload["hypothesis"] = (
        "After the accepted exp-20260516-042 ISRG core-risk adaptation, the "
        "strongest available non-core alpha family may still be the default-off "
        "event bundle. Within that paper sleeve, rotation_breakout_leadership "
        "events should retain superior replacement quality and deserve a "
        "bounded extra paper-notional tilt."
    )
    payload["alpha_hypothesis"] = {
        "category": "allocation/event-quality",
        "entry_exit_ranking_or_allocation": "allocation",
        "why_this_now": (
            "The playbook points away from LLM/SEC soft-ranking when PIT-safe "
            "fields are thin and away from nearby core scalar retunes after the "
            "recent accepted TSM/ISRG ticker adaptations. This run keeps the "
            "fixed core candidate set, avoids noisy universe expansion, and "
            "revalidates the best deterministic default-off event allocation "
            "state on the latest accepted core baseline."
        ),
    }
    history = dict(payload.get("historical_experiment_check") or {})
    history["exp-20260516-040"] = (
        "The same all-source rotation surface cleared the paper-only gate after "
        "the exp039 TSM core-risk adaptation."
    )
    history["exp-20260516-042"] = (
        "Accepted ISRG core-risk adaptation moved the latest core stack to "
        "aggregate EV 7.8348, so the default-off event allocation evidence "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            ...
            "retain superior replacement value after the exp042 core stack."
        ),
        "2_history_check": (
            "exp013, exp028, and exp040 passed for the same all-source rotation "
            "surface; exp030 rejected only a source-specific split. exp042 later "
            "changed the accepted core baseline, while recent LLM/FINRA/Space/"
            "ATR branches are blocked or rejected."
        ),
        ...
        "5_reproducibility": (
            ...
            "exp_20260516_044_event_rotation_surface_tilt_after_exp042.py"
        ),
    }
    ...
    payload["production_impact"] = {
        ...
        "promotion_blocker_if_positive": (
            "Before live/default capital, enable a shared trade-enabled event "
            "adapter used by both run.py and backtester.py, add parity tests, "
    ...
    payload["why_not_other_attractive_points"] = (
        ...
        "old-window noise; avoided nearby core scalar retunes after exp042 "
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Rotation-Surface Tilt After Exp042",
        "",
        ...
        (
            ...
            "event-surface add-on after the accepted exp-20260516-042 core stack."
        ),
        "",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    compact = exp040.exp028.base.prior._compact_log(payload)
    ...
    _parent()._write_json(
        TICKET_JSON,
        {
            ...
            "title": "Event rotation-surface tilt after exp042",
...
def main() -> int:
    _configure_modules()
    payload = exp040.exp028.base.prior._retag_payload(_parent().build_payload())
    payload = exp040.exp028.base._finalize_payload(payload)
    payload = _retag_after_exp042(payload)
    persist(payload)
    best = payload["best_variant"]
    gate = payload["delta_metrics"]["variant_vs_current_lead"][best]
    print(

"""exp-20260530-008: SEC first/isolated vs follow-on event attribution.

Read-only alpha discovery for a new event-graph field:
``sec_first_reaction_followon_event_bucket_v1``. The experiment checks whether
SEC filings that follow a prior same-ticker, same-event-family filing within
30 calendar days have better 10-trading-day forward outcomes than first or
isolated filings.
...
EXPERIMENT_ID = "exp-20260530-008"
STEM = "sec_first_reaction_followon_attribution"
RULE_VERSION = "sec_first_reaction_followon_event_bucket_v1"
BASE_NOTIONAL_USD = 10000.0
ROUND_TRIP_COST_BPS = 35.0
HOLD_TRADING_DAYS = 10
LOOKBACK_CALENDAR_DAYS = 30
FOLLOWON_SHORT_DAYS = 7
...
def _sequence_bucket(prior_count_30d: int, days_since_prior: int | None) -> str:
    if prior_count_30d <= 0 or days_since_prior is None:
        return "first_or_isolated"
    if prior_count_30d >= 2:
        return "repeat_cluster_3plus_30d"
    if days_since_prior <= FOLLOWON_SHORT_DAYS:
        return "followon_1_7d"
    return "followon_8_30d"


def _build_sequence_features(rows: list[dict[str, Any]]) -> dict[tuple[str, str, str], dict[str, Any]]:
    ...
    for (_ticker, _family), events in grouped.items():
        events.sort(
            key=lambda item: (
                item["date_obj"],
                item["accepted_at"],
        ...
        for current_date in sorted(by_date):
            recent_prior_dates = [
                ...
                if 0 < (current_date - prior).days <= LOOKBACK_CALENDAR_DAYS
            ]
            most_recent_prior = max(recent_prior_dates) if recent_prior_dates else None
            days_since_prior = (
                (current_date - most_recent_prior).days if most_recent_prior is not None else None
            )
            prior_count_30d = len(recent_prior_dates)
            bucket = _sequence_bucket(prior_count_30d, days_since_prior)
            sequence_index = len(prior_dates)
            for item in by_date[current_date]:
                ...
                features[key] = {
                    "event_sequence_bucket": bucket,
                    "followon_event": prior_count_30d > 0,
                    "prior_same_family_event_count_30d": prior_count_30d,
                    "days_since_prior_same_family_event": days_since_prior,
                    "sequence_index_in_ticker_family": sequence_index,
...
def _analyze_events() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    sequence_features = _build_sequence_features(source_rows)
    ...
    for row in source_rows:
        ...
        if outcome is None:
            ...
            continue
        family = _event_family_bucket(row)
        sequence = sequence_features.get(
            (ticker, accession, signal_date),
            {
                "event_sequence_bucket": "first_or_isolated",
                "followon_event": False,
                "prior_same_family_event_count_30d": 0,
                "days_since_prior_same_family_event": None,
                "sequence_index_in_ticker_family": 0,
                "lookback_calendar_days": LOOKBACK_CALENDAR_DAYS,
                "same_trade_date_siblings_not_counted_as_prior": True,
            },
        )
        analyzed.append(
            {
                ...
                "known_at": (
                    "after_prior_same_ticker_same_family_sec_accepted_at_history_"
    ...
    audit = {
        ...
        "sequence_rule": {
            "lookback_calendar_days": LOOKBACK_CALENDAR_DAYS,
            "followon_short_days": FOLLOWON_SHORT_DAYS,
            "same_trade_date_siblings_not_counted_as_prior": True,
        },
    }
...
def _summaries(rows: list[dict[str, Any]]) -> dict[str, Any]:
    sequence_buckets = [
        ...
        "followon_1_7d",
        "followon_8_30d",
        "repeat_cluster_3plus_30d",
    ]
    by_sequence = {
        bucket: _summary([row for row in rows if row["event_sequence_bucket"] == bucket])
        for bucket in sequence_buckets
    }
    first_rows = [row for row in rows if not row.get("followon_event")]
    followon_rows = [row for row in rows if row.get("followon_event")]
    by_window: dict[str, dict[str, Any]] = {}
    for label in WINDOWS:
        window_rows = [row for row in rows if row["window"] == label]
        window_first = [row for row in window_rows if not row.get("followon_event")]
        window_followon = [row for row in window_rows if row.get("followon_event")]
        by_window[label] = {
            "all": _summary(window_rows),
            "first_or_isolated": _summary(window_first),
            "followon_event": _summary(window_followon),
        }
    by_family = {}
    for family in sorted({row["event_family_bucket"] for row in rows}):
        ...
        by_family[family] = {
            ...
            "first_or_isolated": _summary([row for row in family_rows if not row.get("followon_event")]),
            "followon_event": _summary([row for row in family_rows if row.get("followon_event")]),
        }
    return {
        "all": _summary(rows),
        "first_or_isolated": _summary(first_rows),
        "followon_event": _summary(followon_rows),
        "by_sequence_bucket": by_sequence,
        "by_window": by_window,
...
def _decision(summary: dict[str, Any]) -> tuple[str, str, dict[str, Any], str, str | None]:
    followon = summary["followon_event"]
    first = summary["first_or_isolated"]
    windows = summary["by_window"]
    followon_count_ok = followon["count"] >= MIN_FOLLOWON_ROWS
    avg_return_lift = (
        followon["avg_return"] - first["avg_return"]
        if followon["avg_return"] is not None and first["avg_return"] is not None
        else None
    )
    avg_pnl_lift = (
        followon["avg_pnl"] - first["avg_pnl"]
        if followon["avg_pnl"] is not None and first["avg_pnl"] is not None
        else None
    )
    ...
    concentration_share = followon["positive_concentration"]["top_ticker_positive_share"]
    concentration_ok = (
    ...
    for row in windows.values():
        wf = row["followon_event"]
        wi = row["first_or_isolated"]
        if (
    ...
    evidence = {
        ...
        "followon_count_ok": followon_count_ok,
        "followon_count": followon["count"],
        "min_followon_rows": MIN_FOLLOWON_ROWS,
        "avg_return_lift_vs_first_or_isolated": _round(avg_return_lift, 6),
        "min_avg_return_lift": MIN_AVG_RETURN_LIFT,
        "return_lift_ok": return_lift_ok,
        "avg_pnl_lift_vs_first_or_isolated": _round(avg_pnl_lift, 4),
    ...
    if passed:
        return (
            "observed_only_candidate_sec_first_reaction_followon_field",
            "observed_only",
            evidence,
            (
                "SEC same-ticker same-family follow-on events cleared the observed-only "
    ...
    failed_parts = [
        key
        for key, ok in {
            "followon_count": followon_count_ok,
    ...
    reason = "Rejected: SEC follow-on sequencing did not clear " + ", ".join(failed_parts) + "."
    return (
        "rejected_sec_first_reaction_followon_field",
        "rejected",
        evidence,
        reason,
        reason,
...
def _calibration(
    ...
    if "count" in reason:
        realized_failure_mode = "thin_followon_sample"
        ...
        realized_failure_mode = "weak_followon_separation"
...
def _build_payload() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    before_metrics = summary["first_or_isolated"]
    after_metrics = summary["followon_event"]
    delta_metrics = {
        "avg_return": evidence["avg_return_lift_vs_first_or_isolated"],
        "avg_pnl": evidence["avg_pnl_lift_vs_first_or_isolated"],
    ...
    return (
        {
            ...
            or (
                "SEC follow-on filings in the same ticker/event-family may identify "
                "continuing information absorption."
            ),
            "change_summary": (
                "Read-only attribution of SEC filings by first/isolated versus "
                "30-calendar-day same-ticker same-family follow-on sequence bucket "
            ...
            "trial_family": "sec_first_reaction_followon_attribution",
            ...
            "new_evidence_type": "new_event_sequence_graph_field",
            "summary": summary_text,
            "source_audit": audit,
            "preflight_questions": {
                "1_alpha_hypothesis": (
                    "alpha_discovery/event graph: follow-on SEC filings may identify "
                    "continuing information absorption versus first or isolated filings."
                ),
                "2_history_check": (
                    "exp-20260530-006 rejected raw same-day same-family burst count; "
                    "older SEC negative first-reaction work used reaction/language packets, "
                    "not same-ticker event-family sequence buckets."
                ),
                "3_single_causal_variable": RULE_VERSION,
                "4_acceptance_standard": (
                    "Follow-on bucket >=30 closed outcomes, >=$150 avg PnL lift and "
                    ">=1.5pp avg return lift versus first/isolated, positive PnL not "
                    "dominated by one ticker, and at least two windows with follow-on "
                    "avg PnL above first/isolated."
                ),
                "5_reproducibility": (
                    ".\\.venv\\Scripts\\python.exe -B quant\\experiments\\"
                    "exp_20260530_008_sec_first_reaction_followon_attribution.py"
                ),
            },
            "backtest_protocol": {
            ...
            "why_not_other_changes": (
                ...
                "and OHLCV pattern-name retunes because those families are frozen, "
...
def _build_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} SEC First/Follow-On Attribution",
        "",
    ...
    for bucket, row in payload["bucket_summaries"]["by_sequence_bucket"].items():
        lines.append(
    ...
    for window, row in payload["bucket_summaries"]["by_window"].items():
        lines.append(
            "| {window} | {fc} | {fp} | {ip} | {fw} | {iw} |".format(
                window=window,
                fc=row["followon_event"]["count"],
                fp=row["followon_event"]["avg_pnl"],
                ip=row["first_or_isolated"]["avg_pnl"],
                fw=row["followon_event"]["win_rate"],
                iw=row["first_or_isolated"]["win_rate"],
            )
        )
    lines.extend(
        [
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        'hub_repo_id: "ginger/experiments/exp-20260530-008"',
        "---",
        "",
        ...
        "",
        f"- Decision: `{payload['decision']}`",
        f"- Follow-on count: `{evidence['followon_count']}`",
        f"- Avg PnL lift vs first/isolated: `{evidence['avg_pnl_lift_vs_first_or_isolated']}`",
        f"- Avg return lift vs first/isolated: `{evidence['avg_return_lift_vs_first_or_isolated']}`",
        ...
        json.dumps(
            {
                ...
                "followon_count": payload["gate4"]["decision_evidence"]["followon_count"],
                "avg_pnl_lift_vs_first_or_isolated": payload["gate4"]["decision_evidence"][
                    "avg_pnl_lift_vs_first_or_isolated"
                ],
                "avg_return_lift_vs_first_or_isolated": payload["gate4"]["decision_evidence"][
                    "avg_return_lift_vs_first_or_isolated"
                ],

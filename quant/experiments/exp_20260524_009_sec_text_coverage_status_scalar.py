"""exp-20260524-009: SEC text-coverage status notional scalar.

Alpha search on one production-visible SEC provenance field. The run tests
whether `sec_text_coverage_status == missing_text_archive` has allocation value
inside the default-off SEC financial-report paper sleeve.
...
EXPERIMENT_ID = "exp-20260524-009"
STEM = "exp_20260524_009_sec_text_coverage_status_scalar"
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_text_coverage_status_scalar.md"
)
EXPERIMENT_LOG_JSONL = REPO_ROOT / "docs" / "experiment_log.jsonl"

ACCEPTED_EARNINGS_RELEASE_SPY_CONTEXT_SCALAR = 1.10
TARGET_FIELD = "sec_text_coverage_status"
TARGET_VALUE = "missing_text_archive"
CHANGED_VARIABLE = "sec_missing_text_archive_notional_scalar"
TRIAL_FAMILY = "sec_text_coverage_status_allocation"
TARGET_SCALARS: "OrderedDict[str, float]" = OrderedDict(
    [
        ("missing_text_scalar_0_50", 0.50),
        ("missing_text_scalar_0_75", 0.75),
        ("missing_text_scalar_1_00", 1.00),
        ("missing_text_scalar_1_25", 1.25),
        ("missing_text_scalar_1_50", 1.50),
        ("missing_text_scalar_2_00", 2.00),
    ]
)
...
def _is_target_position(position: dict[str, Any]) -> bool:
    return _source_candidate(position).get(TARGET_FIELD) == TARGET_VALUE
...
def _patched_notional_for_position(
    ...
    *,
    ...
    return (
        ...
        scalar,
        f"{rule}+sec_text_coverage_status_missing_text_archive_scalar",
    )
            ...
            rows.append(
                {
                    ...
                    "sec_text_coverage_status": candidate.get(TARGET_FIELD),
...
def _gate(summary: dict[str, Any], selection: dict[str, Any]) -> dict[str, Any]:
    ...
    causal_checks = {
        ...
        "reason": (
            "`missing_text_archive` is a data-coverage/provenance state, not a "
            "stable economic or semantic event-quality field. Promoting it would "
            "risk production/backtest inconsistency if text coverage changes."
        ),
    }
    return {
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} SEC Text-Coverage Status Scalar",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    baseline = variants["missing_text_scalar_1_00"]
    for key, scalar in TARGET_SCALARS.items():
    ...
    non_baseline = [key for key in TARGET_SCALARS if key != "missing_text_scalar_1_00"]
    ...
    if (
        ...
        rejection_reason = (
            "No SEC financial-report paper-sleeve closed positions matched "
            "`sec_text_coverage_status == missing_text_archive` across the canonical "
            "three-window replay, so the scalar has no measurable allocation "
            "effect and fails the sample/metric gate."
        )
        ...
        rejection_reason = (
            "The best variant improved aggregate EV/PnL across all three windows, "
            "but failed the numeric SEC paper-sleeve gate on "
            f"{', '.join(failed_checks)}. Keeping it would amplify a tail/concentration "
            "pattern before the causal field is production-stable."
        )
    else:
        rejection_reason = (
            "The best metric variant passed the numeric three-window SEC paper-sleeve "
            "gate, but the changed field is `missing_text_archive`, a data-coverage "
            "state rather than a stable semantic/economic signal. Keeping it would "
            "create production/backtest inconsistency risk as SEC text coverage changes."
        )
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "SEC financial-report paper entries with missing SEC filing-text archives "
            "may proxy for slower periodic-report drift and deserve a bounded "
            "paper-notional scalar, but the field must be rejected if the apparent "
            "edge is caused by data availability rather than a stable event-quality "
            "state."
        ),
        "change_summary": (
            "Sweep one paper-notional scalar for SEC financial-report paper-sleeve "
            "positions whose source candidate has sec_text_coverage_status == "
            "missing_text_archive."
        ),
        ...
        "new_evidence_type": "production_visible_sec_text_provenance_field",
        "parameters": {
        ...
        "next_evidence_needed": (
            "Do not use SEC text archive missingness as an allocation signal. "
            "If this edge is revisited, replace it with a stable production-visible "
            "field such as form type, accession-matched text_event_type, or an "
            "explicit disclosure-quality bucket and rerun Gate 1-4."
        ),
        "production_impact": {
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "capital allocation / event scoring: missing SEC text coverage "
                "may proxy for a periodic-report drift state, but must clear a "
                "production-stability guard."
            ),
            "2_history_check": (
                "Prior SEC semantic scalars around positive/negative/unclassified "
                "language were rejected or concentrated; exp-20260524-004 added "
                "text provenance; exp-20260524-005 rejected same-ticker repeat."
            ),
        ...
        "llm_metrics": {
            ...
            "blocker_relation": (
                "This deterministic SEC provenance field does not change LLM prompts, "

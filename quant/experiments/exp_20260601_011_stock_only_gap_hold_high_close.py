"""exp-20260601-011: stock-only gap-up hold high-close candidate-pool scout.

Lane: alpha_search.
Single causal variable: stock_only_operating_company_candidate_pool_governance_v1.

This replay-only scout keeps the rejected exp-20260601-010 gap-up / intraday
hold / high-close / volume-confirmed source fixed, but adds one candidate-pool
governance variable: exclude non-operating securities such as ETF, ETN, fund,
trust, leveraged/inverse, commodity, crypto-trust, and volatility-proxy rows
using free SEC company-ticker title metadata. It changes no production orders,
core ranking, sizing, exits, LLM/news inputs, or watchlists.
...
EXPERIMENT_ID = "exp-20260601-011"
STEM = "stock_only_gap_hold_high_close"
TRIAL_FAMILY = "stock_only_gap_up_hold_high_close_candidate_pool"
CHANGED_VARIABLE = "stock_only_operating_company_candidate_pool_governance_v1"
            ...
            candidates_by_date.setdefault(asof_str, []).append(
                {
                    ...
                    "stock_only_governance": "sec_title_operating_company_pass",
...
def _artifact(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID}: Stock-Only Gap-Up Hold High-Close Candidate Pool",
        "",
    ...
    lines.extend(
        [
            ...
            "at the signal close plus free SEC company-title metadata for stock-only "
            "governance. It is replay-only/default-off, so no production entry, "
            "ranking, sizing, exit, LLM/news, watchlist, or order behavior changed.",
            "",
...
def _build_payload() -> dict[str, Any]:
    ...
    decision = (
        ...
        if gate4_passed
        else "rejected_stock_only_gap_hold_high_close_candidate_pool"
    )
    rationale = (
    ...
    return {
        ...
        "hypothesis": (
            "Stock-only governance may improve the rejected broad gap-up hold "
            "high-close candidate pool by removing ETF/ETN/leveraged/inverse/"
            "commodity proxy noise while preserving free OHLCV institutional-demand signals."
        ),
        "change_type": "default_off_candidate_pool_scout",
        "changed_variable": CHANGED_VARIABLE,
        "trial_family": TRIAL_FAMILY,
        "trial_variant_id": CHANGED_VARIABLE,
        "mechanism_family": "free_ohlcv_candidate_pool_quality",
        "prior_trial_count": 1,
        "nearby_prior_experiments": ["exp-20260601-010", "exp-20260601-007", "exp-20260601-008"],
        "multiple_testing_risk_bucket": "moderate",
        "new_evidence_type": "free_ohlcv_candidate_pool_governance",
        "backtest_protocol": {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "candidate_pool: stock-only governance may turn the rejected gap-up/"
                "hold/high-close source into a cleaner operating-company demand signal."
            ),
            "2_history_check": (
                "exp-20260601-010 tested the same gap/hold/high-close thresholds and "
                "rejected the broad pool because old_thin regressed and drawdown drifted; "
                "that selected pool included ETF/ETN/commodity/volatility proxies such as "
                "AGQ, BOIL, UCO, UNG, UVIX, VXX, ETHA, and FETH. exp-20260601-007/008 "
                "also warn that broad OHLCV continuation can be ordinary momentum."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260601_011_stock_only_gap_hold_high_close.py"
            ),
        },
        "gate1": {
            "passed": True,
        ...
        "next_retry_requires": [
            ...
            "a richer stock-type classifier if SEC title governance is too coarse",
        ],
        "related_files": [

"""exp-20260620-009: supplier-financing/debt-relief shared 4k adapter.

Full-stack candidate-pool experiment. It tests the production-faithful version
of the supplier-financing/debt-relief free Companyfacts candidate source:
historical replay and daily default-off snapshots share one helper, use the
same $4,000 paper base notional, and apply the same one-way PIT
volatility/liquidity notional scalar.

exp-20260620-008 showed the larger exp-20260620-007 private replay magnitude
was not reproducible through shared semantics. This run keeps that drift as a
diagnostic but judges the corrected executable bundle on its own canonical
three-window economics and production contract.
...
STEM = "supplier_financing_debt_relief_shared_4k_risk_scaled_adapter"
TRIAL_FAMILY = "supplier_financing_debt_relief_shared_4k_risk_scaled_adapter"
TRIAL_VARIANT_ID = RULE_VERSION
CHANGED_VARIABLE = "supplier_financing_debt_relief_shared_4k_risk_scaled_default_off_adapter_v1"
SOURCE_LEAD_EXPERIMENT_ID = "exp-20260620-007"
OWNER = "alpha-search-automation"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260620_009_{STEM}.json"
...
PREDICTION = {
    ...
    "expected_ev_delta": 0.68,
    "expected_pnl_delta": 12_355.0,
    "main_failure_modes": [
        "exp008_positive_is_measurement_artifact",
        "drawdown_drift",
        "accepted_distribution_comparator_not_beaten",
        "production_daily_helper_semantics_diverge",
    ],
    "confidence_reason": (
        "exp-20260620-008 rejected exact exp007 promotion because private "
        "lead magnitude drifted, but the shared $4,000 helper itself was "
        "positive in all three canonical windows, beat accepted comparators, "
        "kept drawdown drift within guardrail, and added parity tests."
    ),
    "recorded_at": "2026-06-20T08:25:53+00:00",
}
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        ...
        "debt-relief Companyfacts source may have real replacement value even "
        "after correcting the exp-20260620-007 private replay notional drift. "
        "This run tests the production-faithful shared $4,000 paper base "
        "notional plus PIT volatility/liquidity one-way scalar bundle."
    ),
    "2_history_check": {
        "exp-20260620-008": (
            "Exact exp007 promotion was rejected because the shared helper did "
            "not reproduce the larger private replay magnitude. The same "
            "production-faithful shared helper was still positive in all three "
            "windows: aggregate EV +0.6801, PnL +$12,355.48, 88 trades, "
            "drawdown drift +0.0016, and accepted comparators beaten."
        ),
        "exp-20260620-007": (
            "Private replay lead reported aggregate EV +1.7519 and PnL "
            "+$30,888.69, but exp008 showed this magnitude should not be used "
            "as a promotion baseline because shared semantics did not "
            "reproduce it."
        ),
        "exp-20260620-005": (
        ...
        "novelty_gate": (
            "Novelty override was accepted on the new evidence axis that "
            "exp008 discovered private-lead notional drift; this run freezes "
            "the production-faithful shared $4,000 bundle and judges it on "
            "canonical economics instead of private-lead reproduction."
        ),
    },
    ...
    "4_acceptance_standard": (
        ...
        "beaten, and the helper remains default-off with no live/default "
        "order changes."
    ),
    "5_reproducibility": (
        ".venv\\Scripts\\python.exe -B quant\\experiments\\"
        "exp_20260620_009_supplier_financing_debt_relief_shared_4k_risk_scaled_adapter.py"
    ),
}
...
def _gate4_canonical(
    *,
    ...
    gate["failed_reasons"] = list(gate.get("failed_reasons") or [])
    gate["passed"] = not gate["failed_reasons"]
    gate["decision"] = (
        "accepted_supplier_financing_debt_relief_shared_4k_risk_scaled_default_off_adapter"
        if gate["passed"]
        else "rejected_supplier_financing_debt_relief_shared_4k_risk_scaled_default_off_adapter"
    )
    gate["private_lead_remeasurement"] = {
        **lead_reproduction,
        "gating_role": "diagnostic_only_not_gate4_failure",
        "interpretation": (
            "exp007's larger private replay magnitude is not promoted; this "
            "experiment judges the production-faithful shared $4,000 bundle."
        ),
    }
...
def build_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "new_evidence_type": "production_faithful_remeasurement_after_private_replay_notional_drift",
    ...
    auxiliary_promotion_gate = evaluate_gate4(full_stack_metrics, check_materiality=False)
    canonical_gate4 = {
        "passed": gate4["passed"],
        "status": "passed" if gate4["passed"] else "blocked",
        "source": "experiment_specific_comparator_gate",
        "binding_gate": gate4,
        "auxiliary_promotion_gate": auxiliary_promotion_gate,
    }
    ...
    if not gate4["passed"]:
        verdict = {
            **verdict,
            ...
            "gate4_passed": False,
            "next_step": (
                "Roll back the sleeve change and log the failure. The "
                "production-faithful shared helper did not pass canonical "
                "Gate 4."
            ),
        }
    ...
    payload["full_stack"] = {
        ...
        "materiality_note": (
            ...
            "accepted comparator after costs. The generic promotion gate is "
    ...
    payload["interpretation"] = (
        "The production-faithful supplier-financing/debt-relief shared $4,000 "
        "risk-scaled bundle passed canonical three-window Gate 4 through a "
        "default-off helper and daily snapshot path; the full-stack verdict "
        "is accepted_paper_pending_forward."
        if accepted
        else (
            "The production-faithful supplier-financing/debt-relief shared "
            "$4,000 risk-scaled bundle failed canonical full-stack promotion; "
            "do not retain the helper as accepted alpha."
        )
    )
    payload["post_run_reflection"] = {
        "why_result_happened": (
            "The shared helper kept the exp-20260620-005 DPO+debt-relief "
            "candidate source, top-1/day, 10-day cooldown, next-open/"
            "10-day-close entry/exit pricing semantics, costs, a "
            "production-faithful $4,000 base paper notional, and the "
            "exp-20260620-007 one-way PIT "
            "volatility/liquidity scalar. Daily observation only relaxes the "
            "future exit-bar requirement needed for same-day pending paper "
            "rows. exp007's larger private replay magnitude remains rejected "
            "as a diagnostic drift, not the accepted alpha magnitude."
            if accepted
            else (
                "The production-faithful shared helper failed canonical "
                "windows, implying the corrected $4,000 bundle was too weak "
                "after removing the private replay magnitude drift."
            )
        ),
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    repro = payload["gate4"]["private_lead_remeasurement"]
    ...
    return "\n".join(
        [
            ...
            *_window_table(payload),
            ...
            "- Private lead remeasurement EV drift: `{:+.6f}`".format(
                repro.get("aggregate_expected_value_score_delta_drift", 0.0)
            ),
            "- Private lead remeasurement PnL drift: `${:+,.2f}`".format(
    ...
    return "\n".join(
        [
            ...
            *_window_table(payload),
            ...
            json.dumps(
                {
                    "private_lead_remeasurement": payload["gate4"]["private_lead_remeasurement"],

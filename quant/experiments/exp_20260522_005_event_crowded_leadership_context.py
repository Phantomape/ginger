"""exp-20260522-005: event crowded leadership context haircut.
...
whether rows in generic balanced leadership or narrow cap-weight leadership
should receive a paper-notional haircut.
...
EXPERIMENT_ID = "exp-20260522-005"
EXPERIMENT_SLUG = "event_crowded_leadership_context"
...
GENERIC_STATE_SURFACE = "balanced_state_leadership"
NARROW_STATE_BUCKET = "narrow_cap_weight_leadership"
    ...
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "crowded_leadership_context_scalar": 1.00,
            },
        ),
        (
            "crowded_leadership_context_090",
            {
                "description": "0.90x paper notional for crowded leadership event rows.",
                "crowded_leadership_context_scalar": 0.90,
            },
        ),
        (
            "crowded_leadership_context_075",
            {
                "description": "0.75x paper notional for crowded leadership event rows.",
                "crowded_leadership_context_scalar": 0.75,
            },
        ),
        (
            "crowded_leadership_context_050",
            {
                "description": "0.50x paper notional for crowded leadership event rows.",
                "crowded_leadership_context_scalar": 0.50,
            },
        ),
        (
            "crowded_leadership_context_025",
            {
                "description": "0.25x paper notional for crowded leadership event rows.",
                "crowded_leadership_context_scalar": 0.25,
            },
        ),
    ]
)
...
def _is_target_crowded_leadership(trade: dict[str, Any]) -> bool:
    return (
        str(trade.get("state_surface") or "") == GENERIC_STATE_SURFACE
        or str(trade.get("state_bucket") or "") == NARROW_STATE_BUCKET
    )
...
def _scaled_trade(
    ...
    target = _is_target_crowded_leadership(trade)
    context_scalar = (
        float(variant["crowded_leadership_context_scalar"]) if target else 1.0
    )
    ...
    return {
        **trade,
        ...
        "crowded_leadership_context_target": target,
        "crowded_leadership_context_scalar": round(context_scalar, 4),
...
def _max_loss_share(rows: list[dict[str, Any]]) -> float | None:
    losses = [abs(float(row.get("pnl") or 0.0)) for row in rows if float(row.get("pnl") or 0.0) < 0]
    total = sum(losses)
    if total <= 0:
        return None
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("crowded_leadership_context_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("crowded_leadership_context_target")
        ]
    ...
    return {
        "target_rule": (
            f"state_surface == {GENERIC_STATE_SURFACE} OR "
            f"state_bucket == {NARROW_STATE_BUCKET}"
        ),
        "target_trade_count": len(targets),
        "target_windows_present": sum(
            1 for row in target_by_window.values() if row["trade_count"] > 0
        ),
        "target_loss_windows_present": sum(
            1 for row in target_by_window.values() if row["total_pnl"] < 0
        ),
        ...
        "target_losses": sum(1 for row in targets if float(row.get("pnl") or 0.0) < 0),
...
def _gate_vs_baseline(
    ...
    sample_ok = (
        (baseline_selection.get("target_trade_count") or 0) >= 8
        and (baseline_selection.get("target_windows_present") or 0) >= 3
        and (baseline_selection.get("target_loss_windows_present") or 0) >= 3
        and len(baseline_selection.get("target_tickers") or []) >= 6
        and (baseline_selection.get("target_losses") or 0) >= 4
        ...
        and (
            ...
            or baseline_selection["target_max_single_loss_pnl_share"] <= 0.55
        )
    )
    ...
    return {
        **gate,
        ...
        "sample_guard": {
            ...
            "max_target_single_loss_pnl_share": 0.55,
            ...
            "actual_target_loss_windows": baseline_selection.get(
                "target_loss_windows_present"
            ),
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -abs(VARIANTS[name]["crowded_leadership_context_scalar"] - 1.0),
        ),
    )
...
def _compact_metrics_by_window(rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return OrderedDict(
        (label, {field: metrics.get(field) for field in fields if field in metrics})
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_default_off_event_crowded_leadership_context"
        if accepted
        else "rejected_event_crowded_leadership_context"
    )
    ...
    compact_parameters = {
        ...
        "selected_crowded_leadership_context_scalar": VARIANTS[best_variant][
            "crowded_leadership_context_scalar"
        ],
        "variant_scalars": {
            name: row["crowded_leadership_context_scalar"]
    ...
    return {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "event_crowding_context_allocation_adapter",
        "mechanism_family": "external_event_satellite_overlay_allocation",
        "trial_family": "event_market_state_crowding_context",
        "trial_variant_id": "crowded_leadership_context_notional_haircut",
        "changed_variable": "event_crowded_leadership_context_scalar",
        "prior_trial_count": 14,
        ...
        "new_evidence_type": "new_event_generic_or_narrow_leadership_crowding_cohort",
        "hypothesis": (
            "Inside the accepted default-off event overlay, event rows exposed "
            "to generic balanced leadership or narrow cap-weight leadership are "
            "lower replacement-value rows. A single paper-notional haircut can "
            "reduce crowded-context tail drag without changing core trades."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event context scoring",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Uses event-family x market-state/crowding decomposition from "
                "the playbook while avoiding LLM soft-ranking, broad-market "
                "identity drift, and state-surface sleeve retunes."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for fixed accepted event rows where "
            "state_surface is balanced_state_leadership or state_bucket is "
            "narrow_cap_weight_leadership; all accepted event scalars and core "
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Crowded/generic leadership event rows may have lower "
                "replacement value; this is event context scoring plus capital allocation."
            ),
            "2_history_check": (
                "Recent event context experiments accepted broad-breadth, "
                "governance source quality, negative reaction, positive state, "
                "and non-narrow context. Same-ticker attention persistence, "
                "low-extension, benchmark momentum, volume confirmation, mixed "
                "breadth, and high dispersion did not clear gates. This run "
                "uses a broader production-visible crowding cohort, not the "
                "4-trade narrow bucket alone."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for the fixed crowded leadership "
                "event cohort changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260522_005_event_crowded_leadership_context.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260521-013": "Accepted non-narrow context adapter; current event baseline.",
            "exp-20260521-019": "Rejected same-ticker attention persistence.",
            "exp-20260522-001": "Rejected event low-extension context.",
            "exp-20260522-002": "Rejected momentum-volume confirmation top-up.",
            "thin_narrow_only_precheck": (
                "The narrow cap-weight bucket alone was only four trades, so "
                "this experiment uses the broader generic-or-narrow leadership "
                "crowding cohort with 8 trades across all three windows."
            ),
        },
        "backtest_protocol": {
            "source": (
        ...
        "gate2": {
            ...
            "passed": bool(operator_check["passed"] and baseline_selection["target_trade_count"] >= 8),
        },
        "gate3": {
        ...
        "decision_rationale": (
            ...
            if accepted
            else "Rejected: crowded leadership context scalar did not clear Gate 4."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "If accepted, keep the 0.25 crowded leadership paper scalar in the "
            "shared default-off event bundle and wait for new forward event rows "
            "before nearby crowding retunes."
        )
        if accepted
        else (
            "Do not retry nearby event crowding scalars on the frozen sample "
            "without new forward rows or a materially distinct crowding field."
        ),
        "why_not_other_attractive_points": (
            "Skipped LLM soft-ranking due attribution limits; skipped "
            "core-misfit because it has zero closed primary outcomes; skipped "
            "broad-market promotion because current replay identity drift blocks "
            "trustworthy comparison; skipped ETF adjacent selector/cap retunes "
            "after the latest vol-cap rejection."
        ),
        "risk_of_change": (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Crowded Leadership Context",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests whether generic balanced leadership or narrow "
            "cap-weight leadership event rows should receive a paper-notional "
            "haircut on top of the accepted event non-narrow context adapter."
        ),
        "",
        "## Gate 4 Result",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "Shared default-off event adapter/reporting changes only if the "
                "experiment is accepted. Core behavior, source capacity, and "
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event crowded leadership context",

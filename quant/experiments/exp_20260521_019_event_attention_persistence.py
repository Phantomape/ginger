"""exp-20260521-019: event attention-persistence context scout.

Alpha search, replay-only. Tests one production-visible event context field on
top of the accepted exp-20260521-013 default-off event overlay adapter: whether
fixed selected event rows whose ticker had another selected event row within
the prior 60 calendar days deserve different paper notional.
...
EXPERIMENT_ID = "exp-20260521-019"
EXPERIMENT_SLUG = "event_attention_persistence"
    ...
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "attention_persistence_scalar": 1.0,
            },
        ),
        (
            "attention_persistence_075",
            {
                "description": "0.75x paper notional for recent same-ticker event rows.",
                "attention_persistence_scalar": 0.75,
            },
        ),
        (
            "attention_persistence_090",
            {
                "description": "0.90x paper notional for recent same-ticker event rows.",
                "attention_persistence_scalar": 0.90,
            },
        ),
        (
            "attention_persistence_110",
            {
                "description": "1.10x paper notional for recent same-ticker event rows.",
                "attention_persistence_scalar": 1.10,
            },
        ),
        (
            "attention_persistence_125",
            {
                "description": "1.25x paper notional for recent same-ticker event rows.",
                "attention_persistence_scalar": 1.25,
            },
        ),
        (
            "attention_persistence_150",
            {
                "description": "1.50x paper notional for recent same-ticker event rows.",
                "attention_persistence_scalar": 1.50,
            },
        ),
    ]
)
...
def _annotate_attention_persistence(
    ...
    for label, rows in rows_by_window.items():
        ...
        indexed_rows.sort(
            key=lambda item: (
                _parse_date(item[1].get("entry_date")) or date.min,
                str(item[1].get("ticker") or ""),
                str(item[1].get("source") or ""),
            )
        )
        last_event_by_ticker: dict[str, dict[str, Any]] = {}
        by_index: dict[int, dict[str, Any]] = {}
        for original_index, row in indexed_rows:
            ticker = str(row.get("ticker") or "")
            current_date = _parse_date(row.get("entry_date"))
            previous = last_event_by_ticker.get(ticker)
            prior_days = None
            flag = False
            previous_source = None
            previous_entry_date = None
            if previous and current_date:
                previous_date = _parse_date(previous.get("entry_date"))
                if previous_date:
                    prior_days = (current_date - previous_date).days
                    flag = 0 < prior_days <= ATTENTION_LOOKBACK_DAYS
                    previous_source = previous.get("source")
                    previous_entry_date = previous.get("entry_date")
            by_index[original_index] = {
                **row,
                "attention_persistence_flag": flag,
                "attention_persistence_lookback_days": ATTENTION_LOOKBACK_DAYS,
                "prior_same_ticker_event_days": prior_days,
                "prior_same_ticker_event_source": previous_source,
                "prior_same_ticker_event_entry_date": previous_entry_date,
            }
            if ticker and current_date:
                last_event_by_ticker[ticker] = row
        annotated_by_window[label] = [by_index[index] for index in range(len(rows))]
    return annotated_by_window
...
def _scaled_trade(
    ...
    target = bool(trade.get("attention_persistence_flag"))
    persistence_scalar = (
        float(variant["attention_persistence_scalar"]) if target else 1.0
    )
    scalar = accepted_scalar * persistence_scalar
    ...
    return {
        **trade,
        ...
        "attention_persistence_target": target,
        "attention_persistence_scalar": round(persistence_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("attention_persistence_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("attention_persistence_target")
        ]
        target_by_window[label] = {
            ...
            "prior_sources": sorted(
                {
                    str(row.get("prior_same_ticker_event_source") or "")
                    for row in window_targets
                }
            ),
            "prior_day_gaps": sorted(
                {
                    int(row.get("prior_same_ticker_event_days"))
                    for row in window_targets
                    if row.get("prior_same_ticker_event_days") is not None
                }
            ),
...
def _gate_vs_baseline(
    ...
    sample_ok = (
        (selection.get("target_trade_count") or 0) >= 8
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -abs(VARIANTS[name]["attention_persistence_scalar"] - 1.0),
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    event_trades = _annotate_attention_persistence(event_trades)
    core_results = {
    ...
    decision = (
        "accepted_replay_only_event_attention_persistence"
        if accepted
        else "rejected_event_attention_persistence"
    )
    ...
    compact_parameters = {
        ...
        "selected_attention_persistence_scalar": VARIANTS[best_variant][
            "attention_persistence_scalar"
        ],
        "variant_scalars": {
            name: row["attention_persistence_scalar"]
    ...
    return {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "event_attention_persistence_allocation_scout",
        "mechanism_family": "external_event_satellite_overlay_allocation",
        "trial_family": "event_attention_persistence_context",
        "trial_variant_id": "recent_same_ticker_event_notional_scalar",
        "changed_variable": "event_attention_persistence_scalar",
        "prior_trial_count": 1,
        ...
        "new_evidence_type": "new_production_visible_context_field",
        "hypothesis": (
            "Inside the accepted default-off event overlay, repeated same-ticker "
            "event attention within 60 calendar days may identify stronger "
            "information persistence or crowding. A single paper-notional scalar "
            "tests that production-visible context field without changing core "
            "trades or event source definitions."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event attention-persistence scoring",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Uses the playbook's event source crowding and attention "
                "persistence lane, while avoiding LLM soft-ranking, state-surface "
                "retunes, Form 4 sparse rows, and nearby SEC special-call/source "
                "scalar retries."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for fixed event overlay rows whose ticker had "
            "a prior selected event row within 60 calendar days; event definitions, "
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Recent same-ticker event attention may signal persistence or "
                "crowding; this is event context scoring plus capital allocation."
            ),
            "2_history_check": (
                "Broad-market same-ticker repeat was rejected in exp-20260520-014, "
                "event source capacity was rejected in exp-20260521-003, event "
                "non-narrow state was accepted in exp-20260521-013, and SEC "
                "special-call context was rejected in exp-20260521-016. This run "
                "uses a different event attention-persistence field."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for fixed recent same-ticker event "
                "rows changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260521_019_event_attention_persistence.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260520-014": (
                "Rejected broad-market recent same-ticker repeat; not the same "
                "event overlay source/context field."
            ),
            "exp-20260521-003": "Rejected increasing event source capacity.",
            "exp-20260521-013": "Accepted non-narrow state-bucket context; current event baseline.",
            "exp-20260521-016": "Rejected SEC special-call disclosure quality scout.",
        },
        "backtest_protocol": {
            "source": (
        ...
        "gate2": {
            "required_fields": [
                ...
                "state_bucket",
                "reaction_bucket",
                "prior same-ticker selected event date computed from event rows",
            ],
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                ...
                "state/history metadata, report output, parity tests, and rerun "
                "before any retained adapter behavior."
            ),
        ...
        "llm_metrics": {
            ...
            "blocker_relation": (
                ...
                "deterministic event row chronology only."
            ),
        },
        "decision_rationale": (
            "Accepted as replay-only alpha evidence; no production/shared policy was changed."
            if accepted
            else "Rejected: attention-persistence scalar did not clear Gate 4."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "Do not retry nearby event attention-persistence scalars on the frozen "
            "sample without new forward rows or a shared production-visible "
            "history field."
        ),
        "why_not_other_attractive_points": (
            ...
            "candidates; skipped Space because recent forward same-theme evidence "
            "is RKLB/LUNR concentrated; skipped state-surface and broad-market "
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Attention Persistence",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests whether selected event rows with a prior "
            "same-ticker event in the last 60 calendar days deserve a "
            "paper-notional scalar on top of the accepted event non-narrow state "
            "context adapter."
        ),
        "",
        "## Gate 4 Result",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event attention persistence",

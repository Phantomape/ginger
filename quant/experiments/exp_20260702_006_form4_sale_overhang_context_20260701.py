"""exp-20260702-006: materialize 2026-07-01 Form4 sale-overhang context.

This measurement repair uses the existing shared Form4 context helper to write
the 2026-07-01 canonical context rows that the daily non-OHLCV snapshot skipped.
...
EXPERIMENT_ID = "exp-20260702-006"
OWNER = "alpha-explore"
LANE = "measurement_repair"
SLUG = "form4_sale_overhang_context_20260701"
RUNNER = f"quant/experiments/exp_20260702_006_{SLUG}.py"
RUNNER_COMMAND = ".\\.venv\\Scripts\\python.exe -B " + RUNNER.replace("/", "\\")

AS_OF = "2026-07-01"
CHANGED_VARIABLE = "form4_sale_overhang_context_20260701_materialization_v1"
TRIAL_FAMILY = "form4_sale_overhang_shared_daily_context_logger"
TRIAL_VARIANT_ID = "20260701_context_materialization_v1"
...
CURRENT_CONTEXT_JSONL = NON_OHLCV_DIR / "form4_sale_overhang_context_20260701.jsonl"
CURRENT_CONTEXT_SUMMARY = NON_OHLCV_DIR / "form4_sale_overhang_context_summary_20260701.json"
CURRENT_DAILY_SNAPSHOT = NON_OHLCV_DIR / "daily_non_ohlcv_snapshot_20260701.json"
CURRENT_FORM4_SUMMARY = NON_OHLCV_DIR / "form4_backfill_summary_20260701.json"
PRIOR_FORM4_SUMMARY = NON_OHLCV_DIR / "form4_backfill_summary_20260630.json"
PRIOR_CONTEXT_ARTIFACT = (
    REPO_ROOT
    / "data"
    / "experiments"
    / "exp-20260701-011"
    / "exp_20260701_011_form4_sale_overhang_context_20260630.json"
)

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260702_006_{SLUG}.json"
...
CAUSAL_COMPONENTS = [
    ...
    "20260701 canonical context rows",
    "baseline identity",
    "audit closeout",
...
def build_payload() -> dict[str, Any]:
    ...
    delta = {
        ...
        "raw_form4_rows_delta_vs_20260630": raw_rows_delta,
        "context_rows_delta_vs_exp20260701_011": int(context_summary.get("rows_written") or 0)
        - int(prior_context_summary.get("rows_written") or 0),
        "high_sale_overhang_rows_delta_vs_exp20260701_011": int(
    ...
    calibration = {
        ...
        "surprise_note": (
            "The current raw Form4 archive advanced and the skipped 2026-07-01 context "
    ...
    return {
        ...
        "decision": (
            "accepted_measurement_repair_form4_20260701_context_materialization"
            if accepted
            else "blocked_form4_20260701_context_materialization"
        ),
        "accepted": accepted,
        ...
        or "Materialize 2026-07-01 Form4 sale-overhang context rows.",
        ...
        "nearby_prior_experiments": ["exp-20260629-005", "exp-20260701-011"],
        ...
        "pre_run_questions": {
            ...
            "2_history_check": (
                ...
                "logger and exp-20260701-011 materialized the prior as-of rows while "
                "blocking risk-response retries until counts "
                "advance. This run materializes a new as-of row set after raw 2026-07-01 "
            ...
            "4_acceptance_standard": (
                ...
                "2026-07-01 raw Form4 rows advanced versus 2026-06-30, canonical context "
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "The raw 2026-07-01 Form4 archive existed, but the daily non-OHLCV "
        ...
        "claim_note": (
            "Claim had no locked-variable conflicts; this run touched only exp-006 files "
            "and the 20260701 Form4 context outputs."
        ),
        "related_files": [
            RUNNER,
...
def build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: Form4 20260701 context materialization",
            "",
            ...
            f"- Raw Form4 rows delta vs 20260630: `{counts['raw_rows_delta']}`",

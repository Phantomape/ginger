"""exp-20260521-005: event governance-source quality scout.

Alpha search, replay-only. Tests one production-visible event source-quality
field on top of the accepted exp-20260521-001 default-off event overlay:
whether already-selected `sec_governance_procedural` rows deserve extra paper
notional.
...
EXPERIMENT_ID = "exp-20260521-005"
EXPERIMENT_SLUG = "event_governance_source_quality"
...
VARIANTS: "OrderedDict[str, dict[str, Any]]" = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
                "description": "Accepted exp-20260521-001 event adapter with source capacity, front-rank rotation, broad-breadth, and non-generic positive event-state scalars fixed.",
                "governance_source_scalar": 1.0,
            },
        ),
        (
            "governance_source_125",
            {
                "description": "Multiply accepted event paper notional by 1.25x when source is sec_governance_procedural.",
                "governance_source_scalar": 1.25,
            },
        ),
        (
            "governance_source_150",
            {
                "description": "Multiply accepted event paper notional by 1.50x when source is sec_governance_procedural.",
                "governance_source_scalar": 1.50,
            },
        ),
        (
            "governance_source_175",
            {
                "description": "Multiply accepted event paper notional by 1.75x when source is sec_governance_procedural.",
                "governance_source_scalar": 1.75,
            },
        ),
        (
            "governance_source_200",
            {
                "description": "Multiply accepted event paper notional by 2.00x when source is sec_governance_procedural.",
                "governance_source_scalar": 2.00,
            },
        ),
    ]
)
...
def _is_target_source(trade: dict[str, Any]) -> bool:
...
def _scaled_trade(
    ...
    target = _is_target_source(trade)
    source_scalar = float(variant["governance_source_scalar"]) if target else 1.0
    ...
    return {
        **trade,
        ...
        "governance_source_target": target,
        "governance_source_scalar": round(source_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("governance_source_target")]
    for label, rows in rows_by_window.items():
        window_targets = [row for row in rows if row.get("governance_source_target")]
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -VARIANTS[name]["governance_source_scalar"],
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "promising_replay_only_event_governance_source_quality"
        if accepted
        else "rejected_event_governance_source_quality"
    )
    ...
    return {
        ...
        "trial_family": "event_source_quality_maturation",
        "trial_variant_id": "sec_governance_procedural_source_quality",
        ...
        "new_evidence_type": "new_production_visible_event_source_field",
        "hypothesis": (
            "Inside the accepted default-off event overlay, already-selected "
            "sec_governance_procedural rows may deserve extra paper notional "
            "because they are deterministic regulatory/procedural events whose "
            "selected historical outcomes are positive across all three standard "
            "windows without the single-source capacity expansion that recently "
            "failed."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event-source quality",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Uses a production-visible event-family field in the event "
                "replacement-value lane; avoids data-limited LLM soft-ranking, "
                "state-surface scalar mining, broad-market retunes, source "
                "capacity expansion, and recent dispersion/state-rank retries."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for event overlay rows whose source is "
        ...
        "parameters": {
            ...
            "target_source": TARGET_SOURCE,
            "selected_governance_source_scalar": VARIANTS[best_variant][
                "governance_source_scalar"
            ],
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Selected sec_governance_procedural event rows may carry cleaner "
                "replacement value; this is capital allocation/event-source "
                "quality and matches the event replacement-value lane."
            ),
            "2_history_check": (
                "Event rotation/front-rank/broad-breadth passed in "
                "exp042/043/044/021-001. High-dispersion, source capacity, and "
                "state_rank_pct failed in exp021-002/003/004. This tests source "
                "quality, not capacity, dispersion, state rank, or another broad "
                "breadth scalar."
            ),
            "3_single_causal_variable": (
                "Only the sec_governance_procedural source paper-notional scalar "
                "changes; core behavior, event definitions, event hold, existing "
                "state scalars, and source capacity stay fixed."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260521_005_event_governance_source_quality.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260516-030": (
                "Rejected source-specific negative-reaction tilt; this tests a "
                "different source family with selected rows positive across all "
                "three windows."
            ),
            ...
            "exp-20260521-002": "Rejected high-dispersion context because a window regressed.",
            "exp-20260521-003": "Rejected source capacity expansion because aggregate EV regressed.",
            "exp-20260521-004": "Rejected state_rank_pct quality because materiality did not clear.",
        },
        "backtest_protocol": {
            "source": (
        ...
        "production_impact": {
            ...
            "promotion_blocker_if_positive": (
                "A positive scout still requires shared adapter parity and closed "
                "forward replacement-value evidence before any live/default capital."
            ),
        },
        "llm_metrics": {
            "used_llm": False,
            ...
            "blocker_relation": (
                ...
                "deterministic PIT event source fields only."
            ),
        },
        "decision_rationale": (
            "Promising replay-only alpha scout. The best source scalar cleared "
            "the three-window Gate 4, but no shared behavior is promoted in this "
            "experiment; adapter parity and forward replacement-value evidence "
            "remain required."
            if accepted
            else "Rejected. The best variant did not satisfy the repository Gate 4 materiality/window/sample rule."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "Do not enable live/default capital. If pursued, the next experiment "
            "must promote this through the shared default-off event adapter with "
            "parity tests and continue collecting forward replacement value."
            if accepted
            else "Do not retry nearby event source scalars on the frozen sample without new forward replacement-value evidence or a distinct source-quality field."
        ),
        "why_not_other_attractive_points": (
            ...
            "retunes due strict anti-repeat rules; skipped event high-dispersion, "
            "source-capacity, and state_rank_pct because those just failed."
        ),
        "risk_of_change": (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Governance-Source Quality",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search, replay-only. Tests whether selected "
            "sec_governance_procedural event rows deserve extra paper notional "
            "on top of the accepted default-off event broad-breadth adapter."
        ),
        "",
        "## Gate 4 Result",
        "",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event governance-source quality",

"""exp-20260524-013: low theme-participation component top-up scout.

Alpha search. Tests whether the entry-day cross-sectional ranking surface's
theme_participation component identifies less-crowded already-qualified core
signals. The only causal variable is a cap-aware post-sizing multiplier for
signals with alpha_score_components.theme_participation <= 0.30.
...
EXPERIMENT_ID = "exp-20260524-013"
STEM = "core_low_theme_component_topup"
MULTIPLIER_KEY = "low_theme_participation_topup_applied"

THEME_PARTICIPATION_MAX = 0.30
...
VARIANTS = OrderedDict(
    [
        ("low_theme_component_topup_1025", {"multiplier": 1.025}),
        ("low_theme_component_topup_1050", {"multiplier": 1.05}),
        ("low_theme_component_topup_1075", {"multiplier": 1.075}),
        ("low_theme_component_topup_1100", {"multiplier": 1.10}),
    ]
)
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]):
    def wrapped(signals, features_dict, atr_target_mult=None):
        ...
        for sig in enriched:
            ...
            sig["low_theme_participation_state"] = (
...
def _apply_topup_to_sizing(
    ...
    if sig.get("low_theme_participation_state") is not True:
        return sig, None
    ...
    sizing["low_theme_participation_state"] = True
    sizing["theme_participation_component"] = sig.get("theme_participation_component")
    sizing["theme_participation_component_max"] = THEME_PARTICIPATION_MAX
    sizing["low_theme_baseline_shares"] = old_shares
    sizing["low_theme_desired_shares"] = desired_shares
    sizing["low_theme_cap_shares"] = cap_shares
    sizing["low_theme_new_shares"] = new_shares
...
def _rejection_reason(gate: dict[str, Any]) -> str:
    if gate["adjusted_signal_count"] < gate["guardrails"]["min_adjusted_signal_count"]:
        return (
            "Best variant failed adjusted-signal sample guard; low "
            "theme-participation did not touch enough sized core signals."
        )
...
def _retag_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "The new point-in-time component attribution shows low "
                "theme_participation already-filled core trades had higher "
                "average realized PnL in all three canonical windows. A low "
                "theme-participation state may identify less-crowded core "
                "entries that deserve a small cap-aware top-up without "
                "changing entries, exits, ranking, universe, news, or LLM logic."
            ),
            "change_summary": (
                "Compute the experiment-only entry-day ranking surface and "
                "sweep a cap-aware top-up for "
                "alpha_score_components.theme_participation <= 0.30."
            ),
    ...
    payload["parameters"] = {
        "theme_participation_component_max": THEME_PARTICIPATION_MAX,
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "capital allocation: low theme participation may mark less crowded "
            "already-qualified core entries with better replacement value."
        ),
        "2_history_check": (
            "exp-20260523-015 top-level alpha-score, exp-20260524-003 "
            "relative-strength, exp-20260524-007 breadth-alignment, and "
            "exp-20260524-011 trend component top-ups failed Gate 4. "
            "exp-20260524-012 newly showed low theme_participation had higher "
            "average PnL in all three canonical windows; no exact prior trial "
            "tested this component."
        ),
        "3_single_causal_variable": (
            "post-sizing multiplier for alpha_score_components.theme_participation <= 0.30"
        ),
        ...
        "5_reproducibility": (
            ...
            "exp_20260524_013_core_low_theme_component_topup.py"
        ),
    }
    ...
    payload["production_impact"][
        ...
        "Move the theme_participation component field and risk policy into "
    ...
    payload["why_not_other_changes"] = (
        "Skipped raw expectation_revision, post_earnings_drift, and trend "
        "component retries because exp-20260524-012 showed those fields are "
        "constant or saturated on the canonical windows. Skipped SEC/event/"
        "state-surface/broad-market/ETF/candidate-pool scalars due recent "
        "rejected or strict-gated lanes."
    )
    payload["known_risks"] = [
        "High multiple-testing risk because adjacent ranking-component scouts failed.",
        "The low-theme bucket has only nine filled historical trades before sizing effects.",
    ...
    if payload["status"] == "rejected":
        payload["next_retry_requires"] = [
            "Do not retry adjacent theme_participation thresholds or multipliers on the same frozen windows without new forward rows or a materially different interaction.",
            "A valid retry needs replacement-value evidence, concentration governance, or a broader PIT component interaction.",
        ]
    else:
        payload["next_retry_requires"] = [
    ...
    payload["related_files"] = [
        "quant/experiments/exp_20260524_013_core_low_theme_component_topup.py",

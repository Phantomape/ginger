"""exp-20260428-004 Technology breakout target-width screen.

Hypothesis:
    `breakout_long | Technology` may contain convex winners that the shared
    breakout target clips too early. This tests that sleeve directly instead of
    extending the accepted Technology trend result by assumption. Entries,
    sizing, ordering, add-ons, scarce-slot routing, LLM/news replay, and all
    other exits stay unchanged.
...
OUT_JSON = OUT_DIR / "exp_20260428_004_technology_breakout_target_width.json"
...
VARIANTS = OrderedDict([
    ...
    ("technology_breakout_target_5_0", {"target_mult": 5.0}),
    ("technology_breakout_target_6_0", {"target_mult": 6.0}),
    ("technology_breakout_target_7_0", {"target_mult": 7.0}),
])
...
def _eligible(sig: dict) -> bool:
    return sig.get("strategy") == "breakout_long" and sig.get("sector") == "Technology"
...
def _retarget(sig: dict, target_mult: float) -> dict:
    ...
    return {
        **sig,
        ...
        "technology_breakout_target_width_applied": target_mult,
    }
...
def _run_window(universe: list[str], label: str, cfg: dict,
    ...
    trades = [
        ...
        if t.get("strategy") == "breakout_long" and t.get("sector") == "Technology"
    ]
    return {
        "window": label,
        "start": cfg["start"],
        ...
        "technology_breakout_trade_count": len(trades),
        "technology_breakout_pnl": round(sum(float(t.get("pnl") or 0.0) for t in trades), 2),
        "technology_breakout_exits": [
            {
                "ticker": t.get("ticker"),
...
def main() -> int:
        ...
        for label, cfg in WINDOWS.items():
            for variant_name, variant_cfg in VARIANTS.items():
                ...
                print(
                    ...
                    f"tech_bo_pnl={row['technology_breakout_pnl']} "
    ...
    payload = {
        ...
        "hypothesis": (
            "`breakout_long | Technology` may contain convex winners that the "
            "shared breakout target clips too early; wider ATR targets for this "
        ...
        "parameters": {
            "single_causal_variable": "breakout_long Technology target width",

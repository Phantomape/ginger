"""exp-20260522-024: risk-on stock breakout top-up scout.

Alpha search. Tests whether already-qualified non-ETF/non-Commodity
``breakout_long`` signals in a risk-on exit regime deserve a small cap-aware
post-sizing risk top-up. The only causal variable is the top-up multiplier.

This runner is experiment-only. If a variant passes Gate 4, promotion must
move the sizing rule into shared production/backtest policy before orders
change.
"""
...
EXPERIMENT_ID = "exp-20260522-024"
STEM = "risk_on_breakout_topup"
MULTIPLIER_KEY = "risk_on_stock_breakout_risk_multiplier_applied"
...
VARIANTS = OrderedDict(
    [
        ("risk_on_breakout_topup_1025", {"multiplier": 1.025}),
        ("risk_on_breakout_topup_1050", {"multiplier": 1.05}),
        ("risk_on_breakout_topup_1075", {"multiplier": 1.075}),
        ("risk_on_breakout_topup_1100", {"multiplier": 1.10}),
    ]
)
...
def _apply_topup_to_sizing(
    ...
    sizing["risk_on_stock_breakout_state"] = True
    sizing["risk_on_stock_breakout_baseline_shares"] = old_shares
    sizing["risk_on_stock_breakout_desired_shares"] = desired_shares
    sizing["risk_on_stock_breakout_cap_shares"] = cap_shares
    sizing["risk_on_stock_breakout_new_shares"] = new_shares
...
def _adjustment_summary(adjustments: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    multiplier_sources: dict[str, int] = {}
    for rows in adjustments.values():
        for row in rows:
            ...
            for key in (row.get("existing_sizing_multipliers") or {}):
                multiplier_sources[key] = multiplier_sources.get(key, 0) + 1
    return {
        ...
        "existing_sizing_multiplier_counts": dict(sorted(multiplier_sources.items())),
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Risk-on stock breakout_long signals have a strong current-stack "
            "three-window contribution and may deserve a small cap-aware "
            "post-sizing top-up. The test should improve EV without changing "
            "entries, filters, ranking, exits, universe, LLM, or news logic."
        ),
        "change_summary": (
            "Sweep one experiment-only cap-aware post-sizing multiplier for "
            "non-ETF/non-Commodity breakout_long signals whose regime_exit_bucket "
            "is risk_on."
        ),
        "change_type": "capital_allocation",
        "mechanism_family": "core_breakout_regime_allocation",
        ...
        "component": "quant/portfolio_engine.py",
        "parameters": {
            "target_strategy": "breakout_long",
            "target_regime_exit_bucket": "risk_on",
        ...
        "gate2": {
            ...
            "rule_dependencies": [
                ...
                "sizing.max_position_pct_applied or MAX_POSITION_PCT",
            ],
        },
        "gate3": {
            "adds_filter": False,
        ...
        "production_impact": {
            ...
            "promotion_required_if_accepted": (
                "Move the risk-on stock breakout sizing rule into shared "
                "portfolio_engine sizing constants/logic, ensure run.py and "
                "backtester.py both use it through size_signals, add tests, and "
                "rerun the same three-window protocol."
            ),
        },
        "why_not_other_changes": (
            "Skipped LLM soft-ranking because attribution rows remain sparse, "
            "SEC semantics because phrase provenance/fact-tone buckets remain "
            "insufficient, broad-market because recent forward evidence shows "
            "identity drift, event/source scalars because near-neighbor variants "
            "recently failed drawdown/sample guards, and state-surface scalar/"
            "profile retunes because the same-family hard materiality gate is high."
        ),
        "known_risks": [
            "Breakout allocation has many nearby prior tests, so moderate multiple-testing risk remains.",
            "Some target signals already have accepted sector or state sizing rules; this tests an overlay, not a new entry edge.",
            "Cap-aware top-ups can be immaterial where existing sizing is already position-cap constrained.",
        ],
        "rejection_reason": rejection_reason,
        "next_retry_requires": (
            [
                "Do not retry adjacent risk-on breakout multipliers on the same frozen windows without new forward rows or a materially different explanatory field.",
                "If breakout allocation remains interesting, prefer a source of new evidence such as fresh closed trades, candidate replacement value, or a broader production-visible regime context.",
            ]
            if not passed
            else [
                "Promote only via shared portfolio_engine policy plus parity tests; no replay-only production behavior is allowed.",
                "After promotion, rerun the exact docs/backtesting.md three-window protocol.",
            ]
        ),
    }
    ...
    ticket = {
        "experiment_id": EXPERIMENT_ID,
        "title": "Risk-on stock breakout top-up scout",
        "status": status,
        "decision": decision,
        "changed_variable": payload["changed_variable"],
        "backtest_protocol": payload["date_range"],
    ...
    _append_jsonl(EXPERIMENT_LOG_JSONL, log_record)
    return payload
    ...
    print(json.dumps(
        {
            ...
            "expected_value_score_delta": aggregate["expected_value_score_sum"],
            "total_pnl_delta": aggregate["total_pnl_sum"],
            "artifact": str(OUT_JSON),
        },
        indent=2,
        sort_keys=True,

"""exp-20260429-015 trend Financials risk boost.

Alpha search. Test one capital-allocation variable: whether existing
trend_long Financials candidates deserve 1.5x risk budget. Entry generation,
filters, ranking, exits, slot caps, add-ons, and LLM/news replay remain locked.
"""
...
EXPERIMENT_ID = "exp-20260429-015"
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / "exp_20260429_015_trend_financials_risk_boost.json"
...
BASELINE_METRICS = {
    "late_strong": {
        "expected_value_score": 2.0154,
        "sharpe_daily": 4.27,
        "total_pnl": 47200.59,
        "total_return_pct": 0.472,
    ...
    "mid_weak": {
        "expected_value_score": 0.8004,
        "sharpe_daily": 2.51,
        "total_pnl": 31892.95,
        "total_return_pct": 0.3189,
...
def run_experiment() -> dict:
    ...
    payload = {
        ...
        "hypothesis": (
            "Existing trend_long Financials candidates are a repeat positive "
            "sleeve and deserve 1.5x risk budget."
        ),
        "parameters": {
            "single_causal_variable": "trend_long + Financials risk multiplier",
            "old_value": 1.0,
            "new_value": 1.5,
            "locked_variables": [
                "signal generation",
        ...
        "delta_metrics": {
            **{label: values["delta"] for label, values in by_window.items()},
            "aggregate": aggregate,
            "gate4_basis": (
                "Accepted: improves mid_weak and old_thin, leaves late_strong "
        ...
        "history_guardrails": {
            "not_repeating_sector_priority_ordering": (
                "This sizes already-selected trend Financials; it does not reorder "
                "same-day candidates or add sector priority."
            ),
            "not_repeating_global_slot_count": True,
            "not_repeating_broad_financials_entry": (
                "No new Financials entries, filters, or universe candidates were added."
            ),
        },
        "rows": rows,
        "rejection_reason": None if accepted else "Failed Gate 4.",
        "next_retry_requires": [
            "Do not turn this into Financials sector priority without new evidence.",

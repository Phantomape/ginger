"""exp-20260617-007: CapEx / D&A reinvestment-cycle candidate scout.
...
annual capital expenditures are rising versus depreciation and amortization,
with revenue not contracting and liquid SPY-relative leadership, may identify
productive reinvestment cycles before a 10-day continuation leg.

This is not a low-capex threshold retry, D&A burden retry, or fixed-asset
turnover retry. It tests the capex-to-D&A replacement-cycle context explicitly
named as a valid next evidence surface after the D&A and fixed-asset turnover
scouts produced positive aggregate evidence but failed old_thin/window gates.
...
EXPERIMENT_ID = "exp-20260617-007"
STEM = "capex_depreciation_reinvestment_cycle"
TRIAL_FAMILY = "capex_depreciation_reinvestment_cycle_candidate_pool"
TRIAL_VARIANT_ID = "capex_depreciation_reinvestment_cycle_top1_next_open_10d_v1"
CHANGED_VARIABLE = "raw_sec_companyfacts_capex_depreciation_reinvestment_cycle_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_007_{STEM}.json"
...
MIN_CURRENT_CAPEX = 50_000_000.0
MIN_CURRENT_DA = 25_000_000.0
MIN_CURRENT_CAPEX_TO_REVENUE = 0.015
MAX_CURRENT_CAPEX_TO_REVENUE = 0.45
MIN_CURRENT_CAPEX_TO_DA = 1.05
MAX_CURRENT_CAPEX_TO_DA = 3.50
MIN_PRIOR_CAPEX_TO_DA = 0.35
MIN_CAPEX_TO_DA_IMPROVEMENT = 0.12
MIN_REVENUE_GROWTH = 0.0
MIN_CAPEX_GROWTH_MINUS_REVENUE_GROWTH = -0.10
MAX_CAPEX_GROWTH_MINUS_REVENUE_GROWTH = 0.80
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "window_regression",
        "drawdown_drift",
        "capex_field_semantics_noisy",
        "accepted_distribution_comparator_not_beaten",
    ],
    "confidence_reason": (
        "The D&A and fixed-asset turnover scouts were positive in aggregate but "
        "failed old_thin/drawdown; both explicitly named capex-to-D&A "
        "replacement-cycle context as materially different PIT evidence. The "
        "risk is high because recent raw Companyfacts quality sources often "
        "relabel liquid momentum and fail at least one canonical window."
    ),
    "recorded_at": "2026-06-17T05:04:32+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC annual CapEx/D&A/revenue pair, missing prior annual "
    ...
    "parity_note": (
        ...
        "SEC tag mapping, filed-date PIT annual CapEx/D&A reinvestment-cycle "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts annual CapEx-to-D&A "
        "replacement-cycle acceleration (CapEx/D&A rising into a productive "
        "range, revenue not contracting, and CapEx growth not wildly outpacing "
        "revenue) paired with liquid SPY-relative leadership may identify "
        "reinvestment-cycle winners with 10-day continuation beyond generic "
        "momentum."
    ),
    "2_history_check": {
        "exp-20260529-003": (
            "Rejected low-capex-intensity support. This run is not low-capex; "
            "it tests productive replacement-cycle acceleration versus D&A."
        ),
        "exp-20260616-003": (
            "Rejected R&D intensity acceleration. This run tests tangible "
            "reinvestment relative to depreciation, not innovation expense."
        ),
        "exp-20260617-005": (
            "Rejected D&A burden relief despite positive aggregate EV/PnL; its "
            "reflection named capex-to-D&A replacement-cycle context as new "
            "evidence required for a valid retry."
        ),
        "exp-20260617-006": (
            "Rejected fixed-asset turnover improvement with old_thin regression; "
            "its reflection again named capex-to-D&A replacement-cycle context "
            "as a materially different PIT field."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_007_capex_depreciation_reinvestment_cycle.py"
    ),
}
...
def _annual_cashflow_facts(usgaap: dict[str, Any], tags: tuple[str, ...]) -> list[dict[str, Any]]:
...
def _latest_capex_revenue_da_pair(
    ...
    *,
    ...
    for capex in capex_candidates:
        revenue = _latest_period_fact(facts["revenue"], asof=asof, end=capex["end"])
        da = _latest_period_fact(facts["depreciation"], asof=asof, end=capex["end"])
        if revenue is not None and da is not None:
            return capex, revenue, da
    return None
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        capex_facts = _annual_cashflow_facts(usgaap, CAPEX_TAGS)
        revenue_facts = _annual_cashflow_facts(usgaap, REVENUE_TAGS)
        depreciation_facts = _annual_cashflow_facts(usgaap, DA_TAGS)
        if not capex_facts:
            stats["tickers_missing_raw_annual_capex"] += 1
            continue
        if not revenue_facts:
            stats["tickers_missing_raw_annual_revenue"] += 1
            continue
        if not depreciation_facts:
            stats["tickers_missing_raw_annual_depreciation"] += 1
            continue
        index[ticker] = {
            "capex": capex_facts,
            "revenue": revenue_facts,
            "depreciation": depreciation_facts,
        }
        stats["tickers_with_raw_annual_capex_revenue_depreciation"] += 1
        stats["raw_annual_capex_fact_count"] += len(capex_facts)
        stats["raw_annual_revenue_fact_count"] += len(revenue_facts)
        stats["raw_annual_depreciation_fact_count"] += len(depreciation_facts)
...
def _capex_reinvestment_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    current_pair = _latest_capex_revenue_da_pair(facts, asof=asof)
    if current_pair is None:
        return None
    current_capex, current_revenue, current_da = current_pair
    current_filed = max(current_capex["filed"], current_revenue["filed"], current_da["filed"])
    if base._days_between(asof, current_filed) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None

    prior_pair = _latest_capex_revenue_da_pair(
        facts,
        asof=asof,
        before_end=current_capex["end"],
    )
    if prior_pair is None:
        return None
    prior_capex, prior_revenue, prior_da = prior_pair

    current_capex_value = abs(float(current_capex["value"]))
    prior_capex_value = abs(float(prior_capex["value"]))
    current_revenue_value = float(current_revenue["value"])
    prior_revenue_value = float(prior_revenue["value"])
    current_da_value = abs(float(current_da["value"]))
    prior_da_value = abs(float(prior_da["value"]))
    if min(current_capex_value, prior_capex_value, current_revenue_value, prior_revenue_value, current_da_value, prior_da_value) <= 0.0:
        return None
    if current_revenue_value < MIN_CURRENT_REVENUE:
        return None
    if current_capex_value < MIN_CURRENT_CAPEX or current_da_value < MIN_CURRENT_DA:
        return None

    current_capex_to_da = current_capex_value / current_da_value
    prior_capex_to_da = prior_capex_value / prior_da_value
    capex_to_da_improvement = current_capex_to_da - prior_capex_to_da
    current_capex_to_revenue = current_capex_value / current_revenue_value
    prior_capex_to_revenue = prior_capex_value / prior_revenue_value
    revenue_growth = current_revenue_value / prior_revenue_value - 1.0
    capex_growth = current_capex_value / prior_capex_value - 1.0
    da_growth = current_da_value / prior_da_value - 1.0
    capex_growth_spread = capex_growth - revenue_growth

    if current_capex_to_revenue < MIN_CURRENT_CAPEX_TO_REVENUE:
        return None
    if current_capex_to_revenue > MAX_CURRENT_CAPEX_TO_REVENUE:
        return None
    if current_capex_to_da < MIN_CURRENT_CAPEX_TO_DA:
        return None
    if current_capex_to_da > MAX_CURRENT_CAPEX_TO_DA:
        return None
    if prior_capex_to_da < MIN_PRIOR_CAPEX_TO_DA:
        return None
    if capex_to_da_improvement < MIN_CAPEX_TO_DA_IMPROVEMENT:
        return None
    if revenue_growth < MIN_REVENUE_GROWTH:
        return None
    if capex_growth_spread < MIN_CAPEX_GROWTH_MINUS_REVENUE_GROWTH:
        return None
    if capex_growth_spread > MAX_CAPEX_GROWTH_MINUS_REVENUE_GROWTH:
        return None

    return {
        "current_period_end": current_capex["end"],
        "prior_period_end": prior_capex["end"],
        "current_capex_filed": current_capex["filed"],
        "prior_capex_filed": prior_capex["filed"],
        "current_revenue_filed": current_revenue["filed"],
        "current_depreciation_filed": current_da["filed"],
        "current_capex_tag": current_capex["tag"],
        "current_depreciation_tag": current_da["tag"],
        "current_revenue_tag": current_revenue["tag"],
        "current_capex": _round(current_capex_value, 2),
        "prior_capex": _round(prior_capex_value, 2),
        "current_depreciation": _round(current_da_value, 2),
        "prior_depreciation": _round(prior_da_value, 2),
        "current_revenue": _round(current_revenue_value, 2),
        "prior_revenue": _round(prior_revenue_value, 2),
        "current_capex_to_depreciation": _round(current_capex_to_da, 6),
        "prior_capex_to_depreciation": _round(prior_capex_to_da, 6),
        "capex_to_depreciation_improvement": _round(capex_to_da_improvement, 6),
        "current_capex_to_revenue": _round(current_capex_to_revenue, 6),
        "prior_capex_to_revenue": _round(prior_capex_to_revenue, 6),
        "revenue_growth": _round(revenue_growth, 6),
        "capex_growth": _round(capex_growth, 6),
        "depreciation_growth": _round(da_growth, 6),
        "capex_growth_minus_revenue_growth": _round(capex_growth_spread, 6),
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            if meta.get("sector") in EXCLUDED_SECTORS:
                ...
                continue
            quality = _capex_reinvestment_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_capex_reinvestment_gate"] += 1
                continue
            ...
            score = (
                0.80 * min(float(quality["capex_to_depreciation_improvement"] or 0.0), 1.25)
                + 0.35 * min(float(quality["current_capex_to_depreciation"] or 0.0), 3.5)
                + 0.25 * min(float(quality["revenue_growth"] or 0.0), 1.0)
                + 0.25 * max(min(float(quality["capex_growth_minus_revenue_growth"] or 0.0), 0.60), -0.10)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_CAPEX_DEPRECIATION_REINVESTMENT_CYCLE_PAPER",
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["reinvestment_capex_to_depreciation_improvement"] or 0.0),
            -float(row["reinvestment_current_capex_to_depreciation"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        ...
        "min_current_capex": MIN_CURRENT_CAPEX,
        "min_current_depreciation": MIN_CURRENT_DA,
        "min_current_capex_to_revenue": MIN_CURRENT_CAPEX_TO_REVENUE,
        "max_current_capex_to_revenue": MAX_CURRENT_CAPEX_TO_REVENUE,
        "min_current_capex_to_depreciation": MIN_CURRENT_CAPEX_TO_DA,
        "max_current_capex_to_depreciation": MAX_CURRENT_CAPEX_TO_DA,
        "min_prior_capex_to_depreciation": MIN_PRIOR_CAPEX_TO_DA,
        "min_capex_to_depreciation_improvement": MIN_CAPEX_TO_DA_IMPROVEMENT,
        "min_revenue_growth": MIN_REVENUE_GROWTH,
        "min_capex_growth_minus_revenue_growth": MIN_CAPEX_GROWTH_MINUS_REVENUE_GROWTH,
        "max_capex_growth_minus_revenue_growth": MAX_CAPEX_GROWTH_MINUS_REVENUE_GROWTH,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_capex_depreciation_reinvestment_cycle"
        if gate["passed"]
        else "rejected_capex_depreciation_reinvestment_cycle_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The raw SEC CapEx/D&A reinvestment-cycle source cleared the "
            "numeric three-window replay screen, but remains only a replay lead "
            "because no shared daily/backtest helper was promoted."
        )
    else:
        interpretation = (
            "The raw SEC CapEx/D&A reinvestment-cycle source did not clear "
    ...
    payload.update(
        {
            ...
            "new_evidence_type": "raw_sec_companyfacts_capex_to_depreciation_reinvestment_cycle_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual CapEx, depreciation/amortization, and revenue are read from raw "
        ...
        "and revenue by fiscal-year end. The rule requires CapEx/D&A to rise "
        "into a productive replacement-cycle range while revenue is not "
        "contracting and CapEx growth is bounded relative to revenue growth. "
    ...
    payload["next_evidence_needed"] = (
        ...
        "industry-normalized replacement-cycle productivity, segment/customer "
        "capacity disclosures, or closed forward replacement-value rows. Do not "
        "sweep CapEx/D&A tags, CapEx/D&A thresholds, revenue floor, sector "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping CapEx/D&A tag lists, replacement-cycle "
            "ratio thresholds, revenue floor, sector exclusions, annual fact "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} CapEx / D&A Reinvestment Cycle",
            "",

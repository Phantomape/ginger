"""exp-20260506-002: industrial infrastructure/defense basket replay.

Alpha search. Tests a narrow candidate-pool extension after broad universe
growth failed. The replay adds only full-coverage industrial infrastructure
and defense-adjacent names from the fresh OHLCV archive. It patches sector
metadata inside the experiment so existing shared policy still applies the
accepted 0x trend Industrials sleeve; no production universe, order path,
...
SUB_BASKET = ["AGX", "AIT", "AVAV", "LHX", "MTZ", "WAB"]
SECTOR_OVERRIDES = {ticker: "Industrials" for ticker in SUB_BASKET}
...
OUT_JSON = OUT_DIR / "industrial_infra_defense_basket.json"
...
ARTIFACT_MD = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_industrial_infra_defense_basket.md"
)
...
def _write_artifact(payload: dict[str, Any]) -> None:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Industrial Infrastructure/Defense Basket",
        "",
    ...
    lines.extend(
        [
            ...
            "## Parity",
            "",
            "No production universe or order path changed. The replay uses Industrials sector metadata so existing shared trend-Industrials sizing remains active. Any promotion requires universe governance or a default-off pilot adapter plus run/backtester parity.",
            "",
        ]
    )
...
def build_payload() -> dict[str, Any]:
    ...
    payload = {
        ...
        "mechanism_family": "industrial_infrastructure_defense_universe_subbasket",
        "hypothesis": (
            "A narrow industrial infrastructure and defense-adjacent basket may "
            "add production-compatible breakout opportunities without repeating "
            "the rejected trend Industrials reactivation. AGX, AIT, AVAV, LHX, "
            "MTZ, and WAB have full fresh-snapshot OHLCV coverage and map to "
            "Industrials, so the existing 0x trend Industrials sleeve remains "
            "in force while breakout candidates can still compete for scarce slots."
        ),
        "alpha_hypothesis": {
            "category": "entry / universe governance",
            "why_this_now": (
                "LLM soft-ranking is sample-limited and event-bundle promotion "
                "needs forward paper outcomes. Broad watchlist growth, consumer "
                "platforms, enterprise infra incumbents, and cybersecurity "
                "sub-baskets were already rejected, while the industrial capex "
                "basket tests a distinct non-LLM entry surface with sector "
                "metadata and no strategy-rule change."
            ),
        },
        "historical_experiment_check": {
            "blocked_repeats": {
                "exp-20260505-009": "Rejected broad historical attention-list expansion.",
                "exp-20260505-011": "Rejected HOOD/RBLX/SOFI consumer platform basket.",
                "exp-20260505-032": "Rejected AKAM/ORCL enterprise infrastructure basket.",
                "exp-20260505-033": "Rejected CHKP/CRWD cybersecurity basket.",
                "exp-20260503-001": (
                    "Rejected trend Industrials reactivation. This replay keeps "
                    "the accepted 0x trend Industrials sleeve active through "
                    "sector metadata and only tests new candidate availability."
                ),
            },
            "mechanism_insight_check": (
                "This avoids recent no-go zones: no LLM ranking without samples, "
                "no event retune, no broad ticker growth, no sector cap, no "
                "ranking threshold, and no trend Industrials multiplier change."
            ),
            "why_not_simple_repeat": (
                "The single causal variable is a full-coverage six-name "
                "industrial infrastructure/defense candidate basket; all "
                "strategy thresholds and allocation policy remain locked."
            ),
        },
        "parameters": {
            "single_causal_variable": (
                "candidate universe includes AGX/AIT/AVAV/LHX/MTZ/WAB sub-basket"
            ),
        ...
        "risk_of_change": (
            "Industrial candidates may consume scarce slots while the accepted "
            "0x trend sleeve suppresses much of the theme; any production "
            "promotion could still crowd existing CAT/DE/GE/RTX exposure."
        ),
        "why_not_other_attractive_points": {
            ...
            "sector_sizing_retune": "Nearby sector and Industrials reactivation variants were rejected.",
            "same_sample_event_retuning": "Blocked by recent mechanism insights.",
        },
        "related_files": [
            "quant/experiments/exp_20260506_002_industrial_infra_defense_basket.py",
...
def main() -> int:
    ...
    _write_json(
        TICKET_JSON,
        {
            ...
            "title": "Industrial infrastructure defense basket",

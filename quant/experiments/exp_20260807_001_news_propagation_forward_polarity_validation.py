"""exp-20260807-001: out-of-replay forward validation of the inverted-polarity
news-propagation lead (exp-20260702-021 / exp-20260703-005).

Frozen contract (predeclared outcome-blind in the ticket and in
data/alpha_search/news_propagation_forward_readiness_20260807.json):

- Cohort: closed rows of data/non_ohlcv/news_event_exposure_observations/rows.jsonl
  with event_date >= 2026-07-01 (strictly out-of-replay; replay rows cover
  2026-01..06) and event_polarity in {negative, positive}.
- Settlement: the observer-materialized excess_10d (exposure-ticker
  next-open-to-10-session-close return minus SPY same-window return). No
  settlement, polarity, taxonomy, or matching semantics are recomputed here.
- Falsifier bars (ALL must pass to confirm the forward lead):
  F1 floors: negative closed rows >= 200, negative events >= 20, negative
     event dates >= 10, positive control rows >= 200.
  F2 event-level primary: mean of per-event mean excess_10d, negative > positive.
  F3 row-level: pooled mean AND pooled median excess_10d, negative > positive.
  F4 halves: splitting each side's events at its median unique event_date
     (lower-middle element; first half = events dated <= median), the
     event-level negative-minus-positive separation is > 0 in both halves.
  F5 concentration: no single negative event > 40% of the negative side's
     summed positive per-event mean excess; no single exposure ticker > 40%
     of the negative side's row-level positive excess mass.
- Result ceiling observed_only either way; a full pass promotes nothing.

Repro:
    .\\.venv\\Scripts\\python.exe -B -m quant.experiments.exp_20260807_001_news_propagation_forward_polarity_validation
"""
...
READINESS = Path("data/alpha_search/news_propagation_forward_readiness_20260807.json")
OUT = Path(
    "data/experiments/exp-20260807-001/"
    "exp_20260807_001_news_propagation_forward_polarity_validation.json"
)
...
def main() -> None:
    ...
    artifact = {
        ...
        "separation": {
            ...
            "row_level_mean_bp": (neg["row_mean"] - pos["row_mean"]) * 1e4,
        ...
        "decision": (
            "observed_only_forward_polarity_separation_confirmed"
            if all_pass
            else "rejected_forward_polarity_separation_not_confirmed"
        ),
        "result_ceiling": "observed_only",
        "trade_enabled": False,
        "notes": (
            "Descriptive only; excess_10d is the observer-materialized SPY-excess. "
            "Negative-versus-zero and per-relation-type numbers are recorded "
            "descriptively without verdicts."
        ),
        "descriptive": {

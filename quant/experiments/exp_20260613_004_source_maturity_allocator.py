"""exp-20260613-004: source-maturity allocator arbitration scout.

Alpha search, replay-only. The policy under test is an ex-ante source
maturity score for same-day accepted allocator source conflicts. The score only
uses source-family candidate trades whose exit_date is strictly before the
current signal_date. No production/shared helper is changed in this runner.
...
EXPERIMENT_ID = "exp-20260613-004"
STEM = "source_maturity_allocator"
OWNER = "alpha-search-automation"
TRIAL_FAMILY = "accepted_allocator_source_arbitration"
TRIAL_VARIANT_ID = "source_maturity_trailing_closed_pnl_arbitration_v1"
CHANGED_VARIABLE = TRIAL_VARIANT_ID
SOURCE_MATURITY_RULE_VERSION = "source_maturity_trailing_closed_pnl_arbitration_v1"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260613_004_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "exp-20260613-003 showed a large same-day source-choice oracle gap, "
        "but prior direct rank/state router retunes failed; a PIT trailing "
        "closed-outcome score is a materially different source-maturity field, "
        "with high risk of noisy performance chasing."
    ),
    "recorded_at": "2026-06-13T02:03:39+00:00",
}

PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool/allocation: accepted allocator same-day source "
        "conflicts may be improved by an ex-ante source-maturity score "
        "computed only from source-family trades whose exit_date is before the "
        "signal date, allowing the allocator to prefer recently proven sources "
        "while falling back to fixed priority when history is too thin."
    ),
    "2_history_check": {
        "exp-20260611-005": (
        ...
        "exp-20260611-010": (
            "Industry-laggard source pruning was replay-positive versus core "
            "but not promoted; do not simply delete or reprioritize sources."
        ),
        "exp-20260613-002": (
            "Industry-stable state router failed direct comparison versus the "
            "accepted allocator."
        ),
        "exp-20260613-003": (
            "Observed-only source-choice oracle found a material gap, but used "
            "future PnL and required a PIT source-arbitration field."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260613_004_source_maturity_allocator.py"
    ),
}
...
PRODUCTION_IMPACT = {
    "trade_enabled": False,
    "alters_orders": False,
    "adapter_status": "replay_only_source_maturity_scout",
    ...
    "execution_envelope": {
        ...
        "notes": (
            "Same accepted-helper allocator bucket as exp-20260612-024. This "
    ...
    "parity_note": (
        "Replay-only source-maturity scout. It reuses accepted allocator source "
        "rows and only uses prior closed source-family outcomes at each signal "
...
def _history_stats(
    *,
    source_family: str,
    signal_date: str,
    source_history: dict[str, list[dict[str, Any]]],
) -> dict[str, Any]:
    closed = [
        row
        for row in source_history.get(source_family, [])
        if _date10(row.get("exit_date")) < signal_date
    ]
    closed.sort(key=lambda row: (_date10(row.get("exit_date")), _date10(row.get("signal_date"))))
    sample = closed[-LOOKBACK_CLOSED_TRADES:]
    count = len(sample)
    if count <= 0:
        return {
            "history_count": 0,
    ...
    pnls = [_float(row.get("pnl")) for row in sample]
    avg_pnl = sum(pnls) / count
    win_rate = sum(1 for value in pnls if value > 0.0) / count
    latest_exit = max(_date10(row.get("exit_date")) for row in sample)
    return {
        "history_count": count,
        "history_ready": count >= MIN_HISTORY_TRADES,
        "avg_pnl": round(avg_pnl, 6),
        "win_rate": round(win_rate, 6),
        "maturity_score": round(avg_pnl, 6),
...
def _prepared_candidate(
    ...
    *,
    ...
    out = {
        **deepcopy(row),
        **normalised,
        "source": "SOURCE_MATURITY_ALLOCATOR_REPLAY_ONLY",
        "sleeve": "SOURCE_MATURITY_ALLOCATOR_REPLAY_ONLY",
        "rule_version": SOURCE_MATURITY_RULE_VERSION,
        "source_rule_version": ACCEPTED_ALLOCATOR_SOURCE_RULE_VERSION,
        "decision_id": (
            "SOURCE_MATURITY_ALLOCATOR_REPLAY_ONLY:"
            f"{SOURCE_MATURITY_RULE_VERSION}:"
        ...
        "trade_enabled": False,
        "alters_orders": False,
        "source_maturity": history_stats,
        "source_maturity_selected_by": selected_by,
    }
    return out


def _select_source_maturity_rows(
    *,
    ...
    source_history: dict[str, list[dict[str, Any]]] = {}
    for row in candidates:
        source_history.setdefault(str(row.get("source_family") or "unknown"), []).append(row)
    ...
    maturity_score_samples: list[dict[str, Any]] = []
    ...
    for signal_date in sorted(by_date):
        ...
        for row in by_date[signal_date]:
            ...
            annotated = {**row, "source_maturity": stats}
        ...
        mature_rows = [
            row for row in day_rows if (row.get("source_maturity") or {}).get("history_ready")
        ]
        if mature_rows:
            winner = max(
                mature_rows,
                key=lambda row: (
                    _float((row.get("source_maturity") or {}).get("maturity_score")),
                    _float((row.get("source_maturity") or {}).get("win_rate")),
                    -int(row.get("source_priority_rank") or 999),
                    _float(row.get("source_priority_score")),
                    str(row.get("ticker") or ""),
                ),
            )
            selected_by = "source_maturity"
            maturity_selected += 1
        else:
        ...
        winner_out = _prepared_candidate(
            winner,
            history_stats=winner.get("source_maturity") or {},
        ...
        for row in day_rows:
            ...
            rejected_row = {
                **row,
                "filter_reason": "daily_top1_source_maturity_limit",
                "source_maturity_selected_winner": _row_key(winner),
            }
            rejected.append(rejected_row)
            rejected_reasons["daily_top1_source_maturity_limit"] += 1

        if len(maturity_score_samples) < 30:
            for row in sorted(
                day_rows,
                key=lambda item: (
                    -_float((item.get("source_maturity") or {}).get("maturity_score")),
                    int(item.get("source_priority_rank") or 999),
                    str(item.get("ticker") or ""),
                ),
            )[:3]:
                maturity_score_samples.append(
                    {
                        "signal_date": signal_date,
    ...
    audit = {
        ...
        "maturity_selected_count": maturity_selected,
        ...
        "maturity_score_samples": maturity_score_samples,
        "known_at": (
            "after prior selected/source paper trades have closed; rows with "
            "exit_date >= signal_date are excluded from the score"
        ),
    }
...
def _gate4(
    *,
    ...
    accepted_to_maturity_rows: OrderedDict[str, dict[str, Any]],
    ...
    for label, row in accepted_to_maturity_rows.items():
        delta = row["delta"]
    ...
    if numeric_passed:
        decision = "positive_replay_lead_not_promoted_source_maturity_allocator"
...
def build_payload() -> dict[str, Any]:
    ...
    maturity_metrics: OrderedDict[str, dict[str, Any]] = OrderedDict()
    core_to_accepted_rows: OrderedDict[str, dict[str, Any]] = OrderedDict()
    core_to_maturity_rows: OrderedDict[str, dict[str, Any]] = OrderedDict()
    accepted_to_maturity_rows: OrderedDict[str, dict[str, Any]] = OrderedDict()
    accepted_trades_by_window: OrderedDict[str, list[dict[str, Any]]] = OrderedDict()
    maturity_trades_by_window: OrderedDict[str, list[dict[str, Any]]] = OrderedDict()
    ...
    maturity_audit_by_window: OrderedDict[str, dict[str, Any]] = OrderedDict()
    ...
    for label, cfg in framework.WINDOWS.items():
        print(f"[{label}] source-maturity allocator")
        ...
        maturity_selected, maturity_filtered, maturity_audit = _select_source_maturity_rows(
            source_rows=source_trades,
            trading_dates=dates,
        )
        ...
        maturity_overlay = framework.sleeve._overlay_from_paper_trades(
            before_result,
            maturity_selected,
        )
        ...
        maturity_after = framework.overlay_helper._metrics_with_overlay(
            before_result,
            maturity_overlay,
        )
        accepted_delta = framework.overlay_helper._delta(accepted_after, before)
        maturity_delta = framework.overlay_helper._delta(maturity_after, before)
        direct_delta = framework.overlay_helper._delta(maturity_after, accepted_after)

        accepted_keys = {_row_key(row) for row in accepted_selected}
        maturity_keys = {_row_key(row) for row in maturity_selected}
        changed_keys = sorted(accepted_keys.symmetric_difference(maturity_keys))

        accepted_metrics[label] = accepted_after
        maturity_metrics[label] = maturity_after
        accepted_trades_by_window[label] = accepted_selected
        maturity_trades_by_window[label] = maturity_selected
        source_audit_by_window[label] = source_audit
        accepted_priority_audit_by_window[label] = accepted_priority_audit
        maturity_audit_by_window[label] = maturity_audit
        ...
        core_to_maturity_rows[label] = {
            "before": before,
            "after": maturity_after,
            "delta": maturity_delta,
            "target_trade_count": len(maturity_selected),
            "selected_source_counts": _source_counts(maturity_selected),
            "maturity_selected_count": maturity_audit["maturity_selected_count"],
            "fallback_selected_count": maturity_audit["fallback_selected_count"],
            "changed_selection_count": len(changed_keys) // 2,
            "source_trade_counts": source_audit["source_trade_counts"],
        }
        accepted_to_maturity_rows[label] = {
            ...
            "maturity_selected_source_counts": _source_counts(maturity_selected),
            "maturity_rejected_count": len(maturity_filtered),
        }

    aggregate_core_to_accepted = framework._aggregate_window_rows(core_to_accepted_rows)
    aggregate_core_to_maturity = framework._aggregate_window_rows(core_to_maturity_rows)
    aggregate_accepted_to_maturity = framework._aggregate_window_rows(
        accepted_to_maturity_rows
    )
    maturity_summary = _target_summary(maturity_trades_by_window)
    accepted_summary = _target_summary(accepted_trades_by_window)
    changed_selection_count = sum(
        int(row["changed_selection_count"]) for row in accepted_to_maturity_rows.values()
    )
    gate4 = _gate4(
        aggregate_vs_core=aggregate_core_to_maturity,
        aggregate_vs_accepted=aggregate_accepted_to_maturity,
        target_summary=maturity_summary,
        before_metrics=before_metrics,
        accepted_to_maturity_rows=accepted_to_maturity_rows,
        changed_selection_count=changed_selection_count,
    )
    ...
    if gate4["numeric_gate4_passed"]:
        ...
        interpretation = (
            "The source-maturity allocator numerically beat the accepted "
            "allocator, but it is not retained because shared helper and daily "
            "snapshot parity were not implemented."
        )
        reflection = (
            "Trailing source-family closed outcomes appear to explain part of "
            "the same-day source-choice gap without future PnL. The result is "
            "only a lead until the same field is implemented in the shared "
            "default-off allocator helper and production snapshot."
        )
    else:
        status = "rejected"
        interpretation = (
            "The source-maturity allocator failed the accepted allocator "
            "comparison; no strategy or production behavior is retained."
        )
        reflection = (
            "Recent source-family performance was either too thin or too noisy "
            "to arbitrate same-day source conflicts better than the accepted "
            "fixed priority. The oracle gap from exp-20260613-003 is not "
            "explained by this simple trailing closed-PnL maturity field."
        )
    ...
    return {
        ...
        "new_evidence_type": "new_production_visible_source_maturity_field",
        ...
        "backtest_protocol": {
            ...
            "source_maturity_rule_version": SOURCE_MATURITY_RULE_VERSION,
            "replay_llm": False,
            "replay_news": False,
        ...
        "parameters": {
            "min_history_trades": MIN_HISTORY_TRADES,
            "lookback_closed_trades": LOOKBACK_CLOSED_TRADES,
            "source_maturity_score": "avg_pnl_usd_of_prior_closed_source_family_trades",
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "source rows entry_date/exit_date/pnl for prior closed maturity score",
            ],
        },
        "gate3": {
            "passed": min(
        ...
        "source_maturity_metrics": maturity_metrics,
        "delta_metrics": {
            ...
            "core_to_source_maturity": {
                "by_window": OrderedDict(
                    (label, row["delta"]) for label, row in core_to_maturity_rows.items()
                ),
                "aggregate": aggregate_core_to_maturity,
            },
            "accepted_allocator_to_source_maturity": {
                "by_window": OrderedDict(
                    (label, row["delta"])
                    for label, row in accepted_to_maturity_rows.items()
                ),
                "aggregate": aggregate_accepted_to_maturity,
            },
        },
        "window_rows": {
            "core_to_accepted_allocator": core_to_accepted_rows,
            "core_to_source_maturity": core_to_maturity_rows,
            "accepted_allocator_to_source_maturity": accepted_to_maturity_rows,
        },
        "accepted_trade_summary": accepted_summary,
        "source_maturity_trade_summary": maturity_summary,
        "source_audit_by_window": source_audit_by_window,
        "accepted_priority_audit_by_window": accepted_priority_audit_by_window,
        "source_maturity_audit_by_window": maturity_audit_by_window,
        ...
        "post_run_reflection": {
            ...
            "negative_reflection": (
                "If rejected, the source-maturity field chased noisy source "
                "family history and did not explain the oracle gap. If positive, "
                "the result remains non-accepted because shared helper parity is missing."
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retry by sweeping source-maturity lookback, min history, "
                "win-rate weight, source rank, top-N, notional, hold days, or "
                "cooldown on the same frozen windows."
            ),
            "new_evidence_required": (
                "A valid retry needs closed forward source-competition rows or "
                "a materially richer PIT source-quality field such as relation "
                "age, independent confirmation freshness, or source-specific "
                "replacement value recorded in daily snapshots."
            ),
        },
        "next_retry_requires": [
            "closed forward source-competition replacement rows",
            "shared helper plus daily parity if any source-maturity field is promoted",
            "no frozen-window lookback/min-history sweep",
        ],
        "related_files": [
...
def _window_table(payload: dict[str, Any]) -> list[str]:
    ...
    for label in framework.WINDOWS:
        ...
        maturity_row = payload["window_rows"]["core_to_source_maturity"][label]
        direct_row = payload["window_rows"]["accepted_allocator_to_source_maturity"][label]
        rows.append(
            "| {label} | {core_ev:.4f} | {accepted_ev:.4f} | {maturity_ev:.4f} | {direct_ev:+.4f} | ${core_pnl:,.2f} | ${accepted_dpnl:+,.2f} | ${maturity_dpnl:+,.2f} | ${direct_dpnl:+,.2f} | {changed} | {maturity_selected} |".format(
                label=label,
                core_ev=core["expected_value_score"],
                accepted_ev=accepted_row["after"]["expected_value_score"],
                maturity_ev=maturity_row["after"]["expected_value_score"],
                direct_ev=direct_row["delta"]["expected_value_score"],
                core_pnl=core["total_pnl"],
                accepted_dpnl=accepted_row["delta"]["total_pnl"],
                maturity_dpnl=maturity_row["delta"]["total_pnl"],
                direct_dpnl=direct_row["delta"]["total_pnl"],
                changed=direct_row["changed_selection_count"],
                maturity_selected=maturity_row["maturity_selected_count"],
            )
        )
    return rows
...
def _build_card(payload: dict[str, Any]) -> str:
    direct = payload["delta_metrics"]["accepted_allocator_to_source_maturity"]["aggregate"]
    core_to_maturity = payload["delta_metrics"]["core_to_source_maturity"]["aggregate"]
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Source Maturity Allocator",
            "",
            ...
            *_window_table(payload),
            ...
            "- Maturity aggregate EV delta vs core: `{:+.4f}`".format(
                core_to_maturity["expected_value_score_delta_sum"]
            ),
            "- Maturity aggregate PnL delta vs core: `${:+,.2f}`".format(
                core_to_maturity["total_pnl_delta_sum"]
            ),
...
def _build_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "aggregate_expected_value_delta": core_to_maturity[
            "expected_value_score_delta_sum"
        ],
        "aggregate_strategy_total_pnl_delta": core_to_maturity["total_pnl_delta_sum"],
        ...
        "windows": [
            {
                ...
                "source_maturity_expected_value": payload["source_maturity_metrics"][label][
                    "expected_value_score"
                ],
...
def _write_manifest(payload: dict[str, Any]) -> None:
    ...
    manifest = {
        ...
        "claim_note": (
            "Claim used --force only after stale broad-scope conflicts from "
            "exp-20260530-010 and exp-20260602-008 showed no locked-variable "
            "overlap with this allocator/source-maturity trial."
        ),
    }

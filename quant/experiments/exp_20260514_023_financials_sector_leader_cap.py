"""exp-20260514-023: Financials sector-leader position cap.

Tests one production-visible capital-allocation variable on the accepted core:
the already-accepted `trend_long + Financials + financials_sector_leader` sleeve
may be under-allocated because its 2.5x risk budget can hit the default 40%
single-position cap. This tests only that sleeve's max position cap.
"""
...
EXPERIMENT_ID = "exp-20260514-023"
EXPERIMENT_SLUG = "financials_sector_leader_cap"
CAP_KEY = "trend_financials_sector_leader_max_position_pct_applied"
CAP_SWEEP = [0.45, 0.50]
...
def _resize_with_cap(
    ...
    out = dict(sizing)
    out["trend_financials_sector_leader_cap_baseline_shares"] = old_shares
    out["trend_financials_sector_leader_cap_raw_shares"] = raw_shares
    out["trend_financials_sector_leader_cap_old_cap_shares"] = old_cap_shares
    out["trend_financials_sector_leader_cap_new_cap_shares"] = new_cap_shares
    out["trend_financials_sector_leader_cap_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "raw_shares": adjusted_sizing.get(
                                "trend_financials_sector_leader_cap_raw_shares"
                            ),
                            "old_cap_shares": adjusted_sizing.get(
                                "trend_financials_sector_leader_cap_old_cap_shares"
                            ),
                            "new_cap_shares": adjusted_sizing.get(
                                "trend_financials_sector_leader_cap_new_cap_shares"
                            ),
...
def _run_window(label: str, max_position_pct: float | None) -> dict[str, Any]:
    ...
    original_shared_financials_cap = (
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Financials Sector-Leader Cap",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        else "rejected_financials_sector_leader_cap"
    )
    interpretation = (
        "The existing Financials sector-leader sleeve remained cap-bound and the selected sleeve-specific cap improved the canonical three-window stack without EV regression."
        if detailed_selected["passed"]
        else "The sleeve-specific cap did not clear the canonical three-window gate; do not promote a Financials cap change without a new production-visible discriminator."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted trend_long Financials sector-leader sleeve is cap-bound by the default 40% single-position limit. Raising only that sleeve's cap should unlock profitable leader convexity without changing entries or the accepted Financials risk multipliers."
        ),
        "change_type": "capital_allocation_shared_policy",
        "changed_variable": "trend_financials_sector_leader_max_position_pct",
        "single_causal_variable": (
            "max_position_pct for trend_long Financials signals with financials_sector_leader=true"
        ),
        "parameters": {
            ...
            "locked_variables": [
                ...
                "raw Financials sector risk multiplier",
                "raw Financials sector-leader risk multiplier",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "capital allocation: unlock cap room only for an accepted, production-visible Financials trend sector-leader sleeve",
            "2_history_check": {
                "accepted_financials_sleeve": "current shared policy already has trend Financials 1.5x and Financials sector-leader 2.5x total risk",
                "exp-20260506-005": "Financials target-width variants were rejected; this run does not change targets",
                "exp-20260505-017": "Financials leader day-2 add-on cap was separate; this run tests initial signal sizing cap",
                "exp-20260514-018": "commodities near-high cap accepted on a different sleeve; this run tests the analogous cap-bound allocation idea for Financials leaders",
                "not_repeated": "does not alter raw Financials multiplier, target width, Space, VSAT, dual-RS, or LLM soft-ranking",
            },
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_023_financials_sector_leader_cap.py",
        },
        "backtest_protocol": {
        ...
        if detailed_selected["passed"]
        else "Use forward cap-room attribution or a new production-visible Financials leader state before retrying this allocation family.",
        "related_files": [
            "quant/experiments/exp_20260514_023_financials_sector_leader_cap.py",
            "data/experiments/exp-20260514-023/financials_sector_leader_cap.json",
            "docs/experiments/logs/exp-20260514-023.json",
            "docs/experiments/tickets/exp-20260514-023.json",
            "docs/experiments/artifacts/exp-20260514-023_financials_sector_leader_cap.md",

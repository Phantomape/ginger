...
Tests one risk-allocation variable on top of the accepted exp-20260515-021
Space stack: whether source-diverse official Space trend signals should get a
separate scalar when the ticker is still a Space peer nonleader.

This avoids LLM soft-ranking, ticker-pool expansion, defense-budget scalar
retries, entry/exit changes, ranking changes, and live Space slots.
"""
...
STEM = "space_source_diversity_peer_nonleader_trend_risk"
...
MARKER = "space_source_diversity_peer_nonleader_trend_risk"

ACCEPTED_DEFENSE_BUDGET_SAME_THEME_WINNER_SCALAR = 1.05
SCALARS = (1.0, 0.75, 0.9, 1.025, 1.05, 1.075)
    ...
    *,
    ...
    def patched_scale_and_record(
        *,
        ...
        if str(signal.get("space_peer_momentum_state") or "") != TARGET_PEER_STATE:
            counts[f"{MARKER}_skipped_peer_state"] += 1
            counts[f"{MARKER}_skipped_peer_state_{ticker}"] += 1
            return
        if not profile:
            counts[f"{MARKER}_missing_source_diversity_profile"] += 1
            return
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_source_diversity_peer_nonleader_trend_scalar": (
            peer_nonleader_scalar
        ),
        "target_strategy": TARGET_STRATEGY,
        "target_peer_momentum_state": TARGET_PEER_STATE,
    }
    variant["source_diversity_peer_nonleader_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["source_diversity_peer_nonleader_counts_by_window"] = by_window_counts
    variant["source_diversity_peer_nonleader_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(peer_nonleader_adjustments)
    )
    variant["source_diversity_peer_nonleader_adjustment_sample"] = (
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("source_diversity_peer_nonleader_counts") or {}
    ...
    missing_profile_count = int(
        counts.get(f"{MARKER}_missing_source_diversity_profile", 0)
    )
    scalar = float(
        variant["parameters"]["space_source_diversity_peer_nonleader_trend_scalar"]
    )
    passed = bool(
        scalar != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "missing_source_diversity_profile_count": missing_profile_count,
        "reasons": {
            "non_identity_scalar": scalar != 1.0,
            "changed_signals": changed_count,
            "source_diversity_profile_present": missing_profile_count == 0,
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Source-diverse official Space trend signals may need different "
            "allocation when the ticker is still a Space peer nonleader; this "
            "tests whether peer nonleadership is an under- or over-sizing state "
            "inside the accepted source-diversity trend sleeve."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_source_diversity_peer_nonleader_trend_scalar",
        "parameters": {
        ...
        "gate_answers": {
            ...
            "2_prior_similar_experiments": [
                ...
                "exp-20260514-044 accepted benchmark-breadth peer-nonleader trend risk.",
                "exp-20260513-019 rejected customer-source peer-nonleader risk.",
                "No prior experiment found that isolates source-diversity trend signals by peer-nonleader state on top of exp-20260515-021.",
            ],
            "3_single_causal_variable": (
                "Only the source-diversity peer-nonleader trend scalar changes."
            ),
            "4_success_criteria": (
            ...
            "5_reproducibility": (
                ...
                "exp_20260515_024_space_source_diversity_peer_nonleader_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the source-diversity peer-nonleader trend scalar "
            "did not improve enough fixed windows without regression."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby source-diversity peer-state scalars on these "
            "frozen windows; next Space evidence should be a broader mature "
            "forward cohort or a new production-visible catalyst-quality field."
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited, recent Space ticker/ETF "
            "admission failed, and the latest defense-budget early-RS60 scalar "
            "was too concentrated. This run isolates one existing production-visible "
            "source-diversity x peer-state allocation variable."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity peer-nonleader trend risk",
        "",
        "## Hypothesis",
        (
        ...
        (
            "`space_source_diversity_peer_nonleader_trend_scalar` on top of "
        ...
        f"- missing source-diversity profile count: `{gate['missing_source_diversity_profile_count']}`",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_source_diversity_peer_nonleader_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_peer_nonleader_signal_count']}`",
            f"- adjusted signals: `{gate['changed_peer_nonleader_signal_count']}`",
            f"- adjusted counts: `{best['source_diversity_peer_nonleader_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            f"Source-diversity peer-nonleader trend scalar "
            f"{best['parameters']['space_source_diversity_peer_nonleader_trend_scalar']} "
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            ...
            "peer nonleader may be mis-sized in the accepted stack."
        ),
        "changed_variable": "space_source_diversity_peer_nonleader_trend_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "missing_source_diversity_profile": gate[
                        "missing_source_diversity_profile_count"
                    ],

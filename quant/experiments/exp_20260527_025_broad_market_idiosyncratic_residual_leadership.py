...
This run changes one causal variable: replace the accepted raw SPY-relative
20-day momentum field with a beta-adjusted idiosyncratic residual return field
for candidate selection / ranking. The candidate source, price/liquidity
hygiene, hold period, notional profile, active-position limits, exits, and
production order path stay fixed.
...
EXPERIMENT_ID = "exp-20260527-025"
EXPERIMENT_SLUG = "broad_market_idiosyncratic_residual_leadership"
...
VARIANTS: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            "baseline_original_profile",
            {
                "rank_mode": "original_score",
                "profile_mode": "original_raw_spy_relative",
                "residual_ret20_min": None,
            },
        ),
        (
            "residual_rank_original_profile",
            {
                "rank_mode": "idiosyncratic_residual_score",
                "profile_mode": "original_raw_spy_relative",
                "residual_ret20_min": None,
            },
        ),
        (
            "residual_profile_min_0p025",
            {
                "rank_mode": "idiosyncratic_residual_score",
                "profile_mode": "beta_adjusted_residual_substitution",
                "residual_ret20_min": 0.025,
            },
        ),
        (
            "residual_profile_min_0p035",
            {
                "rank_mode": "idiosyncratic_residual_score",
                "profile_mode": "beta_adjusted_residual_substitution",
                "residual_ret20_min": 0.035,
            },
        ),
        (
            "residual_profile_min_0p050",
            {
                "rank_mode": "idiosyncratic_residual_score",
                "profile_mode": "beta_adjusted_residual_substitution",
                "residual_ret20_min": 0.050,
            },
        ),
    ]
)
...
def _daily_returns(rows: list[dict[str, Any]], idx: int, lookback: int) -> list[float]:
    if idx < lookback:
        return []
    values: list[float] = []
    for cursor in range(idx - lookback + 1, idx + 1):
        prev_close = _close(rows[cursor - 1])
        close = _close(rows[cursor])
...
def _beta_and_corr(left: list[float], right: list[float]) -> tuple[float | None, float | None]:
    size = min(len(left), len(right))
    if size < MIN_RETURN_PAIRS:
        return None, None
    left = left[-size:]
    right = right[-size:]
    ...
    var_right = sum((b - right_mean) ** 2 for b in right)
    var_left = sum((a - left_mean) ** 2 for a in left)
    if var_right <= 0 or var_left <= 0:
        return None, None
    beta = cov / var_right
    corr = cov / math.sqrt(var_left * var_right)
    if not math.isfinite(beta) or not math.isfinite(corr):
        return None, None
    return beta, max(-1.0, min(1.0, corr))


def _annotate_idiosyncratic_feature(
    *,
    ...
    returns = _daily_returns(rows, idx, LOOKBACK_DAYS)
    spy_returns = _daily_returns(spy_rows, spy_idx, LOOKBACK_DAYS)
    beta, corr = _beta_and_corr(returns, spy_returns)
    if beta is None or corr is None:
        return None

    ret20 = float(feature["ret20"])
    spy_ret20 = float(feature["spy_ret20"])
    residual_ret20 = ret20 - beta * spy_ret20
    residual_score = (
        residual_ret20
        + 0.50 * float(feature["ret60"])
        + 0.04 * min(float(feature["volume_ratio_20"]), 5.0)
        + 0.20 * (float(feature["near_high_60"]) - 0.90)
    )
    annotated = dict(feature)
    annotated["beta_60_to_spy"] = round(beta, 6)
    annotated["corr_60_to_spy"] = round(corr, 6)
    annotated["idiosyncratic_residual_ret20"] = round(residual_ret20, 6)
    annotated["idiosyncratic_residual_score"] = round(residual_score, 6)
    annotated["idiosyncratic_rule_version"] = "beta60_residual_ret20_v1"
    return annotated


def _passes_variant(feature: dict[str, Any], variant: dict[str, Any]) -> bool:
    profile_mode = str(variant["profile_mode"])
    if profile_mode == "original_raw_spy_relative":
        return candidate_passes_profile(feature, PROFILE_CONFIG)

    residual_min = float(variant["residual_ret20_min"])
    return bool(
        float(feature["idiosyncratic_residual_ret20"]) >= residual_min
        and float(feature["ret60"]) >= float(PROFILE_CONFIG["ret60_min"])
        and float(feature["near_high_60"]) >= float(PROFILE_CONFIG["near_high_60_min"])
        and float(feature["volume_ratio_20"]) >= float(PROFILE_CONFIG["volume_ratio_20_min"])
        and float(feature["close"]) >= float(PROFILE_CONFIG["decision_close_price_min"])
    )


def _rank_key(feature: dict[str, Any], variant: dict[str, Any]) -> tuple[float, float, float, str]:
    if variant["rank_mode"] == "original_score":
        return (
            float(feature["score"]),
            float(feature["ret20_excess_spy"]),
            float(feature["volume_ratio_20"]),
            str(feature["ticker"]),
        )
    return (
        float(feature["idiosyncratic_residual_score"]),
        float(feature["idiosyncratic_residual_ret20"]),
        float(feature["positive_day_ratio_20"]),
        str(feature["ticker"]),
    )
    ...
    *,
    ...
    for day in days:
        ...
        for ticker in candidate_tickers:
            ...
            if feature is None:
                continue
            if variant["rank_mode"] != "original_score" or variant["profile_mode"] != "original_raw_spy_relative":
                annotated = _annotate_idiosyncratic_feature(
                    feature=feature,
                    rows=rows,
                    idx=idx,
                    spy_rows=spy_rows,
                ...
                if annotated is None:
                    ...
                    continue
                feature = annotated
            if _passes_variant(feature, variant):
                features.append(feature)

        ranked = sorted(features, key=lambda row: _rank_key(row, variant), reverse=True)
        daily_counts[day] = len(ranked)
        selected_count = 0
        ...
        for queue_rank, feature in enumerate(ranked, start=1):
            ...
            if trade is None:
                continue
            trade["window"] = label
            trade["variant_name"] = variant_name
            trade["profile_mode"] = variant["profile_mode"]
            trade["rank_mode"] = variant["rank_mode"]
            trade["candidate_queue_rank"] = queue_rank
            trade["day_candidate_count"] = len(ranked)
            trade["beta_60_to_spy"] = feature.get("beta_60_to_spy")
            trade["corr_60_to_spy"] = feature.get("corr_60_to_spy")
            trade["idiosyncratic_residual_ret20"] = feature.get(
                "idiosyncratic_residual_ret20"
            )
            trade["idiosyncratic_residual_score"] = feature.get(
                "idiosyncratic_residual_score"
            )
            trades.append(trade)
        ...
        rows.append(
            {
                ...
                "beta_60_to_spy": trade.get("beta_60_to_spy"),
                "corr_60_to_spy": trade.get("corr_60_to_spy"),
                "idiosyncratic_residual_ret20": trade.get(
                    "idiosyncratic_residual_ret20"
                ),
                "idiosyncratic_residual_score": trade.get(
                    "idiosyncratic_residual_score"
                ),
...
def _window_sleeve_summary(scout: dict[str, Any]) -> dict[str, Any]:
    ...
    base["missing_residual_count"] = scout["missing_residual_count"]
...
def _variant_payload(
    *,
    ...
    for label, spec in WINDOWS.items():
        scout = _select_trades_for_window(
            label=label,
            variant_name=variant_name,
            variant=variant,
    ...
    gate4_passed = bool(
        variant_name != "baseline_original_profile"
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pool = passing or [
        row for row in variants if row["variant_name"] != "baseline_original_profile"
    ]
    return sorted(
        pool,
        key=lambda row: (
    ...
    return [
        {
            ...
            "profile_mode": row["variant"]["profile_mode"],
            "rank_mode": row["variant"]["rank_mode"],
            "residual_ret20_min": row["variant"]["residual_ret20_min"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Idiosyncratic Residual Leadership",
        "",
        f"Decision: `{payload['decision']}`.",
        "",
        (
            "Single causal variable: beta-adjusted 20-day idiosyncratic residual "
            "return substituted for raw SPY-relative 20-day momentum in the "
            "broad-market paper candidate surface."
        ),
        "",
        "## Sweep",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {gate} | {replaced} | {ev:+.4f} | {rel:+.2%} | ${pnl:+,.2f} | {wi} | {wr} | {pr} | {dd:+.4%} | {top5} |".format(
...
def build_payload() -> dict[str, Any]:
    ...
    identity = _variant_payload(
        variant_name="baseline_original_profile",
        variant=VARIANTS["baseline_original_profile"],
    ...
    identity_keys = {
        _trade_key(row)
        for label in WINDOWS
        for row in _select_trades_for_window(
            label=label,
            variant_name="baseline_original_profile",
            variant=VARIANTS["baseline_original_profile"],
            candidate_tickers=frozen_tickers,
    ...
    variants.extend(
        ...
        if name != "baseline_original_profile"
    )
    ...
    decision = (
        "accepted_default_off_broad_market_idiosyncratic_residual_leadership"
        if accepted
        else "rejected_broad_market_idiosyncratic_residual_leadership"
    )
    ...
    production_impact = {
        ...
        "trade_enabled": False,
        "note": (
            "No production behavior changed. Promotion would require the same "
            "beta-adjusted residual calculation in a shared default-off paper "
            "adapter and parity tests."
        ),
    }
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "Broad-market momentum winners with positive raw SPY-relative returns "
            "may still be only high-beta market proxies. Replacing raw ret20 "
            "excess-SPY with beta-adjusted idiosyncratic residual leadership may "
            "improve the candidate pool without adding noisy tickers."
        ),
        "change_type": "default_off_paper_candidate_selection",
        "changed_variable": "broad_market_beta_adjusted_idiosyncratic_residual_ret20",
        "single_causal_variable": (
            "substitute beta60-adjusted residual 20-day return for raw "
            "SPY-relative 20-day momentum in broad-market candidate selection/ranking"
        ),
        "component": _repo_rel(Path(__file__)),
        "trial_accounting": {
            "trial_family": "broad_market_idiosyncratic_residual_leadership",
            "new_evidence_type": "free_ohlcv_beta_adjusted_residual_momentum",
            "nearby_prior_experiments": REFERENCE_EXPERIMENTS,
            "anti_repeat": (
                "This is not a liquidity/cost scalar and not the prior correlation "
                "crowding cap; it tests a beta-adjusted replacement for the "
                "candidate's primary 20-day leadership field."
            ),
        },
        "parameters": {
            ...
            "minimum_return_pairs": MIN_RETURN_PAIRS,
            "minimum_relative_ev_improvement": MIN_RELATIVE_EV_IMPROVEMENT,
            "variants": VARIANTS,
        ...
        "backtest_protocol": (
            ...
            "state; after state replays beta-adjusted residual candidate selection."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "ranking/candidate_pool: beta-adjusted idiosyncratic residual leadership should distinguish true single-name momentum from high-beta market proxies.",
            "2_past_similar_experiments": "exp-20260524-023 correlation crowding improved all windows but missed 10% materiality; exp-20260524-030 and exp-20260527-024 liquidity/cost variants failed; exp-20260527-023 Companyfacts+RS improved aggregate but failed window/risk constraints.",
            "3_single_variable": "Only the 20-day leadership field changes from raw ret20 excess-SPY to beta60-adjusted residual ret20; candidate source, hygiene, notional profile, hold, slots, and exits stay fixed.",
            "4_acceptance": "Gate 4 requires positive aggregate EV/PnL, all 3 windows EV-positive, no EV/PnL regression windows, >=4 replacements across >=2 windows, concentration guard, <=0.5pp max drawdown worsening, identity control, and >=10% aggregate EV improvement because this is a frozen broad-market paper surface retune.",
            "5_reproducibility": "Script, JSON artifact, log, ticket, markdown artifact, and JSONL identify windows, baseline/control artifacts, variants, selected trades, and metrics.",
        },
        "interpretation": (
            "The direction uses only free OHLCV and directly targets the playbook's "
            "hidden-beta/crowding concern, but no shared adapter is changed unless "
            "the strict materiality gate is cleared."
        ),
        "rejection_reason": None
        if accepted
        else "Idiosyncratic residual leadership did not clear the strict three-window broad-market materiality gate.",
        "next_evidence_needed": (
            "If future broad-market forward paper logs show high-beta raw leaders "
            "underperforming beta-adjusted leaders, retest with live replacement "

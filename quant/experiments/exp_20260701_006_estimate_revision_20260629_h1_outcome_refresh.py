"""exp-20260701-006: refresh h1 outcomes for 2026-06-29 estimate revisions.

Measurement repair only. Exp-20260630-022 materialized the five
estimate-revision matched candidate rows with h0 outcomes while the hot
warehouse ended at 2026-06-29. The warehouse now reaches 2026-06-30, so this
runner closes the same rows' h1 cash/SPY/QQQ replacement values without changing
strategy behavior.
"""
...
EXPERIMENT_ID = "exp-20260701-006"
OWNER = "alpha-explore"
SLUG = "estimate_revision_20260629_h1_outcome_refresh"
RUNNER = f"quant/experiments/exp_20260701_006_{SLUG}.py"
RUNNER_COMMAND = ".\\.venv\\Scripts\\python.exe -B " + RUNNER.replace("/", "\\")

DATA_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = DATA_DIR / f"exp_20260701_006_{SLUG}.json"
...
PRIOR_OUTCOME_ARTIFACT = (
    REPO_ROOT
    / "data"
    / "experiments"
    / "exp-20260630-022"
    / "exp_20260630_022_estimate_revision_20260629_candidate_match_hot_outcome_ledger.json"
)
...
LATEST_COMPLETE_TRADING_DAY = "2026-06-30"
HORIZONS = (0, 1, 3, 5, 10)
COMPARATORS = ("SPY", "QQQ")
PROXY_NOTIONAL_USD = 4000.0

HYPOTHESIS = (
    "Alpha-enabling repair: the 2026-06-29 estimate-revision matched candidate "
    "rows can now close h1 replacement outcomes because the hot warehouse "
    "advanced to 2026-06-30; materialize that outcome ledger without changing "
    "strategy behavior."
)
ALPHA_HYPOTHESIS = (
    "Estimate-revision direction may have replacement value when it overlaps "
    "same-day production-visible candidate rows, but it needs closed "
    "replacement-value outcomes before any threshold, direction, ranking, or "
    "allocation rule can be judged."
)
CHANGED_VARIABLE = "estimate_revision_20260629_h1_forward_outcome_refresh_v1"
TRIAL_FAMILY = "estimate_revision_candidate_match_outcome_ledger"
TRIAL_VARIANT_ID = "post_quant_signal_20260629_h1_hot_warehouse_outcomes_v1"
MECHANISM_FAMILY = "production_visible_estimate_revision_candidate_match_attribution"
NEW_EVIDENCE_AXIS = (
    "Hot warehouse max date advanced from 2026-06-29 to 2026-06-30, so the "
    "same predeclared 2026-06-29 matched candidate rows gain genuinely new "
    "settled h1 cash/SPY/QQQ replacement-value outcomes; no revision "
    "thresholds, rank, hold, notional, or condition slices change."
)
ALLOWED_WRITE_SCOPE = [
    RUNNER,
    f"data/experiments/{EXPERIMENT_ID}/exp_20260701_006_{SLUG}.json",
...
def warehouse_date_range() -> dict[str, Any]:
    ...
    with sqlite3.connect(f"file:{HOT_WAREHOUSE}?mode=ro", uri=True) as con:
...
def load_matched_candidate_rows() -> list[dict[str, Any]]:
    matched = [row for row in read_jsonl(REVISION_LEDGER) if row.get("matched_candidate_today")]
...
def build_outcome_rows() -> tuple[list[dict[str, Any]], dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "prior_outcome_experiment_id": "exp-20260630-022",
    ...
    source_metadata = {
        ...
        "prior_outcome_artifact": repo_rel(PRIOR_OUTCOME_ARTIFACT),
        "prior_outcome_artifact_exists": PRIOR_OUTCOME_ARTIFACT.exists(),
        ...
        "hot_warehouse_missing_matched_tickers": sorted(
            tickers - set(COMPARATORS) - set(loaded_tickers)
        ),
...
def build_calibration(
    ...
    semantic_hit = (
        "alpha_still_not_ready_thin_sample" in predicted
        and "matched_nonflat_sample_too_thin" in alpha_blockers
    )
    exact_hit = bool(predicted & set(realized))
    return {
        ...
        "surprise_note": (
            "Expected: warehouse advance closed h1 while sample and h3/h5/h10 "
            "alpha-readiness blockers remained."
            if measurement_passed
            else "Unexpected: the h1 refresh could not fully materialize."
        ),
    }
...
def build_payload() -> dict[str, Any]:
    ...
    if not BASELINE_RESULT.exists() or baseline.get("window_count") != 3:
        measurement_blockers.append("canonical_baseline_missing_or_invalid")
    if not SOURCE_EXPERIMENT_ARTIFACT.exists():
        measurement_blockers.append("source_exp20260630_021_artifact_missing")
    if not PRIOR_OUTCOME_ARTIFACT.exists():
        measurement_blockers.append("prior_exp20260630_022_outcome_artifact_missing")
    ...
    max_hot_date = source_metadata["hot_warehouse_date_range"].get("max_date") or ""
    if max_hot_date < LATEST_COMPLETE_TRADING_DAY:
        measurement_blockers.append("hot_warehouse_missing_20260630_bars")
    if source_metadata["hot_warehouse_missing_matched_tickers"]:
        measurement_blockers.append("hot_warehouse_missing_matched_tickers")
    if summary["closed_rows_by_horizon"]["h1"] < source_metadata["matched_candidate_rows"]:
        measurement_blockers.append("h1_outcomes_not_fully_settled")
    if any(
        row.get("h1_status") == "closed"
        and (
            row.get("h1_replacement_value_vs_spy_usd") is None
            or row.get("h1_replacement_value_vs_qqq_usd") is None
        )
        for row in rows
    ):
        measurement_blockers.append("h1_comparator_missing")
    ...
    if summary["closed_rows_by_horizon"]["h1"] < 20:
        alpha_blockers.append("forward_1d_outcomes_below_alpha_floor")
    ...
    decision = (
        "accepted_measurement_repair_estimate_revision_20260629_h1_outcome_refresh"
        if measurement_passed
        else "blocked_estimate_revision_20260629_h1_outcome_refresh"
    )
    before_after_delta = {
    ...
    return {
        ...
        "implementation_mode": "measurement_repair_hot_warehouse_h1_outcome_refresh",
        ...
        "causal_components": [
            ...
            "hot warehouse h1 replacement values through 2026-06-30",
            "no strategy behavior change",
        ],
        "nearby_prior_experiments": ["exp-20260630-021", "exp-20260630-022"],
        "multiple_testing_risk_bucket": "low",
        "new_evidence_type": "settled_forward_rows",
        ...
        "pre_run_questions": {
            ...
            "2_history_check": {
                ...
                "exp-20260630-022": (
                    "Accepted h0 outcome ledger while hot warehouse ended at "
                    "2026-06-29; h1/h3/h5/h10 remained pending."
                ),
                "novelty_gate": ticket.get("novelty"),
            },
            "3_single_policy_bundle": (
                "One measurement bundle: refresh the same five 2026-06-29 matched "
                "candidate rows with h1 replacement values after the warehouse "
                "advanced to 2026-06-30, with no threshold/rank/hold/notional changes."
            ),
            "4_success_failure_standard": (
                "Accept only as measurement repair if baseline remains unchanged, "
                "all five matched candidate rows have h1 cash/SPY/QQQ values, "
                "and strategy delta is zero. Do not accept alpha from this sample."
            ),
        ...
        "gate2": {
            ...
            and PRIOR_OUTCOME_ARTIFACT.exists()
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "The hot warehouse advanced to 2026-06-30, closing h1 outcomes "
                "for the same five 2026-06-29 matched estimate-revision rows. "
                "Alpha remains blocked because there is only one non-flat usable "
                "revision row, h1 count is five below the 20-row floor, and h3/h5/h10 "
                "replacement outcomes are still pending."
                if measurement_passed
                else "The hot-warehouse h1 outcome refresh could not fully materialize."
            ),
            ...
            "new_evidence_required": (
                "Next alpha-compliant revision work needs h3/h5/h10 settlement for "
                "these rows plus materially more matched/non-flat rows, another "
                "settled production revision cohort, or a different unsaturated PIT "
                "expectation source."
            ),
        },
        "related_files": [
            ...
            repo_rel(PRIOR_OUTCOME_ARTIFACT),
...
def build_card(payload: dict[str, Any]) -> str:
    ...
    h1_cash = summary["replacement_value_by_horizon"]["h1"]["vs_cash"]
    h1_spy = summary["replacement_value_by_horizon"]["h1"]["vs_spy"]
    h1_qqq = summary["replacement_value_by_horizon"]["h1"]["vs_qqq"]
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: estimate revision 2026-06-29 h1 outcome refresh",
            "",
            ...
            f"- Closed h1 rows: `{summary['closed_rows_by_horizon']['h1']}`",
            f"- Closed h3/h5/h10 rows: `{summary['closed_rows_by_horizon']['h3']} / {summary['closed_rows_by_horizon']['h5']} / {summary['closed_rows_by_horizon']['h10']}`",
            f"- Mean h1 replacement vs cash: `{h1_cash.get('mean')}`",
            f"- Mean h1 replacement vs SPY: `{h1_spy.get('mean')}`",
            f"- Mean h1 replacement vs QQQ: `{h1_qqq.get('mean')}`",
        ...
        json.dumps(
            {
                ...
                "h1_replacement_vs_cash": payload["outcome_summary"]["replacement_value_by_horizon"]["h1"]["vs_cash"],
                "h1_replacement_vs_spy": payload["outcome_summary"]["replacement_value_by_horizon"]["h1"]["vs_spy"],
                "h1_replacement_vs_qqq": payload["outcome_summary"]["replacement_value_by_horizon"]["h1"]["vs_qqq"],

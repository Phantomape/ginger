"""exp-20260607-023: volatility-stress resilient-leadership scout.

Replay-only alpha search. This tests one production-visible free-OHLCV
candidate-source variable: on days where VIXY spikes while SPY and QQQ close
weak, select liquid stocks that still show same-day resilience as next-open,
...
EXPERIMENT_ID = "exp-20260607-023"
STEM = "volatility_stress_resilient_leadership"
TRIAL_FAMILY = "volatility_stress_resilient_leadership_candidate_pool"
TRIAL_VARIANT_ID = "vixy_up_spy_qqq_weak_resilient_stock_leadership_top2_10d_v1"
CHANGED_VARIABLE = "volatility_stress_resilient_leadership_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = previous.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260607_023_{STEM}.json"
...
MIN_VIXY_STRESS_RETURN = 0.035
MIN_VIXY_CLOSE_LOCATION = 0.55
MAX_SPY_STRESS_RETURN = -0.003
MAX_QQQ_STRESS_RETURN = -0.004
MAX_SPY_CLOSE_LOCATION = 0.45
MAX_QQQ_CLOSE_LOCATION = 0.45
MIN_PRICE = 10.0
MIN_AVG_DOLLAR_VOLUME_20D = 50_000_000.0
MIN_SIGNAL_RETURN = 0.000
MIN_RELATIVE_VS_SPY = 0.012
MIN_RELATIVE_VS_QQQ = 0.010
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "macro_stress_resilience_neighbor_failed",
        "window_regression",
        "drawdown_drift",
        "thin_sample",
    ...
    "confidence_reason": (
        "Accepted VIXY relief shows the volatility product contains useful "
        "state information, but macro-stress resilient leadership failed and "
        "this mirror relation may be defensive beta rather than continuation "
        "alpha; run only a fixed replay scout before any shared helper."
    ),
    "recorded_at": "2026-06-07T19:24:10+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "require a shared default-off adapter exposing the same VIXY/SPY/QQQ "
        "volatility-stress context, sector-known liquid stock universe, "
        "resilient-stock leadership fields, same-ticker core-overlap exclusion, "
...
def _load_window_snapshot(
    *,
    ...
    return BASE_LOAD_WINDOW_SNAPSHOT(
        cfg=cfg,
        eligible_tickers=set(eligible_tickers) | {"VIXY"},
    )
...
def _stress_context_for_day(
    ...
    vixy_close_location = _range_location(vixy_rows[vixy_idx])
    ...
    context = {
        ...
        "min_vixy_stress_return": MIN_VIXY_STRESS_RETURN,
        "min_vixy_close_location": MIN_VIXY_CLOSE_LOCATION,
    ...
    if vixy_return < MIN_VIXY_STRESS_RETURN:
        return {**context, "passed": False, "reason": "vixy_stress_return_too_low"}
    if vixy_close_location < MIN_VIXY_CLOSE_LOCATION:
        return {**context, "passed": False, "reason": "vixy_close_not_strong_enough"}
    if spy_return > MAX_SPY_STRESS_RETURN:
        return {**context, "passed": False, "reason": "spy_stress_return_too_high"}
    if qqq_return > MAX_QQQ_STRESS_RETURN:
        return {**context, "passed": False, "reason": "qqq_stress_return_too_high"}
    if spy_close_location > MAX_SPY_CLOSE_LOCATION:
        return {**context, "passed": False, "reason": "spy_close_not_weak_enough"}
    if qqq_close_location > MAX_QQQ_CLOSE_LOCATION:
        return {**context, "passed": False, "reason": "qqq_close_not_weak_enough"}
    return {**context, "passed": True, "reason": "vixy_spy_qqq_volatility_stress_passed"}
...
def _candidate_for_ticker(
    *,
    ...
    if row is None:
        return None
    row["source"] = "VOLATILITY_STRESS_RESILIENT_LEADERSHIP_PAPER"
    row["volatility_stress_context"] = row.pop("macro_relief_context", context)
...
def _candidate_rows_for_window(
    *,
    ...
    scan = {
        ...
        "volatility_stress_days": 0,
        "non_stress_days": 0,
        "days_with_raw_volatility_stress_candidates": 0,
        "raw_volatility_stress_candidates": 0,
    }
    for signal_date in dates:
        ...
        if not context.get("passed"):
            scan["non_stress_days"] += 1
            continue
        scan["volatility_stress_days"] += 1
        ...
        scan["days_with_raw_volatility_stress_candidates"] += 1
        scan["raw_volatility_stress_candidates"] += len(day_rows)
        contexts.append(
            {
                **context,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_volatility_stress_resilient_leadership"
        if gate["passed"]
        else "rejected_volatility_stress_resilient_leadership_candidate_pool"
    )
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "VIXY volatility-stress days with weak SPY/QQQ may identify "
                "liquid resilient stock leaders whose next-open continuation "
                "beats cash without retuning the accepted VIXY relief adapter."
            ),
            ...
            "new_evidence_type": "production_visible_free_ohlcv_vixy_stress_state",
            ...
            "interpretation": (
                "The volatility-stress resilient-leadership source cleared Gate 4 "
                "as a replay-only/default-off lead, but no production surface "
                "was promoted."
                if accepted
                else (
                    "The volatility-stress resilient-leadership source did not "
                    "clear Gate 4. Do not promote it or answer by retuning "
                    "VIXY stress, SPY/QQQ weakness, stock resilience, top-N, "
                    "hold-day, cooldown, or notional on these frozen windows."
                )
            ),
            "rejection_reason": None if accepted else "; ".join(gate4["failed_reasons"]),
            "negative_reflection": (
                "If rejected, the likely reason is that VIXY stress resilience "
                "is defensive beta, gets overwhelmed by risk-off continuation, "
                "or is too sparse after next-open execution and costs."
            ),
            "post_run_reflection": {
                "why_result_happened": (
                    "The source produced positive replacement value in all "
                    "three windows without breaching drawdown, survival, or "
                    "concentration guardrails, suggesting volatility-expansion "
                    "resilience added a distinct defensive continuation state."
                    if accepted
                    else (
                        "The source failed to add robust replacement value "
                        "after next-open entry and costs. That suggests VIXY "
                        "stress did not reliably separate durable resilient "
                        "stock leadership from broad risk-off noise in the "
                        "canonical windows."
                    )
                ),
                "forbidden_near_neighbor_retry": (
                    "Do not retry by sweeping VIXY stress return, VIXY "
                    "close-location, SPY/QQQ weakness, stock close-location, "
                ...
                "new_evidence_required": (
                    ...
                    "such as VIX term structure, options flow, realized-vol "
                    "risk-transfer fields, or closed forward replacement-value "
    ...
    payload["backtest_protocol"]["execution_model"] = (
        ...
        "VIXY, SPY, and QQQ daily return/range context plus stock resilience, "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: volatility-premium expansion with SPY/QQQ "
            "weakness may identify resilient stock leaders before the next "
            "session fully reprices defensive leadership."
        ),
        "2_history_check": {
            "exp-20260606-027": (
                "Macro stress resilient leadership failed with thin sample and "
                "negative EV; this uses observed VIXY volatility expansion "
                "across all days rather than only official event stress days."
            ),
            "exp-20260607-018/019": (
                "VIXY relief leadership was accepted; this is the mirror "
                "stress-resilience relation, not a threshold retune."
            ),
            "exp-20260607-021/022": (
                "UUP and crypto risk-on proxy leadership failed; this tests "
                "risk-off resilience rather than another risk-on beta proxy."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260607_023_volatility_stress_resilient_leadership.py"
        ),
    }
    payload["pre_run_questions"] = payload["gate_questions"]
    payload["gate3"]["note"] = (
        "No new core filter or entry rule was added. The volatility-stress "
        "resilient-leadership source is additive default-off paper, so core signals "
        "generated/survived are unchanged from baseline."
    )
    runtime_fields = payload.setdefault("gate2", {}).setdefault("runtime_fields", [])
    if "VIXY daily OHLCV" not in runtime_fields:
        runtime_fields.insert(3, "VIXY daily OHLCV")
    return payload
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    for label in framework.WINDOWS:
        ...
        rows.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {dd:+.4f} | {days} | {trades} |".format(
                ...
                days=scan.get("volatility_stress_days", 0),
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Volatility Stress Resilient Leadership",
            "",
        ...
        "windows": [
            {
                ...
                "volatility_stress_day_count": payload["context_scan_by_window"][label].get(
                    "volatility_stress_days"
                ),
...
def _patch_framework() -> None:
    ...
    previous.MIN_SPY_RELIEF_RETURN = MAX_SPY_STRESS_RETURN
    previous.MIN_QQQ_RELIEF_RETURN = MAX_QQQ_STRESS_RETURN
    previous.MIN_SPY_CLOSE_LOCATION = MAX_SPY_CLOSE_LOCATION
    previous.MIN_QQQ_CLOSE_LOCATION = MAX_QQQ_CLOSE_LOCATION
    ...
    previous._relief_context_for_day = _stress_context_for_day

"""exp-20260621-011: available-proxy residual idiosyncratic leadership scout.
...
SPY and production-visible free OHLCV proxy returns can identify idiosyncratic
demand instead of generic market/rate/sector beta.
...
EXPERIMENT_ID = "exp-20260621-011"
STEM = "proxy_residual_idiosyncratic_leadership"
TRIAL_FAMILY = "available_proxy_residual_idiosyncratic_leadership_candidate_pool"
TRIAL_VARIANT_ID = "available_proxy_residual_top1_next_open_10d_v1"
CHANGED_VARIABLE = "available_proxy_residual_idiosyncratic_leadership_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE
OWNER = "codex-alpha-search"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260621_011_{STEM}.json"
...
FACTOR_ETF_TICKERS = ("IWM", "TLT", "XLE", "XLP", "XLU")
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "generic_proxy_residual_relabel",
    ],
    "confidence_reason": (
        "exp-20260621-003 was blocked because MTUM/QUAL/VLUE/USMV/SIZE were "
        "not production-visible in the main OHLCV warehouse. This run uses "
        "only SPY/IWM/TLT/XLE/XLP/XLU, which are free, warehouse-backed, and "
        "available across the three canonical windows."
    ),
    "recorded_at": "2026-06-21T12:06:50+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "uses_available_proxy_reference_context": True,
    ...
    "execution_envelope": {
        ...
        "failure_handling": (
            "missing SPY/IWM/TLT/XLE/XLP/XLU OHLCV, insufficient beta-fit history, "
    ...
    "parity_note": (
        ...
        "available proxy reference context, PIT rolling beta fit, residual "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        ...
        "that remains positive after rolling residualization against SPY/IWM/"
        "TLT/XLE/XLP/XLU warehouse OHLCV reference returns may capture "
        "idiosyncratic demand, expanding the default-off paper candidate pool "
        "without relying on generic momentum beta."
    ),
    "2_history_check": {
        "novelty_gate": (
            "Novelty gate warned on idiosyncratic-leader near neighbors. "
            "The override records the new evidence axis: only production-"
            "visible warehouse OHLCV proxy tickers with full three-window "
            "coverage, unlike exp-20260621-003's missing factor ETF surface."
        ),
        "exp-20260614-006": (
            ...
            "this run uses broad market/rate/sector proxy residualization."
        ),
        "exp-20260620-020": (
        ...
        "exp-20260621-003": (
            "Blocked because MTUM/QUAL/VLUE/USMV/SIZE were absent from the "
            "production-visible OHLCV warehouse. This run deliberately uses "
            "SPY/IWM/TLT/XLE/XLP/XLU, which are present in the warehouse."
        ),
        "exp-20260621-002": (
    ...
    "5_reproducibility": (
        ...
        "exp_20260621_011_proxy_residual_idiosyncratic_leadership.py"
    ),
}
...
def _factor_residual_metrics(
    *,
    ...
    for offset, ticker in enumerate(FACTOR_ETF_TICKERS, start=2):
        metrics[f"factor_beta_{ticker.lower()}"] = _round(coeffs[offset], 6)
    return metrics
    ...
    *,
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "AVAILABLE_PROXY_RESIDUAL_IDIOSYNCRATIC_LEADERSHIP_PAPER",
    ...
    return candidates, {
        **dict(scan),
        ...
        "available_proxy_tickers": list(FACTOR_ETF_TICKERS),
...
def _configure_template() -> None:
    ...
    template.GROWTH_PROXY_TICKER = "IWM"
...
def _finalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    missing_proxy_surface_windows = [
        label
        for label in framework.WINDOWS
        if not payload["context_scan_by_window"][label].get("factor_feature_dates", 0)
    ]
    proxy_surface_blocked = bool(missing_proxy_surface_windows)
    decision = (
        "blocked_available_proxy_residual_reference_surface_missing"
        if proxy_surface_blocked
        else
        "positive_replay_lead_not_promoted_available_proxy_residual_idiosyncratic_leadership"
        if gate4["passed"]
        else "rejected_available_proxy_residual_idiosyncratic_leadership_candidate_pool"
    )
    if proxy_surface_blocked:
        payload["status"] = "blocked"
    payload["decision"] = decision
    payload["gate4"]["decision"] = decision
    if proxy_surface_blocked and "available_proxy_reference_history_gap" not in payload["gate4"]["failed_reasons"]:
        payload["gate4"]["failed_reasons"].insert(0, "available_proxy_reference_history_gap")
    payload["mechanism_family"] = "production_visible_free_proxy_residual_ohlcv_candidate_pool"
    payload["new_evidence_type"] = "production_visible_free_ohlcv_proxy_surface"
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SPY, IWM, TLT, XLE, XLP, XLU, and candidate stock features are "
    ...
    payload["gate2"]["runtime_fields"] = [
        ...
        "IWM OHLCV Date/Open/High/Low/Close/Volume",
        "TLT OHLCV Date/Open/High/Low/Close/Volume",
        "XLE OHLCV Date/Open/High/Low/Close/Volume",
    ...
    if proxy_surface_blocked:
        ...
        payload["gate2"]["blocking_item"] = (
            "warehouse_main.sqlite lacks one or more SPY/IWM/TLT/XLE/XLP/XLU "
            "OHLCV reference rows across the canonical windows, so the "
            "residual candidate source cannot be evaluated without breaking "
            "the production/backtest parity contract."
        )
    ...
    payload["interpretation"] = (
        "The available-proxy residual idiosyncratic leadership scout is "
        "blocked at Gate 2: one or more SPY/IWM/TLT/XLE/XLP/XLU reference "
        "rows are absent from the production-visible OHLCV warehouse, so no "
        "trustworthy residual candidate source can be evaluated."
        if proxy_surface_blocked
        else
        "The available-proxy residual idiosyncratic leadership source cleared "
        "Gate 4 as a replay-only/default-off lead, but no production surface "
        "was promoted."
        if gate4["passed"]
        else (
            "The available-proxy residual idiosyncratic leadership source did "
            "not clear Gate 4 (failed: "
            + (", ".join(gate4["failed_reasons"]) or "none")
            + "). Do not promote or tune this fixed proxy-residual OHLCV "
            "bundle on the same frozen windows."
        )
    )
    payload["next_evidence_needed"] = (
        "A retry needs materially different PIT flow, ownership, borrow/options, "
        "or closed forward replacement-value evidence. Do not sweep proxy "
    ...
    payload["post_run_reflection"] = {
        "why_result_happened": (
            ...
            "SPY/IWM/TLT/XLE/XLP/XLU reference rows, but warehouse_main.sqlite "
            "does not contain a full reference surface in the canonical "
            "windows. Using any sidecar-only data path would break the "
            ...
            if gate4["passed"]
                ...
                "Rejected. Available-proxy residual leadership did not add robust "
        ...
        "outcome_summary": (
            ...
            "max drawdown drift {:+.4f}; {} paper trades; proxy surface "
            "blocked windows: {}.".format(
                aggregate["expected_value_score_delta_sum"],
                aggregate["total_pnl_delta_sum"],
                float(aggregate["max_drawdown_delta_max"] or 0.0),
                target_summary["total_trade_count"],
                ", ".join(missing_proxy_surface_windows) or "none",
            )
        ),
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping proxy lists, beta lookbacks, residual "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Available Proxy Residual Idiosyncratic Leadership",
            "",

...
after-hours operational 8-K items (`1.01`, `7.01`, `8.01`) whose first usable
trading day confirms prior trend/RS quality. To preserve production/backtest
parity, the paper entry is delayed until the next open after that close is
known.
...
EXP_ID = "exp-20260605-019"
STEM = "sec_after_hours_8k_trend_candidate_pool"
TRIAL_FAMILY = "sec_after_hours_operational_8k_candidate_pool"
TRIAL_VARIANT_ID = "sec_after_hours_8k_trend_top1_delayed_entry_v1"
CHANGED_VARIABLE = "sec_after_hours_operational_8k_prior_trend_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXP_ID
OUT_JSON = OUT_DIR / f"exp_20260605_019_{STEM}.json"
...
MIN_ACCEPTED_MINUTES_AFTER_MIDNIGHT = 20 * 60
MIN_PRICE = 10.0
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "a separate shared default-off SEC filing-timing adapter and parity "
        "tests before any report queue, candidate priority, or order surface "
        "could change."
    ),
}
...
def _accepted_minutes(raw: Any) -> int | None:
    text = str(raw or "")
    if len(text) < 16:
        return None
    try:
...
def _candidate_from_feature_row(
    ...
    *,
    ...
    accepted_minutes = _accepted_minutes(row.get("accepted_datetime"))
    if accepted_minutes is None:
        return None, "missing_accepted_datetime"
    if accepted_minutes < MIN_ACCEPTED_MINUTES_AFTER_MIDNIGHT:
        return None, "not_after_hours"
    ...
    if volume_ratio_20d is None:
        return None, "missing_volume_context"
    ...
    if signal_day_return is None:
        return None, "missing_signal_day_return"
    ...
    after_hours_score = min(max((accepted_minutes - MIN_ACCEPTED_MINUTES_AFTER_MIDNIGHT) / 180.0, 0.0), 1.0)
    candidate_score = (
        item_score
        + close_location
    ...
    return {
        ...
        "accepted_minutes_after_midnight": accepted_minutes,
        "after_hours_score": round(after_hours_score, 6),
        "status": "price_ready",
        "strategy": STEM,
        ...
        "known_at": "after_hours_sec_filing_plus_usable_trade_date_close_before_next_open_delayed_paper_entry",
        "trade_enabled": False,
        "alters_orders": False,
    }, "candidate_ready"
...
def _gate4(
    ...
    if passed:
        ...
        rationale = (
            "The after-hours operational 8-K trend source improved all "
        ...
        status = "rejected"
        rationale = (
            "One or more Gate 4 checks failed, so this SEC after-hours 8-K "
            "trend candidate source is not retained or promoted."
        )
    return {
        "decision": decision,
        "status": status,
...
def _write_artifact(payload: dict[str, Any]) -> None:
    ...
    lines = [
        f"# {EXP_ID} SEC After-Hours 8-K Trend Candidate Pool",
        "",
    ...
    lines.extend(
        [
            ...
            (
                ...
                "items, require `accepted_datetime` at or after 20:00, require "
                "first usable trading-day close-location >= "
                f"{MIN_SIGNAL_CLOSE_LOCATION}, and require nonnegative 20d "
                "excess return versus SPY. Entry is delayed to the next open "
            ...
            (
                ...
                "quant\\experiments\\exp_20260605_019_sec_after_hours_8k_trend_candidate_pool.py"
            ),
            "",
...
def build_payload() -> dict[str, Any]:
    ...
    ticket_prediction = (_load_json(TICKET_JSON, {}).get("prediction") or {})
    prediction = ticket_prediction if isinstance(ticket_prediction, dict) else {}
    return {
        "experiment_id": EXP_ID,
        ...
        "preflight": {
            "alpha_hypothesis": (
                "After-hours operational SEC 8-K filings with prior trend/RS "
                "quality may identify next-open delayed-continuation "
                "default-off paper candidates before the event is fully absorbed."
            ),
            "category": "entry_candidate_pool",
            "playbook_alignment": (
                "Uses a free, production-visible SEC feature layer and tests a "
                "filing-timing candidate-pool source instead of LLM soft-ranking, Companyfacts "
                "peer retunes, FTD/FINRA retunes, post-earnings support stack "
                "retunes, or broad OHLCV-only pattern mining."
            ),
            "nearby_prior_experiments": {
                "exp-20260605-018": "Rejected operational 8-K quiet absorption; this changes timing, removes quiet-volume gating, and keeps delayed next-open entry.",
                "exp-20260605-006": "Rejected SEC business-development source-span direct issuer events.",
                "exp-20260604-014": "Rejected SEC same-sector peer propagation.",
                "exp-20260531-012": "Rejected same-sector OHLCV peer shock candidate pool.",
                "exp-20260602-020": "Rejected moderate same-sector OHLCV peer shock candidate pool.",
            },
        ...
        "parameters": {
            ...
            "min_accepted_minutes_after_midnight": MIN_ACCEPTED_MINUTES_AFTER_MIDNIGHT,
            "min_price": MIN_PRICE,
        ...
        "next_action": (
            "If positive, build a shared default-off SEC filing-timing adapter "
            "with after-hours timing, delayed-entry semantics, and parity tests before promotion."
            if gate4["passed"]
            else "Do not retune nearby after-hours 8-K timing, operational item, or delayed-entry thresholds on this frozen sample; pivot to a different free-data candidate-pool mechanism or forward replacement rows."
        ),
    }

"""exp-20260707-006: daily-equity replay for the rank-7 portfolio-lane candidate.

This consumes the observed-only ranking from exp-20260706-022. It does not
retune the industry breadth-repair second-line source from exp-20260607-010
and does not change live, paper, ranking, sizing, entry, or exit behavior. The
only tested decision hypothesis is whether this ranked rejected source still
...
EXPERIMENT_ID = "exp-20260707-006"
OWNER = "alpha-explore"
LANE = "alpha_search"
STEM = "industry_breadth_repair_daily_equity_overlay"
STATUS_POSITIVE = "observed_only_positive_industry_breadth_repair_daily_equity_lead_not_activation_ready"
STATUS_REJECTED = "observed_only_rejected_industry_breadth_repair_daily_equity_overlay"
DECISION_POSITIVE = "observed_only_positive_industry_breadth_repair_daily_equity_overlay"
DECISION_REJECTED = "observed_only_rejected_industry_breadth_repair_daily_equity_overlay"

TRIAL_FAMILY = "portfolio_covariance_daily_equity_overlay"
TRIAL_VARIANT_ID = "industry_breadth_repair_rank7_10pct_daily_equity_overlay_v1"
CHANGED_VARIABLE = "industry_breadth_repair_rank7_portfolio_daily_equity_overlay_v1"
...
NEW_EVIDENCE_AXIS = (
    ...
    "overlay gate: consume exp-20260706-022 rank-7 industry breadth-repair "
    "second-line rows from exp-20260607-010. Prior daily-equity overlays "
    "consumed fixed-asset-turnover, sector-breadth, deferred-revenue, FINRA "
    "short-pressure, purchase-obligation, and receivables DSO; this does not "
    "retune overlay weight, thresholds, top-N, hold days, cooldown, notional, "
    "or correlation cutoffs."
)

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260707_006_{STEM}.json"
...
TARGET_SOURCE_ARTIFACT = (
    "data/experiments/exp-20260607-010/"
    "exp_20260607_010_industry_breadth_repair_second_line.json"
)
TARGET_SOURCE_EXPERIMENT_ID = "exp-20260607-010"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "old_thin_breadth_chop",
    ],
    "confidence_reason": (
        "exp-20260706-022 ranked industry breadth-repair second-line seventh "
        "with positive exit-cashflow portfolio proxy and broad 214-trade "
        "sample, but the source previously failed old_thin and recent "
        "daily-MTM overlay attempts have often turned proxy edge into window "
        "regression; this consumes a new ranked family without retuning "
        "thresholds or overlay weight."
    ),
    "recorded_at": "2026-07-07T05:06:53+00:00",
}

HYPOTHESIS = (
    "Portfolio lane: the exp-20260706-022 rank-7 industry breadth-repair "
    "second-line rejected source may add value as a fixed 10 percent paper "
    "overlay when replayed with true daily mark-to-market equity, without "
    "retuning the OHLCV relation source or overlay weight."
)
ALPHA_HYPOTHESIS = (
...
RUNNER = f"quant/experiments/exp_20260707_006_{STEM}.py"
RUNNER_COMMAND = f".\\.venv\\Scripts\\python.exe -B {RUNNER}"
RUNNER_WINDOWS = f"quant\\experiments\\exp_20260707_006_{STEM}.py"

CHANGED_FILES = [
    RUNNER,
    f"data/experiments/{EXPERIMENT_ID}/exp_20260707_006_{STEM}.json",
...
def source_ranking_audit() -> dict[str, Any]:
    ...
    ranking = payload.get("candidate_ranking") if isinstance(payload, dict) else None
    target_candidate = None
    if isinstance(ranking, list):
        for row in ranking:
            if not isinstance(row, dict):
                continue
            if row.get("experiment_id") != TARGET_SOURCE_EXPERIMENT_ID:
                continue
            target_candidate = {
                ...
                "path": row.get("path"),
                "target_windows": row.get("target_windows"),
                "usable_trade_count": row.get("usable_trade_count"),
                "core_correlation_exit_cashflow": row.get("core_correlation_exit_cashflow"),
                "portfolio_delta_proxy": row.get("portfolio_delta_proxy"),
            }
            break
    return {
    ...
    payload = {
        ...
        "post_run_reflection": {
            ...
            "forbidden_near_neighbor_retry": (
                "Do not rerun industry breadth-repair portfolio overlays by "
                "changing overlay weight, breadth/participation thresholds, "
                "industry grouping, RS/close/volume gates, top-N, hold days, "
                "cooldown, correlation cutoffs, or source filters. A legal "
                "retry needs materially new ranked families, fresh closed "
                "forward replacement-value rows for this unchanged source, "
            ...
            "new_evidence_required": (
                ...
                "industry breadth-repair source."
            ),
        },
        "rejection_reason": (
            None
        ...
        "next_retry_requires": [
            ...
            "materially more closed industry breadth-repair forward replacement-value rows",
            "no overlay-weight, breadth-threshold, top-N, hold-day, source-filter, or correlation retune",
        ],

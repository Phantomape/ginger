"""exp-20260617-019: SEC filing-timeliness (early-disclosure) candidate scout.

Replay-only alpha search. The single decision hypothesis is a PIT free SEC
EDGAR/Companyfacts event source: liquid-universe companies that file their
latest annual report (10-K) ABNORMALLY EARLY versus their own trailing
filing-lag norm (filed sooner after fiscal-year end than the company usually
does) may exhibit positive post-filing drift, because faster disclosure proxies
clean books / management confidence (the Griffin 2003 / filing-delay anomaly,
inverted) and the market underreacts to the timeliness signal itself.
...
Deliberately NOT a momentum-confirmed Companyfacts quality pool: only a light
liquidity gate (price, dollar-volume) is applied; no SPY-relative leadership
filter is required. This isolates whether disclosure timeliness itself carries
drift, and tests whether dropping the price-confirmation gate that made recent
quality pools fail old_thin produces a chop-robust event edge.
...
STEM = "filing_timeliness_early_disclosure_scout"
TRIAL_FAMILY = "free_sec_companyfacts_filing_timeliness_candidate_pool"
TRIAL_VARIANT_ID = "annual_10k_early_filing_vs_own_norm_top1_next_open_10d_v1"
CHANGED_VARIABLE = "sec_filing_timeliness_early_disclosure_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_019_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "drawdown_drift",
        "timeliness_signal_too_weak_without_price_confirmation",
        "fails_accepted_comparator",
    ],
    "confidence_reason": (
        "Mechanism: the SEC filing-delay/timeliness anomaly (Griffin 2003; late "
        "filers underperform) is a documented, production-visible (EDGAR filed "
        "date vs fiscal-year end), PIT-safe event distinct from the accepted "
        "filing-RECENCY freshness support (exp-20260528-016) and from the frozen "
        "Companyfacts relief/overhang ratio family. Disconfirmer: recent quality "
        "pools (SBC exp-20260616-015, CapEx exp-20260617-007, accruals "
        "exp-20260614-020) were aggregate-positive but failed old_thin chop and "
        "drawdown; dropping the price-confirmation gate could either expose a "
        "chop-robust event edge or just add noise, and early-filing events may be "
        "too thin/seasonal to clear the sample and concentration gates."
    ),
    "recorded_at": "2026-06-17T16:20:04Z",
}
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: liquid-universe companies whose latest annual 10-K is "
        "filed abnormally early versus their own trailing filing-lag norm may "
        "drift up over the next 10 trading days, because prompt disclosure "
        "proxies clean books / management confidence and the market underreacts "
        "to the timeliness signal. Tested with a light liquidity gate only, no "
        "SPY-relative momentum confirmation."
    ),
    "2_history_check": {
        "exp-20260528-016": (
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_019_filing_timeliness_early_disclosure_scout.py"
    ),
}
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_filing_timeliness_early_disclosure"
        if gate["passed"]
        else "rejected_filing_timeliness_early_disclosure_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The SEC filing-timeliness early-disclosure event source cleared the "
            "numeric three-window replay screen with only a light liquidity gate, "

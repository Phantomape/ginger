"""exp-20260428-017 adverse gap-down entry-cancel screen.

Hypothesis:
    Current execution cancels entries that gap too far above signal entry and
    entries already below stop, but still admits weak next-open fills between
    entry and stop. The loss taxonomy shows repeated low-MFE stopouts with no
    winner collateral, so a small adverse-gap cancel may improve entry quality
    without changing signal generation, exits, sizing, add-ons, scarce-slot
    routing, LLM/news replay, or earnings.
...
OUT_JSON = OUT_DIR / "exp_20260428_017_adverse_gap_down_entry_cancel.json"
...
VARIANTS = OrderedDict([
    ("baseline_no_adverse_gap_cancel", None),
    ("adverse_gap_down_cancel_1pct", 0.01),
    ("adverse_gap_down_cancel_2pct", 0.02),
    ("adverse_gap_down_cancel_3pct", 0.03),
])
...
def _run_window(universe: list[str], cfg: dict, adverse_gap_pct: float | None) -> dict:
    engine = BacktestEngine(
        ...
        config={
            ...
            "ADVERSE_GAP_CANCEL_PCT": adverse_gap_pct,
        },
        replay_llm=False,
        replay_news=False,
...
def run_experiment() -> dict:
    ...
    for label, cfg in WINDOWS.items():
        for variant, adverse_gap_pct in VARIANTS.items():
            result = _run_window(universe, cfg, adverse_gap_pct)
            rows.append({
                "window": label,
                "start": cfg["start"],
                "end": cfg["end"],
                ...
                "variant": variant,
                "adverse_gap_down_cancel_pct": adverse_gap_pct,
                **result,
            })
            m = result["metrics"]
            print(
    ...
    for variant in list(VARIANTS.keys())[1:]:
        ...
        for label in WINDOWS:
            baseline = next(
                r for r in rows
                if r["window"] == label and r["variant"] == "baseline_no_adverse_gap_cancel"
            )
            candidate = next(
                r for r in rows
    ...
    return {
        ...
        "lane": "alpha_search",
        "change_type": "entry_execution_cancel_screen",
        "single_causal_variable": "adverse next-open gap-down entry cancel threshold",
        "hypothesis": (
            "Canceling entries whose next open is modestly below signal entry may "
            "remove low-MFE stopouts without touching signal generation or exits."
        ),
        "history_guardrails": {
        ...
        "parameters": {
            "baseline": "existing high-gap cancel plus stop-breach cancel",
            "tested_adverse_gap_down_cancel_pct": [0.01, 0.02, 0.03],
            "locked_variables": [
                "signal generation",
                "risk sizing",
                "exits",

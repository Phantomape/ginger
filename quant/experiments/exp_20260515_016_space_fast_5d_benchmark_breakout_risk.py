...
Tests one default-off Space allocation variable on top of the accepted
exp-20260514-053 stack: official Space breakout signals whose mature 5d event
state is positive versus cash, same-theme replacement, SPY, QQQ, UFO, and ARKX.

This keeps the candidate pool, entries, exits, ranking, LLM authority, and live
Space slots fixed. No JavaScript is used.
"""
...
EXPERIMENT_ID = "exp-20260515-016"
STEM = "space_fast_5d_benchmark_breakout_risk"
...
MARKER = "space_fast_5d_benchmark_breakout_risk"

ACCEPTED_IWM_LEADER_SCALAR = 1.0125
SCALARS = (1.0, 0.0, 0.25, 0.5, 0.75, 0.9, 1.0125, 1.025, 1.05, 1.075)
...
def _fast_5d_benchmark_gate() -> dict[str, Any]:
    ...
    for row in _latest_event_rows(LEDGER_PATH):
        ...
        if existing is None:
            profiles[ticker] = {
                "passed": True,
                "positive_5d_event_count": 1,
                "events": [profile],
            }
        else:
            existing["positive_5d_event_count"] += 1
            existing["events"].append(profile)

    target_tickers = sorted(profiles)
    return {
        "passed": bool(target_tickers),
        "source_gate": "space_fast_5d_benchmark_profile",
        "path": str(LEDGER_PATH.relative_to(ROOT)),
        "target_definition": (
            "official Space ticker with mature 5d event-state row positive versus "
            "cash, same-theme replacement, SPY, QQQ, UFO, and ARKX"
        ),
        "target_tickers": target_tickers,
        "target_profile_row_count": sum(
            int(profile.get("positive_5d_event_count") or 0)
...
def _run_exp053_stack_variant(
    ...
    *,
    ...
    target_tickers = set(gates["fast_5d_benchmark_gate"]["target_tickers"])
    target_profiles = gates["fast_5d_benchmark_gate"]["profiles"]
    adjustments: list[dict[str, Any]] = []
    ...
    def wrapped_size_signals(
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and strategy == TARGET_STRATEGY and sizing:
                _scale_and_record(
                    signal=signal,
                    sizing=sizing,
                    scalar=fast_5d_breakout_scalar,
                    portfolio_value=portfolio_value,
                    counts=counts,
                    counts_by_window=counts_by_window,
                    window_label=current_window["label"],
                    adjustments=adjustments,
                    profile=target_profiles.get(ticker),
                )
                signal = {
                    **signal,
                    "sizing": sizing,
                    "space_fast_5d_benchmark_breakout_bucket": True,
                    "space_fast_5d_benchmark_breakout_scalar": (
                        fast_5d_breakout_scalar
                    ),
                    "space_fast_5d_benchmark_profile": target_profiles.get(ticker),
                }
            out.append(signal)
        return out
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_fast_5d_benchmark_breakout_scalar": fast_5d_breakout_scalar,
        "target_strategy": TARGET_STRATEGY,
        "fast_5d_benchmark_target_tickers": sorted(target_tickers),
    }
    variant["fast_5d_benchmark_breakout_counts"] = dict(sorted(counts.items()))
    variant["fast_5d_benchmark_breakout_counts_by_window"] = (
        counts_by_window_serialized
    )
    variant["fast_5d_benchmark_breakout_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(adjustments)
    )
    variant["fast_5d_benchmark_breakout_adjustment_sample"] = adjustments[:25]
    for window_name, row in variant["by_window"].items():
        row["fast_5d_benchmark_breakout_counts"] = (
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("fast_5d_benchmark_breakout_counts") or {}
    changed_count = int(counts.get(f"{MARKER}_changed_signal", 0))
    eligible_count = int(counts.get(f"{MARKER}_eligible_signal", 0))
    scalar = float(variant["parameters"]["space_fast_5d_benchmark_breakout_scalar"])
    passed = bool(
        scalar != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "eligible_fast_5d_breakout_signal_count": eligible_count,
        "changed_fast_5d_breakout_signal_count": changed_count,
        "reasons": {
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Official Space breakout signals with mature all-positive 5d event "
            "state may need different default-off allocation than broad 10d "
            "benchmark-breadth breakouts."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_fast_5d_benchmark_breakout_scalar",
        "parameters": {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: resize only official Space breakout signals "
                "with mature all-positive 5d benchmark confirmation."
            ),
            "2_history_check": {
                "exp-20260515-012": (
                    "Rejected fast-5d benchmark trend risk because it had zero "
                    "runtime changed signals; this run tests breakout signals."
                ),
                "exp-20260515-004/005": (
                    "Rejected broad 10d benchmark-breadth breakout variants due "
                    "old_thin regression; this uses the narrower 5d all-positive "
                    "event-state field."
                ),
                "exp-20260515-013/015": (
                    "Rejected satcom candidate-pool expansion; this keeps the "
                    "official Space candidate set fixed."
                ),
            },
            "3_single_causal_variable": (
                "space_fast_5d_benchmark_breakout_scalar. Candidate pool, "
                "accepted Space stack, entries, exits, ranking, stops, add-ons, "
                "LLM, and live slots stay fixed."
            ),
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260515_016_space_fast_5d_benchmark_breakout_risk.py"
            ),
        },
        "decision": decision,
        ...
        if promoted
            ...
            "Gate 4 failed: no tested fast-5d benchmark breakout scalar improved "
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited, broad satcom/ticker expansion "
            "failed, and recent benchmark-breadth trend/IWM/peer interactions are "
            "accepted, rejected, or sample-limited. This keeps the candidate set "
            "fixed and tests one production-visible 5d catalyst-quality field."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space fast-5d benchmark breakout risk",
        "",
        "## Hypothesis",
        (
            "Official Space `breakout_long` signals with mature all-positive 5d "
            "confirmation versus cash, same-theme replacement, SPY, QQQ, UFO, "
            "and ARKX may be mis-sized by the accepted stack."
        ),
        "",
        "## Single Changed Variable",
        (
            "`space_fast_5d_benchmark_breakout_scalar` on top of the accepted "
        ...
        f"- target tickers: `{best['parameters']['fast_5d_benchmark_target_tickers']}`",
    ...
    for label, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["fast_5d_benchmark_breakout_counts_by_window"].get(
            label,
            {},
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_fast_5d_benchmark_breakout_scalar']}`",
            f"- eligible signals: `{gate['eligible_fast_5d_breakout_signal_count']}`",
            f"- adjusted signals: `{gate['changed_fast_5d_breakout_signal_count']}`",
            f"- adjusted counts: `{best['fast_5d_benchmark_breakout_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            f"Fast-5d benchmark breakout scalar "
            f"{best['parameters']['space_fast_5d_benchmark_breakout_scalar']} "
            f"changed {gate['changed_fast_5d_breakout_signal_count']} signals "
...
def run() -> dict[str, Any]:
    ...
    variants = [
        _run_exp053_stack_variant(
            ...
            fast_5d_breakout_scalar=scalar,
            gates=gates,
        )
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            "Fast-5d all-benchmark-positive Space breakout signals are "
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate["eligible_fast_5d_breakout_signal_count"],
                    "adjusted_signals": gate["changed_fast_5d_breakout_signal_count"],

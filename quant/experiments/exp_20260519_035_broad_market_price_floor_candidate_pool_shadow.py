...
test one default-off paper candidate-pool variable: after the rejected
exp-20260519-034 broad-market leadership scout, require a minimum
decision-date close before non-current-tradeable all-window liquid leadership
candidates can receive paper capital.
...
EXPERIMENT_ID = "exp-20260519-035"
EXPERIMENT_SLUG = "broad_market_price_floor_candidate_pool_shadow"
BASELINE_EXPERIMENT_ID = "exp-20260519-033"
WAREHOUSE_EXPERIMENT_ID = "exp-20260519-030"
PARENT_EXPERIMENT_ID = "exp-20260519-034"
RULE_VERSION = "broad_market_price_floor_candidate_pool_shadow_v1"
...
BASE_LEADERSHIP_PROFILE: dict[str, Any] = {
    "ret20_excess_spy_min": 0.035,
    "ret60_min": 0.08,
    "near_high_60_min": 0.93,
    "volume_ratio_20_min": 1.00,
    "description": "exp-20260519-034 leadership_broad parent profile",
}

PRICE_FLOOR_PROFILES: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            "price_floor_30",
            {
                **BASE_LEADERSHIP_PROFILE,
                "decision_close_price_min": 30.0,
                "description": "broad leadership with decision-date close >= $30",
                "aggression_order": 1,
            },
        ),
        (
            ...
            {
                **BASE_LEADERSHIP_PROFILE,
                "decision_close_price_min": 35.0,
                "description": "broad leadership with decision-date close >= $35",
                "aggression_order": 2,
            },
        ),
        (
            "price_floor_40",
            {
                **BASE_LEADERSHIP_PROFILE,
                ...
                "description": "broad leadership with decision-date close >= $40",
                "aggression_order": 3,
            },
        ),
        (
            ...
            {
                **BASE_LEADERSHIP_PROFILE,
                "decision_close_price_min": 45.0,
                "description": "broad leadership with decision-date close >= $45",
                "aggression_order": 4,
            },
        ),
        (
            ...
            {
                **BASE_LEADERSHIP_PROFILE,
                "decision_close_price_min": 50.0,
                "description": "broad leadership with decision-date close >= $50",
                "aggression_order": 5,
            },
        ),
    ]
...
def _qualifies(feature: dict[str, Any], profile: dict[str, Any]) -> bool:
    return bool(
        ...
        and float(feature["close"]) >= float(profile["decision_close_price_min"])
    )
...
def _make_trade(
    *,
    ...
    return {
        ...
        "decision_close_price_min": profile_name.replace("price_floor_", ""),
        "score": feature["score"],
    }
        ...
        rows.append(
            {
                ...
                "decision_close": trade.get("decision_close"),
            }
        )
    return rows
...
def _variant_payload(
    *,
    ...
    return {
        ...
        "variant_type": "broad_market_price_floor_candidate_pool_profile",
        "profile": profile,
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Price-Floor Candidate-Pool Shadow",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for name, profile in PRICE_FLOOR_PROFILES.items()
    ]
    selected = _choose_best(variants)
    accepted = [row for row in variants if row["gate4"]["passed"]]
    decision = (
        "observed_promising_default_off_broad_market_price_floor_candidate_pool"
        if accepted
        else "rejected_broad_market_price_floor_candidate_pool_shadow"
    )
    status = "observed_only" if accepted else "rejected"
    interpretation = (
        "A decision-date price floor improved the broad-market leadership paper pool across the canonical three-window replay, but it remains default-off paper only until a shared production-visible paper adapter is added."
        if accepted
        else "No broad-market price-floor profile cleared the canonical three-window replacement-value gate; do not promote this candidate-pool profile without new evidence."
    )
    ...
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "The rejected broad-market leadership paper pool may become usable "
            "when candidates are restricted to higher-priced, institutionally "
            "tradable equities at the decision close, because low-priced "
            "warehouse winners/lossers introduced drawdown and late-window EV "
            "instability."
        ),
        "alpha_hypothesis": {
            ...
            "playbook_alignment": (
                ...
                "discovery direction while adding one production-visible "
                "quality field instead of retrying state-surface scalars or "
                "LLM soft-ranking."
            ),
            "why_now": (
                "exp-20260519-034 found broad-market leadership had aggregate "
                "replacement value but failed Gate 4 on one EV-regressed window; "
                "price floor is a simple decision-time quality discriminator."
            ),
        },
        "history_check": {
            "nearby_experiments": [
                ...
                },
                {
                    ...
                    "lesson": "broad-market leadership produced positive aggregate PnL but regressed late_strong EV and failed Gate 4",
                },
                {
            ...
            "anti_repeat": (
                "This keeps the exp-20260519-034 leadership profile fixed and "
                "only sweeps the new decision-close price floor."
            ),
        },
        "change_type": "default_off_paper_candidate_pool",
        "changed_variable": "broad_market_decision_close_price_floor",
        "single_causal_variable": (
            "decision-date close price floor applied to the fixed broad-market "
            "leadership_broad parent candidate-pool profile"
        ),
        "parameters": {
            "warehouse_experiment_id": WAREHOUSE_EXPERIMENT_ID,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
            "parent_experiment_id": PARENT_EXPERIMENT_ID,
            "parent_profile": BASE_LEADERSHIP_PROFILE,
            "profiles": PRICE_FLOOR_PROFILES,
            "notional": PAPER_NOTIONAL,
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "entry/candidate_pool: broad-market leadership candidates need an institutional price floor to avoid noisy low-priced warehouse exposure.",
            "2_past_similar_experiments": "exp-20260519-034 tested the same leadership parent profile and failed Gate 4; exp-20260519-030 provides the warehouse; exp-20260519-033 is latest accepted baseline.",
            "3_single_variable": "Only decision_close_price_min changes; parent leadership thresholds, hold, notional, slots, and universe controls stay fixed.",
        ...
        if accepted
        else "Best broad-market price-floor profile failed one or more Gate 4 replacement-value constraints.",
        "next_evidence_needed": (
            "Add a shared default-off paper adapter and forward-paper audit before any production promotion; monitor concentration, tail loss, and warehouse coverage drift."
            if accepted
        ...
        "why_not_other_changes": [
            ...
            "This run keeps exp-20260519-034 leadership thresholds fixed and changes only the price floor.",
        ],
        "known_risks": [
            ...
            "A price floor is a simple tradability-quality field; promotion needs forward paper evidence and production-visible warehouse maintenance.",
        ],

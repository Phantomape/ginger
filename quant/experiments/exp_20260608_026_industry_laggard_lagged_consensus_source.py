"""exp-20260608-026: industry laggard-repair as lagged consensus source.

Replay-only alpha search. Tests one causal variable: add the accepted
industry-relative laggard-repair paper rows as a new independent source family
...
import industry_relative_laggard_repair_paper_sleeve as industry_laggard  # noqa: E402


EXPERIMENT_ID = "exp-20260608-026"
STEM = "industry_laggard_lagged_consensus_source"
TRIAL_FAMILY = "accepted_free_data_cross_source_consensus_new_independent_source_family"
TRIAL_VARIANT_ID = "industry_relative_laggard_repair_lagged_consensus_source_family_v1"
CHANGED_VARIABLE = "industry_relative_laggard_repair_source_family_added_to_accepted_lagged_consensus_v1"
RULE_VERSION = "industry_relative_laggard_repair_lagged_consensus_source_family_v1"

INDUSTRY_SOURCE_NAME = industry_laggard.SLEEVE_NAME
INDUSTRY_SOURCE_FAMILY = "industry_relative_laggard_repair"
INDUSTRY_SOURCE_EXPERIMENT_ID = "exp-20260607-008"
INDUSTRY_REPLAY_LEAD_ID = "exp-20260607-007"
INDUSTRY_SHARED_ARTIFACT = (
    REPO_ROOT
    / "data"
    / "experiments"
    / INDUSTRY_SOURCE_EXPERIMENT_ID
    / "exp_20260607_008_industry_relative_laggard_repair_shared_adapter.json"
)
INDUSTRY_REPLAY_LEAD_ARTIFACT = (
    REPO_ROOT
    / "data"
    / "experiments"
    / INDUSTRY_REPLAY_LEAD_ID
...
OUT_JSON = OUT_DIR / f"exp_20260608_026_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "industry_laggard_redundant_with_existing_sources",
        "window_regression",
        "source_rows_selected_only_as_prior_confirmation",
        "concentration_failed",
    ],
    "confidence_reason": (
        "The industry laggard-repair helper cleared Gate 4 as a standalone "
        "default-off paper source, but previous source-family additions often "
        "fail once measured against the current accepted lagged comparator."
    ),
    "recorded_at": "2026-06-08T22:18:00+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "industry-relative laggard-repair paper snapshot as the same source "
        "family in both replay and daily production, with parity tests, before "
        "any daily report queue, candidate priority, paper notional, watchlist, "
...
def _patch_source_family_context() -> None:
    ...
    same_day.SOURCE_FAMILIES[INDUSTRY_SOURCE_NAME] = INDUSTRY_SOURCE_FAMILY
    same_day.SOURCE_EXPERIMENT_IDS[INDUSTRY_SOURCE_NAME] = INDUSTRY_SOURCE_EXPERIMENT_ID


def _source_row_from_industry_trade(
    trade: dict[str, Any],
    *,
    label: str,
) -> dict[str, Any] | None:
    ...
    industry_context = trade.get("industry_context") if isinstance(trade.get("industry_context"), dict) else {}
    return {
        "source_name": INDUSTRY_SOURCE_NAME,
        "source_experiment_id": INDUSTRY_SOURCE_EXPERIMENT_ID,
        "source_family": INDUSTRY_SOURCE_FAMILY,
        "date": signal_date,
        ...
        "candidate_group_key": trade.get("candidate_group_key"),
        "industry": trade.get("industry"),
        "sector": trade.get("sector"),
        "candidate_industry_lag_20d": trade.get("candidate_industry_lag_20d"),
        "candidate_repair_vs_group_5d": trade.get("candidate_repair_vs_group_5d"),
        "candidate_signal_relative_vs_spy": trade.get("candidate_signal_relative_vs_spy"),
        ...
        "group_median_ret20_excess_spy": industry_context.get("median_ret20_excess_spy"),
        "group_median_ret5_excess_spy": industry_context.get("median_ret5_excess_spy"),
        "group_ret20_positive_fraction": industry_context.get("ret20_positive_fraction"),
        "group_liquid_count": industry_context.get("liquid_group_count"),
        ...
        "helper_rule_version": industry_laggard.RULE_VERSION,
        "helper_source_rule_version": industry_laggard.SOURCE_RULE_VERSION,
        "source_artifact": _repo_rel(INDUSTRY_SHARED_ARTIFACT),
        "historical_replay_experiment_id": INDUSTRY_REPLAY_LEAD_ID,
        "historical_replay_artifact": _repo_rel(INDUSTRY_REPLAY_LEAD_ARTIFACT),
...
def _industry_source_rows_by_window() -> tuple[
    dict[str, dict[tuple[str, str], list[dict[str, Any]]]],
    dict[str, Any],
]:
    payload = _load_json(INDUSTRY_SHARED_ARTIFACT)
    ...
    diagnostics: dict[str, Any] = {
        "source_path": _repo_rel(INDUSTRY_SHARED_ARTIFACT),
        "source_name": INDUSTRY_SOURCE_NAME,
        "source_family": INDUSTRY_SOURCE_FAMILY,
        "source_experiment_id": INDUSTRY_SOURCE_EXPERIMENT_ID,
        "historical_replay_experiment_id": INDUSTRY_REPLAY_LEAD_ID,
        ...
        "legacy_artifact_source_values": {},
    }
    for label in same_day.prior.base.WINDOWS:
        trades = [row for row in rows_by_window.get(label, []) if isinstance(row, dict)]
        tickers: set[str] = set()
        legacy_sources = Counter(str(row.get("source") or "") for row in trades)
        for trade in trades:
            source_row = _source_row_from_industry_trade(trade, label=label)
        ...
        diagnostics["legacy_artifact_source_values"][label] = dict(sorted(legacy_sources.items()))
...
def _source_addition_summary(
    ...
    selected_with_industry = [
        row for row in all_selected if INDUSTRY_SOURCE_NAME in (row.get("source_names") or [])
    ]
    current_industry = [
        row
        for row in selected_with_industry
        if INDUSTRY_SOURCE_NAME in (row.get("current_source_names") or [])
    ]
    prior_industry = [
        row
        for row in selected_with_industry
        if any(
            source_row.get("source_name") == INDUSTRY_SOURCE_NAME
    ...
    return {
        "added_source_name": INDUSTRY_SOURCE_NAME,
        "added_source_family": INDUSTRY_SOURCE_FAMILY,
        "added_source_rows_by_window": dict(sorted(added_key_counts.items())),
        "selected_trade_count": len(all_selected),
        "selected_with_industry_source_count": len(selected_with_industry),
        "selected_with_current_industry_count": len(current_industry),
        "selected_with_prior_industry_count": len(prior_industry),
        "selected_with_industry_pnl_usd": round(
            sum(_safe_float(row.get("pnl")) for row in selected_with_industry),
            2,
        ),
...
def _gate4(
    ...
    source_selected = int(source_summary["selected_with_industry_source_count"]) > 0
    gates = {
        **base_gate["gates"],
    ...
    if passed:
        decision = "positive_replay_lead_requires_industry_laggard_lagged_consensus_shared_adapter"
        rationale = (
            "Adding industry-relative laggard-repair as an independent source "
        ...
        decision = "rejected_industry_laggard_lagged_consensus_no_selected_source_rows"
        rationale = "The industry laggard-repair source produced no selected lagged-consensus trades."
        ...
        decision = "rejected_industry_laggard_lagged_consensus_gate4_failed"
...
def _preflight_payload() -> dict[str, Any]:
    return {
        "alpha_hypothesis": (
            "Accepted industry-relative laggard-repair paper rows may improve "
            "the accepted lagged free-data consensus scout when treated as a "
            "new independent production-visible source family."
        ),
        ...
        "playbook_alignment": (
            ...
            "soft-ranking data, SEC phrase retunes, state-surface allocation "
        ...
        "prior_difference": (
            "exp-20260607-007/008 accepted industry laggard-repair as a "
            "standalone shared default-off paper adapter. exp-20260606-028 "
            "rejected rolling-correlation peer-shock as a source-family addition "
            "because it did not beat the accepted lagged comparator. This run "
            "tests a different accepted relation alpha as an independent source, "
            "not an industry threshold, hold, cooldown, top-N, or notional retune."
        ),
        ...
        "reproducibility": (
            ...
            "quant\\experiments\\exp_20260608_026_industry_laggard_lagged_consensus_source.py"
        ),
    }
...
def _experiment_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "change_summary": (
            "Added accepted industry-relative laggard-repair paper rows as a "
        ...
        "new_evidence_type": "accepted_production_visible_industry_relative_laggard_repair_source_family",
        ...
        "negative_reflection": (
            "If rejected, the industry laggard-repair source is likely a good "
            "standalone relation alpha but not a replacement-value source for "
            "the accepted lagged consensus stack. Its rows may cluster on dates "
            "where existing accepted source families already choose the same "
            "top-1 paper trade, so adding confirmation does not improve all "
            "windows. Do not retry with local industry lag, hold, cooldown, "
            "top-N, or notional sweeps; the next retry needs forward "
            "replacement-value evidence or a materially different source "
            "selection surface."
        ),
        "next_retry_requires": [
        ...
        "related_files": [
            ...
            _repo_rel(INDUSTRY_SHARED_ARTIFACT),
...
def _write_card(payload: dict[str, Any]) -> None:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Industry Laggard Lagged Consensus Source",
        "",
        "## Decision",
        "",
        ...
        f"- Selected trades with industry source: `{source['selected_with_industry_source_count']}`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Source Diagnostics",
            "",
            f"- Industry source rows by window: `{source['added_source_rows_by_window']}`",
            f"- Current industry confirmations selected: `{source['selected_with_current_industry_count']}`",
            f"- Prior industry confirmations selected: `{source['selected_with_prior_industry_count']}`",
            "",
            "## Production Boundary",
            "",
...
def main() -> None:
    ...
    industry_rows, industry_diagnostics = _industry_source_rows_by_window()
    extended_source_rows = _merge_source_rows(base_source_rows, industry_rows)
    ...
    source_summary = _source_addition_summary(target_trades_by_window, industry_rows)
    ...
    payload = {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "entry/candidate_pool: accepted industry-relative laggard-repair "
                "paper rows may improve lagged accepted-source consensus quality."
            ),
            "2_history_check": {
                "exp-20260607-007/008": (
                    "Industry laggard-repair accepted as a standalone shared "
                    "default-off paper adapter with positive three-window evidence."
                ),
                "exp-20260606-028": (
                    "Rolling-correlation peer-shock source-family addition failed "
                ...
                "exp-20260605-008": (
                    "SEC negative-reaction source-family addition improved only "
                    "2/3 windows versus the accepted lagged comparator and was rejected."
                ),
            },
        ...
        "source_files": {
            ...
            INDUSTRY_SOURCE_NAME: _repo_rel(INDUSTRY_SHARED_ARTIFACT),
        },
        "rule": {
            "rule_version": RULE_VERSION,
            "added_source_name": INDUSTRY_SOURCE_NAME,
        ...
        "industry_source_diagnostics": industry_diagnostics,

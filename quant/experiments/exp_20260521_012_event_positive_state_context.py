"""exp-20260521-012: event positive-state context scout.

Alpha search. Tests and promotes one production-visible event market-state
context field on top of the accepted exp-20260521-009 default-off event overlay
adapter: whether selected rows with positive state-surface scores deserve a
different paper-notional scalar.
...
import exp_20260521_011_event_mixed_breadth_context as exp011


EXPERIMENT_ID = "exp-20260521-012"
EXPERIMENT_SLUG = "event_positive_state_context"

REPO_ROOT = exp011.REPO_ROOT
...
VARIANTS: "OrderedDict[str, dict[str, Any]]" = OrderedDict(
    [
        ...
        (
            "positive_state_context_075",
            {
                "description": "0.75x paper notional for positive state-score event rows.",
                "positive_state_context_scalar": 0.75,
            },
        ),
        (
            "positive_state_context_090",
            {
                "description": "0.90x paper notional for positive state-score event rows.",
                "positive_state_context_scalar": 0.90,
            },
        ),
        (
            "positive_state_context_110",
            {
                "description": "1.10x paper notional for positive state-score event rows.",
                "positive_state_context_scalar": 1.10,
            },
        ),
        (
            "positive_state_context_125",
            {
                "description": "1.25x paper notional for positive state-score event rows.",
                "positive_state_context_scalar": 1.25,
            },
        ),
    ]
)
...
def _configure_modules() -> None:
    exp011._configure_modules()


def _is_target_positive_state(trade: dict[str, Any]) -> bool:
    return trade.get("state_score_positive") is True
...
def _scaled_trade(
    ...
    target = _is_target_positive_state(trade)
    context_scalar = (
        float(variant["positive_state_context_scalar"]) if target else 1.0
    )
    ...
    return {
        **trade,
        ...
        "positive_state_context_target": target,
        "positive_state_context_scalar": round(context_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("positive_state_context_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("positive_state_context_target")
        ]
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -abs(VARIANTS[name]["positive_state_context_scalar"] - 1.0),
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_default_off_event_positive_state_context"
        if accepted
        else "rejected_event_positive_state_context"
    )
    ...
    compact_parameters = {
        ...
        "target_field": "state_score_positive",
        "target_value": True,
        "selected_positive_state_context_scalar": VARIANTS[best_variant][
            "positive_state_context_scalar"
        ],
        "variant_scalars": {
            name: row["positive_state_context_scalar"] for name, row in VARIANTS.items()
        },
    ...
    return {
        ...
        "trial_variant_id": "positive_state_score_notional_scalar",
        "changed_variable": "event_positive_state_context_scalar",
        ...
        "new_evidence_type": "new_event_market_state_context_field",
        "hypothesis": (
            "Inside the accepted default-off event overlay, selected event rows "
            "with positive state-surface scores may have better replacement "
            "value than event rows without market-state confirmation. A single "
            "paper-notional scalar tests whether this production-visible state "
            "confirmation improves event allocation without changing core trades."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event context scoring",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Uses event-family x market-state decomposition from the "
                "playbook, while avoiding LLM soft-ranking, source-capacity "
                "retries, broad-market scalar mining, and candidate-pool noise."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for fixed event overlay rows where "
            "state_score_positive is true; event definitions, accepted source/"
            "reaction scalars, hold period, source capacity, and core strategy "
            "stay fixed"
        ),
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Positive event market-state confirmation may identify higher "
                "replacement value; this is event context scoring plus capital "
                "allocation."
            ),
            "2_history_check": (
                "Broad-breadth and negative-reaction event context were accepted; "
                "high dispersion, state-rank top quartile, source capacity, "
                "mixed breadth, and governance semantic-cell scouts were tested "
                "recently. This run uses a different production-visible boolean "
                "market-state confirmation field."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for fixed state_score_positive "
                "event rows changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260521_012_event_positive_state_context.py"
            ),
        },
        ...
        "decision_rationale": (
            ...
            if accepted
            else "Rejected: positive-state context scalar did not clear Gate 4."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "Do not retry nearby event state-score or state-rank scalars on the "
        ...
        "why_not_other_attractive_points": (
            ...
            "mixed-breadth, and governance shareholder-vote retests after recent "
            "Gate 4 failures."
        ),
        "risk_of_change": (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Positive-State Context",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests and promotes whether selected event rows with "
            "positive state-surface scores deserve a different paper-notional "
            "scalar on top of the accepted event adapter."
        ),
        "",
        "## Gate 4 Result",
        "",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event positive-state context scout",

...
interaction.  This tests only covered ``earnings_release_text`` rows with
``neutral_or_mixed_language`` and muted T+1 excess reaction.  It does not
change core entries, exits, queue eligibility, queue capacity, hold days, LLM,
news, or live/default orders.
...
EXPERIMENT_ID = "exp-20260520-010"
STEM = "exp_20260520_010_sec_neutral_earnings_underreaction_notional"
...
scout.DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_neutral_earnings_underreaction_notional.md"
)
...
def _notional_for_position(
    ...
    *,
    ...
    if _is_target_position(position):
        ...
        rule_parts.append("neutral_earnings_underreaction_scalar")
    return (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} SEC Neutral Earnings Underreaction Notional",
        "",
...
def _rename_variant_fields(payload: dict[str, Any]) -> None:
    payload["best_variant"] = payload["best_variant"].replace(
        ...
        "neutral_earnings_underreaction_scalar",
    )
    ...
    for row in payload.get("variant_summaries", {}).values():
        ...
        row["neutral_earnings_underreaction_notional_scalar"] = scalar
    ...
    payload.update(
        {
            ...
            "decision": (
                "promising_sec_neutral_earnings_underreaction_notional"
                if passed
                else "rejected_sec_neutral_earnings_underreaction_notional"
            ),
            "hypothesis": (
                ...
                "and muted T+1 excess reaction may be underreaction candidates. "
            ...
            "change_summary": (
                ...
                "earnings-release rows with t1_excess_return_vs_spy <= 0.02."
            ),
            "change_type": "alpha_search_semantic_reaction_notional_allocation",
            "component": "quant/sec_financial_report_event_sleeve.py",
            "changed_variable": (
                "sec_neutral_earnings_underreaction_notional_scalar"
            ),
            "single_causal_variable": (
                "neutral/mixed earnings-release plus T+1 excess <= 2% "
                "paper-notional scalar"
            ),
            "interpretation": (
                "Neutral/mixed SEC earnings-release rows with muted T+1 excess "
                ...
                if passed
                else "No neutral/mixed earnings-release underreaction scalar "
            ...
            if passed
                ...
                "No neutral/mixed earnings-release underreaction scalar cleared "
            ...
            "next_evidence_needed": (
                ...
                if passed
                else "Do not retry nearby SEC earnings-release/reaction scalars "
                "on the frozen sample without a new semantic field or forward "
                "evidence."
            ),
            "why_not_other_changes": (
                "State-surface scalar/profile mining is under strict anti-repeat; "
                "broad-market direct pool expansion and adjacent support scalars "
                "recently failed; LLM soft-ranking lacks replay-safe attribution; "
                "broad positive/negative language scalars already failed without "
                "a narrower semantic field."
            ),
            "protocol_answers": {
                "1_alpha_hypothesis": (
                    ...
                    "priority for SEC semantic expansion with event and reaction "
                    "context fields."
                ),
                "2_history_check": (
                    "exp-20260518-014 accepted neutral underreaction with market "
                    "context; exp-20260519-008 accepted earnings-release SPY "
                    "context; exp-20260519-032 rejected earnings-release T+1 "
                    "strength; exp-20260520-008 rejected positive-language low "
                    "reaction. This run tests only the neutral/mixed earnings "
                    "underreaction intersection."
                ),

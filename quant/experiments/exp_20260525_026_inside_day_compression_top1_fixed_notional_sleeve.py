...
This alpha search converts the old observed-only inside-day compression
breakout audit into the current three-window fixed-notional paper replay.

The single variable is an additive default-off paper sleeve that admits at most
one candidate per signal day from the inside-day compression breakout source,
enters at the next available open, and exits after ten trading days at the
close.
...
import exp_20260426_057_inside_day_compression_shadow as shadow  # noqa: E402
...
EXPERIMENT_ID = "exp-20260525-026"
STEM = "inside_day_compression_top1_fixed_notional_sleeve"
...
def _candidate_rows_for_window(
    ...
    for ticker in sorted(set(universe).intersection(snapshot)):
        if ticker in shadow.EXCLUDED_TICKERS:
            continue
        candidates.extend(shadow._candidate_rows(snapshot, ticker, dates))
    candidates.sort(
        key=lambda row: (
            row["date"],
            row["inside_to_prior_range_ratio"],
            -row["candidate_day_rs_vs_spy"],
            -row["breakout_above_inside_high_pct"],
            -row["dollar_volume"],
            row["ticker"],
        )
    )
...
def _build_payload() -> dict[str, Any]:
    ...
    decision = (
        "promising_replay_only_inside_day_compression_top1_fixed_notional_sleeve"
        if gate4_passed
        else "rejected_inside_day_compression_top1_fixed_notional_sleeve"
    )
    ...
    return {
        ...
        "decision": decision,
        "hypothesis": (
            "A production-visible inside-day compression breakout source may add "
            "candidate-pool alpha by identifying short-range contraction followed "
            "by a relative-strength close above the compression high. It expands "
            "the candidate pool without adding arbitrary noisy tickers."
        ),
        "change_type": "inside_day_compression_default_off_paper_sleeve",
        ...
        "parameters": {
            ...
            "shadow_entry_filters": {
                "compression_lookback_days": shadow.COMPRESSION_LOOKBACK_DAYS,
                "max_inside_range_to_avg_prior_range": shadow.MAX_INSIDE_RANGE_TO_AVG_RANGE,
                "candidate_close_must_be_above_open": True,
                "candidate_close_must_be_above_inside_day_high": True,
                "candidate_day_rs_vs_spy_min": shadow.MIN_CANDIDATE_RS_VS_SPY,
                "min_candidate_day_dollar_volume": shadow.MIN_DOLLAR_VOLUME,
            },
            "selection_rank": [
                "signal_date",
                "inside_to_prior_range_ratio asc",
                "candidate_day_rs_vs_spy desc",
                "breakout_above_inside_high_pct desc",
                "dollar_volume desc",
                "ticker asc",
            ],
            "locked_variables": [
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "entry / candidate_pool: inside-day compression breakouts may "
                "surface a free-OHLCV continuation candidate source."
            ),
            "2_history_check": {
                "exp-20260426-057": (
                    "Observed-only inside-day compression audit with forward-return "
                    "statistics, but no current three-window next-open fixed-notional "
                    "before/after EV replay."
                ),
                "exp-20260525-020/022": (
                    "Recent volatility-contraction source was tested and then QQQ "
                    "confirmed; this uses a one-bar inside-day compression pattern, "
                    "not ATR compression or QQQ/SPY confirmation."
                ),
                "exp-20260525-011/012": (
                    "Opening-range source variants were recent; this does not use "
                    "prior-day/opening-range continuation."
                ),
                "exp-20260525-916": (
                    "Sector-leadership source was rejected due window regression, "
                    "drawdown drift, and concentration; this is not sector-rank based."
                ),
            },
            ...
            "5_reproducibility": (
                ...
                "exp_20260525_026_inside_day_compression_top1_fixed_notional_sleeve.py"
            ),
        },
        "gate1": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "inside_day high/low within prior bar",
                "inside_to_prior_range_ratio",
        ...
        "why_not_other_changes": (
            "Skipped LLM soft-ranking and expectation-residual because replay-safe "
            "coverage remains sparse. Skipped recent AI/compute/consumer/Space, "
            "broad-market, sector-leadership, opening-range, and volatility-"
            "contraction nearby retreads due fresh anti-repeat evidence. This uses "
            "a distinct free-OHLCV one-bar compression source."
        ),
        "interpretation": (
            "The inside-day compression top-1 paper sleeve cleared Gate 4 as a "
            "replay-only lead, but no production/shared policy was promoted."
            if gate4_passed
            else (
                "The inside-day compression top-1 paper sleeve did not clear Gate "
                "4. Do not promote or retry nearby inside-day compression gates "
                "without a new production-visible discriminator or broader forward sample."
            )
        ),
        ...
        "next_evidence_needed": (
            ...
            "confirmation, market-state confirmation, or forward paper rows; do "
            "not just retune the same inside-range/relative-strength thresholds on "
            "this frozen sample."
        ),
        "related_files": [
...
def _build_report(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Inside-Day Compression Top-1 Fixed-Notional Sleeve",
            "",
...
def _experiment_log_entry(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "change_summary": (
            "Replay-only default-off inside-day compression top-1 fixed-notional "
            "paper sleeve using next-open entry and 10-trading-day exit."
        ),
        "change_type": payload["change_type"],
        "mechanism_family": "inside_day_compression_free_ohlcv_candidate_pool",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Inside-day compression top-1 fixed-notional sleeve",
            "status": payload["status"],
            "decision": payload["decision"],

"""exp-20260522-006: SEC governance no-5.03 disclosure-quality scout.
...
whether governance/procedural event rows without item 5.03 deserve higher
paper notional than governance rows that include charter/securities-change
language.
...
EXPERIMENT_ID = "exp-20260522-006"
EXPERIMENT_SLUG = "event_governance_no_503_disclosure_quality"
    ...
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "governance_no_503_scalar": 1.00,
            },
        ),
        (
            "governance_no_503_110",
            {
                "description": "1.10x paper notional for governance rows without item 5.03.",
                "governance_no_503_scalar": 1.10,
            },
        ),
        (
            "governance_no_503_125",
            {
                "description": "1.25x paper notional for governance rows without item 5.03.",
                "governance_no_503_scalar": 1.25,
            },
        ),
        (
            "governance_no_503_150",
            {
                "description": "1.50x paper notional for governance rows without item 5.03.",
                "governance_no_503_scalar": 1.50,
            },
        ),
        (
            "governance_no_503_200",
            {
                "description": "2.00x paper notional for governance rows without item 5.03.",
                "governance_no_503_scalar": 2.00,
            },
        ),
    ]
)
...
def _is_target_governance_no_503(trade: dict[str, Any]) -> bool:
    return (
...
def _scaled_trade(
    ...
    target = _is_target_governance_no_503(trade)
    quality_scalar = float(variant["governance_no_503_scalar"]) if target else 1.0
    ...
    return {
        **trade,
        ...
        "governance_no_503_target": target,
        "governance_no_503_scalar": round(quality_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("governance_no_503_target")]
    for label, rows in rows_by_window.items():
        window_targets = [row for row in rows if row.get("governance_no_503_target")]
...
def _gate_vs_baseline(
    ...
    sample_ok = (
        ...
        and (
            ...
            or baseline_selection["target_max_single_positive_pnl_share"] <= 0.55
        )
    )
    ...
    return {
        **gate,
        ...
        "sample_guard": {
            ...
            "max_target_single_positive_pnl_share": 0.55,
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -abs(VARIANTS[name]["governance_no_503_scalar"] - 1.0),
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_default_off_event_governance_no_503_disclosure_quality"
        if accepted
        else "rejected_event_governance_no_503_disclosure_quality"
    )
    ...
    compact_parameters = {
        ...
        "selected_governance_no_503_scalar": VARIANTS[best_variant][
            "governance_no_503_scalar"
        ],
        "variant_scalars": {
            name: row["governance_no_503_scalar"] for name, row in VARIANTS.items()
        },
    ...
    return {
        ...
        "trial_variant_id": "governance_no_5_03_notional_boost",
        "changed_variable": "event_governance_no_5_03_disclosure_quality_scalar",
        "prior_trial_count": 9,
        ...
        "new_evidence_type": "specific_item_code_absence_5_03_disclosure_quality_field",
        "hypothesis": (
            "Inside the accepted default-off event overlay, SEC governance/"
            "procedural rows without item 5.03 may be cleaner disclosure-quality "
            "events than charter/securities-change governance rows. A single "
            "paper-notional scalar tests this production-visible field without "
            "changing core trades."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event semantic field",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Follows the playbook's SEC/event disclosure-quality lane and "
                "uses one production-visible item-code field instead of LLM "
                "soft-ranking, broad-market replay mining, or state-surface "
                "notional retunes."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for fixed accepted event rows where source is "
            "sec_governance_procedural and item code 5.03 is absent; all accepted "
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Capital allocation / event semantic field: boost SEC governance "
                "paper rows only when item 5.03 is absent."
            ),
            "2_history_check": (
                "Prior governance semantic-cell and multi-item complexity scouts "
                "were rejected. This tests a specific disclosure-quality field: "
                "absence of item 5.03, not general item-count complexity."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for the fixed no-5.03 governance "
                "cohort changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260522_006_event_governance_no_503_disclosure_quality.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260521-010": (
                "Rejected shareholder-vote negative semantic-cell scalar due "
                "one EV-regressed window and sample guard."
            ),
            "exp-20260521-014": (
            ...
            "exp-20260521-015": (
                "Rejected SEC-negative source scalar due one EV-regressed window "
                "and concentration."
            ),
            "exp-20260522-005": (
        ...
        "gate2": {
            ...
            "passed": bool(
                ...
                and baseline_selection["target_trade_count"] >= 8
            ),
        },
        "gate3": {
        ...
        "decision_rationale": (
            ...
            if accepted
            else "Rejected: no-5.03 governance disclosure scalar did not clear Gate 4."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "If accepted, keep the no-5.03 governance paper scalar in the shared "
            "default-off event bundle and wait for new forward event rows before "
            "nearby disclosure-quality retunes."
        )
        if accepted
        else (
            "Do not retry nearby SEC governance item-code quality scalars on the "
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Governance No-5.03 Disclosure Quality",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests whether SEC governance/procedural event rows "
            "without item 5.03 should receive a paper-notional boost on top of "
            "the accepted event non-narrow context adapter."
        ),
        "",
        "## Gate 4 Result",
        "",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event governance no-5.03 disclosure quality",

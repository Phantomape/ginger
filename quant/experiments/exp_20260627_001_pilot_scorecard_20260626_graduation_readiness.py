"""exp-20260627-001: 2026-06-26 pilot scorecard graduation readiness.

Observed-only alpha/risk-allocation audit over the generated 2026-06-26
...
replacement value, drawdown headroom, clean kill-verdict behavior, and
auditable overlap context to graduate. It does not change signals, entries,
exits, ranking, sizing, pilot state, recommendations, or orders.
"""
...
EXPERIMENT_ID = "exp-20260627-001"
OWNER = "alpha-explore"
SLUG = "pilot_scorecard_20260626_graduation_readiness"
RUNNER = f"quant/experiments/exp_20260627_001_{SLUG}.py"
RUNNER_COMMAND = ".\\.venv\\Scripts\\python.exe -B " + RUNNER.replace("/", "\\")

DATA_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = DATA_DIR / f"exp_20260627_001_{SLUG}.json"
...
PILOT_RECS = REPO_ROOT / "data" / "pilots" / "pilot_recommendations_2026-06-26.json"
PILOT_TRACKER_MD = REPO_ROOT / "data" / "pilots" / "pilot_tracker.md"

HYPOTHESIS = (
    "Observed-only alpha/risk allocation: the 2026-06-26 live pilot scorecard "
    "may identify a default-off pilot sleeve ready to graduate only if newly "
    "closed pilot rows, positive replacement value versus SPY, drawdown "
    "envelope, kill verdict, and overlap checks all pass."
)
...
TRIAL_VARIANT_ID = "current_pilot_scorecard_20260626_v1"
CHANGED_VARIABLE = "pilot_scorecard_20260626_graduation_readiness_v1"
CAUSAL_COMPONENTS = [
...
NEW_EVIDENCE_AXIS = (
    "New 2026-06-26 generated pilot_scorecard and pilot_recommendations rows "
    "after exp-20260625-007; fundamental_growth_rs closed pilot rows increased "
    "from 6 to 7 and cross_pilot_overlap changed from one kill-overlap row to "
    "zero. This is a new forward row/readiness state, not a threshold retune."
)

ALLOWED_WRITE_SCOPE = [
    RUNNER,
    "data/experiments/exp-20260627-001/exp_20260627_001_pilot_scorecard_20260626_graduation_readiness.json",
    "experiments/cards/exp-20260627-001.md",
    "experiments/manifests/exp-20260627-001.json",
    "experiments/tickets/exp-20260627-001.json",
    "experiments/logs/exp-20260627-001.json",
...
def graduate_rule(scorecard_payload: dict[str, Any]) -> dict[str, float | int]:
    ...
    if not isinstance(rule, dict):
        rule = {}
    rv_spy = safe_float(rule.get("min_rv_spy_usd"))
    dd_pct = safe_float(rule.get("max_book_dd_pct"))
    return {
        "min_closed": safe_int(rule.get("min_closed") or pilot_tracker.GRADUATE_MIN_CLOSED),
        "min_rv_spy_usd": float(
            rv_spy if rv_spy is not None else pilot_tracker.GRADUATE_MIN_RV_SPY_USD
        ),
        "max_book_dd_pct": float(
            dd_pct if dd_pct is not None else pilot_tracker.GRADUATE_MAX_BOOK_DD_PCT
        ),
    }
...
def summarize_overlap(scorecard_payload: dict[str, Any], rec_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    all_contexts_have_entry_date = True
    for row in rows or []:
        ...
        all_contexts_have_entry_date = all_contexts_have_entry_date and all(
            "entry_date" in ctx for ctx in contexts
        )
        ...
        overlap_rows.append(
            {
                ...
                "participant_context_entry_date_present": all(
                    "entry_date" in ctx for ctx in contexts
                ),
    ...
    return {
        ...
        "all_overlap_contexts_have_entry_date": all_contexts_have_entry_date,
...
def readiness_decision(
    ...
    checks = {
        ...
        "entry_date_auditable_if_overlap_exists": overlap_summary[
            "all_overlap_contexts_have_entry_date"
        ],
    ...
    if not checks["entry_date_auditable_if_overlap_exists"]:
        failed.append("cross_pilot_overlap_missing_entry_date")
...
def calibration(
    ...
    for mode in prediction.get("main_failure_modes") or []:
        ...
        if mode_text == "closed_rows_too_few" and "closed_rows_too_few" in failed_reasons:
            ...
            mode_text == "negative_replacement_value"
            and "replacement_value_vs_spy_not_positive" in failed_reasons
        ):
            realized_modes.append(mode_text)
        elif (
            mode_text == "drawdown_kill_breach"
            and "book_drawdown_ceiling_breached" in failed_reasons
        ):
            realized_modes.append(mode_text)
        elif mode_text == "pilot_verdict_kill" and "pilot_verdict_kill" in failed_reasons:
            realized_modes.append(mode_text)
        elif (
            mode_text == "cross_pilot_overlap_risk"
            and "cross_pilot_overlap_with_kill_verdict" in failed_reasons
        ):
            realized_modes.append(mode_text)
    return {
...
def build_payload() -> dict[str, Any]:
    ...
    overlap_count = overlap_summary["overlap_count"]
    overlap_context_ok = overlap_count == 0 or overlap_summary[
        "all_overlap_rows_have_participant_context"
    ]
    overlap_entry_date_ok = overlap_count == 0 or overlap_summary[
        "all_overlap_contexts_have_entry_date"
    ]
    payload = {
        ...
        "pre_run_questions": {
            ...
            "history_check": (
                "Novelty gate allowed the current 2026-06-26 scorecard rows. "
                "Nearby priors were exp-20260625-007 pilot graduation readiness, "
                "exp-20260624-014 allocator current-open readiness, and "
                "exp-20260624-011 allocator pilot price attribution."
            ),
            ...
            "success_standard": (
                ...
                "and overlap context is auditable when overlap rows exist."
            ),
        ...
        "gate2": {
            ...
            "required_fields": [
                ...
                "cross_pilot_overlap.participant_context if overlap rows exist",
                "cross_pilot_overlap.participant_context.entry_date if overlap rows exist",
            ],
            "overlap_count": overlap_count,
            "overlap_context_auditable": overlap_context_ok,
            "entry_date_present_in_overlap_context": overlap_entry_date_ok,
        ...
        "post_run_reflection": {
            "why_result_happened": (
                ...
                "have zero closed trades. fundamental_growth_rs has seven closed "
                "trades versus the 20-trade threshold, RV vs SPY of -631.47, "
                "and a 24.37% book drawdown that breaches the 15% envelope. "
                "The prior cross-pilot overlap blocker cleared, but that is not "
                "enough to make any sleeve allocation-ready."
            ),
            "forbidden_near_neighbor_retry": (
                ...
                "thresholds on the same 2026-06-26 scorecard. Reopen only with "
            ...
            "reproducibility": (
                "The runner reads the baseline, pilot scorecard, and 2026-06-26 "
...
def build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: 2026-06-26 pilot graduation readiness",
            "",
            ...
            f"- Cross-pilot overlap rows: `{overlap['overlap_count']}`",
        ...
        json.dumps(
            {
                ...
                "cross_pilot_overlap_count": payload["cross_pilot_overlap_summary"][
                    "overlap_count"
                ],

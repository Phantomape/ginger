...
stack: whether official Space trend signals that already have broad 10d
benchmark-breadth confirmation and high setup quality deserve another small
...
STEM = "space_benchmark_breadth_setup_quality_trend_risk"
...
TQS_FLOOR = 0.95
MARKER = "space_benchmark_breadth_setup_quality_trend_risk"
...
def _is_setup_quality_signal(signal: dict[str, Any]) -> bool:
    if str(signal.get("strategy") or "") != TARGET_STRATEGY:
        return False
    score = _as_float(signal.get("trade_quality_score"))
    return score is not None and score >= TQS_FLOOR


def _apply_setup_quality_scale(
    *,
    signal: dict[str, Any],
    sizing: dict[str, Any],
    scalar: float,
    ...
    adjustments.append(
        {
            ...
            "benchmark_breadth_profile": profile,
...
def _run_current_stack_variant(
    ...
    *,
    ...
    setup_quality_adjustments: list[dict[str, Any]] = []
    ...
    def patched_extra(
        *,
        ...
        if not sizing or not profile:
            return
        if not _is_setup_quality_signal(signal):
            return
        _apply_setup_quality_scale(
            signal=signal,
            sizing=sizing,
            scalar=setup_quality_scalar,
            portfolio_value=portfolio_value,
            counts=counts,
            adjustments=setup_quality_adjustments,
            profile=profile,
        )
        signal["space_benchmark_breadth_setup_quality_trend_bucket"] = True
        signal["space_benchmark_breadth_setup_quality_trend_scalar"] = (
            setup_quality_scalar
        )
        signal["space_benchmark_breadth_setup_quality_profile"] = profile
    ...
    try:
        variant = exp021._run_exp051_stack_variant(
            label,
            defense_same_theme_winner_scalar=ACCEPTED_DEFENSE_BUDGET_SAME_THEME_WINNER_SCALAR,
            gates=gates,
        )
    finally:
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_benchmark_breadth_setup_quality_trend_scalar": setup_quality_scalar,
        "target_strategy": TARGET_STRATEGY,
        "target_trade_quality_score_floor": TQS_FLOOR,
    }
    ...
    variant["benchmark_breadth_setup_quality_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["benchmark_breadth_setup_quality_counts_by_window"] = by_window_counts
    variant["benchmark_breadth_setup_quality_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(setup_quality_adjustments)
    )
    variant["benchmark_breadth_setup_quality_adjustment_sample"] = (
        setup_quality_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("benchmark_breadth_setup_quality_counts") or {}
    changed_count = int(counts.get(f"{MARKER}_changed_signal", 0))
    eligible_count = int(counts.get(f"{MARKER}_eligible_signal", 0))
    scalar = float(
        variant["parameters"]["space_benchmark_breadth_setup_quality_trend_scalar"]
    )
    passed = bool(
        scalar != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "eligible_setup_quality_signal_count": eligible_count,
        "changed_setup_quality_signal_count": changed_count,
        "reasons": {
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Official Space trend signals with broad 10d benchmark-breadth "
            "confirmation and trade_quality_score >= 0.95 may be mis-sized by "
            "the accepted default-off Space stack."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_benchmark_breadth_setup_quality_trend_scalar",
        "parameters": {
            "scalars_tested": list(SCALARS),
            "selected_scalar": best["parameters"][
                "space_benchmark_breadth_setup_quality_trend_scalar"
            ],
            "target_strategy": TARGET_STRATEGY,
            "target_trade_quality_score_floor": TQS_FLOOR,
            "accepted_before_experiment": BEFORE_EXPERIMENT_ID,
        ...
        "gate_answers": {
            "1_alpha_hypothesis": (
                "Risk allocation: benchmark-breadth Space trend signals with "
                "trade_quality_score >= 0.95 may deserve a distinct scalar."
            ),
            "2_prior_similar_experiments": [
                "exp-20260512-004 accepted perfect Space TQS risk.",
                "exp-20260512-008 accepted near-perfect Space trend TQS risk.",
                "exp-20260514-041 accepted benchmark-breadth trend risk.",
                ...
                "exp-20260515-045/047/048 rejected source-type and theme-segment splits on the current stack.",
                "No prior current-stack run isolated high setup quality inside benchmark-breadth Space trend profiles.",
            ],
            "3_single_causal_variable": (
                "Only the benchmark-breadth setup-quality trend scalar changes."
            ),
            "4_success_criteria": (
            ...
            "5_reproducibility": (
                ...
                "exp_20260516_001_space_benchmark_breadth_setup_quality_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: benchmark-breadth setup-quality trend scalar did "
            "not improve the fixed windows while satisfying PnL, window, and "
            "drawdown guardrails."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby benchmark-breadth setup-quality Space trend "
        ...
        "why_not_other_changes": (
            ...
            "ticker/ETF admissions and source-type splits regressed or were "
            "single-window. This run keeps the fixed pool and tests a "
            "production-visible catalyst-quality plus setup-quality field."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space benchmark-breadth setup-quality trend risk",
        "",
        "## Hypothesis",
        (
            "Official Space `trend_long` signals may need different default-off "
            "allocation when they combine broad 10d benchmark-breadth "
            "confirmation with `trade_quality_score >= 0.95`."
        ),
        "",
        "## Single Changed Variable",
        f"`space_benchmark_breadth_setup_quality_trend_scalar` on top of accepted `{BEFORE_EXPERIMENT_ID}`.",
        "",
        "## Gate 1 Baseline",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_benchmark_breadth_setup_quality_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_setup_quality_signal_count']}`",
            f"- adjusted signals: `{gate['changed_setup_quality_signal_count']}`",
            f"- adjusted counts: `{best['benchmark_breadth_setup_quality_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Benchmark-breadth setup-quality trend scalar "
            f"{best['parameters']['space_benchmark_breadth_setup_quality_trend_scalar']} "
            f"changed {gate['changed_setup_quality_signal_count']} signals with "
            "aggregate EV delta "
...
def run() -> dict[str, Any]:
    ...
    if not gates["benchmark_breadth_gate"]["passed"]:
        raise RuntimeError(
            f"Benchmark-breadth gate failed: {gates['benchmark_breadth_gate']}"
        )

    variants = [
        _run_current_stack_variant(
            label=f"{STEM}_{str(scalar).replace('.', '_')}",
            setup_quality_scalar=scalar,
            gates=gates,
        )
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            "Benchmark-breadth Space trend signals with trade_quality_score >= "
            "0.95 may be mis-sized in the accepted stack."
        ),
        "changed_variable": "space_benchmark_breadth_setup_quality_trend_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate["eligible_setup_quality_signal_count"],
                    "adjusted_signals": gate["changed_setup_quality_signal_count"],

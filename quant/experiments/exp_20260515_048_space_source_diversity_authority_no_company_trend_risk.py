...
stack: whether source-diverse official Space trend signals backed by
non-company authoritative source types deserve a separate default-off risk
scalar.
...
EXPERIMENT_ID = "exp-20260515-048"
STEM = "space_source_diversity_authority_no_company_trend_risk"
...
TARGET_EXCLUDED_SOURCE_TYPE = "company_release"
TARGET_AUTHORITY_SOURCE_TYPES = (
    "official_government_release",
    "official_or_primary_release",
    "official_regulatory_release",
)
MARKER = "space_source_diversity_authority_no_company_trend_risk"
...
def _is_authority_no_company_profile(profile: dict[str, Any] | None) -> bool:
    if not profile:
        return False
    source_types = {str(item) for item in profile.get("source_types") or []}
    return (
        TARGET_EXCLUDED_SOURCE_TYPE not in source_types
        and bool(source_types.intersection(TARGET_AUTHORITY_SOURCE_TYPES))
    )


def _field_check_authority_no_company_profiles() -> dict[str, Any]:
    base = exp041.source_diversity_exp._field_check_source_diversity_profile()
    ...
    for line in path.read_text(encoding="utf-8-sig").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        if not exp041.source_diversity_exp._is_non_attention_official_event(row):
            continue
        for ticker in row.get("tickers") or []:
            ticker = str(ticker).upper()
            if ticker not in exp041.source_diversity_exp.OFFICIAL_SPACE_TICKERS:
                continue
    ...
    target_profiles = {
        ...
        if _is_authority_no_company_profile(profile)
    }
    return {
        **base,
        "passed": bool(base.get("passed")) and bool(target_profiles),
        "target_excluded_source_type": TARGET_EXCLUDED_SOURCE_TYPE,
        "target_authority_source_types": list(TARGET_AUTHORITY_SOURCE_TYPES),
        "target_profiles": target_profiles,
        "target_tickers": sorted(target_profiles),
    }


def _apply_authority_no_company_scale(
    *,
    signal: dict[str, Any],
    sizing: dict[str, Any],
    scalar: float,
    ...
    *,
    ...
    def patched_scale_and_record(
        *,
        ...
        if not _is_authority_no_company_profile(profile):
            return
        _apply_authority_no_company_scale(
            signal=signal,
            sizing=sizing,
            scalar=authority_no_company_scalar,
            portfolio_value=portfolio_value,
            counts=counts,
            adjustments=authority_no_company_adjustments,
            profile=profile,
        )
        signal["space_source_diversity_authority_no_company_trend_bucket"] = True
        signal["space_source_diversity_authority_no_company_trend_scalar"] = (
            authority_no_company_scalar
        )
        signal["space_source_diversity_authority_no_company_profile"] = profile
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_source_diversity_authority_no_company_trend_scalar": (
            authority_no_company_scalar
        ),
    ...
    variant["source_diversity_authority_no_company_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["source_diversity_authority_no_company_counts_by_window"] = by_window_counts
    variant["source_diversity_authority_no_company_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(
            authority_no_company_adjustments
        )
    )
    variant["source_diversity_authority_no_company_adjustment_sample"] = (
        authority_no_company_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("source_diversity_authority_no_company_counts") or {}
    ...
    return {
        ...
        "eligible_authority_no_company_signal_count": eligible_count,
        "changed_authority_no_company_signal_count": changed_count,
        "reasons": {
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Source-diverse official Space trend signals with authoritative "
            "non-company source support may be mis-sized by the accepted "
            "default-off Space stack."
        ),
        ...
        "gate_answers": {
            "1_alpha_hypothesis": (
                ...
                "official non-company authority source support may deserve a "
                "distinct scalar."
            ),
            "2_prior_similar_experiments": [
                ...
                "No prior current-stack run isolated authoritative non-company source profiles on top of exp-20260515-044.",
            ],
            "3_single_causal_variable": (
                "Only the authority-no-company source-diversity trend scalar changes."
            ),
            "4_success_criteria": (
            ...
            "5_reproducibility": (
                ...
                "exp_20260515_048_space_source_diversity_authority_no_company_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the authority-no-company source-diversity trend "
            "scalar did not improve the fixed windows while satisfying PnL "
            "and drawdown guardrails."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby authority/no-company source-diversity trend "
            "scalars on these frozen windows without new closed forward rows "
            "or a different production-visible catalyst-quality field."
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            ...
            "keeps the fixed pool and tests a source-profile catalyst-quality "
            "field instead of another ticker/segment admission."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity authority-no-company trend risk",
        "",
        "## Hypothesis",
        (
            "Source-diverse official Space `trend_long` signals may need a "
            "different default-off allocation when the source-diversity "
            "profile has authoritative non-company source support."
        ),
        "",
        "## Single Changed Variable",
        (
            "`space_source_diversity_authority_no_company_trend_scalar` on top of "
        ...
        f"- source profile field check passed: `{payload['authority_profile_field_check']['passed']}`",
        f"- excluded source type: `{TARGET_EXCLUDED_SOURCE_TYPE}`",
        f"- authority source types: `{list(TARGET_AUTHORITY_SOURCE_TYPES)}`",
        f"- target tickers: `{payload['authority_profile_field_check'].get('target_tickers')}`",
        "",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_source_diversity_authority_no_company_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_authority_no_company_signal_count']}`",
            f"- adjusted signals: `{gate['changed_authority_no_company_signal_count']}`",
            f"- adjusted counts: `{best['source_diversity_authority_no_company_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Authority-no-company source-diversity trend scalar "
            f"{best['parameters']['space_source_diversity_authority_no_company_trend_scalar']} "
            f"changed {gate['changed_authority_no_company_signal_count']} signals with "
            "aggregate EV delta "
...
def run() -> dict[str, Any]:
    ...
    authority_profile_field_check = _field_check_authority_no_company_profiles()
    if not field_check["passed"]:
        raise RuntimeError(f"Open-position field check failed: {field_check}")
    if not authority_profile_field_check["passed"]:
        raise RuntimeError(
            f"Authority source-profile field check failed: {authority_profile_field_check}"
        )

    variants = [
        _run_current_stack_variant(
            label=f"{STEM}_{str(scalar).replace('.', '_')}",
            authority_no_company_scalar=scalar,
            gates=gates,
        )
    ...
    payload = {
        ...
        "authority_profile_field_check": authority_profile_field_check,
        "variants": variants,
        ...
        "alpha_hypothesis": (
            "Source-diverse Space trend signals with authoritative non-company "
            "source support may be mis-sized in the accepted stack."
        ),
        "changed_variable": (
            "space_source_diversity_authority_no_company_trend_scalar"
        ),

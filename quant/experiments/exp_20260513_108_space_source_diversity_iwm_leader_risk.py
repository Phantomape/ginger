...
signals only when IWM 20-day momentum is above SPY 20-day momentum.

This avoids LLM soft-ranking, ticker-pool expansion, target/stop changes, and
nearby source-diversity or peer-leader scalar retunes. The alpha question is
whether heterogeneous official catalyst evidence deserves extra risk when the
small-cap tape is also favorable.
"""
...
STEM = "space_source_diversity_iwm_leader_risk"
...
SOURCE_DIVERSITY_IWM_LEADER_RISK_SCALARS = (
    0.75,
    0.90,
    1.00,
    1.025,
...
def _runtime_state_gate(before: dict[str, Any]) -> dict[str, Any]:
    ...
    for payload in before.get("by_window", {}).values():
        counts = payload.get("source_diversity_iwm_leader_counts") or {}
        source_diverse += int(counts.get("source_diverse_eligible_signal", 0) or 0)
        peer_leader += int(counts.get("source_diverse_peer_leader_eligible_signal", 0) or 0)
        iwm_leader += int(counts.get("source_diverse_iwm_leader_eligible_signal", 0) or 0)
        summary = payload.get("source_diversity_iwm_leader_adjustment") or {}
        for row in summary.get("sample_adjusted") or []:
            ...
            if len(samples) >= 12:
                break
    return {
        "passed": source_diverse > 0 and iwm_leader > 0,
        ...
        "sample_source_diverse_iwm_rows": samples,
    }


def _run_iwm_leader_variant(
    label: str,
    ...
    *,
    ...
    def size_with_source_diversity_iwm_leader(
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                ...
                if signal.get("space_peer_momentum_state") == PEER_LEADER_STATE:
                ...
                if signal.get("space_iwm_relative_state") == IWM_LEADER_STATE:
                    ...
                    if shares_after_iwm != shares_before_iwm:
                        counts["source_diverse_iwm_leader_changed_signal"] += 1
                        counts[f"source_diverse_iwm_leader_changed_{ticker}"] += 1
                    iwm_adjustments.append(
                        {
                            ...
                            "shares_after_scalar": shares_after_iwm,
                            "source_diversity_profile": profile,
                            "space_iwm_relative_state": signal.get(
                                "space_iwm_relative_state"
                            ),
    ...
    portfolio_engine.size_signals = size_with_source_diversity_iwm_leader
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "source_diversity_iwm_leader_counts": count_delta,
            }
        ...
        return {
            ...
            "parameters": {
                ...
                "space_source_diversity_iwm_leader_scalar": iwm_leader_scalar,
            ...
            "source_diversity_iwm_leader_adjustment_counts": dict(
                sorted(counts.items())
            ),
            "source_diversity_iwm_leader_adjustment_sample": iwm_adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["source_diversity_iwm_leader_adjustment_counts"].get(
            "source_diverse_iwm_leader_changed_signal", 0
        )
    )
    eligible_count = int(
        variant["source_diversity_iwm_leader_adjustment_counts"].get(
            "source_diverse_iwm_leader_eligible_signal", 0
        )
    )
    scalar = float(variant["parameters"]["space_source_diversity_iwm_leader_scalar"])
    ...
    return {
        ...
        "eligible_source_diversity_iwm_leader_signal_count": eligible_count,
        "changed_source_diversity_iwm_leader_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity IWM-leader risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_source_diversity_iwm_leader_scalar` after the accepted exp-039 "
            "source-diversity peer-leader stack. Candidate pool, event labels, "
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_source_diversity_iwm_leader_scalar']}`"
        ),
        (
        ...
        (
            "- Source-diverse IWM-leader signals changed: "
            f"`{gate['changed_source_diversity_iwm_leader_signal_count']}` of "
            f"`{gate['eligible_source_diversity_iwm_leader_signal_count']}` eligible"
        ),
        "",
...
def run() -> dict[str, Any]:
    ...
    before = _run_iwm_leader_variant(
    ...
    gate2 = {
        ...
        "source_diversity_iwm_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    variants = [
        ...
        for scalar in SOURCE_DIVERSITY_IWM_LEADER_RISK_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested source-diverse IWM-leader scalar improved aggregate EV/PnL "
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "source-diverse official Space signals may deserve extra risk when "
            "IWM 20d momentum beats SPY 20d momentum. This tests a production-visible "
            "catalyst-quality plus small-cap risk-appetite interaction without "
            "changing the Space pool, event metadata, ranking, targets, stops, LLM "
            "boundary, or live slots."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_source_diversity_iwm_leader_risk_scalar",
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                ...
                "when IWM confirms small-cap risk appetite."
            ),
            "q2_prior_experiments": [
                ...
                "exp-20260512-031 accepted broad IWM>SPY Space risk.",
                "exp-20260513-020 accepted IWM+peer-leader trend risk, but target widening for that state failed in exp-026.",
                "exp-20260513-106 rejected regulatory customer-source risk because only mid_weak improved.",
            ],
            "q3_single_causal_variable": (
                "Only the source-diversity IWM-leader scalar changes; accepted exp-039 "
                "risk stack and all entry/exit/ranking/candidate variables stay fixed."
            ),
            "q4_acceptance_standard": (
                ...
                "and real adjusted source-diverse IWM-leader signals."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            ...
            "tested_source_diversity_iwm_leader_scalars": list(
                SOURCE_DIVERSITY_IWM_LEADER_RISK_SCALARS
            ),
            "locked_variables": [
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_source_diversity_iwm_leader_risk"
            if decision == "accepted"
            else "rejected_space_source_diversity_iwm_leader_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not keep tuning source-diversity IWM-leader scalars on this "
        ...
        "why_not_other_changes": (
            ...
            "This isolates a production-visible catalyst quality plus risk-appetite axis."
        ),
        "known_risks": [
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_source_diversity_iwm_leader_signal_count": result[
            "best_variant_gate"
        ]["changed_source_diversity_iwm_leader_signal_count"],

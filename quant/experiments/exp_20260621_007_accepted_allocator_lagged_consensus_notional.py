"""exp-20260621-007: lagged-consensus source notional scalar.

Alpha search. Tests one attributable allocation hypothesis: keep the accepted
helper source-priority allocator selection fixed, keep the accepted
laggard/revision/peer-shock/turn-of-month source scalars fixed, and add only a
small default-off paper notional scalar to selected lagged_cross_source_consensus rows.
...
import exp_20260621_001_accepted_allocator_peer_shock_notional as peer_template
...
EXPERIMENT_ID = "exp-20260621-007"
OWNER = "alpha-search-automation"
STEM = "accepted_allocator_lagged_consensus_notional"
TRIAL_FAMILY = "accepted_default_off_helper_source_priority_allocation"
TRIAL_VARIANT_ID = "accepted_allocator_lagged_consensus_source_notional_scalar_v1"
CHANGED_VARIABLE = TRIAL_VARIANT_ID
TARGET_SOURCE = "lagged_cross_source_consensus"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260621_007_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "consensus_rows_already_fully_sized",
        "source_notional_near_neighbor",
        "drawdown_drift",
        "window_regression",
        "accepted_allocator_comparator_not_beaten",
    ],
    "confidence_reason": (
        "Mechanism is capital allocation to an already selected accepted "
        "allocator source rather than changing source rank or adding tickers. "
        "The current allocator probe shows lagged_cross_source_consensus is "
        "still selected 42 times across all three canonical windows with about "
        "$12,074 net paper PnL after the accepted laggard/revision/peer/turn "
        "source scalars, but this is a near-neighbor allocation test and may "
        "be in-sample."
    ),
    "recorded_at": "2026-06-21T08:10:08+00:00",
}

PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "allocation: selected lagged_cross_source_consensus rows in the accepted helper allocator "
        "may deserve a small default-off paper notional scalar because the "
        "current accepted allocator still selects rank-1 consensus rows across "
        "all three canonical windows with strong realized paper PnL after the "
        "accepted laggard, revision, peer-shock, and turn-of-month source scalars."
    ),
    "2_history_check": {
        "novelty_gate": (
            "experiment.py new warned on allocator/revision near-neighbors. The "
            "override is corrected current allocator after-arbitration evidence: "
            "lagged_cross_source_consensus selected rows (42 trades across all "
            "three canonical windows, about $12,074 net PnL) have not been tested "
            "for source-notional support; this keeps source set, source rank, "
            "timing, hold, cooldown, and candidate selection fixed."
        ),
        "exp-20260611-005": (
            "Accepted lagged cross-source consensus as rank-1 source inside the "
            "shared default-off accepted-helper allocator. This run keeps that "
            "source definition and rank fixed."
        ),
        "exp-20260609-027": (
            "Accepted turn-of-month liquid leadership as a shared default-off "
            "paper adapter. This run includes its already accepted 1.25x source "
            "notional scalar in the before state."
        ),
        "exp-20260620-032": (
            "Accepted 1.25x source-notional scalars for selected "
            "industry_laggard_repair and revision_surprise_low_extension rows."
        ),
        "exp-20260620-033": (
            "Corrected independence map and current allocator probe supplied the "
            "new after-arbitration evidence used here; the target consensus rows "
            "are selected by the current shared allocator, not by a new source set."
        ),
        "exp-20260621-001": (
            "Accepted the same 1.25x scalar for selected rolling_peer_shock rows; "
            "this run keeps that before state fixed."
        ),
        "exp-20260621-006": (
            "Accepted the same 1.25x scalar for selected turn_of_month rows; this "
            "run keeps that before state fixed."
        ),
        "exp-20260621-004": (
            "Rejected volatility_relief scalar because the current allocator "
            "selected zero volatility_relief rows after arbitration."
        ),
    },
    "3_single_causal_variable": (
        "One fixed policy bundle: add source_notional_scalar=1.25 for selected "
        "lagged_cross_source_consensus allocator rows, while accepted laggard/"
        "revision/peer-shock/turn-of-month scalars, source selection, rank, "
        "daily top-1, hold, costs, cooldown, core behavior, LLM/news, and "
        "live/default orders remain fixed."
    ),
    "4_acceptance_standard": (
        "Use docs/backtesting.md canonical three windows. Before is the current "
        "accepted allocator including exp-032, exp-001, and exp-006 source scalars; after "
        "adds only lagged_cross_source_consensus 1.25x. Accept only if aggregate EV/PnL improve, "
        "no window EV/PnL regression, affected consensus sample spans all "
        "three windows with >=20 rows, survival/concentration/drawdown guards "
        "pass, and the after allocator still beats accepted comparators."
    ),
    "5_reproducibility": (
        ".venv\\Scripts\\python.exe -B quant\\experiments\\"
        "exp_20260621_007_accepted_allocator_lagged_consensus_notional.py"
    ),
}
...
def _binding_gate4(
    *,
    ...
    return {
        "passed": accepted,
        "decision": (
            "accepted_allocator_lagged_consensus_source_notional_scalar"
            if accepted
            else "rejected_allocator_lagged_consensus_source_notional_scalar"
        ),
...
def build_payload() -> dict[str, Any]:
    ...
    try:
        current_payload = template._run_allocator_pass(
            "before_current_exp032_exp001_exp006", dict(CURRENT_SCALARS)
        )
        after_payload = template._run_allocator_pass(
            "after_lagged_consensus_scaled", dict(AFTER_SCALARS)
        )
    finally:
    ...
    for label in framework.WINDOWS:
        ...
        window_rows[label] = {
            "before": before,
            "after": after,
            "delta": peer_template._delta(after, before),
    ...
    aggregate = peer_template._aggregate_incremental(window_rows)
    ...
    return {
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                "Before: current accepted allocator with exp-032 laggard/revision "
                "plus exp-001 peer_shock and exp-006 turn_of_month "
                "source_notional_scalars. After: same "
                "source selection with only lagged_cross_source_consensus added at 1.25x paper "
                "notional. PnL is recomputed from selected row pnl_pct_net and "
                "scaled notional."
            ),
        },
        "parameters": {
        ...
        "interpretation": (
            "Lagged-consensus source notional improved the current accepted allocator "
            "and should be retained only as shared default-off paper observation."
            if accepted
            else "Lagged-consensus source notional failed Gate 4 and is not retained."
        ),
        "rejection_reason": None if accepted else "; ".join(gate4["failed_reasons"]),
        "post_run_reflection": {
            "why_result_happened": (
                "The current after-arbitration lagged_cross_source_consensus signal translated into "
                "incremental after-arbitration paper capital without changing "
                "selection."
                if accepted
                else (
                    "The current allocator selected lagged_cross_source_consensus rows, but the "
                    "incremental scaled capital did not clear the window, risk, "
                    "sample, concentration, or accepted-comparator gates."
                )
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retry by sweeping lagged consensus scalar, source rank, "
                "allocator top-N, source-family map, prior-confirmation timing, "
                "hold days, cooldown, or notional on the frozen windows."
            ),
            "new_evidence_required": (
                "Closed forward allocator replacement-value rows tagged by "
                "lagged_cross_source_consensus source family, or a materially new out-of-sample "
                "source independence surface."
            ),
        },
        "next_retry_requires": [
            "closed forward lagged_cross_source_consensus replacement-value rows",
            "out-of-sample source independence map",
            "no frozen-window lagged consensus source scalar sweep",
        ],
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Accepted Allocator Lagged Consensus Notional",
            "",
            ...
            *_window_table(payload),
            ...
            "- Lagged-consensus affected trades: `{}` across `{}` windows".format(

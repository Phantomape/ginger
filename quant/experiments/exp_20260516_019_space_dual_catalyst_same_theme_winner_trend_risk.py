...
Tests one allocation variable on top of accepted exp-20260516-015: whether
source-diverse official Space trend signals with both customer and government
catalysts deserve a small extra scalar when closed defense-budget event rows
also show positive same-theme replacement value.
...
EXPERIMENT_ID = "exp-20260516-019"
STEM = "space_dual_catalyst_same_theme_winner_trend_risk"
...
def _run_variant(
    *,
    same_theme_winner_scalar: float,
    gates: dict[str, Any],
) -> dict[str, Any]:
    ...
    def patched_scale_and_record(
        *,
        ...
        base._apply_scale(
            signal=signal,
            sizing=sizing,
            scalar=same_theme_winner_scalar,
            portfolio_value=portfolio_value,
            marker=MARKER,
            counts=counts,
    ...
    try:
        variant = base.exp021._run_exp051_stack_variant(
            f"{STEM}_{str(same_theme_winner_scalar).replace('.', '_')}",
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_dual_catalyst_same_theme_winner_trend_scalar": (
            same_theme_winner_scalar
        ),
        "target_strategy": TARGET_STRATEGY,
        "target_profile_gate": "defense_budget_same_theme_winner_gate",
        "target_tickers": sorted(same_theme_tickers),
    }
    ...
    variant["dual_catalyst_same_theme_winner_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["dual_catalyst_same_theme_winner_counts_by_window"] = by_window_counts
    variant["dual_catalyst_same_theme_winner_adjustment_summary"] = (
        base.exp041.source_diversity_exp._adjustment_summary(
            dual_same_theme_adjustments
        )
    )
    variant["dual_catalyst_same_theme_winner_adjustment_sample"] = (
        dual_same_theme_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("dual_catalyst_same_theme_winner_counts") or {}
    by_window_counts = (
        variant.get("dual_catalyst_same_theme_winner_counts_by_window") or {}
    )
    ...
    return {
        ...
        "eligible_dual_same_theme_winner_signal_count": eligible_count,
        "changed_dual_same_theme_winner_signal_count": changed_count,
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "changed_variable": "space_dual_catalyst_same_theme_winner_trend_scalar",
        "parameters": {
        ...
        "gate_answers": {
            ...
            "2_prior_similar_experiments": [
                ...
                "No prior run isolated defense-budget same-theme winner evidence inside the accepted dual-catalyst sleeve.",
            ],
            "3_single_causal_variable": (
                "Only the incremental dual-catalyst same-theme winner scalar "
            ...
            "5_reproducibility": (
                ...
                "exp_20260516_019_space_dual_catalyst_same_theme_winner_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the dual-catalyst same-theme winner scalar did not "
            "satisfy the fixed three-window EV/PnL, drawdown, and cohort guards."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby same-theme winner scalars on these frozen "
        ...
        "why_not_other_changes": (
            "LLM soft-ranking lacks dense downstream attribution, recent pool "
            "expansion was fragile in old_thin, and peer-state refinements were "
            "just rejected. This keeps the fixed Space pool and tests one "
            "production-visible mature catalyst-quality discriminator."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space dual-catalyst same-theme winner trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_dual_catalyst_same_theme_winner_trend_scalar` on top of "
        ...
        f"- same-theme winner gate passed: `{payload['same_theme_winner_gate']['passed']}`",
        f"- target tickers: `{payload['same_theme_winner_gate'].get('target_tickers')}`",
        "",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_dual_catalyst_same_theme_winner_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_dual_same_theme_winner_signal_count']}`",
            f"- changed signals: `{gate['changed_dual_same_theme_winner_signal_count']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Dual-catalyst same-theme winner Space scalar "
            f"{best['parameters']['space_dual_catalyst_same_theme_winner_trend_scalar']} "
            f"changed {gate['changed_dual_same_theme_winner_signal_count']} "
...
def run() -> dict[str, Any]:
    ...
    same_theme_winner_gate = gates["defense_budget_same_theme_winner_gate"]
    ...
    if not same_theme_winner_gate["passed"]:
        raise RuntimeError(
            f"Same-theme winner gate field check failed: {same_theme_winner_gate}"
        )

    variants = [
        _run_variant(same_theme_winner_scalar=scalar, gates=gates)
    ...
    payload = {
        ...
        "same_theme_winner_gate": same_theme_winner_gate,
        "variants": variants,
        ...
        "hypothesis": (
            ...
            "signals may still be under-sized when closed defense-budget "
            "government_space_contract rows also prove positive same-theme "
            "replacement value."
        ),
        "changed_variable": "space_dual_catalyst_same_theme_winner_trend_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate[
                        "eligible_dual_same_theme_winner_signal_count"
                    ],
                    "changed_signals": gate[
                        "changed_dual_same_theme_winner_signal_count"
                    ],

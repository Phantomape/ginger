"""exp-20260510-009 trend follow-through add-on upper-bound replay.

Alpha search, replay-only. The prior raw add-on heat ceiling was positive but
not production-safe, and the breakout-only upper bound was too small. This
experiment changes one causal variable: the eligible add-on cohort is existing
`trend_long` positions that already passed the accepted day-2 follow-through
checkpoint. It estimates the optimistic ceiling before any shared
production/backtest budget policy is worth building.
"""
...
EXPERIMENT_ID = "exp-20260510-009"
STEM = "trend_addon_upper_bound"
...
def _extra_pnl_for_event(event: dict[str, Any], parent: dict[str, Any] | None) -> dict[str, Any]:
...
def _curve_metrics(
    ...
    expected_value_score = round(strategy_return * sharpe_daily, 4) if sharpe_daily is not None else None
...
def _top_positive_share(rows: dict[str, Any]) -> float | None:
    ...
    for row in rows.values():
        for event in row["eligible_events"]:
            ...
            if pnl > 0:
                by_ticker[str(event.get("ticker") or "").upper()] += pnl
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID}: Trend Add-on Upper Bound",
        "",
        ...
        f"- Single-ticker positive contribution share: `{payload['sample_guard']['max_single_positive_share']}`",
        "",
        "## Window Results",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    if gate_passed:
        rationale = (
            "The optimistic trend_long add-on full-fill upper bound cleared the "
            "three-window materiality and sample gates. This is not production "
            "approval; the next step would be a shared budget/reserve policy that "
            "keeps hard heat and concentration controls explicit."
        )
        rejection_reason = None
        next_action = (
            "Design a default-off shared add-on budget policy for trend_long candidates "
            "only if it can preserve global heat semantics in both run.py and backtester.py."
        )
    else:
        rationale = (
            "Rejected: even the optimistic upper bound for filling all unfilled trend_long "
            "follow-through add-on shares did not clear the EV-first three-window Gate 4 "
            "and/or sample concentration guard. Do not spend production work on a "
            "trend-only add-on budget without new forward evidence."
        )
        rejection_reason = rationale
        next_action = (
            "Keep the existing add-on cap/heat policy unchanged; prefer a different "
            "capital-allocation discriminator or forward paper replacement-value evidence."
        )
    ...
    payload = {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "addon_trend_upper_bound_replay",
        "mechanism_family": "followthrough_addon_materiality_ceiling",
        "hypothesis": (
            "Existing trend_long positions that already pass the accepted day-2 "
            "follow-through add-on checkpoint may have enough marginal add-on value "
            "to justify future budget/reserve semantics, after the breakout-only "
            "upper bound proved too small."
        ),
        "alpha_hypothesis": {
            "category": "allocation/lifecycle",
            "entry_exit_ranking_or_allocation": "allocation",
            "playbook_alignment": (
                "Priority 1 remains add-on materiality ceiling, but nearby trigger, "
                "fraction, cap, heat, second-add, and breakout-specific variants are "
                "blocked or rejected. This tests the orthogonal trend_long cohort."
            ),
        },
        "single_causal_variable": (
            "trend_long existing-position cohort as the add-on value discriminator; "
            "trigger, fraction, exits, hard production defaults, universe, and LLM/news stay locked"
        ),
        "parameters": {
            "target_strategy": TARGET_STRATEGY,
            "upper_bound_assumption": (
                "All unfilled requested add-on shares for eligible trend_long events are "
                "filled at the original scheduled add-on fill price and exit with the "
                "matched parent trade. This is intentionally optimistic and does not alter "
        ...
        "gate2_field_check": {
            "operator_open_positions_entry_date_missing": 0,
            "operator_open_positions_target_price_missing": 0,
            "experiment_runtime_fields": [
                "addon_attribution.events.strategy",
                "addon_attribution.events.requested_shares",
                "addon_attribution.events.scheduled_fill_date",
                "trades.entry_date",
                "trades.exit_date",
                "trades.exit_price",
            ],
        },
        "missing_required_docs_observed": [
            "root backtesting.md missing; docs/backtesting.md used as the repository backtesting protocol",
            "docs/current_state.md missing before this run",
        ],
        "historical_experiment_check": {
            ...
            "exp-20260510-004": (
                "breakout_long upper bound improved all windows but missed materiality; "
                "this run changes the cohort to trend_long rather than repeating breakout."
            ),
            "why_not_simple_repeat": (
                "It is not a trigger threshold retune, heat-cap relaxation, same-day ordering key, "
                "generic reserve, second add-on, or broad add-on cap sweep."
            ),
        },
        ...
        "gate4": {
            ...
            "basis": (
                "Three canonical docs/backtesting.md windows. Because this is an optimistic "
                "upper-bound screen, acceptance requires material aggregate improvement or at "
                "least two window Gate 4 passes, no EV regression, and sample concentration guard."
            ),
        },
        ...
        "why_not_other_changes": (
            "Skipped LLM soft-ranking, SEC/earnings filing shock, ETF overlay retunes, "
            "event/source retunes, raw heat relaxation, generic reserves, and breakout "
            "add-on variants because recent records mark them data-limited, forward-only, "
            "rejected, or saturated."
        ),
        "known_risks": (
            "The upper bound ignores cash/heat interactions and could overstate real "
            "fillability. It is a pre-policy materiality screen, not an executable order rule."
        ),
        "artifacts": {

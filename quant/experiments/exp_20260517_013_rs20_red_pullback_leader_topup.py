"""exp-20260517-013: RS20 leader red-pullback top-up scout.

Tests one production-visible allocation discriminator on the accepted core
stack: already-qualified trend/breakout signals that are RS20 leaders but whose
own signal-day candle is not green. The thesis is pullback absorption inside
established relative-strength leadership, not another broad RS20 scalar retune.
...
EXPERIMENT_ID = "exp-20260517-013"
EXPERIMENT_SLUG = "rs20_red_pullback_leader_topup"
MULTIPLIER_KEY = "rs20_red_pullback_leader_risk_multiplier_applied"
STATE_KEY = "rs20_red_pullback_leader_state"
BASELINE_RISK_MULTIPLIER = 1.0
RISK_MULTIPLIER_SWEEP = [1.0, 1.0125, 1.025, 1.05]
STATE_STRATEGIES = {"trend_long", "breakout_long"}
EXCLUDED_SECTORS = {"ETF"}
...
def _topup_sizing(
    ...
    out = dict(sizing)
    out["rs20_red_pullback_leader_baseline_shares"] = shares
    out["rs20_red_pullback_leader_desired_shares"] = desired_shares
    out["rs20_red_pullback_leader_cap_shares"] = cap_shares
    out["rs20_red_pullback_leader_new_shares"] = new_shares
                    ...
                    scout.base.ADJUSTMENTS.append(
                        {
                            ...
                            "desired_shares": adjusted_sizing.get(
                                "rs20_red_pullback_leader_desired_shares"
                            ),
                            "cap_shares": adjusted_sizing.get(
                                "rs20_red_pullback_leader_cap_shares"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} RS20 Leader Red-Pullback Top-Up",
            "",
...
def _rewrite_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_rs20_red_pullback_leader_topup"
    )
    interpretation = (
        "The RS20 leader red-pullback state cleared Gate 4 and requires shared policy promotion plus rerun before production use."
        if passed
        else "The RS20 leader red-pullback state did not clear the canonical three-window gate."
    )
    payload.update(
        {
            ...
            "decision": decision,
            "hypothesis": (
                "The accepted stack already rewards RS20 leadership and own-green "
                "confirmation separately. A different production-visible state is "
                "RS20 leadership on a non-green signal day: this can mark pullback "
                "absorption inside an established relative-strength trend rather "
                "than simple chase momentum. A small cap-aware top-up may improve "
                "allocation without changing candidate generation or ranking."
            ),
            "change_type": "risk_allocation_shadow",
            "changed_variable": "rs20_red_pullback_leader_risk_multiplier",
            "single_causal_variable": (
                "post-sizing cap-aware top-up for trend/breakout non-ETF signals "
                "with rs20_entry_state_leader=true and signal_day_ticker_green_candle!=true"
            ),
            ...
            "next_evidence_needed": (
                None
                if passed
                else "Do not retry RS20 red-pullback top-ups without forward pullback-leader attribution or a materially different production-visible quality field."
            ),
            "anti_js": "No JavaScript was used.",
            "related_files": [
                "quant/experiments/exp_20260517_013_rs20_red_pullback_leader_topup.py",
    ...
    payload["parameters"].update(
        {
            "state_definition": {
                ...
                "requires_signal_day_ticker_green_candle": False,
                "cap_aware": True,
            },
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "risk allocation on a production-visible RS20 leader red-pullback "
            "state; this fits the playbook preference for fixed-candidate allocation"
        ),
        "2_history_check": {
            "accepted_RS20_and_own_green": (
                "RS20 leadership and own-green top-ups are already accepted, but "
                "nearby raw RS20/green scalar retunes are frozen. This tests their "
                "orthogonal interaction: established RS20 leadership on a red/non-green "
                "signal day."
            ),
            "exp-20260516-047": (
                "Rejected broad relative-weakness haircut; this is not a mirror "
                "haircut and requires RS20 leadership, so it targets pullback "
                "absorption rather than weak momentum."
            ),
            "exp-20260517-011": (
                "Rejected overstacked generic-boost no-confirmation haircut was "
                "old-window sample-thin; this state is different and is present "
                "across all three windows."
            ),
            "event_rotation": (
                "exp-20260517-010 remains the strongest paper event lane, but its "
                "next step is forward/trade-adapter maturation, not another frozen "
                "notional sweep."
            ),
        },
        "3_single_causal_variable": (
            "rs20_red_pullback_leader_risk_multiplier with fixed state definition"
        ),
        "4_acceptance_standard": (
            ...
            "at least six affected signals across all three windows, and max drawdown "
            "drift <= 0.5 pp."
        ),
        "5_reproducibility": (
    ...
    payload["llm_metrics"] = {
        "used_llm": False,
        "blocker_relation": (
            "LLM soft-ranking and SEC semantic branches remain sparse or "
            "concentration-limited for current three-window alpha judgment; this "
            "deterministic allocation state does not depend on them."
        ),
    }
    payload["production_impact"].update(
        {
            ...
            "promotion_requirement": (
                "If accepted, add the RS20 red-pullback state and sizing key in "
    ...
    payload["why_not_other_changes"] = (
        "LLM soft-ranking remains field/coverage-limited and the latest SEC "
        "semantic branch failed concentration. Candidate-pool expansion has "
        "recently added old-window noise. Event rotation is promising but should "
        "mature forward/trade-adapter evidence before more same-sample scalar work. "
        "This tests one deterministic pullback-leadership allocation state."
    )
    payload["known_risks"] = [
        "Some non-green RS20 leaders are fading rather than absorbing pullbacks.",
        "Commodity rows can contribute to this state; sector concentration must be reviewed before promotion.",

"""exp-20260606-009: broad Companyfacts growth acceleration + RS.

Replay-only alpha search. This tests whether filed-date-safe SEC Companyfacts
growth acceleration, not just static dual growth or loss-to-profit inflection,
can identify a cleaner broad default-off paper candidate pool when confirmed by
liquid OHLCV relative strength.

No production adapter, shared policy, live/default orders, ranking, sizing,
exits, LLM/news path, or watchlist behavior is changed. No JavaScript is used.
"""

from __future__ import annotations

from pathlib import Path
from typing import Any

import pandas as pd

import exp_20260605_011_broad_companyfacts_dual_growth_rs_candidate_pool as base


_BASE_GATE4 = base._gate4
_BASE_EXPERIMENT_LOG_RECORD = base._experiment_log_record

EXP_ID = "exp-20260606-009"
STEM = "broad_companyfacts_growth_acceleration_rs"
TRIAL_FAMILY = "broad_companyfacts_growth_acceleration_candidate_pool"
TRIAL_VARIANT_ID = "growth_acceleration_rs_top1_v1"
CHANGED_VARIABLE = "broad_companyfacts_growth_acceleration_rs_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

MIN_REVENUE_YOY_GROWTH = 0.10
MIN_REVENUE_ACCELERATION = 0.05
MIN_PROFIT_YOY_GROWTH = 0.10
MIN_PROFIT_ACCELERATION = 0.10
MIN_RET20_EXCESS_SPY = 0.02
MIN_VOLUME_RATIO_20D = 1.00

REPO_ROOT = base.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXP_ID
OUT_JSON = OUT_DIR / f"exp_20260606_009_{STEM}.json"
BEFORE_JSON = OUT_DIR / f"{STEM}_before_aggregate.json"
AFTER_JSON = OUT_DIR / f"{STEM}_after_aggregate.json"
LOG_JSON = REPO_ROOT / "experiments" / "logs" / f"{EXP_ID}.json"
TICKET_JSON = REPO_ROOT / "experiments" / "tickets" / f"{EXP_ID}.json"
CARD_MD = REPO_ROOT / "experiments" / "cards" / f"{EXP_ID}.md"
ARTIFACT_MD = REPO_ROOT / "experiments" / "artifacts" / f"{EXP_ID}_{STEM}.md"

PRODUCTION_IMPACT = {
    "trade_enabled": False,
    "alters_orders": False,
    "adapter_status": "replay_only_no_live_adapter",
    "shared_policy_changed": False,
    "backtester_adapter_changed": False,
    "run_adapter_changed": False,
    "replay_only": True,
    "parity_test_added": False,
    "production_signal_path_changed": False,
    "production_orders_changed": False,
    "production_watchlist_changed": False,
    "alters_signal_generation": False,
    "alters_candidate_ranking": False,
    "alters_sizing": False,
    "alters_exits": False,
    "parity_note": (
        "This runner changes no production code. A positive result would require "
        "a separate shared default-off Companyfacts growth-acceleration adapter, "
        "daily production exposure of the same filed-date-safe acceleration "
        "fields, warehouse/snapshot replay parity, and focused tests before any "
        "report queue, paper ledger, candidate priority, watchlist, sizing, or "
        "order surface could change."
    ),
}


def _previous_growth_row(
    growth_index: dict[str, dict[str, list[dict[str, Any]]]],
    ticker: str,
    canonical: str,
    latest_row: dict[str, Any],
) -> dict[str, Any] | None:
    rows = growth_index.get(ticker, {}).get(canonical) or []
    latest_asof = str(latest_row.get("asof_date") or "")
    previous = None
    for row in rows:
        asof = str(row.get("asof_date") or "")
        if not asof or asof >= latest_asof:
            break
        if row.get("yoy_growth") is not None:
            previous = row
    return previous


def _accelerating_row(
    growth_index: dict[str, dict[str, list[dict[str, Any]]]],
    ticker: str,
    canonical: str,
    signal_day: str,
    *,
    min_growth: float,
    min_acceleration: float,
) -> dict[str, Any] | None:
    latest = base._latest_growth_row(growth_index, ticker, canonical, signal_day)
    if latest is None:
        return None
    previous = _previous_growth_row(growth_index, ticker, canonical, latest)
    if previous is None:
        return None
    latest_growth = float(latest["yoy_growth"])
    previous_growth = float(previous["yoy_growth"])
    acceleration = latest_growth - previous_growth
    if latest_growth < min_growth or acceleration < min_acceleration:
        return None
    return {
        **latest,
        "previous_yoy_growth": previous_growth,
        "previous_asof_date": previous.get("asof_date"),
        "growth_acceleration": acceleration,
    }


def _profit_acceleration_row(
    growth_index: dict[str, dict[str, list[dict[str, Any]]]],
    ticker: str,
    signal_day: str,
) -> dict[str, Any] | None:
    candidates = []
    for canonical in ("eps_diluted", "eps_basic", "net_income"):
        row = _accelerating_row(
            growth_index,
            ticker,
            canonical,
            signal_day,
            min_growth=MIN_PROFIT_YOY_GROWTH,
            min_acceleration=MIN_PROFIT_ACCELERATION,
        )
        if row is None:
            continue
        if row.get("current_value") is None or float(row["current_value"]) <= 0.0:
            continue
        if row.get("prior_value") is None or float(row["prior_value"]) <= 0.0:
            continue
        priority = {"eps_diluted": 3, "eps_basic": 2, "net_income": 1}[canonical]
        candidates.append({**row, "canonical_priority": priority})
    if not candidates:
        return None
    return max(
        candidates,
        key=lambda row: (
            int(row["canonical_priority"]),
            float(row["growth_acceleration"]),
            float(row["yoy_growth"]),
            str(row["asof_date"]),
        ),
    )


def _score_candidate(
    *,
    revenue_growth: float,
    revenue_acceleration: float,
    profit_growth: float,
    profit_acceleration: float,
    ret20_excess_spy: float,
    close_location: float,
    volume_ratio_20d: float,
) -> float:
    return (
        min(max(revenue_growth, -1.0), 1.5)
        + 0.75 * min(max(revenue_acceleration, 0.0), 1.0)
        + min(max(profit_growth, -1.0), 1.5)
        + 0.75 * min(max(profit_acceleration, 0.0), 1.0)
        + 4.0 * ret20_excess_spy
        + close_location
        + 0.15 * min(volume_ratio_20d, 3.0)
    )


def _candidate_for_ticker_day(
    *,
    ticker: str,
    frame: pd.DataFrame,
    spy_frame: pd.DataFrame,
    signal_day: pd.Timestamp,
    growth_index: dict[str, dict[str, list[dict[str, Any]]]],
) -> dict[str, Any] | None:
    pos = base._frame_pos(frame, signal_day)
    spy_pos = base._frame_pos(spy_frame, signal_day)
    if pos is None or spy_pos is None or pos < 20 or spy_pos < 20:
        return None

    signal_day_s = str(signal_day.date())
    revenue = _accelerating_row(
        growth_index,
        ticker,
        "revenue",
        signal_day_s,
        min_growth=MIN_REVENUE_YOY_GROWTH,
        min_acceleration=MIN_REVENUE_ACCELERATION,
    )
    profit = _profit_acceleration_row(growth_index, ticker, signal_day_s)
    if revenue is None or profit is None:
        return None
    if revenue.get("current_value") is None or float(revenue["current_value"]) <= 0.0:
        return None

    close = float(frame["Close"].iloc[pos])
    if close < base.MIN_PRICE:
        return None
    adv20 = base._avg_dollar_volume(frame, pos)
    if adv20 is None or adv20 < base.MIN_AVG_DOLLAR_VOLUME_20D:
        return None
    volume_ratio_20d = base._volume_ratio(frame, pos)
    if volume_ratio_20d is None or volume_ratio_20d < MIN_VOLUME_RATIO_20D:
        return None
    close_location = base._close_location(frame, pos)
    if close_location is None or close_location < base.MIN_CLOSE_LOCATION:
        return None
    ret20 = base._ret(frame, pos, 20)
    spy_ret20 = base._ret(spy_frame, spy_pos, 20)
    if ret20 is None or spy_ret20 is None:
        return None
    ret20_excess_spy = ret20 - spy_ret20
    if ret20_excess_spy < MIN_RET20_EXCESS_SPY:
        return None

    revenue_growth = float(revenue["yoy_growth"])
    revenue_acceleration = float(revenue["growth_acceleration"])
    profit_growth = float(profit["yoy_growth"])
    profit_acceleration = float(profit["growth_acceleration"])
    score = _score_candidate(
        revenue_growth=revenue_growth,
        revenue_acceleration=revenue_acceleration,
        profit_growth=profit_growth,
        profit_acceleration=profit_acceleration,
        ret20_excess_spy=ret20_excess_spy,
        close_location=close_location,
        volume_ratio_20d=volume_ratio_20d,
    )
    metadata = {
        "companyfacts_revenue_yoy_growth": round(revenue_growth, 6),
        "companyfacts_revenue_previous_yoy_growth": round(float(revenue["previous_yoy_growth"]), 6),
        "companyfacts_revenue_growth_acceleration": round(revenue_acceleration, 6),
        "companyfacts_profit_yoy_growth": round(profit_growth, 6),
        "companyfacts_profit_previous_yoy_growth": round(float(profit["previous_yoy_growth"]), 6),
        "companyfacts_profit_growth_acceleration": round(profit_acceleration, 6),
        "companyfacts_profit_canonical": profit["canonical"],
        "companyfacts_revenue_asof_date": revenue["asof_date"],
        "companyfacts_revenue_previous_asof_date": revenue["previous_asof_date"],
        "companyfacts_profit_asof_date": profit["asof_date"],
        "companyfacts_profit_previous_asof_date": profit["previous_asof_date"],
        "companyfacts_revenue_asof_age_days": revenue["asof_age_days"],
        "companyfacts_profit_asof_age_days": profit["asof_age_days"],
        "companyfacts_revenue_form": revenue.get("current_form"),
        "companyfacts_profit_form": profit.get("current_form"),
        "ret20": round(ret20, 6),
        "spy_ret20": round(spy_ret20, 6),
        "ret20_excess_spy": round(ret20_excess_spy, 6),
        "close_location": round(close_location, 6),
        "avg_dollar_volume_20d": round(adv20, 2),
        "volume_ratio_20d": round(volume_ratio_20d, 6),
        "candidate_score": round(score, 6),
        "source": "BROAD_COMPANYFACTS_GROWTH_ACCELERATION_RS_PAPER",
    }
    return base._candidate_trade(ticker, frame, signal_day, pos, metadata)


def _gate4(
    aggregate_comparison: dict[str, Any],
    results: list[dict[str, Any]],
    target_summary: dict[str, Any],
) -> dict[str, Any]:
    gate = _BASE_GATE4(aggregate_comparison, results, target_summary)
    gate["decision"] = (
        "positive_replay_lead_not_promoted_requires_growth_acceleration_shared_adapter"
        if gate["passed"]
        else "rejected_broad_companyfacts_growth_acceleration_rs_candidate_pool"
    )
    return gate


def _experiment_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    record = _BASE_EXPERIMENT_LOG_RECORD(payload)
    record.update(
        {
            "experiment_id": EXP_ID,
            "hypothesis": (
                "Broad Companyfacts YoY revenue plus earnings acceleration, "
                "confirmed by liquid relative strength, may identify cleaner "
                "default-off paper candidates than static dual-growth or "
                "loss-to-profit inflection."
            ),
            "change_summary": (
                "Tested broad SEC Companyfacts revenue/profit YoY acceleration "
                "plus OHLCV confirmation as a replay-only default-off paper "
                "candidate source."
            ),
            "trial_family": TRIAL_FAMILY,
            "trial_variant_id": TRIAL_VARIANT_ID,
            "changed_variable": CHANGED_VARIABLE,
            "nearby_prior_experiments": [
                "exp-20260605-011",
                "exp-20260605-014",
                "exp-20260605-015",
                "exp-20260605-022",
                "exp-20260605-026",
            ],
            "multiple_testing_risk_bucket": "moderate",
            "new_evidence_type": "new_production_visible_field",
            "component": base._repo_rel(Path(__file__)),
            "production_impact": PRODUCTION_IMPACT,
            "decision": payload["gate4"]["decision"],
            "rejection_reason": ";".join(payload["gate4"]["failed_reasons"])
            if payload["gate4"]["failed_reasons"]
            else None,
            "next_retry_requires": [
                "closed forward replacement-value rows",
                "proof that growth acceleration is incremental to static growth and ret20 momentum",
                "shared default-off adapter and parity tests before promotion",
                "avoid nearby Companyfacts acceleration threshold/scalar retunes on this frozen sample",
            ],
        }
    )
    return record


def _patch_base() -> None:
    base.EXP_ID = EXP_ID
    base.STEM = STEM
    base.TRIAL_FAMILY = TRIAL_FAMILY
    base.CHANGED_VARIABLE = CHANGED_VARIABLE
    base.RULE_VERSION = RULE_VERSION
    base.OUT_DIR = OUT_DIR
    base.OUT_JSON = OUT_JSON
    base.BEFORE_JSON = BEFORE_JSON
    base.AFTER_JSON = AFTER_JSON
    base.LOG_JSON = LOG_JSON
    base.TICKET_JSON = TICKET_JSON
    base.CARD_MD = CARD_MD
    base.ARTIFACT_MD = ARTIFACT_MD
    base.PRODUCTION_IMPACT = PRODUCTION_IMPACT
    base.MIN_REVENUE_YOY_GROWTH = MIN_REVENUE_YOY_GROWTH
    base.MIN_PROFIT_YOY_GROWTH = MIN_PROFIT_YOY_GROWTH
    base.MIN_RET20_EXCESS_SPY = MIN_RET20_EXCESS_SPY
    base.MIN_VOLUME_RATIO_20D = MIN_VOLUME_RATIO_20D
    base._candidate_for_ticker_day = _candidate_for_ticker_day
    base._score_candidate = _score_candidate
    base._gate4 = _gate4
    base._experiment_log_record = _experiment_log_record


def main() -> None:
    _patch_base()
    payload = base.build_payload()
    payload["preflight"] = {
        "alpha_hypothesis": (
            "Broad Companyfacts YoY revenue plus earnings acceleration, "
            "confirmed by liquid relative strength, may identify cleaner "
            "default-off paper candidates than static dual-growth or "
            "loss-to-profit inflection."
        ),
        "category": "entry_candidate_pool",
        "nearby_prior_experiments": [
            "exp-20260605-011",
            "exp-20260605-014",
            "exp-20260605-015",
            "exp-20260605-022",
            "exp-20260605-026",
        ],
        "single_causal_variable": CHANGED_VARIABLE,
        "success_standard": (
            "Canonical three-window before/after aggregate EV and PnL must "
            "improve, no window EV/PnL regression, max drawdown drift <= "
            f"{base.MAX_DRAWDOWN_WORSE}, target trades >= {base.MIN_TARGET_TRADES}, "
            "all three windows represented, concentration within guardrails."
        ),
        "reproducible_if_failed": True,
    }
    payload["parameters"].update(
        {
            "min_revenue_acceleration": MIN_REVENUE_ACCELERATION,
            "min_profit_acceleration": MIN_PROFIT_ACCELERATION,
            "daily_selection": "top_1_by_fixed_growth_acceleration_rs_score",
            "profit_canonical_priority": ["eps_diluted", "eps_basic", "net_income"],
        }
    )
    payload["next_retry_requires"] = [
        "closed forward replacement-value rows",
        "proof that growth acceleration is incremental to static growth and ret20 momentum",
        "shared default-off adapter and parity tests before promotion",
        "avoid nearby Companyfacts acceleration threshold/scalar retunes on this frozen sample",
    ]
    payload["related_files"] = [
        base._repo_rel(Path(__file__)),
        base._repo_rel(OUT_JSON),
        base._repo_rel(LOG_JSON),
        base._repo_rel(ARTIFACT_MD),
        base._repo_rel(TICKET_JSON),
        base._repo_rel(base.GROWTH_PATH),
    ]
    base._write_json(OUT_JSON, payload)
    base._write_json(LOG_JSON, _experiment_log_record(payload))
    base._write_json(BEFORE_JSON, payload["aggregate"]["before"])
    base._write_json(AFTER_JSON, payload["aggregate"]["after"])
    base._write_artifact(payload)
    artifact_text = ARTIFACT_MD.read_text(encoding="utf-8")
    artifact_text = artifact_text.replace(
        f"# {EXP_ID} Broad Companyfacts Dual-Growth RS Candidate Pool",
        f"# {EXP_ID} Broad Companyfacts Growth Acceleration RS Candidate Pool",
    )
    artifact_text = artifact_text.replace(
        (
            ".\\.venv\\Scripts\\python.exe -B "
            "quant\\experiments\\exp_20260605_011_broad_companyfacts_dual_growth_rs_candidate_pool.py"
        ),
        (
            ".\\.venv\\Scripts\\python.exe -B "
            "quant\\experiments\\exp_20260606_009_broad_companyfacts_growth_acceleration_rs.py"
        ),
    )
    ARTIFACT_MD.write_text(artifact_text, encoding="utf-8")
    base._update_ticket(payload)
    base._update_registry(payload)
    base._append_experiment_log(_experiment_log_record(payload))
    print(
        base.json.dumps(
            {
                "experiment_id": EXP_ID,
                "decision": payload["gate4"]["decision"],
                "aggregate": payload["aggregate"]["comparison"],
                "target_summary": {
                    "target_trade_count": payload["target_summary"]["target_trade_count"],
                    "target_trade_pnl_usd": payload["target_summary"]["target_trade_pnl_usd"],
                    "max_single_positive_share": payload["target_summary"][
                        "max_single_positive_share"
                    ],
                    "positive_pnl_hhi": payload["target_summary"]["positive_pnl_hhi"],
                },
                "gate4_failed_reasons": payload["gate4"]["failed_reasons"],
                "anti_js": "No JavaScript was used.",
            },
            indent=2,
            sort_keys=True,
        )
    )


if __name__ == "__main__":
    main()

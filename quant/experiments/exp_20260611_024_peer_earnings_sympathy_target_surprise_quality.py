"""exp-20260611-024: peer earnings sympathy target surprise-quality scout.

Replay-only alpha search. It tests one event-peer relation: after a positive
post-earnings issuer reaction, admit one exact-industry liquid peer only when
that peer confirms sympathetically on the same signal day without being a
fully extended momentum chase and has its own repeat positive earnings-surprise
history.
...
EXPERIMENT_ID = "exp-20260611-024"
STEM = "peer_earnings_sympathy_target_surprise_quality"
TRIAL_FAMILY = "peer_earnings_sympathy_target_surprise_quality_candidate_pool"
TRIAL_VARIANT_ID = "peer_earnings_sympathy_target_surprise_quality_top1_next_open_10d_v1"
CHANGED_VARIABLE = "peer_earnings_sympathy_target_surprise_quality_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260611_024_{STEM}.json"
...
MIN_PEER_SIGNAL_RETURN = 0.0030
MIN_PEER_SIGNAL_EXCESS_VS_SPY = 0.0040
MIN_PEER_EVENT_TO_SIGNAL_EXCESS_VS_SPY = 0.0040
MAX_PEER_EVENT_TO_SIGNAL_EXCESS_VS_SPY = 0.0600
...
MIN_CANDIDATE_SURPRISE_HISTORY_COUNT = 4
MIN_CANDIDATE_POSITIVE_SURPRISE_COUNT = 3
MIN_CANDIDATE_POSITIVE_SURPRISE_RATIO = 0.75
MIN_CANDIDATE_AVG_HISTORICAL_SURPRISE_PCT = 0.0
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "target_surprise_history_not_incremental",
        "sympathy_reaction_is_generic_momentum",
    ],
    "confidence_reason": (
        "Peer earnings transfer failed when it was broad same-sector "
        "similarity, and the upcoming-earnings prewarm variant added a DTE "
        "proxy. The previous same-day exact-industry sympathy scout also "
        "failed. This run keeps that event-peer relation fixed and adds only "
        "a PIT target-quality discriminator: the peer must have its own repeat "
        "positive surprise history."
    ),
    "recorded_at": "2026-06-11T18:11:39+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "earnings event, exact-industry relation, same-day peer sympathy "
        "reaction, target peer surprise-history quality, bounded OHLCV "
...
def _target_surprise_quality_context(info: dict[str, Any]) -> dict[str, Any]:
    raw_history = info.get("historical_surprise_pct") or []
    history = [
        value
        for value in (
            earnings_parent.earnings_helper._float_or_none(item)
            for item in raw_history
        )
        if value is not None
    ]
    history_count = len(history)
    positive_count = sum(1 for value in history if value > 0.0)
    positive_ratio = positive_count / history_count if history_count else 0.0
    avg_surprise = earnings_parent.earnings_helper._float_or_none(
        info.get("avg_historical_surprise_pct")
    )
    if avg_surprise is None and history:
        avg_surprise = sum(history) / len(history)
    passed = (
        history_count >= MIN_CANDIDATE_SURPRISE_HISTORY_COUNT
        and positive_count >= MIN_CANDIDATE_POSITIVE_SURPRISE_COUNT
        and positive_ratio >= MIN_CANDIDATE_POSITIVE_SURPRISE_RATIO
        and (avg_surprise or 0.0) >= MIN_CANDIDATE_AVG_HISTORICAL_SURPRISE_PCT
    )
    return {
        "candidate_surprise_quality_passed": passed,
        "candidate_surprise_history_count": history_count,
        "candidate_positive_surprise_count": positive_count,
        "candidate_positive_surprise_ratio": round(positive_ratio, 6),
        "candidate_avg_historical_surprise_pct": round(avg_surprise, 6)
...
def _candidate_for_peer(
    *,
    ...
    surprise_context = _target_surprise_quality_context(peer_info)
    if surprise_context["candidate_surprise_quality_passed"] is not True:
        return None

    score = (
        1.70 * issuer_event_excess
        + 1.45 * signal_excess
        + 1.05 * peer_event_to_signal_excess
        + 0.55 * ret20_excess
        + 0.35 * close_location
    ...
    return {
        "date": signal_date,
        "ticker": peer_ticker,
        "source": "PEER_EARNINGS_SYMPATHY_TARGET_SURPRISE_QUALITY_PAPER",
        ...
        "candidate_days_to_earnings_observed": int(peer_dte)
        if peer_dte is not None
        else None,
        "candidate_earnings_snapshot_source_date": peer_snapshot_date,
        "candidate_next_earnings_date": peer_info.get("next_earnings_date"),
        **surprise_context,
        "candidate_surprise_quality_rule_version": RULE_VERSION,
        ...
        "peer_relation_source": (
            "exact_industry_positive_earnings_same_day_sympathy_"
            "target_surprise_quality"
        ),
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_peer_sympathy_target_surprise_quality"
        if gate["passed"]
        else "rejected_peer_sympathy_target_surprise_quality_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "Exact-industry peer earnings sympathy may work only when "
                "the target peer has its own repeat positive surprise "
                "history, separating real earnings-quality sympathy from "
                "generic peer momentum."
            ),
            ...
            "new_evidence_type": "target_peer_positive_surprise_history_quality_discriminator",
            ...
            "peer_sympathy_contexts_by_window": payload["pressure_contexts_by_window"],
            "peer_sympathy_context_samples_by_window": payload[
                "pressure_context_samples_by_window"
            ],
            "negative_reflection": (
                "If rejected, the likely reason is that exact-industry "
                "post-earnings information plus target surprise history still "
                "does not transfer cleanly into same-day peer candidates, or "
                "the target quality field is already priced and only thins "
                "sample. Do not retry by sweeping issuer reaction, peer "
                "reaction, target surprise thresholds, close-location, "
                "volume, hold-day, cooldown, or notional thresholds on these "
                "frozen windows."
            ),
            "next_evidence_needed": (
                "A retry needs materially new PIT relation evidence, such as "
                "analyst-count revision trajectory, estimate dispersion, true "
                "customer/supplier links, audited peer expectation changes, "
    ...
    payload["parameters"] = {
        ...
        "min_candidate_surprise_history_count": MIN_CANDIDATE_SURPRISE_HISTORY_COUNT,
        "min_candidate_positive_surprise_count": MIN_CANDIDATE_POSITIVE_SURPRISE_COUNT,
        "min_candidate_positive_surprise_ratio": MIN_CANDIDATE_POSITIVE_SURPRISE_RATIO,
        "min_candidate_avg_historical_surprise_pct": (
            MIN_CANDIDATE_AVG_HISTORICAL_SURPRISE_PCT
        ),
    ...
    payload["backtest_protocol"].update(
        {
            "source": (
                ...
                "replay-only post-earnings event to same-day exact-industry "
                "peer-sympathy paper overlay"
            ),
            ...
            "execution_model": (
                "Issuer event confirmation uses only daily earnings snapshots "
                "available as of signal date. Peer admission uses same-day "
                "close-of-day OHLCV plus prior history only, and target peer "
                "surprise-history quality uses the latest earnings snapshot "
                "known by the signal date. Paper entry is next available open "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            ...
            "may transfer into exact-industry peers that confirm "
            "sympathetically on the same day, but only when the target peer "
            "has its own repeat positive surprise history."
        ),
        "2_history_check": {
            "exp-20260607-013": (
                "Upcoming-earnings peer prewarm was rejected; this run removes "
                "the DTE alpha condition and tests direct event-day transfer "
                "with target surprise-history quality."
            ),
            "exp-20260609-024": (
                "Same-day early peer earnings sympathy was rejected; this run "
                "keeps its relation and OHLCV gates fixed while adding only "
                "the target peer positive-surprise quality discriminator."
            ),
            "exp-20260611-016": (
                "Peer revision shock was rejected; this run does not use "
                "revision transfer and instead tests repeat target surprise "
                "quality as a cleaner event-underreaction condition."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260611_024_peer_earnings_sympathy_target_surprise_quality.py"
        ),
    }
    ...
    payload["gate2"]["runtime_fields"] = [
        ...
        "daily earnings snapshot avg_historical_surprise_pct",
    ...
    payload["gate3"]["note"] = (
        "No new core filter or entry rule was added. The peer earnings "
        "sympathy target surprise-quality source is additive default-off "
    ...
    payload["post_run_reflection"] = (
        {
            "why_result_happened": (
                ...
                "replay lead, suggesting target peer positive-surprise "
                "history separated higher-quality same-day exact-industry "
                "peer sympathy from generic peer transfer."
            ),
            "forbidden_near_neighbor_retry": (
                "Do not immediately retune issuer reaction, peer reaction, "
                "target surprise thresholds, top-N, hold-day, cooldown, or "
                "notional thresholds. Promote only through a shared "
                "default-off adapter first."
            ),
        ...
        if payload["gate4"]["passed"]
            ...
            "why_result_happened": (
                ...
                "target surprise-history gate either thinned useful peers or "
                "did not add incremental information beyond same-day "
                "peer-sympathy momentum under next-open entry, 10-day exit, "
                "costs, and slippage."
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retry by sweeping issuer reaction, signal return, "
                "event-to-signal return, target surprise thresholds, "
                "close-location, volume, volatility, top-N, hold-day, "
                "cooldown, or paper notional thresholds on these frozen "
                "windows."
            ),
            "new_evidence_required": (
                "A retry requires materially different PIT evidence such as "
                "analyst-count/estimate trajectory, estimate dispersion, "
                "customer/supplier links, peer expectation revision, or "
                "closed forward replacement rows."
            ),
        }
    )
    payload["interpretation"] = (
        "The peer earnings sympathy target surprise-quality source cleared "
        ...
        if payload["gate4"]["passed"]
            ...
            "The peer earnings sympathy target surprise-quality source did "
            "not clear Gate 4; do not promote or locally retune this "
            "event-peer quality relation on the frozen windows."
        )
    )
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Peer Earnings Sympathy Target Surprise Quality",
            "",

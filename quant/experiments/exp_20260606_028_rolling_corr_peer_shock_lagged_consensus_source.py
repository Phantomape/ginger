"""exp-20260606-028: rolling-corr peer shock as lagged consensus source.

Replay-only alpha search. Tests one causal variable: add the accepted
production-visible rolling-correlation peer-shock paper rows as a new
independent source family in the accepted lagged free-data consensus scout.
...
import rolling_corr_peer_shock_paper_sleeve as peer_shock  # noqa: E402


EXPERIMENT_ID = "exp-20260606-028"
STEM = "rolling_corr_peer_shock_lagged_consensus_source"
TRIAL_FAMILY = "accepted_free_data_cross_source_consensus_new_independent_source_family"
TRIAL_VARIANT_ID = "rolling_corr_peer_shock_lagged_consensus_source_family_v1"
CHANGED_VARIABLE = "rolling_corr_peer_shock_source_family_added_to_accepted_lagged_consensus_v1"
RULE_VERSION = "rolling_corr_peer_shock_lagged_consensus_source_family_v1"

PEER_SOURCE_NAME = peer_shock.SLEEVE_NAME
PEER_SOURCE_FAMILY = "rolling_corr_peer_shock"
PEER_SOURCE_EXPERIMENT_ID = "exp-20260606-025"
PEER_REPLAY_LEAD_ID = "exp-20260606-024"
PEER_REPLAY_ARTIFACT = (
    REPO_ROOT
    / "data"
    / "experiments"
    / PEER_REPLAY_LEAD_ID
...
OUT_JSON = OUT_DIR / f"exp_20260606_028_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "peer_shock_redundant_with_existing_sources",
    ],
    "confidence_reason": (
        "Peer-shock has just cleared Gate 4 and has a shared helper, but "
        "source-family additions often fail the accepted lagged comparator guard."
    ),
    "recorded_at": "2026-06-06T22:10:20+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "rolling_corr_peer_shock paper snapshot as the same source family in "
        "both replay and daily production, with parity tests, before any daily "
        "report queue, candidate priority, paper notional, watchlist, or order "
...
def _patch_source_family_context() -> None:
    ...
    same_day.SOURCE_FAMILIES[PEER_SOURCE_NAME] = PEER_SOURCE_FAMILY
    same_day.SOURCE_EXPERIMENT_IDS[PEER_SOURCE_NAME] = PEER_SOURCE_EXPERIMENT_ID


def _source_row_from_peer_trade(trade: dict[str, Any], *, label: str) -> dict[str, Any] | None:
    signal_date = str(trade.get("signal_date") or trade.get("date") or "")[:10]
    ticker = str(trade.get("ticker") or "").upper()
    if not signal_date or not ticker:
        return None
    return {
        "source_name": PEER_SOURCE_NAME,
        "source_experiment_id": PEER_SOURCE_EXPERIMENT_ID,
        "source_family": PEER_SOURCE_FAMILY,
        "date": signal_date,
        ...
        "rolling_corr_60d": trade.get("rolling_corr_60d"),
        "peer_ticker": trade.get("peer_ticker"),
        "peer_signal_day_return": trade.get("peer_signal_day_return"),
        "peer_relative_vs_spy": trade.get("peer_relative_vs_spy"),
        ...
        "helper_rule_version": peer_shock.RULE_VERSION,
        "helper_source_rule_version": peer_shock.SOURCE_RULE_VERSION,
        "source_artifact": _repo_rel(PEER_REPLAY_ARTIFACT),
        "historical_replay_experiment_id": PEER_REPLAY_LEAD_ID,
...
def _peer_source_rows_by_window() -> tuple[
    dict[str, dict[tuple[str, str], list[dict[str, Any]]]],
    dict[str, Any],
]:
    payload = _load_json(PEER_REPLAY_ARTIFACT)
    ...
    diagnostics: dict[str, Any] = {
        "source_path": _repo_rel(PEER_REPLAY_ARTIFACT),
        "source_name": PEER_SOURCE_NAME,
        "source_family": PEER_SOURCE_FAMILY,
        "source_experiment_id": PEER_SOURCE_EXPERIMENT_ID,
        "historical_replay_experiment_id": PEER_REPLAY_LEAD_ID,
    ...
    for label in same_day.prior.base.WINDOWS:
        ...
        for trade in trades:
            source_row = _source_row_from_peer_trade(trade, label=label)
...
def _source_addition_summary(
    ...
    selected_with_peer = [
        row for row in all_selected if PEER_SOURCE_NAME in (row.get("source_names") or [])
    ]
    current_peer = [
        row
        for row in selected_with_peer
        if PEER_SOURCE_NAME in (row.get("current_source_names") or [])
    ]
    prior_peer = [
        row
        for row in selected_with_peer
        if any(
            source_row.get("source_name") == PEER_SOURCE_NAME
    ...
    return {
        "added_source_name": PEER_SOURCE_NAME,
        "added_source_family": PEER_SOURCE_FAMILY,
        "added_source_rows_by_window": dict(sorted(added_key_counts.items())),
        "selected_trade_count": len(all_selected),
        "selected_with_peer_source_count": len(selected_with_peer),
        "selected_with_current_peer_count": len(current_peer),
        "selected_with_prior_peer_count": len(prior_peer),
        "selected_with_peer_pnl_usd": round(
            sum(_safe_float(row.get("pnl")) for row in selected_with_peer),
            2,
        ),
...
def _gate4(
    ...
    source_selected = int(source_summary["selected_with_peer_source_count"]) > 0
    gates = {
        **base_gate["gates"],
    ...
    if passed:
        decision = "positive_replay_lead_requires_peer_shock_lagged_consensus_shared_adapter"
        rationale = (
            "Adding rolling-correlation peer-shock as an independent source family "
            "improved both core and current accepted lagged consensus across all "
            "three windows. Promotion would require a shared adapter and parity "
            "tests first."
        )
    elif not source_selected:
        decision = "rejected_peer_shock_lagged_consensus_no_selected_source_rows"
        rationale = "The peer-shock source produced no selected lagged-consensus trades."
        ...
        decision = "rejected_peer_shock_lagged_consensus_gate4_failed"
...
def _preflight_payload() -> dict[str, Any]:
    return {
        "alpha_hypothesis": (
            "Accepted rolling-correlation peer-shock paper rows may improve the "
            "accepted lagged free-data consensus scout when treated as a new "
            "independent production-visible source family."
        ),
        "category": "entry/candidate_pool",
        "playbook_alignment": (
            "Matches the playbook preference for production-visible default-off "
            "candidate-pool adapters. It avoids LLM soft-ranking, Companyfacts "
            "threshold retunes, state-surface allocation retunes, and raw OHLCV "
            "winner expansion."
        ),
        ...
        "prior_difference": (
            "exp-20260606-024 accepted peer-shock as a standalone default-off "
            "paper source. exp-20260604-008/009 accepted lagged independent "
            "source-family consensus. exp-20260605-008 added SEC negative "
            "reaction but failed the accepted-comparator guard. This run tests "
            "the newly accepted relation alpha as an independent consensus source, "
            "not a peer-shock threshold, hold, cooldown, top-N, or notional retune."
        ),
        ...
        "reproducibility": (
            ...
            "quant\\experiments\\exp_20260606_028_rolling_corr_peer_shock_lagged_consensus_source.py"
        ),
    }
...
def _experiment_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "change_summary": (
            "Added accepted rolling-correlation peer-shock paper rows as a "
        ...
        "new_evidence_type": "accepted_production_visible_rolling_corr_peer_shock_source_family",
        ...
        "negative_reflection": (
            "If rejected, the accepted peer-shock source likely overlaps the same "
            "risk-on dates/tickers that the lagged consensus already captures, so "
            "it adds confirmation without changing the selected top-1 paper trade "
            "set enough to beat the accepted comparator. Do not retry with local "
            "peer-shock threshold, hold, cooldown, notional, or top-N sweeps; the "
            "next retry needs forward replacement-value evidence or a materially "
            "different independent source relation."
        ),
        "next_retry_requires": [
        ...
        "related_files": [
            ...
            _repo_rel(PEER_REPLAY_ARTIFACT),
...
def _write_card(payload: dict[str, Any]) -> None:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Peer-Shock Lagged Consensus Source",
        "",
        "## Decision",
        "",
        ...
        f"- Selected trades with peer-shock source: `{source['selected_with_peer_source_count']}`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Source Diagnostics",
            "",
            f"- Peer-shock source rows by window: `{source['added_source_rows_by_window']}`",
            f"- Current peer confirmations selected: `{source['selected_with_current_peer_count']}`",
            f"- Prior peer confirmations selected: `{source['selected_with_prior_peer_count']}`",
            "",
            "## Production Boundary",
            "",
...
def main() -> None:
    ...
    peer_rows, peer_diagnostics = _peer_source_rows_by_window()
    extended_source_rows = _merge_source_rows(base_source_rows, peer_rows)
    ...
    source_summary = _source_addition_summary(target_trades_by_window, peer_rows)
    ...
    payload = {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "entry/candidate_pool: accepted rolling-correlation peer-shock "
                "paper rows may improve lagged accepted-source consensus quality."
            ),
            "2_history_check": {
                "exp-20260606-024": (
                    "Core-flow confirmed peer-shock replay lead improved all "
                    "three windows by +0.3845 EV / +$6,107.66 with 48 paper trades."
                ),
                "exp-20260606-025/026": (
                    "Promoted/wired the shared default-off peer-shock helper; "
                    "this run tests consensus source-family use, not helper thresholds."
                ),
        ...
        "source_files": {
            ...
            PEER_SOURCE_NAME: _repo_rel(PEER_REPLAY_ARTIFACT),
        },
        "rule": {
            "rule_version": RULE_VERSION,
            "added_source_name": PEER_SOURCE_NAME,
        ...
        "peer_source_diagnostics": peer_diagnostics,

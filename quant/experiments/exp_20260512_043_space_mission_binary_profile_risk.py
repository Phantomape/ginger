"""exp-20260512-043: Space mission-binary event-guard risk allocation.
...
universe-registry event_guard_profile explicitly names mission-binary risk.
"""
...
EXPERIMENT_ID = "exp-20260512-043"
STEM = "space_mission_binary_profile_risk"
TARGET_PROFILE_TERMS = ("mission_binary",)
MISSION_PROFILE_RISK_SCALARS = (0.50, 0.75, 0.90, 1.00, 1.05, 1.075, 1.10, 1.25)
...
def _append_jsonl_for_this_experiment(path: Path, payload: dict[str, Any]) -> None:
    ...
    if path.exists():
        lines = [
            ...
            and '"experiment_id":"exp-20260512-043"' not in line
            and '"experiment_id": "exp-20260512-043"' not in line
        ]
    else:
        lines = []
...
def _field_check_mission_profiles() -> dict[str, Any]:
...
def _install_space_policy(
    mission_scalar: float,
    mission_gate: dict[str, Any],
    ...
    target_tickers = set(mission_gate["target_tickers"])
    profiles = mission_gate["profiles"]
    mission_adjustments: list[dict[str, Any]] = []
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                mission_counts["eligible_signal"] += 1
                shares_before = int(sizing.get("shares_to_buy") or 0)
                _scale_sizing(
                    sizing,
                    mission_scalar,
                    portfolio_value,
                    "space_mission_binary_profile_risk",
                )
                mission_adjustments.append(
                    {
                        "ticker": ticker,
                        "strategy": signal.get("strategy"),
                        "marker": "space_mission_binary_profile_risk",
                        "event_guard_profile": profiles.get(ticker),
                        "scalar": mission_scalar,
                ...
                signal = {
                    **signal,
                    "sizing": sizing,
                    "space_mission_binary_profile": profiles.get(ticker),
                    "space_mission_binary_profile_eligible": True,
                }
            out.append(signal)
        return out
...
def _run_variant(
    ...
    (
        ...
        basket_counts,
        day_counts,
    ) = _install_space_policy(mission_scalar, mission_gate, financing_gate, source_gate)
    try:
        by_window = {}
        for label, window in WINDOWS.items():
            before_mission = len(mission_adjustments)
            ...
            by_window[label] = {
                ...
                "space_mission_binary_profile_adjustment": _adjustment_summary(
                    mission_adjustments[before_mission:]
                ),
                ...
                "space_mission_binary_profile_signal_counts": dict(
                    sorted(mission_counts.items())
                ),
    ...
    return {
        "variant": name,
        "target_profile_terms": list(TARGET_PROFILE_TERMS),
        "target_tickers": mission_gate["target_tickers"],
        "space_mission_binary_profile_scalar": mission_scalar,
        "by_window": by_window,
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_mission_binary_profile_adjustment"]["adjusted_signal_count"]
    ...
    passed = (
        ...
        and variant["space_mission_binary_profile_scalar"] != 1.0
    )
    return {
        "passed": passed,
        ...
        "space_mission_binary_profile_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space mission-binary profile risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: risk scalar for official Space signals whose "
            "`event_guard_profile` contains mission_binary."
        ),
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | {variant['space_mission_binary_profile_scalar']:.3f} | "
            ...
            f"{gate['space_mission_binary_profile_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
            ...
            (
                ...
                "dPnL | Trades | Max DD | Survival | Mission signals |"
            ),
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for label in WINDOWS:
        ...
        adjusted = best["by_window"][label][
            "space_mission_binary_profile_adjustment"
    ...
    lines.extend(
        [
            ...
            "## Field Check",
            "",
            json.dumps(payload["gate2"]["mission_binary_profiles"], sort_keys=True),
            "",
            "## Interpretation",
            "",
...
def run() -> dict[str, Any]:
    ...
    mission_gate = _field_check_mission_profiles()
    if not mission_gate["passed"]:
        raise RuntimeError(f"Mission profile field check failed: {mission_gate}")
    ...
    variants = {}
    for scalar in MISSION_PROFILE_RISK_SCALARS:
        name = f"mission_binary_profile_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_mission_binary_profile_risk"
        if accepted
        else "rejected_space_mission_binary_profile_risk"
    )
    interpretation = (
        "The mission-binary event-guard profile scalar improved the accepted "
        ...
        if accepted
            ...
            "The mission-binary event-guard profile scalar did not clear the "
            "three-window gate on top of exp-20260512-041. Do not retry adjacent "
            "mission-binary registry-profile scalars on these frozen snapshots."
        )
    )
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "risk fields, the next orthogonal production-observable profile is "
            "mission-binary sensitivity. Lunar mission-binary signals may need a "
            "different default-off risk budget because the payoff distribution is "
            "less like broad contract/news validation."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: scale only official Space signals whose "
                "event_guard_profile includes mission_binary."
            ),
            "2_history_check": {
                "exp-20260512-041": (
                ...
                "exp-20260512-035": (
                    "Rejected data/defense theme-segment scalar; this uses a "
                    "different registry profile field."
                ),
            },
            "3_single_causal_variable": (
                "space_mission_binary_event_guard_profile_risk_scalar. Candidate "
            ...
            "4_acceptance_standard": (
                ...
                "and nonzero adjusted mission-profile signals."
            ),
            "5_reproducibility": (
                "Run .venv\\Scripts\\python.exe "
                "quant\\experiments\\exp_20260512_043_space_mission_binary_profile_risk.py"
            ),
        },
        "parameters": {
            ...
            "target_profile_terms": list(TARGET_PROFILE_TERMS),
            "target_tickers": mission_gate["target_tickers"],
            "event_guard_profiles": mission_gate["profiles"],
            "tested_mission_profile_scalars": list(MISSION_PROFILE_RISK_SCALARS),
        ...
        "gate2": {
            ...
            "mission_binary_profiles": mission_gate,
            "passed": (
                gate2_open["passed"]
                and source_gate["passed"]
                and financing_gate["passed"]
                and mission_gate["passed"]
            ),
        },
        "gate3": {
        ...
        "next_evidence_needed": (
            "Do not retry adjacent mission-binary event-guard profile scalars "
        ...
        "related_files": [
            "quant/experiments/exp_20260512_043_space_mission_binary_profile_risk.py",
            "data/experiments/exp-20260512-043/space_mission_binary_profile_risk.json",
            "docs/experiments/logs/exp-20260512-043.json",
            "docs/experiments/tickets/exp-20260512-043.json",
            "docs/experiments/artifacts/exp-20260512-043_space_mission_binary_profile_risk.md",

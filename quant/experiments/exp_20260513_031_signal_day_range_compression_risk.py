"""exp-20260513-031: signal-day range-compression core risk allocation.
...
the bottom quartile of the non-ETF/non-commodity stock universe.
...
EXPERIMENT_ID = "exp-20260513-031"
EXPERIMENT_SLUG = "signal_day_range_compression_risk"
MULTIPLIER_KEY = "signal_day_range_compression_risk_multiplier_applied"
...
def _range_compression_cutoff(
    ...
    if not values:
        return None
    values.sort()
    index = max(0, math.ceil(len(values) * COMPRESSION_FRACTION) - 1)
    return values[index]
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        cutoff = _range_compression_cutoff(features_dict)
        for sig in enriched:
            ...
            sig["signal_day_range_compression_cutoff"] = cutoff
            sig["signal_day_range_compression_state"] = bool(eligible)
        return enriched
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["signal_day_range_compression_baseline_shares"] = shares
    out["signal_day_range_compression_desired_shares"] = desired_shares
    out["signal_day_range_compression_cap_shares"] = cap_shares
    out["signal_day_range_compression_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("signal_day_range_compression_state") and sizing.get("shares_to_buy"):
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "signal_day_range_compression_cutoff": sig.get(
                                "signal_day_range_compression_cutoff"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Signal-Day Range-Compression Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_signal_day_range_compression_risk"
    )
    interpretation = (
        "Signal-day range-compressed core stock signals cleared the canonical three-window gate and should be promoted only through shared production/backtest policy."
        if passed
        else "Signal-day bottom-quartile range compression did not clear the canonical three-window gate; do not promote this state variable on the frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Already-qualified trend/breakout stock signals with low signal-day range versus ATR may be less exhausted and may deserve a small cap-aware risk top-up."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "signal_day_range_compression_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for trend/breakout stock signals whose PIT daily_range_vs_atr is in the same-day bottom quartile"
        ),
        "parameters": {
            "state_definition": {
                "strategies": ["trend_long", "breakout_long"],
                "excluded_sectors": sorted(EXCLUDED_SECTORS),
                "daily_range_vs_atr": "bottom 25% of feature-complete non-ETF/non-commodity stocks on that signal day",
            },
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "core risk allocation using a production-visible signal-day range-compression state"
            ),
            "2_history_check": {
                "recent_rs20_rs60_own_green": (
                    "nearby momentum/own-green overlays are exhausted or already accepted; this uses range/ATR compression rather than RS or candle color."
                ),
                "recent_gap_cushion_momentum_deceleration": (
                    "positive variants failed drawdown guardrails; this does not change targets or gap/green logic."
                ),
                "llm_soft_ranking": (
            ...
            "3_single_causal_variable": (
                "signal_day_range_compression_risk_multiplier with a fixed cross-sectional bottom-quartile state definition"
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_031_signal_day_range_compression_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        if passed
            ...
            "Use forward range/ATR attribution or a different production-visible state before retrying range-compression overlays."
        ),
        "related_files": [
            "quant/experiments/exp_20260513_031_signal_day_range_compression_risk.py",
            "data/experiments/exp-20260513-031/signal_day_range_compression_risk.json",
            "docs/experiments/logs/exp-20260513-031.json",
            "docs/experiments/tickets/exp-20260513-031.json",
            "docs/experiments/artifacts/exp-20260513-031_signal_day_range_compression_risk.md",

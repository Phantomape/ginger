"""exp-20260529-011: SEC Item 2.02 return-comovement peer transfer.

This alpha search tests one free-data candidate-pool source. After a positive
SEC 8-K Item 2.02 issuer reaction, only liquid same-sector peers with trailing
pre-event OHLCV return co-movement to the issuer can become default-off paper
candidates. Core signals, ranking, sizing, exits, LLM/news, watchlists, and
...
EXPERIMENT_ID = "exp-20260529-011"
STEM = "sec_item202_comovement_peer_transfer"
TRIAL_FAMILY = "peer_earnings_reaction_comovement_candidate_pool"
CHANGED_VARIABLE = "sec_item_202_return_comovement_peer_transfer_candidate_source_v1"
RULE_VERSION = "sec_8k_item_202_return_comovement_peer_transfer_v1"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260529_011_{STEM}.json"
...
COMOVEMENT_DAYS = 60
MIN_COMOVEMENT_OBSERVATIONS = 45
MIN_RETURN_CORRELATION = 0.30
...
def _daily_return_series(
    ...
    if start_idx < 1 or end_idx >= len(rows) or start_idx > end_idx:
        return None
    out: list[float] = []
    for idx in range(start_idx, end_idx + 1):
        ...
        if not prior_close or close is None:
            return None
        out.append((float(close) / float(prior_close)) - 1.0)
    return out


def _correlation(left: list[float], right: list[float]) -> float | None:
    if len(left) != len(right) or len(left) < MIN_COMOVEMENT_OBSERVATIONS:
        return None
    ...
    if left_var <= 0 or right_var <= 0:
        return None
    cov = sum(a * b for a, b in zip(left_dev, right_dev))
    return cov / math.sqrt(left_var * right_var)


def _return_comovement(
    issuer_rows: list[dict[str, Any]],
    issuer_idx: int,
    peer_rows: list[dict[str, Any]],
    peer_idx: int,
) -> dict[str, Any] | None:
    issuer_returns = _daily_return_series(
        issuer_rows, issuer_idx - COMOVEMENT_DAYS, issuer_idx - 1
    )
    peer_returns = _daily_return_series(peer_rows, peer_idx - COMOVEMENT_DAYS, peer_idx - 1)
    if issuer_returns is None or peer_returns is None:
        return None
    corr = _correlation(issuer_returns, peer_returns)
    if corr is None:
        return None
    return {
        "return_correlation_60d": corr,
        "comovement_observations": len(issuer_returns),
    }
...
def _candidate_rows_for_window(
    ...
    for event in reaction_events:
        ...
        if issuer_idx is None or issuer_idx < min_idx:
            audit["issuer_insufficient_comovement_history"] += 1
            continue
        ...
        for ticker in peers_by_sector.get(sector, []):
            ...
            comovement = _return_comovement(issuer_rows, issuer_idx, rows, idx)
            if comovement is None:
                audit["peer_missing_comovement"] += 1
                continue
            corr = float(comovement["return_correlation_60d"])
            if corr < MIN_RETURN_CORRELATION:
                audit["peer_low_return_correlation"] += 1
                continue
            ...
            candidates.append(
                {
                    ...
                    "peer_relation_source": "same_sector_trailing_60d_return_comovement",
                    "return_correlation_60d": framework.base._round(corr, 6),
                    "comovement_observations": comovement["comovement_observations"],
    ...
    candidates.sort(
        key=lambda row: (
            ...
            -float(row["return_correlation_60d"]),
    ...
    return candidates, {
        ...
        "rule_version": RULE_VERSION,
        "relation_field": "same-sector trailing 60 trading-day daily-return correlation",
        "min_return_correlation": MIN_RETURN_CORRELATION,
        "min_comovement_observations": MIN_COMOVEMENT_OBSERVATIONS,
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    gate4 = payload["gate4"]
    decision = (
        "accepted_candidate_sec_item202_comovement_peer_transfer"
        if gate4["passed"]
        else "rejected_sec_item202_comovement_peer_transfer"
    )
    all_target_trades = [
        trade
    ...
    prediction = {
        ...
        "main_failure_modes": [
            ...
            "comovement_relation_noisy",
            "sample_too_thin",
            "concentration",
        ],
        "confidence_reason": (
            "The prior same-sector SEC peer transfer was positive in aggregate "
            "but failed late_strong. A trailing pre-event OHLCV co-movement "
            "relation is a materially sharper free peer field."
        ),
        "recorded_at": "2026-05-29T09:05:48+00:00",
        "brier_score": round((0.36 - actual_success) ** 2, 6),
    }
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                ...
                "to liquid peers whose pre-event daily returns co-moved with the "
                "issuer than to all same-sector peers."
            ),
            ...
            "trial_variant_id": "sec_8k_item_202_return_comovement_peer_transfer_v1",
            "prior_trial_count": 1,
            ...
            "new_evidence_type": (
                "free_sec_event_plus_trailing_ohlcv_peer_comovement_relation"
            ),
            ...
            "parameters": {
                ...
                "comovement_days": COMOVEMENT_DAYS,
                "min_comovement_observations": MIN_COMOVEMENT_OBSERVATIONS,
                "min_return_correlation": MIN_RETURN_CORRELATION,
                ...
                "source_definition": [
                    ...
                    "peer must share the issuer sector from the static reference map",
                    "peer and issuer must have 60 prior trading-day daily return co-movement >= 0.30",
            ...
            "gate_questions": {
                "1_alpha_hypothesis": (
                    ...
                    "may spill over to peers with a measured pre-event return "
                    "relationship to the issuer."
                ),
                "2_history_check": {
                    "exp-20260529-010": (
                        "Same-sector peer transfer improved aggregate EV but "
                        "failed late_strong. This run changes the relation field "
                        "to trailing OHLCV co-movement rather than retuning event "
                        "reaction thresholds."
                    ),
                    "exp-20260528-036": (
                        "Sector/market breadth agreement failed; this uses a "
                        "specific SEC issuer event plus issuer-peer relationship."
                    ),
                    "exp-20260529-005": (
                        "Long-base breadth was a price-pattern pool. This adds "
                        "a non-OHLCV SEC event source plus a PIT OHLCV relation."
                    ),
                },
                ...
                "5_reproducibility": (
                    ...
                    "exp_20260529_011_sec_item202_comovement_peer_transfer.py"
                ),
            },
            "why_not_other_changes": (
                ...
                "tests a sharper free peer-relation field for the prior SEC peer "
                "transfer direction."
            ),
            "production_parity": {
                ...
                "parity_note": (
                    ...
                    "co-movement field in production before any order-affecting use."
                ),
            },
            "interpretation": (
                "The return-comovement SEC peer-transfer sleeve cleared Gate 4 as "
                "a default-off replay lead; live/default orders remain disabled."
                if gate4["passed"]
                else (
                    "The return-comovement SEC peer-transfer sleeve did not clear "
                    "Gate 4. Do not promote it or retry nearby SEC peer-transfer "
                    "relation thresholds on these frozen windows without forward "
                    "rows or a stronger relation source such as customer/supplier "
                    "links or audited industry-level similarity."
                )
            ),
            ...
            "next_evidence_needed": (
                ...
                "as customer/supplier links, official industry classification, or "
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SEC Item 2.02 metadata uses PIT usable_trade_date. Issuer/peer OHLCV "
        "and trailing co-movement are observed through signal-date close; paper "
    ...
    payload["gate2"]["runtime_field_coverage"] = {
        ...
        "peer_relation": {
            ...
            "relation": "same-sector trailing 60 trading-day daily-return correlation",
        },
    }
    payload["gate2"]["target_trade_field_coverage"] = framework._field_coverage(
        ...
        [
            ...
            "return_correlation_60d",
            "comovement_observations",
...
def _persist(payload: dict[str, Any]) -> None:
    ...
    ticket_payload = {
        "experiment_id": EXPERIMENT_ID,
        "title": "SEC Item 2.02 comovement peer transfer",

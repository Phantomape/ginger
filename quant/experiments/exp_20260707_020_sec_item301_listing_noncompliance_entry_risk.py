...
8-K Item 3.01 delisting, exchange listing-rule noncompliance, or continued
listing deficiency notices identify near-term entry risk: liquid tickers with
Item 3.01 should underperform over the next 10 sessions, supporting a
...
EXPERIMENT_ID = "exp-20260707-020"
OWNER = "alpha-explore"
SLUG = "sec_item301_listing_noncompliance_entry_risk"
RUNNER = f"quant/experiments/exp_20260707_020_{SLUG}.py"
...
ITEM301_CODE = "3.01"
ITEM301_FORMS = {"8-K", "8-K/A"}
MIN_SUPPORT_EVENTS_PER_WINDOW = 10
MIN_TOTAL_LIQUID_EVENTS = 30
MAX_SINGLE_TICKER_SHARE = 0.40

HYPOTHESIS = (
    "entry_filter/risk_allocation: SEC 8-K Item 3.01 delisting, exchange "
    "listing-rule noncompliance, or continued-listing deficiency notices are "
    "PIT governance/liquidity risk events; liquid tickers with Item 3.01 "
    "should underperform over the next 10 sessions, supporting a default-off "
    "entry risk gate rather than a long source."
)
CHANGE_TYPE = "entry_filter"
IMPLEMENTATION_MODE = "private_replay_scout"
MECHANISM_FAMILY = "free_sec_listing_compliance_entry_risk"
TRIAL_FAMILY = "sec_8k_item301_listing_noncompliance_entry_risk"
TRIAL_VARIANT_ID = "item301_listing_noncompliance_next_open_10d_v1"
CHANGED_VARIABLE = "sec_8k_item301_listing_noncompliance_10d_entry_risk_gate_v1"
NEW_EVIDENCE_TYPE = "new_gate_shape_sec_8k_item301_listing_noncompliance"
NEW_EVIDENCE_AXIS = (
    "New gate shape: direct same-ticker SEC 8-K Item 3.01 delisting/listing-"
    "rule noncompliance next-open 10d entry-risk attribution. Prior SEC-event "
    "peer-shock used incidental 3.01 anchors only through broad event "
    "provenance; large-filing, restructuring, and complexity text experiments "
    "used delisting as an exclude term, not a direct Item 3.01 risk gate."
)
...
def load_item301_events() -> tuple[list[dict[str, Any]], dict[str, Any]]:
...
def summarize_item301_trades(trades: list[dict[str, Any]]) -> dict[str, Any]:
...
def build_payload() -> dict[str, Any]:
    events, event_diagnostics = load_item301_events()
    ...
    summary = summarize_item301_trades(trades)
    ...
    decision = (
        "observed_positive_lead_sec_item301_entry_risk_gate"
        if support_lead
        else "rejected_sec_item301_entry_risk_gate"
    )
    ...
    return {
        ...
        "parameters": {
            "forms": sorted(ITEM301_FORMS),
            "item_code": ITEM301_CODE,
        ...
        "pre_run_questions": {
            ...
            "2_history_check": {
                "novelty_gate": (
                    "experiment.py new blocked broad SEC governance/text "
                    "neighbors; override was recorded with a legal new gate "
                    "shape: direct same-ticker Item 3.01 entry-risk attribution."
                ),
                "exp-20260608-027": (
                    "SEC-event peer-shock used incidental 3.01 anchors only "
                    "as broad event provenance, not direct same-ticker risk."
                ),
                "exp-20260609-012": (
                    "Large-filing payload used delisting as an exclude term, "
                    "not as a tested Item 3.01 risk signal."
                ),
                "exp-20260522-006/007": (
                    "Governance disclosure quality was a notional scalar; "
                    "this run tests an item-code event-risk gate."
                ),
            },
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "The direct Item 3.01 surface did not produce broad negative "
                "deployable event drift after liquidity gates, or failed the "
                "predeclared support/concentration rule."
            )
            if not support_lead
            else (
                "Item 3.01 listing-noncompliance events showed broad negative "
                "event drift and deserve a shared-helper Gate 4 follow-up."
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retune price, ADV, hold days, cooldown, same-day ranking, "
                "Item 3.01 sub-slices, or response functions on these same cached "
                "rows. A long-source inversion also needs a new gate shape."
            ),
            "new_evidence_required": (
                "A legal retry needs materially new Item 3.01 rows, text-provenance "
                "that separates deficiency notices from listing transfers, or a "
                "different exchange/listing-compliance source."
            ),
        },
        "rejection_reason": ";".join(failed_reasons) if failed_reasons else None,
        "next_retry_requires": [
            "materially new Item 3.01 rows",
            "or text-provenance separating listing deficiency from transfer events",
            "or a different listing-compliance data source",
        ],
...
def make_card(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID}: SEC Item 3.01 listing-noncompliance entry-risk scout",
        "",
    ...
    lines += [
        "",
        f"Raw Item 3.01 rows: {payload['data_diagnostics']['raw_item301_count']}; "

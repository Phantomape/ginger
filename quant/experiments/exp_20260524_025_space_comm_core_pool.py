"""exp-20260524-025: Space communications infrastructure core-pool scout.

Alpha search on one causal variable: add the production-governed full-history
Space communications / satcom research cohort to the core replay universe. This
uses the canonical-window observation-universe OHLCV snapshots from
...
EXPERIMENT_ID = "exp-20260524-025"
STEM = "space_comm_core_pool"
TRIAL_FAMILY = "governed_space_comm_candidate_pool"
TARGET_THEME = "space_comm_infra"
TARGET_SEGMENT = "satellite_connectivity"
...
def _target_universe() -> dict[str, Any]:
    ...
    return {
        ...
        "selection_rule": (
            ...
            "pilot, liquidity_tier in {ok, watch}, history_class full_history, "
            "and not already in core"
        ),
        "why_this_cohort_is_not_noise": (
            "These tickers are governed universe-state Space communications records "
            "with full OHLCV history. The cohort tests satellite-connectivity "
            "candidate-pool replacement value instead of adding arbitrary tickers."
        ),
...
def _apply_space_comm_overrides() -> None:
...
def _patch_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = (
        ...
        if gate4_passed
        else "rejected_space_comm_core_pool"
    )
    payload.update(
        {
            ...
            "hypothesis": (
                ...
                "candidate-pool alpha when the existing trend/breakout engine "
                "finds valid setups, because satellite-connectivity demand is "
                "distinct from the recently rejected AI-infra and consumer "
                "platform cohorts."
            ),
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: governed full-history Space communications "
            "records may add replacement-value opportunities outside recent AI, "
            "SEC/event, state-surface, and broad-market scalar lanes."
        ),
        "2_history_check": {
            "exp-20260511-026": (
                "Mature-satcom breadth low-risk extension was rejected inside the "
                "Space sleeve, but it did not test core-pool candidate membership."
            ),
            "exp-20260511-030": (
                "Space theme ETF momentum risk was rejected; this uses direct "
                "governed ticker membership rather than ETF timing."
            ),
            "exp-20260512-032": (
                "Launch/lunar theme-segment risk was accepted for default-off "
                "Space risk, not broad core-pool admission."
            ),
            "exp-20260523-007": "Space confidence/TQS disagreement trend risk was rejected.",
            "exp-20260523-009": "AI power/datacenter core-pool cohort was rejected.",
            "exp-20260524-015": "Consumer platform core-pool cohort was rejected.",
            "exp-20260524-020": "Residual AI-infra core-pool cohort was rejected.",
        },
        "3_single_causal_variable": (
            "Membership of one governed Space communications/satcom cohort in the "
        ...
        "5_reproducibility": (
            ...
            "exp_20260524_025_space_comm_core_pool.py"
        ),
    }
    ...
    payload["why_not_other_changes"] = (
        ...
        "rejections; skipped arbitrary pool expansion and used current governed "
        "Space communications records with full-history OHLCV instead."
    )
    payload["known_risks"] = [
        "Space remains observe-only/default-off; live promotion would need separate slot/risk governance.",
        "The cohort is thematically high beta and may fail drawdown or concentration guards.",
    ...
    payload["interpretation"] = (
        ...
        if gate4_passed
        else "The cohort did not clear the direct core-pool gate; keep Space communications names in governed research/observe-only paths."
    )
    payload["rejection_reason"] = (
        None
        if gate4_passed
        else (
            "Space communications core-pool admission did not clear the direct "
    ...
    payload["next_evidence_needed"] = (
        ...
        if gate4_passed
        else "Collect forward Space communications replacement-value outcomes or a stronger event-quality field before retrying this cohort."
    )
...
def _build_report(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Space Communications Core-Pool Scout",
            "",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    base._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Space communications core-pool scout",

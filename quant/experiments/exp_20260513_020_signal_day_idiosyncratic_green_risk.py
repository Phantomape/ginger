...
post-sizing top-up for core signals whose own signal-day candle is green while
SPY's same-day candle is red. This is a narrower idiosyncratic-strength state
than the rejected broad ticker-vs-SPY outperformance scout.
"""
...
EXPERIMENT_SLUG = "signal_day_idiosyncratic_green_risk"
MULTIPLIER_KEY = "signal_day_idiosyncratic_green_risk_multiplier_applied"
SCALARS = [1.05, 1.075, 1.10, 1.15, 1.20]
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        for sig in enriched:
            ...
            sig["signal_day_idiosyncratic_green_candle"] = (
                sig.get("strategy") in {"trend_long", "breakout_long"}
                and isinstance(ticker_ret, (int, float))
                and isinstance(spy_ret, (int, float))
                and float(ticker_ret) > 0.0
                and float(spy_ret) < 0.0
            )
        return enriched
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["signal_day_idiosyncratic_green_baseline_shares"] = shares
    out["signal_day_idiosyncratic_green_desired_shares"] = desired_shares
    out["signal_day_idiosyncratic_green_cap_shares"] = cap_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("signal_day_idiosyncratic_green_candle") and sizing.get(
                "shares_to_buy"
            ):
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Signal-Day Idiosyncratic Green Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_signal_day_idiosyncratic_green_risk"
    )
    interpretation = (
        "Signal-day idiosyncratic green risk allocation cleared the canonical three-window gate and should be promoted through shared policy before production use."
        if passed
        else "Signal-day idiosyncratic green risk allocation did not clear the canonical three-window gate; do not promote this weak-tape relative-strength scalar on the frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "When a candidate closes green on its signal day while SPY closes red, "
            "the move may contain cleaner idiosyncratic follow-through than broad "
            "green-candle strength; allocate slightly more risk without changing entries."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "signal_day_idiosyncratic_green_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing scalar for core signals with own signal-day open-to-close return > 0 and SPY signal-day open-to-close return < 0"
        ),
        "parameters": {
            "state_definition": {
                "strategies": ["trend_long", "breakout_long"],
                "ticker_signal_day_open_close_return_pct": "> 0",
                "spy_signal_day_open_close_return_pct": "< 0",
            },
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core state risk allocation using production-knowable idiosyncratic signal-day strength",
            "2_history_check": {
                "exp-20260513-007": "own green-candle 1.05x is accepted and locked; this tests only the incremental weak-market subset.",
                "exp-20260513-010/011": "broad ticker-vs-SPY outperformance was positive but failed drawdown; this narrows to own green plus SPY red, not another excess-margin retune.",
                "exp-20260513-009/013/016/018": "close-location, momentum acceleration, broad high-TQS, and stacked confirmed-quality scalars were rejected; those states are not reused.",
                "llm_soft_ranking": "labeled forward data remains too thin, so this run avoids LLM scoring.",
                "sec_semantics": "same-accession directional fields remain missing, so this run avoids SEC semantic retunes.",
                "space_profile_slicing": "recent Space profile splits are over-mined on frozen snapshots, so this run tests core instead.",
            },
            "3_single_causal_variable": "signal_day_idiosyncratic_green_risk_multiplier with fixed natural zero boundaries",
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_020_signal_day_idiosyncratic_green_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "risk_engine injected signal_day_spy_open_close_return_pct",
        ...
        if passed
        else "Use forward weak-tape idiosyncratic-strength attribution or a materially different production-visible state before retrying.",
        "related_files": [
            "quant/experiments/exp_20260513_020_signal_day_idiosyncratic_green_risk.py",
            "data/experiments/exp-20260513-020/signal_day_idiosyncratic_green_risk.json",
            "experiments/logs/exp-20260513-020.json",
            "experiments/tickets/exp-20260513-020.json",
            "experiments/artifacts/exp-20260513-020_signal_day_idiosyncratic_green_risk.md",
            "docs/experiment_log.jsonl",
        ],
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited; SEC semantic fields are missing; "
            "Space frozen-sample profile scalars are over-mined; broad SPY-outperformance "
            "and confirmed-quality core scalars were recently rejected. This tests a narrower "
            "core allocation state without changing entries or expanding ticker noise."
        ),
    }
    return payload

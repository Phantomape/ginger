...
Item 1.01 material-agreement events whose issuer also shows same-day positive
...
EXPERIMENT_ID = "exp-20260529-016"
STEM = "sec_item101_positive_reaction_candidate_pool"
TRIAL_FAMILY = "sec_item101_positive_reaction_candidate_pool"
CHANGED_VARIABLE = "sec_item101_positive_reaction_candidate_source_v1"
RULE_VERSION = "sec_item101_positive_reaction_v1"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260529_016_{STEM}.json"
...
TARGET_ITEM_CODES = {"1.01"}
EXCLUDED_ITEM_CODES = {"1.02", "2.02", "3.02", "5.01", "5.02", "5.03", "5.07"}
...
_SEC_ITEM101_EVENTS_CACHE: list[dict[str, Any]] | None = None
...
def _load_sec_item101_events() -> list[dict[str, Any]]:
    global _SEC_ITEM101_EVENTS_CACHE
    if _SEC_ITEM101_EVENTS_CACHE is not None:
        return _SEC_ITEM101_EVENTS_CACHE
    ...
    _SEC_ITEM101_EVENTS_CACHE = sorted(
        deduped.values(),
        key=lambda row: (
...
def _candidate_rows_for_window(
    ...
    events = [
        event for event in _load_sec_item101_events() if event["usable_trade_date"] in dates
    ]
    ...
    for event in events:
        ...
        avg_dollar_volume = prior._avg_dollar_volume(
            rows, idx, AVG_DOLLAR_VOLUME_DAYS
        )
        ...
        if key in seen_ticker_dates:
            audit["duplicate_ticker_date_item101"] += 1
            continue
        ...
        candidates.append(
            {
                ...
                "item101_candidate_score": framework.base._round(score, 6),
    ...
    candidates.sort(
        key=lambda row: (
            row["date"],
            -float(row["item101_candidate_score"]),
    ...
    return candidates, {
        ...
        "sec_item101_events_in_window": len(events),
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    gate4 = payload["gate4"]
    decision = (
        "accepted_candidate_sec_item101_positive_reaction"
        if gate4["passed"]
        else "rejected_sec_item101_positive_reaction"
    )
    all_target_trades = [
        trade
    ...
    prediction = {
        ...
        "main_failure_modes": [
            ...
            "contract disclosure is stale",
            "sample too small",
            "single ticker concentration",
        ],
        "confidence_reason": (
            "Meta research favors production-visible candidate-pool alpha. "
            "Item 1.01 is distinct from recent rejected 2.02 peer-transfer and "
            "7.01/8.01 FD/Other tests because it targets definitive agreements."
        ),
        "recorded_at": "2026-05-29T14:07:41+00:00",
    ...
    payload.update(
        {
            ...
            "decision": decision,
            "hypothesis": (
                "PIT-safe SEC 8-K Item 1.01 material-agreement filings with "
                "positive same-day issuer reaction, liquidity, trend, and "
                "relative-strength confirmation may provide a cleaner free-data "
                "continuation candidate pool."
            ),
            ...
            "new_evidence_type": (
                "production_visible_free_sec_item101_material_agreement_plus_ohlcv_confirmation"
            ),
            ...
            "parameters": {
                ...
                "source_definition": [
                    ...
                    "item codes must include 1.01 material definitive agreement",
                    "termination, earnings, offering, and governance item codes are excluded",
                    ...
                    "20-day average dollar volume must be at least USD 25 million",
            ...
            "gate_questions": {
                "1_alpha_hypothesis": (
                    "candidate_pool / entry: material-agreement 8-Ks with immediate "
                    "issuer price confirmation can capture contractual information "
                    "transfer better than generic FD/Other 8-K rows."
                ),
                "2_history_check": {
                    "exp-20260529-010": (
                        "Item 2.02 same-sector peer-transfer failed due late_strong "
                        "regression. This run excludes 2.02 and trades only the "
                        "direct issuer event."
                    ),
                    "exp-20260529-011": (
                        "Comovement peer transfer improved only two windows and was "
                        "sample-thin. This run does not use peer transfer."
                    ),
                    "exp-20260529-015": (
                        "Generic 7.01/8.01 FD/Other direct issuer positive reaction "
                        "regressed all windows. This run targets only Item 1.01 "
                        "definitive-agreement filings."
                    ),
                },
                ...
                "5_reproducibility": (
                    ...
                    "exp_20260529_016_sec_item101_positive_reaction_candidate_pool.py"
                ),
            },
            ...
            "production_parity": {
                ...
                "parity_note": (
                    ...
                    "require a shared SEC Item 1.01 paper sleeve with exact item-code, "
                    "as-of OHLCV, and entry/exit parity tests."
                ),
            },
            "interpretation": (
                "The SEC Item 1.01 positive-reaction candidate pool cleared Gate 4 "
                "as a default-off replay lead, but no production/shared policy was promoted."
                if gate4["passed"]
                else (
                    "The SEC Item 1.01 positive-reaction candidate pool did not clear "
                    "Gate 4. Do not promote it or retry nearby material-agreement "
                    "positive-reaction thresholds on the same frozen windows without "
                    "new forward rows or a sharper structured contract-quality field."
                )
            ),
            "rejection_reason": None if gate4["passed"] else "; ".join(gate4["failed_reasons"]),
            "next_evidence_needed": (
                "Forward replacement-value rows or a materially sharper contract-quality "
                "field such as agreement value, counterparty/source quality, or revenue "
                "visibility extracted with evidence spans."
            ),
    ...
    payload["gate2"]["runtime_field_coverage"] = {
        "sec_events": {
            ...
            "events_loaded": len(_load_sec_item101_events()),
        }
    }
...
def _persist(payload: dict[str, Any]) -> None:
    ...
    ticket_payload = {
        "experiment_id": EXPERIMENT_ID,
        "title": "SEC Item 1.01 positive-reaction candidate pool",

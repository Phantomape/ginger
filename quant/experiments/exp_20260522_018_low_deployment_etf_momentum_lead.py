"""exp-20260522-018 low-deployment ETF momentum-lead scout.
...
position. This experiment changes one production-visible candidate-quality
variable only:

    require the selected ETF's prior 20-day momentum to lead the second-best
    eligible ETF by a swept minimum spread.

The ETF pool, activation threshold, raw momentum selector, notional, core
strategy, LLM/news replay, and live/default order path remain unchanged. If no
variant clears the three-window gate, keep accepted raw-momentum v1.
"""
...
EXPERIMENT_ID = "exp-20260522-018"
STEM = "low_deployment_etf_momentum_lead"
...
MOMENTUM_LEAD_VARIANTS: "OrderedDict[str, float]" = OrderedDict(
    [
        ("lead_25bp", 0.0025),
        ("lead_50bp", 0.0050),
        ("lead_75bp", 0.0075),
...
def _select_overlay_ticker(
    ...
    *,
    ...
    if not candidates:
        return None
    candidates = sorted(candidates, key=lambda row: (row["momentum"], row["ticker"]), reverse=True)
    top = dict(candidates[0])
    second_momentum = candidates[1]["momentum"] if len(candidates) > 1 else 0.0
    momentum_lead = float(top["momentum"]) - float(second_momentum)
    if momentum_lead < min_momentum_lead:
        return None
    top["second_momentum20"] = second_momentum
    top["momentum_lead_vs_second"] = momentum_lead
    return top
    ...
    *,
    ...
    for day, _ in base_curve:
        ...
        selection = _select_overlay_ticker(
            ...
            index_by_ticker_date,
            day,
            min_momentum_lead=min_momentum_lead,
        )
        ...
        overlay_days.append(
            {
                ...
                "second_momentum20": _round(selection["second_momentum20"], 6),
                "momentum_lead_vs_second": _round(
                    selection["momentum_lead_vs_second"], 6
                ),
                "min_momentum_lead": min_momentum_lead,
                "variant": variant,
...
def _gate4(
    ...
    return {
        ...
        "basis": (
            "Three canonical backtesting.md windows, momentum-lead variants "
...
def _write_artifact(payload: dict[str, Any]) -> None:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Low-deployment ETF Momentum Lead",
        "",
    ...
    for variant, row in payload["variant_results"].items():
        ...
        lines.append(
            "| {variant} | {lead:.4f} | {ev:+.4f} | ${pnl:+,.2f} | {ev_imp}/{ev_reg} | {pnl_imp}/{pnl_reg} | {dd:+.4f} | {days} | {gate} |".format(
                variant=variant,
                lead=row["min_momentum_lead"],
...
def _build_payload() -> dict[str, Any]:
    ...
    for variant, lead in MOMENTUM_LEAD_VARIANTS.items():
    ...
    if accepted:
        decision = "accepted_default_off_low_deployment_etf_momentum_lead"
        rejection_reason = None
        decision_rationale = (
            "The momentum-lead variant beat the accepted raw-momentum v1 overlay "
            "across all three canonical windows and cleared materiality, drawdown, "
            "sample, and concentration guards. Promotion would still require moving "
            "the field into the shared default-off paper module and adding parity "
            "tests before any live/default behavior changes."
        )
    else:
        decision = "rejected_low_deployment_etf_momentum_lead"
        rejection_reason = (
            "No momentum-lead variant beat the accepted raw-momentum v1 overlay "
    ...
    log_record = {
        ...
        "hypothesis": (
            ...
            "perform better when raw momentum leadership is unambiguous. Requiring "
            "the selected ETF to lead the second-best eligible ETF by a minimum "
            "prior-20d momentum spread could improve replacement value and reduce "
            "rotation noise without changing the pool or notional."
        ),
        "change_type": "alpha_search_overlay_candidate_quality_filter",
        "changed_variable": "low_deployment_etf_overlay_prior_momentum20_lead_vs_second_min",
        "single_causal_variable": (
            "Only the ETF overlay candidate eligibility minimum momentum lead is "
            "swept; raw momentum ranking, ETF pool, activation threshold, notional, "
            "core strategy logic, LLM/news replay, and live/default order paths stay locked."
        ),
        "trial_accounting": {
            "trial_family": "low_deployment_dynamic_etf_overlay_candidate_quality",
            "changed_variable": "prior_momentum20_lead_vs_second_min",
            "prior_trial_count": 5,
            ...
            "new_evidence_detail": (
                "Uses prior-close OHLCV-derived rank spread between eligible ETF "
                "candidates, a field that can be produced in run.py and backtester.py "
                "without LLM replay."
            ),
        },
        "parameters": {
            "baseline_variant": BASELINE_VARIANT,
            "momentum_lead_variants": dict(MOMENTUM_LEAD_VARIANTS),
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "capital_allocation/ranking: a prior momentum-lead spread may "
            ...
            "2_history_check": {
                ...
                "exp-20260512-777": "ETF candidate-pool variants rejected; this keeps the v1 pool fixed.",
                "exp-20260518-003": "Activation-threshold variants rejected; this keeps <=1 fixed.",
                "exp-20260520-016": "Risk-adjusted selector rejected; this keeps raw momentum ranking fixed.",
                "exp-20260522-004": "Volatility cap rejected; this changes rank-spread confidence, not volatility.",
            },
            "3_single_variable": "prior_momentum20_lead_vs_second_min",
            "4_acceptance_standard": (
                "Compare each lead variant against accepted raw-momentum v1 over "
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260522_018_low_deployment_etf_momentum_lead.py"
            ),
        },
        "alpha_hypothesis": {
            ...
            "playbook_alignment": (
                ...
                "semantic zero-sample issues, state-surface scalar retunes, and "
                "Space candidate-pool over-mining; uses deterministic production-visible "
                "OHLCV rank-spread evidence in the existing low-deployment paper lane."
            ),
        },
        "date_range": {
            label: {
        ...
        "gate3": {
            ...
            "note": (
                ...
                "overlay candidate confidence gate measured by actual overlay day counts."
            ),
            "survival_rates": {
        ...
        "best_variant_details": {
            "min_momentum_lead": best_payload["min_momentum_lead"],
        ...
        "next_action": (
            "If accepted, implement the lead gate only in the shared default-off "
            "ETF paper module with focused parity tests; live orders remain disabled. "
            "If rejected, keep raw-momentum v1 and avoid adjacent ETF selector/cap "
            "formulas without new forward replacement-value evidence."
        ),
        "related_files": [
            ...
            _safe(
                {
                    ...
                    "decision": payload["decision"],
                    "best_variant": best,
                    "best_min_momentum_lead": best_result["min_momentum_lead"],
                    "expected_value_score_delta": payload["expected_value_score_delta"],
                    "total_pnl_delta": payload["total_pnl_delta"],
                    "gate4": payload["gate4"],
                    "artifact": _repo_rel(ARTIFACT_MD),

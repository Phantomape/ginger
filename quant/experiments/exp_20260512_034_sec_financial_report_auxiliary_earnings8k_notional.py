"""exp-20260512-034: SEC financial-report auxiliary earnings 8-K notional.

Alpha search on one causal variable: a paper-notional scalar for earnings 8-K
rows whose item codes include auxiliary disclosure items beyond 2.02/9.01.
The accepted max-3 capacity, T+1 excess floor, 10-trading-day hold, $15k base
notional, periodic-report scalar, 10-Q scalar, queue order, and live orders
stay fixed.
"""
...
EXPERIMENT_ID = "exp-20260512-034"
STEM = "exp_20260512_034_sec_financial_report_auxiliary_earnings8k_notional"
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_financial_report_auxiliary_earnings8k_notional.md"
)
EXPERIMENT_LOG_JSONL = REPO_ROOT / "docs" / "experiment_log.jsonl"

ACCEPTED_10Q_PERIODIC_REPORT_SCALAR = 2.0
BASELINE_AUXILIARY_EARNINGS8K_SCALAR = 1.0
AUXILIARY_EARNINGS8K_SCALAR_VARIANTS = (0.50, 0.75, 1.0, 1.10, 1.25, 1.50)
MIN_PROMOTION_CLOSED_TRADES = 40
MIN_AUXILIARY_EARNINGS8K_CLOSED_TRADES = 10
...
def _is_auxiliary_earnings8k_position(position: dict[str, Any]) -> bool:
    if _event_family(position) != "earnings_8k":
        return False
    item_codes = _normalise_item_codes(_source_candidate(position).get("item_codes"))
    return bool(item_codes) and not item_codes.issubset(PURE_EARNINGS_8K_ITEMS)


def _notional_for_position(
    position: dict[str, Any],
    *,
    auxiliary_earnings8k_scalar: float,
) -> tuple[float, float, str]:
    base = float(DEFAULT_EVENT_NOTIONAL_USD)
    if _event_family(position) == "earnings_8k":
        if _is_auxiliary_earnings8k_position(position):
            scalar = float(auxiliary_earnings8k_scalar)
            return base * scalar, scalar, "auxiliary_earnings8k_scalar"
...
def _pnl_for_position(
    ...
    *,
    ...
    adjusted_notional, _, _ = _notional_for_position(
        position,
        auxiliary_earnings8k_scalar=auxiliary_earnings8k_scalar,
    )
    if closed:
        try:
...
def _adjust_closed_position(
    ...
    *,
    ...
    adjusted["auxiliary_earnings8k_bucket"] = _is_auxiliary_earnings8k_position(
        position
    )
    adjusted["auxiliary_earnings8k_notional_scalar"] = auxiliary_earnings8k_scalar
    adjusted["pnl"] = round(
        _pnl_for_position(
            position,
            auxiliary_earnings8k_scalar=auxiliary_earnings8k_scalar,
...
def _closed_position_breakdown(closed_positions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in closed_positions:
        ...
        if item.get("auxiliary_earnings8k_bucket") is True:
            aux_count += 1
            aux_pnl += pnl
    return {
        ...
        "auxiliary_earnings8k_closed_trade_count": aux_count,
        "auxiliary_earnings8k_total_pnl": _round(aux_pnl, 2),
    }
...
def _auxiliary_closed_trade_count(row: dict[str, Any]) -> int:
    return sum(
        int(
            window["sleeve_metrics"].get(
                "auxiliary_earnings8k_closed_trade_count",
                0,
            )
        )
...
def _window_checks(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    for label in WINDOWS:
        ...
        checks[label] = {
            ...
            "auxiliary_earnings8k_closed_trade_count": int(
                after_sleeve.get("auxiliary_earnings8k_closed_trade_count") or 0
            ),
            "auxiliary_earnings8k_pnl_delta": _round(
                float(after_sleeve.get("auxiliary_earnings8k_total_pnl") or 0.0)
                - float(before_sleeve.get("auxiliary_earnings8k_total_pnl") or 0.0),
                2,
            ),
        }
    return checks
...
def _gate(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    auxiliary_closed_trades_after = _auxiliary_closed_trade_count(after)
    passed = (
        ...
        and auxiliary_closed_trades_after >= MIN_AUXILIARY_EARNINGS8K_CLOSED_TRADES
    )
    return {
        "aggregate_delta": aggregate_delta,
        "auxiliary_earnings8k_closed_trade_count_after": auxiliary_closed_trades_after,
        ...
        "rule": (
            ...
            "auxiliary earnings 8-K closed trades >= 10."
        ),
...
def _best_candidate(variants: OrderedDict[str, dict[str, Any]]) -> str:
    baseline = variants[f"auxiliary_earnings8k_scalar_{BASELINE_AUXILIARY_EARNINGS8K_SCALAR:.2f}"]
    candidates = [
        (name, row, _gate(row, baseline))
        for name, row in variants.items()
        if row["auxiliary_earnings8k_notional_scalar"]
        != BASELINE_AUXILIARY_EARNINGS8K_SCALAR
    ]
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    gate = payload["gate"]
    lines = [
        f"# {payload['experiment_id']} SEC auxiliary earnings 8-K notional",
        "",
    ...
    for label, row in gate["window_checks"].items():
        lines.append(
            "| {label} | {ev} | {pnl} | {dd} | {count} | {aux_pnl} |".format(
                label=label,
                ev=row["ev_delta"],
                pnl=row["pnl_delta"],
                dd=row["max_drawdown_delta"],
                count=row["auxiliary_earnings8k_closed_trade_count"],
                aux_pnl=row["auxiliary_earnings8k_pnl_delta"],
            )
        )
    lines.extend(
        [
...
def main() -> int:
    ...
    for scalar in AUXILIARY_EARNINGS8K_SCALAR_VARIANTS:
        name = f"auxiliary_earnings8k_scalar_{scalar:.2f}"
        row = _run_variant(
            core_results=core_results,
            exp100=exp100,
            auxiliary_earnings8k_scalar=scalar,
        )
        row["auxiliary_earnings8k_notional_scalar"] = scalar
        variants[name] = row

    baseline_key = (
        f"auxiliary_earnings8k_scalar_{BASELINE_AUXILIARY_EARNINGS8K_SCALAR:.2f}"
    )
    ...
    decision = (
        "accepted_auxiliary_earnings8k_notional_scalar"
        if gate["passed"]
        else "rejected_auxiliary_earnings8k_notional_scalar"
    )
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "earnings 8-K rows with auxiliary disclosure items beyond 2.02/9.01 "
            "may have a different event-quality profile than clean earnings "
            "8-K rows, so they may deserve a separate paper-notional scalar."
        ),
        ...
        "parameters": {
            "baseline_auxiliary_earnings8k_notional_scalar": (
                BASELINE_AUXILIARY_EARNINGS8K_SCALAR
            ),
            "auxiliary_earnings8k_definition": (
                "event_family == earnings_8k and item_codes contain any item "
                "outside {2.02, 9.01}"
            ),
            "auxiliary_earnings8k_scalar_variants": list(
                AUXILIARY_EARNINGS8K_SCALAR_VARIANTS
            ),
        ...
        "rejection_reason": (
            None
            if gate["passed"]
            else "No auxiliary earnings 8-K scalar cleared the three-window semantic risk-allocation gate."
        ),
        "next_evidence_needed": (
            ...
            if gate["passed"]
            else "Do not retry auxiliary earnings 8-K notional scalars on this frozen sample; future SEC work needs forward outcomes or a genuinely new earnings-quality field."
        ),
        "production_impact": {
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "risk allocation: use existing SEC item_codes semantics to scale "
                "paper notional only for auxiliary-item earnings 8-K rows."
            ),
            "2_history_check": (
                "exp-20260512-011 tested clean earnings 8-K notional and failed "
                "because late_strong regressed; exp-20260512-020 accepted 10-Q "
                "notional; exp-20260512-025/033 rejected queue priority and "
                "paired-filing dedupe. No logged experiment isolated auxiliary "
                "item earnings 8-K notional on the accepted 10-Q stack."
            ),
            "3_single_causal_variable": (
                "auxiliary earnings 8-K paper-notional scalar only"
            ),
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains sample-limited, Space has just accepted "
            "multiple default-off risk scalars and should wait for forward "
            "replacement value, and SEC floor/hold/global-notional/10-Q-priority/"
            "paired-dedupe variants already have anti-repeat records."
        ),
    }

"""exp-20260512-110: signal-day own-red-candle risk allocation.

Alpha search. Tests one production-knowable core state variable: whether the
candidate's own signal-day close is below its signal-day open. The experiment
changes only a post-sizing risk scalar for that state. It does not change entry
filters, ranking, exits, targets, LLM/news behavior, or universe membership.
"""
...
EXPERIMENT_SLUG = "signal_day_ticker_red_risk"
MULTIPLIER_KEY = "signal_day_ticker_red_risk_multiplier_applied"
SCALARS = (0.0, 0.25, 0.5, 0.75)
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]) -> Callable[..., list[dict[str, Any]]]:
    def wrapped(
        ...
        for sig in enriched:
            ...
            sig["signal_day_ticker_red_candle"] = (
...
def _rescale_sizing(
    ...
    out = dict(sizing)
    out["signal_day_ticker_red_baseline_shares"] = shares
    out["signal_day_ticker_red_new_shares"] = new_shares
...
def _make_size_wrapper(original: Callable[..., list[dict[str, Any]]], scalar: float) -> Callable[..., list[dict[str, Any]]]:
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("signal_day_ticker_red_candle") and sizing.get("shares_to_buy"):
...
def _variant_name(scalar: float) -> str:
    return f"red_scalar_{str(scalar).replace('.', '_')}"
...
def _build_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Signal-Day Ticker Red Risk",
        "",
    ...
    for name, variant in payload["variants"].items():
        gate = variant["gate"]
        lines.append(
            "| {name} | {scalar:.2f} | {ev:+.4f} | ${pnl:+,.2f} | {imp} | {reg} | {adj} | {passed} |".format(
                name=name,
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        else "rejected_signal_day_ticker_red_risk"
    )
    interpretation = (
        "Own red-candle signal-day risk allocation cleared the three-window gate and should be implemented through shared production/backtest policy before use."
        if best_variant["gate"]["passed"]
        else "Own red-candle signal-day risk allocation did not clear the canonical three-window gate; do not promote this state scalar without forward hold-quality evidence or a stronger discriminator."
    )
    ...
    return {
        ...
        "decision": decision,
        "hypothesis": (
            "Signals whose own signal-day candle closes red may have weaker next-session follow-through; reduce their risk rather than filtering them."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "signal_day_ticker_red_candle_risk_scalar",
        "single_causal_variable": (
            "post-sizing scalar for signals whose own signal-day open-to-close return is < 0"
        ),
        "parameters": {
            "signal_day_ticker_red_definition": "ticker open-to-close return < 0 on signal day",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core state risk allocation using production-knowable signal-day own-candle weakness",
            "2_history_check": {
                "exp-20260512-102": "entry-day adverse tape was observed-only and ex-post; this uses signal-day own OHLCV known before next-open execution.",
                "exp-20260512-106": "sector-proxy signal-day adverse tape 0.5x was rejected; this tests the ticker's own red candle, a different state variable.",
                "exp-20260512-107": "positive sector-tape top-up was underpowered; this is not a positive tape top-up.",
            },
            "3_single_causal_variable": "signal_day_ticker_red_candle_risk_scalar",
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260512_110_signal_day_ticker_red_risk.py",
        },
        "backtest_protocol": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "risk_engine signal_day_ticker_red_candle",
        ...
        if best_variant["gate"]["passed"]
        else "Forward/shadow hold-quality evidence or a stronger production-visible candle-quality state before retrying.",
        "related_files": [
            "quant/experiments/exp_20260512_110_signal_day_ticker_red_risk.py",
            "data/experiments/exp-20260512-110/signal_day_ticker_red_risk.json",
            "docs/experiments/logs/exp-20260512-110.json",
            "docs/experiments/tickets/exp-20260512-110.json",
            "docs/experiments/artifacts/exp-20260512-110_signal_day_ticker_red_risk.md",

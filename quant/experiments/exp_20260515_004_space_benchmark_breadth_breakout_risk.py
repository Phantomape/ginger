"""exp-20260515-004: Space benchmark-breadth breakout risk.
...
EXPERIMENT_ID = "exp-20260515-004"
STEM = "space_benchmark_breadth_breakout_risk"
...
MARKER = "space_benchmark_breadth_breakout_risk"

ACCEPTED_IWM_LEADER_SCALAR = 1.0125
SCALARS = (1.0, 0.0, 0.25, 0.5, 0.75, 1.0125, 1.025, 1.05, 1.075)
    ...
    *,
    ...
    def wrapped_size_signals(
        ...
        for signal in sized:
            ...
            if ticker in benchmark_tickers and strategy == TARGET_STRATEGY and sizing:
                _scale_and_record(
                    signal=signal,
                    sizing=sizing,
                ...
                signal = {
                    **signal,
                    "sizing": sizing,
                    "space_benchmark_breadth_breakout_bucket": True,
                    "space_benchmark_breadth_breakout_scalar": breakout_scalar,
    ...
    variant["benchmark_breadth_breakout_counts"] = dict(sorted(counts.items()))
    variant["benchmark_breadth_breakout_counts_by_window"] = (
        counts_by_window_serialized
    )
    variant["benchmark_breadth_breakout_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(adjustments)
    )
    variant["benchmark_breadth_breakout_adjustment_sample"] = adjustments[:25]
    for window_name, row in variant["by_window"].items():
        row["benchmark_breadth_breakout_counts"] = counts_by_window_serialized.get(
            window_name,
            {},
        )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("benchmark_breadth_breakout_counts") or {}
    changed_count = int(counts.get(f"{MARKER}_changed_signal", 0))
    eligible_count = int(counts.get(f"{MARKER}_eligible_signal", 0))
    scalar = float(variant["parameters"]["space_benchmark_breadth_breakout_scalar"])
    passed = bool(
        scalar != 1.0
        and changed_count > 0
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            ...
            "confirmation may need a different risk allocation than ordinary "
            "Space breakouts."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_benchmark_breadth_breakout_scalar",
        "parameters": {
            "scalars_tested": list(SCALARS),
            "selected_scalar": best["parameters"][
                "space_benchmark_breadth_breakout_scalar"
            ],
            "target_strategy": TARGET_STRATEGY,
            "accepted_before_experiment": BEFORE_EXPERIMENT_ID,
            "accepted_benchmark_breadth_iwm_leader_trend_scalar": (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space benchmark-breadth breakout risk",
        "",
        "## Hypothesis",
        (
            "Official Space `breakout_long` signals with broad 10d confirmation "
            "versus cash, SPY, QQQ, UFO, and ARKX may need different sizing than "
            "ordinary Space breakouts."
        ),
        "",
        "## Single Changed Variable",
        (
            "`space_benchmark_breadth_breakout_scalar` on top of the accepted "
    ...
    for label, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["benchmark_breadth_breakout_counts_by_window"].get(
            label,
            {},
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_benchmark_breadth_breakout_scalar']}`",
            f"- eligible signals: `{gate['eligible_breakout_signal_count']}`",
            f"- adjusted signals: `{gate['changed_breakout_signal_count']}`",
            f"- adjusted counts: `{best['benchmark_breadth_breakout_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            f"Benchmark-breadth breakout scalar "
            f"{best['parameters']['space_benchmark_breadth_breakout_scalar']} "
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            ...
            "risk allocation from ordinary Space breakouts."
        ),
        "changed_variable": "space_benchmark_breadth_breakout_scalar",
    }

"""exp-20260630-008: refresh options forward outcome settlement ledger.

Measurement repair only. This combines all current OnclickMedia options
observation ledgers, including the 2026-06-24 through 2026-06-29 deltas, and
re-settles available 1/3/5/10 trading-day outcomes against warehouse OHLCV.
...
EXPERIMENT_ID = "exp-20260630-008"
OWNER = "alpha-explore"
SLUG = "options_forward_outcome_refresh_20260629"
RUNNER = f"quant/experiments/exp_20260630_008_{SLUG}.py"
...
OUT_JSON = DATA_DIR / f"exp_20260630_008_{SLUG}.json"
OUTCOME_LEDGER_JSONL = DATA_DIR / "options_forward_outcome_refresh_all_current_sources.jsonl"
...
HYPOTHESIS = (
    "Refresh the OnclickMedia options forward outcome ledger with the new "
    "2026-06-24 through 2026-06-29 observation deltas so future options-flow "
    "alpha tests use materially current closed cash/SPY/QQQ replacement rows "
    "instead of the stale exp-20260624-026 surface."
)
ALPHA_HYPOTHESIS = (
    "Options put/call volume, open interest, IV skew, and liquidity fields may "
    "become orthogonal demand/protection confirmation alpha only after materially "
    "more options forward observations have settled replacement-value outcomes."
)
CHANGE_TYPE = "identity_or_measurement_repair"
MECHANISM_FAMILY = "options_flow_forward_outcome_surface"
TRIAL_FAMILY = "options_forward_outcome_settlement_refresh"
TRIAL_VARIANT_ID = "v1_all_current_onclickmedia_sources"
CHANGED_VARIABLE = "onclickmedia_options_forward_outcome_ledger_refresh_all_current_sources_v1"
NEW_EVIDENCE_TYPE = "materially_more_options_forward_observation_rows"
NEW_EVIDENCE_AXIS = (
    "Adds newly collected options observation delta ledgers from 2026-06-24 "
    "through 2026-06-29 and re-settles cash/SPY/QQQ outcomes; this is not an "
    "options threshold or response-curve retry on the old exp-20260624-026 ledger."
)
...
CAUSAL_COMPONENTS = [
    "all current OnclickMedia options observation ledgers",
    "warehouse OHLCV outcome settlement",
    "cash SPY QQQ replacement-value fields",
    "previous exp-20260624-026 reference comparison",
    "no strategy behavior change",
]
ALLOWED_WRITE_SCOPE = [
    RUNNER,
    "data/experiments/exp-20260630-008/",
    "experiments/logs/exp-20260630-008.json",
    "experiments/cards/exp-20260630-008.md",
    "experiments/manifests/exp-20260630-008.json",
    "experiments/tickets/exp-20260630-008.json",
...
DEFAULT_PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "new_delta_ledgers_missing",
        "no_settlement_progress_vs_exp026",
        "benchmark_settlement_missing",
    ],
    "confidence_reason": (
        "Exp-20260624-026 already settled the first two source ledgers. Later "
        "experiments added 241 more options observation rows, and the warehouse "
        "has more recent bars, so a refresh should either increase closed counts "
        "or at least make the new rows explicit pending rows for future options "
        "alpha tests without touching strategy behavior."
    ),
}
...
def quality_pass(row: Mapping[str, Any]) -> bool:
...
def load_prices(rows: list[dict[str, Any]]) -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    ...
    latest_dates = {
        ticker: ticker_rows[-1]["Date"] if ticker_rows else None
        for ticker, ticker_rows in prices.items()
    }
    return prices, {
        ...
        "start": start,
        "end": end,
        "latest_loaded_dates": latest_dates,
    }
...
def summarize_reference_ledger(path: Path) -> dict[str, Any]:
    rows = read_jsonl(path)
    summary = summarize_outcomes(rows) if rows else {"horizons": {}}
    counts = {
        str(horizon): int(summary.get("horizons", {}).get(str(horizon), {}).get("settled_rows") or 0)
        for horizon in HORIZONS
    }
    quote_dates = sorted({str(row.get("quote_date")) for row in rows if row.get("quote_date")})
    source_experiments = sorted(
        {str(row.get("source_experiment_id")) for row in rows if row.get("source_experiment_id")}
    )
    return {
        "reference_ledger": repo_rel(path),
        "exists": path.exists(),
        "outcome_rows": len(rows),
        "settled_counts": counts,
        "quote_date_start": quote_dates[0] if quote_dates else None,
        "quote_date_end": quote_dates[-1] if quote_dates else None,
...
def evaluate_gate4(
    ...
    reference_counts = reference_summary["settled_counts"]
    source_rows_added = source_metadata["deduped_source_rows"] - int(reference_summary["outcome_rows"] or 0)
    settled_deltas = {
        str(horizon): int(counts.get(str(horizon), 0)) - int(reference_counts.get(str(horizon), 0))
        for horizon in HORIZONS
    }
    ledgers_exist = all(item["exists"] for item in source_metadata["source_ledgers"])
    settlement_progress = (
        any(value > 0 for value in settled_deltas.values())
        or source_rows_added > 0
    )
    checks = {
        ...
        "source_rows_added_vs_exp026": source_rows_added > 0,
        "settled_5d_non_decreasing_vs_exp026": settled_deltas["5"] >= 0,
        "settled_10d_non_decreasing_vs_exp026": settled_deltas["10"] >= 0,
        "settlement_progress_or_new_pending_rows": settlement_progress,
    ...
    return {
        "passed": passed,
        "decision": (
            "accepted_measurement_repair_options_forward_outcomes_refreshed"
            if passed
            else "blocked_options_forward_outcome_refresh"
        ),
        "acceptance_checks": checks,
        "failed_reasons": failed,
        "reference_exp026": reference_summary,
        "source_rows_added_vs_exp026": source_rows_added,
        "settled_count_deltas_vs_exp026": settled_deltas,
...
def calibration(prediction: Mapping[str, Any], success: bool, failed_reasons: list[str]) -> dict[str, Any]:
    ...
    mode_map = {
        "warehouse_missing_recent_rows": {"benchmark_5d_complete", "benchmark_10d_complete"},
        "duplicate_observation_ids": {"no_duplicate_observation_ids"},
        "new_delta_ledgers_missing": {"source_ledgers_exist", "source_rows_added_vs_exp026"},
        "no_settlement_progress_vs_exp026": {"settlement_progress_or_new_pending_rows"},
        "benchmark_settlement_missing": {"benchmark_5d_complete", "benchmark_10d_complete"},
    }
    hit = [
        mode
        for mode in predicted_failures
        if set(failed_reasons) & mode_map.get(mode, {mode})
    ]
    return {
...
def compact_outcome_summary(summary: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "outcome_rows": summary["outcome_rows"],
        "ticker_count": summary["ticker_count"],
        "quote_date_start": summary["quote_date_start"],
        "quote_date_end": summary["quote_date_end"],
        "entry_date_start": summary["entry_date_start"],
        "entry_date_end": summary["entry_date_end"],
        "outcome_status_counts": summary["outcome_status_counts"],
        "quality_pass_rows": summary["quality_pass_rows"],
        "horizons": summary["horizons"],
        "sample_rows": summary["sample_rows"][:2],
    }
...
def build_payload() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    reference_summary = summarize_reference_ledger(PREVIOUS_REFERENCE_LEDGER)
    gate4 = evaluate_gate4(
        source_metadata,
        settlement_metadata,
        outcome_summary,
        reference_summary,
    ...
    settled_counts = settlement_metadata["horizon_settled_counts"]
    why = (
        "The refresh loaded all four current source ledgers, added "
        f"{gate4['source_rows_added_vs_exp026']} rows versus exp-20260624-026, "
        "and kept cash/SPY/QQQ settlement counts non-decreasing while marking "
        "new immature rows as pending."
        if success
        else "The refresh did not meet the predeclared measurement acceptance checks; "
        "failed checks identify the missing dependency."
    )
    payload = {
        ...
        "implementation_mode": "measurement_repair_outcome_ledger_refresh",
        ...
        "pre_run_questions": {
            ...
            "2_history_check": {
                "exp-20260624-026": (
                    "Accepted reusable outcome settlement for exp009 and exp020 "
                    "source ledgers; its reopen condition required materially more "
                    "closed forward rows, PIT vendor/asof controls, borrow context, "
                    "or historical PIT options chains."
                ),
                "exp-20260625-001": (
                    "Rejected options demand-quality attribution on the old exp026 "
                    "ledger and forbade put/call, IV, liquidity, spread, OI, top-N, "
                    "hold, cooldown, notional, and threshold variants on the same rows."
                ),
                "exp-20260629-014": (
                    "Accepted 2026-06-24 through 2026-06-26 options observation deltas."
                ),
                "exp-20260630-003": (
                    "Accepted 2026-06-29 options observation delta rows."
                ),
                "novelty_gate": (
                    "Reservation passed without override; this is an outcome-surface "
                    "refresh using materially more observation rows, not an options "
                    "threshold or response-curve retry."
                ),
            },
            "3_single_policy_bundle": (
                "One measurement bundle: combine all current OnclickMedia options "
                "observation ledgers, settle available 1/3/5/10 trading-day horizons "
                "against warehouse OHLCV, and write cash/SPY/QQQ replacement fields."
            ),
            "4_success_failure_standard": (
                "Accept as measurement repair only if source ledgers load, duplicate "
                "observation IDs are zero, outcome rows match deduped source rows, "
                "rows were added versus exp026, settled counts are non-decreasing, "
                "SPY/QQQ comparators are complete for 5d/10d, and strategy metrics "
                "remain unchanged."
            ),
        ...
        "parameters": {
            ...
            "previous_reference_ledger": repo_rel(PREVIOUS_REFERENCE_LEDGER),
        ...
        "gate2": {
            ...
            "target_price_scope": (
                "Not applicable: options observations are not executable strategy "
                "candidates and this repair uses fixed forward horizons, not target-price exits."
            ),
            "failed_reasons": failed,
        },
        "gate3": {
        ...
        "delta_metrics": {
            ...
            "source_rows_added_vs_exp026": gate4["source_rows_added_vs_exp026"],
            "outcome_rows_written": len(outcome_rows),
            "settled_1d_rows": settled_counts.get("1", 0),
            "settled_3d_rows": settled_counts.get("3", 0),
            "settled_5d_rows": settled_counts.get("5", 0),
            "settled_10d_rows": settled_counts.get("10", 0),
            "settled_count_deltas_vs_exp026": gate4["settled_count_deltas_vs_exp026"],
        },
        ...
        "post_run_reflection": {
            ...
            "forbidden_near_neighbor_retry": (
                ...
                "expiration, moneyness, liquidity, spread, top-N, hold, cooldown, "
                "notional, or threshold rules from this repair. It creates outcome "
                "fields only and does not accept an options alpha."
            ),
            "new_evidence_required": (
                "A valid options alpha retry still needs materially more closed forward "
                "rows, PIT vendor/asof controls, borrow or loan-availability context, "
                "or historical PIT options chains covering canonical windows before "
                "any shared helper promotion."
            ),
        },
        "related_files": [
            RUNNER,
...
def compact_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "outcome_summary": compact_outcome_summary(payload["outcome_summary"]),
...
def build_card(payload: Mapping[str, Any]) -> str:
    ...
    deltas = payload["gate4"]["settled_count_deltas_vs_exp026"]
    horizons = payload["outcome_summary"]["horizons"]
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: options forward outcome refresh",
            "",
            f"- Status: `{payload['status']}`",
            f"- Decision: `{payload['decision']}`",
            f"- Source rows: `{payload['source_metadata']['deduped_source_rows']}`",
            f"- Rows added vs exp-20260624-026: `{payload['gate4']['source_rows_added_vs_exp026']}`",
            f"- Outcome rows: `{payload['outcome_summary']['outcome_rows']}`",
            f"- Settled 1d / 3d / 5d / 10d rows: `{counts.get('1', 0)}` / `{counts.get('3', 0)}` / `{counts.get('5', 0)}` / `{counts.get('10', 0)}`",
            f"- Settled deltas vs exp026 1d / 3d / 5d / 10d: `{deltas.get('1', 0)}` / `{deltas.get('3', 0)}` / `{deltas.get('5', 0)}` / `{deltas.get('10', 0)}`",
        ...
        json.dumps(
            {
                ...
                "rows_added_vs_exp026": payload["gate4"]["source_rows_added_vs_exp026"],
                "outcome_rows": payload["outcome_summary"]["outcome_rows"],
                "settled_1d_rows": counts.get("1", 0),
                "settled_3d_rows": counts.get("3", 0),
                "settled_5d_rows": counts.get("5", 0),
                "settled_10d_rows": counts.get("10", 0),
                "settled_count_deltas_vs_exp026": payload["gate4"]["settled_count_deltas_vs_exp026"],

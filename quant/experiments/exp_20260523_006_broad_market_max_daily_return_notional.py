"""exp-20260523-006: broad-market max-daily-return paper notional.
...
already-selected broad-market candidates whose last 20 close-to-close path is
not dominated by a single outsized up day.
...
EXPERIMENT_ID = "exp-20260523-006"
EXPERIMENT_SLUG = "broad_market_max_daily_return_notional"
...
PROFILE_CONFIG = {
    **DEFAULT_CONFIG,
    ...
    "max_daily_return_20_max": 0.10,
    "max_daily_return_20_notional_scalar": 1.0,
    "max_active_positions": 5,
    "daily_entry_slots": 3,
    "hold_days": 20,
}

BASELINE_VARIANT = "baseline_no_max_daily_return_support"
MAX_DAILY_RETURN_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict(
    [(BASELINE_VARIANT, {"max_daily_return_20_max": 0.10, "scalar": 1.0})]
)
for threshold in (0.08, 0.09, 0.10, 0.11, 0.12):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15):
        ...
        MAX_DAILY_RETURN_SWEEP[
            f"maxday20_lte_{threshold_text}_scalar_{scalar_text}"
        ] = {
            "max_daily_return_20_max": threshold,
            "scalar": scalar,
        }
...
def _variant_trades(
    *,
    ...
    cfg = {
        **PROFILE_CONFIG,
        "max_daily_return_20_max": max_daily_return_20_max,
        "max_daily_return_20_notional_scalar": scalar,
    }
    ...
    for row in base_rows:
        ...
        if trade is None:
            continue
        max_daily_return_20 = float(row["feature"]["max_daily_return_20"])
        support_applied = (
            max_daily_return_20 <= float(max_daily_return_20_max) and float(scalar) != 1.0
        )
        if support_applied:
        ...
        trade["window"] = label
        trade["max_daily_return_20"] = max_daily_return_20
        trade["max_daily_return_20_max"] = max_daily_return_20_max
        trade["max_daily_return_20_support_notional_scalar"] = scalar
        trade["max_daily_return_20_notional_multiplier"] = scalar if support_applied else 1.0
        trade["max_daily_return_20_support_applied"] = support_applied
        trades.append(trade)
    return trades
        ...
        rows.append(
            {
                ...
                "max_daily_return_20_support_applied": trade.get(
                    "max_daily_return_20_support_applied"
                ),
                "max_daily_return_20_notional_multiplier": trade.get(
                    "max_daily_return_20_notional_multiplier"
                ),
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("max_daily_return_20_support_applied")]
    base["max_daily_return_adjusted_trade_count"] = len(adjusted)
    base["max_daily_return_adjusted_pnl"] = round(
...
def _variant_payload(
    *,
    ...
    adjusted = [row for row in all_trades if row.get("max_daily_return_20_support_applied")]
    ...
    return [
        {
            ...
            "max_daily_return_20_max": row["max_daily_return_20_max"],
            "scalar": row["scalar"],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Max-Daily-Return Support Notional",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        _variant_payload(
            variant_name=name,
            max_daily_return_20_max=values["max_daily_return_20_max"],
        ...
        for name, values in MAX_DAILY_RETURN_SWEEP.items()
    ]
    ...
    decision = (
        "accepted_default_off_broad_market_max_daily_return_notional"
        if accepted
        else "rejected_broad_market_max_daily_return_notional"
    )
    ...
    payload: dict[str, Any] = {
        ...
        "hypothesis": (
            ...
            "20-day path is not spike-dominated may convert rank into more stable "
            "allocation quality than names dependent on one outsized day."
        ),
        "change_type": "default_off_paper_allocation",
        "changed_variable": "broad_market_max_daily_return_support_notional_scalar",
        "single_causal_variable": "max daily return cap support scalar for fixed broad-market selected candidates",
        "trial_accounting": {
            "trial_family": "broad_market_state_surface_notional_quality_support",
            "changed_variable": "max_daily_return_20_max_and_scalar",
            "prior_trial_count": 0,
        ...
        "parameters": {
            ...
            "selected_max_daily_return_20_max": selected["max_daily_return_20_max"],
            "selected_max_daily_return_support_scalar": selected["scalar"],
            "sweep": MAX_DAILY_RETURN_SWEEP,
            "profile_config": {
        ...
        "backtest_protocol": (
            ...
            "state; after state adds a max-daily-return paper-notional support field "
            "on the frozen accepted candidate universe."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "selected_variant": {
            ...
            "max_daily_return_20_max": selected["max_daily_return_20_max"],
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "capital allocation / path quality: already-selected broad-market leaders with less spike-dominated 20-day paths may deserve extra paper notional support.",
            "2_past_similar_experiments": "Broad-market rank-notional, low-extension, high-volatility, trend-persistence, trend-efficiency, red-day pullback, strong-close crowding, and repeat-name support were tested; max_daily_return_20 path-quality support was not previously formalized.",
            "3_single_variable": "Only max_daily_return_20_max/scalar changes inside one notional-support field; eligibility, rank profile, low-extension, high-volatility, trend-persistence, hold, slots, and frozen universe stay fixed.",
        ...
        "interpretation": (
            "Max daily return is a new production-visible path-quality field aligned "
            "with the playbook's daily-return-pattern direction. Promotion still "
            "requires identity control plus the same three-window gate used for "
            "other broad-market paper allocation fields, with the stricter >10% "
            "relative aggregate EV rule for this state-surface family."
        ),
        "rejection_reason": None if accepted else "Max-daily-return support did not clear the broad-market three-window gate, the stricter >10% relative aggregate EV rule, and/or identity control.",
        "next_evidence_needed": (
            "Collect more forward broad-market paper outcomes with max_daily_return_20 "
            "metadata; do not retry nearby thresholds on the frozen sample without new "
            "forward evidence or a distinct path-quality field."
        ),
        "related_files": {

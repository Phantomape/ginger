"""exp-20260520-012: SEC neutral earnings-release notional scout.

Alpha search on one production-visible SEC paper-sleeve semantic field. This
tests covered ``earnings_release_text`` rows whose filing text is classified as
``neutral_or_mixed_language`` as a full cohort, instead of splitting the cohort
again by T+1 reaction buckets. It does not change core entries, exits, queue
eligibility, queue capacity, hold days, LLM, news, or live/default orders.
...
EXPERIMENT_ID = "exp-20260520-012"
STEM = "exp_20260520_012_sec_neutral_earnings_release_notional"
...
scout.DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_neutral_earnings_release_notional.md"
)
scout.TARGET_SCALAR_VARIANTS = OrderedDict(
    [
        ...
        ("negative_reaction_scalar_1_025", 1.025),
        ("negative_reaction_scalar_1_05", 1.05),
        ("negative_reaction_scalar_1_075", 1.075),
...
def _notional_for_position(
    ...
    *,
    ...
    if _is_target_position(position):
        ...
        rule_parts.append("neutral_earnings_release_scalar")
    return (
...
def _target_coverage_summary(exp100: dict[str, Any]) -> dict[str, Any]:
    ...
    for label, window in exp100.get("windows", {}).items():
        ...
        target = sum(1 for row in rows if _is_target_candidate(row))
        by_window[label] = {
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} SEC Neutral Earnings-Release Notional",
        "",
    ...
    payload.update(
        {
            ...
            "decision": (
                "promising_sec_neutral_earnings_release_notional"
                if passed
                else "rejected_sec_neutral_earnings_release_notional"
            ),
            "hypothesis": (
                ...
                "may represent factual underreaction without promotional excess. "
            ...
            "change_summary": (
                ...
                "earnings-release rows as one full semantic cohort."
            ),
            "change_type": "alpha_search_semantic_notional_allocation",
            "component": "quant/sec_financial_report_event_sleeve.py",
            "changed_variable": "sec_neutral_earnings_release_notional_scalar",
            "single_causal_variable": (
                "neutral/mixed earnings-release paper-notional scalar"
            ),
            "interpretation": (
                "Neutral/mixed SEC earnings-release rows cleared the paper-sleeve "
                ...
                "parity tests."
                if passed
                else "No neutral/mixed earnings-release scalar cleared the "
            ...
            if passed
                ...
                "No neutral/mixed earnings-release scalar cleared the "
            ...
            "next_evidence_needed": (
                ...
                if passed
                else "Do not retry broad neutral/mixed earnings-release scalars "
                "on the frozen sample without a new semantic field, richer "
                "issuer context, or forward evidence."
            ),
            "why_not_other_changes": (
                "State-surface scalar/profile mining is under strict anti-repeat; "
                "broad-market support scalars have a replay identity issue; LLM "
                "soft-ranking lacks replay-safe attribution; the latest SEC "
                "underreaction and moderate-reaction micro-slices failed tail/"
                "sample guards, so this tests the full neutral earnings-release "
                "cohort instead of a smaller reaction bucket."
            ),
            "protocol_answers": {
                "1_alpha_hypothesis": (
                    "capital allocation: scale SEC paper rows only when "
                    "text_event_type=earnings_release_text and "
                    "language_bucket=neutral_or_mixed_language. This matches "
                    "the playbook priority for SEC semantic expansion while "
                    "avoiding sparse LLM soft-ranking."
                ),
                "2_history_check": (
                    "exp-20260518-014 accepted neutral underreaction with market "
                    "context; exp-20260519-008 accepted earnings-release SPY "
                    "context; exp-20260520-010/011 rejected smaller neutral "
                    "reaction slices because of tail/sample instability."
                ),
                "3_single_causal_variable": (
                    "sec_neutral_earnings_release_notional_scalar"
                ),

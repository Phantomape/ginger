"""exp-20260606-020: macro relief top-2 shared paper adapter validation.

Alpha search with production consistency validation. This promotes the positive
exp-20260606-019 replay lead into a shared default-off paper adapter: on
official CPI/FOMC/NFP dates where both SPY and QQQ rally and close high in the
daily range, admit the two strongest liquid stock leaders as top-2 next-open,
10-trading-day paper candidates.
...
import macro_relief_leadership_paper_sleeve as shared_macro
...
BASE_NOTIONAL_USD = shared_macro.BASE_NOTIONAL_USD
HOLD_DAYS = shared_macro.HOLD_DAYS
MAX_PAPER_TRADES_PER_DAY = shared_macro.MAX_PAPER_TRADES_PER_DAY
SAME_TICKER_COOLDOWN_DAYS = shared_macro.SAME_TICKER_COOLDOWN_DAYS

MIN_SPY_RELIEF_RETURN = shared_macro.MIN_SPY_RELIEF_RETURN
MIN_QQQ_RELIEF_RETURN = shared_macro.MIN_QQQ_RELIEF_RETURN
MIN_SPY_CLOSE_LOCATION = shared_macro.MIN_SPY_CLOSE_LOCATION
MIN_QQQ_CLOSE_LOCATION = shared_macro.MIN_QQQ_CLOSE_LOCATION
MIN_PRICE = shared_macro.MIN_PRICE
MIN_AVG_DOLLAR_VOLUME_20D = shared_macro.MIN_AVG_DOLLAR_VOLUME_20D
MIN_SIGNAL_RETURN = shared_macro.MIN_SIGNAL_RETURN
MIN_RELATIVE_VS_SPY = shared_macro.MIN_RELATIVE_VS_SPY
MIN_RELATIVE_VS_QQQ = shared_macro.MIN_RELATIVE_VS_QQQ
MIN_CLOSE_LOCATION = shared_macro.MIN_CLOSE_LOCATION
MIN_VOLUME_RATIO_20D = shared_macro.MIN_VOLUME_RATIO_20D
MIN_RET20_EXCESS_SPY = shared_macro.MIN_RET20_EXCESS_SPY
MIN_RET60_EXCESS_SPY = shared_macro.MIN_RET60_EXCESS_SPY
MIN_RET5 = shared_macro.MIN_RET5
MAX_RET5 = shared_macro.MAX_RET5
MAX_REALIZED_VOL_20D = shared_macro.MAX_REALIZED_VOL_20D
...
PREDICTION = {
    ...
    "confidence_reason": (
        "exp-20260606-019 passed Gate 4 on all three canonical windows, but "
        "was replay-only. This test changes only implementation boundary: the "
        "same candidate and selection semantics are run through a shared "
        "default-off adapter that daily production can observe."
    ),
    "recorded_at": "2026-06-06T18:17:00Z",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        "The same shared adapter exposes the official CPI/FOMC/NFP calendar, "
        "SPY/QQQ relief-day test, broad-market sector-known liquid stock "
        "universe, stock leadership fields, same-ticker core-overlap "
        "exclusion, top-2/day selection, next-open paper entry, 10-trading-day "
        "exit, costs, cooldown, and concentration controls in replay and daily "
        "production. The daily adapter is default-off and paper-only; it cannot "
        "alter candidate priority, sizing, watchlists, or orders without a "
...
def _macro_event_map() -> dict[str, list[dict[str, Any]]]:
    ...
    for row in shared_macro.MACRO_EVENTS:
...
def _candidate_for_ticker(
    *,
    ...
    return {
        "date": signal_date,
        "ticker": ticker,
        "source": "MACRO_RELIEF_LEADERSHIP_PAPER",
...
def _candidate_rows_for_window(
    *,
    ...
    candidate_universe = {
        "status": "exp020_broad_warehouse_sector_known",
        "tickers": sorted(sector_entries),
        "records": sector_entries,
    }
    return shared_macro.candidate_rows_for_window(
        snapshot=snapshot,
        dates=dates,
...
def _select_paper_trades(
    *,
    ...
    rows_by_ticker = shared_macro._normalise_ohlcv_by_ticker(snapshot)
    return shared_macro.select_paper_trades(
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "accepted_macro_relief_top2_shared_default_off_paper_adapter"
        if gate["passed"]
        else "rejected_macro_relief_top2_shared_default_off_paper_adapter"
    )
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "The positive exp-20260606-019 macro relief top-2 stock "
                "leadership source can be implemented as one shared "
                "default-off paper adapter without changing the three-window "
                "evidence or creating a production/backtest mismatch."
            ),
            ...
            "new_evidence_type": "shared_adapter_production_parity_validation",
            "nearby_prior_experiments": [
                "exp-20260606-017",
                "exp-20260606-019",
            ],
            "prior_trial_count": 2,
            "multiple_testing_risk_bucket": "low_for_shared_implementation_high_for_live_activation",
            "prediction": PREDICTION,
            "production_impact": PRODUCTION_IMPACT,
            "anti_js": "No JavaScript was used.",
            "negative_reflection": (
                "If rejected, the shared helper changed replay semantics or "
                "the production-visible universe path is materially different "
                "from the broad warehouse used by exp019. A retry would need "
                "a precise parity repair or forward replacement-value rows, "
                "not relief threshold/top-N/notional retuning."
            ),
        }
    )
    ...
    payload["gate_questions"] = {
            "1_alpha_hypothesis": (
                "candidate_pool/production_parity: macro relief top-2 "
                "leadership already passed three-window replay in exp019; "
                "the alpha value now is preserving that source in a shared "
                "default-off adapter that production can observe without "
                "changing live trading."
            ),
            "2_history_check": {
                "exp-20260606-017": (
                    "Top-1 official macro relief leadership improved all "
                    "three windows by aggregate EV +0.1078 and PnL +$1,819.71 "
                    "but failed because it produced only 10 trades."
                ),
                "exp-20260606-019": (
                    "Top-2 official macro relief leadership passed Gate 4 "
                    "as replay-only with all three windows positive, 20 "
                    "target trades, and aggregate EV +0.1813 / PnL +$3,062.78."
                ),
        },
        "3_single_causal_variable": CHANGED_VARIABLE,
        "4_acceptance_standard": (
            "Use docs/backtesting.md three canonical windows. Aggregate EV/PnL "
            "must remain positive versus the canonical core baseline with no "
            "window regression, target sample >=20 across all 3 windows, "
            "survival >=5%, drawdown drift <=0.5pp, and concentration guard "
            "must pass. Production impact must remain paper-only/default-off."
        ),
        "5_reproducibility": (
    ...
    payload["interpretation"] = (
        "The official macro relief leadership candidate pool cleared Gate 4 "
        "through the shared default-off paper adapter. It is production-visible "
        "for forward observation, but still cannot alter live/default orders."
        if payload["gate4"]["passed"]
        else (
            "The shared macro relief leadership adapter did not clear Gate 4. "
            "Do not promote or retune this macro-relief stock leadership "
            "definition on the frozen windows without a concrete parity repair "
    ...
    payload["next_evidence_needed"] = (
        "Forward default-off paper rows from the shared adapter, and then a "
        "separate activation experiment if those rows pass the forward gate. "
        "Do not retune top-N, relief returns, close-location thresholds, hold "
        "days, cooldown, or paper notional on the frozen windows."
    )
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Macro Relief Top-2 Shared Paper Adapter",
            "",
            ...
            *rows,
            ...
            "",
            (
                "Shared default-off paper adapter only. Daily production can "
                "observe the surface, but trade_enabled remains false and no "
                "production watchlist, order path, core entry, ranking, sizing, "
                "or exit behavior changes."
            ),
            "",

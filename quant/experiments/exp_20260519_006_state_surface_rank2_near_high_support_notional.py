...
exp-20260519-004 and the rejected exp-20260519-005 front-rank result, then
tests one narrower production-visible allocation profile: only queue rank 2
candidates whose own price is near its 60-day high receive bounded default-off
paper-notional support.
...
import exp_20260519_005_state_surface_front_rank_near_high_support_notional as prev


EXPERIMENT_ID = "exp-20260519-006"
EXPERIMENT_SLUG = "state_surface_rank2_near_high_support_notional"
...
RULE_VERSION = "state_surface_rank2_near_high_support_notional_v1"

DOC_HELPERS = prev.parent.parent
CORE_HELPERS = prev.parent.parent.repeat.parent.parent.parent
CONCENTRATION_HELPERS = prev.parent.parent.repeat.parent.parent

RANK2_NEAR_HIGH_VARIANTS: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
        ...
        (
            "rank2_near_high_ge_098_scalar_125",
            {
                "near_high_min": 0.98,
                "scalar": 1.25,
                "target_queue_rank": 2,
                "aggression_order": 1,
                "description": "rank 2 candidate within 2% of 60-day high receives 25% support",
            },
        ),
        (
            "rank2_near_high_ge_098_scalar_150",
            {
                "near_high_min": 0.98,
                "scalar": 1.50,
                "target_queue_rank": 2,
                "aggression_order": 2,
                "description": "rank 2 candidate within 2% of 60-day high receives 50% support",
            },
        ),
        (
            "rank2_near_high_ge_0975_scalar_125",
            {
                "near_high_min": 0.975,
                "scalar": 1.25,
                "target_queue_rank": 2,
                "aggression_order": 3,
                "description": "rank 2 candidate within 2.5% of 60-day high receives 25% support",
            },
        ),
        (
            "rank2_near_high_ge_0975_scalar_150",
            {
                "near_high_min": 0.975,
                "scalar": 1.50,
                "target_queue_rank": 2,
                "aggression_order": 4,
                "description": "rank 2 candidate within 2.5% of 60-day high receives 50% support",
            },
        ),
    ]
)
...
def _apply_rank2_near_high_support(
    ...
    *,
    ...
    for trade in trades:
        ...
        near_high = _float(features.get("near_high_60"))
        applies = (
        ...
        row["rank2_near_high_support_variant"] = variant_name
        row["rank2_near_high_support_rule_version"] = RULE_VERSION
        row["rank2_near_high_60"] = near_high
        row["rank2_near_high_support_target_queue_rank"] = target_rank
        row["rank2_near_high_support_min"] = threshold
        row["rank2_near_high_support_configured_scalar"] = scalar
        row["rank2_near_high_support_scalar"] = scalar if applies else None
        row["rank2_near_high_support_applied"] = bool(applies)
        row["rank2_near_high_support_profile_name"] = (
        ...
        row["rank_notional_rank2_near_high_support_rule_version"] = RULE_VERSION
        ...
        if applies:
            ...
            row["rank2_near_high_support_base_notional"] = base_notional
        ...
        rows.append(
            {
                ...
                "rank2_near_high_support_min": trade.get(
                    "rank2_near_high_support_min"
                ),
                "rank2_near_high_support_scalar": trade.get(
                    "rank2_near_high_support_scalar"
                ),
                "rank2_near_high_support_profile_name": trade.get(
...
def _notional_by_support_profile(trades: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for trade in trades:
        key = str(trade.get("rank2_near_high_support_profile_name") or "baseline")
...
def _variant_payload(
    *,
    ...
    selected = _apply_rank2_near_high_support(
        baseline_trades,
    ...
    for label in WINDOWS:
        ...
        applied = [row for row in adjusted if row.get("rank2_near_high_support_applied")]
        surface_sleeve[label] = {
            "selected_trade_count": len(adjusted),
            "rank2_near_high_support_adjusted_trade_count": len(applied),
            "rank2_near_high_support_adjusted_pnl": round(
            ...
            "notional_by_rank2_near_high_support_profile": (
                _notional_by_support_profile(adjusted)
            ),
            "selected_trades": _selected_trade_rows(adjusted),
        }
    applied_all = [row for row in selected if row.get("rank2_near_high_support_applied")]
    applied_windows = {str(row.get("window")) for row in applied_all if row.get("window")}
    return {
        "variant_name": variant_name,
        "variant_type": "rank2_near_high_support_notional_profile",
        ...
        "rank2_near_high_support_adjusted_trade_count": len(applied_all),
        "rank2_near_high_support_adjusted_windows": sorted(applied_windows),
...
def _gate4_for_variant(
    *,
    ...
    adjusted_guard_passed = (
        variant["rank2_near_high_support_adjusted_trade_count"] >= MIN_ADJUSTED_TRADES
        and len(variant["rank2_near_high_support_adjusted_windows"])
    ...
    return {
        ...
        "rank2_near_high_support_adjusted_trade_count": variant[
            "rank2_near_high_support_adjusted_trade_count"
        ],
        "rank2_near_high_support_adjusted_windows": variant[
            "rank2_near_high_support_adjusted_windows"
        ],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rank-2 Near-High Support Notional",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {passed} | {threshold} | {scalar} | {rank} | {ev:+.4f} | ${pnl:+,.2f} | {wi} | {wr} | {adj} | {dd:+.4%} | {share} |".format(
                ...
                adj=row["gate4"]["rank2_near_high_support_adjusted_trade_count"],
    ...
    for label in WINDOWS:
        ...
        lines.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {bdd:.2%} | {add:.2%} | {trades} |".format(
                ...
                trades=sleeve["rank2_near_high_support_adjusted_trade_count"],
            )
        )
    lines.extend(
        [
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for variant_name, variant in RANK2_NEAR_HIGH_VARIANTS.items()
    ]
    sweep_summary = []
    ...
    decision = (
        "accepted_default_off_state_surface_rank2_near_high_support_notional"
        if passed
        else "rejected_state_surface_rank2_near_high_support_notional"
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": "Rank-2 rotation state-surface candidates near their own 60-day highs represent resilient continuation alternatives and deserve bounded default-off paper-notional support.",
        "alpha_hypothesis": {
            "category": "capital allocation",
            "entry_exit_ranking_or_allocation": "default-off paper allocation",
            "playbook_alignment": "Continues state-surface maturation with a rank-depth quality field, avoids LLM soft-ranking data limits, and avoids broad noisy candidate-pool expansion.",
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "rank2_near_high_support_notional_profile",
        ...
        "parameters": {
            ...
            "profile_priority": "applies after accepted rank-notional and rank3 support profiles, before pending paper entry; applies only to queue_rank == 2",
            "locked_variables": [
                "core entries",
                "core exits",
                "core sizing",
                ...
                "front-rank near-high support rejected in exp-20260519-005",
                "LLM/news",
                "live/default orders",
            ],
        },
        ...
        "history_check": {
            "exp-20260519-005": "Rejected front-rank near-high support because mid_weak EV regressed and drawdown drift exceeded guardrail; this experiment removes queue_rank 1 exposure from the causal variable.",
            "exp-20260519-004": "Accepted rank-3 near-high support; this experiment freezes it and tests rank-2 own near-high confirmation, not rank3 threshold/scalar retuning.",
            "anti_repeat": "Not a score-gap, ret20, ret60, sector-cohesion, repeat-lookback, candidate-pool, or LLM soft-ranking retry.",
        },
        "llm_metrics": {
            "used_llm": False,
        ...
        "interpretation": (
            "Rank-2 near-high support improved the default-off state-surface paper overlay in all three windows while staying paper-only and production-visible."
            if passed
            else "Rank-2 near-high support did not clear Gate 4; keep the accepted rank3-support stack unchanged."
        ),
        ...
        "next_evidence_needed": (
            ...
            if passed
            else "Do not retry nearby rank2 near-high profiles without forward evidence or a distinct quality field."
        ),
        "protocol_answers": {
            "1_alpha_hypothesis": "capital allocation: queue_rank == 2 candidates close to 60-day highs indicate resilient continuation quality and deserve bounded paper-notional support.",
            "2_history_check": "Prior accepted work covered rank1 isolation and rank3 queue-depth support; exp-20260519-005 rejected broad rank1/2 near-high support, so this isolates rank2 only.",
            "3_single_causal_variable": "rank2_near_high_support_notional_profile",
...
def main() -> None:
    ...
    DOC_HELPERS._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rank-2 near-high support notional",
            "status": payload["status"],
            "decision": payload["decision"],
            "artifact": DOC_HELPERS._repo_rel(OUT_JSON),
            "summary": (
                f"Rank-2 near-high profile {payload['parameters']['best_variant']} "

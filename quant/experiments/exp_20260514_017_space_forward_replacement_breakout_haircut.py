...
risk for breakout_long signals after the trend-only interaction worked.
...
EXPERIMENT_ID = "exp-20260514-017"
STEM = "space_forward_replacement_breakout_haircut"
...
BREAKOUT_HAIRCUT_SCALARS = (1.00, 0.90, 0.75, 0.50, 0.25, 0.00)
...
def _run_variant(
    ...
    *,
    ...
    def size_with_breakout_haircut_scalar(
        ...
        for signal in sized:
            ...
            is_breakout_strength_signal = (
                ticker in strength_tickers and strategy == "breakout_long"
            )
            if is_breakout_strength_signal and sizing:
                counts["eligible_signal"] += 1
                counts[f"eligible_{ticker}"] += 1
                shares_before, shares_after = _scale_and_count(
                    sizing=sizing,
                    scalar=breakout_haircut_scalar,
                    portfolio_value=portfolio_value,
                    marker="space_forward_replacement_breakout_haircut",
                    counts=counts,
                    ticker=ticker,
                )
                ...
                signal = {
                    **signal,
                    "space_forward_replacement_breakout_haircut_bucket": True,
                    "space_forward_replacement_breakout_haircut_scalar": (
                        breakout_haircut_scalar
                    ),
                }
    ...
    portfolio_engine.size_signals = size_with_breakout_haircut_scalar
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "breakout_haircut_trade_attribution": _space_trade_attribution_for(
                    result,
                    set(str(row.get("ticker") or "").upper() for row in adjustments[before_adjustments:]),
                ),
                "breakout_haircut_adjustment": source_diversity_exp._adjustment_summary(
                    adjustments[before_adjustments:]
                ),
                "breakout_haircut_counts": dict(sorted((counts - before_counts).items())),
            }
        ...
        return {
            ...
            "breakout_haircut_adjustment_summary": (
                source_diversity_exp._adjustment_summary(adjustments)
            ),
            "breakout_haircut_counts": dict(sorted(counts.items())),
            "breakout_haircut_adjustment_sample": adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["breakout_haircut_counts"].get(
            "space_forward_replacement_breakout_haircut_changed_signal",
            0,
        )
    )
    eligible_count = int(variant["breakout_haircut_counts"].get("eligible_signal", 0))
    scalar = float(
        variant["parameters"]["space_forward_replacement_breakout_haircut_scalar"]
    )
    return {
        ...
        "eligible_breakout_haircut_signal_count": eligible_count,
        "changed_breakout_haircut_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _gate2_runtime_state(before: dict[str, Any]) -> dict[str, Any]:
    ...
    for window_name, payload in before.get("by_window", {}).items():
        counts = payload.get("breakout_haircut_counts") or {}
        eligible += int(counts.get("eligible_signal", 0) or 0)
        summary = payload.get("breakout_haircut_adjustment") or {}
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space forward replacement breakout haircut",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_forward_replacement_breakout_haircut_scalar` for `breakout_long` "
            "signals already in the accepted forward same-theme replacement-strength "
            "bucket. Candidate pool, "
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_forward_replacement_breakout_haircut_scalar']}`"
        ),
        (
        ...
        (
            "- Breakout-strength signals changed: "
            f"`{gate['changed_breakout_haircut_signal_count']}` of "
            f"`{gate['eligible_breakout_haircut_signal_count']}` eligible"
        ),
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["by_window"][name]["breakout_haircut_adjustment"][
...
def run() -> dict[str, Any]:
    ...
    before = _run_variant(
        "accepted_exp009_breakout_neutral",
        breakout_haircut_scalar=1.0,
    ...
    gate2 = {
        ...
        "breakout_haircut_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    variants = [
        _run_variant(
            ...
            breakout_haircut_scalar=scalar,
            forward_gate=forward_gate,
            source_diversity_gate=source_diversity_gate,
            attention_gate=attention_gate,
            single_event_gate=single_event_gate,
        ...
        for scalar in BREAKOUT_HAIRCUT_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested breakout haircut for the replacement-strength bucket improved "
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Exp-20260514-009 showed the forward same-theme replacement-strength "
            "edge was strongest as a trend continuation helper. On top of accepted "
            "exp-20260514-009, this tests whether the same bucket should haircut "
            "breakout_long signals that may carry more tail noise "
        ...
        "changed_variable": "space_forward_replacement_breakout_haircut_scalar",
        "single_causal_variable": (
            "risk scalar for breakout_long signals in the accepted forward "
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "risk allocation: haircut Space breakout_long signals whose closed "
                "10d forward profile passes the accepted same-theme "
                "replacement-strength bucket."
            ),
            "q2_prior_experiments": [
                ...
                "No prior record found for a same-theme replacement-strength breakout haircut.",
            ],
            "q3_single_causal_variable": (
                "Only the breakout_long replacement-strength scalar changes; accepted "
        ...
        "parameters": {
            ...
            "tested_breakout_haircut_scalars": list(BREAKOUT_HAIRCUT_SCALARS),
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_forward_replacement_breakout_haircut"
            if decision == "accepted"
            else "rejected_space_forward_replacement_breakout_haircut"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not promote replacement-strength breakout haircut risk from "
        ...
        "why_not_other_changes": (
            ...
            "and tests whether the accepted replacement-value bucket should be "
            "strategy-aware on the risk-allocation side."
        ),
        "known_risks": [
            ...
            "The strategy split is same-sample and should stay conservative even if accepted.",
        ],
    }
    return payload
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_breakout_haircut_signal_count": result["best_variant_gate"][
            "changed_breakout_haircut_signal_count"
        ],

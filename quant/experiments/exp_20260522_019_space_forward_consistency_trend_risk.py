...
whether source-diverse dual-catalyst trend signals deserve a small extra risk
scalar when closed 10d forward replacement rows show multi-event consistency.

The tested field is production-visible and already emitted in the shared Space
forward profile: at least two closed events, with every closed row positive
...
boundary, benchmark-breadth scalar, and live Space slots fixed.
"""
...
EXPERIMENT_ID = "exp-20260522-019"
STEM = "space_forward_consistency_trend_risk"
...
TARGET_MARKER = "space_dual_catalyst_source_diversity_trend_risk"
MARKER = "space_forward_consistency_trend_risk"
MIN_CLOSED_EVENT_COUNT = 2
SCALARS = (1.0125, 1.025, 1.05)

COUNT_FIELDS = (
    "positive_cash_count",
    "positive_same_theme_count",
    "positive_spy_count",
    "positive_qqq_count",
    "positive_ufo_count",
    "positive_arkx_count",
...
REQUIRED_PROFILE_FIELDS = ("closed_event_count", *COUNT_FIELDS, *AVG_FIELDS)
...
def _missing_profile_fields(profile: dict[str, Any] | None) -> list[str]:
    if not profile:
...
def _is_forward_consistency_profile(profile: dict[str, Any] | None) -> bool:
    if not profile:
        return False
    closed_count = _safe_int(profile.get("closed_event_count"))
    if closed_count is None or closed_count < MIN_CLOSED_EVENT_COUNT:
        return False
    for field in COUNT_FIELDS:
        positive_count = _safe_int(profile.get(field))
        if positive_count is None or positive_count < closed_count:
            return False
    for field in AVG_FIELDS:
        value = _safe_float(profile.get(field))
        if value is None or value <= 0.0:
            return False
    return True


def _target_profiles(gates: dict[str, Any]) -> dict[str, dict[str, Any]]:
    _ = gates
    profiles = space_catalyst_forward_replacement_positive_profiles()
    return {
        ticker: profile
        for ticker, profile in sorted(profiles.items())
        if _is_forward_consistency_profile(profile)
    }


def _run_variant(
    *,
    consistency_scalar: float,
    gates: dict[str, Any],
) -> dict[str, Any]:
    original_apply_scale = BASE._apply_scale
    consistency_profiles = _target_profiles(gates)
    consistency_adjustments: list[dict[str, Any]] = []
    ...
    def patched_apply_scale(
        *,
        ...
        forward_profile = consistency_profiles.get(ticker)
        counts[f"{MARKER}_candidate_signal"] += 1
        counts[f"{MARKER}_candidate_{ticker}"] += 1
        if forward_profile is None:
            counts[f"{MARKER}_below_consistency_signal"] += 1
            counts[f"{MARKER}_below_consistency_{ticker}"] += 1
            return

        original_apply_scale(
            signal=signal,
            sizing=sizing,
            scalar=consistency_scalar,
            portfolio_value=portfolio_value,
            marker=MARKER,
            counts=counts,
        ...
        signal["space_forward_consistency_trend_bucket"] = True
        signal["space_forward_consistency_trend_scalar"] = consistency_scalar
        signal["space_forward_consistency_closed_event_count"] = (
            forward_profile.get("closed_event_count")
        )
        signal["space_forward_consistency_event_ids"] = list(
            forward_profile.get("event_ids") or []
        )
    ...
    target_profiles = _target_profiles(gates)
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_forward_consistency_trend_scalar": consistency_scalar,
        "min_closed_event_count": MIN_CLOSED_EVENT_COUNT,
        "required_positive_count_fields": list(COUNT_FIELDS),
        "required_positive_avg_fields": list(AVG_FIELDS),
        "target_strategy": TARGET_STRATEGY,
        "target_marker": TARGET_MARKER,
        "target_tickers": sorted(target_profiles),
    }
    variant["forward_consistency_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["forward_consistency_counts_by_window"] = by_window_counts
    variant["forward_consistency_adjustment_summary"] = summarizer(
        consistency_adjustments
    )
    variant["forward_consistency_adjustment_sample"] = consistency_adjustments[:25]
    return variant
...
def _forward_consistency_profile_field_check(gates: dict[str, Any]) -> dict[str, Any]:
    _ = gates
    profiles = space_catalyst_forward_replacement_positive_profiles()
    target_profiles = _target_profiles(gates)
    missing_by_ticker = {
        ticker: _missing_profile_fields(profile)
        for ticker, profile in sorted(profiles.items())
    }
    missing_by_ticker = {
        ticker: fields for ticker, fields in missing_by_ticker.items() if fields
    }
    return {
        "passed": bool(target_profiles) and not missing_by_ticker,
        ...
        "target_tickers": sorted(target_profiles),
        "all_profile_tickers": sorted(profiles),
        "missing_profile_fields": missing_by_ticker,
        "target_profiles": target_profiles,
    }


def _forward_consistency_signal_field_check(variant: dict[str, Any]) -> dict[str, Any]:
    counts = variant.get("forward_consistency_counts") or {}
    candidate_count = int(counts.get(f"{MARKER}_candidate_signal", 0) or 0)
    missing_count = int(counts.get(f"{MARKER}_missing_profile_signal", 0) or 0)
    return {
        "passed": candidate_count > 0 and missing_count == 0,
        "candidate_signal_count": candidate_count,
        "missing_profile_signal_count": missing_count,
        "counts": counts,
    }
...
def _gate_variant(variant: dict[str, Any], current_variant: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("forward_consistency_counts") or {}
    by_window_counts = variant.get("forward_consistency_counts_by_window") or {}
    ...
    scalar = float(variant["parameters"]["space_forward_consistency_trend_scalar"])
    current_ev = float(current_variant["aggregate"]["expected_value_score_sum"])
    required_ev_delta = current_ev * 0.10
    passed = bool(
        scalar > 1.0
    ...
    return {
        ...
        "reasons": {
            ...
            "profile_fields_complete": missing_count == 0,
...
def _record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "parameters": {
            ...
            "selected_scalar": selected["space_forward_consistency_trend_scalar"],
            "min_closed_event_count": MIN_CLOSED_EVENT_COUNT,
            "required_positive_count_fields": list(COUNT_FIELDS),
            "required_positive_avg_fields": list(AVG_FIELDS),
            "target_tickers": selected["target_tickers"],
            "accepted_before_experiment": CURRENT_ACCEPTED_EXPERIMENT_ID,
        ...
        "gate_answers": {
            ...
            "2_prior_similar_experiments": [
                "exp-20260514-047 accepted benchmark breadth plus same-theme strength on an older stack.",
                "exp-20260519-027 accepted the final dual-catalyst benchmark-breadth precision scalar; nearby scalar mining needs a different field.",
                "exp-20260521-017 rejected a mature same-theme replacement pool change; this run keeps the pool fixed and tests row-level multi-event consistency.",
            ],
            "3_single_causal_variable": (
                "Only one Space capital-allocation variable changes: an extra "
                "forward-consistency scalar. Pool, entry, exit, ranking, LLM/news, "
                "event profiles, benchmark-breadth scalar, and live slots remain fixed."
            ),
            "4_success_criteria": (
                "Aggregate EV delta must exceed 10% of current Space aggregate EV, "
                "aggregate PnL must improve, at least two windows must improve, no "
                "window may regress, drawdown drift <= 0.5 pp, survival >= 5%, "
                "trade count >= 50, fields complete, and adjusted cohort spans at "
                "least two tickers and two windows."
            ),
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\"
                "exp_20260522_019_space_forward_consistency_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        "field_checks": {
            ...
            "profile": payload["profile_field_check"],
        ...
        if promoted
            ...
            "Gate 4 failed: no forward-consistency scalar cleared the strict "
            "10% EV, multi-window, no-regression, cohort breadth, drawdown, "
            "survival, and trade-count acceptance gates."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not promote this forward-consistency scalar from the current "
            "sample. Revisit only after new closed Space forward rows broaden "
            "the multi-event consistency cohort beyond its current support."
        ),
        "production_impact": {
        ...
        "why_not_other_changes": (
            "LLM soft-ranking is still underpowered; noisy ticker expansion would "
            "violate candidate-governance guidance; and the recent accepted/rejected "
            "Space work already mined adjacent benchmark-breadth and pool variants. "
            "This run tests a different production-visible forward-quality field."
        ),
        "related_files": [
            "quant/experiments/exp_20260522_019_space_forward_consistency_trend_risk.py",
            "data/experiments/exp-20260522-019/space_forward_consistency_trend_risk.json",
            "experiments/logs/exp-20260522-019.json",
            "experiments/tickets/exp-20260522-019.json",
            "experiments/artifacts/exp-20260522-019_space_forward_consistency_trend_risk.md",
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space forward-consistency trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        f"- forward profile field check passed: `{payload['profile_field_check']['passed']}`",
        f"- signal field check passed: `{payload['signal_field_check']['passed']}`",
        f"- target tickers: `{payload['profile_field_check']['target_tickers']}`",
        f"- candidate signals: `{payload['signal_field_check']['candidate_signal_count']}`",
        f"- missing profile signals: `{payload['signal_field_check']['missing_profile_signal_count']}`",
        "",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{selected['space_forward_consistency_trend_scalar']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Space forward-consistency scalar "
            f"{selected['space_forward_consistency_trend_scalar']} changed "
            f"{gate['changed_signal_count']} signals with EV delta "
...
def run() -> dict[str, Any]:
    ...
    profile_field_check = _forward_consistency_profile_field_check(gates)
    ...
    if not profile_field_check["passed"]:
        raise RuntimeError(
            f"Forward-consistency profile field check failed: {profile_field_check}"
        )

    current_variant = _load_current_variant()
    variants = [
        _run_variant(consistency_scalar=scalar, gates=gates)
        for scalar in SCALARS
    ]
    signal_field_check = _forward_consistency_signal_field_check(variants[0])
    if not signal_field_check["passed"]:
        raise RuntimeError(
            f"Forward-consistency signal field check failed: {signal_field_check}"
        )
    for variant in variants:
        variant["gate"] = _gate_variant(variant, current_variant)

    accepted = [variant for variant in variants if variant["gate"]["passed"]]
    selection_pool = accepted or variants
    best = max(
        selection_pool,
        key=lambda item: (
    ...
    payload = {
        ...
        "profile_field_check": profile_field_check,
        ...
        "gate_results_by_variant": [
            {
                "scalar": variant["parameters"][
                    "space_forward_consistency_trend_scalar"
                ],
                **variant["gate"],
            }
            for variant in variants
        ...
        "hypothesis": (
            ...
            "forward rows that show multi-event, all-benchmark positive "
            "consistency should represent higher-quality catalyst continuation "
            "than average-only replacement strength; a small risk scalar could "
            "improve EV without changing the candidate pool or production/LLM "
            "boundaries."
        ),
        "changed_variable": "space_forward_consistency_trend_risk_scalar",
    }
            ...
            _safe(
                {
                    "experiment_id": EXPERIMENT_ID,
                    "decision": payload["decision"],
                    "best_scalar": selected["space_forward_consistency_trend_scalar"],

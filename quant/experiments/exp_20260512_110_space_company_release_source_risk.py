"""exp-20260512-110: Space company-release customer-source risk.
...
profile has a customer-win catalyst from a company_release source. This is the
source-type complement to the rejected primary-authority split in exp-20260512-944.
"""
...
STEM = "space_company_release_source_risk"
TARGET_EVENT_FIELD = "customer_win"
TARGET_SOURCE_TYPES = ("company_release",)
COMPANY_RELEASE_SOURCE_RISK_SCALARS = (0.75, 0.90, 1.00, 1.05, 1.075, 1.10, 1.25)
...
def _field_check_company_release_source() -> dict[str, Any]:
...
def _install_space_policy(
    ...
    target_tickers = set(company_release_gate["target_tickers"])
    profiles = company_release_gate["profiles"]
    company_release_adjustments: list[dict[str, Any]] = []
    company_release_counts = Counter()
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                company_release_counts["eligible_signal"] += 1
                shares_before = int(sizing.get("shares_to_buy") or 0)
                _scale_sizing(
                    sizing,
                    company_release_scalar,
                    portfolio_value,
                    "space_company_release_source_risk",
                )
                company_release_adjustments.append(
                    {
                        "ticker": ticker,
                        "strategy": signal.get("strategy"),
                        "marker": "space_company_release_source_risk",
                        "space_company_release_source_profile": profiles.get(ticker),
                        "scalar": company_release_scalar,
                ...
                signal = {
                    **signal,
                    "sizing": sizing,
                    "space_company_release_source_profile": profiles.get(ticker),
                    "space_company_release_source_eligible": True,
                }
            out.append(signal)
        return out
...
def _run_variant(
    ...
    return {
        ...
        "target_event_field": TARGET_EVENT_FIELD,
        "target_source_types": list(TARGET_SOURCE_TYPES),
        "target_tickers": company_release_gate["target_tickers"],
        "space_company_release_source_scalar": company_release_scalar,
        "by_window": by_window,
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_company_release_source_adjustment"]["adjusted_signal_count"]
    ...
    passed = (
        ...
        and variant["space_company_release_source_scalar"] != 1.0
    )
    return {
        "passed": passed,
        ...
        "space_company_release_source_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space company-release source risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: risk scalar for official Space customer-win signals "
            "from company_release event seeds."
        ),
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | {variant['space_company_release_source_scalar']:.3f} | "
            ...
            f"{gate['space_company_release_source_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
            ...
            (
                ...
                "dPnL | Trades | Max DD | Survival | Company-release signals |"
            ),
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for label in WINDOWS:
        ...
        adjusted = best["by_window"][label][
            "space_company_release_source_adjustment"
    ...
    lines.extend(
        [
            ...
            "## Field Check",
            "",
            json.dumps(payload["gate2"]["company_release_source_profile"], sort_keys=True),
            "",
            "## Interpretation",
            "",
...
def run() -> dict[str, Any]:
    ...
    if not company_release_gate["passed"]:
        raise RuntimeError(
            f"Company-release source field check failed: {company_release_gate}"
        )
    ...
    variants = {}
    for scalar in COMPANY_RELEASE_SOURCE_RISK_SCALARS:
        name = f"company_release_source_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_company_release_source_risk"
        if accepted
        else "rejected_space_company_release_source_risk"
    )
    interpretation = (
        "The company-release customer-source scalar improved the accepted "
        ...
        if accepted
            ...
            "The company-release customer-source scalar did not clear the "
            "three-window gate on top of exp-20260512-041. Do not retry adjacent "
            "customer-source type splits on these frozen snapshots without forward "
    ...
    payload = {
        ...
        "changed_variable": "space_company_release_customer_source_risk_scalar",
        "single_causal_variable": (
            "risk scalar for official Space signals whose event seed profile has "
            "customer_win from company_release source_type"
        ),
        "hypothesis": (
            "The accepted broad customer-source field in exp-20260512-038 may be "
            "hiding source-type dispersion. Since the official/regulatory split in "
            "exp-20260512-944 failed, company-release customer-win signals may carry "
            "the remaining replacement value without touching candidate pool, "
            "ranking, LLM, exits, or live slots."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
            ...
            "2_history_check": {
                "exp-20260512-038": (
                    ...
                    "whether its company_release subset deserves a distinct budget."
                ),
                "exp-20260512-041": (
                    "Accepted financing/dilution profile 1.075x scalar; this is the "
                    "fixed before state."
                ),
                "exp-20260512-944": (
                    "Rejected official/regulatory customer-source split; this run "
                    "tests the complementary company_release subset."
                ),
                "exp-20260512-023": (
            ...
            "3_single_causal_variable": (
                "space_company_release_customer_source_risk_scalar. Candidate pool, "
            ...
            "4_acceptance_standard": (
                ...
                "and nonzero adjusted company-release signals."
            ),
            "5_reproducibility": (
                "Run .venv\\Scripts\\python.exe "
                "quant\\experiments\\exp_20260512_110_space_company_release_source_risk.py"
            ),
        },
        "parameters": {
            ...
            "target_source_types": list(TARGET_SOURCE_TYPES),
            "target_tickers": company_release_gate["target_tickers"],
            "source_profiles": company_release_gate["profiles"],
            "tested_company_release_source_scalars": list(
                COMPANY_RELEASE_SOURCE_RISK_SCALARS
            ),
        ...
        "gate2": {
            ...
            "company_release_source_profile": company_release_gate,
            "passed": (
                gate2_open["passed"]
        ...
        "next_evidence_needed": (
            "If rejected, stop splitting Space customer-source type scalars on these "
            "frozen snapshots. Future Space work should test forward replacement value "
            "by catalyst family/source quality or add higher-coverage official "
            "catalyst attribution without expanding into noisy tickers."
        ),
        "related_files": [
            "quant/experiments/exp_20260512_110_space_company_release_source_risk.py",
            "data/experiments/exp-20260512-110/space_company_release_source_risk.json",
            "docs/experiments/logs/exp-20260512-110.json",
            "docs/experiments/tickets/exp-20260512-110.json",
            "docs/experiments/artifacts/exp-20260512-110_space_company_release_source_risk.md",
        ...
        "why_not_other_changes": (
            "LLM soft-ranking is data-limited; broad defense-budget and official/"
            "regulatory source scalars failed; GSAT and mature satcom candidate "
            "expansions are rejected; this tests the remaining production event-source "
            "quality split without changing the candidate pool."
        ),
    }
    return payload

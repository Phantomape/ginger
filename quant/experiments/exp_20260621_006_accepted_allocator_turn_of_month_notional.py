"""exp-20260621-006: turn-of-month source notional scalar.
...
laggard/revision/peer-shock source scalars fixed, and add only a small
default-off paper notional scalar to selected turn_of_month rows.
...
EXPERIMENT_ID = "exp-20260621-006"
OWNER = "alpha-search-automation"
STEM = "accepted_allocator_turn_of_month_notional"
TRIAL_FAMILY = "accepted_default_off_helper_source_priority_allocation"
TRIAL_VARIANT_ID = "accepted_allocator_turn_of_month_source_notional_scalar_v1"
CHANGED_VARIABLE = TRIAL_VARIANT_ID
TARGET_SOURCE = "turn_of_month"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260621_006_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "turn_of_month_is_calendar_momentum_relabel",
    ...
    "confidence_reason": (
        ...
        "Corrected exp-20260620-033 shows turn_of_month has positive standalone "
        "PnL but higher overlap than laggard/revision/peer, so success odds are "
        "modest and the main risk is that calendar-flow rows are redundant "
        "after current source arbitration."
    ),
    "recorded_at": "2026-06-21T05:08:41+00:00",
}

PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "allocation: selected turn_of_month rows in the accepted helper allocator "
        "may deserve a small default-off paper notional scalar because the "
        "corrected accepted-sleeve independence map shows positive standalone "
        "turn-of-month returns and the current allocator still selects "
        "turn_of_month rows across all three canonical windows after laggard, "
        "revision, and peer-shock source scalars."
    ),
    "2_history_check": {
        "novelty_gate": (
            "experiment.py new warned on allocator/revision near-neighbors. The "
            "override is the corrected accepted-sleeve independence map plus "
            "current after-arbitration turn_of_month selected rows across all "
            "three canonical windows; this is not a calendar threshold, hold, "
            "cooldown, source-rank, or standalone turn-of-month notional retry."
        ),
        "exp-20260609-027": (
            "Accepted turn-of-month liquid leadership as a shared default-off "
            "paper adapter. This run keeps the source definition fixed."
        ),
        "exp-20260620-032": (
        ...
        "exp-20260620-033": (
            "Corrected independence map: turn_of_month had 45 harness trades, "
            "mean net return 2.179%, total PnL $3,921.56, mean abs correlation "
            "0.094, and mean ticker-date overlap 0.0161."
        ),
        "exp-20260621-001": (
    ...
    "3_single_causal_variable": (
        ...
        "turn_of_month allocator rows, while accepted laggard/revision/"
        "peer-shock scalars, source selection, rank, daily top-1, hold, costs, "
    ...
    "4_acceptance_standard": (
        ...
        "accepted allocator including exp-032 and exp-001 source scalars; after "
        "adds only turn_of_month 1.25x. Accept only if aggregate EV/PnL improve, "
        "no window EV/PnL regression, affected turn_of_month sample spans all "
    ...
    "5_reproducibility": (
        ...
        "exp_20260621_006_accepted_allocator_turn_of_month_notional.py"
    ),
}
...
def _binding_gate4(
    *,
    ...
    return {
        "passed": accepted,
        "decision": (
            "accepted_allocator_turn_of_month_source_notional_scalar"
            if accepted
            else "rejected_allocator_turn_of_month_source_notional_scalar"
        ),
...
def build_payload() -> dict[str, Any]:
    ...
    try:
        current_payload = template._run_allocator_pass(
            "before_current_exp032_exp001", dict(CURRENT_SCALARS)
        )
        after_payload = template._run_allocator_pass(
            "after_turn_of_month_scaled", dict(AFTER_SCALARS)
        )
    finally:
    ...
    return {
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                ...
                "and exp-001 peer_shock source_notional_scalars. After: same "
                "source selection with only turn_of_month added at 1.25x paper "
        ...
        "interpretation": (
            "Turn-of-month source notional improved the current accepted allocator "
            "and should be retained only as shared default-off paper observation."
            if accepted
            else "Turn-of-month source notional failed Gate 4 and is not retained."
        ),
        "rejection_reason": None if accepted else "; ".join(gate4["failed_reasons"]),
        "post_run_reflection": {
            "why_result_happened": (
                "The corrected turn_of_month independence signal translated into "
                "incremental after-arbitration paper capital without changing "
                "selection."
                if accepted
                else (
                    "The current allocator selected turn_of_month rows, but the "
                    "incremental scaled capital did not clear the window, risk, "
                    "sample, concentration, or accepted-comparator gates."
                )
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retry by sweeping turn_of_month scalar, source rank, "
                "allocator top-N, hold days, cooldown, calendar day labels, "
                "ret20/ret60 thresholds, close-location, volume, or volatility "
                "guards on the frozen windows."
            ),
            "new_evidence_required": (
                "Closed forward allocator replacement-value rows tagged by "
                "turn_of_month source family, or a materially new out-of-sample "
                "source independence surface."
            ),
        },
        "next_retry_requires": [
            "closed forward turn_of_month replacement-value rows",
            "out-of-sample source independence map",
            "no frozen-window turn_of_month source scalar sweep",
        ],
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Accepted Allocator Turn-Of-Month Notional",
            "",
            ...
            *_window_table(payload),
            ...
            "- Turn-of-month affected trades: `{}` across `{}` windows".format(

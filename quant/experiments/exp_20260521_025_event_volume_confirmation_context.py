...
event rows whose state-surface context has 20-day volume confirmation deserve
modest extra paper notional.
...
EXPERIMENT_ID = "exp-20260521-025"
EXPERIMENT_SLUG = "event_volume_confirmation_context"
    ...
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "volume_confirmation_scalar": 1.0,
            },
        ),
        (
            "volume_confirmation_105",
            {
                "description": "1.05x paper notional for event rows with volume_ratio_20 >= 1.10.",
                "volume_confirmation_scalar": 1.05,
            },
        ),
        (
            "volume_confirmation_110",
            {
                "description": "1.10x paper notional for event rows with volume_ratio_20 >= 1.10.",
                "volume_confirmation_scalar": 1.10,
            },
        ),
        (
            "volume_confirmation_115",
            {
                "description": "1.15x paper notional for event rows with volume_ratio_20 >= 1.10.",
                "volume_confirmation_scalar": 1.15,
            },
        ),
        (
            "volume_confirmation_120",
            {
                "description": "1.20x paper notional for event rows with volume_ratio_20 >= 1.10.",
                "volume_confirmation_scalar": 1.20,
            },
        ),
    ]
)
...
def _is_target_volume_confirmation(trade: dict[str, Any]) -> bool:
    value = _state_feature_float(trade, TARGET_FIELD)
    return value is not None and value >= TARGET_MIN_VOLUME_RATIO
...
def _scaled_trade(
    ...
    target = _is_target_volume_confirmation(trade)
    volume_scalar = (
        float(variant["volume_confirmation_scalar"]) if target else 1.0
    )
    scalar = accepted_scalar * volume_scalar
    ...
    return {
        **trade,
        ...
        "volume_confirmation_target": target,
        "volume_confirmation_scalar": round(volume_scalar, 4),
        "volume_ratio_20": _state_feature_float(trade, TARGET_FIELD),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    rows_with_field = [row for row in all_rows if row.get("volume_ratio_20") is not None]
    targets = [row for row in rows_with_field if row.get("volume_confirmation_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("volume_confirmation_target")
        ]
    ...
    return {
        ...
        "target_rule": f"{TARGET_FIELD} >= {TARGET_MIN_VOLUME_RATIO}",
...
def _gate_vs_baseline(
    ...
    sample_ok = (
        (selection.get("target_trade_count") or 0) >= MIN_TARGET_TRADES
        ...
        and (selection.get("rows_with_field_count") or 0) >= MIN_ROWS_WITH_FIELD
        and (
    ...
    return {
        **gate,
        ...
        "sample_guard": {
            ...
            "min_rows_with_field": MIN_ROWS_WITH_FIELD,
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -abs(VARIANTS[name]["volume_confirmation_scalar"] - 1.0),
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_default_off_event_volume_confirmation_context"
        if accepted
        else "rejected_event_volume_confirmation_context"
    )
    ...
    compact_parameters = {
        ...
        "target_field": TARGET_FIELD,
        "target_rule": f"{TARGET_FIELD} >= {TARGET_MIN_VOLUME_RATIO}",
        "selected_volume_confirmation_scalar": VARIANTS[best_variant][
            "volume_confirmation_scalar"
        ],
        "variant_scalars": {
            name: row["volume_confirmation_scalar"]
    ...
    return {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "event_volume_confirmation_allocation_adapter",
        "mechanism_family": "external_event_satellite_overlay_allocation",
        "trial_family": "event_volume_confirmation_context",
        "trial_variant_id": "volume_ratio_20_ge_1p10_notional_scalar",
        "changed_variable": "event_volume_confirmation_scalar",
        "prior_trial_count": 3,
        ...
        "hypothesis": (
            ...
            "state_features.volume_ratio_20 >= 1.10 should have better "
            "replacement value because the event is confirmed by same-day "
            "attention/participation rather than only price momentum. A single "
            "paper-notional scalar tests that production-visible context field."
        ),
        "alpha_hypothesis": {
            ...
            "playbook_alignment": (
                ...
                "decomposition and one production-visible field. It avoids LLM "
                "soft-ranking, SEC sparse text slices, broad-market threshold "
                "retunes, core slot retunes, and state-surface sleeve mining."
            ),
        },
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Event rows with production-visible volume confirmation may "
                "have better replacement value; this is event context scoring "
                "plus capital allocation."
            ),
            "2_history_check": (
                "Volume confirmation exists as accepted support in the separate "
                "state-surface sleeve, while event benchmark momentum just failed "
                "on drawdown. This run tests an untried event-row volume context "
                "field, not a ret20 retry."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for fixed volume-confirmed event "
                "rows changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260521_025_event_volume_confirmation_context.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260519-021": "Accepted state-surface rank-2 volume confirmation; different sleeve.",
            "exp-20260519-033": "Accepted state-surface rank-depth score-volume support; different sleeve.",
            "exp-20260521-013": "Accepted event non-narrow state-bucket context; current event baseline.",
            "exp-20260521-024": "Rejected event benchmark-momentum context because drawdown guard failed.",
        },
        "backtest_protocol": {
            "source": (
        ...
        "gate2": {
            "required_fields": [
                ...
                "exit_date",
                "pnl",
                "state_features.volume_ratio_20",
                "reaction_bucket",
                "state_bucket",
            ],
            ...
            "passed": bool(
                ...
                and selection_by_variant[BASELINE_VARIANT]["rows_with_field_count"]
                >= MIN_ROWS_WITH_FIELD
            ),
        },
        "gate3": {
        ...
        "decision_rationale": (
            ...
            if accepted
            else "Rejected: volume-confirmation context scalar did not clear Gate 4."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "If accepted, promote only through shared event_sleeve_bundle metadata "
            "and parity tests; do not retry nearby volume thresholds or broader "
            "state-feature scalars without forward rows."
        ),
        "why_not_other_attractive_points": (
            "Skipped LLM soft-ranking due attribution limits; skipped SEC no-"
            "guidance/special-call/mild-reaction/attention-persistence nearby "
            "fields due recent rejection or sample limits; skipped event ret20 "
            "retry after exp-20260521-024 failed the drawdown guard; skipped "
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Volume Confirmation Context",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests whether selected event rows with "
            "`state_features.volume_ratio_20 >= 1.10` deserve a paper-notional "
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event volume confirmation context",

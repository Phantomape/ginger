"""exp-20260512-008: Space near-perfect TQS trend risk allocation.

Tests whether official Space trend signals with high but not capped
trade-quality-score deserve a separate risk top-up after the accepted
perfect-TQS risk rule. This keeps ticker pool, targets, stops, ranking,
add-ons, LLM/news, and live Space slots fixed.
"""
...
EXPERIMENT_ID = "exp-20260512-008"
STEM = "space_near_perfect_tqs_trend_risk"
ACCEPTED_SPACE_BASKET_POSITIVE_SCALAR = 1.10
ACCEPTED_SPACE_PERFECT_TQS_RISK_SCALAR = 1.50
NEAR_PERFECT_TQS_FLOOR = 0.95
NEAR_PERFECT_TQS_CEILING = 1.0
NEAR_PERFECT_TQS_TREND_SCALARS = (1.10, 1.25, 1.50)


def _is_perfect_tqs(signal: dict[str, Any]) -> bool:
    value = _round(signal.get("trade_quality_score"), 6)
    return value is not None and value >= NEAR_PERFECT_TQS_CEILING
...
def _is_near_perfect_tqs_trend(signal: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and NEAR_PERFECT_TQS_FLOOR <= value < NEAR_PERFECT_TQS_CEILING
    )
...
def _install_space_policy(near_perfect_scalar: float):
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in OFFICIAL_SPACE_TICKERS and sizing:
                ...
                if _is_near_perfect_tqs_trend(signal) and near_perfect_scalar != 1.0:
...
def _run_variant(name: str, near_perfect_scalar: float) -> dict[str, Any]:
    ...
    (
        ...
        basket_counts,
        day_counts,
    ) = _install_space_policy(near_perfect_scalar)
    try:
        by_window = {}
    ...
    return {
        "variant": name,
        "space_near_perfect_tqs_trend_scalar": near_perfect_scalar,
        "by_window": by_window,
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_near_perfect_tqs_trend_adjustment"]["adjusted_signal_count"]
    ...
    return {
        ...
        "space_near_perfect_tqs_trend_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space near-perfect TQS trend risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: extra risk scalar for official Space trend_long "
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | {variant['space_near_perfect_tqs_trend_scalar']:.2f} | "
            ...
            f"{gate['space_near_perfect_tqs_trend_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
...
def run() -> dict[str, Any]:
    ...
    variants = {
        "accepted_exp004_stack": _run_variant("accepted_exp004_stack", 1.0)
    }
    for scalar in NEAR_PERFECT_TQS_TREND_SCALARS:
        name = f"near_perfect_tqs_trend_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_near_perfect_tqs_trend_risk"
        if accepted
        else "rejected_space_near_perfect_tqs_trend_risk"
    )
    interpretation = (
        "The near-perfect Space trend TQS bucket improved the accepted default-off "
        ...
        if accepted
            ...
            "The near-perfect Space trend TQS bucket did not beat the accepted "
            "exp-20260512-004 stack under the three-window gate. Keep accepted "
            "perfect-TQS risk only and require forward catalyst evidence before "
            "another adjacent TQS bucket retry."
        )
    )
    ...
    payload = {
        ...
        "single_causal_variable": (
            "extra risk scalar for official Space trend_long signals with "
            "0.95 <= trade_quality_score < 1.0"
        ),
        "hypothesis": (
            "After exp-20260512-004 showed capped Space TQS supports more risk, "
            "near-perfect but uncapped Space trend signals may be the adjacent "
            "quality bucket with enough lifecycle strength to justify a smaller "
            "bounded risk top-up."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: top up only official Space trend_long candidates "
                "with 0.95 <= existing trade_quality_score < 1.0."
            ),
            "2_history_check": {
                "exp-20260512-004": (
                    "Accepted perfect/capped TQS 1.5x risk top-up; this tests the "
                    "adjacent uncapped trend-only quality bucket, not the same bucket."
                ),
                "exp-20260512-005": (
                    "Rejected perfect-TQS target widening; this keeps targets fixed "
                    "and changes only risk allocation."
                ),
                "exp-20260511-109": "Rejected PL/BKSY data-vendor trend risk as a ticker bucket.",
                "exp-20260511-115": "Accepted official Space basket-positive risk scalar.",
            },
            "3_single_causal_variable": (
                "space_near_perfect_tqs_trend_risk_scalar; candidate pool, accepted "
            ...
            "5_reproducibility": (
                ...
                "and each near-perfect TQS trend scalar across the canonical augmented snapshots."
            ),
        },
        "parameters": {
            "official_space_tickers": list(OFFICIAL_SPACE_TICKERS),
            "accepted_before_experiment": "exp-20260512-004",
            "near_perfect_tqs_floor": NEAR_PERFECT_TQS_FLOOR,
            "near_perfect_tqs_ceiling": NEAR_PERFECT_TQS_CEILING,
            "tested_near_perfect_tqs_trend_scalars": list(
                NEAR_PERFECT_TQS_TREND_SCALARS
            ),
        ...
        "next_evidence_needed": (
            "If rejected, do not retry nearby near-perfect Space TQS risk buckets "
        ...
        "related_files": [
            "quant/experiments/exp_20260512_008_space_near_perfect_tqs_trend_risk.py",
            "data/experiments/exp-20260512-008/space_near_perfect_tqs_trend_risk.json",
            "docs/experiments/logs/exp-20260512-008.json",
            "docs/experiments/tickets/exp-20260512-008.json",
            "docs/experiments/artifacts/exp-20260512-008_space_near_perfect_tqs_trend_risk.md",
        ...
        "why_not_other_changes": (
            ...
            "and lunar/manufacturing target broadening already failed."
        ),
    }
    return payload

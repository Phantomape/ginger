"""exp-20260706-023: daily-equity replay for the top portfolio-lane candidate.

This consumes the observed-only ranking from exp-20260706-022. It does not
retune the fixed-asset-turnover source from exp-20260617-006 and does not
change live, paper, ranking, sizing, entry, or exit behavior. The only tested
decision hypothesis is whether the top-ranked rejected source still looks
...
EXPERIMENT_ID = "exp-20260706-023"
OWNER = "alpha-explore"
LANE = "alpha_search"
STEM = "portfolio_daily_equity_overlay"
STATUS_POSITIVE = "observed_only_positive_daily_equity_lead_not_activation_ready"
STATUS_REJECTED = "observed_only_rejected_daily_equity_overlay"
DECISION_POSITIVE = "observed_only_positive_fixed_asset_turnover_daily_equity_overlay"
DECISION_REJECTED = "observed_only_rejected_fixed_asset_turnover_daily_equity_overlay"

TRIAL_FAMILY = "portfolio_covariance_daily_equity_overlay"
TRIAL_VARIANT_ID = "fixed_asset_turnover_10pct_daily_equity_overlay_v1"
CHANGED_VARIABLE = "fixed_asset_turnover_top_ranked_portfolio_daily_equity_overlay_v1"
MECHANISM_FAMILY = "portfolio_covariance_lane"
CHANGE_TYPE = "risk_allocation"
NEW_EVIDENCE_TYPE = "new_gate_shape_daily_mark_to_market_portfolio_overlay"
NEW_EVIDENCE_AXIS = (
    "New gate shape: consume exp-20260706-022 portfolio-covariance ranking in "
    "a true daily mark-to-market equity replay; no fixed-asset-turnover "
    "threshold/top-N/hold/notional retune and no candidate source behavior "
    "change."
)

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260706_023_{STEM}.json"
...
TARGET_SOURCE_ARTIFACT = (
    "data/experiments/exp-20260617-006/"
    "exp_20260617_006_fixed_asset_turnover_improvement.json"
)
...
PREDICTION = {
    ...
    "confidence_reason": (
        "exp-20260706-022 found this family has positive rejected-source PnL "
        "and low exit-cashflow correlation to core, but the prior scan admitted "
        "that exit-date cashflow can misstate drawdown and correlation; this "
        "experiment consumes that ranking with a new daily-equity replay gate "
        "shape without retuning the source."
    ),
    "recorded_at": "2026-07-06T21:03:35+00:00",
}

HYPOTHESIS = (
    "Portfolio lane: the exp-20260706-022 top-ranked fixed-asset-turnover "
    "rejected source may add value as a fixed 10 percent paper overlay when "
    "replayed with true daily mark-to-market equity instead of exit-date "
    "cashflow proxies."
)
ALPHA_HYPOTHESIS = (
...
RUNNER = f"quant/experiments/exp_20260706_023_{STEM}.py"
RUNNER_COMMAND = f".\\.venv\\Scripts\\python.exe -B {RUNNER}"
RUNNER_WINDOWS = f"quant\\experiments\\exp_20260706_023_{STEM}.py"

CHANGED_FILES = [
    RUNNER,
    f"data/experiments/{EXPERIMENT_ID}/exp_20260706_023_{STEM}.json",
...
def build_result() -> dict[str, Any]:
    ...
    payload = {
        ...
        "nearby_prior_experiments": ["exp-20260706-022", "exp-20260617-006"],
        ...
        "post_run_reflection": {
            ...
            "forbidden_near_neighbor_retry": (
                "Do not rerun fixed-asset-turnover portfolio overlays by "
                "changing overlay weight, thresholds, hold days, top-N, "
                "correlation cutoffs, or source filters. A legal retry needs "
                "materially new families in the portfolio ranking or a shared "
                "paper helper/activation-envelope experiment."
            ),
            "new_evidence_required": (
                "A shared default-off helper or activation-envelope Gate 1-4 "
                "that implements a fixed portfolio lane, or materially new "
                "ranked candidate families with replayable daily-equity paths."
            ),
        },
        "rejection_reason": (
            None

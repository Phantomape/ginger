"""exp-20260524-003: core relative-strength component band top-up scout.

Alpha search. Tests whether the entry-day cross-sectional ranking surface's
relative_strength component has a moderate-strength sweet spot for already
qualified core stock signals. The only causal variable is a cap-aware
post-sizing multiplier for signals with 0.50 <= relative_strength <= 0.65.
...
EXPERIMENT_ID = "exp-20260524-003"
STEM = "core_relative_strength_component_band_topup"
MULTIPLIER_KEY = "relative_strength_component_band_topup_applied"

RELATIVE_STRENGTH_COMPONENT_MIN = 0.50
RELATIVE_STRENGTH_COMPONENT_MAX = 0.65
...
VARIANTS = OrderedDict(
    [
        ("rs_component_band_topup_1025", {"multiplier": 1.025}),
        ("rs_component_band_topup_1050", {"multiplier": 1.05}),
        ("rs_component_band_topup_1075", {"multiplier": 1.075}),
        ("rs_component_band_topup_1100", {"multiplier": 1.10}),
    ]
)
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]):
    def wrapped(signals, features_dict, atr_target_mult=None):
        ...
        for sig in enriched:
            ...
            relative_strength = base._safe_float(components.get("relative_strength"))
            ...
            sig["relative_strength_component"] = relative_strength
            sig["relative_strength_component_band_state"] = (
                sig.get("strategy") in base.TARGET_STRATEGIES
                and sector not in base.EXCLUDED_SECTORS
                and relative_strength is not None
                and RELATIVE_STRENGTH_COMPONENT_MIN
                <= relative_strength
                <= RELATIVE_STRENGTH_COMPONENT_MAX
            )
        return enriched
...
def _apply_topup_to_sizing(
    ...
    if sig.get("relative_strength_component_band_state") is not True:
        return sig, None
    ...
    sizing["relative_strength_component_band_state"] = True
    sizing["relative_strength_component"] = sig.get("relative_strength_component")
    sizing["relative_strength_component_band_min"] = RELATIVE_STRENGTH_COMPONENT_MIN
    sizing["relative_strength_component_band_max"] = RELATIVE_STRENGTH_COMPONENT_MAX
    sizing["relative_strength_component_band_baseline_shares"] = old_shares
    sizing["relative_strength_component_band_desired_shares"] = desired_shares
    sizing["relative_strength_component_band_cap_shares"] = cap_shares
    sizing["relative_strength_component_band_new_shares"] = new_shares
    ...
    record = {
        ...
        "relative_strength_component": sig.get("relative_strength_component"),
...
def _rejection_reason(gate: dict[str, Any]) -> str:
    if gate["adjusted_signal_count"] < gate["guardrails"]["min_adjusted_signal_count"]:
        return (
            "Best variant failed adjusted-signal sample guard; the "
            "relative-strength component band did not touch enough sized core "
            "signals."
        )
...
def _retag_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                ...
                "trades, but its relative_strength component may still "
                "distinguish a moderate-strength sweet spot. Already-qualified "
                "non-ETF/non-Commodity core signals with relative_strength "
                "component between 0.50 and 0.65 may deserve a small cap-aware "
                "top-up without changing entries, exits, ranking, universe, "
                "news, or LLM logic."
            ),
            "change_summary": (
                "Compute the experiment-only entry-day ranking surface and "
                "sweep a cap-aware top-up for the relative_strength component "
                "band 0.50 through 0.65."
            ),
    ...
    payload["parameters"] = {
        "relative_strength_component_min": RELATIVE_STRENGTH_COMPONENT_MIN,
        "relative_strength_component_max": RELATIVE_STRENGTH_COMPONENT_MAX,
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "capital allocation: a moderate relative-strength component band "
            "may be safer than either weak or overheated cross-sectional "
            "leadership among already-qualified core entries."
        ),
        "2_history_check": (
            "Accepted RS20/RS60 top-ups use coarse return leadership. "
            "exp-20260523-012 showed full PIT ranking-component coverage; "
            "exp-20260523-015 tested the top-level alpha bucket and failed "
            "Gate 4. No prior log entry tested this component band."
        ),
        "3_single_causal_variable": (
            "post-sizing multiplier for 0.50 <= relative_strength_component <= 0.65"
        ),
        ...
        "5_reproducibility": (
            ...
            "exp_20260524_003_core_relative_strength_component_band_topup.py"
        ),
    }
    payload["gate2"]["rule_dependencies"] = [
        ...
        "alpha_score_components.relative_strength",
        "sector",
        "strategy",
    ...
    payload["production_impact"][
        ...
        "Move the relative-strength component field and risk policy into "
    ...
    payload["why_not_other_changes"] = (
        ...
        "Skipped state-surface, broad-market, event, SEC, ETF, candidate-pool, "
        "and canonical-vector near-neighbor scalar retunes due recent rejected "
        "or strict-gated lanes. This uses a production-computable ranking "
        "component rather than the saturated top-level alpha score."
    )
    payload["known_risks"] = [
    ...
    if payload["status"] == "rejected":
        payload["next_retry_requires"] = [
            "Do not retry adjacent relative-strength component band multipliers on the same frozen windows without new forward rows or a materially different ranking field.",
    ...
    payload["related_files"] = [
        "quant/experiments/exp_20260524_003_core_relative_strength_component_band_topup.py",

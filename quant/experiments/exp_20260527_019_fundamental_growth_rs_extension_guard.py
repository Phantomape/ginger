"""exp-20260527-019: fundamental growth + RS extension-guard scout.

Alpha search. This retests the exp-20260527-017 free-data candidate source
with one pre-entry drawdown-control field: selected default-off paper candidates
must not be more than 15% above their own 50-day moving average on the signal
date. Core entries, ranking, sizing, exits, LLM/news paths, watchlists, and
...
EXPERIMENT_ID = "exp-20260527-019"
STEM = "fundamental_growth_rs_extension_guard"
TRIAL_FAMILY = "fundamental_growth_rs_default_off_candidate_pool"
CHANGED_VARIABLE = "fundamental_growth_rs_pct_above_50d_extension_guard_v1"
...
EXTENSION_AUDIT: "OrderedDict[str, dict[str, Any]]" = OrderedDict()
...
def _candidate_rows_for_window(
    ...
    for row in rows:
        pct_above = prev._float(row.get("pct_above_50d_ma"))
        if pct_above is None or pct_above > MAX_PCT_ABOVE_50D_MA:
            filtered.append(
                {
                    **row,
                    "filter_reason": "pct_above_50d_extension_guard",
        ...
        retained.append(
            {
                **row,
                ...
                "extension_guard_max_pct_above_50d_ma": MAX_PCT_ABOVE_50D_MA,
            }
        )

    label = _window_label(cfg)
    prev.FUNDAMENTAL_RS_AUDIT[label]["post_extension_guard_candidates"] = len(retained)
    prev.FUNDAMENTAL_RS_AUDIT[label]["post_extension_guard_days"] = len(
        {row["date"] for row in retained}
    )
    prev.FUNDAMENTAL_RS_AUDIT[label]["post_extension_guard_tickers"] = len(
        {row["ticker"] for row in retained}
    )
    EXTENSION_AUDIT[label] = {
        "rule_version": RULE_VERSION,
        "max_pct_above_50d_ma": MAX_PCT_ABOVE_50D_MA,
...
def _reference_comparison(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_window_delta_after_vs_unguarded": by_window,
        "aggregate_delta_after_vs_unguarded": {
...
def _update_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = (
        "promising_replay_only_fundamental_growth_rs_extension_guard"
        if gate4_passed
        else "rejected_fundamental_growth_rs_extension_guard"
    )
    ...
    payload["hypothesis"] = (
        ...
        "ticker concentration. A production-visible pre-entry extension guard "
        "may remove chase-risk candidates while preserving the source edge, "
        "without retuning growth thresholds, RS percentiles, cooldown length, "
        "paper notional, hold period, or any live strategy path."
    )
    payload["change_type"] = "fundamental_growth_rs_extension_guard_default_off_paper_sleeve"
    ...
    payload["new_evidence_type"] = "production_visible_pre_entry_drawdown_control_field"
    payload["parameters"]["extension_guard"] = {
        "max_pct_above_50d_ma": MAX_PCT_ABOVE_50D_MA,
        "applies_to": "exp-20260527-017 default-off paper candidate rows before daily top-1 selection",
        "uses_future_data": False,
        "production_visible_field": "signal-date close relative to trailing 50-day moving average",
    }
    payload["parameters"]["selection_rank"] = [
        "signal_date",
        "skip if pct_above_50d_ma > 0.15",
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "candidate_pool / entry alpha with drawdown control: a 50-day extension "
            "guard can convert a high-EV but unsafe Companyfacts+RS candidate source "
            "into a safer default-off paper sleeve."
        ),
        "2_history_check": {
            "exp-20260527-017": (
            ...
            "exp-20260527-018": (
                "A 20-trading-day same-ticker cooldown reduced concentration but "
                "still failed drawdown (+3.96pp) and regressed late_strong."
            ),
            "playbook": (
                "Do not retry growth/RS percentile, top-N, fixed-notional, or "
                "cooldown thresholds on the frozen sample; only a materially new "
                "source-quality/drawdown-control field is eligible."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260527_019_fundamental_growth_rs_extension_guard.py"
        ),
    }
    payload["gate2"]["runtime_fields"] = [
        ...
        "signal-date trailing 50-day moving average from OHLCV rows <= signal_date",
    ]
    payload["gate2"]["note"] = (
        "SEC growth rows are filtered by filed <= signal_date. RS, trend, liquidity, "
        "returns, and pct_above_50d_ma use signal-date or trailing OHLCV only. "
        "Paper entry occurs at the next open; no LLM, news, hidden event field, or "
        "future bar is used."
    )
    payload["gate3"]["candidate_pool_changed"] = True
    payload["gate3"]["note"] = (
        "No new core filter or live entry rule was added. The extension guard only "
        "changes default-off paper sleeve selection, so core survival is unchanged."
    )
    payload["extension_guard_audit"] = EXTENSION_AUDIT
    payload["fundamental_growth_rs_audit"] = prev.FUNDAMENTAL_RS_AUDIT
    payload["reference_unguarded_exp17_comparison"] = _reference_comparison(payload)
    payload["why_not_other_changes"] = (
        ...
        "VBB, state-surface, broad-market, and nearby Companyfacts+RS threshold, "
        "top-N, fixed-notional, and cooldown retunes per playbook freeze guidance. "
        "This run tests a distinct production-visible drawdown-control field."
    )
    payload["interpretation"] = (
        "The extension-guard Companyfacts+RS sleeve cleared Gate 4 as a replay-only "
        ...
        if gate4_passed
            ...
            "The extension-guard Companyfacts+RS sleeve did not clear Gate 4. Do not "
            "promote it or retry nearby growth/RS/extension/cooldown thresholds on "
            "the frozen windows without forward rows or a materially new source-quality field."
        )
    )
    payload["next_evidence_needed"] = (
        ...
        "nearby extension, cooldown, growth, or RS threshold retune."
    )
...
def _build_report(payload: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        (
            "Single variable: add a pre-entry guard to the exp-20260527-017 "
            "Companyfacts-growth + OHLCV-RS default-off paper candidate source: "
            "skip candidates whose signal-date close is more than 15% above their "
            "trailing 50-day moving average."
        ),
        "",
        "## Three-Window Result",
        "",
    ...
    for label in prev.base.WINDOWS:
        ...
        audit = payload["extension_guard_audit"].get(label, {})
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            "```json",
            json.dumps(payload["extension_guard_audit"], indent=2, sort_keys=True),
            "```",
            "",
            "## Gate 4",
            "",
...
def _persist(payload: dict[str, Any]) -> None:
    ticket = {
        ...
        "title": "Fundamental growth + RS extension guard scout",

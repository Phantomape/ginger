...
sector-known common-stock-like tickers during monthly options-expiration week,
requiring the same fixed liquid-leadership gates used by the accepted
turn-of-month lead before a top-1 next-open default-off paper entry with a
...
EXPERIMENT_ID = "exp-20260610-001"
STEM = "opex_week_liquid_leadership"
TRIAL_FAMILY = "opex_week_liquid_leadership_candidate_pool"
TRIAL_VARIANT_ID = "opex_week_liquid_leadership_top1_next_open_10d_v1"
CHANGED_VARIABLE = "opex_week_liquid_leadership_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260610_001_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "generic_momentum_not_opex_edge",
        "window_regression",
    ...
    "confidence_reason": (
        "Free PIT calendar plus OHLCV is production-visible and recent "
        "turn-of-month evidence suggests calendar-flow can work, but scheduled "
        "macro/calendar neighbors are mixed and this must not retune the "
        "accepted turn-of-month route."
    ),
    "recorded_at": "2026-06-10T00:06:39+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "monthly options-expiration week labels, sector-known liquid stock "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "entry/candidate_pool: monthly options-expiration-week dealer roll and "
        "rebalance flow may make liquid sector-known stocks that already show "
        "SPY-relative leadership and high close quality continue after "
        "next-open entry. It uses only free calendar and OHLCV data."
    ),
    "2_history_check": {
        "no_exact_prior_found": (
            "Search found macro scheduled-event, turn-of-month, and price-"
            "formation candidate-pool experiments, but no exact OPEX-week "
            "liquid leadership source."
        ),
        "nearby_calendar_trials": (
            "exp-20260605-030 and exp-20260605-032 tested official macro "
            "release-day beta actions and failed strict gates. exp-20260606-020 "
            "accepted macro relief stock leadership. exp-20260609-027 accepted "
            "turn-of-month liquid leadership, but it is a month-end/month-start "
            "flow route, not OPEX week."
        ),
        ...
        "frozen_lanes_avoided": (
            ...
            "or turn-of-month threshold sweep is involved."
        ),
    },
    "3_single_causal_variable": (
        "One fixed policy bundle: the five trading days ending on the monthly "
        "options-expiration trading date, liquid sector-known stock universe, "
        "the accepted turn-of-month leadership gates held fixed, same-ticker "
        "core-overlap exclusion, top-1 next-open paper entry, 10-day hold, "
    ...
    "5_reproducibility": (
        ...
        "exp_20260610_001_opex_week_liquid_leadership.py"
    ),
}
...
def _opex_week_labels(dates: list[str]) -> dict[str, str]:
    ...
    for month_key, month_dates in by_month.items():
        ...
        expiry = _third_friday(year, month).isoformat()
        expiry_trade_date = None
        for date_value in reversed(ordered):
            if date_value <= expiry:
                expiry_trade_date = date_value
                break
        if expiry_trade_date is None:
            continue
        expiry_index = ordered.index(expiry_trade_date)
        start_index = max(0, expiry_index - 4)
        for offset, date_value in enumerate(ordered[start_index : expiry_index + 1]):
            distance = expiry_index - (start_index + offset)
            labels[date_value] = "opex_day" if distance == 0 else f"opex_minus_{distance}"
    return labels
...
def _candidate_rows_for_window(
    *,
    ...
    opex_labels = _opex_week_labels(all_dates)
    dates = [
        date_value
    ...
    opex_label_distribution: dict[str, int] = {}
    scan = {
        "scanned_trading_days": len(dates),
        "opex_week_days": 0,
        "days_with_raw_opex_week_candidates": 0,
        "raw_opex_week_candidates": 0,
    }

    for signal_date in dates:
        opex_label = opex_labels.get(signal_date)
        if opex_label is None:
            continue
        scan["opex_week_days"] += 1
        opex_label_distribution[opex_label] = opex_label_distribution.get(opex_label, 0) + 1
        day_rows: list[dict[str, Any]] = []
        for ticker in sorted(sector_entries):
            row = previous._candidate_for_ticker(
                snapshot=snapshot,
                indices=indices,
                sector_entries=sector_entries,
                ticker=ticker,
                signal_date=signal_date,
                month_label=opex_label,
            )
            if row is None:
                continue
            row["source"] = "OPEX_WEEK_LIQUID_LEADERSHIP_PAPER"
            row["candidate_opex_week_label"] = row.pop("candidate_month_label", opex_label)
        ...
        scan["days_with_raw_opex_week_candidates"] += 1
        scan["raw_opex_week_candidates"] += len(day_rows)
        day_contexts.append(
            {
                "date": signal_date,
                "opex_label": opex_label,
    ...
    scan.update(
        {
            ...
            "opex_label_distribution": dict(sorted(opex_label_distribution.items())),
            "calendar_window": "five_trading_days_ending_monthly_options_expiration_trade_date",
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_opex_week_liquid_leadership"
        if gate["passed"]
        else "rejected_opex_week_liquid_leadership_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "new_evidence_type": "production_visible_free_calendar_opex_week_liquid_leadership_field",
            ...
            "negative_reflection": (
                "If rejected, the likely reason is that OPEX-week leadership is "
                "generic liquid momentum or noisy calendar flow rather than a "
                "distinct replacement-value candidate pool after next-open "
                "execution, costs, cooldown, and same-ticker core-overlap "
                "exclusion. Do not answer by sweeping OPEX week length, "
                "ret20/ret60 thresholds, close-location, volume bounds, top-N, "
                "hold-day, cooldown, or notional on these frozen windows."
            ),
            "next_evidence_needed": (
                "A retry needs materially new evidence that identifies true "
                "OPEX flow beneficiaries, such as point-in-time options open "
                "interest, dealer gamma proxies, index/ETF rebalance rows, or "
                "forward daily-snapshot replacement value. Pure threshold "
                "retunes stay frozen."
            ),
        }
    )
    payload["parameters"] = {
        **payload.get("parameters", {}),
        "calendar_window": "five_trading_days_ending_monthly_options_expiration_trade_date",
        "option_expiration_rule": "third_friday_or_prior_trading_day_if_holiday",
    ...
    payload["post_run_reflection"] = {
        "why_result_happened": (
            "The OPEX-week liquid leadership bundle cleared the canonical "
            "three-window gates and beat the accepted calendar/compression "
            "comparators, suggesting expiry-week flow contributed replacement "
            "value beyond generic liquid momentum. It remains only a replay "
            "lead because no shared daily adapter or production parity path was added."
            if passed
            else (
                "The OPEX-week liquid leadership bundle failed Gate 4. The "
                "result implies expiry-week timing did not add enough distinct "
                "edge beyond liquid momentum and the accepted turn-of-month "
                "calendar route after next-open execution, costs, cooldown, "
                "and overlap/concentration controls."
            )
        ),
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping OPEX week length, third-Friday handling, "
            "ret20/ret60 relative-strength thresholds, signal-day return, "
            "close-location, volume-ratio bounds, top-N, hold-day, cooldown, "
            "or paper notional on the same frozen windows."
        ),
        "new_evidence_required": payload["next_evidence_needed"],
    }
    payload["interpretation"] = (
        "The OPEX-week liquid leadership source passed as a replay-only "
        "promotion lead, but no production surface changed and a shared "
        "default-off parity adapter is required before use."
        if passed
        else (
            "The OPEX-week liquid leadership source was rejected; it did not "
            "establish a distinct free-calendar/OHLCV candidate-pool edge under "
            "the standard three-window protocol and accepted-comparator checks."
        )
    )
    return payload
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    for label in framework.WINDOWS:
        ...
        rows.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {dd:+.4f} | {opex_days} | {days} | {trades} |".format(
                label=label,
                ...
                opex_days=scan.get("opex_week_days", 0),
                days=scan.get("days_with_raw_opex_week_candidates", 0),
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} OPEX-Week Liquid Leadership Candidate Pool",
            "",
        ...
        "windows": [
            {
                ...
                "opex_week_day_count": payload["context_scan_by_window"][label].get(
                    "opex_week_days"
                ),
                "raw_candidate_count": payload["context_scan_by_window"][label].get(
                    "raw_opex_week_candidates"
                ),

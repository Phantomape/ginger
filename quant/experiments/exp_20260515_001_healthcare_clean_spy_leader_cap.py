"""exp-20260515-001: Healthcare clean-SPY leader cap scout.

Tests one production-visible allocation variable on the accepted core stack:
already-qualified Healthcare trend/breakout signals with active clean-SPY
leader sizing may be capped too tightly by the generic clean-SPY max-position
cap. This is a shadow scout; it does not change entries, exits, ranking,
universe, LLM/news, Space sleeves, heat, or slots.
"""
...
EXPERIMENT_ID = "exp-20260515-001"
EXPERIMENT_SLUG = "healthcare_clean_spy_leader_cap"
CAP_KEY = "healthcare_clean_spy_leader_max_position_pct_applied"
...
def _is_target_sleeve(sig: dict[str, Any], sizing: dict[str, Any]) -> bool:
    clean_spy_mult = sizing.get("clean_spy_leader_signal_day_risk_multiplier_applied")
    return bool(
        sig.get("sector") == "Healthcare"
...
def _resize_with_cap(
    ...
    out = dict(sizing)
    out["healthcare_clean_spy_leader_cap_baseline_shares"] = old_shares
    out["healthcare_clean_spy_leader_cap_raw_shares"] = raw_shares
    out["healthcare_clean_spy_leader_cap_old_cap_shares"] = old_cap_shares
    out["healthcare_clean_spy_leader_cap_new_cap_shares"] = new_cap_shares
    out["healthcare_clean_spy_leader_cap_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "raw_shares": adjusted_sizing.get(
                                "healthcare_clean_spy_leader_cap_raw_shares"
                            ),
                            "old_cap_shares": adjusted_sizing.get(
                                "healthcare_clean_spy_leader_cap_old_cap_shares"
                            ),
                            "new_cap_shares": adjusted_sizing.get(
                                "healthcare_clean_spy_leader_cap_new_cap_shares"
                            ),
                            "clean_spy_leader_multiplier": sizing.get(
                                "clean_spy_leader_signal_day_risk_multiplier_applied"
                            ),
                            "breakout_healthcare_dte_risk_multiplier": sizing.get(
                                "breakout_healthcare_dte_risk_multiplier_applied"
                            ),
                            "trend_healthcare_dte_risk_multiplier": sizing.get(
                                "trend_healthcare_dte_risk_multiplier_applied"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Healthcare Clean-SPY Leader Cap",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if selected["passed"]
        else "rejected_healthcare_clean_spy_leader_cap"
    )
    interpretation = (
        "Healthcare clean-SPY leaders were cap-bound and the selected cap improved the canonical three-window stack without EV regression."
        if selected["passed"]
        else "Healthcare clean-SPY leader cap expansion did not clear the canonical three-window gate."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Healthcare is already defensible in shallow-risk regimes, but broad Healthcare/DTE retries have been weak. The narrower production-visible clean-SPY leader subset may be a stronger allocation pocket that is capped by the generic clean-SPY max-position limit."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "max_position_pct_for_healthcare_clean_spy_leaders",
        "single_causal_variable": (
            "max_position_pct for Healthcare trend/breakout signals with active clean-SPY leader sizing"
        ),
        "parameters": {
            "cap_sweep": CAP_SWEEP,
            "baseline_generic_clean_spy_max_position_pct": 0.525,
            ...
            "locked_variables": [
                ...
                "raw Healthcare DTE multipliers",
        ...
        "historical_experiment_check": {
            "similar_prior_results": {
                "exp-20260501-023": (
                    "Rejected Healthcare relative-laggard risk cap; this run is not a Healthcare laggard or broad de-risking retry."
                ),
                "exp-20260505-016": (
                    "Rejected Financials/Healthcare breakout DTE zero-risk; this run keeps accepted Healthcare DTE multipliers unchanged."
                ),
                "exp-20260514-027": (
                    "Accepted generic clean-SPY leader cap expansion; this run tests whether Healthcare clean-SPY leaders still need incremental cap room."
                ),
                "exp-20260514-034": (
                    "Rejected requiring own green candle for clean-SPY leaders; this run does not add a filter."
                ),
                "exp-20260514-052": (
                    "Rejected Financials breakout leader cap; this run avoids nearby Financials breakout retuning."
                ),
                "exp-20260514-054": (
                    "Rejected Technology breakout persistent leader cap; this run tests a different sector state and includes trend/breakout Healthcare."
                ),
            },
            "why_not_llm_or_space": (
                "LLM soft-ranking and additional Space semantic refinements are sample/attribution constrained right now. This run uses deterministic fields already visible to production sizing."
            ),
        },
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital allocation: Healthcare clean-SPY leaders may deserve more cap room than the generic clean-SPY leader cap."
            ),
            "2_history_check": (
                "Broad Healthcare laggard/DTE retries and recent Tech/Financials cap scouts were rejected; no prior Healthcare clean-SPY leader cap scout was found."
            ),
            "3_single_causal_variable": "Healthcare clean-SPY leader max_position_pct only.",
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260515_001_healthcare_clean_spy_leader_cap.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "portfolio_engine breakout_healthcare_dte_risk_multiplier_applied",
                "portfolio_engine trend_healthcare_dte_risk_multiplier_applied",
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, add the Healthcare clean-SPY cap through shared portfolio_engine.size_signals, include the attribution key in backtester.py, and add focused production/backtest parity tests before live orders change."
            ),
        },
        ...
        "next_evidence_needed": (
            ...
            if selected["passed"]
            else "Do not retry nearby Healthcare clean-SPY cap values without forward cap-room evidence or a new production-visible Healthcare quality discriminator."
        ),
        "related_files": [

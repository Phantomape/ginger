...
Alpha search, replay-only. The entry-state oracle in exp-20260507-032 showed
that candidate rows tagged `pre_earnings_46_plus` had positive forward-return
evidence across all three canonical windows. This experiment asks the tradable
version: when the accepted system already enters such a candidate, is there
cap-aware risk-budget headroom worth using?

Only already-entered trades tagged `pre_earnings_46_plus` are resized. Entries,
ranking, exits, add-ons, universe, LLM/news, and event sleeves are locked.
"""
...
EXPERIMENT_ID = "exp-20260507-033"
STEM = "far_earnings_entry_state_risk"
SOURCE_EXPERIMENT_ID = "exp-20260507-032"

INITIAL_CAPITAL = 100_000.0
TREATMENT_TAG = "pre_earnings_46_plus"
...
VARIANTS = OrderedDict(
    [
        ("far_earnings_1_25x_cap_aware", {"risk_multiplier": 1.25}),
        ("far_earnings_1_50x_cap_aware", {"risk_multiplier": 1.50}),
    ]
)
...
def _load_earnings_for_events(events: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for event in events:
        date_str = _normalise_date(event.get("date") or event.get("signal_date"))
...
def _entry_state_rows(
    ...
    actual_trade_keys = {
        (trade.get("entry_date"), str(trade.get("ticker") or "").upper())
    ...
    *,
    ...
    for trade in trades:
        ...
        replacement = _resize_trade(
            ...
            reason="cap_aware_far_from_earnings_entry_state_risk_multiplier",
            cap_pct=cap_pct,
            cap_source=cap_source,
...
def _replay_window(name: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    entry_rows = _entry_state_rows(result, snapshot, candidate_events)
    ...
    entered_tag_counts: Counter[str] = Counter()
    for row in entry_rows:
        for tag in row.get("tags") or []:
            tag_counts[tag] += 1
            if row.get("decision") == "entered":
                entered_tag_counts[tag] += 1
...
def _aggregate(by_window: dict[str, Any]) -> dict[str, Any]:
    ...
    for variant_name in VARIANTS:
        ...
        gate_passed = (
            ...
            and pnl_delta_sum > baseline_pnl_sum * 0.05
            and improved >= 2
            and regressed == 0
        ...
        out[variant_name] = {
            ...
            "total_pnl_delta_pct": _round(pnl_delta_sum / baseline_pnl_sum if baseline_pnl_sum else None, 6),
...
def _log_record(
    ...
    return {
        ...
        "mechanism_family": "entry_state_far_from_earnings_lifecycle_allocation",
        "single_causal_variable": "pre_earnings_46_plus_entry_state_risk_multiplier",
        "date_range": {
...
def _ticket(best_variant: str, decision: str, aggregate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "experiment_id": EXPERIMENT_ID,
        "title": "Far-from-earnings entry-state risk replay",
        "decision": decision,
        "best_variant": best_variant,
        "expected_value_score_delta_sum": best["expected_value_score_delta_sum"],
        "total_pnl_delta_sum": best["total_pnl_delta_sum"],
        "next_action": (
            "Promote only after shared risk policy plus run/backtester parity tests."
            if best["proxy_gate4_passed"]
            else "Do not promote; avoid nearby far-from-earnings risk scalars without new evidence."
        ),
    }
...
def _artifact_markdown(
    ...
    lines = [
        f"# {EXPERIMENT_ID} Far-From-Earnings Entry-State Risk Replay",
        "",
        f"Decision: `{decision}`",
...
def main() -> None:
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Accepted A/B trades whose signal-date entry state is at least 46 "
            "calendar days before next earnings have less near-event overhang "
            "and may deserve cap-aware risk add-on capital."
        ),
        ...
        "history_check": {
            "mechanism_insight_conflict": (
                "No direct conflict: this is not C-sleeve enablement, LLM ranking, "
                "event-source pruning, broad universe growth, runner exits, or "
                "nearby SPY-relative leader multiplier tuning."
            ),
            "nearby_rejected": {
                "exp-20260507-011": (
                    "Re-enabled earnings_event_long regressed all windows; this "
                    "experiment does not add C trades or use EPS surprise."
                ),
                "exp-20260505-016": (
                    "Breakout DTE zero-risk failed; this test rewards far-from-earnings "
                    "accepted trades rather than adding another near-event veto."
                ),
                "exp-20260507-027": (
                    "Core-platform cap-aware risk add-on was positive but immaterial "
                    "and single-ticker concentrated."
                ),
                "exp-20260507-032": (
                    "Entry-state oracle was accepted only as diagnostic; this run "
                    "converts one PIT tag into a candidate-level replay."
                ),
            },
            "why_not_simple_repeat": (
                "The treatment tag is an entry-state distance-from-earnings field "
                "computed before entry. It does not retune existing sector DTE "
                "haircuts, enable C, or alter source/event semantics."
            ),
        },
        "parameters": {
            "treatment_tag": TREATMENT_TAG,
        ...
        "next_retry_requires": [
            "Do not retry nearby far-from-earnings risk scalars if rejected.",
            "A valid retry needs forward evidence or a materially different event-quality discriminator.",
            "If promoted later, implement as shared risk policy consumed by run.py and backtester.py with parity tests.",
        ],
        "risk_of_change": (
            "A far-from-earnings add-on may overweight mature winners already cap-bound "
            "by existing SPY-relative leader rules, or underprice non-earnings event risk."
        ),
        "why_not_other_attractive_points": (
            "LLM soft-ranking remains outcome-join sparse, C/earnings enablement is "
            "recently rejected, event source/price/momentum tilts have fresh failures, "
            "and static universe baskets need forward replacement-value evidence."
        ),
        "related_files": [
    ...
    artifact = _artifact_markdown(payload, aggregate, best_variant, decision, rejection_reason)

"""exp-20260517-005: two-slot rank-1 cap-aware top-up scout.

Tests one allocation extension on the accepted core stack: when exactly two
entry slots remain, apply a small cap-aware top-up only to the already-selected
rank-1 signal. The accepted one-slot top-up remains unchanged in both baseline
and variants; this scout only asks whether the scarce-slot effect generalizes
one step wider.
"""
...
EXPERIMENT_ID = "exp-20260517-005"
EXPERIMENT_SLUG = "two_slot_rank1_topup"
MULTIPLIER_KEY = "two_slot_rank1_risk_multiplier_applied"
BASELINE_RISK_MULTIPLIER = 1.0
RISK_MULTIPLIER_SWEEP = [1.0, 1.025, 1.05, 1.075]
...
def _apply_two_slot_topup(signals: list[dict[str, Any]], available_slots: int):
    if available_slots != 2 or not signals or CURRENT_RISK_MULTIPLIER <= 1.0:
        return signals, []
    ...
    sizing["two_slot_rank1_state"] = True
    sizing["two_slot_rank1_available_slots"] = available_slots
    sizing["two_slot_rank1_baseline_shares"] = old_shares
    sizing["two_slot_rank1_desired_shares"] = desired_shares
    sizing["two_slot_rank1_cap_shares"] = cap_shares
    sizing["two_slot_rank1_new_shares"] = new_shares
...
def _make_scarce_slot_wrapper(original):
    def wrapped(signals, available_slots, multiplier=None):
        if available_slots == 2:
            return _apply_two_slot_topup(signals, available_slots)
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Two-Slot Rank-1 Top-Up",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if selected["passed"]
        else "rejected_two_slot_rank1_topup"
    )
    interpretation = (
        "The two-slot rank-1 top-up cleared the canonical three-window scout and requires shared production_parity promotion plus rerun before production use."
        if selected["passed"]
        else "The accepted one-slot top-up should not be generalized to available_slots == 2 on the frozen three-window evidence."
    )
    ...
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted scarce-slot rank-1 top-up may reflect rank confidence under "
            "capital scarcity. If the effect extends to the still-constrained two-slot "
            "state, a small cap-aware top-up on the already-selected rank-1 signal can "
            "improve EV without changing entries or ranking."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "two_slot_rank1_risk_multiplier",
        "single_causal_variable": (
            "post-selection cap-aware top-up on candidate_rank=1 when available_slots == 2"
        ),
        "parameters": {
            "state_definition": {
                "available_slots": 2,
        ...
        "gate_questions": {
            ...
            "2_history_check": {
                "exp-20260517-004": (
                    ...
                    "this explicitly tests whether that state can generalize one slot wider."
                ),
                "recent_capital_allocation_failures": (
                    "FINRA squeeze and ATR/relative-weakness scouts failed or were "
                    "data/sample limited; this uses an already production-visible "
                    "entry-loop state with cross-window sample coverage."
                ),
            ...
            "3_single_causal_variable": (
                "two_slot_rank1_risk_multiplier applied only after shared candidate "
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260517_005_two_slot_rank1_topup.py"
            ),
        },
        "backtest_protocol": {
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, add the two-slot top-up to shared production_parity.py "
        ...
        "why_not_other_changes": (
            ...
            "failed, avoids nearby FINRA/ATR/relative-weakness retries after their "
            "three-window rejections, and avoids candidate-pool expansion because "
            "recent breadth additions added noise."
        ),
        "known_risks": [
            "The state is adjacent to the accepted one-slot top-up, so it must be rejected if any window regresses.",
            "A rank-1 two-slot top-up can clip future flexibility by using more heat before the second selected signal resolves.",
        ...
        "next_evidence_needed": (
            None
            if selected["passed"]
            else "Keep the one-slot top-up narrow; test a different allocation state rather than broadening scarce-slot rank-1 exposure."
        ),

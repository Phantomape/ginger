"""exp-20260517-025: rotation state-surface daily candidate count.

Alpha search. Tests one production-visible default-off paper-sleeve variable:
how many top-ranked rotation state-surface candidates are allowed into the
daily paper candidate queue after the accepted ret20_excess_spy >= 0.0 gate.
Core A/B signals, ranking, sizing, exits, LLM/news, event bundle definitions,
hold days, active capacity, and live orders are unchanged.
...
EXPERIMENT_ID = "exp-20260517-025"
EXPERIMENT_SLUG = "state_surface_rotation_daily_candidate_count"
TARGET_SURFACE = "rotation_breakout_leadership"
BASELINE_DAILY_CANDIDATE_COUNT = 3
DAILY_CANDIDATE_COUNT_VARIANTS = [1, 2, 3, 4, 5, 6]
...
def _rotation_candidates_for_count(
    *,
    ...
    parent.base.DAILY_CANDIDATE_COUNT = daily_candidate_count
    try:
...
def _variant_payload(
    *,
    ...
    for label, window in WINDOWS.items():
        candidates = _rotation_candidates_for_count(
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rotation Daily Candidate Count",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {count} | {passed} | {ev:+.4f} | ${pnl:+,.2f} | {wi} | {wr} | {trades} | {dd:+.4%} | {share} |".format(
                count=row["daily_candidate_count"],
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for count in DAILY_CANDIDATE_COUNT_VARIANTS
    ]
    baseline = next(
        row
        for row in variants
        if row["daily_candidate_count"] == BASELINE_DAILY_CANDIDATE_COUNT
    )
    ...
    for variant in variants:
        ...
        sweep_summary.append(
            {
                "daily_candidate_count": variant["daily_candidate_count"],
                "is_identity_control": (
                    variant["daily_candidate_count"] == BASELINE_DAILY_CANDIDATE_COUNT
                ),
                "selected_trade_count": variant["selected_trade_count"],
                "single_ticker_positive_share": variant["single_ticker_positive_share"],
                "gate4": gate4,
    ...
    best_variant = next(
        row
        for row in variants
        if row["daily_candidate_count"] == best_summary["daily_candidate_count"]
    )
    ...
    if best_summary["gate4"]["passed"]:
        ...
        status = "accepted"
        interpretation = (
            "Changing the daily rotation paper candidate count improved the accepted "
            "ret20-spy-gated state-surface sleeve. The passing count is promoted to "
            "the shared default-off state_surface_sleeve.py policy with a parity test; "
            "live/default orders remain disabled."
        )
    else:
        decision = "rejected_state_surface_daily_candidate_count"
        status = "rejected"
        interpretation = (
            "No tested daily candidate count improved the accepted rotation-only "
    ...
    return {
        ...
        "decision": decision,
        "lane": "alpha_search",
        "change_type": "state_surface_rotation_daily_candidate_count",
        "changed_variable": "daily_candidate_count",
        "change_summary": (
            "Sweep how many top-ranked rotation-only state-surface paper candidates "
            "are admitted per day after the accepted ret20_excess_spy gate."
        ),
        "component": "quant/experiments",
        "mechanism_family": "state_aware_candidate_pool_capacity",
        "hypothesis": (
            "The accepted rotation-only state-surface paper sleeve may be too narrow "
            "or too broad at exactly three top-ranked daily candidates. Testing top-N "
            "candidate count measures whether controlled candidate-pool expansion or "
            "contraction improves replacement value without adding arbitrary tickers, "
            "changing scoring, or touching live orders."
        ),
        "alpha_hypothesis": {
            "category": "candidate_pool_extension",
            "entry_exit_ranking_or_allocation": "satellite paper candidate eligibility",
            "playbook_alignment": (
                "Targets the playbook's event-rotation replacement-value lane and "
                "candidate-pool alpha search. It avoids LLM soft-ranking data limits "
        ...
        "history_check": {
            ...
            "exp-20260517-022": (
                "Rejected active-position capacity sweep, but it explicitly locked "
                "daily candidate rank count."
            ),
            ...
            "anti_repeat_boundary": (
                ...
                "not an active-position capacity retry, and not a notional scalar sweep."
            ),
        },
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "candidate-pool alpha: vary the top-N daily rotation paper candidate "
                "count after the accepted ret20_excess_spy >= 0 gate"
            ),
            "2_history_check": (
                "Recent state-surface work tested surface, ret20/ret5/ret60/near-high/"
                "volume gates and active capacity. No current-stack daily top-N count "
                "experiment after the accepted ret20 gate was found."
            ),
            "3_single_causal_variable": "daily_candidate_count",
            "4_acceptance_standard": (
                "docs/backtesting.md three fixed windows; best non-control count must "
                "improve aggregate EV/PnL versus current accepted top-3 ret20-spy-gated "
                "control, improve at least two windows, regress zero windows, keep "
            ...
            "5_reproducibility": (
                ...
                "exp_20260517_025_state_surface_rotation_daily_candidate_count.py"
            ),
        },
        "parameters": {
            "single_causal_variable": "daily_candidate_count",
            "baseline_daily_candidate_count": BASELINE_DAILY_CANDIDATE_COUNT,
            "daily_candidate_count_variants": DAILY_CANDIDATE_COUNT_VARIANTS,
            "best_daily_candidate_count": best_summary["daily_candidate_count"],
        ...
        "gate1": {
            ...
            "current_state_surface_top3_ret20_spy_gate_baseline_metrics": baseline_metrics,
            "baseline_daily_candidate_count": BASELINE_DAILY_CANDIDATE_COUNT,
        },
        "gate2": {
        ...
        "production_impact": {
            ...
            "production_impact": (
                "Shared default-off paper policy is changed only for the state-surface "
                "candidate queue. The same state_surface_sleeve.py path is used by "
                "production and tests; live/default orders remain disabled."
            ),
        },
        "llm_metrics": {
            "used_llm": False,
            "why_not_llm": (
                ...
                "state-surface paper candidate-count test uses replayable OHLCV fields."
            ),
        },
        "interpretation": interpretation,
        "rejection_reason": None if best_summary["gate4"]["passed"] else interpretation,
        "next_evidence_needed": (
            "Monitor forward closed replacement-value outcomes from the shared top-five default-off paper queue before enabling any trade adapter."
            if best_summary["gate4"]["passed"]
            else "Do not change daily state-surface candidate count on frozen windows; next state-surface alpha should use a different production-visible discriminator or forward replacement-value evidence."
        ),
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rotation daily candidate count",
            "status": payload["status"],
            "decision": payload["decision"],
            "changed_variable": payload["parameters"]["single_causal_variable"],
            "best_daily_candidate_count": payload["parameters"]["best_daily_candidate_count"],

...
Tests one causal variable on top of accepted exp-20260514-024: whether the
accepted forward same-theme replacement-strength trend bucket deserves a
further small top-up only when the ticker also has a company-release
customer-win source profile.
...
EXPERIMENT_ID = "exp-20260514-026"
STEM = "space_forward_replacement_company_source_trend_risk"
BEFORE_EXPERIMENT_ID = "exp-20260514-024"
...
COMPANY_SOURCE_TREND_RISK_SCALARS = (1.00, 1.025, 1.05, 1.075, 1.10)
...
def _neutralize_promoted_company_source_helper_for_replay_base() -> None:
    """Keep the replay baseline at accepted exp-024 after this experiment is promoted."""
    space_catalyst_sleeve.SPACE_CATALYST_FORWARD_REPLACEMENT_COMPANY_SOURCE_TREND_RISK_SCALAR = (
        1.0
    )
...
def _run_variant(
    label: str,
    *,
    company_source_trend_scalar: float,
    ...
    _neutralize_promoted_company_source_helper_for_replay_base()
    universe = sorted(
        set(get_universe())
    ...
    def size_with_company_source_trend_scalar(
        ...
        for signal in sized:
            ...
            if is_company_source_trend and sizing:
                ...
                shares_before, shares_after = iwm_trend_exp._scale_and_count(
                    sizing=sizing,
                    scalar=company_source_trend_scalar,
                    portfolio_value=portfolio_value,
                    marker="space_forward_replacement_company_source_trend_risk",
                    counts=counts,
                    ticker=ticker,
                )
                adjustments.append(
                    {
                        ...
                        "company_source_profile": company_source_profiles.get(ticker),
                ...
                signal = {
                    **signal,
                    ...
                    "space_forward_replacement_company_source_trend_scalar": (
                        company_source_trend_scalar
                    ),
                    "space_company_release_source_profile": (
                        company_source_profiles.get(ticker)
                    ),
                }
    ...
    portfolio_engine.size_signals = size_with_company_source_trend_scalar
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            by_window[name] = {
                ...
                "company_source_trend_trade_attribution": (
                ...
                "company_source_trend_adjustment": (
                    source_diversity_exp._adjustment_summary(window_adjustments)
                ),
                "company_source_trend_counts": dict(
        ...
        return {
            ...
            "company_source_trend_adjustment_summary": (
                source_diversity_exp._adjustment_summary(adjustments)
            ),
            "company_source_trend_counts": dict(sorted(counts.items())),
            "company_source_trend_adjustment_sample": adjustments[:25],
        }
    finally:
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["company_source_trend_counts"].get(
            "space_forward_replacement_company_source_trend_risk_changed_signal",
            0,
        )
    )
    eligible_count = int(variant["company_source_trend_counts"].get("eligible_signal", 0))
    scalar = float(
        variant["parameters"]["space_forward_replacement_company_source_trend_scalar"]
    )
    return {
        ...
        "eligible_company_source_trend_signal_count": eligible_count,
        "changed_company_source_trend_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _gate2_runtime_state(before: dict[str, Any]) -> dict[str, Any]:
    eligible = int(before["company_source_trend_counts"].get("eligible_signal", 0))
    samples = before.get("company_source_trend_adjustment_sample", [])
    return {
        "passed": eligible > 0,
        "required_runtime_fields": [
            "data/space_catalyst_event_seeds.jsonl source_type=company_release",
            "data/space_catalyst_event_seeds.jsonl event_fields contains customer_win",
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space forward replacement company-source trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_forward_replacement_company_source_trend_scalar` for "
            "`trend_long` signals already in the accepted forward same-theme "
            "replacement-strength bucket and whose event seed profile has a "
            "`company_release` customer-win source. Candidate pool, ranking, "
            "targets, stops, LLM/news, accepted exp-024 stack, and live Space "
            "slots stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_forward_replacement_company_source_trend_scalar']}`"
        ),
        (
            "- Aggregate delta vs exp-024: "
            f"EV `{gate['aggregate_delta_vs_before']['expected_value_score_sum']:.6f}`, "
            f"PnL `{gate['aggregate_delta_vs_before']['total_pnl_sum']:.2f}`"
        ),
        (
            "- Company-source trend signals changed: "
            f"`{gate['changed_company_source_trend_signal_count']}` of "
            f"`{gate['eligible_company_source_trend_signal_count']}` eligible"
        ),
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        adjusted = best["by_window"][name]["company_source_trend_adjustment"][
...
def run() -> dict[str, Any]:
    ...
    before = _run_variant(
        "accepted_exp024_company_source_neutral",
        company_source_trend_scalar=1.0,
    ...
    gate2 = {
        ...
        "company_source_trend_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    variants = [
        _run_variant(
            ...
            company_source_trend_scalar=scalar,
            forward_gate=forward_gate,
            source_diversity_gate=source_diversity_gate,
            attention_gate=attention_gate,
            single_event_gate=single_event_gate,
        ...
        for scalar in COMPANY_SOURCE_TREND_RISK_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested company-release confirmation scalar improved aggregate "
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Space replacement-strength allocation should be strongest when closed "
            "10d same-theme replacement value, trend continuation, and a "
            "company-release customer-win catalyst all agree. On top of accepted "
            "exp-20260514-024, a single extra company-source trend scalar tests "
            "that catalyst-quality interaction without adding tickers, broad "
            "filters, LLM authority, lifecycle rules, or live Space slots."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_forward_replacement_company_source_trend_risk_scalar",
        "single_causal_variable": (
            "extra risk scalar for trend_long signals in the accepted forward "
            "same-theme replacement-strength bucket when the production event "
            "seed profile includes company_release customer_win"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                ...
                "same-theme replacement-strength bucket and whose event seed "
                "profile has company_release customer_win."
            ),
            "q2_prior_experiments": [
                ...
                "exp-20260514-024 accepted same-theme replacement-strength trend plus IWM-leader interaction at 1.025x.",
                "No prior record found for same-theme replacement-strength trend plus company-release customer-win interaction.",
            ],
            "q3_single_causal_variable": (
                "Only the additional company-source trend risk scalar changes; "
                "accepted exp-024 stack and all entries/exits/ranking/targets stay fixed."
            ),
        ...
        "parameters": {
            ...
            "accepted_iwm_leader_trend_scalar": (
                ACCEPTED_FORWARD_REPLACEMENT_IWM_LEADER_TREND_RISK_SCALAR
            ),
            "tested_company_source_trend_scalars": list(
                COMPANY_SOURCE_TREND_RISK_SCALARS
            ),
            "base_forward_replacement_target_tickers": sorted(
            ...
            "company_source_target_tickers": sorted(
                company_release_gate["target_tickers"]
            ),
            "target_strategy": "trend_long",
            "locked_variables": [
                ...
                "accepted exp-20260514-024 IWM-leader trend scalar",
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_forward_replacement_company_source_trend_risk"
            if decision == "accepted"
            else "rejected_space_forward_replacement_company_source_trend_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not promote company-source replacement-strength risk from "
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains label-thin; VSAT/IRDM and benchmark-laggard "
            "candidate expansion failed the gate; 5d confirmation and peer-leader "
            "trend variants were rejected. This keeps the candidate pool fixed and "
            "tests a production-visible source-quality interaction with replacement value."
        ),
        "known_risks": [
            "The Space sleeve remains default-off and historical Space snapshots are frozen research copies.",
            "The company-release slice is narrow and should stay conservative even if accepted.",
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_company_source_trend_signal_count": result["best_variant_gate"][
            "changed_company_source_trend_signal_count"
        ],

"""exp-20260514-015: Space VSAT forward-candidate risk.

Tests one candidate-pool/risk variable on top of the accepted exp-20260514-009
default-off Space stack: whether VSAT alone deserves a bounded risk budget after
the closed Golden Dome/defense-budget forward ledger shows positive 10d cash,
same-theme, UFO, and SPY-relative replacement value.
...
EXPERIMENT_ID = "exp-20260514-015"
STEM = "space_vsat_forward_candidate_risk"
...
def _trade_attribution_for(result: dict[str, Any], tickers: set[str]) -> dict[str, Any]:
    trades = [
        trade
        ...
        for signal in sized:
            ...
            if ticker in vsat_tickers and sizing:
                ...
                shares_before, shares_after = _scale_and_count(
                    sizing=sizing,
                    scalar=vsat_candidate_scalar,
                    portfolio_value=portfolio_value,
                    marker="space_vsat_forward_candidate_risk",
                    counts=counts,
                    ticker=ticker,
                )
                ...
                signal = {
                    **signal,
                    "space_vsat_forward_candidate_bucket": True,
                    "space_vsat_forward_candidate_scalar": vsat_candidate_scalar,
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["vsat_candidate_counts"].get(
            "space_vsat_forward_candidate_risk_changed_signal",
            0,
        )
    )
    eligible_count = int(variant["vsat_candidate_counts"].get("eligible_signal", 0))
    scalar = float(variant["parameters"]["space_vsat_forward_candidate_scalar"])
    ...
    return {
        ...
        "changed_vsat_candidate_signal_count": changed_count,
        "vsat_trade_attribution": vsat_trade_attr,
        "accepted": bool(
            scalar > 0.0
            and changed_count > 0
            and vsat_trade_attr["trade_count"] > 0
...
def _gate2_runtime_state(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for variant in variants:
        for window_name, payload in variant.get("by_window", {}).items():
            ...
            changed += int(
                counts.get("space_vsat_forward_candidate_risk_changed_signal", 0)
                or 0
            )
    ...
    return {
        "passed": eligible > 0 and changed > 0,
        ...
        "changed_signal_count_across_tested_variants": changed,
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space VSAT forward-candidate risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_vsat_forward_candidate_scalar`, where the accepted exp-009 "
            "default-off Space stack is unchanged and only VSAT is admitted with "
            "bounded risk after passing the closed official Golden Dome forward "
            "replacement profile. Broad satcom, GSAT, LLM/news, entries, exits, "
            "ranking, targets, and live Space slots stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_vsat_forward_candidate_scalar']}`"
        ),
        (
        ...
        (
            "- VSAT signals changed: "
            f"`{gate['changed_vsat_candidate_signal_count']}` of "
            f"`{gate['eligible_vsat_candidate_signal_count']}` eligible"
        ),
        (
            "- VSAT trades: "
            f"`{gate['vsat_trade_attribution']['trade_count']}`, "
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        vsat_trades = best["by_window"][name]["vsat_trade_attribution"]["trade_count"]
        lines.append(
            "| {name} | {ev:.6f} | {pnl:.2f} | {dd:.6f} | {trades} | {survival:.6f} | {adjusted} | {vsat_trades} |".format(
                name=name,
...
def run() -> dict[str, Any]:
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested VSAT-only forward-candidate scalar improved aggregate "
            "EV/PnL across the three windows without a window-level EV regression, "
            "drawdown or survival violation, or zero-adjustment/zero-trade result."
        )
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "same-theme, SPY-relative, and UFO-relative replacement value. A "
            "bounded VSAT-only risk budget may add replacement alpha on top of "
            "the accepted exp-20260514-009 default-off Space stack."
        ),
        "change_type": "candidate_pool_risk_allocation_shadow_sweep",
        "changed_variable": "space_vsat_forward_candidate_risk_scalar",
        "single_causal_variable": (
            "admit only VSAT with a bounded risk scalar after the closed official "
            "defense-budget forward profile passes cash, same-theme, SPY, and "
            "UFO-relative gates"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "candidate pool / risk allocation: add only VSAT after the "
                "official Golden Dome defense-budget forward ledger shows "
                "positive 10d cash, same-theme, SPY, and UFO-relative evidence."
            ),
            ...
            "q3_single_causal_variable": (
                "Only the VSAT forward-candidate risk scalar changes; accepted "
                "exp-009 stack, entries, exits, ranking, targets, LLM/news, and "
            ...
            "q4_acceptance_standard": (
                ...
                "5%, >=50 aggregate trades, actual VSAT changed signals, and "
                "actual VSAT trades."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            "tested_vsat_forward_candidate_scalars": list(
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_vsat_forward_candidate_risk"
            if decision == "accepted"
            else "rejected_space_vsat_forward_candidate_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not promote VSAT-only candidate admission from this frozen "
                "replay. Wait for more closed official forward evidence or test "
                "a different non-noisy candidate-quality axis."
            )
        ),
        "production_impact": {
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "best_scalar": result["best_variant"]["parameters"][
            "space_vsat_forward_candidate_scalar"
        ],
        ...
        "changed_vsat_candidate_signal_count": result["best_variant_gate"][
            "changed_vsat_candidate_signal_count"
        ],

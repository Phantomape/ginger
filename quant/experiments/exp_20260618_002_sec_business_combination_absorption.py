"""exp-20260618-002: SEC business-combination absorption scout.

Replay-only alpha search. The single decision hypothesis is that PIT SEC
business-combination, merger-proxy, and tender-offer filings can become positive
candidate-pool events when signal-day price action absorbs deal/dilution risk
versus SPY before next-open paper entry.

This is distinct from offering/prospectus absorption, proxy-pressure forms,
8-K restructuring events, SEC text contract economics, and filing-timeliness
scouts. The form family itself is the event source; no LLM/text semantics are
used. No production code, shared adapter, live/default orders, ranking, sizing,
...
EXPERIMENT_ID = "exp-20260618-002"
STEM = "sec_business_combination_absorption"
TRIAL_FAMILY = "free_sec_submissions_business_combination_absorption_candidate_pool"
TRIAL_VARIANT_ID = "sec_business_combination_absorption_top1_next_open_10d_v1"
CHANGED_VARIABLE = "sec_business_combination_absorption_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260618_002_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "corporate_action_events_are_priced_before_next_open",
        "merger_arb_has_capped_drift",
        "old_thin_regression",
        "accepted_distribution_comparator_not_beaten",
    ],
    "confidence_reason": (
        "M&A/tender SEC forms are a free PIT submissions surface with enough "
        "broad-universe coverage and are materially different from rejected "
        "financing/proxy/restructuring/accounting fields; the main risk is "
        "that deal economics are already priced or capped before next-open "
        "execution."
    ),
    "recorded_at": "2026-06-18T01:06:55+00:00",
}
...
PRODUCTION_IMPACT = {
    **runner.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **runner.PRODUCTION_IMPACT["execution_envelope"],
        ...
        "failure_handling": (
            ...
            "business-combination/tender filing rows, missing OHLCV, missing "
    ...
    "parity_note": (
        ...
        "SEC business-combination/tender forms, acceptance-time signal date, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: PIT SEC business-combination and tender-offer filings "
        "(S-4, merger proxy, and tender forms) may identify corporate-action "
        "events that become positive when signal-day price action absorbs "
        "deal/dilution risk versus SPY before next-open default-off paper entry."
    ),
    "2_history_check": {
        "exp-20260617-023": (
            "Rejected SEC offering/prospectus absorption. This run excludes "
            "generic S-1/S-3/F-3/424B financing forms and tests business-"
            "combination registration/proxy/tender forms."
        ),
        "exp-20260617-027": (
            "Rejected non-management proxy-pressure absorption. This run is not "
            "activist/proxy-pressure provenance; it uses issuer or deal-party "
            "business-combination and tender-offer forms."
        ),
        "exp-20260618-001": (
            "Rejected 8-K Item 2.05 restructuring absorption on thin sample. "
            "This run does not retry 8-K item codes or restructuring economics."
        ),
        "exp-20260617-011": (
            "Rejected SEC text contract economics and explicitly excluded "
            "merger/tender text from contract parsing. This run avoids text "
            "regexes and tests the structured SEC form family directly."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260618_002_sec_business_combination_absorption.py"
    ),
}
...
def _load_event_index() -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        for i in range(min(len(forms), len(filing_dates))):
            ...
            if form not in BUSINESS_COMBINATION_FORMS:
                continue
        ...
        if events:
            ...
            index[ticker] = events
            stats["tickers_with_business_combination_events"] += 1
            stats["business_combination_event_count"] += len(events)
...
def _build_quality_index(
    ...
    return index, {
        **summary,
        ...
        "field_source": "sec_submissions_business_combination_forms_not_companyfacts",
    }
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "SEC_BUSINESS_COMBINATION_ABSORPTION_PAPER",
                    "candidate_score": runner._round(score, 6),
                    "rule_version": RULE_VERSION,
                    "source_rule_version": RULE_VERSION,
                    "known_at": "sec_business_combination_form_and_signal_close_before_next_open_paper_entry",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    "business_combination_form": event["form"],
                    "business_combination_filing_date": event["filing_date"],
                    "business_combination_accepted_after_close": event["accepted_after_close"],
                    "business_combination_acceptance_datetime": event["acceptance_datetime"],
                    "business_combination_accession_number": event["accession_number"],
                    "business_combination_primary_document": event["primary_document"],
                    "business_combination_form_weight": event["form_weight"],
                    **confirm,
                }
            )
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_sec_business_combination_absorption"
        if gate["passed"]
        else "rejected_sec_business_combination_absorption_candidate_pool"
    )
    return gate
...
def _interpretation(payload: dict[str, Any]) -> str:
    ...
    if gate4["passed"]:
        return (
            "The SEC business-combination/tender absorption source cleared the "
            "numeric three-window replay screen, but remains only a replay lead "
            "because no shared daily/backtest helper was promoted."
        )
    return (
        "The SEC business-combination/tender absorption source did not clear "
        f"Gate 4 (failed: {', '.join(gate4['failed_reasons']) or 'none'}). The "
        "fixed bundle tested S-4, merger proxy, and tender-offer form families "
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "mechanism_family": (
                "production_visible_free_sec_submissions_business_combination_candidate_pool"
            ),
            "new_evidence_type": "sec_submissions_business_combination_forms_with_price_absorption",
    ...
    payload["parameters"] = {
        ...
        "excluded_recent_form_families": [
            ...
            "DFAN14A/PX14A6G/PREC14A/PRRN14A proxy-pressure forms",
            "8-K item-code events",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SEC business-combination events are read from EDGAR submissions cache "
        "recent filings for forms S-4, S-4/A, DEFM14A, PREM14A, SC TO-I, "
        "SC TO-T, SC 14D9, and SC 14D9/A. The signal date is the filing date "
        "unless the SEC acceptance timestamp is after 16:00, in which case it "
        "is the next trading day. Candidates must show signal-day price "
        "absorption before next-open paper entry: non-negative daily return, "
        "return minus SPY >= 0.5%, close location >= 0.56, volume ratio >= "
        "0.75, realized vol <= 12%, ret20 excess vs SPY >= -5%, price >= $10, "
        "and ADV20 >= $50M. Paper entry is the next available open with entry "
    ...
    payload["gate2"]["runtime_fields"] = [
        "SEC submissions recent.form (business-combination/tender forms)",
    ...
    payload["next_evidence_needed"] = (
        "If this fixed business-combination/tender absorption bundle fails, do "
        "not retry by sweeping form weights, S-4 amendment inclusion, signal "
        "excess, close-location, volume, volatility, ret20, price/ADV, event "
        "age, top-N, hold days, cooldown, or notional on these frozen windows. "
        "A valid retry needs materially richer PIT deal economics such as cash/"
        "stock consideration, premium/discount, bidder/target role, break-fee, "
        "regulatory condition, shareholder vote status, or closed forward "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping business-combination/tender form weights, "
            "S-4 amendment inclusion, signal excess, close-location, volume, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} SEC Business-Combination Absorption",
            "",
...
def _install() -> None:
    ...
    runner.S8_FORMS = BUSINESS_COMBINATION_FORMS
    runner.FORM_WEIGHTS = FORM_WEIGHTS
    ...
    runner._signal_date_for_event = _signal_date_for_event

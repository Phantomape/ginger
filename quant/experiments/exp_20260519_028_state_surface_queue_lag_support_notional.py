...
exp-20260519-027, then tests one production-visible allocation variable:
already-selected default-off paper candidates whose raw score rank lags their
queue position receive a bounded notional support scalar.
...
import exp_20260519_027_state_surface_sleeve_capacity_notional as baseline_exp


EXPERIMENT_ID = "exp-20260519-028"
EXPERIMENT_SLUG = "state_surface_queue_lag_support_notional"

REPO_ROOT = baseline_exp.REPO_ROOT
WINDOWS = baseline_exp.WINDOWS
BASELINE_VARIANT = "accepted_sleeve_capacity_notional"
...
RULE_VERSION = "state_surface_queue_lag_support_notional_v1"

QUEUE_LAG_VARIANTS: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
                "scalar": None,
                "aggression_order": 0,
                "description": "accepted stack through sleeve-capacity notional",
            },
        ),
        (
            "queue_lag_scalar_103",
            {
                "scalar": 1.03,
                "aggression_order": 1,
                "description": "rank > queue_rank selected candidates receive 3% notional support",
            },
        ),
        (
            "queue_lag_scalar_105",
            {
                "scalar": 1.05,
                "aggression_order": 2,
                "description": "rank > queue_rank selected candidates receive 5% notional support",
            },
        ),
        (
            "queue_lag_scalar_110",
            {
                "scalar": 1.10,
                "aggression_order": 3,
                "description": "rank > queue_rank selected candidates receive 10% notional support",
            },
        ),
        (
            "queue_lag_scalar_115",
            {
                "scalar": 1.15,
                "aggression_order": 4,
                "description": "rank > queue_rank selected candidates receive 15% notional support",
            },
        ),
        (
            "queue_lag_scalar_125",
            {
                "scalar": 1.25,
                "aggression_order": 5,
                "description": "rank > queue_rank selected candidates receive 25% notional support",
            },
        ),
    ]
)

BASELINE_JSON = baseline_exp.OUT_JSON
...
def _profile_name(scalar: float) -> str:
    ...
    return f"queue_lag_support_{scalar_text.replace('.', 'p')}x"


def _queue_lag_applies(trade: dict[str, Any]) -> bool:
    try:
        rank = int(trade.get("rank"))
        queue_rank = int(trade.get("queue_rank"))
    except (TypeError, ValueError):
        return False
    return rank > queue_rank > 0


def _apply_queue_lag_support(
    ...
    *,
    ...
    for trade in trades:
        ...
        qualifies = _queue_lag_applies(row)
        applies = (
            variant_name != BASELINE_VARIANT and scalar is not None and qualifies
        )
        rank = prev._float(row.get("rank"))
        queue_rank = prev._float(row.get("queue_rank"))
        row["queue_lag_support_variant"] = variant_name
        row["queue_lag_support_rule_version"] = RULE_VERSION
        row["rank_notional_queue_lag_support_rule_version"] = RULE_VERSION
        row["queue_lag_support_applied"] = bool(applies)
        row["queue_lag_support_qualified"] = bool(qualifies)
        row["queue_lag_support_rank"] = int(rank) if rank is not None else None
        row["queue_lag_support_queue_rank"] = (
            int(queue_rank) if queue_rank is not None else None
        )
        row["queue_lag_support_delta"] = (
            int(rank) - int(queue_rank)
            if rank is not None and queue_rank is not None
            else None
        )
        row["queue_lag_support_configured_scalar"] = scalar
        row["queue_lag_support_scalar"] = scalar if applies else None
        row["queue_lag_support_profile_name"] = (
            _profile_name(scalar)
            if variant_name != BASELINE_VARIANT and scalar is not None
            else None
        )
        row["queue_lag_support_base_multiplier"] = prev._float(
        ...
        if applies:
            ...
            row["queue_lag_support_base_notional"] = base_notional
...
def _selected_trade_rows(trades: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row, trade in zip(rows, trades):
        row["queue_lag_support_applied"] = trade.get("queue_lag_support_applied")
        row["queue_lag_support_qualified"] = trade.get(
            "queue_lag_support_qualified"
        )
        row["queue_lag_support_scalar"] = trade.get("queue_lag_support_scalar")
        row["queue_lag_support_delta"] = trade.get("queue_lag_support_delta")
        row["queue_lag_support_base_multiplier"] = trade.get(
...
def _variant_payload(
    *,
    ...
    for label, window in WINDOWS.items():
        ...
        selected = _apply_queue_lag_support(
            baseline_trades,
        ...
        qualified = [row for row in selected if row.get("queue_lag_support_qualified")]
        applied = [row for row in selected if row.get("queue_lag_support_applied")]
        surface_sleeve[label] = {
            "selected_trade_count": len(selected),
            "queue_lag_support_qualified_trade_count": len(qualified),
            "queue_lag_support_adjusted_trade_count": len(applied),
            "queue_lag_support_adjusted_pnl": round(
                sum(float(row.get("pnl") or 0.0) for row in applied),
                2,
            ),
            "queue_lag_support_qualified_pnl": round(
    ...
    applied_all = [row for row in selected_all if row.get("queue_lag_support_applied")]
    qualified_all = [
        row for row in selected_all if row.get("queue_lag_support_qualified")
    ]
    applied_windows = {
    ...
    return {
        "variant_name": variant_name,
        "variant_type": "queue_lag_support_notional_profile",
        ...
        "queue_lag_support_qualified_trade_count": len(qualified_all),
        "queue_lag_support_qualified_windows": sorted(qualified_windows),
        "queue_lag_support_adjusted_trade_count": len(applied_all),
        "queue_lag_support_adjusted_windows": sorted(applied_windows),
...
def _gate4_for_variant(
    *,
    ...
    adjusted_guard_passed = (
        variant["queue_lag_support_adjusted_trade_count"] >= MIN_ADJUSTED_TRADES
        and len(variant["queue_lag_support_adjusted_windows"])
    ...
    return {
        ...
        "queue_lag_support_adjusted_trade_count": variant[
            "queue_lag_support_adjusted_trade_count"
        ],
        "queue_lag_support_adjusted_windows": variant[
            "queue_lag_support_adjusted_windows"
        ],
        "queue_lag_support_qualified_trade_count": variant[
            "queue_lag_support_qualified_trade_count"
        ],
        "queue_lag_support_qualified_windows": variant[
            "queue_lag_support_qualified_windows"
        ],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Queue-Lag Support Notional",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {passed} | {scalar} | {ev:+.4f} | ${pnl:+,.2f} | {wi} | {wr} | {adj} | {dd:+.4%} | {share} |".format(
                ...
                adj=row["gate4"]["queue_lag_support_adjusted_trade_count"],
    ...
    for label in WINDOWS:
        ...
        lines.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {bdd:.2%} | {add:.2%} | {trades} |".format(
                ...
                trades=sleeve["queue_lag_support_adjusted_trade_count"],
            )
        )
    lines.extend(
        [
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for name, variant in QUEUE_LAG_VARIANTS.items()
    ]
    sweep_summary = []
    ...
    decision = (
        "accepted_default_off_state_surface_queue_lag_support_notional"
        if passed
        else "rejected_state_surface_queue_lag_support_notional"
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": "Selected state-surface paper candidates whose raw rank lags their queue position are replacement-value winners created by core exclusions/slot ordering, so they deserve a bounded notional support scalar.",
        "alpha_hypothesis": {
            "category": "capital allocation",
            "entry_exit_ranking_or_allocation": "default-off paper allocation",
            "playbook_alignment": "Tests a deterministic state-surface quality field rather than LLM soft-ranking or noisy ticker-pool expansion.",
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "queue_lag_support_notional_profile",
        "component": "quant/state_surface_sleeve.py",
        "parameters": {
            "best_variant": best["variant_name"],
            "best_scalar": best["scalar"],
            "condition": "rank > queue_rank",
            "profile_priority": "applies after the accepted sleeve-capacity notional stack and before recent-repeat paper metadata",
            "locked_variables": [
                "core entries",
                "core exits",
                "core sizing",
        ...
        "backtest_protocol": "docs/backtesting.md canonical three fixed windows; accepted exp-20260519-027 baseline artifact plus default-off state-surface paper overlay replay.",
        ...
        "gate1": {
            ...
            "baseline_note": "Uses accepted exp-20260519-027 sleeve-capacity notional as Gate 1 baseline.",
        },
        "gate2": {
        ...
        "history_check": {
            "exp-20260519-027": "Accepted sleeve-capacity notional; this experiment freezes it and tests a distinct selected-trade queue-lag quality field.",
            "nearby_rejections": "Avoids global capacity retunes, broad SEC tone/text scalars, rank4 no-sample retries, and LLM soft-ranking.",
            "anti_repeat": "Not a ret5/ret20/ret60 threshold, near-high, volume-threshold, sector-cohesion, score-gap, candidate-pool, SEC text, or LLM soft-ranking retry.",
        },
        "llm_metrics": {
            "used_llm": False,
            "why_not_llm": "LLM soft-ranking data remains sparse/PIT-limited; this tests a replayable rank/queue displacement field.",
        },
        "production_impact": {
        ...
        "interpretation": (
            "Queue-lag support improved the default-off state-surface paper overlay without changing core trades, filters, ranking, or live/default orders."
            if passed
            else "Queue-lag support did not clear Gate 4; keep the accepted sleeve-capacity stack unchanged."
        ),
        ...
        "next_evidence_needed": (
            ...
            if passed
            else "Do not retry nearby queue-rank support scalars without a broader sample or a distinct displacement/risk field."
        ),
        "protocol_answers": {
            "1_alpha_hypothesis": "capital allocation: selected state-surface candidates with raw rank worse than queue rank may have replacement-value alpha from core exclusions/queue displacement.",
            "2_history_check": "No prior accepted experiment tested rank > queue_rank support after exp-20260519-027. Recent LLM/pool lanes are data-limited and global capacity was just accepted.",
            "3_single_causal_variable": "queue_lag_support_notional_profile",
...
def main() -> None:
    ...
    prev._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface queue-lag support notional",
            "status": payload["status"],
            "decision": payload["decision"],
            "artifact": prev._repo_rel(OUT_JSON),
            "summary": (
                f"Queue-lag profile {payload['parameters']['best_variant']} "

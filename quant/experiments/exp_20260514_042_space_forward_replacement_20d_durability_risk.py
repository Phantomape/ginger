...
Tests one causal variable on top of accepted exp-20260514-041: whether
official Space trend candidates whose closed 20d event-state profile remains
positive versus cash, same-theme replacement, SPY, QQQ, UFO, and ARKX deserve
an additional conservative default-off risk scalar.

This is an alpha search, not a repair. It avoids LLM soft-ranking, noisy ticker
expansion, lifecycle changes, and nearby 10d benchmark-breadth scalar retunes.
"""
...
import exp_20260514_041_space_benchmark_breadth_trend_risk as benchmark_exp


logging.getLogger("yfinance").setLevel(logging.CRITICAL)
logging.getLogger("portfolio_engine").setLevel(logging.ERROR)

EXPERIMENT_ID = "exp-20260514-042"
STEM = "space_forward_replacement_20d_durability_trend_risk"
...
ACCEPTED_BENCHMARK_BREADTH_TREND_RISK_SCALAR = 1.025
TWENTY_DAY_DURABILITY_RISK_SCALARS = (1.0, 1.025, 1.05, 1.075, 1.10)
TARGET_STRATEGY = "trend_long"

MIN_20D_CASH = 0.0
MIN_20D_SAME_THEME = 0.0
MIN_20D_SPY_RELATIVE = 0.0
MIN_20D_QQQ_RELATIVE = 0.0
MIN_20D_UFO_RELATIVE = 0.0
MIN_20D_ARKX_RELATIVE = 0.0
...
def _as_float(value: Any) -> float | None:
    return benchmark_exp._as_float(value)


def _avg(values: list[float]) -> float | None:
    return benchmark_exp._avg(values)


def _latest_closed_rows(path: Path) -> list[dict[str, Any]]:
    return benchmark_exp._latest_closed_rows(path)


def _horizon(row: dict[str, Any], name: str) -> dict[str, Any] | None:
    return benchmark_exp._horizon(row, name)


def _twenty_day_durability_profile_gate() -> dict[str, Any]:
    ...
    for row in _latest_closed_rows(path):
        ...
        if str(row.get("semantic_bucket") or "") == "attention_only":
            ...
            continue
        h20 = _horizon(row, "20d")
        if h20 is None:
            skipped["missing_mature_20d"] += 1
            continue
        fields = {
            "20d_cash_relative_pnl": _as_float(h20.get("cash_relative_pnl")),
            "20d_same_theme_replacement_value": _as_float(
                h20.get("same_theme_replacement_value")
            ),
            "20d_spy_relative_value": _as_float(h20.get("spy_relative_value")),
            "20d_qqq_relative_value": _as_float(h20.get("qqq_relative_value")),
            "20d_ufo_relative_value": _as_float(h20.get("ufo_relative_value")),
            "20d_arkx_relative_value": _as_float(h20.get("arkx_relative_value")),
        }
    ...
    for ticker, rows in sorted(rows_by_ticker.items()):
        avg_20d_cash = _avg([float(row["20d_cash_relative_pnl"]) for row in rows])
        avg_20d_same = _avg(
            [float(row["20d_same_theme_replacement_value"]) for row in rows]
        )
        avg_20d_spy = _avg([float(row["20d_spy_relative_value"]) for row in rows])
        avg_20d_qqq = _avg([float(row["20d_qqq_relative_value"]) for row in rows])
        avg_20d_ufo = _avg([float(row["20d_ufo_relative_value"]) for row in rows])
        avg_20d_arkx = _avg([float(row["20d_arkx_relative_value"]) for row in rows])
        passed = bool(
            avg_20d_cash is not None
            and avg_20d_same is not None
            and avg_20d_spy is not None
            and avg_20d_qqq is not None
            and avg_20d_ufo is not None
            and avg_20d_arkx is not None
            and avg_20d_cash > MIN_20D_CASH
            and avg_20d_same > MIN_20D_SAME_THEME
            and avg_20d_spy > MIN_20D_SPY_RELATIVE
            and avg_20d_qqq > MIN_20D_QQQ_RELATIVE
            and avg_20d_ufo > MIN_20D_UFO_RELATIVE
            and avg_20d_arkx > MIN_20D_ARKX_RELATIVE
        )
        profiles[ticker] = {
            "passed": passed,
            "closed_event_count": len(rows),
            "avg_20d_cash_relative_pnl": avg_20d_cash,
            "avg_20d_same_theme_replacement_value": avg_20d_same,
            "avg_20d_spy_relative_value": avg_20d_spy,
            "avg_20d_qqq_relative_value": avg_20d_qqq,
            "avg_20d_ufo_relative_value": avg_20d_ufo,
            "avg_20d_arkx_relative_value": avg_20d_arkx,
            "events": rows,
        }
        if passed:
    ...
    return {
        ...
        "profiles": profiles,
        "thresholds": {
            "min_avg_20d_cash_relative_pnl": MIN_20D_CASH,
            "min_avg_20d_same_theme_replacement_value": MIN_20D_SAME_THEME,
            "min_avg_20d_spy_relative_value": MIN_20D_SPY_RELATIVE,
            "min_avg_20d_qqq_relative_value": MIN_20D_QQQ_RELATIVE,
            "min_avg_20d_ufo_relative_value": MIN_20D_UFO_RELATIVE,
            "min_avg_20d_arkx_relative_value": MIN_20D_ARKX_RELATIVE,
        },
...
def _run_exp041_stack_variant(
    label: str,
    *,
    durability_scalar: float,
    durability_gate: dict[str, Any],
    ...
    durability_tickers = set(durability_gate["target_tickers"])
    durability_profiles = durability_gate["profiles"]
    delayed_adjustments: list[dict[str, Any]] = []
    benchmark_adjustments: list[dict[str, Any]] = []
    durability_adjustments: list[dict[str, Any]] = []
        ...
        *,
        ...
        if ticker in benchmark_tickers:
            _scale_and_record_extra(
                signal=signal,
                sizing=sizing,
                scalar=ACCEPTED_BENCHMARK_BREADTH_TREND_RISK_SCALAR,
                portfolio_value=portfolio_value,
            ...
            signal["space_benchmark_breadth_trend_bucket"] = True
            signal["space_benchmark_breadth_trend_scalar"] = (
                ACCEPTED_BENCHMARK_BREADTH_TREND_RISK_SCALAR
            )
            signal["space_benchmark_breadth_profile"] = benchmark_profiles.get(ticker)
        if ticker in durability_tickers:
            _scale_and_record_extra(
                signal=signal,
                sizing=sizing,
                scalar=durability_scalar,
                portfolio_value=portfolio_value,
                marker="space_forward_replacement_20d_durability_trend_risk",
                counts=counts,
                adjustments=durability_adjustments,
                profile=durability_profiles.get(ticker),
            )
            signal["space_forward_replacement_20d_durability_trend_bucket"] = True
            signal["space_forward_replacement_20d_durability_trend_scalar"] = (
                durability_scalar
            )
            signal["space_forward_replacement_20d_durability_profile"] = (
                durability_profiles.get(ticker)
            )
    ...
    durability_counts = {
        ...
        if "space_forward_replacement_20d_durability_trend_risk" in key
    }
    by_window_counts = {
        name: {
            key: value
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_forward_replacement_20d_durability_trend_scalar": (
            durability_scalar
        ),
        "twenty_day_durability_target_tickers": sorted(durability_tickers),
        "twenty_day_durability_thresholds": durability_gate["thresholds"],
    }
    variant["twenty_day_durability_counts"] = durability_counts
    variant["twenty_day_durability_counts_by_window"] = by_window_counts
    ...
    variant["twenty_day_durability_adjustment_summary"] = (
        source_diversity_exp._adjustment_summary(durability_adjustments)
    )
    variant["twenty_day_durability_adjustment_sample"] = (
        durability_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("twenty_day_durability_counts") or {}
    changed_count = int(
        counts.get(
            "space_forward_replacement_20d_durability_trend_risk_changed_signal",
            0,
        )
    )
    eligible_count = int(
        counts.get(
            "space_forward_replacement_20d_durability_trend_risk_eligible_signal",
            0,
        )
    )
    scalar = float(
    ...
    return {
        ...
        "eligible_20d_durability_signal_count": eligible_count,
        "changed_20d_durability_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space 20d durability trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_forward_replacement_20d_durability_trend_scalar` for "
            "official Space `trend_long` signals whose mature 20d event-state "
            "profile is positive versus cash, same-theme replacement, SPY, "
            "QQQ, UFO, and ARKX. Candidate pool, ranking, targets, stops, "
            "LLM/news, live slots, and the accepted exp041 stack stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_forward_replacement_20d_durability_trend_scalar']}`"
        ),
        (
        ...
        (
            "- 20d-durability signals changed: "
            f"`{gate['changed_20d_durability_signal_count']}` of "
            f"`{gate['eligible_20d_durability_signal_count']}` eligible"
        ),
        (
            "- Target tickers: "
            f"`{', '.join(best['parameters']['twenty_day_durability_target_tickers'])}`"
        ),
        "",
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        counts = best["twenty_day_durability_counts_by_window"][name]
        adjusted = counts.get(
            "space_forward_replacement_20d_durability_trend_risk_changed_signal",
            0,
        )
...
def run() -> dict[str, Any]:
    ...
    benchmark_breadth_gate = benchmark_exp._benchmark_breadth_profile_gate()
    durability_gate = _twenty_day_durability_profile_gate()

    variants = [
        _run_exp041_stack_variant(
            f"{STEM}_{str(scalar).replace('.', '_')}",
            durability_scalar=scalar,
        ...
        for scalar in TWENTY_DAY_DURABILITY_RISK_SCALARS
    ]
    before = variants[0]
    ...
    runtime_gate = {
        "passed": best["gate"]["eligible_20d_durability_signal_count"] > 0,
        "eligible_signal_count": best["gate"][
            "eligible_20d_durability_signal_count"
        ],
        "required_runtime_fields": [
            "data/space_catalyst_event_state_shadow_ledger.jsonl horizons.20d",
            "horizons.20d.cash_relative_pnl",
            "horizons.20d.same_theme_replacement_value",
            "horizons.20d.spy_relative_value",
            "horizons.20d.qqq_relative_value",
            "horizons.20d.ufo_relative_value",
            "horizons.20d.arkx_relative_value",
            "signal.strategy",
            "signal.sizing.shares_to_buy",
        ],
        "sample_rows": best.get("twenty_day_durability_adjustment_sample", [])[:10],
    }
    gate2 = {
        ...
        "twenty_day_durability_profile": durability_gate,
        "twenty_day_durability_runtime_state": runtime_gate,
    }
    gate2["passed"] = all(
        [
    ...
    status = (
        "accepted_default_off_space_20d_durability_trend_risk"
        if decision == "accepted"
        else "rejected_space_20d_durability_trend_risk"
    )
    rejection_reason = ""
    if decision == "rejected":
        rejection_reason = (
            "The 20d durable replacement trend state did not clear Gate 4 "
            "versus the accepted exp041 stack. Do not retry this exact 20d "
            "durability cohort without more mature closed forward rows."
        )
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Space official catalysts whose 20d closed event-state profile is "
            "still positive versus cash, same-theme replacement, SPY, QQQ, "
            "UFO, and ARKX may deserve an extra conservative trend allocation "
            "because durability should be a stronger continuation state than "
            "the accepted 10d benchmark-breadth signal."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_forward_replacement_20d_durability_trend_scalar",
        "single_causal_variable": (
            "extra trend_long risk scalar for the mature 20d durability profile; "
            "the accepted exp041 Space stack stays fixed"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "risk allocation: mature 20d durability across cash, "
                "same-theme, and broad benchmarks may identify Space trend "
                "continuation candidates worth incremental default-off risk."
            ),
            "q2_prior_experiments": [
                "exp-20260514-041 accepted 10d benchmark-breadth trend risk at 1.025x.",
                "exp-20260514-036 rejected fast 5d confirmation because it had no runtime signal coverage.",
                "exp-20260514-033 rejected a 10d benchmark-breadth delayed substitute with no changed runtime signals.",
                "No prior experiment tested a mature 20d durability state across same-theme and broad benchmarks.",
            ],
            "q3_single_causal_variable": (
                "Only the 20d durability trend scalar changes; no entry, exit, "
                "ranking, target, LLM/news, ticker pool, or live-slot change."
            ),
            "q4_acceptance_standard": (
                ...
                "changed 20d durability signals."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            "tested_scalars": list(TWENTY_DAY_DURABILITY_RISK_SCALARS),
            "selected_scalar": best["parameters"][
                "space_forward_replacement_20d_durability_trend_scalar"
            ],
            "twenty_day_durability_target_tickers": durability_gate["target_tickers"],
            "twenty_day_durability_thresholds": durability_gate["thresholds"],
            "locked_variables": [
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
            else "Needs more mature 20d closed Space forward rows or a different "
        ...
        "why_not_other_changes": (
            "LLM soft-ranking remains sample-limited, nearby 10d benchmark-breadth "
            "and delayed-absorption scalar retunes are anti-repeat families, and "
            "broad ticker expansion has repeatedly failed. This run tests a new "
            "closed-forward maturity horizon instead."
        ),
        "known_risks": [
            "Space remains default-off; this does not authorize live Space slots.",
            "The 20d mature sample may be too sparse for acceptance even if the profile is directionally interesting.",
        ],
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "target_tickers": result["parameters"][
            "twenty_day_durability_target_tickers"
        ],
        "aggregate_delta_vs_before": result["best_variant_gate"][
            "aggregate_delta_vs_before"
        ],
        "eligible_20d_durability_signal_count": result["best_variant_gate"][
            "eligible_20d_durability_signal_count"
        ],
        "changed_20d_durability_signal_count": result["best_variant_gate"][
            "changed_20d_durability_signal_count"
        ],
    }

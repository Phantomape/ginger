...
apply only to established Space TQS quality buckets. This is a scope test for
an accepted production-visible helper, not a scalar retune, candidate-pool
expansion, LLM ranking change, or live-slot change.
"""
...
STEM = "space_watch_liquidity_tqs_scope"
WATCH_LIQUIDITY_RISK_SCALAR = 1.10

# Uses the already-established Space TQS buckets from exp-20260512-004 and
# exp-20260512-008. No new numeric threshold family is introduced here.
TQS_SCOPE_VARIANTS = {
    "watch_liquidity_tqs_near_perfect_or_better": {
        "min_score": 0.95,
        "max_score": None,
    },
    "watch_liquidity_tqs_perfect_only": {
        "min_score": 1.0,
        "max_score": None,
    },
    "watch_liquidity_tqs_below_near_perfect": {
        "min_score": None,
        "max_score": 0.95,
    },
    "watch_liquidity_tqs_nonperfect": {
        "min_score": None,
        "max_score": 1.0,
    },
}
...
def _tqs_in_scope(score: Any, scope: dict[str, float | None]) -> bool:
    value = _as_float(score)
    if value is None:
        return False
    min_score = scope.get("min_score")
    max_score = scope.get("max_score")
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                ...
                score = signal.get("trade_quality_score")
                if _tqs_in_scope(score, tqs_scope):
                    _scale_sizing(
                        sizing,
                        WATCH_LIQUIDITY_RISK_SCALAR,
                        portfolio_value,
                        "space_watch_liquidity_tqs_scope",
                    )
                    ...
                    watch_adjustments.append(
                        {
                            ...
                            "marker": "space_watch_liquidity_tqs_scope",
                            "space_liquidity_tier": tiers.get(ticker),
                            "tqs_scope": deepcopy(tqs_scope),
                    ...
                    signal = {
                        **signal,
                        ...
                        "space_watch_liquidity_tqs_scope": deepcopy(tqs_scope),
                        "space_watch_liquidity_tqs_scope_eligible": True,
                    }
                else:
                    ...
                    watch_out_of_scope.append(
                        {
                            "ticker": ticker,
                            "strategy": signal.get("strategy"),
                            "marker": "space_watch_liquidity_tqs_scope_skipped",
                            "space_liquidity_tier": tiers.get(ticker),
                            "tqs_scope": deepcopy(tqs_scope),
    ...
    try:
        ...
        for label, window in WINDOWS.items():
            ...
            by_window[label] = {
                ...
                "space_watch_liquidity_tqs_scope_adjustment": _adjustment_summary(
                    watch_adjustments[before_watch:]
                ),
                "space_watch_liquidity_tqs_scope_skipped": _adjustment_summary(
                    watch_out_of_scope[before_skipped:]
                ),
                ...
                "space_watch_liquidity_tqs_scope_signal_counts": dict(
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_watch_liquidity_tqs_scope_adjustment"]["adjusted_signal_count"]
        for row in variant["by_window"].values()
    )
    skipped_count = sum(
        row["space_watch_liquidity_tqs_scope_skipped"]["adjusted_signal_count"]
        for row in variant["by_window"].values()
    )
    passed = (
    ...
    return {
        ...
        "space_watch_liquidity_tqs_scope_adjusted_signal_count": adjusted_count,
        "space_watch_liquidity_tqs_scope_skipped_signal_count": skipped_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space watch-liquidity TQS scope",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: TQS bucket scope for the accepted watch-liquidity "
            "1.10x top-up."
        ),
        ...
        "",
        (
            "| Variant | TQS scope | Gate | dEV | dPnL | Improved windows | "
    ...
    for name, variant in payload["variants"].items():
        ...
        scope = variant["space_watch_liquidity_tqs_scope"]
        lines.append(
            f"| {name} | min={scope.get('min_score')} max={scope.get('max_score')} | "
            ...
            f"{gate['space_watch_liquidity_tqs_scope_adjusted_signal_count']} | "
            f"{gate['space_watch_liquidity_tqs_scope_skipped_signal_count']} |"
        )
    lines.extend(
        [
            "",
    ...
    for label in WINDOWS:
        ...
        adjusted = best["by_window"][label][
            "space_watch_liquidity_tqs_scope_adjustment"
        ]["adjusted_signal_count"]
        skipped = best["by_window"][label][
            "space_watch_liquidity_tqs_scope_skipped"
        ]["adjusted_signal_count"]
        lines.append(
...
def run() -> dict[str, Any]:
    ...
    before = _run_accepted_exp112_variant(
        "accepted_exp112_watch_liquidity_all_tqs_buckets",
        WATCH_LIQUIDITY_RISK_SCALAR,
        liquidity_gate,
        company_release_gate,
        financing_gate,
    ...
    variants = {}
    for name, scope in TQS_SCOPE_VARIANTS.items():
    ...
    decision = (
        "accepted_default_off_space_watch_liquidity_tqs_scope"
        if accepted
        else "rejected_space_watch_liquidity_tqs_scope"
    )
    interpretation = (
        "Restricting the accepted watch-liquidity top-up by established TQS bucket "
        "improved the default-off Space stack under the three-window gate. Promotion "
        "must remain shared and metadata-only with live Space slots at zero."
        if accepted
        else (
            "Restricting the accepted watch-liquidity top-up by established TQS bucket "
            "did not clear the three-window gate versus exp-20260512-112. Keep the "
            "current all-TQS watch-liquidity helper and do not retry nearby TQS-scope "
            "splits on the same frozen snapshots without forward evidence."
        )
    )
    ...
    payload = {
        ...
        "changed_variable": "space_watch_liquidity_tqs_scope",
        "single_causal_variable": (
            "TQS bucket scope for the accepted 1.10x watch-liquidity risk top-up"
        ),
        "hypothesis": (
            "The accepted Space watch-liquidity scalar may be carried by particular "
            "signal-quality buckets. Since near-perfect/perfect TQS already define "
            "accepted Space quality ladders, scoping the existing watch-liquidity "
            "1.10x helper by those buckets may improve replacement value without "
            "changing candidate pool, scalar value, ranking, exits, LLM/news, or "
            "live slots."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: keep the accepted watch-liquidity scalar at 1.10x "
                "but test its established TQS bucket scope."
            ),
            "2_history_check": {
                "exp-20260512-112": (
                    "Accepted liquidity_tier=watch 1.10x across all TQS buckets; "
                    "this is the fixed before state."
                ),
                "exp-20260512-008": (
                    "Accepted near-perfect Space trend TQS bucket. This reuses that "
                    "bucket definition instead of creating an arbitrary threshold."
                ),
                "exp-20260512-004": (
                    "Accepted perfect Space TQS bucket. This reuses that bucket "
                    "definition for a helper-scope test."
                ),
                "exp-20260512-114_and_951": (
                    "Rejected strategy-scope splits for watch liquidity."
                ),
                "exp-20260513-003": (
                    "Rejected peer-state scope splits for watch liquidity."
                ),
            },
            "3_single_causal_variable": (
                "space_watch_liquidity_tqs_scope. Candidate pool, scalar value, "
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260513_010_space_watch_liquidity_tqs_scope.py"
            ),
        },
        "parameters": {
            ...
            "tested_tqs_scopes": deepcopy(TQS_SCOPE_VARIANTS),
        ...
        "gate1": {
            ...
            "known_bias": (
                ...
                "from a 2026-05-10 research universe. TQS and liquidity-tier metadata "
                "are production-observable in the shared Space sleeve, but any accepted "
                "change must remain default-off until forward evidence matures."
            ),
        },
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "signal.trade_quality_score",
                "data/universe_registry.json liquidity_tier",
            ],
            "passed": (
                gate2_open["passed"]
        ...
        "gate3": {
            ...
            "tqs_scope_refinement_added": True,
        ...
        "next_evidence_needed": (
            "If rejected, do not retry nearby TQS-scope splits for the accepted "
        ...
        "related_files": [
            "quant/experiments/exp_20260513_010_space_watch_liquidity_tqs_scope.py",
            "data/experiments/exp-20260513-010/space_watch_liquidity_tqs_scope.json",
            "docs/experiments/logs/exp-20260513-010.json",
            "docs/experiments/tickets/exp-20260513-010.json",
            "docs/experiments/artifacts/exp-20260513-010_space_watch_liquidity_tqs_scope.md",
        ...
        "why_not_other_changes": (
            ...
            "contract-profile scalars, generic risk caps, strategy-scope splits, "
            "and peer-scope splits are already rejected. This tests one production-"
            "visible TQS-scope variable for an accepted helper."
        ),
    }
    return payload

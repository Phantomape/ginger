"""exp-20260528-021: Companyfacts+RS liquidity sweet-spot support.

Alpha search follow-up to the accepted Companyfacts operating-profit + RS
paper lead, closed-ledger governor, low-volume support, filing-recency support,
and low-liability support. This keeps the accepted candidate source, daily
top-1 selection, 10-day hold, and all prior paper-notional supports fixed. It
changes one production-visible free OHLCV variable: support selected paper
candidates whose 20-day average dollar volume sits in an institutional
liquidity sweet spot rather than the mega-cap tail.
...
EXPERIMENT_ID = "exp-20260528-021"
STEM = "fundamental_growth_rs_liquidity_sweet_spot"
TRIAL_FAMILY = "fundamental_growth_rs_liquidity_sweet_spot_support"
CHANGED_VARIABLE = "fundamental_growth_rs_liquidity_sweet_spot_notional_scalar_v1"
RULE_VERSION = "fundamental_growth_rs_liquidity_sweet_spot_support_v1"

LIQUIDITY_SWEET_SPOT_MIN_AVG_DOLLAR_VOLUME_20 = 250_000_000.0
LIQUIDITY_SWEET_SPOT_MAX_AVG_DOLLAR_VOLUME_20 = 10_000_000_000.0
...
LIQUIDITY_AUDIT: "OrderedDict[str, dict[str, Any]]" = OrderedDict()
...
def _configure_modules() -> None:
    ...
    LIQUIDITY_AUDIT.clear()
    prev._configure_modules()
    _base()._select_paper_trades = _select_liquidity_supported_paper_trades
...
def _liquidity_sweet_spot_pass(avg_dollar_volume_20: Any) -> bool:
    value = _float(avg_dollar_volume_20)
    return (
        value is not None
        and value >= LIQUIDITY_SWEET_SPOT_MIN_AVG_DOLLAR_VOLUME_20
        and value < LIQUIDITY_SWEET_SPOT_MAX_AVG_DOLLAR_VOLUME_20
    )


def _liquidity_sweet_spot_scalar(avg_dollar_volume_20: Any) -> float:
    if _liquidity_sweet_spot_pass(avg_dollar_volume_20):
        return LIQUIDITY_SWEET_SPOT_NOTIONAL_SCALAR
    return 1.0


def _liquidity_bucket(avg_dollar_volume_20: Any) -> str:
    value = _float(avg_dollar_volume_20)
    if value is None:
        return "missing"
    if value < LIQUIDITY_SWEET_SPOT_MIN_AVG_DOLLAR_VOLUME_20:
        return "below_250m"
    if value < LIQUIDITY_SWEET_SPOT_MAX_AVG_DOLLAR_VOLUME_20:
        return "sweet_spot_250m_to_10b"
    return "mega_gte_10b"


def _select_liquidity_supported_paper_trades(
    ...
    for row in candidates:
        ...
        avg_dollar_volume = _float(row.get("avg_dollar_volume_20"))
        liquidity_scalar = _liquidity_sweet_spot_scalar(avg_dollar_volume)
        notional_scalar_before_liquidity = (
            ticker_profit_scalar
            * global_drawdown_scalar
            * low_volume_scalar
        ...
        notional_scalar = notional_scalar_before_liquidity * liquidity_scalar
        ...
        liquidity_bucket = _liquidity_bucket(avg_dollar_volume)
        bucket_counts[liquidity_bucket] += 1
        ...
        if liquidity_scalar > 1.0:
            audit["liquidity_sweet_spot_supported"] += 1
        ...
        pre_liquidity_notional = base_notional * notional_scalar_before_liquidity
        supported_notional = base_notional * notional_scalar
        pre_liquidity_pnl = pre_liquidity_notional * pnl_pct_net
        supported_pnl = supported_notional * pnl_pct_net
        if liquidity_scalar > 1.0:
            support_pnl_delta_by_ticker[ticker] += supported_pnl - pre_liquidity_pnl
        ...
        supported_trade = {
            ...
            **balance_context,
            "liquidity_sweet_spot_rule_version": RULE_VERSION,
            "liquidity_sweet_spot_known_at": "daily OHLCV 20-day average dollar volume with date <= signal_date",
            "liquidity_sweet_spot_trade_enabled": False,
            "liquidity_sweet_spot_alters_orders": False,
            "liquidity_sweet_spot_min_avg_dollar_volume_20": LIQUIDITY_SWEET_SPOT_MIN_AVG_DOLLAR_VOLUME_20,
            "liquidity_sweet_spot_max_avg_dollar_volume_20": LIQUIDITY_SWEET_SPOT_MAX_AVG_DOLLAR_VOLUME_20,
            "avg_dollar_volume_20": _round(avg_dollar_volume, 2),
            "liquidity_sweet_spot_bucket": liquidity_bucket,
            "liquidity_sweet_spot_pass_v1": liquidity_scalar > 1.0,
            "liquidity_sweet_spot_notional_scalar": liquidity_scalar,
            ...
            "pnl_without_liquidity_sweet_spot_support": _round(pre_liquidity_pnl, 2),
            "liquidity_sweet_spot_support_pnl_delta": _round(supported_pnl - pre_liquidity_pnl, 2),
    ...
    LIQUIDITY_AUDIT[label] = {
        "rule_version": RULE_VERSION,
        "liquidity_sweet_spot_min_avg_dollar_volume_20": LIQUIDITY_SWEET_SPOT_MIN_AVG_DOLLAR_VOLUME_20,
        "liquidity_sweet_spot_max_avg_dollar_volume_20": LIQUIDITY_SWEET_SPOT_MAX_AVG_DOLLAR_VOLUME_20,
        "liquidity_sweet_spot_notional_scalar": LIQUIDITY_SWEET_SPOT_NOTIONAL_SCALAR,
        "input_candidates": len(candidates),
        "selected_trades": len(selected),
        "filtered_candidates": len(filtered),
        "liquidity_sweet_spot_supported": int(audit.get("liquidity_sweet_spot_supported", 0)),
        ...
        "liquidity_bucket_counts": dict(sorted(bucket_counts.items())),
        "liquidity_sweet_spot_supported_ticker_counts": dict(sorted(supported_ticker_counts.items())),
...
def _update_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = (
        "accepted_candidate_fundamental_growth_rs_liquidity_sweet_spot_support"
        if gate4_passed
        else "rejected_fundamental_growth_rs_liquidity_sweet_spot_support"
    )
    payload["status"] = decision
    payload["decision"] = decision
    payload["hypothesis"] = (
        "candidate_pool / capital allocation: within the accepted Companyfacts "
        "operating-profit + RS paper source, selected candidates with 20-day "
        "average dollar volume between $250M and $10B may offer better "
        "cost-adjusted replacement value than the ultra-low-liquidity and "
        "mega-cap tails. This uses free OHLCV already visible at signal time "
        "and applies only a mild default-off paper notional support."
    )
    payload["change_type"] = "fundamental_growth_rs_liquidity_sweet_spot_default_off_paper_sleeve"
    payload["mechanism_family"] = "free_ohlcv_cost_liquidity_candidate_pool"
    ...
    payload["new_evidence_type"] = "production_visible_free_ohlcv_cost_liquidity_state"
    payload["parameters"]["liquidity_sweet_spot_support"] = {
        "min_avg_dollar_volume_20": LIQUIDITY_SWEET_SPOT_MIN_AVG_DOLLAR_VOLUME_20,
        "max_avg_dollar_volume_20": LIQUIDITY_SWEET_SPOT_MAX_AVG_DOLLAR_VOLUME_20,
        "paper_notional_scalar": LIQUIDITY_SWEET_SPOT_NOTIONAL_SCALAR,
        "applies_to": (
        ...
        "production_visible_field": (
            "avg_dollar_volume_20 computed from prior 20 OHLCV rows with dates <= signal_date"
        ),
    }
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "candidate_pool / capital allocation alpha: absolute 20-day average "
            "dollar volume can separate a liquid-but-not-mega-cap sweet spot for "
            "the accepted Companyfacts+RS paper pool."
        ),
        "2_history_check": {
            "exp-20260528-015": (
                "Accepted relative signal-day low-volume participation support; "
                "this run does not retune that threshold and instead uses absolute "
                "20-day dollar liquidity as a cost/capacity state."
            ),
            "exp-20260528-017": "Accepted low-liability support on the current governed Companyfacts+RS baseline.",
            "exp-20260528-019": "Working-capital support rejected because selected trades had zero receivables/inventory coverage.",
            "exp-20260528-020": "Dual-growth support rejected versus the current exp017 accepted stack.",
            "exp-20260527-905": (
                "VCP cost/liquidity scalar was rejected in a different sleeve; "
                "this run tests the accepted Companyfacts+RS paper pool."
            ),
            "current_difference": (
                "This run does not add a hard filter or retune Companyfacts growth, "
                "RS, top-N, hold, low-volume, filing-age, liabilities/assets, or "
                "governor thresholds; it tests one OHLCV liquidity support scalar."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260528_021_fundamental_growth_rs_liquidity_sweet_spot.py"
        ),
    }
    payload["gate2"]["runtime_fields"] = [
        ...
        "avg_dollar_volume_20 computed from OHLCV rows dated <= signal_date",
    ]
    payload["gate2"]["note"] = (
        "The only new field is 20-day average dollar volume, using existing "
        "date-safe OHLCV already required by the Companyfacts+RS paper sleeve. "
    ...
    payload["liquidity_sweet_spot_audit"] = LIQUIDITY_AUDIT
    ...
    payload["why_not_other_changes"] = (
        ...
        "This run tests only a free OHLCV cost/liquidity support scalar."
    )
    payload["interpretation"] = (
        "The liquidity sweet-spot support cleared Gate 4 on top of the "
        ...
        if gate4_passed
            ...
            "The liquidity sweet-spot support did not clear Gate 4. Do not promote it "
            "or retry nearby liquidity thresholds on the frozen windows without forward "
            "rows or materially new evidence."
        )
    )
    ...
    payload["production_impact"].update(
        {
            ...
            "promotion_requirement": (
                ...
                if gate4_passed
                    ...
                    "A retained positive result would require the same liquidity "
...
def _build_report(payload: dict[str, Any]) -> str:
    lines = [
        ...
        (
            ...
            "20-day average dollar volume is >= $250M and < $10B."
        ),
        "",
        "## Three-Window Result",
        "",
    ...
    for label in _base().WINDOWS:
        ...
        audit = payload["liquidity_sweet_spot_audit"].get(label, {})
        lines.append(
            ...
            "${apnl:,.2f} | ${dpnl:+,.2f} | {dd:+.4f} | {trades} | {supported} |".format(
                ...
                supported=audit.get("liquidity_sweet_spot_supported"),
            )
        )
    ...
    lines.extend(
        [
            ...
            "",
            "```json",
            json.dumps(payload["liquidity_sweet_spot_audit"], indent=2, sort_keys=True),
            "```",
            "",
...
def _persist(payload: dict[str, Any]) -> None:
    ticket = {
        ...
        "title": "Fundamental growth + RS liquidity sweet-spot support",
            ...
            _base()._safe(
                {
                    ...
                    "gate4": payload["gate4"],
                    "liquidity_sweet_spot_audit": payload["liquidity_sweet_spot_audit"],

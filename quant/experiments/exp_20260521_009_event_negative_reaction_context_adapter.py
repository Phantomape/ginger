"""exp-20260521-009: promote event negative-reaction context adapter.

Alpha search. Tests and promotes a single event context allocation variable on
top of the accepted exp-20260521-006 default-off event overlay adapter: whether
already-selected rows with negative first-reaction buckets deserve extra paper
notional. The shared adapter remains default-off for live/default orders.
...
import exp_20260521_007_event_negative_phrase_evidence as base_scout


EXPERIMENT_ID = "exp-20260521-009"
EXPERIMENT_SLUG = "event_negative_reaction_context_adapter"

REPO_ROOT = base_scout.REPO_ROOT
    ...
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "negative_reaction_context_scalar": 1.0,
            },
        ),
        (
            "negative_reaction_context_125",
            {
                "description": (
                    "Multiply accepted event paper notional by 1.25x when "
                    "reaction_bucket is a negative first-reaction bucket."
                ),
                "negative_reaction_context_scalar": 1.25,
            },
        ),
        (
            "negative_reaction_context_150",
            {
                "description": (
                    "Multiply accepted event paper notional by 1.50x when "
                    "reaction_bucket is a negative first-reaction bucket."
                ),
                "negative_reaction_context_scalar": 1.50,
            },
        ),
        (
            "negative_reaction_context_200",
            {
                "description": (
                    "Multiply accepted event paper notional by 2.00x when "
                    "reaction_bucket is a negative first-reaction bucket."
                ),
                "negative_reaction_context_scalar": 2.00,
            },
        ),
    ]
)
...
def _accepted_event_scalar_after_exp006(trade: dict[str, Any]) -> float:
    return base_scout._accepted_event_scalar_after_exp006(trade)


def _is_target_negative_reaction(trade: dict[str, Any]) -> bool:
    return str(trade.get("reaction_bucket") or "") in TARGET_REACTION_BUCKETS
...
def _scaled_trade(
    ...
    target = _is_target_negative_reaction(trade)
    reaction_scalar = (
        float(variant["negative_reaction_context_scalar"]) if target else 1.0
    )
    ...
    return {
        **trade,
        ...
        "negative_reaction_context_target": target,
        "negative_reaction_context_scalar": round(reaction_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("negative_reaction_context_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("negative_reaction_context_target")
        ]
    ...
    return {
        "target_reaction_buckets": list(TARGET_REACTION_BUCKETS),
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -VARIANTS[name]["negative_reaction_context_scalar"],
        ),
    )
...
def _adapter_contract() -> dict[str, Any]:
    ...
    checks = {
        "negative_source_broad_reaction_stacks_to_5x": (
            neg.get("broad_breadth_tilt") is True
            and neg.get("negative_reaction_tilt") is True
            and neg.get("state_surface_scalar") == 2.5
            and neg.get("scalar") == 5.0
            and by_ticker["NNEG"].get("paper_event_notional_usd") == 50_000.0
        ),
        "governance_generic_negative_reaction_stacks_to_4x": (
            gov_neg.get("source_quality_tilt") is True
            and gov_neg.get("negative_reaction_tilt") is True
            and gov_neg.get("state_surface_scalar") == 1.0
            and gov_neg.get("scalar") == 4.0
            and by_ticker["GNEG"].get("paper_event_notional_usd") == 40_000.0
        ),
        "positive_reaction_does_not_get_reaction_tilt": (
            gov_pos.get("source_quality_tilt") is True
            and gov_pos.get("negative_reaction_tilt") is False
            and gov_pos.get("scalar") == 2.0
        ),
        ...
        "summary_counts_negative_reaction": (
            summary.get("negative_reaction_tilt_candidate_count") == 2
            and summary.get("negative_reaction_tilt_incremental_notional_usd")
            == 45_000.0
            and summary.get("source_quality_tilt_candidate_count") == 2
        ),
    }
    return {
...
def build_payload() -> dict[str, Any]:
    ...
    accepted = bool(best_gate["passed"] and adapter_contract["passed"])
    decision = (
        "accepted_default_off_event_negative_reaction_context_adapter"
        if accepted
        else "rejected_event_negative_reaction_context_adapter"
    )
    ...
    if not accepted:
        rejection_reason = (
            ...
            f"adapter_contract_passed={adapter_contract['passed']}."
        )
    ...
    return {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "event_context_allocation_adapter",
        "mechanism_family": "external_event_satellite_overlay_allocation",
        "trial_family": "event_reaction_context_quality",
        "trial_variant_id": "negative_first_reaction_notional",
        "changed_variable": "event_negative_reaction_paper_notional_scalar",
        "prior_trial_count": 7,
        ...
        "new_evidence_type": "new_event_reaction_context_bucket",
        "hypothesis": (
            "Inside the accepted default-off event overlay, already-selected "
            "rows with negative first-reaction buckets carry stronger "
            "overreaction/replacement value than neutral or positive first "
            "reactions, so they deserve extra paper notional while live/default "
        ...
        "alpha_hypothesis": {
            ...
            "playbook_alignment": (
                "Stays in the event replacement-value lane, uses a production-"
                "visible context field, and avoids LLM soft-ranking, broad-market "
                "feed blockers, source-capacity retries, and state-surface retunes."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for event overlay rows whose reaction_bucket "
            "is one of the fixed negative first-reaction buckets"
        ),
        "parameters": {
            "variants": VARIANTS,
            ...
            "target_reaction_buckets": list(TARGET_REACTION_BUCKETS),
            "selected_negative_reaction_context_scalar": VARIANTS[best_variant][
                "negative_reaction_context_scalar"
            ],
            ...
            "locked_variables": [
                ...
                "dispersion bucket scalars",
                "SEC negative phrase-hit scalar",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Selected event rows with negative first reaction may carry "
                "overreaction/replacement value; this is event context scoring "
                "plus capital allocation."
            ),
            "2_history_check": (
                "Governance-source was accepted in exp-20260521-006. SEC negative "
                "phrase evidence failed in exp-20260521-007. Mid-dispersion failed "
                "in exp-20260521-008. This tests a broader reaction-bucket cohort "
                "with stricter sample guards."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for the fixed negative-reaction "
                "cohort changes."
            ),
            "4_acceptance_standard": (
                ...
                "guard pass, and shared default-off adapter parity."
            ),
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\"
                "exp_20260521_009_event_negative_reaction_context_adapter.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260516-030": "Rejected simple source-specific SEC negative-reaction tilt.",
            ...
            "exp-20260521-008": "Rejected mid-dispersion context due thin touched cohort.",
        },
        "backtest_protocol": {
            "source": (
        ...
        "gate4": {
            **best_gate,
            "adapter_contract_passed": adapter_contract["passed"],
            "passed": bool(best_gate["passed"] and adapter_contract["passed"]),
            "basis": (
        ...
        "llm_metrics": {
            ...
            "blocker_relation": (
                ...
                "deterministic event reaction context only."
            ),
        },
        "decision_rationale": (
            "Accepted as a shared default-off paper adapter change."
            if accepted
            else "Rejected. The best variant did not clear all Gate 4 and adapter-contract checks."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "Keep live/default capital disabled. Monitor forward closed event "
            "outcomes before any trade-enabled event bundle promotion."
        ),
        "why_not_other_attractive_points": (
            "Skipped LLM soft-ranking due attribution limits; skipped broad-market "
            "forward maturation because its candidate universe feed is blocked; "
            "skipped same-family source/dispersion/phrase scalars after recent "
            "failed nearby experiments."
        ),
        "risk_of_change": (
            "No live orders, ranking, core sizing, exits, or source capacity "
            "changed. Risk is paper attribution overfit until forward outcomes mature."
        ),
        "related_files": [
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Negative-Reaction Context Adapter",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests and promotes a shared default-off paper "
            "notional scalar for selected event rows in negative first-reaction "
            "buckets."
        ),
        "",
        "## Gate 4 Result",
...
def _concise_selection(selection: dict[str, Any]) -> dict[str, Any]:
    return {
        "target_reaction_buckets": selection["target_reaction_buckets"],
...
def _compact_log(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    compact_parameters = {
        ...
        "target_reaction_buckets": payload["parameters"]["target_reaction_buckets"],
        "selected_negative_reaction_context_scalar": payload["parameters"][
            "selected_negative_reaction_context_scalar"
        ],
        "variant_scalars": {
            name: row["negative_reaction_context_scalar"]
    ...
    return {
        ...
        "gate4": {
            ...
            "adapter_contract_passed": payload["gate4"]["adapter_contract_passed"],
        },
        "before_metrics": {
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event negative-reaction context adapter",
            "status": payload["status"],
            "decision": payload["decision"],

"""exp-20260609-027: shared turn-of-month liquid leadership adapter.

This alpha experiment promotes the positive exp-20260609-026 replay lead into
quant/turn_of_month_liquid_leadership_paper_sleeve.py. Historical replay and
...
from turn_of_month_liquid_leadership_paper_sleeve import (  # noqa: E402
    DEFAULT_CONFIG,
    RULE_VERSION,
    SOURCE_RULE_VERSION,
...
STEM = "turn_of_month_liquid_leadership_shared_adapter"
TRIAL_FAMILY = "turn_of_month_liquid_leadership_shared_default_off_adapter"
TRIAL_VARIANT_ID = RULE_VERSION
CHANGED_VARIABLE = "shared_turn_of_month_liquid_leadership_default_off_adapter_v1"
SOURCE_LEAD_EXPERIMENT_ID = "exp-20260609-026"
OWNER = "alpha-search-automation"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260609_027_{STEM}.json"
...
SOURCE_LEAD_JSON = (
    ...
    / "exp_20260609_026_turn_of_month_liquid_leadership.json"
)
...
PREDICTION = {
    ...
    "expected_ev_delta": 0.2774,
    "expected_pnl_delta": 5287.69,
    "main_failure_modes": [
        "shared_helper_drift",
        "window_regression",
    ...
    "confidence_reason": (
        "The fixed exp-20260609-026 bundle already improved all three canonical "
        "windows and beat the accepted compression comparator; the main risk is "
        "implementation drift when moving from private runner logic to a shared "
        "default-off helper."
    ),
    "recorded_at": "2026-06-09T23:04:43+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "daily_snapshot_note": (
        "quant/turn_of_month_liquid_leadership_paper_sleeve.py exposes the "
    ...
    "calendar_parity_note": (
        "Historical replay passes the full loaded trading calendar into the "
        "shared helper. Daily snapshots must not infer last-trading-day labels "
        "from truncated OHLCV; month-end observation requires explicit "
        "calendar_dates or known_month_end_dates and otherwise fails closed."
    ),
    ...
    "activation_envelope": {
        ...
        "liquidity_slippage_model": (
            "historical replay uses price >= $10, ADV20 >= $75M, next-open "
    ...
    "parity_note": (
        ...
        "quant/turn_of_month_liquid_leadership_paper_sleeve.py. The helper is "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool/shared_adapter: turn-of-month institutional "
        "contribution and rebalance flow should make liquid sector-known stocks "
        "that already show 20-day SPY-relative leadership and a high signal-day "
        "close continue after next-open entry. exp026 showed this as a positive "
        "free calendar/OHLCV replay lead; exp027 only tests whether the fixed "
        "policy bundle survives shared default-off historical/daily semantics."
    ),
    "2_history_check": {
        "exp-20260609-026": (
            "Positive replay lead: aggregate EV +0.2774, PnL +$5,287.69, "
            "73 target trades, all three canonical windows positive, and "
            "accepted compression comparator beaten. It was not promoted "
            "because no shared helper/daily snapshot/parity path existed."
        ),
        "exp-20260608-013": (
            "Accepted narrow-range compression shared adapter: aggregate EV "
            "+0.1608 and PnL +$2,248.98. This run must reproduce exp026, "
            "therefore retaining the stronger comparator outcome."
        ),
        "exp-20260607-019": (
            "Accepted volatility relief leadership shared adapter; useful "
            "neighbor showing free OHLCV leadership can be default-off "
            "production-visible when replay and daily semantics share a helper."
        ),
        "exp-20260606-020": (
            "Accepted macro relief leadership shared adapter; nearby evidence "
            "that event/calendar timing can work when not retuned on frozen windows."
        ),
    },
    ...
    "4_acceptance_standard": (
        ...
        "passes, accepted compression comparator remains beaten, and the shared "
        "helper must reproduce exp026 within drift tolerance."
    ),
    "5_reproducibility": (
        ".venv\\Scripts\\python.exe -B quant\\experiments\\"
        "exp_20260609_027_turn_of_month_liquid_leadership_shared_adapter.py"
    ),
}
...
def _lead_reproduction_check(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for label in framework.WINDOWS:
        ...
        by_window[label] = {
            ...
            "source_target_trade_count": len(((lead.get("target_trades_by_window") or {}).get(label)) or []),
        }
    passed = (
...
def _gate4(
    *,
    ...
    if float(aggregate["expected_value_score_delta_sum"] or 0.0) <= ACCEPTED_COMPRESSION_COMPARATOR[
        "expected_value_score_delta_sum"
    ]:
        failed.append("accepted_compression_ev_not_beaten")
    if float(aggregate["total_pnl_delta_sum"] or 0.0) <= ACCEPTED_COMPRESSION_COMPARATOR[
        "total_pnl_delta_sum"
    ]:
    ...
    return {
        ...
        "decision": (
            ...
            if passed
            else "rejected_turn_of_month_liquid_leadership_shared_default_off_adapter"
        ),
        ...
        "parity_test_added": True,
        "shared_adapter_module": "quant/turn_of_month_liquid_leadership_paper_sleeve.py",
    }
...
def build_payload() -> dict[str, Any]:
    ...
    for label, cfg in framework.WINDOWS.items():
        print(f"[{label}] shared turn-of-month liquid leadership adapter replay")
        ...
        warehouse_coverage_by_window[label] = {
            ...
            "calendar_dates_passed_to_shared_helper": True,
        }
        window_rows[label] = {
            "before": before,
            "after": after,
    ...
    payload: dict[str, Any] = {
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                "Signal uses only free signal-date OHLCV and a deterministic "
                "trading calendar: last trading day through first three trading "
                "days, SPY-relative leadership, close/volume/volatility guards, "
                "same-ticker selected-core overlap exclusion, top-1/day, "
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "SPY daily OHLCV",
                "complete loaded trading calendar for historical month-end labels",
                "explicit calendar_dates or known_month_end_dates for daily month-end labels",
    ...
    payload["interpretation"] = (
        "The positive turn-of-month liquid leadership lead reproduced through "
        "a shared default-off helper and daily snapshot API."
        if gate4["passed"]
        else (
            "The turn-of-month liquid leadership lead failed shared-helper "
    ...
    payload["post_run_reflection"] = {
        "why_result_happened": (
            ...
            "kept the exact calendar window, liquid sector-known universe, "
            "SPY-relative leadership, close/volume/volatility guards, "
            "same-ticker selected-core overlap exclusion, next-open entry, "
            "10-day exit, cost, top-1, and cooldown semantics while adding "
            "daily pending/open/closed state handling with month-end fail-closed "
            "calendar parity."
            if gate4["passed"]
            else (
        ...
        "forbidden_near_neighbor_retry": (
            "Do not sweep month-start/end day counts, ret20/ret60 leadership "
            "thresholds, close-location, volume bounds, volatility bounds, "
            "top-N, hold-day, cooldown, or paper notional on the frozen windows."
        ),
        "new_evidence_required": (
            "Next useful evidence is closed forward replacement-value rows "
            "from the shared default-off ledger or a genuinely new point-in-time "
            "flow beneficiary field, not another threshold retune."
        ),
    }
    payload["next_retry_requires"] = [
        "closed forward replacement-value rows",
        "point-in-time flow beneficiary evidence",
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Turn-of-Month Liquid Leadership Shared Adapter",
            "",
            ...
            *_window_table(payload),
            ...
            (
                ...
                "exits, LLM/news, and watchlists are unchanged. Daily month-end "
                "observation fails closed without explicit calendar support."
            ),
            "",
            "## Reflection",
            "",
...
def _build_artifact(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            (
                "Last trading day through first three trading days, liquid "
                "sector-known stock universe, 20-day SPY-relative leadership, "
                "positive 60-day excess trend, signal-day return, high close "
                "location, volume and volatility guards, same-ticker selected "
                "core overlap exclusion, top-1/day, fixed $4,000 paper notional, "
                "next-open entry, 10-trading-day close exit, slippage, round-trip "
                "cost, and same-ticker cooldown."
            ),
            "",
            ...
            *_window_table(payload),
            ...
            "",
            PRODUCTION_IMPACT["calendar_parity_note"],
            "",
            "## Reflection",
            "",
            payload["post_run_reflection"]["why_result_happened"],
...
def _write_manifest(payload: dict[str, Any]) -> None:
    paths = [
        Path(__file__),
        REPO_ROOT / "quant" / "turn_of_month_liquid_leadership_paper_sleeve.py",
        REPO_ROOT / "quant" / "test_turn_of_month_liquid_leadership_paper_sleeve.py",

"""exp-20260514-012: non-Financials sector-relative leader core risk.

Tests one production-visible allocation variable on the accepted core stack:
already-qualified non-ETF/non-commodity/non-Financials stock signals whose
20-day return is above the equal-weight same-sector 20-day return may deserve a
...
EXPERIMENT_ID = "exp-20260514-012"
EXPERIMENT_SLUG = "core_nonfinancial_sector_relative_leader_risk"
MULTIPLIER_KEY = "core_nonfinancial_sector_relative_leader_risk_multiplier_applied"
RISK_MULTIPLIER_SWEEP = [1.025, 1.05, 1.10]
EXCLUDED_SECTORS = {"ETF", "Commodities", "Financials", "Unknown"}
...
def _sector_ret20_averages(
    ...
    for ticker, features in (features_dict or {}).items():
        if not features:
            continue
        sector = base.risk_engine.SECTOR_MAP.get(str(ticker).upper(), "Unknown")
        if sector in EXCLUDED_SECTORS:
            continue
        ret20 = _as_float(features.get("momentum_20d_pct"))
        if ret20 is not None:
            values_by_sector.setdefault(sector, []).append(ret20)
    ...
    for sector, values in values_by_sector.items():
        if len(values) < 2:
            continue
        out[sector] = {
            "sector_avg_ret20_pct": sum(values) / len(values),
            "sector_ret20_member_count": len(values),
        }
    return out
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        sector_state = _sector_ret20_averages(features_dict)
        for sig in enriched:
            ticker = str(sig.get("ticker") or "").upper()
            sector = str(sig.get("sector") or "Unknown")
            ticker_ret20 = _as_float((features_dict.get(ticker) or {}).get("momentum_20d_pct"))
            state = sector_state.get(sector) or {}
            sector_avg = _as_float(state.get("sector_avg_ret20_pct"))
            rel20 = (
            ...
            sig["sector_avg_ret20_pct"] = (
                round(sector_avg, 6) if sector_avg is not None else None
            )
            sig["sector_ret20_member_count"] = state.get("sector_ret20_member_count")
            sig["ticker_ret20_minus_sector_avg_pct"] = (
                round(rel20, 6) if rel20 is not None else None
            )
            sig["core_nonfinancial_sector_relative_leader_state"] = bool(eligible)
        return enriched
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["core_nonfinancial_sector_relative_leader_baseline_shares"] = shares
    out["core_nonfinancial_sector_relative_leader_desired_shares"] = desired_shares
    out["core_nonfinancial_sector_relative_leader_cap_shares"] = cap_shares
    out["core_nonfinancial_sector_relative_leader_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if (
                sig.get("core_nonfinancial_sector_relative_leader_state")
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "sector_ret20_member_count": sig.get(
                                "sector_ret20_member_count"
                            ),
                            "ticker_ret20_minus_sector_avg_pct": sig.get(
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Non-Financials Sector-Relative Leader Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_core_nonfinancial_sector_relative_leader_risk"
    )
    interpretation = (
        "Non-Financials sector-relative leadership cleared the canonical three-window gate and should be promoted only through shared production/backtest policy."
        if passed
        else "Non-Financials sector-relative 20d leadership did not clear Gate 4 as a standalone core sizing top-up."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted Financials sector-leader result may reflect a broader "
            "mechanism: already-qualified stock signals that lead their own "
            "sector over 20 trading days may deserve a small allocation top-up. "
            "Exclude Financials so this is not a nearby retry of the accepted "
            "Financials multiplier."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "core_nonfinancial_sector_relative_leader_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for non-Financials stock signals whose 20d return is above same-sector average"
        ),
        "parameters": {
            "state_definition": {
                "condition": "ticker momentum_20d_pct > equal-weight same-sector momentum_20d_pct",
                "strategies": ["trend_long", "breakout_long"],
                "excluded_sectors": sorted(EXCLUDED_SECTORS),
                "minimum_sector_member_count": 2,
            },
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital/risk allocation: same-sector 20d leadership may identify higher-quality already-qualified non-Financials core signals."
            ),
            "2_history_check": {
                "exp-20260501-006": (
                    "Financials sector-leader sizing was accepted; this excludes Financials to avoid a nearby retry and tests whether the mechanism generalizes."
                ),
                "exp-20260430-023": (
                    "Trend-Tech sector-leader de-risk was a narrow Technology haircut; this tests a small cross-sector top-up on accepted core signals."
                ),
                "exp-20260501-007": (
                    "Synchronous sector risk-on allocation was a broad sector-state risk-on rule; this uses candidate-vs-sector leadership instead."
                ),
                "exp-20260426-049/exp-20260428-014": (
                    "Sector-relative shadow universe work was observed-only and entry/universe oriented; this keeps the production candidate set fixed."
                ),
                "llm_soft_ranking": (
            ...
            "3_single_causal_variable": (
                "core_nonfinancial_sector_relative_leader_risk_multiplier with fixed ticker-vs-sector 20d leadership definition."
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_012_core_nonfinancial_sector_relative_leader_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, implement same-sector relative-leader fields and sizing top-up in shared risk/portfolio modules called by both run.py and backtester.py."
            ),
        },
        ...
        if passed
            ...
            "Use forward candidate-vs-sector attribution or a narrower production-visible sector-relative discriminator before retrying this top-up family."
        ),
        "related_files": [

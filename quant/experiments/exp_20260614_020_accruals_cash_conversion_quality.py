"""exp-20260614-020: accruals / cash-conversion earnings-quality candidate pool.

Replay-only alpha search. The single decision hypothesis is the Sloan (1996)
accrual anomaly applied as a free PIT SEC-Companyfacts candidate source:
among the production universe, candidates whose most recent ANNUAL (10-K)
earnings are backed by operating cash flow (low accruals = NetIncome minus
OperatingCashFlow small or negative relative to total assets, equivalently a
high cash-conversion ratio) and that also show same-day liquid SPY-relative
price confirmation may be higher-quality, less-reversal-prone demand than
generic momentum. The candidate enters paper at the next open with a fixed
10-trading-day hold.
...
STEM = "accruals_cash_conversion_quality"
TRIAL_FAMILY = "accruals_cash_conversion_quality_candidate_pool"
TRIAL_VARIANT_ID = "companyfacts_annual_accruals_cash_conversion_top1_next_open_10d_v1"
CHANGED_VARIABLE = "accruals_cash_conversion_quality_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE
OWNER = "alpha-search-automation"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260614_020_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "The accruals anomaly (Sloan 1996) is a robust, economically distinct "
        "earnings-quality effect; net_income and operating_cash_flow exist in "
        "the PIT companyfacts archive but the accepted fundamental index never "
        "loads them, so this is genuinely untested free data. Main disconfirmer: "
        "in a momentum-led liquid universe the cash-conversion gate may merely "
        "re-select existing winners and fail to beat the accepted candidate-pool "
        "comparators after costs."
    ),
    "recorded_at": "2026-06-14T16:15:15+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "annual accruals / cash-conversion quality gate, the same liquid "
        "SPY-relative confirmation, core-overlap context, cooldown, next-open "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: production-universe names whose most recent annual "
        "earnings are backed by operating cash flow (low accruals / high "
        "cash-conversion) and that show liquid SPY-relative price confirmation "
        "are higher-quality, less-reversal-prone demand."
    ),
    "2_history_check": {
        ...
        "exp-20260614-007": (
            "Rejected SEC earnings-catalyst peer underreaction; this run is a "
            "direct single-name fundamental-quality source, not a peer relation."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260614_020_accruals_cash_conversion_quality.py"
    ),
}
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_accruals_cash_conversion_quality"
        if gate["passed"]
        else "rejected_accruals_cash_conversion_quality_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Production-universe names whose most recent annual earnings are "
            "backed by operating cash flow (low accruals / high cash-conversion) "
            "and that show liquid SPY-relative price confirmation may be "
            "higher-quality, less-reversal-prone demand than generic momentum."
        ),
        ...
        "new_evidence_type": "free_sec_companyfacts_accruals_cash_conversion_plus_ohlcv",
        ...
        "interpretation": (
            "The accruals / cash-conversion earnings-quality source cleared Gate 4 "
            "as a replay-only/default-off lead, but no production surface was "
            "promoted."
            if gate4["passed"]
            else (
                "The accruals / cash-conversion source is positively predictive "
                "(all-window positive EV/PnL beating the accepted comparators on "
                "gross deltas, concentration clean) but was REJECTED on drawdown "
                "drift: at 369 overlaid paper trades it is a large momentum-tilted "
                "sleeve whose drawdown exceeds the guardrail. It is a lead for a "
                "drawdown-aware, smaller-deployment redesign, not a promotable "
                "fixed bundle. Do not tune this fixed definition on these frozen "
                "windows."
                if drawdown_only_failure
                else (
                    "The accruals / cash-conversion earnings-quality source did "
                    "not clear Gate 4 (failed: "
                    + (", ".join(gate4["failed_reasons"]) or "none")
                    + "). Do not promote or tune this fixed quality definition on "
        ...
        "next_evidence_needed": (
            ...
            "(for example PIT TTM accruals with same-period cash flow, "
            "change-in-accruals momentum, or forward replacement-value rows), not "
            "a sweep of cash-conversion, accruals, freshness, RS, close, volume, "
            "top-N, hold, or cooldown thresholds on the same frozen windows."
        ),
        "post_run_reflection": {
            "why_result_happened": (
                (
                    "Rejected on drawdown drift only. The accruals / cash-conversion "
                    "quality source was positively predictive: aggregate EV "
                    "{:+.4f} and PnL ${:+,.2f} with positive EV/PnL in all three "
                    "windows and no regression, beating the accepted compression "
                    "and distribution comparators on the gross deltas, and "
                    "concentration passed. The binding failure is that 369 paper "
                    "trades (~1/day held 10 days, ~10 concurrent positions of "
                    "momentum-confirmed quality names) form a large, "
                    "highly-deployed sleeve whose drawdown drift (+{:.4f}) exceeds "
                    "the 0.5pp guardrail. The gross-delta beat over the much "
                    "smaller accepted comparators is partly a deployment-size "
                    "artifact, not proven risk-adjusted superiority; 36/47 "
                    "universe names pass the quality gate, so selection is "
                    "discriminated mostly by price confirmation, i.e. the sleeve "
                    "behaves like a broad cash-conversion-tilted momentum overlay."
                ).format(
                ...
                if drawdown_only_failure
                    ...
                    "Rejected. Annual cash-conversion quality plus liquid "
                    "SPY-relative confirmation did not clear Gate 4 versus the "
                    "accepted compression and distribution candidate-pool "
                    "comparators (failed: {}). The accruals signal may be too "
                    "slow-moving (annual cadence) for the 10-day paper horizon, or "
                    "it mostly re-selects names already captured by momentum/"
                    "quality sources."
            ...
            "new_evidence_required": (
                "The positive-EV-but-drawdown-failing readout means the next "
                "valid test is a DIFFERENT decision hypothesis: a drawdown-aware "
                "deployment of this quality source (concurrent-position cap, "
                "per-name protective stop, or a risk-budgeted sub-sleeve) sized to "
                "the accepted comparators, or a sharper quality discriminator "
                "(PIT TTM same-period accruals, accrual-change momentum, or "
                "quarterly cash-flow when reported) that selects fewer, "
                "higher-conviction names. A drawdown-controlled redesign is not a "
                "threshold sweep of this rejected fixed bundle."
                if drawdown_only_failure
                else (
                    ...
                    "rows before revisiting accruals quality."
                )
            ),
        },
        "related_files": [
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Accruals / Cash-Conversion Quality",
            "",

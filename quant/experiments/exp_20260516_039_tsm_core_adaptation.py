"""exp-20260516-039: TSM core medium-term adaptation scout.

Two-part replay-only experiment:

Local artifact id is 039 because 038 is already occupied by an uncommitted
signal-day ATR topup experiment. This script implements the user-plan 038A/038B
TSM lifecycle and core-risk scalar tests.

* 038A observes TSM lifecycle behavior on the accepted core stack.
* 038B sweeps one ticker-specific core risk scalar for existing TSM long
  signals, without changing entries, ranking, exits, targets, slots, heat,
  universe, LLM, or news behavior.
"""
...
EXPERIMENT_ID = "exp-20260516-039"
EXPERIMENT_SLUG = "tsm_core_adaptation"
MULTIPLIER_KEY = "tsm_core_risk_multiplier_applied"
TARGET_TICKER = "TSM"
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["tsm_core_baseline_shares"] = shares
    out["tsm_core_new_shares"] = new_shares
...
def _net_return(exit_price: float, entry_price: float) -> float | None:
    if entry_price <= 0:
        return None
    return round((exit_price / entry_price) - 1.0 - base.portfolio_engine.ROUND_TRIP_COST_PCT, 6)
...
def _diagnose_trade(
    ...
    out["entry_gap_vs_signal_pct"] = None
    entry_open = trade.get("entry_open_price")
    if isinstance(entry_open, (int, float)) and entry_price:
        out["entry_gap_vs_signal_pct"] = round((float(entry_open) / entry_price) - 1.0, 6)
...
def _build_lifecycle_diagnostic(
    ...
    for label, run in before_runs.items():
        rows = _load_ohlcv_rows(WINDOWS[label]["snapshot"], TARGET_TICKER)
    ...
    return {
        ...
        "branch_recommendation": (
            ...
            else "prioritize_core_quarantine"
        ),
...
def _candidate_payload(
    ...
    global CURRENT_RISK_MULTIPLIER, CANDIDATE_SIGNALS
...
def _markdown(payload: dict[str, Any]) -> str:
    diag = payload["lifecycle_diagnostic"]
    sweep_rows = [
    ...
    for row in payload["sweep_summary"]:
        sweep_rows.append(
            "| {mult:.2f} | {passed} | {dev:+.4f} | ${dpnl:+,.2f} | {improved} | {regressed} | {adj} | {wins} | {trades} | {dd:+.4f} |".format(
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} TSM Core Adaptation",
            "",
            f"Decision: `{payload['decision']}`.",
            "",
            "## 038A lifecycle diagnostic",
            "",
            f"- TSM trade count: `{diag['trade_count']}`",
            f"- TSM total PnL: `${diag['total_pnl']:,.2f}`",
            f"- Fast-target supported: `{diag['fast_target_supported']}`",
            f"- Branch recommendation: `{diag['branch_recommendation']}`",
            "",
            ...
            *window_rows,
            "",
            "Production impact: replay-only scout unless the selected scalar is promoted into shared constants/portfolio sizing and rerun through the canonical three-window protocol.",
        ]
    )
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_tsm_core_quarantine"
    )
    interpretation = (
        "TSM core risk quarantine cleared the canonical three-window scout; promote only as a ticker-specific medium-term risk-budget rule, not as a semiconductor rule."
        if passed
        else "TSM remained a negative cohort, but the tested ticker-specific core scalar did not satisfy the canonical promotion gate."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "TSM is persistently negative in the current medium-term core long "
            "system; if it cannot be rescued by faster lifecycle evidence, a "
            "ticker-specific core risk scalar should remove or sharply reduce "
            "its medium-term risk budget."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "tsm_core_risk_multiplier",
        "single_causal_variable": (
            "post-sizing risk scalar for existing core long signals where ticker == TSM"
        ),
        ...
        "parameters": {
            ...
            "locked_variables": [
                ...
                "all non-TSM sizing multipliers",
                "portfolio heat",
                "slot planning",
                "LLM/news replay",
                "event sleeves",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: TSM may be structurally unsuitable for the "
                "current medium-term core long holding model."
            ),
            "2_history_check": {
                "exp-20260516-012": (
                    "A broader semiconductor non-green trend haircut was "
                    "directionally positive but failed sample guard; current "
                    "ticker-level evidence adds four TSM losses across the "
                    "accepted current stack."
                ),
                "ticker_effectiveness_audit": (
                    "TSM had four trades, zero winners, and negative aggregate "
                    "PnL across late_strong, mid_weak, and old_thin."
                ),
            },
            "3_single_causal_variable": (
                "Only the TSM core risk scalar changes in 038B; 038A is observed-only."
            ),
            "4_acceptance_standard": (
                "docs/backtesting.md canonical three fixed windows; aggregate "
                "EV/PnL positive, >=2 improved windows, no regressed windows, "
                "drawdown drift <=0.5pp, trade_count_sum >=58, affected TSM "
                "signals >=4 across >=2 windows, and survival >=5%."
            ),
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\"
                "exp_20260516_039_tsm_core_adaptation.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate1": {
            ...
            "baseline_artifact": (
                "data/experiments/exp-20260516-020/"
                "trend_tech_dte_residual_risk.json"
            ),
        },
        "gate2": {
        ...
        "llm_metrics": {
            "used_llm": False,
            "blocker_relation": (
                "No LLM behavior changed; this is a deterministic ticker-specific "
                "risk-budget scout."
            ),
        },
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, add TSM_CORE_RISK_MULTIPLIER to shared constants, "
                "apply it in portfolio_engine.py for TSM core long sizing, add "
                "the attribution key to backtester.py, add focused tests, update "
                "parity docs, then rerun the canonical three windows."
            ),
        },
        "why_not_other_changes": (
            "This does not touch V/DDOG/ISRG or the whole semiconductor cohort, "
            "because the strongest clean evidence is TSM-specific and the "
            "playbook warns against broad sector/ticker expansion from thin "
            "cohorts."
        ),
        "known_risks": [
            "Ticker-specific rules can overfit if promoted without forward evidence.",
            "A zero scalar can free slots and alter replacement trades even though no entry filter is added.",
            "The rule must not be generalized to semiconductor names without a separate industry-level field.",
        ],
        ...
        "next_evidence_needed": (
            None
            if passed
            else "Do not retry nearby TSM core scalars without new forward ticker-level evidence or a distinct lifecycle discriminator."
        ),
        "anti_js": "No JavaScript was used.",
        "related_files": [
            "quant/experiments/exp_20260516_039_tsm_core_adaptation.py",
        ...
        json.dumps(
            {
                ...
                "lifecycle_branch": result["lifecycle_diagnostic"][
                    "branch_recommendation"
                ],
                "fast_target_supported": result["lifecycle_diagnostic"][

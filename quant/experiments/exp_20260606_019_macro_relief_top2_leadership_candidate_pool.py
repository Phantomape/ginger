...
candidate source: on official CPI/FOMC/NFP dates where both SPY and QQQ rally
and close high in the daily range, admit the two strongest liquid stock leaders
as top-2 next-open default-off paper candidates with a fixed 10-trading-day
hold.
...
EXPERIMENT_ID = "exp-20260606-019"
STEM = "macro_relief_top2_leadership_candidate_pool"
TRIAL_FAMILY = "macro_relief_leadership_candidate_pool"
TRIAL_VARIANT_ID = "macro_relief_top2_next_open_10d_v1"
CHANGED_VARIABLE = "official_macro_relief_day_stock_leadership_top2_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = framework.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260606_019_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "second_candidate_noise",
        "broad_momentum_relabeling",
        "old_thin_regression",
        "drawdown_drift",
        "concentration_failed",
    ],
    "confidence_reason": (
        "exp-20260606-017 improved EV and PnL in all three windows but failed "
        "only the sample-count gate with 10 trades. This test keeps the same "
        "official-event relief context and stock scoring, changing only the "
        "daily selected count from top-1 to top-2."
    ),
    "recorded_at": "2026-06-06T16:45:06Z",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "CPI/FOMC/NFP calendar, SPY/QQQ relief-day test, sector-known liquid "
        "stock universe, stock leadership fields, same-ticker core-overlap "
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_macro_relief_top2_leadership_candidate_pool"
        if gate["passed"]
        else "rejected_macro_relief_top2_leadership_candidate_pool"
    )
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "Official CPI/FOMC/NFP relief days where SPY and QQQ both "
                "rally and close strong may support two liquid stock leaders "
                "per event day, increasing sample size while preserving the "
                "positive three-window top-1 edge."
            ),
            ...
            "new_evidence_type": "materially_broader_same_event_candidate_count",
            ...
            "negative_reflection": (
                "If rejected, the likely reason is that official macro relief "
                "second leaders add noise or the selected stocks are still a "
                "broad momentum relabeling. A retry would need a true PIT "
                "macro surprise/consensus field or forward replacement-value "
                "evidence, not top-N or threshold retuning."
            ),
        }
    )
    ...
    payload["gate_questions"] = {
            "1_alpha_hypothesis": (
                "entry/candidate_pool: official macro relief days may create "
                "cleaner leadership continuation in more than one liquid "
                "leader because a macro uncertainty release can force fast "
                "risk reallocation into the day's strongest stocks."
            ),
            "2_history_check": {
                "exp-20260606-017": (
                ...
                "exp-20260525-901": (
                    "Opening-range macro-confirmed gate improved EV only +0.2926 "
                    "and had one PnL-regression window; rejected."
            ),
            "exp-20260605-030/032": (
                "NFP and scheduled macro selloff QQQ actions were too thin "
                "and failed strict materiality; this test is a stock candidate "
                "pool, not ETF beta allocation."
            ),
            "exp-20260606-004/005": (
                "Broad 5-day winner continuation was positive but rejected "
                "for drawdown/tail and old_thin weakness; this test only "
                "admits official macro relief dates."
            ),
        },
        "3_single_causal_variable": CHANGED_VARIABLE,
        "4_acceptance_standard": (
            "Use docs/backtesting.md three canonical windows. Aggregate EV/PnL "
            "must improve, at least two windows must improve, target sample "
            ">=20 across all 3 windows, survival >=5%, drawdown drift <=0.5pp, "
            "and concentration guard must pass."
        ),
        "5_reproducibility": (
            ".venv\\Scripts\\python.exe -B quant\\experiments\\"
            "exp_20260606_019_macro_relief_top2_leadership_candidate_pool.py"
        ),
    }
    ...
    payload["interpretation"] = (
        ...
        if payload["gate4"]["passed"]
            ...
            "The official macro relief leadership candidate pool did not clear "
            "Gate 4. Do not promote or retune this macro-relief stock leadership "
            "definition on the frozen windows without materially new PIT macro "
            "surprise/consensus data or forward replacement-value evidence."
        )
    )
    ...
    payload["next_evidence_needed"] = (
        ...
        "forward replacement-value rows; do not simply retune top-N, relief "
        "returns, close-location thresholds, hold days, cooldown, or paper notional."
    )
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Macro Relief Top-2 Leadership Candidate Pool",
            "",

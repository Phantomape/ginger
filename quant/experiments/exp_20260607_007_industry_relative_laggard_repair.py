"""exp-20260607-007: industry-relative laggard repair candidate pool.

Replay-only alpha search. It tests one broad, free-OHLCV relation source:
within liquid industry groups that already show 20-day relative strength,
select the liquid stock that lagged the group but reclaims strength on the
...
EXPERIMENT_ID = "exp-20260607-007"
STEM = "industry_relative_laggard_repair"
TRIAL_FAMILY = "industry_relative_laggard_repair_candidate_pool"
TRIAL_VARIANT_ID = "industry_laggard_signal_day_reclaim_top1_next_open_10d_v1"
CHANGED_VARIABLE = "industry_relative_laggard_repair_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = framework.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / "exp_20260607_007_industry_relative_laggard_repair.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "drawdown_drift",
        "laggard_value_trap",
        "sector_laggard_relabeling",
        "sample_too_small",
        "concentration_failed",
    ],
    "confidence_reason": (
        "Recent relation-aware free-OHLCV peer shock worked only after "
        "core-flow confirmation, while sector ETF laggards and short-horizon "
        "reversal failed. Industry-relative repair is a different free "
        "relation source but likely cost/tail sensitive."
    ),
    "recorded_at": "2026-06-07T05:10:20Z",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "industry 20-day relative-strength medians, individual lag/reclaim "
...
def _candidate_from_metrics(
    *,
    ...
    if group_lag_20d < MIN_INDUSTRY_LAG_20D or group_lag_20d > MAX_INDUSTRY_LAG_20D:
        return None
    ...
    return {
        ...
        "source": "INDUSTRY_RELATIVE_LAGGARD_REPAIR_PAPER",
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_industry_relative_laggard_repair"
        if gate["passed"]
        else "rejected_industry_relative_laggard_repair_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                "Within strong liquid industry groups, stocks that lag the "
                "group over 20 trading days but reclaim relative strength on "
                "the signal day may add neglected catch-up candidates without "
                "simply expanding into noisy broad momentum tickers."
            ),
            ...
            "new_evidence_type": "free_ohlcv_industry_relative_laggard_repair",
            ...
            "interpretation": (
                "The industry-relative laggard repair source cleared Gate 4 "
                "as a replay-only/default-off lead, but no production surface "
                "was promoted."
                if gate4["passed"]
                else (
                    "The industry-relative laggard repair source did not clear "
                    "Gate 4. Do not promote it or answer by retuning industry "
                    "lag, group-strength, signal-day reclaim, hold-day, "
                    "cooldown, top-N, or notional thresholds on these frozen "
                    "windows."
                )
            ),
            "negative_reflection": (
                "If rejected, the likely reason is that industry laggards are "
                "usually laggards for a reason, and a single-day reclaim is "
                "not enough to separate healthy rotation from value traps after "
                "costs. A retry needs a materially new PIT relation or event "
                "field, or closed forward replacement evidence."
            ),
            "post_run_reflection": {
                "why_result_happened": (
                    "Pending run result. Replace with measured Gate 4 behavior "
                    "after the three-window replay."
                ),
                "forbidden_near_neighbor_retry": (
                    "Do not retry by sweeping industry lag, group median RS, "
                    "positive fraction, signal-day reclaim, volume, volatility, "
                    "hold-day, top-N, cooldown, or paper notional thresholds."
                ),
                "new_evidence_required": (
                    "A retry requires independent PIT confirmation such as "
                    "event/news context, borrow/options/ownership data with "
                    "adequate historical coverage, or closed forward "
                    "replacement rows from a shared default-off adapter."
                ),
            },
            "next_evidence_needed": (
                "Independent PIT context or forward shared-adapter replacement "
                "rows are required before retrying near-neighbor industry "
                "laggard repair variants."
            ),
            "related_files": [
    ...
    if not gate4["passed"]:
        payload["post_run_reflection"]["why_result_happened"] = (
            ...
            "value or regressed at least one canonical window. That means "
            "industry-relative lag plus a signal-day reclaim did not reliably "
            "distinguish catch-up rotation from weak laggards under existing "
            "costs, slippage, and 10-day exit assumptions."
        )
    else:
        payload["post_run_reflection"]["why_result_happened"] = (
            "The source produced positive replacement value in all three "
            "canonical windows without breaching drawdown, survival, or "
            "concentration guardrails, suggesting industry-relative lag plus "
            "same-day repair captured a distinct rotation relation."
        )
    ...
    payload["backtest_protocol"]["execution_model"] = (
        ...
        "5-day repair, 60-day trend guard, ADV, volume ratio, and realized "
        "volatility. Paper entry is next available open with existing entry "
        "slippage; exit is the close 10 trading days after the signal with "
        "target-side sell slippage and ROUND_TRIP_COST_PCT."
    )
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            "entry/candidate_pool: in strong liquid industry groups, a 20-day "
            "laggard that reclaims SPY-relative strength on the signal day may "
            "capture catch-up rotation."
        ),
        "2_history_check": {
            "exp-20260606-029": (
                "Sector ETF same-sector laggard failed; this uses PIT industry "
                "group medians from individual stock OHLCV, not ETF leadership."
            ),
            "exp-20260607-005": (
                "Short-horizon trend reversal failed; this is not a raw ret3 "
                "selloff/reclaim and requires group-level 20-day strength."
            ),
            "exp-20260607-006": (
                "Macro relief sector-confirmed leadership had mixed window "
                "comparison; this is not a macro relief threshold or sector "
                "median top-N retune."
            ),
            "exp-20260606-024": (
                "Core-flow rolling-correlation peer shock was a positive lead; "
                "this borrows the relation-aware lesson but uses industry "
                "cross-section displacement instead of pair correlation."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260607_007_industry_relative_laggard_repair.py"
        ),
    }
    payload["pre_run_questions"] = payload["gate_questions"]
    payload["gate3"]["note"] = (
        "No new core filter or entry rule was added. The industry-relative "
        "laggard repair source is additive default-off paper, so core signals "
        "generated/survived are unchanged from baseline."
    )
    payload["context_alias"] = "industry_repair_candidate_day_contexts"
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Industry-Relative Laggard Repair",
            "",

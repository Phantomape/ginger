"""exp-20260512-011: SEC financial-report clean earnings 8-K notional.

Alpha search on one causal variable: a paper-notional scalar for clean
earnings 8-K candidates in the accepted default-off SEC financial-report T+1
sleeve. Clean means Item 2.02 with no auxiliary items beyond 9.01.
"""
...
EXPERIMENT_ID = "exp-20260512-011"
STEM = "exp_20260512_011_sec_financial_report_clean_earnings8k_notional"
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_financial_report_clean_earnings8k_notional.md"
)
EXPERIMENT_LOG_JSONL = REPO_ROOT / "docs" / "experiment_log.jsonl"

BASELINE_CLEAN_EARNINGS8K_SCALAR = 1.0
CLEAN_EARNINGS8K_SCALAR_VARIANTS = (0.50, 0.75, 1.0, 1.10, 1.25, 1.50)
MIN_PROMOTION_CLOSED_TRADES = 40
MIN_CLEAN_EARNINGS8K_CLOSED_TRADES = 10
MAX_DRAWDOWN_WORSENING = 0.005
PURE_EARNINGS_8K_ITEMS = {"2.02", "9.01"}
...
def _normalise_item_codes(value: Any) -> set[str]:
    if value is None:
        return set()
...
def _is_clean_earnings8k_position(position: dict[str, Any]) -> bool:
    candidate = position.get("source_candidate") or {}
    if str(candidate.get("event_family") or "") != "earnings_8k":
        return False
    item_codes = _normalise_item_codes(candidate.get("item_codes"))
    return "2.02" in item_codes and item_codes.issubset(PURE_EARNINGS_8K_ITEMS)


def _pnl_for_position(
    position: dict[str, Any],
    *,
    clean_earnings8k_scalar: float,
    closed: bool,
) -> float:
    scalar = (
        float(clean_earnings8k_scalar)
        if _is_clean_earnings8k_position(position)
        else 1.0
    )
    if closed:
...
def _adjust_closed_position(
    ...
    *,
    ...
    adjusted["clean_earnings8k_notional_scalar"] = scalar
    adjusted["clean_earnings8k_bucket"] = _is_clean_earnings8k_position(position)
    ...
    *,
    ...
    for as_of, prices in prices_by_date.items():
        ...
        queue = {
            ...
            "rule_version": "exp-20260512-011-replay",
            "enabled": False,
            "asof_date": as_of,
        ...
        clean_filled_count += sum(
            1
            for item in snapshot.get("filled_entries") or []
            if _is_clean_earnings8k_position(item)
        )

        realized = sum(
            _pnl_for_position(
                item,
                clean_earnings8k_scalar=clean_earnings8k_scalar,
                closed=True,
            )
    ...
    clean_closed = [item for item in closed_positions if item["clean_earnings8k_bucket"]]
    sleeve_curve = [
    ...
    standalone_metrics.update(
        {
            ...
            "clean_earnings8k_closed_trade_count": len(clean_closed),
            "clean_earnings8k_filled_count": clean_filled_count,
            "clean_earnings8k_total_pnl": _round(
                sum(float(item.get("pnl") or 0.0) for item in clean_closed),
                2,
            ),
        }
    )
...
def _window_checks(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    for label in WINDOWS:
        ...
        checks[label] = {
            ...
            "clean_earnings8k_closed_trade_count": int(
                after["by_window"][label]["sleeve_metrics"].get(
                    "clean_earnings8k_closed_trade_count"
                )
                or 0
            ),
            "clean_earnings8k_total_pnl": _round(
                after["by_window"][label]["sleeve_metrics"].get(
                    "clean_earnings8k_total_pnl"
                ),
                2,
            ),
        }
...
def _gate(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    clean_trades_after = sum(
        row["clean_earnings8k_closed_trade_count"] for row in checks.values()
    )
    passed = (
        ...
        and clean_trades_after >= MIN_CLEAN_EARNINGS8K_CLOSED_TRADES
    )
    return {
        "aggregate_delta": aggregate_delta,
        "clean_earnings8k_closed_trade_count_after": clean_trades_after,
        ...
        "rule": (
            ...
            "closed trades, and the clean earnings 8-K bucket has at least "
            "10 closed trades."
        ),
...
def _best_candidate(variants: OrderedDict[str, dict[str, Any]]) -> str:
    baseline = variants[f"clean_earnings8k_scalar_{BASELINE_CLEAN_EARNINGS8K_SCALAR:.2f}"]
    candidates = [
        (name, row, _gate(row, baseline))
        for name, row in variants.items()
        if row["clean_earnings8k_notional_scalar"] != BASELINE_CLEAN_EARNINGS8K_SCALAR
    ]
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} SEC financial-report clean earnings 8-K notional",
        "",
        ...
        (
            "- Clean earnings 8-K closed trades: "
            f"`{payload['gate']['clean_earnings8k_closed_trade_count_after']}`"
        ),
        "",
        "This changes only default-off paper sizing for clean Item 2.02 earnings "
        "8-K candidates. It changes no live orders, queue qualification, queue "
        "sorting, capacity, hold period, or core signal path.",
        "",
    ]
    return "\n".join(lines)
...
def main() -> int:
    ...
    for scalar in CLEAN_EARNINGS8K_SCALAR_VARIANTS:
        name = f"clean_earnings8k_scalar_{scalar:.2f}"
        row = _run_variant(
            core_results=core_results,
            exp100=exp100,
            clean_earnings8k_scalar=scalar,
        )
        row["clean_earnings8k_notional_scalar"] = scalar
        variants[name] = row

    baseline_key = f"clean_earnings8k_scalar_{BASELINE_CLEAN_EARNINGS8K_SCALAR:.2f}"
    ...
    decision = (
        f"accepted_default_off_clean_earnings8k_notional_{best['clean_earnings8k_notional_scalar']:.2f}x"
        if gate["passed"]
        else "rejected_clean_earnings8k_notional_scalar"
    )
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Inside the accepted SEC financial-report T+1 paper sleeve, clean "
            "earnings 8-K filings with Item 2.02 and no auxiliary items beyond "
            "9.01 may deserve different paper risk than noisier earnings 8-Ks "
            "because the semantic event is more directly tied to results."
        ),
        "change_type": "alpha_search_semantic_risk_allocation",
        "changed_variable": "sec_financial_report_clean_earnings8k_notional_scalar",
        "single_causal_variable": "clean Item 2.02 earnings 8-K paper-notional scalar only",
        "parameters": {
            "baseline_clean_earnings8k_notional_scalar": BASELINE_CLEAN_EARNINGS8K_SCALAR,
            "clean_earnings8k_scalar_variants": list(CLEAN_EARNINGS8K_SCALAR_VARIANTS),
            "clean_earnings8k_definition": (
                "event_family == earnings_8k and item_codes include 2.02 and "
                "are subset of {2.02, 9.01}"
            ),
        ...
        "gate": gate,
        "gate4": {"passed": gate["passed"], "rule": gate["rule"]},
        "decision": decision,
        "rejection_reason": (
            None
            if gate["passed"]
            else "No clean earnings 8-K notional scalar cleared the three-window gate."
        ),
        "next_evidence_needed": (
            ...
            if gate["passed"]
            else "Forward replacement-value evidence or a richer same-accession earnings-quality field before retrying clean 8-K sizing."
        ),
        "production_impact": {
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "risk allocation: change paper notional only for clean Item 2.02 "
                "earnings 8-K SEC financial-report T+1 candidates."
            ),
            "2_history_check": (
                "exp-20260511-114 rejected excluding 10-K rows; exp-20260512-006 "
                "accepted global event notional; exp-20260512-007 accepted "
                "periodic_report family notional; exp-20260512-009 rejected "
                "queue-rank notional. No logged run isolated clean earnings 8-K "
                "item semantics inside this SEC sleeve."
            ),
            "3_single_causal_variable": "clean earnings 8-K paper-notional scalar only",
            "4_acceptance_standard": (
                "Three fixed windows, aggregate EV and sleeve PnL improve, "
                "EV/PnL improve in all three windows, max drawdown drift <=0.5pp, "
                "at least 40 closed sleeve trades, and at least 10 closed clean "
                "earnings 8-K trades."
            ),
            "5_reproducibility": (
        ...
        "why_not_other_changes": (
            ...
            "notional, periodic-report scalar, and queue-rank notional variants "
            "are already logged, so this run changes only clean earnings 8-K "
            "semantic risk allocation."
        ),
    }

"""exp-20260515-020: dual slow-trend leadership risk allocation.

Tests one production-visible allocation state on the accepted core stack:
already-qualified trend/breakout stock signals where both accepted slow-trend
states are true:

* rs60_top_quintile_state
* price_vs_200ma_extension_state

This is a cap-aware post-sizing risk top-up scout. It does not change entry,
ranking, exits, targets, universe, LLM/news behavior, filters, heat, or slots.
"""
...
EXPERIMENT_ID = "exp-20260515-020"
EXPERIMENT_SLUG = "dual_slow_trend_leadership_risk"
MULTIPLIER_KEY = "dual_slow_trend_leadership_risk_multiplier_applied"
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        return original(signals, features_dict, atr_target_mult=atr_target_mult)

    return wrapped


def _dual_slow_trend_state(sig: dict[str, Any]) -> bool:
    return (
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["dual_slow_trend_leadership_baseline_shares"] = shares
    out["dual_slow_trend_leadership_desired_shares"] = desired_shares
    out["dual_slow_trend_leadership_cap_shares"] = cap_shares
    out["dual_slow_trend_leadership_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if _dual_slow_trend_state(sig) and sizing.get("shares_to_buy"):
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Dual Slow-Trend Leadership Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_dual_slow_trend_leadership_risk"
    )
    interpretation = (
        "Dual slow-trend leadership cleared the canonical three-window gate and should be promoted only through shared production/backtest portfolio sizing."
        if passed
        else "Dual slow-trend leadership did not clear the canonical three-window gate; do not add an extra overlap top-up on the frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The current accepted stack rewards RS60 leadership and price-vs-200MA "
            "extension independently. The remaining alpha question is whether their "
            "intersection identifies a smaller set of persistent slow-trend leaders "
            "that can support a tiny additional cap-aware sizing top-up without "
            "adding filters or expanding the candidate pool."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "dual_slow_trend_leadership_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for trend/breakout stock signals where rs60_top_quintile_state and price_vs_200ma_extension_state are both true"
        ),
        "parameters": {
            "state_definition": {
                ...
                "notes": (
                    "Both component states are already production-visible shared risk fields; this scout tests only their overlap sizing multiplier."
                ),
            },
            ...
            "locked_variables": [
                ...
                "component RS60 and price-vs-200MA state definitions",
                "portfolio heat",
                "MAX_POSITIONS",
                "LLM/news replay",
                "pilot/event sleeves",
        ...
        "historical_experiment_check": {
            "similar_prior_results": {
                "exp-20260513-030": (
                    "Accepted RS60 top-quintile allocation as a standalone slow-momentum state."
                ),
                "exp-20260515-018": (
                    "Accepted price-vs-200MA extension as a standalone slow-trend state."
                ),
                "exp-20260513-009": (
                    "Rejected signal-day close-location; this run avoids that repeated candlestick variable."
                ),
                "exp-20260514-045": (
                    "Rejected a broader triple-momentum cap stack; this run tests only the intersection of two already accepted slow-trend states and does not change caps."
                ),
                "exp-20260515-019": (
                    "Rejected Space candidate-pool expansion; this keeps the candidate pool fixed."
                ),
                "LLM_soft_ranking": (
            ...
            "direct_duplicate_check": (
                "No prior rs60_top_quintile_state x price_vs_200ma_extension_state overlap sizing experiment was found in docs, artifacts, data, or quant/experiments."
            ),
            "why_this_branch": (
                "It follows the playbook's allocation-before-filtering prior and tests an orthogonal production-visible overlap rather than another scalar retune, LLM branch, or noisy universe expansion."
            ),
        },
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: signals that are both RS60 top-quintile leaders and top-quartile price-vs-200MA extension leaders may deserve a tiny extra allocation."
            ),
            "2_history_check": (
                "The component states were accepted separately, close-location/range/volume/candidate-pool branches were rejected or data-limited, and no direct overlap risk scout was found."
            ),
            "3_single_causal_variable": (
                "dual_slow_trend_leadership_risk_multiplier; component state definitions stay fixed."
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260515_020_dual_slow_trend_leadership_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        "llm_metrics": {
            "used_llm": False,
            "blocker_relation": (
                "LLM soft-ranking remains sample/data limited, so this run used deterministic production-visible allocation fields instead."
            ),
        },
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, implement the overlap top-up in shared portfolio_engine.size_signals and add the attribution key in backtester.py plus focused production/backtest parity tests."
            ),
        },
        ...
        if passed
            ...
            "Do not retry RS60 x price-vs-200MA overlap top-ups on frozen windows without new forward attribution or a distinct production-visible drawdown discriminator."
        ),
        "related_files": [
            "quant/experiments/exp_20260515_020_dual_slow_trend_leadership_risk.py",
            "data/experiments/exp-20260515-020/dual_slow_trend_leadership_risk.json",
            "docs/experiments/logs/exp-20260515-020_dual_slow_trend_leadership_risk.json",
            "docs/experiments/tickets/exp-20260515-020_dual_slow_trend_leadership_risk.json",
            "docs/experiments/artifacts/exp-20260515-020_dual_slow_trend_leadership_risk.md",
    ...
    if path.exists():
        for existing in path.read_text(encoding="utf-8").splitlines():
            ...
            same_experiment = (
                ...
                == "dual_slow_trend_leadership_risk_multiplier"
            )
            if same_experiment:
                if not replaced:
                    rows.append(line)

"""exp-20260520-041: current-stack DTE non-confirming candle risk scout.

Alpha search. Tests one current-stack risk-allocation variable: whether signals
already tagged by existing DTE risk multipliers and lacking same-day green
candle confirmation still deserve residual risk.
...
EXPERIMENT_ID = "exp-20260520-041"
EXPERIMENT_SLUG = "current_dte_nonconfirming_candle_risk"
MULTIPLIER_KEY = "current_dte_nonconfirming_candle_risk_multiplier_applied"
DTE_KEYS = {
...
TRIAL_ACCOUNTING = {
    ...
    "changed_variable": "current_dte_nonconfirming_candle_risk_multiplier",
    "prior_trial_count": 5,
...
def _target_signal(sig: dict[str, Any]) -> bool:
    return bool(_active_dte_keys(sig)) and sig.get("signal_day_ticker_green_candle") is False
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["current_dte_nonconfirming_candle_active_keys"] = [
        key for key in sorted(DTE_KEYS) if sizing.get(key) not in (None, 1, 1.0, False)
    ]
    out["current_dte_nonconfirming_candle_baseline_shares"] = shares
    out["current_dte_nonconfirming_candle_new_shares"] = new_shares
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Current-Stack DTE Non-Confirming Candle Risk",
            "",
            ...
            *sweep_rows,
            "",
            f"Selected non-control non-confirming scalar: `{payload['parameters']['selected_risk_multiplier']}`.",
            "",
...
def _configure_prior() -> None:
    ...
    prior.TARGET_TICKER = "CURRENT_DTE_NONCONFIRMING_CANDLE"
...
def build_payload() -> dict[str, Any]:
    ...
    payload["gate1"]["baseline_note"] = (
        ...
        "signals already tagged by existing DTE risk multipliers and lacking "
        "same-day green candle confirmation."
    )
    payload.update(TRIAL_ACCOUNTING)
    payload["status"] = (
        "accepted_for_shared_policy_implementation"
        if payload["gate4"]["passed"]
        else "rejected_current_dte_nonconfirming_candle_risk"
    )
    payload["decision"] = payload["status"]
    payload["hypothesis"] = (
        "Existing DTE proximity haircuts appear weaker when the same signal-day "
        "candle fails to confirm green. Applying one additional post-sizing "
        "scalar only when a signal already carries a non-neutral DTE risk tag "
        "and signal_day_ticker_green_candle is false may improve EV without "
        "changing entry, ranking, exits, slots, or LLM/news behavior."
    )
    payload["change_type"] = "risk_allocation_shadow"
    payload["changed_variable"] = "current_dte_nonconfirming_candle_risk_multiplier"
    payload["single_causal_variable"] = (
        "post-sizing residual risk scalar for existing core signals with active DTE risk tags and signal_day_ticker_green_candle == false"
    )
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            ...
            "multipliers may consume too much residual capital when the signal-day "
            "candle does not confirm green."
        ),
        "2_history_check": {
            "nearby_rejections": (
            ...
            "new_evidence": (
                "exp-20260520-038 showed the broad DTE residual haircut improved "
                "late_strong and mid_weak but regressed old_thin. The new evidence "
                "is the non-confirming candle subcohort, which avoids the green "
                "DTE trades that caused most old_thin collateral."
            ),
            "anti_repeat": (
        ...
        "3_single_causal_variable": "current_dte_nonconfirming_candle_risk_multiplier",
        "4_acceptance_standard": (
            ...
            ">=5%, affected signals >=4 across at least 2 windows, and drawdown drift <=0.5pp."
        ),
        "5_reproducibility": (
            ".venv\\Scripts\\python.exe quant\\experiments\\"
            "exp_20260520_041_current_dte_nonconfirming_candle_risk.py"
        ),
    }
    payload["production_impact"] = {
        ...
        "promotion_requirement": (
            "If accepted, add a shared DTE non-confirming candle risk policy used by both "
    ...
    payload["llm_metrics"] = {
        ...
        "blocker_relation": (
            ...
            "DTE non-confirming candle risk-budget scout."
        ),
    }
    payload["why_not_other_changes"] = (
        ...
        "work lacks phrase provenance. The failed broad DTE residual scout exposed "
        "a production-visible subcohort worth testing without adding a filter."
    )
    payload["known_risks"] = [
    ...
    if payload["gate4"]["passed"]:
        payload["interpretation"] = (
            "The current-stack DTE non-confirming candle scalar cleared the replay gate, but "
            "promotion still needs shared policy/parity implementation."
        )
        payload["rejection_reason"] = None
    else:
        payload["interpretation"] = (
            "The current-stack DTE non-confirming candle scalar did not clear Gate 4; keep "
            "existing DTE policies unchanged."
        )
        payload["rejection_reason"] = payload["interpretation"]
    payload["next_evidence_needed"] = (
        "Do not retry nearby DTE candle/DTE residual scalars without new forward "
        "DTE-tagged outcomes or a distinct event-quality field."
    )
    payload["anti_js"] = "No JavaScript was used."
    payload["related_files"] = [
        "quant/experiments/exp_20260520_041_current_dte_nonconfirming_candle_risk.py",
        ...
        json.dumps(
            {
                ...
                "selected_nonconfirming_scalar": payload["parameters"][

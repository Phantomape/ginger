...
stack: whether source-diverse official Space trend signals whose event
evidence includes a company_release deserve a separate default-off risk scalar.
...
STEM = "space_source_diversity_company_release_trend_risk"
...
TARGET_SOURCE_TYPE = "company_release"
MARKER = "space_source_diversity_company_release_trend_risk"
...
SCALARS = (1.0, 0.75, 0.9, 0.975, 1.025, 1.05, 1.075, 1.1)
...
def _profile_has_company_release(profile: dict[str, Any] | None) -> bool:
    if not profile:
        return False
    source_types = {str(item) for item in (profile.get("source_types") or [])}
    return TARGET_SOURCE_TYPE in source_types


def _apply_company_release_scale(
    *,
    signal: dict[str, Any],
    sizing: dict[str, Any],
    scalar: float,
    ...
    *,
    ...
    def patched_scale_and_record(
        *,
        ...
        if not _profile_has_company_release(profile):
            return
        _apply_company_release_scale(
            signal=signal,
            sizing=sizing,
            scalar=company_release_scalar,
            portfolio_value=portfolio_value,
            counts=counts,
            adjustments=company_release_adjustments,
            profile=profile,
        )
        signal["space_source_diversity_company_release_trend_bucket"] = True
        signal["space_source_diversity_company_release_trend_scalar"] = (
            company_release_scalar
        )
        signal["space_source_diversity_company_release_profile"] = profile
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_source_diversity_company_release_trend_scalar": (
            company_release_scalar
        ),
    ...
    variant["source_diversity_company_release_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["source_diversity_company_release_counts_by_window"] = by_window_counts
    variant["source_diversity_company_release_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(company_release_adjustments)
    )
    variant["source_diversity_company_release_adjustment_sample"] = (
        company_release_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("source_diversity_company_release_counts") or {}
    changed_count = int(counts.get(f"{MARKER}_changed_signal", 0))
    eligible_count = int(counts.get(f"{MARKER}_eligible_signal", 0))
    scalar = float(
        variant["parameters"]["space_source_diversity_company_release_trend_scalar"]
    )
    passed = bool(
        scalar != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "eligible_company_release_signal_count": eligible_count,
        "changed_company_release_signal_count": changed_count,
        "reasons": {
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            ...
            "includes company_release may be mis-sized by the accepted "
            "default-off Space stack."
        ),
        "change_type": "alpha_search",
        "changed_variable": "space_source_diversity_company_release_trend_scalar",
        "parameters": {
        ...
        "gate_answers": {
            "1_alpha_hypothesis": (
                "Risk allocation: source-diverse Space trend signals with a "
                "company_release source in the evidence profile may be mis-sized."
            ),
            "2_prior_similar_experiments": [
                "exp-20260515-025 rejected authority-only source-diversity trend because only mid_weak improved.",
                ...
                "No prior current-stack run isolated company_release source-diverse trend profiles on top of exp-20260515-044.",
            ],
            "3_single_causal_variable": (
                "Only the company_release source-diversity trend scalar changes."
            ),
            "4_success_criteria": (
            ...
            "5_reproducibility": (
                ...
                "exp_20260515_045_space_source_diversity_company_release_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the company-release source-diversity trend scalar "
        ...
        if promoted
            ...
            "Do not retry nearby company-release source-diversity Space trend "
        ...
        "why_not_other_changes": (
            ...
            "keeps the fixed pool and tests the complement to the rejected "
            "authority-only source-diversity branch."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity company-release trend risk",
        "",
        "## Hypothesis",
        (
            "Source-diverse official Space `trend_long` signals may need a "
            "different default-off allocation when their source-diversity "
            "evidence profile includes `company_release`."
        ),
        "",
        "## Single Changed Variable",
        (
            "`space_source_diversity_company_release_trend_scalar` on top of "
        ...
        f"- target source type: `{TARGET_SOURCE_TYPE}`",
        "",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_source_diversity_company_release_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_company_release_signal_count']}`",
            f"- adjusted signals: `{gate['changed_company_release_signal_count']}`",
            f"- adjusted counts: `{best['source_diversity_company_release_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Company-release source-diversity trend scalar "
            f"{best['parameters']['space_source_diversity_company_release_trend_scalar']} "
            f"changed {gate['changed_company_release_signal_count']} signals with "
            "aggregate EV delta "
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            "Source-diverse Space trend signals whose evidence profile includes "
            "company_release may be mis-sized in the accepted stack."
        ),
        "changed_variable": "space_source_diversity_company_release_trend_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate["eligible_company_release_signal_count"],
                    "adjusted_signals": gate["changed_company_release_signal_count"],

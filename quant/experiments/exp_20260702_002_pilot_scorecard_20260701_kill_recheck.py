...
EXPERIMENT_ID = "exp-20260702-002"
OWNER = "alpha-explore"
LANE = "alpha_search"
SLUG = "pilot_scorecard_20260701_kill_recheck"
...
RUNNER = f"quant/experiments/exp_20260702_002_{SLUG}.py"
RUNNER_COMMAND = ".\\.venv\\Scripts\\python.exe -B " + RUNNER.replace("/", "\\")
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260702_002_{SLUG}.json"
...
PILOT_RECOMMENDATIONS_JSON = (
    REPO_ROOT / "data" / "pilots" / "pilot_recommendations_2026-07-01.json"
)
FORWARD_RV_JSONL = REPO_ROOT / "data" / "paper_sleeves" / "forward_replacement_value.jsonl"
PRIOR_LOG_JSON = REPO_ROOT / "experiments" / "logs" / "exp-20260628-010.json"
...
def recommendation_summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for rec in payload.get("recommendations", []) or []:
        ...
        per_pilot[pilot] = {
            ...
            "row_count": len(rows),
            "entry_date_present": sum(1 for _, row in rows if row.get("entry_date")),
            "target_price_present": sum(1 for _, row in rows if row.get("target_price")),
...
def prior_scorecard_comparison(current_rows: list[dict[str, Any]]) -> dict[str, Any]:
    prior = read_json(PRIOR_LOG_JSON, {})
    prior_rows = ((prior.get("scorecard") or {}).get("scorecards") or []) if isinstance(prior, dict) else []
    prior_by_pilot = {str(row.get("pilot") or ""): row for row in prior_rows if row.get("pilot")}
    current_by_pilot = {str(row.get("pilot") or ""): row for row in current_rows if row.get("pilot")}
    pilot_deltas: dict[str, Any] = {}
    for pilot in sorted(set(prior_by_pilot) | set(current_by_pilot)):
        before = prior_by_pilot.get(pilot) or {}
        after = current_by_pilot.get(pilot) or {}
        pilot_deltas[pilot] = {
            "prior_closed_trades": safe_int(before.get("closed_trades")),
            "current_closed_trades": safe_int(after.get("closed_trades")),
            "closed_trade_delta": safe_int(after.get("closed_trades")) - safe_int(before.get("closed_trades")),
            "prior_verdict": before.get("verdict"),
            "current_verdict": after.get("verdict"),
            "prior_rv_vs_spy_usd": round_or_none(before.get("rv_vs_spy_usd"), 2),
            "current_rv_vs_spy_usd": round_or_none(after.get("rv_vs_spy_usd"), 2),
            "prior_book_max_drawdown_pct": round_or_none(before.get("book_max_drawdown_pct"), 4),
            "current_book_max_drawdown_pct": round_or_none(after.get("book_max_drawdown_pct"), 4),
        }
    prior_total = sum(safe_int(row.get("closed_trades")) for row in prior_rows)
    current_total = sum(safe_int(row.get("closed_trades")) for row in current_rows)
    return {
        "prior_experiment_id": "exp-20260628-010",
        "prior_log": repo_rel(PRIOR_LOG_JSON),
        "prior_log_loaded": PRIOR_LOG_JSON.exists(),
        "prior_total_closed_scorecard_rows": prior_total,
        "current_total_closed_scorecard_rows": current_total,
        "total_closed_row_delta": current_total - prior_total,
        "pilot_deltas": pilot_deltas,
    }


def metric_sentence(row: dict[str, Any], comparison: dict[str, Any]) -> str:
    pilot = str(row.get("pilot") or "fundamental_growth_rs")
    delta = ((comparison.get("pilot_deltas") or {}).get(pilot) or {}).get("closed_trade_delta")
    return (
        f"{pilot} now has {row.get('closed_trades')} closed rows "
        f"({delta:+d} versus exp-20260628-010), RV vs SPY ${row.get('rv_vs_spy_usd'):,.2f}, "
        f"RV vs QQQ ${row.get('rv_vs_qqq_usd'):,.2f}, and book drawdown "
        f"{(safe_float(row.get('book_max_drawdown_pct')) or 0.0) * 100:.1f}%."
    )
...
def build_payload() -> dict[str, Any]:
    ...
    comparison = prior_scorecard_comparison(evaluation["per_pilot"])
    ...
    scorecard_fields_ok = bool(scorecards) and all(
        ...
        and row.get("rv_vs_qqq_usd") is not None
    ...
    observed_success = bool(
        evaluation["kill_rule_reproducible"]
        and evaluation["blocked_new_entries_ok"]
        and comparison["total_closed_row_delta"] > 0
    )
    status = "observed_only"
    decision = (
        "observed_only_pilot_scorecard_kill_rule_reconfirmed_with_new_closed_rows"
        if observed_success
        else "blocked_pilot_scorecard_kill_rule_not_reproducible_or_no_new_rows"
    )
    failed_reasons: list[str] = []
    if not evaluation["kill_rule_reproducible"]:
        failed_reasons.append("kill_rule_not_reproducible")
    if not evaluation["blocked_new_entries_ok"]:
        failed_reasons.append("killed_pilot_new_entries_not_blocked")
    if comparison["total_closed_row_delta"] <= 0:
        failed_reasons.append("duplicate_readiness_without_count_progress")
    ...
    fundamental = next(
        (row for row in evaluation["per_pilot"] if row.get("pilot") == "fundamental_growth_rs"),
        {},
    )
    why_result = (
        "The precommitted scorecard reconfirmed the fundamental_growth_rs hard KILL "
        "with materially new settled evidence. "
        + metric_sentence(fundamental, comparison)
        + " The positive RV comparators are insufficient for promotion because the "
        "unchanged 15% book-drawdown ceiling is already breached; allocator_top1 "
        "and distribution_absorption remain COLLECTING."
    )
    post_run_reflection = {
        "why_result_happened": why_result,
        "forbidden_near_neighbor_retry": (
            "Do not retune graduate/kill thresholds, pilot notional, scorecard drawdown "
            "ceilings, or current open-row slices on the same 2026-07-01 pilot surface."
        ),
    ...
    payload = {
        ...
        "pre_run_questions": {
            ...
            "acceptance_standard": (
                ...
                "to be reproducible from closed rows, RV and drawdown with positive closed-row "
                "progress versus exp-20260628-010. It cannot promote live capital or accepted "
                "alpha without later Gate 1-4 evidence."
            ),
            "reproducibility": (
                "Runner reads fixed 2026-07-01 scorecard/recommendation artifacts, compares "
                "the prior exp-20260628-010 log, and writes deterministic closeout files."
            ),
        },
        ...
        "delta_metrics": {
            ...
            "prior_total_closed_scorecard_rows": comparison["prior_total_closed_scorecard_rows"],
            "total_closed_row_delta": comparison["total_closed_row_delta"],
        ...
        "gate2": {
            ...
            "entry_date_coverage_note": (
                "Current open recommendation rows are audited for entry_date; closed rows "
                "are validated through scorecard/forward-RV aggregates."
            ),
        ...
        "gate4": {
            ...
            "prior_scorecard_comparison": comparison,
        ...
        "calibration": {
            ...
            "surprise_level": "low" if observed_success else "medium",
            "surprise_note": (
                "The new closed-row evidence moved RV positive but did not change the "
                "precommitted kill decision because drawdown remains above the ceiling."
            ),
        },
        "production_impact": {
        ...
        "rejection_reason": (
            "No accepted alpha or live promotion: fundamental_growth_rs remains KILL "
            "because book drawdown breaches the unchanged ceiling; no pilot meets the "
            "precommitted promotion row-count and risk guard."
        ),
...
def build_card(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID}: pilot scorecard 2026-07-01 kill recheck",
        "",
        f"- Status: `{payload['status']}`",
        f"- Decision: `{payload['decision']}`",
        "- Production behavior changed: no",
        "- Accepted alpha: no",
        f"- Closed row progress: `{payload['delta_metrics']['prior_total_closed_scorecard_rows']}` -> `{payload['delta_metrics']['total_closed_scorecard_rows']}`",
        "",
        "## Result",
        "",
    ]
...
def persist(payload: dict[str, Any]) -> None:
    ...
    result = {
        ...
        "total_closed_row_delta": payload["delta_metrics"]["total_closed_row_delta"],

"""exp-20260501-001 breakout target extension.

Alpha search. Test one exit/lifecycle variable: whether all ``breakout_long``
signals should use a wider ATR target than the current regime-aware profile.
Entries, sizing, ranking, add-ons, slots, LLM/news replay, and trend targets
remain locked.
"""
...
EXPERIMENT_ID = "exp-20260501-001"
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / "breakout_target_extension.json"
...
def _metrics(result: dict) -> dict:
    ...
    target_breakouts = [
        trade for trade in breakout_trades
    ...
    return {
        ...
        "breakout_trade_count": len(breakout_trades),
        "breakout_target_exit_count": len(target_breakouts),
        "breakout_pnl": round(
            sum(float(t.get("pnl") or 0.0) for t in breakout_trades),
            2,
        ),
    }
...
def _install_breakout_target_override(target_mult: float | None):
    ...
    def wrapped(signals, features_dict, atr_target_mult=None):
        ...
        for sig in enriched:
            ...
            if sig.get("strategy") == "breakout_long" and atr:
                sig = risk_engine._retarget_signal_with_atr_mult(
                    sig,
                    atr,
                    target_mult,
                )
                sig["breakout_target_width_applied"] = target_mult
            adjusted.append(sig)
        return adjusted
...
def _run_variant(universe: list[str], target_mult: float | None) -> OrderedDict:
    original, _ = _install_breakout_target_override(target_mult)
    try:
        rows = OrderedDict()
        for label, cfg in WINDOWS.items():
            ...
            print(
                f"[{label}] breakout_target={target_mult or 'baseline'} "
                f"EV={m['expected_value_score']} PnL={m['total_pnl']} "
                f"SharpeD={m['sharpe_daily']} DD={m['max_drawdown_pct']} "
                f"breakouts={m['breakout_trade_count']} "
                f"target_breakouts={m['breakout_target_exit_count']}"
            )
        return rows
    finally:
...
def build_payload() -> dict:
    ...
    best_aggregate = aggregates[best_variant]
    accepted = _passes_gate4(best_aggregate)
    ...
    return {
        ...
        "lane": "alpha_search",
        "change_type": "exit_lifecycle_breakout_target_width",
        "hypothesis": (
            "Breakout winners may be clipped by the current regime-aware target "
            "width; a modest wider target could improve winner capture without "
            "adding new entries or filters."
        ),
        "parameters": {
            "single_causal_variable": "breakout_long ATR target width",
        ...
        "history_guardrails": {
            "not_technology_breakout_target_retry": True,
            "not_commodity_breakout_target_retry": True,
            "not_entry_or_ranking_change": True,
            "why_not_simple_repeat": (
                "This tests strategy-level breakout lifecycle width after the "
                "current accepted stack, not a sector-specific Technology or "
                "Commodity breakout target retry and not a breakout sorting key."
            ),
        },
        "rejection_reason": None if accepted else (
            "No tested breakout target width passed Gate 4 across the fixed windows."
        ),
        "next_retry_requires": [
            "Do not continue nearby breakout target-width tuning without event/news or lifecycle context.",
            "A valid retry needs a discriminator for which breakout winners deserve more room.",
            "Keep any promoted retry in shared risk policy to preserve production/backtest parity.",
        ],
        "related_files": [
            "quant/experiments/exp_20260501_001_breakout_target_extension.py",
            "data/experiments/exp-20260501-001/breakout_target_extension.json",
            "docs/experiments/logs/exp-20260501-001.json",
            "docs/experiments/tickets/exp-20260501-001.json",
        ],
    }
...
def main() -> int:
    ...
    TICKET_JSON.write_text(json.dumps({
        ...
        "title": "Breakout target extension",
        "summary": (

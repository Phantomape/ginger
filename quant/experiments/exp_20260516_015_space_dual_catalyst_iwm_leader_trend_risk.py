...
Tests one allocation variable on top of accepted exp-20260516-014: whether
source-diverse official Space trend signals with both customer and government
catalysts deserve another small default-off scalar only when the existing
Space IWM-relative state is smallcap_leader.
...
import exp_20260516_014_space_dual_catalyst_trend_risk as exp014


LOGGER = logging.getLogger(__name__)

EXPERIMENT_ID = "exp-20260516-015"
STEM = "space_dual_catalyst_iwm_leader_trend_risk"
BEFORE_EXPERIMENT_ID = "exp-20260516-014"
BEFORE_STEM = "space_dual_catalyst_source_diversity_trend_risk"
...
def _is_target_signal(signal: dict[str, Any], profile: dict[str, Any] | None) -> bool:
    return (
        exp014._is_dual_catalyst_profile(profile)
    ...
    *,
    ...
    def patched_scale_and_record(
        *,
        ...
        if not exp014._is_dual_catalyst_profile(profile):
            return
        _apply_scale(
            signal=signal,
            sizing=sizing,
            scalar=ACCEPTED_DUAL_CATALYST_TREND_SCALAR,
            portfolio_value=portfolio_value,
        ...
        if not _is_target_signal(signal, profile):
            return
        _apply_scale(
            signal=signal,
            sizing=sizing,
            scalar=dual_iwm_scalar,
            portfolio_value=portfolio_value,
            marker=MARKER,
            counts=counts,
        ...
        signal["space_dual_catalyst_iwm_leader_trend_scalar"] = dual_iwm_scalar
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "accepted_dual_catalyst_trend_scalar": ACCEPTED_DUAL_CATALYST_TREND_SCALAR,
        "space_dual_catalyst_iwm_leader_trend_scalar": dual_iwm_scalar,
    ...
    variant["dual_catalyst_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(dual_adjustments)
    )
    variant["dual_catalyst_iwm_leader_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["dual_catalyst_iwm_leader_counts_by_window"] = by_window_counts
    variant["dual_catalyst_iwm_leader_adjustment_summary"] = (
        exp041.source_diversity_exp._adjustment_summary(dual_iwm_adjustments)
    )
    variant["dual_catalyst_iwm_leader_adjustment_sample"] = (
        dual_iwm_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("dual_catalyst_iwm_leader_counts") or {}
    by_window_counts = variant.get("dual_catalyst_iwm_leader_counts_by_window") or {}
    ...
    scalar = float(
        variant["parameters"]["space_dual_catalyst_iwm_leader_trend_scalar"]
    )
    passed = bool(
        scalar != 1.0
        and changed_count > 0
        and len(tickers) >= MIN_ADJUSTED_TICKERS
        and len(windows) >= MIN_ADJUSTED_WINDOWS
    ...
    return {
        ...
        "reasons": {
            ...
            "adjusted_ticker_count_ok": len(tickers) >= MIN_ADJUSTED_TICKERS,
            "adjusted_window_count_ok": len(windows) >= MIN_ADJUSTED_WINDOWS,
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    selected_scalar = best["parameters"]["space_dual_catalyst_iwm_leader_trend_scalar"]
    return {
        "experiment_id": EXPERIMENT_ID,
        "date": payload["completed_at"],
        "hypothesis": payload["hypothesis"],
        "change_type": "alpha_search",
        "changed_variable": "space_dual_catalyst_iwm_leader_trend_scalar",
        "parameters": {
        ...
        "gate_answers": {
            "1_alpha_hypothesis": (
                "Risk allocation: accepted dual-catalyst source-diversity "
                "Space trend signals may still be under-sized only when "
                "IWM-relative small-cap appetite is confirmed."
            ),
            "2_prior_similar_experiments": [
                "exp-20260512-031 accepted broad IWM-relative Space risk.",
                "exp-20260513-020 accepted IWM-plus-peer-leader trend risk.",
                "exp-20260513-108/110 accepted source-diversity IWM and peer+IWM helpers.",
                "exp-20260516-014 accepted dual-catalyst source-diversity trend risk.",
                "No prior run isolated IWM-relative confirmation inside the accepted dual-catalyst source-diversity trend sleeve.",
            ],
            "3_single_causal_variable": (
                "Only the incremental dual-catalyst IWM-leader trend scalar "
                "changes; candidate pool, entries, exits, ranking, LLM/news, "
            ...
            "5_reproducibility": (
                ...
                "exp_20260516_015_space_dual_catalyst_iwm_leader_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the dual-catalyst IWM-leader trend scalar did not "
            "improve the fixed windows while satisfying PnL, drawdown, and "
            "cohort guardrails."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby dual-catalyst/IWM scalar interactions on "
        ...
        "why_not_other_changes": (
            ...
            "risk-appetite confirmation inside the accepted dual-catalyst field."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space dual-catalyst IWM-leader trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_dual_catalyst_iwm_leader_trend_scalar` on top of accepted "
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_dual_catalyst_iwm_leader_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_dual_iwm_signal_count']}`",
            f"- adjusted signals: `{gate['changed_dual_iwm_signal_count']}`",
            f"- changed tickers: `{gate['changed_tickers']}`",
            f"- changed windows: `{gate['changed_windows']}`",
            f"- adjusted counts: `{best['dual_catalyst_iwm_leader_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Dual-catalyst IWM-leader Space scalar "
            f"{best['parameters']['space_dual_catalyst_iwm_leader_trend_scalar']} "
            f"changed {gate['changed_dual_iwm_signal_count']} signals with "
            "aggregate EV delta "
...
def run() -> dict[str, Any]:
    ...
    dual_profile_field_check = exp014._field_check_dual_catalyst_profiles()
    ...
    payload = {
        ...
        "gate_results_by_scalar": [
            {
                "scalar": variant["parameters"][
                    "space_dual_catalyst_iwm_leader_trend_scalar"
                ],
                **variant["gate"],
            }
            for variant in variants
        ...
        "hypothesis": (
            ...
            "signals may still be under-sized only when IWM-relative small-cap "
            "risk appetite is confirmed."
        ),
        "changed_variable": "space_dual_catalyst_iwm_leader_trend_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate["eligible_dual_iwm_signal_count"],
                    "adjusted_signals": gate["changed_dual_iwm_signal_count"],

...
official Space trend candidates whose closed event-state profile is already
cash-positive and same-theme positive at 5d, and remains same-theme strong at
10d, deserve a conservative extra default-off risk scalar.

This is the mirror timing cohort to delayed absorption. It avoids LLM
soft-ranking, ticker-pool expansion, lifecycle changes, and nearby delayed
absorption scalar retunes.
"""
...
EXPERIMENT_ID = "exp-20260514-036"
STEM = "space_early_confirmation_trend_risk"
...
EARLY_CONFIRMATION_RISK_SCALARS = (1.0, 1.025, 1.05, 1.075, 1.10)
TARGET_STRATEGY = "trend_long"

MIN_5D_CASH = 0.0
MIN_5D_SAME_THEME = 0.0
MIN_10D_CASH = 0.0
MIN_10D_SAME_THEME = 500.0
...
def _early_confirmation_profile_gate() -> dict[str, Any]:
    ...
    for ticker, rows in sorted(rows_by_ticker.items()):
        ...
        avg_5d_same = _avg(
            [float(row["5d_same_theme_replacement_value"]) for row in rows]
        )
        ...
        passed = bool(
            ...
            and avg_5d_same is not None
            and avg_10d_cash is not None
            and avg_10d_same is not None
            and avg_5d_cash > MIN_5D_CASH
            and avg_5d_same > MIN_5D_SAME_THEME
            and avg_10d_cash > MIN_10D_CASH
            and avg_10d_same >= MIN_10D_SAME_THEME
        )
        profiles[ticker] = {
            "passed": passed,
    ...
    return {
        ...
        "profiles": profiles,
        "thresholds": {
            "min_avg_5d_cash_relative_pnl": MIN_5D_CASH,
            "min_avg_5d_same_theme_replacement_value": MIN_5D_SAME_THEME,
...
def _run_exp030_stack_variant(
    label: str,
    *,
    early_confirmation_scalar: float,
    early_confirmation_gate: dict[str, Any],
    ...
    early_tickers = set(early_confirmation_gate["target_tickers"])
    early_profiles = early_confirmation_gate["profiles"]
    ...
    def patched_scale_and_record(
        *,
        ...
        if ticker in early_tickers:
            _scale_and_record_extra(
                signal=signal,
                sizing=sizing,
                scalar=early_confirmation_scalar,
                portfolio_value=portfolio_value,
                marker="space_early_confirmation_trend_risk",
                counts=counts,
                adjustments=early_adjustments,
                profile=early_profiles.get(ticker),
            )
            signal["space_early_confirmation_trend_bucket"] = True
            signal["space_early_confirmation_trend_scalar"] = early_confirmation_scalar
            signal["space_early_confirmation_profile"] = early_profiles.get(ticker)
    ...
    early_counts = {
        ...
        if "space_early_confirmation_trend_risk" in key
    }
    delayed_counts = {
        key: value
        for key, value in sorted(counts.items())
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_early_confirmation_trend_scalar": early_confirmation_scalar,
        "early_confirmation_target_tickers": sorted(early_tickers),
        "early_confirmation_thresholds": early_confirmation_gate["thresholds"],
    }
    variant["delayed_absorption_counts"] = delayed_counts
    variant["early_confirmation_counts"] = early_counts
    variant["early_confirmation_counts_by_window"] = by_window_counts
    variant["delayed_absorption_adjustment_summary"] = (
        source_diversity_exp._adjustment_summary(delayed_adjustments)
    )
    variant["early_confirmation_adjustment_summary"] = (
        source_diversity_exp._adjustment_summary(early_adjustments)
    )
    variant["early_confirmation_adjustment_sample"] = early_adjustments[:25]
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("early_confirmation_counts") or {}
    changed_count = int(
        counts.get("space_early_confirmation_trend_risk_changed_signal", 0)
    )
    eligible_count = int(
        counts.get("space_early_confirmation_trend_risk_eligible_signal", 0)
    )
    scalar = float(variant["parameters"]["space_early_confirmation_trend_scalar"])
    return {
        "aggregate_delta_vs_before": aggregate_delta,
        "by_window_delta_vs_before": by_window_delta,
        "ev_improved_windows": ev_improvements,
        "ev_regressed_windows": ev_regressions,
        "eligible_early_confirmation_signal_count": eligible_count,
        "changed_early_confirmation_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space early-confirmation trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_early_confirmation_trend_scalar` for official Space "
            "`trend_long` signals whose closed event-state profile has positive "
            "average 5d cash and same-theme reaction plus 10d same-theme value "
            "at or above the accepted $500 strength floor. Candidate pool, "
            "ranking, targets, stops, LLM/news, and accepted exp030 stack stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        f"- Decision: `{payload['decision']}`",
        f"- Best scalar: `{best['parameters']['space_early_confirmation_trend_scalar']}`",
        (
        ...
        (
            "- Early-confirmation signals changed: "
            f"`{gate['changed_early_confirmation_signal_count']}` of "
            f"`{gate['eligible_early_confirmation_signal_count']}` eligible"
        ),
        (
            "- Target tickers: "
            f"`{', '.join(best['parameters']['early_confirmation_target_tickers'])}`"
        ),
        "",
    ...
    for name, delta in gate["by_window_delta_vs_before"].items():
        ...
        counts = best["early_confirmation_counts_by_window"][name]
        adjusted = counts.get(
            "space_early_confirmation_trend_risk_changed_signal",
            0,
        )
        lines.append(
...
def run() -> dict[str, Any]:
    ...
    early_confirmation_gate = _early_confirmation_profile_gate()

    variants = [
        _run_exp030_stack_variant(
            f"{STEM}_{str(scalar).replace('.', '_')}",
            early_confirmation_scalar=scalar,
            early_confirmation_gate=early_confirmation_gate,
        ...
        for scalar in EARLY_CONFIRMATION_RISK_SCALARS
    ]
    before = variants[0]
    ...
    runtime_gate = {
        "passed": best["gate"]["eligible_early_confirmation_signal_count"] > 0,
        "eligible_signal_count": best["gate"][
            "eligible_early_confirmation_signal_count"
        ],
        "required_runtime_fields": [
            ...
            "horizons.5d.same_theme_replacement_value",
            "horizons.10d.cash_relative_pnl",
            "horizons.10d.same_theme_replacement_value",
            "signal.strategy",
            "signal.sizing.shares_to_buy",
        ],
        "sample_rows": best.get("early_confirmation_adjustment_sample", [])[:10],
    }
    gate2 = {
        ...
        "early_confirmation_profile": early_confirmation_gate,
        "early_confirmation_runtime_state": runtime_gate,
    }
    gate2["passed"] = all(
        [
            ...
            forward_gate["passed"],
            delayed_gate["passed"],
            early_confirmation_gate["passed"],
            runtime_gate["passed"],
        ]
    )
    ...
    status = (
        "accepted_default_off_space_early_confirmation_trend_risk"
        if decision == "accepted"
        else "rejected_space_early_confirmation_trend_risk"
    )
    rejection_reason = ""
    if decision == "rejected":
        rejection_reason = (
            "The early-confirmation trend state did not clear Gate 4 versus "
            "the accepted exp030 stack. Do not retry this exact fast-absorption "
            "timing cohort without new closed forward rows."
        )
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Space official catalysts with immediate 5d cash and same-theme "
            "confirmation may deserve conservative trend risk allocation if "
            "the 10d same-theme replacement profile stays above the accepted "
            "$500 strength floor. This tests a fast-absorption timing cohort "
            "without adding tickers, LLM authority, or lifecycle changes."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_early_confirmation_trend_scalar",
        "single_causal_variable": (
            "extra trend_long risk scalar for the early-confirmed forward "
            "profile; all accepted Space helpers through exp030 stay fixed"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                "risk allocation: fast 5d confirmation plus strong 10d "
                "same-theme replacement may identify trend candidates worth "
                "incremental default-off Space risk."
            ),
            "q2_prior_experiments": [
                "exp-20260514-030 accepted delayed absorption: weak 5d but strong 10d same-theme evidence.",
                "exp-20260514-031 rejected broadening delayed absorption beyond the source-diverse stack.",
                "exp-20260514-033 rejected a broad-benchmark substitute for same-theme strength.",
                "Nearby source-diversity, company-source, and delayed scalars are anti-repeat families.",
            ],
            "q3_single_causal_variable": (
                "Only this early-confirmed trend scalar changes; no entry, "
                "exit, ranking, target, LLM/news, or live-slot change."
            ),
            "q4_acceptance_standard": (
                ...
                "changed early-confirmation signals."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            "tested_scalars": list(EARLY_CONFIRMATION_RISK_SCALARS),
            "selected_scalar": best["parameters"][
                "space_early_confirmation_trend_scalar"
            ],
            "early_confirmation_target_tickers": early_confirmation_gate[
                "target_tickers"
            ],
            "early_confirmation_thresholds": early_confirmation_gate["thresholds"],
            "locked_variables": [
        ...
        "why_not_other_changes": (
            ...
            "new closed forward timing state using already logged 5d and 10d "
            "replacement values."
        ),
        "known_risks": [
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "target_tickers": result["parameters"]["early_confirmation_target_tickers"],
        "aggregate_delta_vs_before": result["best_variant_gate"][
            "aggregate_delta_vs_before"
        ],
        "eligible_early_confirmation_signal_count": result["best_variant_gate"][
            "eligible_early_confirmation_signal_count"
        ],
        "changed_early_confirmation_signal_count": result["best_variant_gate"][
            "changed_early_confirmation_signal_count"
        ],
    }

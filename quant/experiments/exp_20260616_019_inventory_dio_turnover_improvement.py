"""exp-20260616-019: inventory DIO/turnover improvement scout.
...
annual days-in-inventory outstanding (DIO) is improving year over year because
inventory is turning faster versus CostOfRevenue, while COGS is not materially
contracting. This should separate true sell-through from the broader rejected
inventory/revenue leanness field.

Why this is a materially new free-data edge: InventoryNet is a balance-sheet
INSTANT fact (no FY duration) while CostOfRevenue is a filed annual flow. This
runner matches InventoryNet to the current, prior, and pre-prior fiscal-year
ends and computes current/prior DIO from average inventory divided by annual
COGS. The playbook explicitly named days-inventory-outstanding trajectory as a
valid materially different PIT discriminator after exp-20260616-018 rejected
raw inventory/revenue leanness on old_thin regression and drawdown drift.
...
private scout because CostOfRevenue + three inventory-point coverage is not yet
proven sufficient across the three canonical windows.
...
EXPERIMENT_ID = "exp-20260616-019"
STEM = "inventory_dio_turnover_improvement"
TRIAL_FAMILY = "inventory_dio_turnover_improvement_candidate_pool"
TRIAL_VARIANT_ID = "inventory_dio_turnover_improvement_top1_next_open_10d_v1"
CHANGED_VARIABLE = "inventory_dio_turnover_improvement_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260616_019_{STEM}.json"
...
MAX_CURRENT_DIO_DAYS = 240.0
MIN_DIO_IMPROVEMENT_DAYS = 5.0
...
PREDICTION = {
    ...
    "confidence_reason": (
        "The latest inventory/revenue scout was directionally strong but failed "
        "old_thin and drawdown; the playbook explicitly names days-inventory-"
        "outstanding trajectory as the materially different PIT evidence needed. "
        "This tests InventoryNet plus CostOfRevenue turnover rather than another "
        "inventory/revenue threshold. Risk is high because CostOfRevenue coverage "
        "may be thin and the field may still relabel liquid momentum."
    ),
    "recorded_at": "2026-06-16T17:03:27+00:00",
}
...
PRODUCTION_IMPACT = {
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            ...
            "FY CostOfRevenue, missing pre-prior inventory point, stale facts, missing "
    ...
    "parity_note": (
        ...
        "SEC InventoryNet instant mapping, filed-date PIT CostOfRevenue annual "
        "facts, DIO/turnover improvement gate, liquid SPY-relative confirmation, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts annual DIO falling year over year "
        "(average InventoryNet / CostOfRevenue improving = faster inventory "
        "turnover and cleaner demand sell-through), with non-collapsing COGS and "
    ...
    "2_history_check": {
        "exp-20260528-019": (
            ...
            "standalone candidate-pool source testing DIO/turnover trajectory, "
            "not inventory/revenue stability."
        ),
        "exp-20260613-031": (
            "Rejected operating-efficiency (asset turnover = revenue/assets). "
            "This run tests inventory turnover/DIO specifically, a demand-side "
            "balance-sheet field tied to COGS, not total asset turnover."
        ),
        "exp-20260615-002": (
        ...
        "exp-20260616-018": (
            ...
            "Its closeout and the playbook allow DIO trajectory as the materially "
            "different inventory-quality evidence."
        ),
        "exp-20260615-016": (
    ...
    "5_reproducibility": (
        ...
        "exp_20260616_019_inventory_dio_turnover_improvement.py"
    ),
}
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        cogs_facts = rd._raw_annual_facts(usgaap, COGS_TAGS)
        if not inventory_facts:
            stats["tickers_missing_raw_inventory"] += 1
            continue
        if not cogs_facts:
            stats["tickers_missing_raw_annual_cogs"] += 1
            continue
        index[ticker] = {
        ...
        stats["raw_annual_cogs_fact_count"] += len(cogs_facts)
...
def _inventory_observation(
    ...
    if current_cogs is None:
        return None
    if base._days_between(asof, current_cogs["filed"]) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None
    ...
    prior_cogs = rd._latest_period_fact(
        facts["cogs"], asof=asof, before_end=current_cogs["end"]
    )
    ...
    if prior_inventory is None:
        return None
    pre_prior_cogs = rd._latest_period_fact(
        facts["cogs"], asof=asof, before_end=prior_cogs["end"]
    )
    if pre_prior_cogs is None:
        return None
    pre_prior_inventory = rd._latest_period_fact(
        facts["inventory"], asof=asof, end=pre_prior_cogs["end"]
    )
    if pre_prior_inventory is None:
        return None

    current_inventory_value = abs(float(current_inventory["value"]))
    prior_inventory_value = abs(float(prior_inventory["value"]))
    pre_prior_inventory_value = abs(float(pre_prior_inventory["value"]))
    ...
    if (
        ...
        or pre_prior_inventory_value <= 0.0
    ):
        return None

    current_avg_inventory = (current_inventory_value + prior_inventory_value) / 2.0
    prior_avg_inventory = (prior_inventory_value + pre_prior_inventory_value) / 2.0
    if current_avg_inventory <= 0.0 or prior_avg_inventory <= 0.0:
        return None

    current_dio = 365.0 * current_avg_inventory / current_cogs_value
    prior_dio = 365.0 * prior_avg_inventory / prior_cogs_value
    ...
    return {
        ...
        "pre_prior_inventory_value": _round(pre_prior_inventory_value, 2),
        ...
        "pre_prior_period_end": pre_prior_inventory["end"],
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "INVENTORY_DIO_TURNOVER_PAPER",
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_inventory_dio_turnover_improvement"
        if gate["passed"]
        else "rejected_inventory_dio_turnover_improvement_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The inventory DIO/turnover-improvement source cleared the numeric "
            "three-window replay screen, but remains only a replay lead because "
            "no shared daily/backtest helper was promoted."
        )
    else:
        interpretation = (
            "The inventory DIO/turnover-improvement source did not clear Gate 4 "
            f"(failed: {', '.join(gate4['failed_reasons']) or 'none'}). Do not "
            "promote or tune this fixed DIO/turnover-quality bundle on the same "
            "frozen windows."
        )
    payload.update(
        {
            ...
            "new_evidence_type": "raw_sec_companyfacts_inventory_dio_turnover_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        ...
        "INSTANT fact (no FY duration) and matched to annual CostOfRevenue "
        "ending the same fiscal-year date; both are known only by filed date "
        "(<= signal date). Current DIO uses average current/prior inventory "
        "over current COGS, prior DIO uses average prior/pre-prior inventory "
        "over prior COGS, and the rule requires DIO to improve year over year. "
    ...
    payload["gate2"]["runtime_fields"] = [
        ...
        "raw SEC companyfacts annual CostOfRevenue facts",
    ...
    payload["next_evidence_needed"] = (
        ...
        "raw-materials decomposition, or closed forward replacement-value rows. "
        "Do not sweep the inventory/COGS tag list, DIO threshold, COGS-growth "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping the inventory/COGS tag list, DIO "
            "improvement threshold, COGS-growth floor, annual fact freshness, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Inventory DIO Turnover Improvement",
            "",

"""exp-20260630-003: options forward ledger refresh for 2026-06-29.

Measurement repair only. A new OnclickMedia options snapshot for 2026-06-29 is
present after exp-20260629-014. This runner materializes only the new pending
observation rows so future options demand/protection work can wait for closed
cash/SPY/QQQ replacement outcomes without changing strategy behavior.
"""
...
OWNER = "alpha-explore"
SLUG = "options_forward_ledger_refresh_20260629"
RUNNER = f"quant/experiments/exp_20260630_003_{SLUG}.py"
RUNNER_COMMAND = ".\\.venv\\Scripts\\python.exe -B " + RUNNER.replace("/", "\\")

DATA_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = DATA_DIR / f"exp_20260630_003_{SLUG}.json"
DELTA_LEDGER_JSONL = DATA_DIR / "options_forward_observation_ledger_delta_20260629.jsonl"
...
TARGET_QUOTE_DATES = ("2026-06-29",)

HYPOTHESIS = (
    "Alpha-enabling options forward ledger refresh: the new 2026-06-29 "
    "OnclickMedia snapshot should add PIT pending rows for future options "
    "demand/protection attribution without changing strategy behavior."
)
ALPHA_HYPOTHESIS = (
...
TRIAL_VARIANT_ID = "post_exp014_20260629_pending_rows_v1"
CHANGED_VARIABLE = "onclickmedia_options_forward_observation_ledger_refresh_20260629_v1"
NEW_EVIDENCE_TYPE = "new_forward_options_snapshot_rows"
NEW_EVIDENCE_AXIS = (
    "One new non-empty OnclickMedia options snapshot date after exp-20260629-014 "
    "(2026-06-29) creates pending forward observation rows. This is not an "
    "options threshold retune or an observed-only reslice of the same closed rows."
)
...
ALLOWED_WRITE_SCOPE = [
    RUNNER,
    f"data/experiments/{EXPERIMENT_ID}/exp_20260630_003_{SLUG}.json",
    f"data/experiments/{EXPERIMENT_ID}/options_forward_observation_ledger_delta_20260629.jsonl",
...
def load_prediction() -> dict[str, Any]:
    ...
    return prediction or {
        ...
        "confidence_reason": (
            "The 2026-06-29 chain file is present and non-empty; prior adjacent "
            "snapshots normalized cleanly."
        ),
        "recorded_at": utc_now(),
    }
...
def calibration(prediction: dict[str, Any], accepted: bool, failed: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "actual_decision": (
            "accepted_measurement_repair_options_forward_ledger_refreshed_20260629"
            if accepted
            else "blocked_options_forward_ledger_refresh_20260629"
        ),
        ...
        "surprise_note": (
            "The 2026-06-29 snapshot normalized cleanly into delta observation rows."
            if accepted
            else "The 2026-06-29 snapshot could not be cleanly normalized into a delta ledger."
        ),
    }
...
def build_payload(delta_rows: list[dict[str, Any]], refresh: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "pre_run_questions": {
            ...
            "2_history_check": {
                "exp-20260623-009": "Accepted original options observation ledger through 2026-06-22.",
                "exp-20260624-020": "Accepted 2026-06-23 pending delta rows.",
                "exp-20260629-014": "Accepted 2026-06-24..2026-06-26 pending delta rows.",
                "exp-20260625-001": "Rejected options demand attribution; forbids threshold retries without materially more closed rows.",
                "novelty_gate": "Reservation passed without override for a new pending row date.",
            },
            "3_single_policy_bundle": (
                "Normalize only the 2026-06-29 OnclickMedia options snapshot "
                "into experiment-owned pending observation rows."
            ),
            "4_acceptance_standard": (
                "Accept as measurement repair only if prior ledgers exist, the "
                "target snapshot has rows, required raw fields cover at least "
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "The 2026-06-29 OnclickMedia snapshot has enough normalized "
                "contract fields to create pending ticker-date observations. "
                "This increases future options evidence capacity but still "
                "provides no accepted options alpha because the new rows need "
                "closed replacement-value outcomes."
                if accepted
                else "The 2026-06-29 options snapshot did not satisfy the fixed ledger-refresh checks."
            ),
            ...
            "new_evidence_required": (
                "Wait for these 2026-06-29 rows to close with cash/SPY/QQQ "
                "replacement value, add PIT borrow/loan availability context, or "
                "backfill historical PIT options chains with vendor/as-of controls "
                "before another options alpha claim."
            ),
        },
        "related_files": [
            RUNNER,
            ...
            "quant/experiments/exp_20260629_014_options_forward_ledger_refresh_20260624_20260626.py",
...
def compact_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    record["refresh_summary"] = {
        ...
        "duplicate_new_observation_ids": payload["refresh_summary"]["duplicate_new_observation_ids"],

...
Tests one causal variable on top of the accepted exp-20260513-014 default-off
Space stack: an extra risk scalar only when a signal has a production-visible
government-space-contract event profile and is also leading the official Space
peer basket. This avoids broad defense-budget source scaling and noisy ticker
expansion.
"""
...
from exp_20260513_014_space_customer_source_peer_leader_risk import (  # noqa: E402
    ...
    _install_space_policy as _install_accepted_exp014_policy,
    _is_peer_leader,
    _run_variant as _run_accepted_exp014_variant,
)
...
STEM = "space_government_contract_peer_leader_risk"
ACCEPTED_CUSTOMER_SOURCE_PEER_LEADER_RISK_SCALAR = 1.10
TARGET_EVENT_FIELD = "government_space_contract"
TARGET_SOURCE_TYPES = ("official_or_primary_release", "official_government_release")
EXCLUDED_SEMANTIC_BUCKETS = ("attention_only",)
GOVERNMENT_CONTRACT_PEER_LEADER_RISK_SCALARS = (
    0.75,
    0.90,
    1.00,
    1.025,
...
def _field_check_government_contract_profile() -> dict[str, Any]:
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), 1):
        ...
        if semantic_bucket in EXCLUDED_SEMANTIC_BUCKETS:
            continue
        if TARGET_EVENT_FIELD not in fields:
            continue
    ...
    return {
        "passed": not missing_fields and bool(serialized_profiles),
        "path": str(path.relative_to(PROJECT_ROOT)),
        "event_seed_count": len(rows),
        "target_definition": (
            "official Space ticker with government_space_contract from an "
            "official non-attention source"
        ),
...
def _install_space_policy(
    government_contract_peer_leader_scalar: float,
    ...
    installed = _install_accepted_exp014_policy(
    ...
    target_tickers = set(government_contract_gate["target_tickers"])
    profiles = government_contract_gate["profiles"]
    government_contract_adjustments: list[dict[str, Any]] = []
    government_contract_counts = Counter()
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if eligible and sizing:
                government_contract_counts["eligible_signal"] += 1
                shares_before = int(sizing.get("shares_to_buy") or 0)
                _scale_sizing(
                    sizing,
                    government_contract_peer_leader_scalar,
                    portfolio_value,
                    "space_government_contract_peer_leader_risk",
                )
                government_contract_adjustments.append(
                    {
                        "ticker": ticker,
                        "strategy": signal.get("strategy"),
                        "marker": "space_government_contract_peer_leader_risk",
                        ...
                        "scalar": government_contract_peer_leader_scalar,
                ...
                signal = {
                    **signal,
                    "sizing": sizing,
                    "space_government_contract_profile": profiles.get(ticker),
                    "space_government_contract_peer_leader_eligible": True,
                }
            out.append(signal)
        return out
...
def _run_variant(
    ...
    try:
        ...
        for label, window in WINDOWS.items():
            before_government = len(government_contract_adjustments)
            before_source_peer = len(source_peer_leader_adjustments)
            ...
            by_window[label] = {
                ...
                "space_government_contract_peer_leader_adjustment": (
                    _adjustment_summary(government_contract_adjustments[before_government:])
                ),
                "space_customer_source_peer_leader_adjustment": _adjustment_summary(
                    source_peer_leader_adjustments[before_source_peer:]
                ),
                ...
                "space_government_contract_peer_leader_signal_counts": dict(
                    sorted(government_contract_counts.items())
                ),
    ...
    return {
        "variant": name,
        "target_definition": (
            "government_space_contract event profile and peer momentum leader"
        ),
        "target_tickers": government_contract_gate["target_tickers"],
...
def _gate(
    ...
    adjusted_count = sum(
        row["space_government_contract_peer_leader_adjustment"]["adjusted_signal_count"]
    ...
    passed = (
        ...
        and variant["space_government_contract_peer_leader_risk_scalar"] != 1.0
    )
    return {
        "passed": passed,
        ...
        "space_government_contract_peer_leader_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space government-contract peer-leader risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: risk scalar for official Space government-contract "
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | "
            f"{variant['space_government_contract_peer_leader_risk_scalar']:.3f} | "
            ...
            f"{gate['space_government_contract_peer_leader_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
            ...
            (
                ...
                "dPnL | Trades | Max DD | Survival | Gov-contract peer-leader signals |"
            ),
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "## Field Checks",
            "",
            json.dumps(payload["gate2"]["government_contract_profile"], sort_keys=True),
            "",
...
def run() -> dict[str, Any]:
    ...
    before = _run_accepted_exp014_variant(
        "accepted_exp014_customer_source_peer_leader_stack",
        ACCEPTED_CUSTOMER_SOURCE_PEER_LEADER_RISK_SCALAR,
        source_gate,
        multi_event_gate,
        liquidity_gate,
    ...
    variants = {}
    for scalar in GOVERNMENT_CONTRACT_PEER_LEADER_RISK_SCALARS:
        name = f"government_contract_peer_leader_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_government_contract_peer_leader_risk"
        if accepted
        else "rejected_space_government_contract_peer_leader_risk"
    )
    interpretation = (
        "The government-contract plus peer-leader scalar improved the accepted "
        ...
        if accepted
            ...
            "The government-contract plus peer-leader scalar did not clear the "
            "three-window gate on top of exp-20260513-014. Do not promote this "
            "intersection as a shared helper on these frozen snapshots."
        )
    )
    ...
    payload = {
        ...
        "changed_variable": "space_government_contract_peer_leader_risk_scalar",
        "single_causal_variable": (
            "risk scalar for official Space signals with government_space_contract "
            "event profile whose own 20d momentum is above the official Space basket"
        ),
        "hypothesis": (
            "Space replacement value should be stronger when an official government "
            "contract catalyst lines up with in-basket relative strength. Broad "
            "defense-budget source scaling had too much drawdown cost, so this "
            "tests the narrower production-visible intersection without changing "
            "pool membership, rankings, exits, LLM/news logic, or live slots."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: scale official Space government-contract signals "
                "only when they are peer momentum leaders."
            ),
            "2_history_check": {
                "exp-20260512-040": (
                    "Rejected broad defense-budget source scalar because drawdown "
                    "cost exceeded Gate 4. This run adds the peer-leader condition "
                    "and includes official NASA/government contract profiles."
                ),
                "exp-20260512-944": (
                    "Rejected primary-authority customer-source scalar. This run "
                    "uses a different event field, government_space_contract, and "
                    "requires peer leadership."
                ),
                "exp-20260513-002": (
                    "Rejected customer-win hard pool pruning. This run keeps the "
                    "pool fixed and changes only risk allocation."
                ),
                "exp-20260513-014": (
                    "Accepted customer-source peer-leader 1.10x helper; this is "
                    "the fixed before state and is not retuned."
                ),
            },
            "3_single_causal_variable": (
                "space_government_contract_peer_leader_risk_scalar. Candidate pool, "
            ...
            "4_acceptance_standard": (
                ...
                "EV/PnL versus exp-014 accepted stack, at least 2/3 improved EV "
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260513_015_space_government_contract_peer_leader_risk.py"
            ),
        },
        "parameters": {
            ...
            "excluded_semantic_buckets": list(EXCLUDED_SEMANTIC_BUCKETS),
            "target_tickers": government_contract_gate["target_tickers"],
            "target_profiles": government_contract_gate["profiles"],
            "peer_leader_definition": (
                "ticker momentum_20d_pct > equal-weight official Space basket "
                "momentum_20d_pct"
            ),
            "tested_government_contract_peer_leader_scalars": list(
                GOVERNMENT_CONTRACT_PEER_LEADER_RISK_SCALARS
            ),
            "accepted_before_experiment": "exp-20260513-014",
            "accepted_customer_source_peer_leader_risk_scalar": (
                ACCEPTED_CUSTOMER_SOURCE_PEER_LEADER_RISK_SCALAR
            ),
            "accepted_customer_source_peer_leader_tested_scalars": list(
                CUSTOMER_SOURCE_PEER_LEADER_RISK_SCALARS
            ),
            "accepted_multi_event_depth_risk_scalar": (
                ACCEPTED_MULTI_EVENT_DEPTH_RISK_SCALAR
        ...
        "backtest_protocol": (
            ...
            "The accepted_before variant reproduces exp-20260513-014 policy semantics."
        ),
        "gate1": {
            ...
            "known_bias": (
                ...
                "from a 2026-05-10 research universe. Event profiles and peer-state "
                "fields are production-visible, but any accepted Space change must "
        ...
        "llm_metrics": {
            ...
            "why_not_llm_soft_ranking": (
                ...
                "production-visible government-contract and peer-momentum metadata."
            ),
        },
        "production_impact": {
        ...
        "next_evidence_needed": (
            "If rejected, do not promote government-contract peer-leader scaling on "
            "these frozen snapshots. Future Space alpha should either wait for "
            "closed forward replacement value by catalyst/source bucket or test "
            "another production-visible catalyst-quality field with broader coverage."
        ),
        "related_files": [
            "quant/experiments/exp_20260513_015_space_government_contract_peer_leader_risk.py",
            "data/experiments/exp-20260513-015/space_government_contract_peer_leader_risk.json",
            "docs/experiments/logs/exp-20260513-015.json",
            "docs/experiments/tickets/exp-20260513-015.json",
            "docs/experiments/artifacts/exp-20260513-015_space_government_contract_peer_leader_risk.md",
            "docs/experiment_log.jsonl",
        ],
        "why_not_other_changes": (
            "LLM soft-ranking is data-limited; noisy ticker additions, "
            "watch-liquidity TQS/peer/strategy scopes, broad defense-budget source "
            "scalars, primary-authority source scalars, contract-profile scalars, "
            "generic peer-leader risk, and cumulative risk caps were already "
            "rejected or underpowered. This tests one new government-contract plus "
            "peer-leadership intersection."
        ),
    }
    return payload

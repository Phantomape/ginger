"""exp-20260512-016: Space basket breadth-conditioned risk.

Tests whether the accepted Space basket-positive 1.10x risk scalar should only
apply when the official Space operating basket has broad positive participation,
not merely a positive equal-weight average pulled by a few tickers. This starts
from the accepted exp-20260512-013 default-off Space stack and changes only the
minimum number of positive official Space basket members required for the
basket-positive risk top-up.
"""
...
EXPERIMENT_ID = "exp-20260512-016"
STEM = "space_basket_breadth_risk"
ACCEPTED_SPACE_PEER_NONLEADER_BREAKOUT_RISK_SCALAR = 0.0
SPACE_BASKET_POSITIVE_BREADTH_MIN_COUNTS = (2, 3, 4, 5, 6)
...
def _basket_breadth_state(basket: dict[str, Any]) -> dict[str, Any]:
    values = basket.get("values") or {}
    positive_tickers = [
        ticker for ticker, value in values.items() if value is not None and value > 0
    ]
    nonpositive_tickers = [
        ticker for ticker, value in values.items() if value is not None and value <= 0
    ]
    return {
        "positive_count": len(positive_tickers),
        "available_count": len(values),
        "positive_tickers": sorted(positive_tickers),
        "nonpositive_tickers": sorted(nonpositive_tickers),
    }


def _breadth_allows_topup(
    basket: dict[str, Any],
    min_positive_count: int | None,
) -> bool:
    if basket.get("state") != "positive":
        return False
    if min_positive_count is None:
        return True
    return (_basket_breadth_state(basket)["positive_count"] or 0) >= min_positive_count


def _basket_adjustment_row(
    signal: dict[str, Any],
    sizing: dict[str, Any],
    shares_before: int,
    marker: str,
    ...
    return {
        **_adjustment_row(signal, sizing, shares_before, scalar, marker),
        "space_basket_positive_count": signal.get("space_basket_positive_count"),
        "space_basket_available_count": signal.get("space_basket_available_count"),
        "space_basket_positive_tickers": signal.get("space_basket_positive_tickers"),
        "space_basket_nonpositive_tickers": signal.get(
            "space_basket_nonpositive_tickers"
        ),
    }


def _install_space_policy(min_positive_count: int | None):
    ...
    skipped_basket_adjustments: list[dict[str, Any]] = []
    ...
    def generate_wrapper(features_dict, *args, **kwargs):
        ...
        breadth = _basket_breadth_state(basket)
        day_counts[basket["state"]] += 1
        breadth_counts[f"{breadth['positive_count']}_of_{breadth['available_count']}"] += 1
        ...
        for signal in signals:
            ...
            signal["space_basket_positive_count"] = breadth["positive_count"]
            signal["space_basket_available_count"] = breadth["available_count"]
            signal["space_basket_positive_tickers"] = breadth["positive_tickers"]
            signal["space_basket_nonpositive_tickers"] = breadth["nonpositive_tickers"]
            signal["space_basket_breadth_allows_topup"] = _breadth_allows_topup(
                basket,
                min_positive_count,
            )
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in OFFICIAL_SPACE_TICKERS and sizing:
                ...
                if basket_positive and signal.get("space_basket_breadth_allows_topup"):
                    shares_after_base = int(sizing.get("shares_to_buy") or 0)
                    _scale_sizing(
                        sizing,
                        ACCEPTED_SPACE_BASKET_POSITIVE_SCALAR,
                        portfolio_value,
                ...
                elif basket_positive:
                    skipped_basket_adjustments.append(
                        _basket_adjustment_row(
                            signal,
                            sizing,
                            int(sizing.get("shares_to_buy") or 0),
                            "space_basket_positive_skipped_low_breadth",
                            1.0,
                        )
                    )
...
def _run_variant(name: str, min_positive_count: int | None) -> dict[str, Any]:
    ...
    (
        ...
        perfect_counts,
        day_counts,
    ) = _install_space_policy(min_positive_count)
    try:
        by_window = {}
        for label, window in WINDOWS.items():
            before_basket = len(basket_adjustments)
            before_skipped = len(skipped_basket_adjustments)
            ...
            by_window[label] = {
                ...
                "space_basket_positive_skipped_low_breadth": _adjustment_summary(
                    skipped_basket_adjustments[before_skipped:]
                ),
                ...
                "space_basket_breadth_day_counts": dict(sorted(breadth_counts.items())),
    ...
    return {
        "variant": name,
        "space_basket_positive_breadth_min_count": min_positive_count,
        "by_window": by_window,
...
def _field_check(variant: dict[str, Any]) -> dict[str, Any]:
    breadth_keys = Counter()
    for row in variant["by_window"].values():
        breadth_keys.update(row.get("space_basket_breadth_day_counts") or {})
    available_counts = set()
    for key in breadth_keys:
        try:
            available_counts.add(int(str(key).split("_of_")[1]))
        except (IndexError, ValueError):
            pass
    return {
        "field": SPACE_BASKET_MOMENTUM_FIELD,
        "source": "feature_layer trend features from augmented Space snapshots",
        "breadth_day_counts": dict(sorted(breadth_keys.items())),
        "available_counts": sorted(available_counts),
        "passed": bool(breadth_keys) and available_counts == {len(OFFICIAL_SPACE_TICKERS)},
    }
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    skipped_count = sum(
        row["space_basket_positive_skipped_low_breadth"]["adjusted_signal_count"]
        for row in variant["by_window"].values()
    )
    ...
    return {
        ...
        "space_basket_positive_skipped_low_breadth_count": skipped_count,
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space basket breadth risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: minimum positive official Space basket member count "
            "required before applying the accepted basket-positive 1.10x top-up."
        ),
    ...
    for name, variant in payload["variants"].items():
        ...
        min_count = variant["space_basket_positive_breadth_min_count"]
        min_label = "avg>0" if min_count is None else str(min_count)
        lines.append(
            f"| {name} | {min_label} | "
            ...
            f"{gate['space_basket_positive_skipped_low_breadth_count']} |"
        )
    lines.extend(
        [
            "",
    ...
    for label in WINDOWS:
        ...
        lines.append(
            ...
            "{trades} | {max_dd:.4f} | {survival:.4f} | {skipped} |".format(
                label=label,
    ...
    lines.extend(
        [
            ...
            "## Field Check",
            "",
            json.dumps(payload["gate2"]["basket_breadth"], sort_keys=True),
            "",
            "## Interpretation",
            "",
...
def run() -> dict[str, Any]:
    ...
    variants = {
        "accepted_exp013_stack": _run_variant("accepted_exp013_stack", None)
    }
    for min_count in SPACE_BASKET_POSITIVE_BREADTH_MIN_COUNTS:
        name = f"basket_breadth_min_{min_count}"
        variants[name] = _run_variant(name, min_count)
    ...
    decision = (
        "accepted_default_off_space_basket_breadth_risk"
        if accepted
        else "rejected_space_basket_breadth_risk"
    )
    interpretation = (
        "Requiring broad positive participation inside the official Space basket "
        ...
        if accepted
            ...
            "Official Space basket breadth did not improve the accepted "
            "exp-20260512-013 stack. Keep the accepted average-positive basket "
            "1.10x top-up unchanged; future Space work should use forward catalyst "
            "replacement value or a genuinely new event-quality field."
        )
    )
    ...
    payload = {
        ...
        "changed_variable": "space_basket_positive_breadth_min_count",
        "single_causal_variable": (
            "minimum number of positive official Space basket tickers required "
            "before the accepted basket-positive 1.10x risk top-up applies"
        ),
        "hypothesis": (
            "The accepted Space basket-positive risk scalar may be stronger when "
            "Space theme breadth is broad rather than concentrated in one or two "
            "tickers. A breadth requirement can improve risk allocation without "
            "adding tickers, changing targets/stops, or using underpowered LLM "
            "soft-ranking."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: apply the accepted Space basket-positive 1.10x "
                "risk top-up only when at least N official Space basket members "
                "have positive 20d momentum."
            ),
            "2_history_check": {
                "exp-20260511-115": (
                    "Accepted the average-positive official Space basket 1.10x "
                    "risk top-up; this tests breadth gating, not a nearby scalar."
                ),
                "exp-20260512-009": (
                    "Rejected peer-leader top-up due old_thin drawdown."
                ),
                "exp-20260512-013": (
                    "Accepted peer-nonleader breakout zero-risk; this is the "
                    "before stack."
                ),
                "exp-20260512-014": "Rejected peer-nonleader trend haircut.",
                "exp-20260512-015": "Rejected Space breakout 52w proximity haircut.",
                "exp-20260511-026": "Rejected mature satcom breadth pool extension.",
            },
            "3_single_causal_variable": (
                "space_basket_positive_breadth_min_count; candidate pool, accepted "
                "Space scalars, targets, stops, ranking, add-ons, LLM/news, and "
            ...
            "4_acceptance_standard": (
                ...
                "max drawdown drift <= 0.5 pp, survival >= 5%, valid breadth fields, "
                "and nonzero skipped accepted top-ups."
            ),
            "5_reproducibility": (
                "This script reruns core, accepted exp-20260512-013 Space stack, "
                "and each breadth-min-count variant across the canonical augmented snapshots."
            ),
        },
        "parameters": {
            ...
            "tested_min_positive_counts": list(
                SPACE_BASKET_POSITIVE_BREADTH_MIN_COUNTS
            ),
            ...
            "locked_variables": [
                ...
                "accepted Space average-positive 1.10x scalar",
        ...
        "gate2": {
            ...
            "basket_breadth": _field_check(before),
        },
        "gate3": {
            "new_core_filter_added": False,
            "space_basket_breadth_condition_added": True,
        ...
        "llm_metrics": {
            ...
            "why_not_llm_soft_ranking": (
                ...
                "gate; this run uses deterministic OHLCV basket breadth."
            ),
        },
        "production_impact": {
        ...
        "next_evidence_needed": (
            "If rejected, do not retry nearby Space basket breadth thresholds on "
            "the same frozen snapshots. Future Space work should use forward "
            "catalyst replacement value, a fresh event-quality field, or a PIT-safe "
            "candidate-pool change."
        ),
        "related_files": [
            "quant/experiments/exp_20260512_016_space_basket_breadth_risk.py",
            "data/experiments/exp-20260512-016/space_basket_breadth_risk.json",
            "docs/experiments/logs/exp-20260512-016.json",
            "docs/experiments/tickets/exp-20260512-016.json",
            "docs/experiments/artifacts/exp-20260512-016_space_basket_breadth_risk.md",
        ...
        "why_not_other_changes": (
            ...
            "broadening, near-perfect TQS breakout risk, peer-leader top-ups, "
            "peer-nonleader trend haircuts, 52w breakout proximity, and "

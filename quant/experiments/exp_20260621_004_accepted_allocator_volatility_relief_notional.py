"""exp-20260621-004: volatility-relief source notional scalar.
...
default-off paper notional scalar to selected volatility_relief rows.
...
EXPERIMENT_ID = "exp-20260621-004"
OWNER = "alpha-search-automation"
STEM = "accepted_allocator_volatility_relief_notional"
TRIAL_FAMILY = "accepted_default_off_helper_source_priority_allocation"
TRIAL_VARIANT_ID = "accepted_allocator_volatility_relief_source_notional_scalar_v1"
CHANGED_VARIABLE = TRIAL_VARIANT_ID
TARGET_SOURCE = "volatility_relief"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260621_004_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "vol_relief_overlap_with_turn_of_month",
    ...
    "confidence_reason": (
        ...
        "allocator source, not rank or candidate selection. The corrected "
        "independence map shows volatility_relief has 51 trades, positive total "
        "PnL, and low mean overlap, but source-notional allocation is heavily "
        "explored and vol_relief has higher max correlation to turn-of-month."
    ),
    "recorded_at": "2026-06-21T03:04:32+00:00",
}

PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "allocation: selected volatility_relief rows in the accepted helper "
        ...
        "showed vol_relief has positive total PnL, broad sample, and low mean "
        "overlap, while exp-20260620-032 and exp-20260621-001 did not scale it."
    ),
    "2_history_check": {
        "novelty_gate": (
            "experiment.py new warned on allocator near-neighbors. The override "
            "is the exp-20260620-033 corrected independence map identifying "
            "volatility_relief as a positive broad-sample source not included "
            "in the accepted laggard/revision/peer-shock scalar set."
        ),
        "exp-20260620-032": (
        ...
        "exp-20260620-033": (
            "Corrected independence map: vol_relief had 51 harness trades, "
            "mean net return 1.653%, total PnL $3,371.70, mean abs correlation "
            "0.053, and mean ticker-date overlap 0.0161."
        ),
        "exp-20260611-005": (
            "Accepted source-priority allocator family; this run does not alter "
            "rank, source selection, daily top-1, hold, costs, or cooldown."
        ),
    },
    "3_single_causal_variable": (
        "One fixed policy bundle: add source_notional_scalar=1.25 for selected "
        "volatility_relief allocator rows, while accepted laggard/revision/"
    ...
    "4_acceptance_standard": (
        ...
        "adds only volatility_relief 1.25x. Accept only if aggregate EV/PnL "
        "improve, no window EV/PnL regression, affected volatility_relief sample "
        "spans all three windows with >=20 rows, survival/concentration/drawdown "
        "guards pass, and the after allocator still beats accepted comparators."
    ),
    "5_reproducibility": (
        ".venv\\Scripts\\python.exe -B quant\\experiments\\"
        "exp_20260621_004_accepted_allocator_volatility_relief_notional.py"
    ),
}
...
def _target_trade_summary(
    ...
    original = peer_template.TARGET_SOURCE
    try:
        peer_template.TARGET_SOURCE = TARGET_SOURCE
        return peer_template._target_trade_summary(trades_by_window)
    finally:
        peer_template.TARGET_SOURCE = original
...
def _binding_gate4(
    *,
    ...
    return {
        "passed": accepted,
        "decision": (
            "accepted_allocator_volatility_relief_source_notional_scalar"
            if accepted
            else "rejected_allocator_volatility_relief_source_notional_scalar"
        ),
...
def build_payload() -> dict[str, Any]:
    ...
    try:
        ...
        after_payload = template._run_allocator_pass(
            "after_volatility_relief_scaled", dict(AFTER_SCALARS)
        )
    finally:
    ...
    return {
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                ...
                "source selection with only volatility_relief added at 1.25x "
                "paper notional. PnL is recomputed from selected row pnl_pct_net "
                "and scaled notional."
            ),
        },
        "parameters": {
        ...
        "interpretation": (
            "Volatility-relief source notional improved the current accepted "
            "allocator and should be retained only as shared default-off paper "
            "observation."
            if accepted
            else "Volatility-relief source notional failed Gate 4 and is not retained."
        ),
        "rejection_reason": None if accepted else "; ".join(gate4["failed_reasons"]),
        "post_run_reflection": {
            "why_result_happened": (
                "The corrected volatility_relief independence signal translated "
                "into incremental after-arbitration paper capital without changing "
                "selection."
                if accepted
                else (
                    "The current accepted allocator selected zero volatility_relief "
                    "rows after higher-priority sources and same-ticker cooldown, "
                    "so the proposed scalar had no after-arbitration exposure and "
                    "could not change EV or PnL."
                )
            ),
            "forbidden_near_neighbor_retry": (
                "Do not retry by sweeping volatility_relief scalar, source rank, "
                "allocator top-N, hold days, cooldown, VIXY/SPY/QQQ relief "
                "thresholds, or leadership thresholds on the frozen windows."
            ),
            "new_evidence_required": (
                "Closed forward allocator replacement-value rows tagged by "
                "volatility_relief source family, or a materially new out-of-sample "
                "source independence surface."
            ),
        },
        "next_retry_requires": [
            "closed forward volatility_relief replacement-value rows",
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Accepted Allocator Volatility-Relief Notional",
            "",
            ...
            *_window_table(payload),
            ...
            "- Volatility-relief affected trades: `{}` across `{}` windows".format(

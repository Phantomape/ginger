"""exp-20260512-950: core RS20-acceleration risk allocation.

Tests one production-visible risk-allocation variable on the accepted core stack:
an extra cap-aware post-sizing top-up only when an existing RS20 entry-state
leader also has fresh 10d-over-20d positive momentum acceleration. This is not
an entry filter, ranking change, exit change, universe expansion, or LLM rule.
"""
...
EXPERIMENT_ID = "exp-20260512-950"
EXPERIMENT_SLUG = "core_rs20_acceleration_risk"
MULTIPLIER_KEY = "core_rs20_acceleration_risk_multiplier_applied"
STATE_KEY = "core_rs20_acceleration_state"
...
def _positive_acceleration(mom10: Any, mom20: Any) -> bool:
    return (
    ...
    def wrapped(
        ...
        for sig in enriched:
            ...
            is_state = (
                ...
                and _positive_acceleration(mom10, mom20)
            )
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["core_rs20_acceleration_baseline_shares"] = shares
    out["core_rs20_acceleration_desired_shares"] = desired_shares
    out["core_rs20_acceleration_cap_shares"] = cap_shares
    out["core_rs20_acceleration_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "existing_rs20_multiplier": sizing.get(
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Core RS20 Acceleration Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_core_rs20_acceleration_risk"
    )
    interpretation = (
        "RS20-plus-acceleration core risk top-up cleared the canonical three-window gate and requires shared policy implementation before production use."
        if passed
        else "RS20-plus-acceleration core risk top-up did not clear the canonical three-window gate; the stronger discriminator was still not robust enough to promote."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted RS20 entry-state leader sleeve may deserve a second capital top-up only when the newest 10-day path is stronger than the full 20-day path."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "core_rs20_acceleration_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for shared RS20 entry-state leaders where momentum_10d_pct >= momentum_20d_pct and both are positive"
        ),
        "parameters": {
            "state_definition": {
            ...
            "locked_variables": [
                ...
                "existing RS20 entry-state sizing multiplier",
                "all other existing sizing multipliers",
                "portfolio heat",
                "LLM/news replay",
                "pilot sleeves",
            ],
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core state risk allocation using the existing shared RS20 leader plus production-visible momentum acceleration",
            "2_history_check": {
                "exp-20260512-111": "broad core momentum acceleration top-up failed because old_thin regressed; this run narrows the state to already accepted RS20 leaders.",
                "exp-20260512-112": "accepted default-off Space watch-liquidity helper; this run avoids Space retuning and live Space slots remain zero.",
                "llm_soft_ranking": "Space/LLM labels remain thin, so this run avoids LLM soft-ranking.",
                "sec_semantics": "same-accession directional semantic labels remain insufficient, so this run avoids SEC semantic expansion.",
            },
            "3_single_causal_variable": "core_rs20_acceleration_risk_multiplier with fixed shared RS20-plus-acceleration state",
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260512_950_core_rs20_acceleration_risk.py",
        },
        "backtest_protocol": {
        ...
        if passed
        else "Treat second-order RS20 momentum acceleration top-ups as rejected unless a materially different hold-quality variable is found.",
        "related_files": [
            "quant/experiments/exp_20260512_950_core_rs20_acceleration_risk.py",
            "data/experiments/exp-20260512-950/core_rs20_acceleration_risk.json",
            "docs/experiments/logs/exp-20260512-950.json",
            "docs/experiments/tickets/exp-20260512-950.json",
            "docs/experiments/artifacts/exp-20260512-950_core_rs20_acceleration_risk.md",

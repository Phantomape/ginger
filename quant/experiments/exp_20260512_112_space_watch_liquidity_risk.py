"""exp-20260512-112: Space watch-liquidity tier risk.

Tests one causal variable on top of the accepted exp-20260512-110 default-off
Space stack: an extra risk scalar for official Space signals whose production
universe registry liquidity_tier is ``watch``. This is a risk-allocation test,
not a candidate-pool expansion, LLM soft-ranking experiment, or source-type
split.
"""
...
from exp_20260512_110_space_company_release_source_risk import (  # noqa: E402
    ...
    _run_variant as _run_accepted_exp110_variant,
)
...
STEM = "space_watch_liquidity_risk"
TARGET_LIQUIDITY_TIER = "watch"
ACCEPTED_COMPANY_RELEASE_SOURCE_RISK_SCALAR = 1.10
WATCH_LIQUIDITY_RISK_SCALARS = (0.50, 0.75, 0.90, 1.00, 1.05, 1.10)
...
def _field_check_watch_liquidity_tier() -> dict[str, Any]:
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                ...
                _scale_sizing(
                    sizing,
                    watch_liquidity_scalar,
                    portfolio_value,
                    "space_watch_liquidity_tier_risk",
                )
                watch_adjustments.append(
                    {
                        "ticker": ticker,
                        "strategy": signal.get("strategy"),
                        "marker": "space_watch_liquidity_tier_risk",
                        "space_liquidity_tier": tiers.get(ticker),
                        "scalar": watch_liquidity_scalar,
                ...
                signal = {
                    **signal,
                    ...
                    "space_watch_liquidity_tier_eligible": True,
                }
            out.append(signal)
        return out
...
def _run_variant(
    ...
    return {
        ...
        "space_watch_liquidity_tier_risk_scalar": watch_liquidity_scalar,
        "by_window": by_window,
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_watch_liquidity_tier_adjustment"]["adjusted_signal_count"]
    ...
    passed = (
        ...
        and variant["space_watch_liquidity_tier_risk_scalar"] != 1.0
    )
    return {
        "passed": passed,
        ...
        "space_watch_liquidity_tier_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space watch-liquidity risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: risk scalar for official Space signals whose "
            "production registry `liquidity_tier` is `watch`."
        ),
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | {variant['space_watch_liquidity_tier_risk_scalar']:.3f} | "
            ...
            f"{gate['space_watch_liquidity_tier_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
            ...
            (
                ...
                "dPnL | Trades | Max DD | Survival | Watch-liquidity signals |"
            ),
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for label in WINDOWS:
        ...
        adjusted = best["by_window"][label]["space_watch_liquidity_tier_adjustment"][
...
def run() -> dict[str, Any]:
    ...
    before = _run_accepted_exp110_variant(
        "accepted_exp110_company_release_stack",
        ACCEPTED_COMPANY_RELEASE_SOURCE_RISK_SCALAR,
        company_release_gate,
        financing_gate,
        source_gate,
    ...
    variants = {}
    for scalar in WATCH_LIQUIDITY_RISK_SCALARS:
        name = f"watch_liquidity_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_watch_liquidity_risk"
        if accepted
        else "rejected_space_watch_liquidity_risk"
    )
    interpretation = (
        "The production registry `liquidity_tier=watch` scalar improved the accepted "
        ...
        if accepted
            ...
            "The production registry `liquidity_tier=watch` scalar did not clear the "
            "three-window gate on top of exp-20260512-110. Do not keep mining nearby "
            "liquidity-tier scalars on these frozen snapshots; future Space work "
            "should use forward replacement value or a genuinely new official "
    ...
    payload = {
        ...
        "changed_variable": "space_watch_liquidity_tier_risk_scalar",
        "single_causal_variable": (
            "risk scalar for official Space signals keyed only to production "
            "universe-registry liquidity_tier == watch"
        ),
        "hypothesis": (
            "After the accepted liquidity_tier=ok anchor top-up and company-release "
            "source scalar, the complementary `watch` liquidity bucket may still "
            "carry different risk-adjusted replacement value. A bounded scalar can "
            "test whether lower-liquidity official Space signals deserve haircut or "
            "small top-up without changing candidate pool, ranking, LLM/news, "
            "targets, stops, or live slots."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: scale official Space signals whose registry "
                "liquidity_tier is watch."
            ),
            "2_history_check": {
                "exp-20260512-037": (
                    "Accepted liquidity_tier=ok 1.10x anchor scalar. This run tests "
                    "the complementary watch bucket on top of the later accepted "
                    "exp-110 stack, not a retune of the ok scalar."
                ),
                "exp-20260512-110": (
                    "Accepted company-release customer-source 1.10x scalar; this is "
                    "the fixed before state."
                ),
                "exp-20260512-109": (
                    "Rejected generic cumulative risk cap; this run is bucket-specific "
                    "and production-observable, not a broad cap."
                ),
                "exp-20260512-023": (
            ...
            "3_single_causal_variable": (
                "space_watch_liquidity_tier_risk_scalar. Candidate pool, accepted "
            ...
            "4_acceptance_standard": (
                ...
                "nonzero adjusted watch-liquidity signals, and non-1.0 scalar."
            ),
            "5_reproducibility": (
                "Run .venv\\Scripts\\python.exe "
                "quant\\experiments\\exp_20260512_112_space_watch_liquidity_risk.py"
            ),
        },
        "parameters": {
            ...
            "tested_watch_liquidity_scalars": list(WATCH_LIQUIDITY_RISK_SCALARS),
            "accepted_before_experiment": "exp-20260512-110",
            "accepted_company_release_source_risk_scalar": (
                ACCEPTED_COMPANY_RELEASE_SOURCE_RISK_SCALAR
            ),
            "accepted_financing_dilution_profile_risk_scalar": (
        ...
        "backtest_protocol": (
            ...
            "The accepted_before variant reproduces exp-20260512-110 policy semantics."
        ),
        "gate1": {
            ...
            "known_bias": (
                ...
                "from a 2026-05-10 research universe. Registry liquidity_tier is "
        ...
        "next_evidence_needed": (
            "If rejected, stop nearby liquidity-tier scalar mining on the same "
            "frozen snapshots. Future Space work should test forward replacement "
            "value by catalyst family/source quality or add higher-coverage official "
            "catalyst attribution without noisy ticker additions."
        ),
        "related_files": [
            "quant/experiments/exp_20260512_112_space_watch_liquidity_risk.py",
            "data/experiments/exp-20260512-112/space_watch_liquidity_risk.json",
            "docs/experiments/logs/exp-20260512-112.json",
            "docs/experiments/tickets/exp-20260512-112.json",
            "docs/experiments/artifacts/exp-20260512-112_space_watch_liquidity_risk.md",
        ...
        "why_not_other_changes": (
            "LLM soft-ranking is data-limited; broad source splits, generic risk caps, "
            "peer leadership, and noisy ticker expansion have already failed or are "
            "underpowered. This tests a single production-visible risk-allocation "
            "field on top of the current accepted Space stack."
        ),
    }
    return payload

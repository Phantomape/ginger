"""exp-20260609-024: early peer earnings sympathy candidate pool.
...
EXPERIMENT_ID = "exp-20260609-024"
STEM = "early_peer_earnings_sympathy"
TRIAL_FAMILY = "early_peer_earnings_sympathy_candidate_pool"
TRIAL_VARIANT_ID = "early_peer_earnings_sympathy_top1_next_open_10d_v1"
CHANGED_VARIABLE = "early_peer_earnings_sympathy_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260609_024_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        ...
        "proxy. This tests a more direct relation field: same-day exact-"
        "industry sympathetic confirmation after a positive issuer earnings "
        "shock, with bounded extension and strict replay-only comparator "
        "discipline."
    ),
    "recorded_at": "2026-06-09T20:06:30+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "reaction, bounded OHLCV fields, same-ticker core-overlap "
...
def _candidate_for_peer(
    *,
    ...
    peer_snapshot_date = latest[0] if latest else None
    peer_info = latest[1] if latest else {}
    ...
    return {
        "date": signal_date,
        "ticker": peer_ticker,
        "source": "EARLY_PEER_EARNINGS_SYMPATHY_PAPER",
        ...
        "peer_relation_source": "exact_industry_positive_earnings_same_day_sympathy",
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_early_peer_earnings_sympathy"
        if gate["passed"]
        else "rejected_early_peer_earnings_sympathy_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                ...
                "industry peers that confirm sympathetically on the same "
                "signal day without being generic momentum chase."
            ),
            ...
            "new_evidence_type": "earnings_event_to_same_day_peer_sympathy_relation",
            ...
            "negative_reflection": (
                ...
                "same-day peer candidates, or the sympathetic reaction field "
                "still selects generic price momentum instead of a distinct "
                "relation. Do not retry by sweeping issuer reaction, peer "
                "reaction, close-location, volume, hold-day, cooldown, or "
            ...
            "next_evidence_needed": (
                ...
                "links, audited peer expectation changes, or closed forward "
                "replacement rows."
            ),
        }
    )
    ...
    payload["backtest_protocol"].update(
        {
            ...
            "execution_model": (
                ...
                "close-of-day OHLCV plus prior history only. Paper entry is "
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            ...
            "sympathetically on the same day without excessive extension."
        ),
        "2_history_check": {
            "exp-20260602-019": (
                "Post-earnings sector peer transfer was rejected; this run "
                "narrows relation quality to exact industry and same-day peer "
                "sympathy, not broad sector transfer."
            ),
            "exp-20260607-013": (
            ...
            "exp-20260609-013": (
                "Asynchronous lead-lag peer catchup failed; this run is "
                "earnings-event anchored and same-day only."
            ),
            "exp-20260609-014": (
                "High-order peer cluster shock failed; this run uses one "
                "interpretable event-peer edge, not generic cluster beta."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260609_024_early_peer_earnings_sympathy.py"
        ),
    }
    ...
    payload["gate3"]["note"] = (
        "No new core filter or entry rule was added. The early peer earnings "
        "sympathy source is additive default-off paper, so core signals "
    ...
    payload["post_run_reflection"] = (
        {
            "why_result_happened": (
                ...
                "replay lead, suggesting same-day exact-industry peer sympathy "
                "added replacement value beyond generic peer transfer."
            ),
        ...
        if payload["gate4"]["passed"]
            ...
            "why_result_happened": (
                ...
                "same-day peer sympathy relation did not sufficiently separate "
                "true event-information transfer from generic peer catch-up "
    ...
    payload["interpretation"] = (
        "The early peer earnings sympathy source cleared Gate "
        ...
        if payload["gate4"]["passed"]
            ...
            "The early peer earnings sympathy source did not "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Early Peer Earnings Sympathy",
            "",

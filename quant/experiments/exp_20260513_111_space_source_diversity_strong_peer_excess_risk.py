...
signals whose ticker 20-day momentum beats the official Space peer basket by at
least 10 percentage points.

This is intentionally not LLM soft-ranking, ticker expansion, target/stop
retuning, or another adjacent source-diversity scalar retry. The alpha question
is whether the already accepted peer-leader state has more value when the
leadership magnitude is large enough to be a production-visible discriminator.
"""
...
import exp_20260513_110_space_source_diversity_peer_iwm_leader_risk as base_exp


source_diversity_exp = base_exp.source_diversity_exp

EXPERIMENT_ID = "exp-20260513-111"
STEM = "space_source_diversity_strong_peer_excess_risk"
BEFORE_EXPERIMENT_ID = base_exp.EXPERIMENT_ID
ACCEPTED_SOURCE_DIVERSITY_RISK_SCALAR = (
    base_exp.ACCEPTED_SOURCE_DIVERSITY_RISK_SCALAR
)
ACCEPTED_SOURCE_DIVERSITY_PEER_LEADER_RISK_SCALAR = (
    base_exp.ACCEPTED_SOURCE_DIVERSITY_PEER_LEADER_RISK_SCALAR
)
ACCEPTED_SOURCE_DIVERSITY_IWM_LEADER_RISK_SCALAR = (
    base_exp.ACCEPTED_SOURCE_DIVERSITY_IWM_LEADER_RISK_SCALAR
)
ACCEPTED_SOURCE_DIVERSITY_PEER_IWM_LEADER_RISK_SCALAR = 1.05
STRONG_PEER_EXCESS_MIN_20D_PCT = 0.10
SOURCE_DIVERSITY_STRONG_PEER_EXCESS_RISK_SCALARS = (
    0.75,
    0.90,
    1.00,
    1.025,
...
IWM_LEADER_STATE = base_exp.IWM_LEADER_STATE
PEER_LEADER_STATE = base_exp.PEER_LEADER_STATE
...
def _run_strong_peer_excess_variant(
    label: str,
    strong_peer_excess_scalar: float,
    *,
    ...
    universe = sorted(
        set(source_diversity_exp.get_universe())
    ...
    strong_peer_adjustments: list[dict[str, Any]] = []
    counts: Counter[str] = Counter()

    def size_with_strong_peer_excess(
        ...
        for signal in sized:
            ...
            if ticker in target_tickers and sizing:
                ...
                peer_excess = _as_float(
                    signal.get("space_peer_excess_momentum_20d_pct")
                )
                is_strong_peer_excess = (
                    is_peer_leader
                    and peer_excess is not None
                    and peer_excess >= STRONG_PEER_EXCESS_MIN_20D_PCT
                )
                    ...
                    peer_adjustments.append(
                        {
                            ...
                            "space_peer_excess_momentum_20d_pct": peer_excess,
                            "space_iwm_relative_state": signal.get(
                                "space_iwm_relative_state"
                            ),
                            "trade_quality_score": signal.get("trade_quality_score"),
                ...
                if is_strong_peer_excess:
                    counts["source_diverse_strong_peer_excess_eligible_signal"] += 1
                    counts[
                        f"source_diverse_strong_peer_excess_eligible_{ticker}"
                    ] += 1
                    shares_before_strong_peer = int(
                        sizing.get("shares_to_buy") or 0
                    )
                    source_diversity_exp._scale_sizing(
                        sizing,
                        strong_peer_excess_scalar,
                        portfolio_value,
                        "space_source_diversity_strong_peer_excess_risk",
                    )
                    shares_after_strong_peer = int(sizing.get("shares_to_buy") or 0)
                    if shares_after_strong_peer != shares_before_strong_peer:
                        counts["source_diverse_strong_peer_excess_changed_signal"] += 1
                        counts[
                            f"source_diverse_strong_peer_excess_changed_{ticker}"
                        ] += 1
                    strong_peer_adjustments.append(
                        {
                            "ticker": ticker,
                            "strategy": signal.get("strategy"),
                            "marker": (
                            ...
                            "scalar": strong_peer_excess_scalar,
                            "peer_excess_floor_20d_pct": (
                                STRONG_PEER_EXCESS_MIN_20D_PCT
                            ),
                            "shares_before_scalar": shares_before_strong_peer,
                            "shares_after_scalar": shares_after_strong_peer,
                            "source_diversity_profile": profile,
                            "space_iwm_relative_state": signal.get(
                                "space_iwm_relative_state"
                            ),
                ...
                signal = {
                    **signal,
                    ...
                    "space_source_diversity_strong_peer_excess_scalar": (
                        strong_peer_excess_scalar
                        if is_strong_peer_excess
                        else 1.0
                    ),
                    "space_source_diversity_strong_peer_excess_bucket": (
                        is_strong_peer_excess
                    ),
    ...
    portfolio_engine.size_signals = size_with_strong_peer_excess
    try:
        ...
        for name, window in source_diversity_exp.WINDOWS.items():
            ...
            before_strong_peer_count = len(strong_peer_adjustments)
            ...
            window_strong_peer = strong_peer_adjustments[before_strong_peer_count:]
            ...
            by_window[name] = {
                ...
                "source_diversity_strong_peer_excess_adjustment": (
                    source_diversity_exp._adjustment_summary(window_strong_peer)
                ),
                "source_diversity_strong_peer_excess_counts": count_delta,
            }
        ...
        return {
            ...
            "parameters": {
                ...
                "strong_peer_excess_min_20d_pct": STRONG_PEER_EXCESS_MIN_20D_PCT,
                "space_source_diversity_strong_peer_excess_scalar": (
                    strong_peer_excess_scalar
                ),
            ...
            "source_diversity_strong_peer_excess_adjustment_summary": (
                source_diversity_exp._adjustment_summary(strong_peer_adjustments)
            ),
            "source_diversity_strong_peer_excess_adjustment_counts": dict(
                sorted(counts.items())
            ),
            "source_diversity_strong_peer_excess_adjustment_sample": (
                strong_peer_adjustments[:25]
            ),
        }
    finally:
...
def _runtime_state_gate(before: dict[str, Any]) -> dict[str, Any]:
    ...
    for payload in before.get("by_window", {}).values():
        counts = payload.get("source_diversity_strong_peer_excess_counts") or {}
        ...
        strong_peer += int(
            counts.get("source_diverse_strong_peer_excess_eligible_signal", 0) or 0
        )
        summary = payload.get("source_diversity_strong_peer_excess_adjustment") or {}
        for row in summary.get("sample_adjusted") or []:
            samples.append(
                {
                    ...
                    "space_peer_excess_momentum_20d_pct": row.get(
            ...
            if len(samples) >= 12:
                break
    return {
        "passed": source_diverse > 0 and peer_leader > 0 and strong_peer > 0,
        ...
        "source_diverse_strong_peer_excess_signal_count": strong_peer,
        "sample_source_diverse_strong_peer_rows": samples,
    }
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    changed_count = int(
        variant["source_diversity_strong_peer_excess_adjustment_counts"].get(
            "source_diverse_strong_peer_excess_changed_signal", 0
        )
    )
    eligible_count = int(
        variant["source_diversity_strong_peer_excess_adjustment_counts"].get(
            "source_diverse_strong_peer_excess_eligible_signal", 0
        )
    )
    scalar = float(
        variant["parameters"][
    ...
    return {
        ...
        "eligible_source_diversity_strong_peer_excess_signal_count": eligible_count,
        "changed_source_diversity_strong_peer_excess_signal_count": changed_count,
        "accepted": bool(
            scalar != 1.0
            and changed_count > 0
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity strong peer-excess risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_source_diversity_strong_peer_excess_scalar` after the accepted "
            "exp-110 source-diversity stack. Candidate pool, event labels, ranking, "
            "targets, stops, LLM/news, and live Space slots stay fixed."
        ),
        "",
        "## Gate 4 Summary",
        ...
        (
            "- Best scalar: "
            f"`{best['parameters']['space_source_diversity_strong_peer_excess_scalar']}`"
        ),
        (
            "- Strong peer-excess floor: "
            f"`{STRONG_PEER_EXCESS_MIN_20D_PCT:.2f}`"
        ),
        (
        ...
        (
            "- Source-diverse strong-peer-excess signals changed: "
            f"`{gate['changed_source_diversity_strong_peer_excess_signal_count']}` of "
            f"`{gate['eligible_source_diversity_strong_peer_excess_signal_count']}` eligible"
        ),
        "",
...
def run() -> dict[str, Any]:
    ...
    before = _run_strong_peer_excess_variant(
        "accepted_exp110_source_diversity_stack",
        1.0,
    ...
    gate2 = {
        ...
        "source_diversity_strong_peer_excess_runtime_state": runtime_state_gate,
    }
    gate2["passed"] = all(
        [
    ...
    variants = [
        _run_strong_peer_excess_variant(
        ...
        for scalar in SOURCE_DIVERSITY_STRONG_PEER_EXCESS_RISK_SCALARS
    ]
    ...
    if decision == "rejected":
        rejection_reason = (
            "No tested strong peer-excess source-diversity scalar improved aggregate "
    ...
    payload = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "On top of accepted exp-110 Space source-diversity stack, source-diverse "
            "official Space signals may deserve extra risk only when peer leadership "
            "is large in magnitude: ticker 20-day momentum must beat the official "
            "Space peer basket by at least 10 percentage points. This tests a "
            "production-visible peer-strength discriminator without changing the "
            "Space pool, event metadata, ranking, targets, stops, LLM boundary, or "
            "live slots."
        ),
        "change_type": "risk_allocation_shadow_sweep",
        "changed_variable": "space_source_diversity_strong_peer_excess_risk_scalar",
        "single_causal_variable": (
            "extra risk scalar for source-diverse official Space signals when "
            "space_peer_excess_momentum_20d_pct >= 0.10"
        ),
        "backtest_protocol": {
        ...
        "gate_questions": {
            "q1_alpha_hypothesis": (
                ...
                "when the existing peer-leader state has strong positive excess "
                "magnitude."
            ),
            "q2_prior_experiments": [
                ...
                "exp-20260513-110 accepted source-diversity peer+IWM-leader risk.",
                "exp-20260513-029 found leave-one-out peer-state identical unless a future run uses excess magnitude; this run uses excess magnitude.",
                "exp-20260513-025 rejected broad peer-leader breakout risk.",
            ],
            "q3_single_causal_variable": (
                "Only the strong peer-excess source-diversity scalar changes; accepted "
            ...
            "q4_acceptance_standard": (
                ...
                "and real adjusted source-diverse strong-peer-excess signals."
            ),
            "q5_reproducibility": (
        ...
        "parameters": {
            ...
            "strong_peer_excess_min_20d_pct": STRONG_PEER_EXCESS_MIN_20D_PCT,
            "tested_source_diversity_strong_peer_excess_scalars": list(
                SOURCE_DIVERSITY_STRONG_PEER_EXCESS_RISK_SCALARS
            ),
            "locked_variables": [
        ...
        "decision": decision,
        "status": (
            "accepted_default_off_space_source_diversity_strong_peer_excess_risk"
            if decision == "accepted"
            else "rejected_space_source_diversity_strong_peer_excess_risk"
        ),
        ...
        "next_evidence_needed": (
            ...
            if decision == "accepted"
                ...
                "Do not keep tuning strong peer-excess source-diversity scalars on "
                "this frozen sample. Prefer forward replacement-value evidence or a "
        ...
        "why_not_other_changes": (
            ...
            "This isolates the next valid peer-leadership idea: leadership magnitude."
        ),
        "known_risks": [
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        "changed_source_diversity_strong_peer_excess_signal_count": result[
            "best_variant_gate"
        ]["changed_source_diversity_strong_peer_excess_signal_count"],

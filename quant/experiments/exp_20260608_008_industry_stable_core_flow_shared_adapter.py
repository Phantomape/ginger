"""exp-20260608-008: shared industry stable core-flow paper adapter.

This alpha experiment promotes the positive exp-20260608-007 replay lead into
quant/industry_stable_core_flow_paper_sleeve.py. Historical replay and daily
...
STEM = "industry_stable_core_flow_shared_adapter"
TRIAL_FAMILY = "industry_stable_core_flow_shared_default_off_adapter"
TRIAL_VARIANT_ID = RULE_VERSION
CHANGED_VARIABLE = RULE_VERSION
SOURCE_LEAD_EXPERIMENT_ID = "exp-20260608-007"

REPO_ROOT = framework.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260608_008_{STEM}.json"
...
SOURCE_LEAD_JSON = (
    ...
    / "exp_20260608_007_industry_stable_core_flow_confirmation.json"
)
...
PREDICTION = {
    ...
    "expected_ev_delta": 0.1459,
    "expected_pnl_delta": 3731.54,
    "main_failure_modes": [
        "shared_helper_drift",
        "production_wiring_conflict",
        "drawdown_or_window_regression",
        "parity_gap",
    ],
    "confidence_reason": (
        "exp-20260608-007 already improved all three canonical windows after "
        "adding same-day core-flow confirmation and same-ticker overlap "
        "exclusion to the industry stable leadership source. This run only "
        "tests whether that fixed policy bundle reproduces through a shared "
        "default-off helper and daily snapshot semantics."
    ),
    "recorded_at": "2026-06-08T07:12:00Z",
}
...
PRODUCTION_IMPACT = {
    ...
    "parity_note": (
        ...
        "quant/industry_stable_core_flow_paper_sleeve.py. The helper is "
...
def _gate4(
    *,
    ...
    return {
        "passed": passed,
        "decision": (
            "accepted_industry_stable_core_flow_shared_default_off_adapter"
            if passed
            else "rejected_industry_stable_core_flow_shared_default_off_adapter"
        ),
        ...
        "parity_test_added": True,
        "shared_adapter_module": "quant/industry_stable_core_flow_paper_sleeve.py",
    }
...
def build_payload() -> dict[str, Any]:
    ...
    for label, cfg in framework.WINDOWS.items():
        print(f"[{label}] shared industry stable core-flow adapter replay")
    ...
    payload: dict[str, Any] = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Stable low-volatility leaders inside strong industries retain "
            "positive replacement value when the same signal date already has "
            "core A/B entry flow, while excluding same-ticker core overlap. "
            "The alpha is only acceptable if that fixed policy bundle can run "
            "through one shared historical and daily default-off helper."
        ),
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                "Signal uses only signal-date close OHLCV, prior 20/5/60-day "
                "history, broad-market sector map industry grouping, same-day "
                "baseline core A/B entry flow, same-ticker overlap exclusion, "
        ...
        "pre_run_questions": {
            "1_alpha_hypothesis": (
                "candidate_pool/shared_adapter: exp007's industry stable "
                "core-flow alpha is only usable if the same helper can drive "
                "historical replay and daily default-off paper observation."
            ),
            "2_history_check": {
                "exp-20260608-004": (
                    "Industry stable leadership improved EV/PnL in all "
                    "windows but failed drawdown; same-day core flow was the "
                    "risk/control repair tested later."
                ),
                "exp-20260608-005": (
                    "Tail guard failed, so this promotion does not retune "
                    "risk-state thresholds."
                ),
                "exp-20260608-007": (
                    "Positive replay lead: aggregate EV +0.1459, PnL "
                    "+$3,731.54, 47 target trades, all three windows positive."
                ),
                "exp-20260607-008/009": (
                    "Industry relation can work when specific, but nearby "
                    "industry retunes and pullback variants have failed."
                ),
            },
            ...
            "5_reproducibility": (
                ...
                "exp_20260608_008_industry_stable_core_flow_shared_adapter.py"
            ),
        },
        "gate1": {
    ...
    payload["interpretation"] = (
        "The positive industry stable core-flow lead reproduced through a "
        "shared default-off helper and daily snapshot surface."
        if gate4["passed"]
        else (
            "The industry stable core-flow lead failed shared-helper promotion; "
            "do not retain the helper as accepted alpha."
        )
    )
    payload["post_run_reflection"] = {
        "why_result_happened": (
            "Core-flow confirmation removed enough pure industry beta to keep "
            "the fixed stable-leader source within Gate 4 bounds, and the "
            "shared helper reproduced the private replay lead without changing "
            "live orders or core ranking."
            if gate4["passed"]
            else (
                "The production-visible helper failed to reproduce the private "
                "lead or breached Gate 4, indicating the replay lead depended "
                "on implementation details or remained too fragile."
            )
        ),
        "forbidden_near_neighbor_retry": (
            "Do not sweep same-day core-flow requirements, same-ticker overlap "
            "handling, industry ret20 strength, industry positive fraction, "
            "dispersion, low-volatility, candidate lead, close-location, "
            "volume, top-N, hold-day, cooldown, or paper notional thresholds "
            "on the frozen windows."
        ),
        "new_evidence_required": (
            "Next useful evidence is closed forward replacement-value rows "
            "from the shared default-off ledger or orthogonal PIT relation "
            "evidence such as peer taxonomy quality, supplier/customer links, "
            "or industry earnings/revision propagation."
        ),
    }
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Industry Stable Core-Flow Shared Adapter",
            "",
...
def _write_manifest(payload: dict[str, Any]) -> None:
    paths = [
        ...
        REPO_ROOT / "quant" / "test_default_off_alpha_attribution.py",

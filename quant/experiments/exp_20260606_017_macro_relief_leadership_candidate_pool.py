...
and close high in the daily range, admit the strongest liquid stock leader as a
top-1 next-open default-off paper candidate with a fixed 10-trading-day hold.
...
EXPERIMENT_ID = "exp-20260606-017"
STEM = "macro_relief_leadership_candidate_pool"
TRIAL_FAMILY = "macro_relief_leadership_candidate_pool"
TRIAL_VARIANT_ID = "macro_relief_top1_next_open_10d_v1"
CHANGED_VARIABLE = "official_macro_relief_day_stock_leadership_candidate_source_v1"
RULE_VERSION = CHANGED_VARIABLE

REPO_ROOT = framework.REPO_ROOT
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260606_017_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "Prior macro allocator and opening-range macro tests were thin, while "
        "broad winner continuation had drawdown/tail problems. This test uses "
        "a narrower official-event relief context and stock leadership rather "
        "than ETF beta or raw daily momentum."
    ),
    "recorded_at": "2026-06-06T15:10:11Z",
}
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_macro_relief_leadership_candidate_pool"
        if gate["passed"]
        else "rejected_macro_relief_leadership_candidate_pool"
    )
...
def _build_payload() -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "hypothesis": (
                ...
                "rally and close strong may identify stock leaders with "
                "cleaner 10-trading-day next-open replacement value than "
                "broad daily momentum."
            ),
            ...
            "new_evidence_type": "official_cpi_fomc_nfp_calendar_conditioned_stock_leadership",
    ...
    payload["gate_questions"] = {
        "1_alpha_hypothesis": (
            ...
            "cleaner leadership continuation because a macro uncertainty "
            "release can force fast risk reallocation into the day's strongest "
            "liquid stocks."
        ),
        "2_history_check": {
            "exp-20260525-901": (
        ...
        "4_acceptance_standard": (
            ...
            ">=12 across all 3 windows, survival >=5%, drawdown drift <=0.5pp, "
            "and concentration guard must pass."
        ),
        "5_reproducibility": (
            ".venv\\Scripts\\python.exe -B quant\\experiments\\"
            "exp_20260606_017_macro_relief_leadership_candidate_pool.py"
        ),
    }
    ...
    payload["next_evidence_needed"] = (
        ...
        "forward replacement-value rows; do not simply retune relief returns, "
        "close-location thresholds, hold days, cooldown, or paper notional."
    )
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Macro Relief Leadership Candidate Pool",
            "",

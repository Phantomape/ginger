"""exp-20260513-016: core near-perfect TQS risk allocation.

Alpha search. Tests one production-visible allocation variable: a cap-aware
post-sizing risk top-up for core trend/breakout signals with near-perfect
trade_quality_score. This does not change entries, filters, ranking, exits,
...
EXPERIMENT_ID = "exp-20260513-016"
EXPERIMENT_SLUG = "core_high_tqs_risk"
MULTIPLIER_KEY = "core_high_tqs_risk_multiplier_applied"
HIGH_TQS_MIN = 0.95
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["core_high_tqs_baseline_shares"] = shares
    out["core_high_tqs_desired_shares"] = desired_shares
    out["core_high_tqs_cap_shares"] = cap_shares
    out["core_high_tqs_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("core_high_tqs_state") and sizing.get("shares_to_buy"):
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Core Near-Perfect TQS Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_core_high_tqs_risk"
    )
    interpretation = (
        "Near-perfect core TQS risk allocation cleared the canonical three-window gate and requires shared policy implementation before production use."
        if passed
        else "Near-perfect core TQS risk allocation did not clear the canonical three-window gate; do not promote broad high-TQS sizing without a stronger discriminator or forward evidence."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Core trend/breakout candidates with near-perfect trade_quality_score may deserve a small cap-aware risk top-up because TQS combines confidence, trend, volume, and momentum into a production-visible setup-quality proxy."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "core_high_tqs_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up multiplier for core trend/breakout signals with trade_quality_score >= 0.95"
        ),
        "parameters": {
            "state_definition": {
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core risk allocation using production-visible near-perfect TQS setup quality",
            "2_history_check": {
                "global_tqs_ordering": "previous global TQS/confidence ordering variants were rejected; this run does not reorder or filter candidates.",
                "exp-20260513-001": "strong-volume breakout scalar was immaterial; this run uses full TQS and both core strategies.",
                "exp-20260513-007": "accepted own-green candle stays locked; this run does not retune that scalar.",
                "exp-20260513-009/011/013": "close-location, SPY-excess, and momentum-acceleration neighbors were rejected; this run avoids those signal-day price-shape variables.",
            ...
            "3_single_causal_variable": "core_high_tqs_risk_multiplier with fixed near-perfect TQS state",
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_016_core_high_tqs_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        if passed
        else "Use forward setup-quality attribution or a narrower production-visible quality discriminator before retrying broad high-TQS sizing.",
        "related_files": [
            "quant/experiments/exp_20260513_016_core_high_tqs_risk.py",
            "data/experiments/exp-20260513-016/core_high_tqs_risk.json",
            "docs/experiments/logs/exp-20260513-016.json",
            "docs/experiments/tickets/exp-20260513-016.json",
            "docs/experiments/artifacts/exp-20260513-016_core_high_tqs_risk.md",
            "docs/experiment_log.jsonl",
        ],
        "why_not_other_changes": (
            "LLM soft-ranking and SEC filing-shock semantics remain data-limited; Space profile slicing is over-mined on frozen snapshots; recent own-candle, close-location, SPY-excess, momentum-acceleration, and strong-volume core neighbors already have anti-repeat guardrails."
        ),
    }
    return payload

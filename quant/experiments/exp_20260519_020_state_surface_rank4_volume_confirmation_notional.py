...
queue-rank 4 candidates with own volume confirmation receive a bounded
...
EXPERIMENT_ID = "exp-20260519-020"
EXPERIMENT_SLUG = "state_surface_rank4_volume_confirmation_notional"
...
RULE_VERSION = "state_surface_rank4_volume_confirmation_notional_v1"

DOC_HELPERS = prev.DOC_HELPERS
CORE_HELPERS = prev.CORE_HELPERS
CONCENTRATION_HELPERS = prev.CONCENTRATION_HELPERS

RANK4_VOLUME_CONFIRMATION_VARIANTS: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
        ...
        (
            "rank4_volume_ge_100_scalar_125",
            {
                "volume_min": 1.00,
                "scalar": 1.25,
                "target_queue_rank": 4,
                "aggression_order": 1,
                "description": "rank 4 volume ratio >= 1.00 receives 25% support",
            },
        ),
        (
            "rank4_volume_ge_100_scalar_150",
            {
                "volume_min": 1.00,
                "scalar": 1.50,
                "target_queue_rank": 4,
                "aggression_order": 2,
                "description": "rank 4 volume ratio >= 1.00 receives 50% support",
            },
        ),
        (
            "rank4_volume_ge_110_scalar_125",
            {
                "volume_min": 1.10,
                "scalar": 1.25,
                "target_queue_rank": 4,
                "aggression_order": 3,
                "description": "rank 4 volume ratio >= 1.10 receives 25% support",
            },
        ),
        (
            "rank4_volume_ge_110_scalar_150",
            {
                "volume_min": 1.10,
                "scalar": 1.50,
                "target_queue_rank": 4,
                "aggression_order": 4,
                "description": "rank 4 volume ratio >= 1.10 receives 50% support",
            },
        ),
        (
            "rank4_volume_ge_120_scalar_125",
            {
                "volume_min": 1.20,
                "scalar": 1.25,
                "target_queue_rank": 4,
                "aggression_order": 5,
                "description": "rank 4 volume ratio >= 1.20 receives 25% support",
            },
        ),
        (
            "rank4_volume_ge_120_scalar_150",
            {
                "volume_min": 1.20,
                "scalar": 1.50,
                "target_queue_rank": 4,
                "aggression_order": 6,
                "description": "rank 4 volume ratio >= 1.20 receives 50% support",
            },
        ),
        (
            "rank4_volume_ge_130_scalar_125",
            {
                "volume_min": 1.30,
                "scalar": 1.25,
                "target_queue_rank": 4,
                "aggression_order": 7,
                "description": "rank 4 volume ratio >= 1.30 receives 25% support",
            },
        ),
        (
            "rank4_volume_ge_130_scalar_150",
            {
                "volume_min": 1.30,
                "scalar": 1.50,
                "target_queue_rank": 4,
                "aggression_order": 8,
                "description": "rank 4 volume ratio >= 1.30 receives 50% support",
            },
        ),
    ]
)
...
def _apply_rank4_volume_confirmation(
    ...
    *,
    ...
    target_rank = int(variant.get("target_queue_rank") or 4)
    ...
    for trade in trades:
        ...
        row["rank4_volume_confirmation_variant"] = variant_name
        row["rank4_volume_confirmation_rule_version"] = RULE_VERSION
        row["rank4_volume_confirmation_volume_ratio_20"] = volume_ratio
        row["rank4_volume_confirmation_target_queue_rank"] = target_rank
        row["rank4_volume_confirmation_min"] = threshold
        row["rank4_volume_confirmation_configured_scalar"] = scalar
        row["rank4_volume_confirmation_scalar"] = scalar if applies else None
        row["rank4_volume_confirmation_applied"] = bool(applies)
        row["rank4_volume_confirmation_profile_name"] = (
        ...
        row["rank4_volume_confirmation_base_multiplier"] = _float(
            row.get("rank_notional_multiplier")
        )
        row["rank_notional_rank4_volume_confirmation_rule_version"] = RULE_VERSION
        ...
        if applies:
            ...
            row["rank4_volume_confirmation_base_notional"] = base_notional
        ...
        rows.append(
            {
                ...
                "rank4_volume_confirmation_applied": trade.get(
                    "rank4_volume_confirmation_applied"
                ),
                "rank4_volume_confirmation_min": trade.get(
                    "rank4_volume_confirmation_min"
                ),
                "rank4_volume_confirmation_scalar": trade.get(
                    "rank4_volume_confirmation_scalar"
                ),
                "rank4_volume_confirmation_profile_name": trade.get(
                    "rank4_volume_confirmation_profile_name"
                ),
...
def _notional_by_volume_profile(trades: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for trade in trades:
        key = str(trade.get("rank4_volume_confirmation_profile_name") or "baseline")
...
def _variant_payload(
    *,
    ...
    selected = _apply_rank4_volume_confirmation(
        baseline_trades,
    ...
    for label in WINDOWS:
        ...
        applied = [
            row for row in adjusted if row.get("rank4_volume_confirmation_applied")
        ]
        surface_sleeve[label] = {
            "selected_trade_count": len(adjusted),
            "rank4_volume_confirmation_adjusted_trade_count": len(applied),
            "rank4_volume_confirmation_adjusted_pnl": round(
            ...
            "notional_by_rank4_volume_confirmation_profile": (
                _notional_by_volume_profile(adjusted)
            ),
            "selected_trades": _selected_trade_rows(adjusted),
        }
    applied_all = [
        row for row in selected if row.get("rank4_volume_confirmation_applied")
    ]
    applied_windows = {str(row.get("window")) for row in applied_all if row.get("window")}
    return {
        "variant_name": variant_name,
        "variant_type": "rank4_volume_confirmation_notional_profile",
        ...
        "rank4_volume_confirmation_adjusted_trade_count": len(applied_all),
        "rank4_volume_confirmation_adjusted_windows": sorted(applied_windows),
...
def _gate4_for_variant(
    *,
    ...
    adjusted_guard_passed = (
        variant["rank4_volume_confirmation_adjusted_trade_count"]
        >= MIN_ADJUSTED_TRADES
        and len(variant["rank4_volume_confirmation_adjusted_windows"])
    ...
    return {
        ...
        "rank4_volume_confirmation_adjusted_trade_count": variant[
            "rank4_volume_confirmation_adjusted_trade_count"
        ],
        "rank4_volume_confirmation_adjusted_windows": variant[
            "rank4_volume_confirmation_adjusted_windows"
        ],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rank-4 Volume Confirmation Notional",
        "",
    ...
    for label in WINDOWS:
        ...
        lines.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {bdd:.2%} | {add:.2%} | {trades} |".format(
                ...
                trades=sleeve["rank4_volume_confirmation_adjusted_trade_count"],
            )
        )
    lines.extend(
        [
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for variant_name, variant in RANK4_VOLUME_CONFIRMATION_VARIANTS.items()
    ]
    sweep_summary = []
    ...
    decision = (
        "accepted_default_off_state_surface_rank4_volume_confirmation_notional"
        if passed
        else "rejected_state_surface_rank4_volume_confirmation_notional"
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": "Rank-4 state-surface candidates with volume confirmation are a deeper rotation-breadth quality field and deserve bounded default-off paper-notional support.",
        "alpha_hypothesis": {
            "category": "capital allocation",
            "entry_exit_ranking_or_allocation": "default-off paper allocation",
            "playbook_alignment": "Continues state-surface maturation through a new rank-depth field while keeping the candidate set fixed and avoiding LLM soft-ranking data limits.",
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "rank4_volume_confirmation_notional_profile",
        ...
        "parameters": {
            ...
            "profile_priority": "applies after accepted rank-3 volume confirmation; applies only to queue_rank == 4",
            "locked_variables": [
                "core entries",
                "core exits",
                "core sizing",
        ...
        "gate3": {
            ...
            "hard_rule": "No filter or candidate gate changed; only queue_rank == 4 paper notional changes.",
        },
        "gate4": best["gate4"],
        "surface_sleeve": best_payload["surface_sleeve"],
        "sweep_summary": sweep_summary,
        "history_check": {
            "exp-20260519-015": "Accepted rank-3 volume confirmation; this experiment freezes it and tests rank-4 own volume confirmation only.",
            "exp-20260519-016_to_018": "Rejected sector diversity/cohesion attempts; this experiment avoids sector profile retuning.",
            "exp-20260519-019": "Rejected core-misfit residual expansion; this experiment keeps candidate pool fixed.",
            "anti_repeat": "Not a rank3 threshold retry, sector-diversity retry, core-misfit expansion, AI-infra cached pool, or LLM soft-ranking experiment.",
        },
        "llm_metrics": {
            "used_llm": False,
        ...
        "interpretation": (
            "Rank-4 volume confirmation improved the default-off state-surface paper overlay without touching core/live orders."
            if passed
            else "Rank-4 volume confirmation did not clear Gate 4; keep the accepted rank3-volume stack unchanged."
        ),
        ...
        "next_evidence_needed": (
            ...
            if passed
            else "Do not retry nearby rank4 volume profiles without forward evidence or a distinct quality field."
        ),
        "protocol_answers": {
            "1_alpha_hypothesis": "capital allocation: queue_rank == 4 candidates with volume_ratio_20 confirmation indicate healthier rotation breadth and deserve bounded paper-notional support.",
            "2_history_check": "Prior accepted work covered rank2 near-high and rank3 volume confirmation; latest failed work covered sector diversity/cohesion and core-misfit residual expansion, so this tests a distinct rank-depth OHLCV confirmation field.",
            "3_single_causal_variable": "rank4_volume_confirmation_notional_profile",
            "4_acceptance_standard": "docs/backtesting.md three fixed windows; positive aggregate EV/PnL, at least two EV-improved windows, zero EV-regressed windows, adjusted trades >=4 across >=2 windows, max DD drift <=0.5pp, single-ticker positive share <=50%.",
...
def main() -> None:
    ...
    DOC_HELPERS._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rank-4 volume confirmation notional",
            "status": payload["status"],
            "decision": payload["decision"],
            "artifact": DOC_HELPERS._repo_rel(OUT_JSON),
            "summary": (
                f"Rank-4 volume profile {payload['parameters']['best_variant']} "

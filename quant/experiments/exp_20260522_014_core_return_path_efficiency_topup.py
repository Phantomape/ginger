"""exp-20260522-014: core return-path efficiency top-up scout.

Alpha search. Tests whether already-qualified core stock signals with a
top-quartile 20-day return-path efficiency deserve a small cap-aware risk
top-up. Return-path efficiency is prior 20-day net return divided by the sum of
absolute close-to-close daily returns over the same window.
...
EXPERIMENT_ID = "exp-20260522-014"
STEM = "core_return_path_efficiency_topup"
MULTIPLIER_KEY = "return_path_efficiency20_top_quartile_risk_multiplier_applied"
...
VARIANTS = OrderedDict(
    [
        ("efficiency20_topup_1025", {"multiplier": 1.025}),
        ("efficiency20_topup_1050", {"multiplier": 1.05}),
        ("efficiency20_topup_1075", {"multiplier": 1.075}),
        ("efficiency20_topup_1100", {"multiplier": 1.10}),
    ]
)
...
def _return_path_efficiency20_features(data) -> dict[str, Any]:
    if data is None or len(data) < 22:
        return {
            "return_path_efficiency_20": None,
            "return_path_efficiency_20_available": False,
        }
    ...
    if absolute_path <= 0.0:
        ...
        efficiency = float(returns.sum()) / absolute_path
    return {
        "return_path_efficiency_20": round(efficiency, 6)
        if efficiency is not None
        else None,
        "return_path_efficiency_20_available": efficiency is not None,
    }
...
def _make_trend_feature_wrapper(original: Callable[..., dict[str, Any] | None]):
    def wrapped(data):
        ...
        if features is None:
            return None
        return {**features, **_return_path_efficiency20_features(data)}
...
def _top_quartile_cutoff(features_dict: dict[str, Any]) -> float | None:
    ...
    for ticker, features in (features_dict or {}).items():
        ...
        if sector in EXCLUDED_SECTORS:
            continue
        value = features.get("return_path_efficiency_20")
    ...
    index = max(
        0,
        math.ceil(len(values) * (1.0 - TOP_QUARTILE_FRACTION)) - 1,
    )
    return values[index]
...
def _make_enrich_wrapper(original: Callable[..., list[dict[str, Any]]]):
    def wrapped(signals, features_dict, atr_target_mult=None):
        ...
        for sig in enriched:
            ...
            sig["return_path_efficiency_20"] = value
            sig["return_path_efficiency_20_top_quartile_cutoff"] = (
                round(float(cutoff), 6) if isinstance(cutoff, (int, float)) else None
            )
            sig["return_path_efficiency_20_top_quartile_state"] = (
...
def _apply_topup_to_sizing(
    ...
    if sig.get("return_path_efficiency_20_top_quartile_state") is not True:
        return sig, None
    ...
    sizing["return_path_efficiency20_top_quartile_state"] = True
    sizing["return_path_efficiency20_top_quartile_baseline_shares"] = old_shares
    sizing["return_path_efficiency20_top_quartile_new_shares"] = new_shares
    sizing["return_path_efficiency20_top_quartile_value"] = sig.get(
        "return_path_efficiency_20"
    )
    sizing["return_path_efficiency20_top_quartile_cutoff"] = sig.get(
        "return_path_efficiency_20_top_quartile_cutoff"
    )
    ...
    record = {
        ...
        "return_path_efficiency_20": sig.get("return_path_efficiency_20"),
        "cutoff": sig.get("return_path_efficiency_20_top_quartile_cutoff"),
...
def run() -> dict[str, Any]:
    ...
    if not passed:
        ...
        elif gate["adjusted_signal_count"] < MIN_ADJUSTED_SIGNAL_COUNT:
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Core stock signals with top-quartile 20-day return-path efficiency "
            "may represent smoother trend accumulation than spike-driven moves. "
            "A small cap-aware risk top-up should improve EV without changing "
            "entry filters, ranking, exits, universe, LLM, or news logic."
        ),
        "change_summary": (
            "Add an experiment-only return_path_efficiency_20 OHLCV field and "
            "sweep a cap-aware risk multiplier for non-ETF/non-Commodity core "
            "signals in the same-day top quartile of that field."
        ),
        ...
        "parameters": {
            "field": "return_path_efficiency_20",
            "field_definition": (
                "sum of last 20 close-to-close returns divided by sum of "
                "absolute last 20 close-to-close returns, known on signal day"
            ),
        ...
        "production_impact": {
            ...
            "promotion_required_if_accepted": (
                "Move return_path_efficiency_20 feature, top-quartile state, and "
                "risk top-up into shared feature_layer/risk_engine/portfolio_engine "
        ...
        "why_not_other_changes": (
            ...
            "exp-20260522-012 just failed old_thin."
        ),
        "known_risks": [
            "Moderate multiple-testing risk because core OHLCV allocation fields have many accepted predecessors.",
            "The top-quartile definition is production-visible but still needs shared implementation if accepted.",
            "Cap-aware top-ups can be immaterial if existing core sizing is already at the default position cap.",
        ],
        "rejection_reason": rejection_reason,
        "next_retry_requires": (
            [
                "Do not retry adjacent return-path-efficiency cutoffs or scalars on the same frozen windows without forward rows or a distinct daily-return path field.",
                "Consider richer reversal-vs-continuation path clusters only if production-visible and backed by fresh evidence.",
            ]
            if not passed
            else [
        ...
        "related_files": [
            "quant/experiments/exp_20260522_014_core_return_path_efficiency_topup.py",

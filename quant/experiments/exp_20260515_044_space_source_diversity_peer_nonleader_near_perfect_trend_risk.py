"""exp-20260515-041: Space source-diversity peer-nonleader near-perfect trend risk.

Tests one allocation variable on top of the accepted exp-20260515-024 Space
stack: whether source-diverse official Space trend signals that are still
peer-nonleaders and near-perfect, but not perfect, by TQS deserve a different
...
EXPERIMENT_ID = "exp-20260515-044"
STEM = "space_source_diversity_peer_nonleader_near_perfect_trend_risk"
...
def _is_target_signal(signal: dict[str, Any], profile: dict[str, Any] | None) -> bool:
    ...
    return (
        score is not None
        and score >= TQS_MIN_INCLUSIVE
        and score < TQS_MAX_EXCLUSIVE
    )


def _apply_extra_scale(
    *,
    signal: dict[str, Any],
    sizing: dict[str, Any],
    scalar: float,
...
def _run_current_stack_variant(
    ...
    *,
    ...
    variant["source_diversity_peer_nonleader_near_perfect_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["source_diversity_peer_nonleader_near_perfect_counts_by_window"] = (
        by_window_counts
    )
    ...
    variant["source_diversity_peer_nonleader_near_perfect_adjustment_sample"] = (
        near_perfect_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("source_diversity_peer_nonleader_near_perfect_counts") or {}
    ...
    return {
        ...
        "eligible_near_perfect_signal_count": eligible_count,
        "changed_near_perfect_signal_count": changed_count,
        "reasons": {
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis": (
            "Source-diverse official Space trend signals that remain peer "
            "nonleaders and have near-perfect but non-perfect TQS may be "
            "mis-sized by the accepted default-off Space stack."
        ),
        ...
        "gate_answers": {
            "1_alpha_hypothesis": (
                ...
                "peer_momentum_state=nonleader and 0.95 <= TQS < 1.0 may be "
                "mis-sized."
            ),
            "2_prior_similar_experiments": [
                "exp-20260512-008 accepted near-perfect Space trend TQS risk.",
                "exp-20260513-010 rejected watch-liquidity TQS-scope splits.",
                ...
                "No prior run isolated near-perfect-but-not-perfect TQS inside the accepted source-diversity peer-nonleader trend sleeve.",
            ],
            "3_single_causal_variable": (
                "Only the near-perfect peer-nonleader source-diversity trend scalar changes."
            ),
            "4_success_criteria": (
            ...
            "5_reproducibility": (
                ...
                "exp_20260515_044_space_source_diversity_peer_nonleader_near_perfect_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the near-perfect peer-nonleader source-diversity "
            "trend scalar did not improve the fixed windows while satisfying "
            "the PnL and drawdown guardrails."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby TQS or peer-state interactions inside the "
            "accepted Space source-diversity trend sleeve on these frozen "
            "windows; Space needs broader mature forward rows or a genuinely "
        ...
        "why_not_other_changes": (
            "Recent VSAT/IRDM/GSAT candidate-pool expansions regressed old_thin, "
            "and LLM soft-ranking lacks dense downstream attribution. This run "
            "keeps the fixed Space pool and isolates one existing "
            "production-visible allocation discriminator."
        ),
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space source-diversity peer-nonleader near-perfect trend risk",
        "",
        "## Hypothesis",
        (
            "Source-diverse official Space `trend_long` signals may need a "
            "different default-off allocation when the ticker is still a Space "
            "peer nonleader and TQS is near-perfect but not perfect."
        ),
        "",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_source_diversity_peer_nonleader_near_perfect_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_near_perfect_signal_count']}`",
            f"- adjusted signals: `{gate['changed_near_perfect_signal_count']}`",
            f"- adjusted counts: `{best['source_diversity_peer_nonleader_near_perfect_counts']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Near-perfect source-diversity peer-nonleader trend scalar "
            f"{best['parameters']['space_source_diversity_peer_nonleader_near_perfect_trend_scalar']} "
            f"changed {gate['changed_near_perfect_signal_count']} signals with "
            "aggregate EV delta "
...
def run() -> dict[str, Any]:
    ...
    payload = {
        ...
        "alpha_hypothesis": (
            "Source-diverse Space trend signals whose ticker is still a Space "
            "peer nonleader and whose TQS is near-perfect but not perfect may "
            "be mis-sized in the accepted stack."
        ),
        "changed_variable": (
            "space_source_diversity_peer_nonleader_near_perfect_trend_scalar"
        ),
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate["eligible_near_perfect_signal_count"],
                    "adjusted_signals": gate["changed_near_perfect_signal_count"],

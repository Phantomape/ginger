"""exp-20260515-013: clean-SPY cap-only + RS20 leader cap scout.

Tests one allocation-only variable on the accepted core stack: whether the
already-promoted clean-SPY cap-only leader sleeve remains cap-bound when it
also has the production-visible RS20 entry-state leader confirmation.

This is a cap scout only. It does not change entries, exits, ranking, raw risk
multipliers, universe, LLM/news behavior, heat, or slots.
"""
...
EXPERIMENT_SLUG = "clean_spy_cap_only_rs20_cap"
CAP_KEY = "clean_spy_cap_only_rs20_leader_max_position_pct_applied"
CAP_SWEEP = [0.625, 0.65, 0.675, 0.70]
MAX_DRAWDOWN_WORSE_GUARDRAIL = 0.005
CURRENT_MAX_POSITION_PCT = 0.65
...
def _is_target_sleeve(sig: dict[str, Any], sizing: dict[str, Any]) -> bool:
    return bool(
        ...
        and sizing.get("clean_spy_cap_only_leader_max_position_pct_applied")
...
def _resize_with_cap(
    ...
    risk_pct = prev._pre_sizing_risk_pct(sizing)
    if old_shares <= 0 or entry <= 0 or net_risk_per_share <= 0 or risk_pct is None:
        return sizing

    old_cap_pct = float(
        sizing.get("max_position_pct_applied")
        or base.portfolio_engine.CLEAN_SPY_CAP_ONLY_LEADER_MAX_POSITION_PCT
    )
    ...
    out = dict(sizing)
    out["clean_spy_cap_only_rs20_cap_baseline_shares"] = old_shares
    out["clean_spy_cap_only_rs20_cap_raw_shares"] = raw_shares
    out["clean_spy_cap_only_rs20_cap_old_cap_pct"] = old_cap_pct
    out["clean_spy_cap_only_rs20_cap_new_cap_pct"] = new_cap_pct
    out["clean_spy_cap_only_rs20_cap_old_cap_shares"] = old_cap_shares
    out["clean_spy_cap_only_rs20_cap_new_cap_shares"] = new_cap_shares
    out["clean_spy_cap_only_rs20_cap_new_shares"] = new_shares
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Clean-SPY Cap-Only RS20 Cap",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if selected["passed"]
        else "rejected_clean_spy_cap_only_rs20_cap"
    )
    interpretation = (
        "Clean-SPY cap-only RS20 leaders were still cap-bound and the selected cap improved the current three-window stack without EV regression."
        if selected["passed"]
        else "Clean-SPY cap-only RS20 leader cap expansion did not clear the canonical three-window gate."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted clean-SPY cap-only sleeve improved all windows at a 60% "
            "cap. The next valid follow-up needs an independent production-visible "
            "discriminator, not a nearby generic cap retry. RS20 entry-state "
            "leadership may identify the cap-only continuation subset where more "
            "cap room is still useful while avoiding weaker non-RS20 cap-only names."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "max_position_pct_for_clean_spy_cap_only_rs20_leaders",
        "single_causal_variable": (
            "max_position_pct for already-qualified clean-SPY cap-only leaders with rs20_entry_state_leader=true"
        ),
        "parameters": {
            "cap_sweep": CAP_SWEEP,
            "baseline_clean_spy_cap_only_cap": (
                base.portfolio_engine.CLEAN_SPY_CAP_ONLY_LEADER_MAX_POSITION_PCT
            ),
            ...
            "locked_variables": [
                ...
                "raw risk_on and clean-SPY multipliers",
                "accepted clean-SPY cap-only 60% cap",
        ...
        "historical_experiment_check": {
            "similar_prior_results": {
                "exp-20260515-008": (
                    "Accepted the broad clean-SPY cap-only 60% cap. This run does not retune that broad cap; it tests a narrower RS20-confirmed cap-only subset."
                ),
                "exp-20260515-003": (
                    "Rejected confirmed RS20 + mid-dispersion cap. This run is a different clean-SPY cap-only continuation sleeve and does not touch mid-dispersion."
                ),
                "exp-20260514-045": (
                    "Rejected triple momentum leader cap. This run requires the already-accepted clean-SPY cap-only state rather than stacking generic momentum confirmations."
                ),
            },
            "why_not_llm_or_space": (
                "LLM soft-ranking remains attribution-limited, SEC filing features had a fresh data gap, and recent Space benchmark-breadth retries were adjacent or underpowered. This run uses deterministic shared fields already surfaced by production sizing."
            ),
        },
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital allocation: clean-SPY cap-only leaders with RS20 confirmation may deserve more cap room than the accepted generic cap-only sleeve"
            ),
            "2_history_check": (
                "The broad cap-only sleeve was accepted in exp-20260515-008; prior RS20 cap combinations were rejected in different sleeves. No prior clean-SPY cap-only + RS20 cap scout was found."
            ),
            "3_single_causal_variable": "clean-SPY cap-only RS20 max_position_pct only",
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260515_013_clean_spy_cap_only_rs20_cap.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate1": {
            ...
            "baseline_note": (
                "Current HEAD includes the accepted exp-20260515-008 clean-SPY cap-only promotion; this run compares against that live code baseline."
            ),
        },
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "portfolio_engine clean_spy_cap_only_leader_max_position_pct_applied",
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, add a shared clean-SPY cap-only RS20 cap constant and cap branch in portfolio_engine.size_signals, include the attribution key in backtester.py, and add focused production/backtest parity tests before live/default orders change."
            ),
        },
        ...
        "next_evidence_needed": (
            "Promote through shared sizing code and parity tests; do not change the broad clean-SPY cap-only cap."
            if selected["passed"]
            else "Do not retry nearby clean-SPY cap-only RS20 cap values without forward cap-room evidence or a materially different production-visible discriminator."
        ),
        "related_files": [

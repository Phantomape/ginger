"""exp-20260619-021: TLT rate-relief growth leadership scout.

Replay-only alpha search. The single decision hypothesis is that a free
cross-asset OHLCV field, TLT rate relief, can separate durable high-duration
growth leadership from generic momentum. When TLT is rising over 20 sessions
and QQQ is not lagging SPY, select one liquid stock with strong SPY/QQQ-relative
leadership for next-open, 10-trading-day default-off paper observation.
...
EXPERIMENT_ID = "exp-20260619-021"
STEM = "tlt_rate_relief_growth_leadership"
TRIAL_FAMILY = "tlt_rate_relief_growth_leadership_candidate_pool"
TRIAL_VARIANT_ID = "tlt_rate_relief_growth_leadership_top1_next_open_10d_v1"
CHANGED_VARIABLE = "tlt_rate_relief_growth_leadership_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260619_021_{STEM}.json"
...
MIN_RET60_EXCESS_SPY = 0.08
MIN_RET20_EXCESS_QQQ = -0.01
...
PREDICTION = {
    ...
    "confidence_reason": (
        ...
        "replacement value; TLT/rate relief has not been tested as a fixed "
        "candidate source in the recent frozen list, but broad ETF proxies "
        "often fail after costs, so confidence is low."
    ),
    "recorded_at": "2026-06-19T23:07:11+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "uses_cross_asset_rate_proxy": True,
    ...
    "execution_envelope": {
        ...
        "failure_handling": (
            "missing TLT/QQQ/SPY OHLCV, failed rate-relief context, missing "
            "candidate OHLCV, missing next open, or missing 10d exit rejects "
            "the paper candidate"
        ),
    },
    "parity_note": (
        "This experiment changes no production code. A positive result is only "
        "a replay lead until a shared default-off helper computes the same TLT "
        "rate-relief context, QQQ-vs-SPY growth-tape context, liquid "
        "SPY/QQQ-relative leadership gate, cooldown, next-open paper entry, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: TLT rate-relief plus liquid growth leadership may "
        "identify high-duration stock continuation candidates when falling-rate "
        "tape supports re-rating, expanding the default-off candidate pool "
        "without using SEC/LLM fields."
    ),
    "2_history_check": {
        "novelty_gate": (
            "Novelty gate warned on generic OHLCV continuation families. The "
            "override declares a new evidence axis: cross-asset duration proxy "
            "from PIT TLT OHLCV with canonical-window broad-warehouse coverage."
        ),
        "exp-20260607-019": (
            "Accepted volatility-relief stock leadership used VIXY volatility "
            "compression. This run uses TLT duration/rate relief, not VIXY."
        ),
        "exp-20260608-002": (
            "Rejected oil-cost relief travel leadership. This run is not a "
            "commodity cost-relief sector proxy."
        ),
        "exp-20260618-006": (
            "Rejected intra-industry lead-lag direction stability. This run is "
            "cross-asset macro context plus single-stock leadership, not "
            "industry diffusion."
        ),
        "exp-20260619-015": (
            "Rejected regime-conditioned intra-industry lead-lag. This run does "
            "not re-slice that relation source or regime bucket."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260619_021_tlt_rate_relief_growth_leadership.py"
    ),
}
...
def _load_window_snapshot(
    *,
    ...
    tickers = sorted(
        ...
        | {RATE_PROXY_TICKER, GROWTH_PROXY_TICKER, MARKET_PROXY_TICKER}
    )
...
def _macro_context(
    *,
    ...
    tlt_rows = framework.shadow._series(snapshot, RATE_PROXY_TICKER)
    ...
    tlt_idx = indices.get(RATE_PROXY_TICKER, {}).get(signal_date)
    qqq_idx = indices.get(GROWTH_PROXY_TICKER, {}).get(signal_date)
    spy_idx = indices.get(MARKET_PROXY_TICKER, {}).get(signal_date)
    if tlt_idx is None or qqq_idx is None or spy_idx is None:
        return None
    if min(tlt_idx, qqq_idx, spy_idx) < 60:
        return None
    tlt_ret20 = framework._ret(tlt_rows, tlt_idx, 20)
    ...
    if any(value is None for value in (tlt_ret20, tlt_ret5, qqq_ret20, spy_ret20)):
        return None
    assert tlt_ret20 is not None and tlt_ret5 is not None
    assert qqq_ret20 is not None and spy_ret20 is not None
    qqq_minus_spy = qqq_ret20 - spy_ret20
    if tlt_ret20 < MIN_TLT_RET20:
        return None
...
def _candidate_rows_for_window(
    *,
    ...
    eligible = sorted(
        ...
        if ticker not in {RATE_PROXY_TICKER, GROWTH_PROXY_TICKER, MARKET_PROXY_TICKER}
    )
    ...
    for signal_date in window_dates:
        ...
        if macro is None:
            scan["failed_macro_context_days"] += 1
            continue
        macro_by_date[signal_date] = macro
        scan["macro_context_pass_days"] += 1
        for ticker in eligible:
            ...
            score = (
                ...
                + 0.35 * float(macro["tlt_ret20"])
                + 0.15 * float(macro["qqq_minus_spy_ret20"])
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "TLT_RATE_RELIEF_GROWTH_LEADERSHIP_PAPER",
    ...
    scan["macro_context_sample"] = list(macro_by_date.items())[:5]
    return rows, {
        **dict(scan),
        "rule_version": RULE_VERSION,
        "rate_proxy_ticker": RATE_PROXY_TICKER,
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_tlt_rate_relief_growth_leadership"
        if gate["passed"]
        else "rejected_tlt_rate_relief_growth_leadership_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    for label, cfg in framework.WINDOWS.items():
        print(f"[{label}] core baseline and TLT rate-relief growth replay")
        ...
        warehouse_coverage_by_window[label] = {
            ...
            "rate_proxy_present": RATE_PROXY_TICKER in snapshot,
    ...
    return {
        ...
        "mechanism_family": "production_visible_free_cross_asset_rate_relief_candidate_pool",
        "new_evidence_type": "free_ohlcv_cross_asset_tlt_duration_proxy",
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                "TLT, QQQ, SPY, and candidate stock features are computed from "
        ...
        "gate2": {
            ...
            "runtime_fields": [
                "TLT OHLCV Date/Open/High/Low/Close/Volume",
        ...
        "interpretation": (
            "The TLT rate-relief growth leadership source cleared Gate 4 as a "
            "replay-only/default-off lead, but no production surface was "
            "promoted."
            if gate4["passed"]
            else (
                "The TLT rate-relief growth leadership source did not clear "
                "Gate 4 (failed: "
                + (", ".join(gate4["failed_reasons"]) or "none")
                + "). Do not promote or tune this fixed cross-asset macro "
                "proxy bundle on the same frozen windows."
            )
        ),
        "rejection_reason": rejection_reason,
        "next_evidence_needed": (
            "A retry needs materially different PIT rate/duration evidence "
            "(for example issuer-level rate sensitivity, financing-duration "
            "exposure, or closed forward replacement-value rows), not a sweep "
            "of TLT lookbacks, TLT thresholds, QQQ/SPY thresholds, RS/close/"
            "volume guards, top-N, hold, cooldown, or notional."
        ),
        "post_run_reflection": {
            "why_result_happened": (
                ...
                if gate4["passed"]
                    ...
                    "Rejected. TLT rate-relief plus liquid growth leadership "
                    ...
                    "growth momentum rather than a distinct duration edge."
            ...
            "forbidden_near_neighbor_retry": (
                "Do not retry by sweeping TLT/IEF lookbacks, rate-relief "
                "thresholds, QQQ-vs-SPY threshold, RS/close/volume/vol guards, "
                "top-N, hold days, cooldown, or notional on these frozen "
                "windows."
            ),
            "new_evidence_required": (
                "Need issuer-level rate-sensitivity/financing-duration data, "
                "borrow/options confirmation, or closed forward replacement-"
                "value rows before revisiting rate-relief growth leadership."
            ),
        },
        "related_files": [
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    for label in framework.WINDOWS:
        ...
        rows.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {dd:+.4f} | {macro} | {raw} | {trades} |".format(
                ...
                macro=scan.get("macro_context_pass_days", 0),
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} TLT Rate-Relief Growth Leadership",
            "",
        ...
        "windows": [
            {
                ...
                "macro_context_pass_days": payload["context_scan_by_window"][label].get("macro_context_pass_days"),

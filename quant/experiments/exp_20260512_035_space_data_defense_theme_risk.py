"""exp-20260512-035: Space data/defense theme-segment risk allocation.

Tests whether the production-visible Space ``theme_segment`` field has a
second useful bucket after the accepted launch/lunar top-up. This keeps the
accepted exp-20260512-032 Space stack fixed and changes only the risk scalar for
official Space signals whose registry theme segment is ``space_data_defense``.
"""
...
EXPERIMENT_ID = "exp-20260512-035"
STEM = "space_data_defense_theme_risk"
TARGET_THEME_SEGMENT = "space_data_defense"
ACCEPTED_LAUNCH_LUNAR_THEME_SCALAR = 1.10
DATA_DEFENSE_THEME_RISK_SCALARS = (0.50, 0.75, 0.90, 1.10, 1.25)
...
def _field_check_data_defense_theme() -> dict[str, Any]:
    ...
    return {
        "passed": registry_gate.get("passed") is True and bool(target_tickers),
        "target_theme_segment": TARGET_THEME_SEGMENT,
        "target_tickers": target_tickers,
        "base_registry_gate": registry_gate,
    }


def _install_space_policy(data_defense_scalar: float):
    ...
    data_defense_adjustments: list[dict[str, Any]] = []
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if (
                ...
                and sizing
                and signal.get("space_theme_segment") == TARGET_THEME_SEGMENT
            ):
                ...
                _scale_sizing(
                    ...
                    data_defense_scalar,
                    portfolio_value,
                    "space_data_defense_theme_segment_risk",
                )
                row = _adjustment_row(
                    signal,
                    sizing,
                ...
                data_defense_adjustments.append(row)
...
def _run_variant(name: str, data_defense_scalar: float) -> dict[str, Any]:
    ...
    (
        ...
        basket_counts,
        day_counts,
    ) = _install_space_policy(data_defense_scalar)
    try:
        by_window = {}
        for label, window in WINDOWS.items():
            before_data_defense = len(data_defense_adjustments)
            ...
            by_window[label] = {
                ...
                "space_data_defense_theme_adjustment": _adjustment_summary(
                    data_defense_adjustments[before_data_defense:]
                ),
    ...
    return {
        "variant": name,
        "target_theme_segment": TARGET_THEME_SEGMENT,
        "space_data_defense_theme_segment_scalar": data_defense_scalar,
        "by_window": by_window,
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_data_defense_theme_adjustment"]["adjusted_signal_count"]
    ...
    return {
        ...
        "space_data_defense_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space data/defense theme-segment risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: risk scalar for official Space signals whose "
            "universe-registry `theme_segment` is `space_data_defense`."
        ),
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | {variant['space_data_defense_theme_segment_scalar']:.2f} | "
            ...
            f"{gate['space_data_defense_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
            ...
            (
                ...
                "dPnL | Trades | Max DD | Survival | Data/defense signals |"
            ),
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for label in WINDOWS:
        ...
        adjusted = best["by_window"][label]["space_data_defense_theme_adjustment"][
    ...
    lines.extend(
        [
            ...
            "## Field Check",
            "",
            json.dumps(payload["gate2"]["data_defense_theme_registry"], sort_keys=True),
            "",
            "## Interpretation",
            "",
...
def run() -> dict[str, Any]:
    ...
    registry_gate = _field_check_data_defense_theme()
    ...
    variants = {}
    for scalar in DATA_DEFENSE_THEME_RISK_SCALARS:
        name = f"data_defense_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_data_defense_theme_risk"
        if accepted
        else "rejected_space_data_defense_theme_risk"
    )
    interpretation = (
        "The space_data_defense theme-segment risk scalar improved the accepted "
        ...
        if accepted
            ...
            "The space_data_defense theme-segment risk scalar did not clear the "
            "three-window gate on top of exp-20260512-032. Do not retry adjacent "
            "Space data/defense theme scalar values on these frozen snapshots; "
            "the next Space edge needs forward catalyst replacement value or a "
            "different production-observable catalyst-quality field."
        )
    )
    ...
    payload = {
        ...
        "single_causal_variable": (
            ...
            "universe-registry theme_segment == space_data_defense"
        ),
        "hypothesis": (
            "After launch/lunar, TQS, peer-breakout, and IWM-relative Space "
            "risk allocation were accepted, the remaining production-visible "
            "candidate-quality question is whether the PL/BKSY/RDW data/defense "
            "bucket deserves a separate risk budget. This avoids LLM soft-ranking "
            "because the forward event-state sample is still too thin, and it "
            "does not add noisy tickers."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
            ...
            "2_history_check": {
                "exp-20260512-032": (
                    "Accepted launch/lunar 1.10x theme-segment scalar; this is "
                    "the fixed before state."
                ),
                "exp-20260512-023": (
                ...
                "exp-20260511-111": (
                    ...
                    "tests a broader production registry bucket, not target width."
                ),
                "exp-20260512-014/015/016/019/021": (
                    "Rejected peer-trend, 52w, breadth, R/R, and breakout volume "
                    "refinements; those variables stay fixed."
                ),
            },
            "3_single_causal_variable": (
                "space_data_defense_theme_segment_risk_scalar. Candidate pool, "
            ...
            "4_acceptance_standard": (
                ...
                ">=50 total trades, and nonzero adjusted data/defense signals."
            ),
            "5_reproducibility": (
                "This script reruns core, accepted exp-20260512-032 Space stack, "
                "and each data/defense theme-segment scalar across the canonical "
        ...
        "parameters": {
            ...
            "target_theme_segment": TARGET_THEME_SEGMENT,
            "target_theme_tickers": registry_gate["target_tickers"],
            "tested_theme_segment_scalars": list(DATA_DEFENSE_THEME_RISK_SCALARS),
        ...
        "gate2": {
            ...
            "data_defense_theme_registry": registry_gate,
        ...
        "next_evidence_needed": (
            "If rejected, do not retry adjacent data/defense theme-segment scalars "
        ...
        "related_files": [
            "quant/experiments/exp_20260512_035_space_data_defense_theme_risk.py",
            "data/experiments/exp-20260512-035/space_data_defense_theme_risk.json",
            "docs/experiments/logs/exp-20260512-035.json",
            "docs/experiments/tickets/exp-20260512-035.json",
            "docs/experiments/artifacts/exp-20260512-035_space_data_defense_theme_risk.md",

"""exp-20260707-024: matured intraday advisory shadow-action attribution.

Observed-only alpha attribution. The single question is whether the same
post-exp024 primary advisory shadow-action surface rejected in exp-20260630-014
becomes useful after materially more h1/h3 forward rows settled through the
2026-07-06 warehouse.
...
EXPERIMENT_ID = "exp-20260707-024"
OWNER = "alpha-explore"
SLUG = "intraday_advisory_shadow_forward_outcome_20260706"
RUNNER = f"quant/experiments/exp_20260707_024_{SLUG}.py"
...
OUT_JSON = DATA_DIR / f"exp_20260707_024_{SLUG}.json"
...
HYPOTHESIS = (
    ...
    "2026-07-06 warehouse; fixed attribution recipe from exp-20260630-014, "
    "no threshold or response retune."
)
...
TRIAL_VARIANT_ID = "post_exp024_matured_through_20260706_v1"
CHANGED_VARIABLE = "intraday_advisory_shadow_action_forward_outcome_20260706_v1"
NEW_EVIDENCE_TYPE = "materially_more_settled_intraday_forward_rows"
NEW_EVIDENCE_AXIS = (
    "Materially more closed intraday advisory shadow-action rows beyond "
    "exp-20260630-014: no-ID reopen preflight found current h1/h3 settled rows "
    "141/117 versus exp-20260630-014's 72/45, with warehouse calendar through "
    "2026-07-06; official run uses the same fixed h1/h3 attribution recipe "
    "without threshold or response retune."
)
NEARBY_PRIOR_EXPERIMENTS = ["exp-20260626-024", "exp-20260627-025", "exp-20260630-014"]
CAUSAL_COMPONENTS = [
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "h1_action_underperformance_not_met",
        "h3_action_underperformance_reverses",
        "no_stable_directional_separation",
        "still_observed_only_no_shared_exit_policy",
    ],
    "confidence_reason": (
        "Rows now clear the reopen-count bar, but exp-20260630-014 already "
        "showed weak h1 separation and reversed h3 action-vs-OK performance, "
        "so this fixed-recipe refresh is likely to reject while usefully "
        "freezing the larger closed-row cohort."
    ),
    "recorded_at": "2026-07-07T23:12:18+00:00",
}
CONFIG = {
    "asof_min_date": "2026-06-22",
    "asof_max_date": "2026-07-07",
    "horizons": [1, 3],
...
def build_payload(source: Any) -> dict[str, Any]:
    ...
    payload = {
        ...
        "timestamp": now,
        "status": status,
        "decision": gate4["decision"] + "_20260706",
        "accepted": accepted,
        ...
        "calibration": {
            ...
            "surprise_note": (
                "The added settled rows confirmed no stable underperformance edge; "
                "action rows outperformed OK/no-action on both h1 and h3 SPY-excess mean."
                if not accepted
        ...
        "summary": {
            ...
            "reopen_guard_progress": {
                ...
                "prior_exp_20260630_014_h1_settled_rows": 72,
                "prior_exp_20260630_014_h3_settled_rows": 45,
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "More settled rows did not improve the advisory-action edge: action "
                "rows outperformed OK/no-action on both h1 and h3 SPY-excess mean, "
                "reversing the intended exit/risk-allocation signal direction."
                if not accepted
            ...
            "forbidden_near_neighbor_retry": (
                ...
                "tilt response functions on the same 2026-06-22..2026-07-07 "
                "snapshot surface."
            ),
            "new_evidence_required": (
                "Reopen only after current h1/h3 settled rows materially exceed "
                "141/117 under the same fixed helper, or after true quote timestamps "
                "or broker bar IDs, materially larger native primary-action coverage, "
                "and slot-reuse/replacement-value accounting are available."
            ),
        },

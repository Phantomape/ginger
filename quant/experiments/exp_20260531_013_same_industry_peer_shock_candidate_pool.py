...
after a strong positive same-industry peer shock, a liquid unshocked peer with
...
EXPERIMENT_ID = "exp-20260531-013"
STEM = "same_industry_peer_shock_candidate_pool"
TRIAL_FAMILY = "same_industry_peer_shock_candidate_pool"
CHANGED_VARIABLE = "same_industry_strong_positive_peer_shock_candidate_source_v1"
RULE_VERSION = "same_industry_strong_positive_peer_shock_top1_10d_v1"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260531_013_{STEM}.json"
...
def _industry_maps(
    ...
    for ticker in sorted(tickers):
        ...
        if industry:
            industry_by_ticker[ticker] = str(industry)
            members[str(industry)] += 1
    return industry_by_ticker, lookup_by_ticker, dict(members), status_counts
...
def _candidate_rows_for_window(
    ...
    industry_by_ticker, lookup_by_ticker, industry_member_counts, industry_status_counts = (
        _industry_maps(eligible)
    )
    ...
    for ticker in sorted(eligible):
        ...
        if not industry:
            ...
            continue
        if industry_member_counts.get(industry, 0) < MIN_INDUSTRY_MEMBERS:
            audit["industry_too_small"] += 1
            continue
        rows = framework.ohlcv_helper._series(snapshot, ticker)
        ...
        for date in dates:
            ...
            for age in range(1, LOOKBACK_DAYS + 1):
                ...
                for event in peer_shocks.get(industry, {}).get(prior_date, []):
            ...
            candidates.append(
                {
                    ...
                    "industry": industry,
                    "sector": lookup_by_ticker.get(ticker, {}).get("sector"),
                    "relation_lookup": lookup_by_ticker.get(ticker),
    ...
    return candidates, {
        ...
        "industry_status_counts": dict(sorted(industry_status_counts.items())),
        "industry_member_counts": dict(sorted(industry_member_counts.items())),
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = (
        ...
        if gate4["passed"]
        else "rejected_same_industry_peer_shock_candidate_pool"
    )
    all_target_trades = [
        trade
    ...
    payload.update(
        {
            ...
            "decision": decision,
            "hypothesis": (
                "Strong positive same-industry peer shocks from free OHLCV may "
            ...
            "prediction": {
                ...
                "confidence_reason": (
                    "Same-sector OHLCV peer shock was noisy. A same-industry "
                    "relation is materially narrower and production-visible, "
                    "but may fail from sample loss or concentration."
                ),
                "recorded_at": "2026-05-31T13:05:57+00:00",
            },
            "calibration": {
                "actual_decision": decision,
                "actual_success": actual_success,
                "predicted_success_probability": 0.28,
                "brier_score": framework.base._round((0.28 - actual_success) ** 2, 6),
                "expected_ev_delta": 0.20,
    ...
    payload["preflight_questions"] = {
        "1_alpha_hypothesis": (
            "Same-industry strong positive peer shocks may create delayed "
            "continuation in liquid unshocked peers; category: candidate-pool "
            "alpha_search. It follows the playbook's preference for free, "
            "production-visible data-edge fields and stronger relation sources."
        ),
        "2_prior_experiments": {
            "exp-20260530-001": (
            ...
            "exp-20260530-023": (
                ...
                "EV/PnL and concentration gates."
            ),
            "exp-20260531-010": (
            ...
            "exp-20260531-012": (
                "Same-sector OHLCV peer-shock source improved aggregate EV only "
                "slightly, regressed late_strong, breached drawdown, and failed "
                "concentration."
            ),
        },
        ...
        "5_reproducibility": (
            ...
            "exp_20260531_013_same_industry_peer_shock_candidate_pool.py"
        ),
    }
    ...
    payload["gate2"]["note"] = (
        ...
        "shock rows, offline industry/sector cache metadata, and SPY OHLCV. It does not "
    ...
    payload["why_not_other_changes"] = (
        ...
        "new forward rows or a materially different field; changed relation "
        "source rather than same-sector peer-shock thresholds."
    )
    payload["interpretation"] = (
        "The same-industry peer-shock source cleared the replay gate but still "
        "needs a shared default-off adapter before any production exposure."
        if gate4["passed"]
        else (
            "The same-industry peer-shock source did not clear Gate 4. Do not "
            "promote it or retry nearby OHLCV peer-shock relation/threshold "
            "variants on the same frozen windows without forward replacement "
            "rows or a stronger non-price relation source."
        )
    )
    ...
    payload["next_evidence_needed"] = (
        ...
        "stronger non-price peer relation source, such as true earnings-calendar "
        "peer events, customer/supplier links, or audited theme propagation."
    )
...
def _persist(payload: dict[str, Any]) -> None:
    ...
    ticket_payload = {
        "experiment_id": EXPERIMENT_ID,
        "title": "Same-industry peer-shock candidate pool",

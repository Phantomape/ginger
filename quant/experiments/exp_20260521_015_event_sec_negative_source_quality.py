"""exp-20260521-015: event SEC negative source-quality scalar.

Alpha search. Tests one production-visible event source field on top of the
accepted exp-20260521-013 default-off event overlay adapter: whether
SEC-negative-reaction event rows deserve a modest paper-notional scalar.

No JavaScript is used. The script records a replay scout only; no live/default
orders, core entries, ranking, sizing, exits, LLM/news behavior, or source
...
EXPERIMENT_ID = "exp-20260521-015"
EXPERIMENT_SLUG = "event_sec_negative_source_quality"
    ...
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "sec_negative_source_scalar": 1.0,
            },
        ),
        (
            "sec_negative_source_075",
            {
                "description": "0.75x paper notional for SEC negative-reaction rows.",
                "sec_negative_source_scalar": 0.75,
            },
        ),
        (
            "sec_negative_source_090",
            {
                "description": "0.90x paper notional for SEC negative-reaction rows.",
                "sec_negative_source_scalar": 0.90,
            },
        ),
        (
            "sec_negative_source_110",
            {
                "description": "1.10x paper notional for SEC negative-reaction rows.",
                "sec_negative_source_scalar": 1.10,
            },
        ),
        (
            "sec_negative_source_125",
            {
                "description": "1.25x paper notional for SEC negative-reaction rows.",
                "sec_negative_source_scalar": 1.25,
            },
        ),
    ]
)
...
def _is_target_sec_negative(trade: dict[str, Any]) -> bool:
    return str(trade.get("source") or "") == TARGET_SOURCE
...
def _scaled_trade(
    ...
    target = _is_target_sec_negative(trade)
    source_scalar = float(variant["sec_negative_source_scalar"]) if target else 1.0
    scalar = accepted_scalar * source_scalar
    ...
    return {
        **trade,
        ...
        "sec_negative_source_quality_target": target,
        "sec_negative_source_scalar": round(source_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("sec_negative_source_quality_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("sec_negative_source_quality_target")
        ]
        target_by_window[label] = {
            ...
            "text_event_types": sorted(
                {str(row.get("text_event_type") or "") for row in window_targets}
            ),
        }
    return {
        "target_field": "source",
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -abs(VARIANTS[name]["sec_negative_source_scalar"] - 1.0),
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_pending_shared_event_adapter_sec_negative_source_quality"
        if accepted
        else "rejected_event_sec_negative_source_quality"
    )
    ...
    compact_parameters = {
        ...
        "target_source": TARGET_SOURCE,
        "target_field": "source",
        "selected_sec_negative_source_scalar": VARIANTS[best_variant][
            "sec_negative_source_scalar"
        ],
        "variant_scalars": {
            name: row["sec_negative_source_scalar"] for name, row in VARIANTS.items()
        },
    ...
    return {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "event_source_quality_allocation_scout",
        "mechanism_family": "external_event_satellite_overlay_allocation",
        "trial_family": "event_source_quality_allocation",
        "trial_variant_id": "sec_negative_reaction_source_notional_scalar",
        "changed_variable": "event_sec_negative_source_scalar",
        "prior_trial_count": 8,
        ...
        "new_evidence_type": "production_visible_source_quality_after_exp013_stack",
        "hypothesis": (
            "Inside the accepted default-off event overlay, SEC negative-reaction "
            "rows may have source-level replacement value beyond the already "
            "accepted negative-reaction bucket tilt. A single source scalar tests "
            "whether this production-visible source family deserves more or less "
            "paper allocation without changing core trades."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event source-quality scoring",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Uses event source/context quality from the playbook and avoids "
                "LLM soft-ranking, broad noisy ticker expansion, state-surface "
                "threshold tuning, and broad-market forward gates that are not "
                "currently measurable."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for fixed event overlay rows whose source is "
            "sec_negative_reaction; source definitions, event capacity, accepted "
            "reaction/state/context scalars, holding period, and core strategy "
            "stay fixed"
        ),
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "SEC negative-reaction source rows may deserve a source-level "
                "allocation scalar; this is event source-quality capital allocation."
            ),
            "2_history_check": (
                "Governance source-quality, negative-reaction context, positive "
                "state-score, and non-narrow state context were accepted recently; "
                "source capacity and governance item-code complexity failed. This "
                "run tests the whole SEC-negative source family after the accepted "
                "exp-20260521-013 stack."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for fixed sec_negative_reaction "
                "event rows changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260521_015_event_sec_negative_source_quality.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260521-003": "Rejected event source capacity due concentration/materiality.",
            "exp-20260521-005": "Governance source-quality scout; nearby source-family evidence.",
            "exp-20260521-006": "Accepted governance source-quality scalar; kept fixed here.",
            "exp-20260521-009": "Accepted negative-reaction bucket context; kept fixed here.",
            "exp-20260521-013": "Accepted non-narrow state-bucket context; current baseline.",
            "exp-20260521-014": "Rejected governance multi-item complexity due sample guard.",
        },
        "backtest_protocol": {
            "source": (
        ...
        "llm_metrics": {
            ...
            "blocker_relation": (
                ...
                "deterministic event source context only."
            ),
        },
        "decision_rationale": (
            "Passed replay gates but was not promoted until a shared adapter patch "
            "and parity test are added."
            if accepted
            else "Rejected: SEC negative source scalar did not clear Gate 4."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "Do not promote or retry nearby SEC-negative source-level scalars on "
            "the frozen sample without new forward rows or a materially different "
            "source-quality field."
        ),
        "why_not_other_attractive_points": (
            "Broad-market alpha is currently blocked by missing forward universe; "
            "core_misfit has zero closed outcomes; state_surface has open/skipped "
            "forward rows but no closed validation; space catalyst has 12 official "
            "closed decisions but failed its same-theme replacement-value gate; "
            "LLM soft-ranking remains attribution/sample-limited."
        ),
        "risk_of_change": (
            "No production change is made by this replay scout. If the scout ever "
            "passes, the scalar must be implemented in quant/event_sleeve_bundle.py "
            "and the run adapter/parity contract before any accepted decision."
        ),
        "related_files": [
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event SEC Negative Source Quality",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search. Tests whether SEC negative-reaction event rows deserve "
            "a source-level paper-notional scalar on top of the accepted exp-20260521-013 "
            "event adapter."
        ),
        "",
        "## Gate 4 Result",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event SEC negative source quality",

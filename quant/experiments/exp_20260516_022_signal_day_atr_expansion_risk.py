"""exp-20260516-022: signal-day ATR expansion risk haircut sweep.

Tests one production-visible allocation discriminator on the accepted core
stack: same-day top-quartile ATR expansion among non-ETF/non-commodity stock
features. This is a replay scout only; no production-default strategy behavior
changes unless a separate shared-policy promotion is made and revalidated.
"""
...
EXPERIMENT_ID = "exp-20260516-022"
EXPERIMENT_SLUG = "signal_day_atr_expansion_risk"
MULTIPLIER_KEY = "signal_day_atr_expansion_risk_multiplier_applied"
STATE_KEY = "signal_day_atr_expansion_top_quartile_state"
BASELINE_RISK_MULTIPLIER = 1.0
RISK_MULTIPLIER_SWEEP = [0.85, 0.9, 0.95, BASELINE_RISK_MULTIPLIER]
TOP_FRACTION = 0.25
...
def _atr_expansion_top_quartile_cutoff(
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        cutoff = _atr_expansion_top_quartile_cutoff(features_dict)
        cutoff_for_log = (
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["signal_day_atr_expansion_baseline_shares"] = shares
    out["signal_day_atr_expansion_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "atr_expansion_top_quartile_cutoff": sig.get(
                                "signal_day_atr_expansion_top_quartile_cutoff"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Signal-Day ATR Expansion Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if selected["passed"]
        else "rejected_signal_day_atr_expansion_risk"
    )
    interpretation = (
        "Signal-day top-quartile ATR expansion cleared the canonical three-window scout and requires shared risk/portfolio promotion plus rerun before production use."
        if selected["passed"]
        else "Signal-day top-quartile ATR expansion did not clear the canonical three-window gate as a standalone risk haircut."
    )
    ...
    payload: dict[str, Any] = {
        ...
        "decision": decision,
        "hypothesis": (
            "After recent scalar and LLM/SEC branches became exhausted or data-limited, "
            "the next useful alpha direction is a fixed-candidate, production-visible "
            "signal-quality discriminator. A signal-day top-quartile ATR expansion state "
            "may mark chase/exhaustion risk inside already-qualified trend/breakout stock "
            "signals, so a small risk haircut can improve EV without filtering entries."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "signal_day_atr_expansion_risk_multiplier",
        "single_causal_variable": (
            "post-sizing risk multiplier for signal-day top-quartile ATR expansion "
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                ...
                "quality state; this follows the playbook preference for fixed "
            ...
            "2_history_check": {
                ...
                "exp-20260516-020": (
                    "Accepted Technology trend DTE residual scalar; this deliberately "
                    "moves away from nearby scalar retunes to a new orthogonal field."
                ),
            ...
            "3_single_causal_variable": (
                "signal_day_atr_expansion_risk_multiplier with fixed top-quartile state"
            ),
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260516_022_signal_day_atr_expansion_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        "why_not_other_changes": (
            ...
            "insufficient, avoids Space/event-sleeve retunes because the latest peer-state "
            "variants failed, avoids nearby Technology DTE scalar retries after exp-20260516-020, "
            "and avoids candidate-pool expansion because recent breadth additions added noise."
        ),
        "known_risks": [
            "ATR-expansion states can overlap with valid breakout momentum, so a haircut may clip strong-tape winners.",
        ...
        "next_evidence_needed": (
            None
            if selected["passed"]
            else "Do not retry signal-day ATR-expansion haircuts without a different orthogonal discriminator, such as event-quality or forward hold-quality labels."
        ),

"""exp-20260512-032: Space launch/lunar theme-segment risk allocation.

Tests whether the production-visible Space ``theme_segment`` field identifies a
better risk allocation pocket after the accepted exp-20260512-031 stack. This is
not an LLM soft-ranking test; the forward event-state sample is still too thin.
It also does not add noisy tickers. The only changed causal variable is the risk
scalar for official Space signals whose universe-registry theme segment is
``launch_lunar``.
"""
...
from exp_20260512_031_space_iwm_relative_momentum_risk import (
    ...
    BENCHMARK_FIELD,
    SMALLCAP_LEADER_STATE,
    _field_check_iwm_spy_snapshots,
    _run_variant as _run_accepted_exp031_variant,
    _smallcap_relative_state,
)
...
STEM = "space_launch_lunar_theme_risk"
TARGET_THEME_SEGMENT = "launch_lunar"
THEME_SEGMENT_RISK_SCALARS = (0.75, 0.90, 1.10, 1.25)
ACCEPTED_IWM_RELATIVE_STATE_SCALARS = {SMALLCAP_LEADER_STATE: 1.10}
...
def _field_check_theme_segments() -> dict[str, Any]:
    ...
    return {
        "passed": not missing and not mismatches and bool(target_tickers),
        ...
        "mismatched_theme_segment": mismatches,
    }


def _install_space_policy(theme_segment_scalar: float):
    ...
    theme_adjustments: list[dict[str, Any]] = []
    ...
    def generate_wrapper(features_dict, *args, **kwargs):
        ...
        for signal in signals:
            ...
            if ticker not in OFFICIAL_SPACE_TICKERS:
                continue
            segment = _theme_segment_for_ticker(ticker)
            signal["space_theme_segment"] = segment
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            if ticker in OFFICIAL_SPACE_TICKERS and sizing:
                ...
                iwm_scalar = ACCEPTED_IWM_RELATIVE_STATE_SCALARS.get(state, 1.0)
                if iwm_scalar != 1.0:
                    shares_after_accepted = int(sizing.get("shares_to_buy") or 0)
                    _scale_sizing(
                        sizing,
                ...
                if signal.get("space_theme_segment") == TARGET_THEME_SEGMENT:
                    shares_after_iwm = int(sizing.get("shares_to_buy") or 0)
                    _scale_sizing(
                        sizing,
                        theme_segment_scalar,
                        portfolio_value,
                        "space_launch_lunar_theme_segment_risk",
                    )
                    row = _adjustment_row(
                        signal,
                        sizing,
...
def _run_variant(name: str, theme_segment_scalar: float) -> dict[str, Any]:
    ...
    (
        ...
        basket_counts,
        day_counts,
    ) = _install_space_policy(theme_segment_scalar)
    try:
        by_window = {}
    ...
    return {
        "variant": name,
        "target_theme_segment": TARGET_THEME_SEGMENT,
        "space_launch_lunar_theme_segment_scalar": theme_segment_scalar,
        "by_window": by_window,
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_launch_lunar_theme_adjustment"]["adjusted_signal_count"]
    ...
    return {
        ...
        "space_launch_lunar_adjusted_signal_count": adjusted_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space launch/lunar theme-segment risk",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: risk scalar for official Space signals whose "
            "universe-registry `theme_segment` is `launch_lunar`."
        ),
    ...
    for name, variant in payload["variants"].items():
        ...
        lines.append(
            f"| {name} | {variant['space_launch_lunar_theme_segment_scalar']:.2f} | "
            ...
            f"{gate['space_launch_lunar_adjusted_signal_count']} |"
        )
    lines.extend(
        [
            "",
            ...
            (
                ...
                "dPnL | Trades | Max DD | Survival | Launch/lunar signals |"
            ),
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for label in WINDOWS:
        ...
        adjusted = best["by_window"][label]["space_launch_lunar_theme_adjustment"][
...
def run() -> dict[str, Any]:
    ...
    registry_gate = _field_check_theme_segments()
    if not registry_gate["passed"]:
        raise RuntimeError(f"Theme segment field check failed: {registry_gate}")
    ...
    before = _run_accepted_exp031_variant(
        "accepted_exp031_stack",
        ACCEPTED_IWM_RELATIVE_STATE_SCALARS,
    )
    variants = {}
    for scalar in THEME_SEGMENT_RISK_SCALARS:
        name = f"launch_lunar_{str(scalar).replace('.', '_')}"
    ...
    decision = (
        "accepted_default_off_space_launch_lunar_theme_risk"
        if accepted
        else "rejected_space_launch_lunar_theme_risk"
    )
    interpretation = (
        "The launch/lunar theme-segment risk scalar improved the accepted "
        "default-off Space stack under the three-window gate. Promotion must be "
        "shared production-visible metadata/helper wiring only; live Space slots "
        "remain zero."
        if accepted
        else (
            "The launch/lunar theme-segment risk scalar did not clear the "
            "three-window gate on top of exp-20260512-031. Do not retry adjacent "
            "launch/lunar scalar values on these frozen snapshots; the next Space "
            "edge needs forward catalyst replacement value or a different "
    ...
    payload = {
        ...
        "single_causal_variable": (
            "risk scalar for official Space signals keyed only to the production "
            "universe-registry theme_segment == launch_lunar"
        ),
        "hypothesis": (
            "After Space TQS, IWM-relative momentum, peer-nonleader breakout, and "
            "basket-positive risk were accepted or fixed, the strongest remaining "
            "production-visible catalyst-quality field is the official Space "
            "theme_segment. Launch/lunar names should have better repeatable "
            "replacement value than noisier satellite-data and mature satcom "
            "adjacent names without adding tickers or relying on thin LLM outcomes."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
            ...
            "2_history_check": {
                "exp-20260512-031": (
                    "Accepted IWM>SPY small-cap leader 1.10x scalar; this is the "
                    "fixed before state."
                ),
                "exp-20260512-023": (
                    "Rejected GSAT candidate-pool expansion; no noisy ticker "
                    "expansion in this test."
                ),
                "exp-20260511-026": (
                    "Rejected mature satcom breadth, so this avoids IRDM/VSAT/SATS."
                ),
                "exp-20260512-014/015/016/019/021": (
                    "Rejected nearby peer-trend, 52w, breadth, R/R, and breakout "
                    "volume refinements."
                ),
            },
            "3_single_causal_variable": (
                "space_launch_lunar_theme_segment_risk_scalar. Candidate pool, "
            ...
            "4_acceptance_standard": (
                ...
                "regressed window, max drawdown drift <= 0.5 pp, survival >= 5%, "
                ">=50 total trades, and nonzero adjusted launch/lunar signals."
            ),
            "5_reproducibility": (
                "This script reruns core, accepted exp-20260512-031 Space stack, "
                "and each launch/lunar theme-segment scalar across the canonical "
        ...
        "parameters": {
            ...
            "target_theme_segment": TARGET_THEME_SEGMENT,
            "target_theme_tickers": sorted(
                ticker
                for ticker, segment in SPACE_THEME_SEGMENTS.items()
                if segment == TARGET_THEME_SEGMENT
            ),
            "tested_theme_segment_scalars": list(THEME_SEGMENT_RISK_SCALARS),
            "accepted_before_experiment": "exp-20260512-031",
        ...
        "backtest_protocol": (
            ...
            "The accepted_before variant reproduces exp-20260512-031 policy semantics."
        ),
        "gate1": {
        ...
        "gate2": {
            ...
            "theme_segment_registry": registry_gate,
            "iwm_spy_snapshot_coverage": benchmark_gate,
            "passed": gate2["passed"]
            and registry_gate["passed"]
        ...
        "next_evidence_needed": (
            "If rejected, do not retry adjacent launch/lunar theme-segment scalars "
        ...
        "related_files": [
            "quant/experiments/exp_20260512_032_space_launch_lunar_theme_risk.py",
            "data/experiments/exp-20260512-032/space_launch_lunar_theme_risk.json",
            "docs/experiments/logs/exp-20260512-032.json",
            "docs/experiments/tickets/exp-20260512-032.json",
            "docs/experiments/artifacts/exp-20260512-032_space_launch_lunar_theme_risk.md",
        ...
        "why_not_other_changes": (
            ...
            "R/R, and volume variants are either accepted context or anti-repeat."
        ),
    }
    return payload

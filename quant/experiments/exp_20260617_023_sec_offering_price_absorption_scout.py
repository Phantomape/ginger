"""exp-20260617-023: SEC offering price-absorption candidate scout.

Replay-only alpha search. The single decision hypothesis is that PIT SEC
offering/prospectus filings, which are usually dilution or financing overhang,
can become positive candidate-pool events when the signal-day price action
absorbs the filing shock versus SPY before the next-open paper entry.

This is distinct from generic SEC contract-economics text parsing: the form
...
STEM = "sec_offering_price_absorption_scout"
TRIAL_FAMILY = "free_sec_submissions_offering_price_absorption_candidate_pool"
TRIAL_VARIANT_ID = "sec_offering_absorption_top1_next_open_10d_v1"
CHANGED_VARIABLE = "sec_offering_price_absorption_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_023_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "offering_events_are_dilution_overhang",
        "thin_sample",
        "old_thin_window_regression",
        "accepted_distribution_comparator_not_beaten",
    ],
    "confidence_reason": (
        "This is a materially distinct SEC submissions event source rather than "
        "a Companyfacts ratio or generic SEC contract text. Prior SEC contract "
        "parsing explicitly excluded underwriting/common-stock/ATM/offering "
        "text, and supply-overhang absorption variants show the mechanism can "
        "matter, but financing events may simply be dilution noise after "
        "next-open execution and accepted comparators."
    ),
    "recorded_at": "2026-06-17T18:03:44+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        ...
        "failure_handling": (
            "missing SEC submissions cache, missing CIK mapping, missing OHLCV, "
    ...
    "parity_note": (
        ...
        "SEC offering-form events, acceptance-time signal date, price-absorption "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: PIT SEC offering/prospectus filings that are normally "
        "dilution-overhang negative may become positive when the signal-day "
        "price action absorbs the financing shock versus SPY, indicating "
        "institutional demand and validated capital use before a 10-trading-day "
        "continuation leg."
    ),
    "2_history_check": {
        "exp-20260617-011": (
            "Rejected SEC contract-economics text. Its regex explicitly excluded "
            "underwriting, ATM offering, common stock, preferred stock, warrants, "
            "convertibles, and debt, so this run tests the excluded financing "
            "event family directly rather than contract text."
        ),
        "exp-20260616-015": (
            "Accepted SBC burden improvement as dilution-quality Companyfacts "
            "source. This run is a distinct SEC filing event, not SBC or "
            "Companyfacts dilution-ratio tuning."
        ),
        "exp-20260612-023": (
            "Rejected Form 144 sale-notice absorption after drawdown drift. This "
            "run tests issuer financing/prospectus filings, not insider planned "
            "sale notices."
        ),
        "exp-20260613-013": (
            "Rejected isolated Form 144 sale-notice absorption. This run uses "
            "issuer SEC offering forms plus signal-day price absorption."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_023_sec_offering_price_absorption_scout.py"
    ),
}
...
def _load_event_index() -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        if events:
            ...
            index[ticker] = events
            stats["tickers_with_offering_events"] += 1
            stats["offering_event_count"] += len(events)
...
def _build_quality_index(
    ...
    return index, {
        **summary,
        ...
        "field_source": "sec_submissions_offering_forms_not_companyfacts",
    }
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "SEC_OFFERING_PRICE_ABSORPTION_PAPER",
                    "candidate_score": _round(score, 6),
                    "rule_version": RULE_VERSION,
                    "source_rule_version": RULE_VERSION,
                    "known_at": "sec_offering_form_and_signal_close_before_next_open_paper_entry",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    "offering_form": event["form"],
                    "offering_filing_date": event["filing_date"],
                    "offering_accepted_after_close": event["accepted_after_close"],
                    "offering_acceptance_datetime": event["acceptance_datetime"],
                    "offering_accession_number": event["accession_number"],
                    "offering_primary_document": event["primary_document"],
                    "offering_form_weight": event["form_weight"],
                    **confirm,
                }
            )
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_sec_offering_price_absorption"
        if gate["passed"]
        else "rejected_sec_offering_price_absorption_candidate_pool"
    )
    return gate
...
def _interpretation(payload: dict[str, Any]) -> str:
    ...
    if gate4["passed"]:
        return (
            "The SEC offering price-absorption source cleared the numeric "
            "three-window replay screen, but remains only a replay lead because "
            "no shared daily/backtest helper was promoted."
        )
    return (
        "The SEC offering price-absorption source did not clear Gate 4 "
        f"(failed: {', '.join(gate4['failed_reasons']) or 'none'}). The fixed "
        "bundle tested issuer offering/prospectus forms plus signal-day "
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_submissions_offering_candidate_pool",
            "new_evidence_type": "sec_submissions_offering_forms_with_price_absorption",
    ...
    payload["parameters"] = {
        ...
        "offering_forms": sorted(OFFERING_FORMS),
        "excluded_overbroad_forms": ["FWP", "424B3", "424B2"],
        "min_price": MIN_PRICE,
        "min_avg_dollar_volume_20d": MIN_AVG_DOLLAR_VOLUME_20D,
        "min_signal_return": MIN_SIGNAL_RETURN,
        "min_signal_excess_spy": MIN_SIGNAL_EXCESS_SPY,
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SEC offering events are read from EDGAR submissions cache recent "
        "filings for forms 424B5/424B4/424B7/S-3/S-3ASR/F-3/F-3ASR/S-1/S-1/A. "
        ...
        "non-negative daily return, return minus SPY >= 1%, close location >= "
        "0.58, volume ratio >= 0.80, realized vol <= 12%, ret20 excess vs SPY "
    ...
    payload["next_evidence_needed"] = (
        "If this fixed offering-absorption bundle fails, do not retry by "
        "sweeping offering form lists, signal excess, close-location, volume, "
        "volatility, ret20, price/ADV, event-age, top-N, hold days, cooldown, or "
        "notional on these frozen windows. A valid retry needs materially richer "
        "PIT financing economics such as offering amount normalized by market "
        "cap, use-of-proceeds, debt vs equity classification, ATM drawdown "
        "amounts, or closed forward replacement-value rows."
    )
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping offering form lists, signal excess, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} SEC Offering Price Absorption",
            "",

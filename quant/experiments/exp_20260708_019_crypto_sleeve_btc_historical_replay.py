"""exp-20260708-019: BTC crypto sleeve shared-policy multi-year historical replay.

Observed-only alpha attribution. The single question is whether the existing
production BTC/USD crypto sleeve target policy (EMA20/EMA100/SMA200 daily trend
switch), replayed through the exact shared production policy functions in
quant/crypto_sleeve.py, adds risk-adjusted value versus fee-aware BTC
buy-and-hold over multi-year historical daily bars covering the 2018 and 2022
bear markets.

This is the legal retry axis declared by exp-20260708-010
("a shared historical replay gate shape"). The runner changes no shared policy,
target threshold, live config, stock order, crypto order, ranking, sizing,
exit rule, or LLM boundary.
"""
...
EXPERIMENT_ID = "exp-20260708-019"
OWNER = "alpha-explore"
SLUG = "crypto_sleeve_btc_historical_replay"
RUNNER = f"quant/experiments/exp_20260708_019_{SLUG}.py"
...
OUT_JSON = DATA_DIR / f"exp_20260708_019_{SLUG}.json"
BARS_JSON = DATA_DIR / "btc_usd_daily_closes.json"
...
HYPOTHESIS = (
    "Observed-only alpha: the production BTC/USD crypto sleeve daily "
    "EMA20/EMA100/SMA200 target policy, replayed through the exact shared "
    "production policy functions over multi-year BTC-USD daily history "
    "(2017-2026 including the 2018 and 2022 bear markets), should add "
    "risk-adjusted value versus fee-aware BTC buy-and-hold if the daily trend "
    "switch is a real edge rather than a short-sample artifact."
)
...
TRIAL_FAMILY = "btc_spot_crypto_sleeve_daily_trend_policy_historical_replay"
TRIAL_VARIANT_ID = "btc_daily_2017_2026_three_windows_v1"
CHANGED_VARIABLE = "crypto_sleeve_shared_policy_multi_year_btc_historical_replay_v1"
NEW_EVIDENCE_TYPE = "new_gate_shape_shared_historical_replay_on_new_btc_history"
NEW_EVIDENCE_AXIS = (
    "New gate shape plus materially new data coverage: shared historical "
    "replay of the exact production crypto_sleeve policy functions over "
    "roughly nine years of BTC-USD daily bars covering the 2018 and 2022 "
    "bear markets, versus exp-20260708-010 which only attributed 66 saved "
    "2026 forward snapshots; this is the legal retry axis explicitly declared "
    "in exp-20260708-010 next_retry_requires (a shared historical replay "
    "gate shape)."
)
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "yfinance_history_unavailable",
        "window_fragility",
    ],
    "confidence_reason": (
        "Daily trend filters on BTC historically cut drawdown deeply but "
        "often lag buy-and-hold on raw return; the policy plausibly wins "
        "risk-adjusted EV in bear-containing windows and loses the 2023-2026 "
        "bull window, so the predeclared 2-of-3 window rule is a real coin "
        "flip."
    ),
    "recorded_at": "2026-07-08T16:13:31+00:00",
}
...
CONFIG = {
    ...
    "benchmark": "fee_aware_btc_buy_and_hold_per_window",
    "acceptance_rule": (
        "positive lead iff policy EV (total_return_pct * sharpe_daily) beats "
        "fee-aware buy-and-hold EV in >=2 of 3 fixed windows AND policy max "
        "drawdown < buy-and-hold max drawdown in all 3 windows AND aggregate "
        "policy total return > 0"
    ),
}
...
def simulate_segment(
    ...
    for index in range(len(rows) - 1):
        ...
        if abs(delta) >= min_rebalance_delta_pct:
            ...
            position = desired
            target_switches += 1
        policy_returns.append(position * btc_return - rebalance_fee)
        buy_hold_fee = fee if index == 0 else 0.0
        if index == len(rows) - 2:
            buy_hold_fee += fee
        buy_hold_returns.append(btc_return - buy_hold_fee)
...
def build_gate4(
    ...
    return {
        ...
        "binding_acceptance_note": (
            "Observed-only historical replay attribution of the unchanged "
            "production crypto sleeve policy. Either verdict changes no "
            "production behavior; a policy change would need its own shared "
            "policy experiment."
        ),
    }
...
def build_payload() -> dict[str, Any]:
    ...
    aggregate = simulate_segment(
        aggregate_rows, fee, min_delta, annualization_days, "aggregate_2017_2026"
    )
    ...
    if positive_lead:
        ...
        why = (
            ...
            "BTC buy-and-hold on the predeclared risk-adjusted window rule over "
            "multi-year history; this validates keeping the current sleeve "
            "policy but changes nothing by itself."
        )
        observed_only_lead = True
    else:
        status = "observed_only"
        decision = "observed_only_rejected_crypto_sleeve_historical_replay"
        ...
        why = (
            ...
            "BTC buy-and-hold: " + rejection_reason
        )
    ...
    payload: dict[str, Any] = {
        ...
        "gate1": {
            ...
            "crypto_baselines": ["fee_aware_btc_buy_and_hold_per_window"],
            "note": (
                "No stock strategy behavior changed. The crypto comparator is "
                "constructed from the same replayed BTC daily closes."
            ),
        },
        "gate2": {
        ...
        "production_impact": {
            ...
            "parity_note": (
                "Read-only historical replay through the same production "
                "policy functions; the production crypto sleeve remains as-is."
            ),
        },
        "post_run_reflection": {
            ...
            "forbidden_near_neighbor_retry": (
                ...
                "same replayed BTC history to flip the verdict."
            ),
            "new_evidence_required": (
                "A legal follow-up is either a separately predeclared shared "
                "policy change with its own Gate 1-4 replay (if the verdict "
                "motivates one), a different crypto data source or asset, or "
                "materially more settled production crypto sleeve forward "
                "snapshots."
            ),
        },
        "rejection_reason": rejection_reason,
        "next_retry_requires": [
            "separately predeclared shared policy change with its own gates",
            "or a different crypto data source or asset",
...
def build_report(payload: dict[str, Any]) -> str:
    gate4 = payload["gate4"]
    lines = [
        f"# {EXPERIMENT_ID}: BTC Crypto Sleeve Shared-Policy Historical Replay",
        "",

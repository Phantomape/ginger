"""exp-20260617-024: SEC S-8 employee-equity registration absorption scout.

Replay-only alpha search. The single decision hypothesis is that PIT SEC
S-8 employee-equity registration filings, which can be dilution/compensation
overhang, become positive candidate-pool events when signal-day price action
absorbs the filing versus SPY before the next-open paper entry.

This is distinct from exp-20260617-023 offering/prospectus absorption: S-8 and
S-8 POS register securities for employee benefit plans rather than issuer
financing/prospectus distribution. The form type itself is the event source, and
no LLM/text semantics are used. No production code, shared adapter, live/default
...
STEM = "s8_employee_equity_registration_absorption_scout"
TRIAL_FAMILY = "free_sec_submissions_s8_employee_equity_absorption_candidate_pool"
TRIAL_VARIANT_ID = "s8_employee_equity_absorption_top1_next_open_10d_v1"
CHANGED_VARIABLE = "sec_s8_employee_equity_registration_absorption_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_024_{STEM}.json"
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "S-8_events_are_routine_dilution_noise",
        "overlaps_accepted_SBC_growth_quality",
        "old_thin_window_regression",
        "accepted_distribution_comparator_not_beaten",
    ],
    "confidence_reason": (
        "S-8 employee-equity registration is a broad free SEC submissions event "
        "with adequate three-window coverage and is not included in the rejected "
        "exp-20260617-023 offering/prospectus form list. The mechanism is that "
        "employee-equity capacity can be a growth/talent investment signal when "
        "price absorbs dilution, but the likely disconfirmer is routine filing "
        "noise or overlap with the accepted SBC burden-improvement source."
    ),
    "recorded_at": "2026-06-17T19:07:24+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        ...
        "failure_handling": (
            "missing SEC submissions cache, missing CIK mapping, missing S-8 "
            "event rows, missing OHLCV, missing next open, or missing 10d exit "
    ...
    "parity_note": (
        ...
        "SEC S-8/S-8 POS events, acceptance-time signal date, price-absorption "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: PIT SEC S-8 employee-equity registration filings may "
        "be positive when signal-day price action absorbs the dilution/talent-"
        "investment signal versus SPY before next-open paper entry, identifying "
        "growth names where employee-equity capacity is accepted rather than "
        "punished."
    ),
    "2_history_check": {
        "exp-20260617-023": (
            ...
            "tests S-8 employee benefit plan registration as a different SEC "
            "submissions form family."
        ),
        "exp-20260616-015": (
            "Accepted SBC burden improvement as a raw Companyfacts dilution-"
            "quality source. This run tests a filing event plus price absorption, "
            "not SBC/revenue thresholds or shared SBC helper tuning."
        ),
        "exp-20260616-017": (
            "Rejected per-share SBC net of buybacks. This run avoids per-share, "
            "buyback, and share-count ratio refinements."
        ),
        "exp-20260617-011": (
            "Rejected SEC contract-economics text. Its regex explicitly excluded "
            "equity incentive terms from contract parsing, so this run tests "
            "the event-form family directly rather than SEC text semantics."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_024_s8_employee_equity_registration_absorption_scout.py"
    ),
}
...
def _load_event_index() -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        if events:
            ...
            index[ticker] = events
            stats["tickers_with_s8_events"] += 1
            stats["s8_event_count"] += len(events)
...
def _build_quality_index(
    ...
    return index, {
        **summary,
        ...
        "field_source": "sec_submissions_s8_forms_not_companyfacts",
    }
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "SEC_S8_EMPLOYEE_EQUITY_ABSORPTION_PAPER",
                    "candidate_score": _round(score, 6),
                    "rule_version": RULE_VERSION,
                    "source_rule_version": RULE_VERSION,
                    "known_at": "sec_s8_form_and_signal_close_before_next_open_paper_entry",
                    ...
                    "trade_enabled": False,
                    "s8_form": event["form"],
                    "s8_filing_date": event["filing_date"],
                    "s8_accepted_after_close": event["accepted_after_close"],
                    "s8_acceptance_datetime": event["acceptance_datetime"],
                    "s8_accession_number": event["accession_number"],
                    "s8_primary_document": event["primary_document"],
                    "s8_form_weight": event["form_weight"],
                    **confirm,
                }
            )
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_sec_s8_employee_equity_absorption"
        if gate["passed"]
        else "rejected_sec_s8_employee_equity_absorption_candidate_pool"
    )
    return gate
...
def _interpretation(payload: dict[str, Any]) -> str:
    ...
    if gate4["passed"]:
        return (
            "The SEC S-8 employee-equity absorption source cleared the numeric "
            "three-window replay screen, but remains only a replay lead because "
            "no shared daily/backtest helper was promoted."
        )
    return (
        "The SEC S-8 employee-equity absorption source did not clear Gate 4 "
        f"(failed: {', '.join(gate4['failed_reasons']) or 'none'}). The fixed "
        "bundle tested S-8/S-8 POS employee benefit plan registration forms "
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_submissions_s8_employee_equity_candidate_pool",
            "new_evidence_type": "sec_submissions_s8_employee_equity_forms_with_price_absorption",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "SEC S-8 employee-equity events are read from EDGAR submissions cache "
        "recent filings for forms S-8 and S-8 POS. "
    ...
    payload["gate2"]["runtime_fields"] = [
        "SEC submissions recent.form (S-8/S-8 POS)",
    ...
    payload["next_evidence_needed"] = (
        "If this fixed S-8 employee-equity absorption bundle fails, do not retry "
        "by sweeping S-8/S-8 POS form lists, signal excess, close-location, "
        ...
        "materially richer PIT employee-equity economics such as registered "
        "share amount normalized by float/market cap, plan amendment context, "
        "joined SBC-burden forward rows, or closed forward replacement-value "
        "observations."
    )
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping S-8/S-8 POS form lists, signal excess, "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} SEC S-8 Employee-Equity Absorption",
            "",

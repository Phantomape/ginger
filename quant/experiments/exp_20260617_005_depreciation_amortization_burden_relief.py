"""exp-20260617-005: depreciation/amortization burden relief candidate scout.
...
annual depreciation and amortization burden (D&A / revenue) is falling while
revenue is not contracting, and whose price already shows liquid SPY-relative
leadership, may identify asset-light operating leverage candidates with 10-day
continuation.
...
carry a canonical D&A burden field. No production code, shared adapter,
...
EXPERIMENT_ID = "exp-20260617-005"
STEM = "depreciation_amortization_burden_relief"
TRIAL_FAMILY = "depreciation_amortization_burden_relief_candidate_pool"
TRIAL_VARIANT_ID = "depreciation_amortization_burden_relief_top1_next_open_10d_v1"
CHANGED_VARIABLE = "raw_sec_companyfacts_depreciation_amortization_burden_relief_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260617_005_{STEM}.json"
...
MIN_DA_EXPENSE = 25_000_000.0
MIN_CURRENT_DA_TO_REVENUE = 0.005
MAX_CURRENT_DA_TO_REVENUE = 0.25
MIN_DA_RATIO_IMPROVEMENT = 0.002
MIN_REVENUE_GROWTH = -0.05
MAX_DA_GROWTH_MINUS_REVENUE_GROWTH = -0.03
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "depreciation_field_semantics_noisy",
        "accepted_distribution_comparator_not_beaten",
    ],
    "confidence_reason": (
        "Raw SEC D&A expense burden is distinct from recent low-capex, "
        "operating-efficiency, SG&A, lease, and debt fields. The economic edge "
        "is asset-light operating leverage when D&A/revenue falls while revenue "
        "is stable and price confirms demand. The disconfirmer is that recent "
        "Companyfacts quality candidate pools are saturated and often fail "
        "window, drawdown, or accepted-comparator gates."
    ),
    "recorded_at": "2026-06-17T03:04:38+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC annual D&A/revenue pair, missing prior annual "
    ...
    "parity_note": (
        ...
        "SEC tag mapping, filed-date PIT annual D&A/revenue burden-relief gate, "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts annual depreciation and "
        "amortization burden relief (D&A/revenue falling year over year, with "
        "revenue not contracting and D&A growing slower than revenue) paired "
        "with liquid SPY-relative leadership may identify asset-light operating "
        "leverage candidates with 10-day continuation beyond generic momentum."
    ),
    "2_history_check": {
        "exp-20260529-003": (
            "Rejected low-capex-intensity support inside the older FGRS stack. "
            "This run is not a capex/revenue scalar; it tests a standalone "
            "candidate source based on filed D&A burden relief."
        ),
        "exp-20260613-031": (
            "Rejected operating efficiency/assets candidate selection. This "
            "run uses period-flow D&A/revenue trajectory rather than a static "
            "asset-efficiency selector."
        ),
        "exp-20260616-010": (
            "Rejected SG&A operating-expense leverage. This run excludes SG&A "
            "and tests asset-consumption burden, not selling/marketing expense."
        ),
        "exp-20260616-025": (
            "Rejected operating lease burden relief. This run does not use "
            "lease liabilities or ROU amortization as the decision field."
        ),
        "exp-20260617-003": (
            "Blocked most non-repeat surfaces after CCC. This D&A field was not "
            "one of the blocked surfaces and has raw Companyfacts coverage."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_005_depreciation_amortization_burden_relief.py"
    ),
}
...
def _latest_da_revenue_pair(
    facts: dict[str, list[dict[str, Any]]],
    *,
    asof: str,
    before_end: str | None = None,
) -> tuple[dict[str, Any], dict[str, Any]] | None:
    da = _latest_period_fact(facts["da_expense"], asof=asof, before_end=before_end)
    if da is None:
        return None
    revenue = _latest_period_fact(facts["revenue"], asof=asof, end=da["end"])
    if revenue is None:
        return None
    return da, revenue
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        da_facts = _raw_annual_facts(usgaap, DA_TAGS)
        revenue_facts = _raw_annual_facts(usgaap, REVENUE_TAGS)
        if not da_facts:
            stats["tickers_missing_raw_annual_da"] += 1
            continue
        ...
        stats["tickers_with_raw_annual_da_and_revenue"] += 1
        stats["raw_annual_da_fact_count"] += len(da_facts)
        stats["raw_annual_revenue_fact_count"] += len(revenue_facts)

    summary = {
        "raw_companyfacts_cache": _repo_rel(RAW_COMPANYFACTS_CACHE),
        "da_tags": list(DA_TAGS),
...
def _da_burden_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    current_pair = _latest_da_revenue_pair(facts, asof=asof)
    if current_pair is None:
        return None
    current_da, current_revenue = current_pair
    if base._days_between(asof, current_da["filed"]) > MAX_ANNUAL_FACT_AGE_DAYS:
        return None

    prior_pair = _latest_da_revenue_pair(facts, asof=asof, before_end=current_da["end"])
    if prior_pair is None:
        return None
    prior_da, prior_revenue = prior_pair
    ...
    if min(current_da_value, current_revenue_value, prior_da_value, prior_revenue_value) <= 0.0:
        return None
    if current_revenue_value < MIN_CURRENT_REVENUE or current_da_value < MIN_DA_EXPENSE:
        return None

    current_ratio = current_da_value / current_revenue_value
    prior_ratio = prior_da_value / prior_revenue_value
    ...
    growth_spread = da_growth - revenue_growth

    if current_ratio < MIN_CURRENT_DA_TO_REVENUE:
        return None
    if current_ratio > MAX_CURRENT_DA_TO_REVENUE:
        return None
    if ratio_improvement < MIN_DA_RATIO_IMPROVEMENT:
        return None
    if revenue_growth < MIN_REVENUE_GROWTH:
        return None
    if growth_spread > MAX_DA_GROWTH_MINUS_REVENUE_GROWTH:
        return None

    return {
        "current_fiscal_year_end": current_da["end"],
        "prior_fiscal_year_end": prior_da["end"],
        "current_da_filed": current_da["filed"],
        "prior_da_filed": prior_da["filed"],
        "current_revenue_filed": current_revenue["filed"],
        "prior_revenue_filed": prior_revenue["filed"],
        "current_da_tag": current_da["tag"],
        "current_revenue_tag": current_revenue["tag"],
        "current_da_expense": _round(current_da_value, 2),
        "prior_da_expense": _round(prior_da_value, 2),
        "current_revenue": _round(current_revenue_value, 2),
        "prior_revenue": _round(prior_revenue_value, 2),
        "current_da_to_revenue": _round(current_ratio, 6),
        "prior_da_to_revenue": _round(prior_ratio, 6),
        "da_ratio_improvement": _round(ratio_improvement, 6),
        "da_expense_growth": _round(da_growth, 6),
        "revenue_growth": _round(revenue_growth, 6),
        "da_growth_minus_revenue_growth": _round(growth_spread, 6),
        "fact_age_days": base._days_between(asof, current_da["filed"]),
    }
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _da_burden_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_da_burden_gate"] += 1
                continue
            ...
            score = (
                1.65 * min(float(quality["da_ratio_improvement"] or 0.0), 0.12)
                + 0.70 * max(min(-float(quality["da_growth_minus_revenue_growth"] or 0.0), 0.60), -0.10)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_DA_BURDEN_RELIEF_PAPER",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    **{f"da_{key}": value for key, value in quality.items()},
                    **confirm,
                }
            )
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["da_da_ratio_improvement"] or 0.0),
            float(row["da_da_growth_minus_revenue_growth"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        ...
        "min_da_expense": MIN_DA_EXPENSE,
        "min_current_da_to_revenue": MIN_CURRENT_DA_TO_REVENUE,
        "max_current_da_to_revenue": MAX_CURRENT_DA_TO_REVENUE,
        "min_da_ratio_improvement": MIN_DA_RATIO_IMPROVEMENT,
        "min_revenue_growth": MIN_REVENUE_GROWTH,
        "max_da_growth_minus_revenue_growth": MAX_DA_GROWTH_MINUS_REVENUE_GROWTH,
    }
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_depreciation_amortization_burden_relief"
        if gate["passed"]
        else "rejected_depreciation_amortization_burden_relief_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The raw SEC D&A burden-relief source cleared the numeric "
            "three-window replay screen, but remains only a replay lead because "
            "no shared daily/backtest helper was promoted."
        )
    else:
        interpretation = (
            "The raw SEC D&A burden-relief source did not clear Gate 4 "
    ...
    payload.update(
        {
            ...
            "mechanism_family": (
                "production_visible_free_sec_companyfacts_asset_light_quality_candidate_pool"
            ),
            "new_evidence_type": "raw_sec_companyfacts_depreciation_amortization_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual D&A expense and revenue are read from raw SEC Companyfacts tags "
        "and are known only by their filed date (<= signal date). Current and "
        "prior annual D&A values are matched to same-period annual revenue by "
        "fiscal year end. The rule requires D&A/revenue burden to fall while "
        "revenue is not contracting and D&A grows at least 3pp slower than "
    ...
    payload["gate2"]["runtime_fields"] = [
        "raw SEC companyfacts depreciation/amortization annual facts",
    ...
    payload["next_evidence_needed"] = (
        "A retry needs materially different PIT asset-light evidence, such as "
        "capex-to-D&A replacement-cycle context, fixed-asset turnover by "
        "industry, or closed forward replacement-value rows. Do not sweep D&A "
        "tags, D&A/revenue thresholds, revenue floor, freshness, RS/close/"
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping D&A/revenue burden, D&A growth spread, revenue "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Depreciation/Amortization Burden Relief",
            "",

...
Item 7.01 / 8.01 filings whose issuer also shows same-day positive excess
reaction, liquidity, trend, and relative-strength confirmation.
...
EXPERIMENT_ID = "exp-20260529-015"
STEM = "sec_fd_other_8k_positive_reaction_candidate_pool"
TRIAL_FAMILY = "sec_fd_other_8k_positive_reaction_candidate_pool"
CHANGED_VARIABLE = "sec_fd_other_8k_positive_reaction_candidate_source_v1"
RULE_VERSION = "sec_fd_other_8k_positive_reaction_v1"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260529_015_{STEM}.json"
...
_SEC_FD_OTHER_EVENTS_CACHE: list[dict[str, Any]] | None = None
...
def _load_sec_fd_other_events() -> list[dict[str, Any]]:
    global _SEC_FD_OTHER_EVENTS_CACHE
    if _SEC_FD_OTHER_EVENTS_CACHE is not None:
        return _SEC_FD_OTHER_EVENTS_CACHE
    ...
    _SEC_FD_OTHER_EVENTS_CACHE = sorted(
        deduped.values(),
        key=lambda row: (
...
def _candidate_rows_for_window(
    ...
    events = [
        event for event in _load_sec_fd_other_events() if event["usable_trade_date"] in dates
    ]
    ...
    for event in events:
        ...
        if key in seen_ticker_dates:
            audit["duplicate_ticker_date_fd_other"] += 1
            continue
        ...
        candidates.append(
            {
                ...
                "fd_other_candidate_score": framework.base._round(score, 6),
    ...
    candidates.sort(
        key=lambda row: (
            row["date"],
            -float(row["fd_other_candidate_score"]),
    ...
    return candidates, {
        ...
        "sec_fd_other_events_in_window": len(events),
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    gate4 = payload["gate4"]
    decision = (
        "accepted_candidate_sec_fd_other_8k_positive_reaction"
        if gate4["passed"]
        else "rejected_sec_fd_other_8k_positive_reaction"
    )
    all_target_trades = [
        trade
    ...
    prediction = {
        ...
        "main_failure_modes": [
            "event item code not alpha",
            "same-day reaction overfits",
            "late_strong regression",
            "concentration",
        ],
        "confidence_reason": (
            "Meta research favors production-visible candidate-pool alpha, while "
            "nearby SEC peer-transfer and 10-K variants recently failed. FD/Other "
            "positive issuer reaction is distinct and sample-rich enough to audit."
        ),
        "recorded_at": "2026-05-29T13:22:10+00:00",
    ...
    payload.update(
        {
            ...
            "decision": decision,
            "hypothesis": (
                "PIT-safe SEC 8-K Item 7.01/8.01 FD/Other filings with positive "
                "same-day issuer reaction, liquidity, trend, and relative-strength "
                "confirmation may provide a free-data continuation candidate pool."
            ),
            ...
            "new_evidence_type": (
                "production_visible_free_sec_item_code_plus_ohlcv_reaction"
            ),
            ...
            "parameters": {
                ...
                "source_definition": [
                    ...
                    "item codes must include 7.01 or 8.01",
                    "earnings 2.02, capital-contract, debt, offering, and governance item codes are excluded",
            ...
            "gate_questions": {
                "1_alpha_hypothesis": (
                    "candidate_pool / entry: FD/Other 8-K disclosures with an "
                    "immediate positive issuer reaction may identify incremental "
                    "continuation candidates beyond earnings and annual reports."
                ),
                "2_history_check": {
                    "exp-20260504-037": (
                        "Tested FD/Other negative-reaction satellite overlay. "
                        "This run tests positive-reaction continuation as a direct "
                        "issuer candidate pool, not negative mean reversion."
                    ),
                    "exp-20260529-010_and_011": (
                        "SEC Item 2.02 peer-transfer variants were rejected. This "
                        "run excludes Item 2.02 and uses direct issuer FD/Other rows."
                    ),
                    "exp-20260529-014": (
                        "Direct 10-K liquidity/RS filing source was rejected. This "
                        "run uses 8-K FD/Other current-report events."
                    ),
                    "recent_dividend_scout": (
                        "A pre-run deterministic SEC dividend-increase text scout "
                        "left only six paper trades and negative PnL, so it was not "
                        "chosen as the formal experiment."
                    ),
                },
                ...
                "5_reproducibility": (
                    ...
                    "exp_20260529_015_sec_fd_other_8k_positive_reaction_candidate_pool.py"
                ),
            },
            "why_not_other_changes": (
                "Skipped LLM soft-ranking because replay attribution remains sparse. "
                "Skipped Companyfacts, VBB, VCP, state-surface, and OHLCV pattern-name "
                "retunes per playbook freeze guidance. This run tests one free SEC "
                "event candidate-source variable only."
            ),
            "production_impact": {
            ...
            "production_parity": {
                ...
                "parity_note": (
                    ...
                    "promotion would require a shared SEC FD/Other paper sleeve with "
                    "exact item-code, as-of OHLCV, and entry/exit parity tests."
                ),
            },
            "interpretation": (
                "The SEC FD/Other 8-K positive-reaction candidate pool cleared Gate 4 "
                "as a default-off replay lead, but no production/shared policy was promoted."
                if gate4["passed"]
                else (
                    "The SEC FD/Other 8-K positive-reaction candidate pool did not "
                    "clear Gate 4. Do not promote it or retry nearby FD/Other "
                    "positive-reaction thresholds on the same frozen windows without "
                    "new forward rows or a sharper structured event-quality field."
                )
            ),
            "rejection_reason": None if gate4["passed"] else "; ".join(gate4["failed_reasons"]),
            "next_evidence_needed": (
                "Forward replacement-value rows or a materially sharper structured "
                "SEC event-quality field such as official-customer source, contract "
                "value, or disclosure-quality provenance."
            ),
    ...
    payload["gate2"]["runtime_field_coverage"] = {
        "sec_events": {
            ...
            "events_loaded": len(_load_sec_fd_other_events()),
        }
    }
...
def _persist(payload: dict[str, Any]) -> None:
    ...
    ticket_payload = {
        "experiment_id": EXPERIMENT_ID,
        "title": "SEC FD/Other 8-K positive-reaction candidate pool",

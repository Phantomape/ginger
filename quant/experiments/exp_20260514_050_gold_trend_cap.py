"""exp-20260514-050: Gold trend near-high cap scout.

Tests one production-visible allocation variable on the accepted core stack:
already-qualified GLD/IAU trend-near-high commodity signals may be capped too
tightly by the generic Commodity trend 50% max-position cap. This does not
alter entries, exits, ranking, raw Commodity multipliers, SLV handling, LLM/news,
...
EXPERIMENT_ID = "exp-20260514-050"
EXPERIMENT_SLUG = "gold_trend_near_high_cap"
CAP_KEY = "trend_gold_near_high_max_position_pct_applied"
...
def _is_target_sleeve(sig: dict[str, Any], sizing: dict[str, Any]) -> bool:
    return bool(
        str(sig.get("ticker") or "").upper() in GOLD_TICKERS
        and sig.get("strategy") == "trend_long"
        and sig.get("sector") == "Commodities"
        and sizing.get("trend_commodities_near_high_max_position_pct_applied")
...
def _resize_with_cap(
    ...
    out = dict(sizing)
    out["trend_gold_near_high_cap_baseline_shares"] = old_shares
    out["trend_gold_near_high_cap_raw_shares"] = raw_shares
    out["trend_gold_near_high_cap_old_cap_shares"] = old_cap_shares
    out["trend_gold_near_high_cap_new_cap_shares"] = new_cap_shares
    out["trend_gold_near_high_cap_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "raw_shares": adjusted_sizing.get(
                                "trend_gold_near_high_cap_raw_shares"
                            ),
                            "old_cap_shares": adjusted_sizing.get(
                                "trend_gold_near_high_cap_old_cap_shares"
                            ),
                            "new_cap_shares": adjusted_sizing.get(
                                "trend_gold_near_high_cap_new_cap_shares"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Gold Trend Near-High Cap",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if selected["passed"]
        else "rejected_gold_trend_near_high_cap"
    )
    interpretation = (
        "Gold trend-near-high signals were cap-bound and the selected cap improved the canonical three-window stack without EV regression."
        if selected["passed"]
        else "Gold trend-near-high cap expansion did not clear the canonical three-window gate."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted Commodity trend near-high sleeve mixes gold ETFs and SLV, "
            "but GLD/IAU already have a separate accepted target policy and show "
            "stable cap-bound trend follow-through across the canonical windows. "
            "A gold-only cap may unlock allocation without retuning raw Commodity "
            "risk or changing SLV."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "max_position_pct_for_gold_trend_near_high_sleeve",
        "single_causal_variable": (
            "max_position_pct for trend_long GLD/IAU signals already in the Commodity near-high sleeve"
        ),
        "parameters": {
            "cap_sweep": CAP_SWEEP,
            "baseline_trend_commodities_near_high_max_position_pct": 0.50,
            "selected_max_position_pct": selected["max_position_pct"],
            "target_sleeve": {
                "strategy": "trend_long",
                "sector": "Commodities",
                "tickers": sorted(GOLD_TICKERS),
                "requires_existing_commodity_near_high_cap": True,
            },
            "locked_variables": [
                "core universe",
                "candidate pool",
        ...
        "historical_experiment_check": {
            "similar_prior_results": {
                "accepted_gold_target": (
                    "GLD/IAU trend targets already use an accepted 8 ATR path, so gold is a production-visible sleeve rather than a single-ticker one-off."
                ),
                "exp-20260514-018": (
                    "Accepted a 50% cap for all trend_long Commodities near 52-week highs; this run tests only the narrower GLD/IAU cap room and leaves SLV unchanged."
                ),
                "exp-20260514-049": (
                    "Accepted Commodity breakout cap; this run does not touch breakout_long or raw Commodity multipliers."
                ),
            },
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital allocation: cap-bound GLD/IAU trend-near-high signals may deserve more position room than SLV inside the accepted Commodity sleeve."
            ),
            "2_history_check": (
                "Commodity trend cap and gold target width were accepted; no prior gold-only max-position cap exists. Commodity breakout cap was separate."
            ),
            "3_single_causal_variable": "gold trend-near-high max_position_pct only",
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_050_gold_trend_cap.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                ...
                "portfolio_engine trend_commodities_near_high_max_position_pct_applied",
            ],
            "passed": gate2["passed"],
        },
        "gate3": {
        ...
        "next_evidence_needed": (
            ...
            if selected["passed"]
            else "Do not retry nearby gold cap values without forward cap-room evidence or a new gold-specific production-visible discriminator."
        ),
        "related_files": [

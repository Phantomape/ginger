"""exp-20260517-019: rotation state-surface ret60 floor.

Alpha search. Tests one production-visible default-off paper-sleeve variable:
the minimum 60-day absolute return required before current accepted
rotation-only state-surface paper candidates enter the paper ledger. The
accepted ret20_excess_spy >= 0.0 gate stays locked. Core A/B signals, ranking,
...
EXPERIMENT_ID = "exp-20260517-019"
EXPERIMENT_SLUG = "state_surface_rotation_ret60_floor"
...
def _floor_label(floor: float | None) -> str:
    return "identity_no_ret60_floor" if floor is None else f"{floor:.2%}"


def _ret60_gate(row: dict[str, Any], *, floor: float | None) -> dict[str, Any]:
    ...
    if enabled:
        if ret60 is None:
            reasons.append("ret60_unavailable")
        elif ret60 < float(floor):
            reasons.append("ret60_below_floor")
    allowed = (not enabled) or not reasons
    return {
        "rule_version": "state_surface_ret60_floor_replay_v1",
        "enabled": enabled,
    ...
    *,
    ...
    for row in candidates:
        if row.get("status") != "price_ready":
            kept.append(row)
            continue
        gate = _ret60_gate(row, floor=floor)
        enriched = {**row, "ret60_floor_gate": gate}
        if gate["allowed"]:
            kept.append(enriched)
        else:
            blocked.append({**enriched, "reason": "ret60_floor_gate_blocked"})
    return kept, blocked
...
def _selected_trade_rows(trades: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for trade in trades:
        ...
        gate = trade.get("ret60_floor_gate") or {}
        rows.append(
            {
                ...
                "ret60_floor_gate_allowed": gate.get("allowed"),
                "pnl": trade.get("pnl"),
...
def _variant_payload(
    *,
    ...
    for label, window in WINDOWS.items():
        ...
        filtered, ret60_blocked = _filter_by_floor(spy_filtered, floor=floor)
        gate_blocked = [*spy_blocked, *ret60_blocked]
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rotation Ret60 Floor",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    if best_summary["gate4"]["passed"]:
        decision = "passed_replay_requires_shared_ret60_gate"
        status = "pending_promotion"
        interpretation = (
            "A ret60 floor improved the accepted rotation-only state-surface paper sleeve. "
        ...
        status = "rejected"
        interpretation = (
            "The ret60 floor did not improve the accepted rotation-only state-surface paper "
            "sleeve across the three fixed windows after locking the accepted ret20_excess_spy gate."
        )
    ...
    return {
        ...
        "decision": decision,
        "lane": "alpha_search",
        "change_type": "state_surface_rotation_ret60_floor",
        ...
        "hypothesis": (
            ...
            "candidates that already beat SPY over 20 days should also retain positive "
            "60-day trend quality. A ret60 floor may improve replacement value without "
        ...
        "history_check": {
            ...
            "exp-20260517-017": "Rejected IWM-relative 20d excess floor.",
            "anti_repeat_boundary": (
                "This is not a ret20_excess_spy threshold retry and not a ticker expansion; "
                "it tests a separate 60-day trend persistence field."
            ),
        },
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "candidate-pool alpha: require positive 60-day trend persistence for "
                "rotation-only state-surface paper candidates already passing ret20_excess_spy >= 0"
            ),
            "2_history_check": (
                "Rotation-only and 20-day SPY/IWM relative gates were tested; no current-stack "
                "ret60 floor experiment was found in experiment_log.jsonl/docs experiments."
            ),
            "3_single_causal_variable": "ret60_min",
            ...
            "5_reproducibility": (
                ...
                "exp_20260517_019_state_surface_rotation_ret60_floor.py"
            ),
        },
        "parameters": {
            "single_causal_variable": "ret60_min",
        ...
        "production_impact": {
            ...
            "production_impact": (
                ...
                "no shared default-off production policy was changed; if a ret60 floor "
        ...
        "next_evidence_needed": (
            "Implement the ret60 floor in shared state_surface_sleeve.py and rerun parity tests before acceptance."
            if best_summary["gate4"]["passed"]
            else "Do not add a ret60 floor on frozen windows; the next state-surface search needs a different production-visible discriminator."
        ),
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rotation ret60 floor",

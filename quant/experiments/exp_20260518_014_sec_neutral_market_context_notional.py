"""exp-20260518-014: SEC neutral underreaction market-context notional.

Alpha search on one production-visible field: within the accepted
neutral-underreaction SEC financial-report paper cohort, test whether broad
market T+1 context identifies safer underreaction follow-through. Core entries,
core exits, SEC queue eligibility, capacity, hold days, and live orders remain
unchanged.
...
EXPERIMENT_ID = "exp-20260518-014"
STEM = "exp_20260518_014_sec_neutral_market_context_notional"
...
import exp_20260518_009_sec_neutral_underreaction_notional as neutral_exp  # noqa: E402
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_neutral_market_context_notional.md"
)
EXPERIMENT_LOG_JSONL = REPO_ROOT / "docs" / "experiment_log.jsonl"

ACCEPTED_NEUTRAL_UNDERREACTION_SCALAR = 2.0
ACCEPTED_NEUTRAL_UNDERREACTION_MAX_T1_EXCESS = 0.020
MARKET_CONTEXT_EXTRA_SCALAR = 1.5
SPY_T1_RETURN_MIN_VARIANTS = (-0.010, -0.0075, -0.005, -0.0025, 0.0)
...
def _market_context_confirmed(position: dict[str, Any], *, spy_t1_min: float) -> bool:
    if not _accepted_neutral_underreaction(position):
        return False
    spy_t1_return = _float(parent._source_candidate(position).get("spy_t1_return"))
    return spy_t1_return is not None and spy_t1_return >= spy_t1_min
...
def _run_variant(
    *,
    ...
    exp100: dict[str, Any],
    spy_t1_min: float | None,
) -> dict[str, Any]:
    if spy_t1_min is None:
        return neutral_exp._run_variant(
            core_results=core_results,
            exp100=exp100,
            max_t1_excess=ACCEPTED_NEUTRAL_UNDERREACTION_MAX_T1_EXCESS,
        )
    ...
    def patched_notional(
        ...
        *,
        ...
        base_notional, scalar, rule = parent._base_notional_for_position(position)
        if _accepted_neutral_underreaction(position):
            ...
            if _market_context_confirmed(position, spy_t1_min=spy_t1_min):
                scalar *= MARKET_CONTEXT_EXTRA_SCALAR
...
def _target_rows_for_floor(
    exp100: dict[str, Any],
    *,
    spy_t1_min: float,
) -> list[dict[str, Any]]:
    accepted_rows = neutral_exp._target_positions_for_variant(
        exp100,
        max_t1_excess=ACCEPTED_NEUTRAL_UNDERREACTION_MAX_T1_EXCESS,
    )
    rows: list[dict[str, Any]] = []
    for row in accepted_rows:
        spy_t1_return = _float(row.get("spy_t1_return"))
        if spy_t1_return is None or spy_t1_return < spy_t1_min:
            continue
        baseline_pnl = float(row.get("adjusted_pnl") or 0.0)
        adjusted_pnl = baseline_pnl * MARKET_CONTEXT_EXTRA_SCALAR
        enriched = dict(row)
        enriched.update(
            {
                "accepted_neutral_underreaction_pnl": _round(baseline_pnl, 2),
                "market_context_adjusted_pnl": _round(adjusted_pnl, 2),
                "market_context_incremental_pnl": _round(
                    adjusted_pnl - baseline_pnl,
                    2,
                ),
                "spy_t1_return_min": spy_t1_min,
                "market_context_extra_scalar": MARKET_CONTEXT_EXTRA_SCALAR,
            }
        )
...
def _selection_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        pnl = float(row.get("market_context_incremental_pnl") or 0.0)
...
def _gate(
    *,
    ...
    return {
        ...
        "rule": (
            "Pass if aggregate EV/PnL improve versus accepted exp-20260518-009, "
            "EV and PnL improve in all three fixed windows, no window regresses, "
            "max drawdown worsens by <=0.5pp, adjusted trades >= "
            f"{MIN_ADJUSTED_TRADES}, and adjusted trades are present in all "
            f"{MIN_WINDOWS_PRESENT} windows."
        ),
    }


def _variant_name(spy_t1_min: float | None) -> str:
    if spy_t1_min is None:
        return "accepted_neutral_underreaction"
    return f"spy_t1_return_gte_{spy_t1_min:+.4f}_extra_scalar_1_50"


def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} SEC Neutral Market-Context Notional",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    baseline = _run_variant(core_results=core_results, exp100=exp100, spy_t1_min=None)
    variants: OrderedDict[str, dict[str, Any]] = OrderedDict()
    summaries: list[dict[str, Any]] = []
    for spy_t1_min in SPY_T1_RETURN_MIN_VARIANTS:
        name = _variant_name(spy_t1_min)
        row = _run_variant(
            core_results=core_results,
            exp100=exp100,
            spy_t1_min=spy_t1_min,
        )
        selection = _selection_summary(
            _target_rows_for_floor(exp100, spy_t1_min=spy_t1_min)
        )
    ...
    if passing:
        best_summary = min(
            passing,
            key=lambda row: (
                abs(row["spy_t1_return_min"] - ACCEPTED_SPY_T1_RETURN_MIN),
    ...
    decision = (
        "accepted_candidate_sec_neutral_market_context_notional"
        if gate["passed"]
        else "rejected_sec_neutral_market_context_notional"
    )
    interpretation = (
        "Neutral-underreaction SEC financial-report paper rows improved when the "
        "T+1 broad market backdrop was not deeply adverse. The best passing "
        "profile keeps live/default orders disabled and only changes default-off "
        "paper notional."
        if gate["passed"]
        else "No neutral-underreaction SPY T+1 context bucket cleared the three-window gate."
    )
    production_impact = {
    ...
    return {
        ...
        "hypothesis": (
            ...
            "cohort, T+1 relative strength should be trusted more when SPY did not "
            "sell off hard on the same T+1 session. A bounded extra paper-notional "
            "scalar should improve replacement value without changing queue "
            "eligibility, capacity, or live orders."
        ),
        "change_summary": (
            "Sweep the SPY T+1 return floor for a fixed 1.5x extra paper-notional "
            "scalar on accepted neutral-underreaction SEC financial-report rows."
        ),
        "change_type": "alpha_search_market_context_notional_allocation",
        "component": "quant/sec_financial_report_event_sleeve.py",
        "changed_variable": "sec_neutral_underreaction_spy_t1_return_min",
        "single_causal_variable": "SPY T+1 return floor for extra neutral-underreaction paper notional",
        "parameters": {
            ...
            "market_context_extra_scalar": MARKET_CONTEXT_EXTRA_SCALAR,
            "best_spy_t1_return_min": best_summary["spy_t1_return_min"],
            "spy_t1_return_min_variants": list(SPY_T1_RETURN_MIN_VARIANTS),
            "definition": (
                "accepted neutral-underreaction candidate and spy_t1_return >= floor"
            ),
        ...
        "next_evidence_needed": (
            "Move the accepted paper-only rule into shared SEC financial-report "
            "sleeve code with parity coverage; keep live/default orders disabled "
            "until closed forward replacement-value evidence matures."
            if gate["passed"]
            else "Do not retry nearby SPY T+1 context floors without new forward "
            "replacement-value evidence."
        ),
        "production_impact": production_impact,
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "capital allocation: add a SPY T+1 market-context field to the "
                "accepted neutral-underreaction SEC paper notional path."
            ),
            "2_history_check": (
                "exp-20260518-009 accepted neutral-underreaction T+1 excess <=2%; "
                "exp-20260518-010 rejected capacity priority; exp-20260518-011/012 "
                "rejected negative/positive language notional. No prior accepted run "
                "isolated SPY T+1 market context on top of neutral-underreaction."
            ),
            "3_single_causal_variable": "SPY T+1 return floor for extra paper notional",
        ...
        "llm_metrics": {
            ...
            "blocker_relation": (
                "LLM soft-ranking remains sparse; this uses deterministic SEC "
                "language/T+1 reaction fields and a production-visible SPY T+1 context."
            ),
        },
        "why_not_other_changes": (
            "State-surface profile retunes are anti-repeat without a new field, "
            "negative/positive SEC language branches just failed, and LLM "
            "soft-ranking remains sample-limited. This tests one new market-context "
            "field on an accepted paper cohort."
        ),
        "related_files": [

"""exp-20260516-034: SEC financial-report guidance-raise notional.
...
contains at least one guidance-raise hit. The accepted queue, T+1 excess
...
EXPERIMENT_ID = "exp-20260516-034"
STEM = "exp_20260516_034_sec_financial_report_guidance_raise_notional"
...
from exp_20260516_033_sec_financial_report_neutral_language_notional import (  # noqa: E402
    ACCEPTED_10Q_PERIODIC_REPORT_SCALAR,
    DEFAULT_PERIODIC_REPORT_NOTIONAL_SCALAR,
    TEXT_ARCHIVE_JSONL,
    _annotate_language_fields,
    _append_jsonl_once,
    _candidate_counts,
    _gate2_open_position_field_check,
    _load_text_rows,
    _text_coverage_summary,
)
...
DOC_ARTIFACT = (
    ...
    / "experiments"
    / "artifacts"
    / f"{EXPERIMENT_ID}_sec_financial_report_guidance_raise_notional.md"
)
...
BASELINE_GUIDANCE_RAISE_SCALAR = 1.0
GUIDANCE_RAISE_SCALAR_VARIANTS = (1.0, 1.1, 1.25, 1.5, 2.0, 2.5)
MIN_PROMOTION_CLOSED_TRADES = 40
MIN_GUIDANCE_RAISE_CLOSED_TRADES = 5
...
def _guidance_raise_hits(position: dict[str, Any]) -> int:
    try:
        return int(_source_candidate(position).get("guidance_raise_hits") or 0)
    except (TypeError, ValueError):
        return 0


def _is_guidance_raise_position(position: dict[str, Any]) -> bool:
    return _coverage_status(position) == "covered" and _guidance_raise_hits(position) > 0
...
def _notional_for_position(
    ...
    *,
    guidance_raise_scalar: float,
) -> tuple[float, float, str]:
    notional, scalar, rule = _base_notional_for_position(position)
    if not _is_guidance_raise_position(position):
        return notional, scalar, rule
    combined_scalar = scalar * float(guidance_raise_scalar)
    return (
        float(DEFAULT_EVENT_NOTIONAL_USD) * combined_scalar,
        combined_scalar,
        f"{rule}+guidance_raise_scalar",
    )
...
def _pnl_for_position(
    ...
    *,
    ...
    adjusted_notional, _, _ = _notional_for_position(
        position,
        guidance_raise_scalar=guidance_raise_scalar,
    )
    if closed:
        try:
...
def _adjust_closed_position(
    ...
    *,
    ...
    adjusted["guidance_raise_hits"] = _guidance_raise_hits(position)
    adjusted["guidance_raise_bucket"] = _is_guidance_raise_position(position)
    adjusted["guidance_raise_notional_scalar"] = guidance_raise_scalar
    adjusted["pnl"] = round(
        _pnl_for_position(
            position,
            guidance_raise_scalar=guidance_raise_scalar,
...
def _closed_position_breakdown(closed_positions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    guidance_raise_count = 0
    guidance_raise_pnl = 0.0
    for item in closed_positions:
        ...
        if item.get("guidance_raise_bucket") is True:
            guidance_raise_count += 1
            guidance_raise_pnl += pnl
    return {
        ...
        "guidance_raise_closed_trade_count": guidance_raise_count,
        "guidance_raise_total_pnl": _round(guidance_raise_pnl, 2),
    }
...
def _guidance_raise_closed_trade_count(row: dict[str, Any]) -> int:
    return sum(
        int(window["sleeve_metrics"].get("guidance_raise_closed_trade_count", 0))
...
def _window_checks(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    for label in WINDOWS:
        ...
        checks[label] = {
            ...
            "guidance_raise_closed_trade_count": int(
                after_sleeve.get("guidance_raise_closed_trade_count") or 0
            ),
            "guidance_raise_pnl_delta": _round(
                float(after_sleeve.get("guidance_raise_total_pnl") or 0.0)
                - float(before_sleeve.get("guidance_raise_total_pnl") or 0.0),
                2,
            ),
        }
    return checks
...
def _gate(after: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    guidance_raise_trades_after = _guidance_raise_closed_trade_count(after)
    passed = (
        ...
        and guidance_raise_trades_after >= MIN_GUIDANCE_RAISE_CLOSED_TRADES
    )
    return {
        "aggregate_delta": aggregate_delta,
        "guidance_raise_closed_trade_count_after": guidance_raise_trades_after,
        ...
        "rule": (
            ...
            "guidance-raise closed trades >= 5."
        ),
...
def _best_candidate(variants: OrderedDict[str, dict[str, Any]]) -> str:
    baseline = variants[f"guidance_raise_scalar_{BASELINE_GUIDANCE_RAISE_SCALAR:.2f}"]
    candidates = [
        (name, row, _gate(row, baseline))
        for name, row in variants.items()
        if row["guidance_raise_notional_scalar"] != BASELINE_GUIDANCE_RAISE_SCALAR
    ]
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {payload['experiment_id']} SEC guidance-raise notional",
        "",
    ...
    for label, row in gate["window_checks"].items():
        lines.append(
            "| {label} | {ev} | {pnl} | {dd} | {count} | {guidance_pnl} |".format(
                label=label,
                ev=row["ev_delta"],
                pnl=row["pnl_delta"],
                dd=row["max_drawdown_delta"],
                count=row["guidance_raise_closed_trade_count"],
                guidance_pnl=row["guidance_raise_pnl_delta"],
            )
        )
    lines.extend(
        [
...
def main() -> int:
    ...
    for scalar in GUIDANCE_RAISE_SCALAR_VARIANTS:
        name = f"guidance_raise_scalar_{scalar:.2f}"
        row = _run_variant(
            core_results=core_results,
            exp100=exp100,
            guidance_raise_scalar=scalar,
        )
        row["guidance_raise_notional_scalar"] = scalar
        variants[name] = row

    baseline_key = f"guidance_raise_scalar_{BASELINE_GUIDANCE_RAISE_SCALAR:.2f}"
    ...
    decision = (
        "observed_only_guidance_raise_notional_positive_field_blocked"
        if metric_gate_passed
        else "rejected_guidance_raise_notional_scalar"
    )
    ...
    payload = {
        ...
        "hypothesis": (
            ...
            "filings with explicit guidance-raise language may be higher-quality "
            "post-earnings drift candidates after positive T+1 confirmation than "
            "the rest of the accepted sleeve, so they may deserve a separate "
            "paper-notional multiplier."
        ),
        "change_summary": (
            "Replay-only paper-notional multiplier for covered SEC financial-report "
            "T+1 sleeve rows with guidance_raise_hits > 0."
        ),
        ...
        "parameters": {
            "baseline_guidance_raise_notional_scalar": BASELINE_GUIDANCE_RAISE_SCALAR,
            "guidance_raise_definition": (
                "sec_text_coverage_status == covered and guidance_raise_hits > 0"
            ),
            "guidance_raise_scalar_variants": list(GUIDANCE_RAISE_SCALAR_VARIANTS),
        ...
        "rejection_reason": (
            None
            if metric_gate_passed
            else "No guidance-raise scalar cleared the three-window semantic allocation gate."
        ),
        "next_evidence_needed": (
            "Metric gate passed on replay, but do not promote until SEC financial-report "
            "production candidates carry guidance_raise_hits/text coverage fields and parity "
            "tests prove the same field is visible in run.py and backtester paths."
            if metric_gate_passed
            else "Do not retry guidance-raise notional scalars on this frozen sample without "
            "more covered guidance-raise rows, fuller text coverage, or forward replacement-value evidence."
        ),
        "production_impact": {
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "risk allocation: use a new replayable SEC filing-text guidance "
                "signal to scale paper notional only for guidance-raise financial-report "
                "T+1 candidates."
            ),
            "2_history_check": (
                ...
                "10-Q-first priority, paired-filing dedupe, buybacks, and neutral-language "
                "notional. No logged experiment isolated guidance_raise_hits > 0 on the "
                "accepted 10-Q stack."
            ),
            "3_single_causal_variable": "guidance-raise paper-notional scalar only",

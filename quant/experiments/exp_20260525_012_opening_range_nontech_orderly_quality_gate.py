...
This alpha search follows the rejected opening-range top-1 paper sleeve from
exp-20260525-011. The single tested variable is a production-visible quality
gate on that already-selected daily top-1 paper candidate:

- sector is not Technology;
- candidate-day return is positive but not above 5%;
- next-open gap versus prior close is not above 3%.

The gate is tested as default-off paper only. Core entries, ranking, sizing,
exits, heat, LLM/news replay, watchlists, and live/default orders are
...
EXPERIMENT_ID = "exp-20260525-012"
STEM = "opening_range_nontech_orderly_quality_gate"
...
MAX_CANDIDATE_DAY_RETURN = 0.05
MAX_OPEN_VS_PRIOR_CLOSE = 0.03
...
def _quality_gate(row: dict[str, Any]) -> tuple[bool, str]:
    sector = str(row.get("sector") or "")
    candidate_day_return = float(row.get("candidate_day_return") or 0.0)
    open_vs_prior_close = float(row.get("open_vs_prior_close") or 0.0)
    if sector == EXCLUDED_SECTOR:
        return False, "technology_blowoff_risk"
    if candidate_day_return <= 0:
        return False, "non_positive_candidate_day"
    if candidate_day_return > MAX_CANDIDATE_DAY_RETURN:
        return False, "candidate_day_blowoff"
    if open_vs_prior_close > MAX_OPEN_VS_PRIOR_CLOSE:
        return False, "next_open_gap_too_hot"
    return True, "passed_nontech_orderly_quality_gate"


def _select_quality_gated_top1(
    snapshot: dict[str, list[dict[str, Any]]],
    candidates: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    original_selected, original_filtered = prior._select_paper_trades(snapshot, candidates)
    selected: list[dict[str, Any]] = []
    rejected: list[dict[str, Any]] = []
    for trade in original_selected:
        passed, reason = _quality_gate(trade)
        enriched = {**trade, "quality_gate_reason": reason}
        if passed:
...
def _build_payload() -> dict[str, Any]:
    ...
    rejected_top1_by_window: "OrderedDict[str, list[dict[str, Any]]]" = OrderedDict()
    raw_candidate_counts: "OrderedDict[str, int]" = OrderedDict()
    original_top1_counts: "OrderedDict[str, int]" = OrderedDict()
    ...
    for label, cfg in WINDOWS.items():
        ...
        selected_trades, rejected_top1, original_filtered = _select_quality_gated_top1(
            snapshot,
            candidates,
        )
        ...
        rejected_top1_by_window[label] = rejected_top1[:200]
        original_filtered_sample_by_window[label] = original_filtered[:100]
        raw_candidate_counts[label] = len(candidates)
        original_top1_counts[label] = len(selected_trades) + len(rejected_top1)
        window_rows[label] = {
            "before": before,
            "after": after,
            "delta": delta,
            "target_trade_count": len(selected_trades),
            "rejected_top1_count": len(rejected_top1),
            "raw_candidate_count": len(candidates),
            "original_top1_count": original_top1_counts[label],
    ...
    decision = (
        "promising_replay_only_opening_range_nontech_orderly_quality_gate"
        if gate4_passed
        else "rejected_opening_range_nontech_orderly_quality_gate"
    )
    ...
    return {
        ...
        "decision": decision,
        "hypothesis": (
            "The rejected opening-range paper source may be too broad because "
            "Technology blow-off paths drive late-window drawdown. Keeping only "
            "already-selected daily top-1 candidates whose sector is non-Tech and "
            "whose signal-day/gap path is orderly may retain broad-market "
            "continuation alpha while reducing tail exposure."
        ),
        "change_type": "opening_range_continuation_quality_gated_paper_sleeve",
        ...
        "new_evidence_type": (
            "production_visible_return_path_cluster_quality_gate_on_existing_"
            "opening_range_top1_source"
        ),
        "backtest_protocol": {
            ...
            "execution_model": (
                "Same as exp-20260525-011: signal uses same-day/prior-day OHLCV; "
                "paper entry is next available open with production entry slippage; "
                "exit is the close ten trading days after the signal with sell "
        ...
        "parameters": {
            ...
            "quality_gate": {
                ...
                "candidate_day_return_lte": MAX_CANDIDATE_DAY_RETURN,
                "open_vs_prior_close_lte": MAX_OPEN_VS_PRIOR_CLOSE,
            },
            "locked_variables": [
                "core universe",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "entry / candidate_pool: a free-OHLCV return-path quality gate "
                "may improve the rejected opening-range paper source by avoiding "
                "Technology blow-off continuations."
            ),
            "2_history_check": {
                "exp-20260525-011": (
                    "Opening-range top-1 fixed-notional paper sleeve improved "
                    "aggregate EV/PnL but failed Gate 4 due late_strong regression "
                    "and max drawdown drift. This keeps the source/ranking fixed "
                    "and changes only a top-1 quality gate."
                ),
                "exp-20260426-041": (
                    "Older opening-range observed-only audit lacked current "
                    "three-window Gate 1-4 evidence."
                ),
            },
            ...
            "5_reproducibility": (
                ...
                "exp_20260525_012_opening_range_nontech_orderly_quality_gate.py"
            ),
        },
        "gate1": {
        ...
        "original_top1_counts": original_top1_counts,
        "target_trades_by_window": target_trades_by_window,
        "rejected_top1_by_window": rejected_top1_by_window,
        ...
        "why_not_other_changes": (
            ...
            "anti-repeat gates; skipped broad-market feed subsets after recent "
            "strict-gate failures. This tests a free OHLCV return-path quality "
            "field on a broad source that failed for identifiable tail reasons."
        ),
        "interpretation": (
            "The non-Tech orderly opening-range quality gate cleared replay-only "
            "Gate 4. This is a research lead only; no production/shared policy was "
            "promoted."
            if gate4_passed
            else (
                "The non-Tech orderly opening-range quality gate did not clear Gate "
                "4. Do not retry nearby opening-range OHLCV gates on the frozen "
                "sample without forward paper rows or an orthogonal source/event "
                "confirmation field."
            )
        ),
        ...
        "next_evidence_needed": (
            ...
            "activation. Do not convert this into core entries directly."
            if gate4_passed
            else (
                "Forward opening-range paper rows or a materially different "
                "source/event confirmation field."
            )
        ),
        "related_files": [
    ...
    for label in WINDOWS:
        ...
        rows.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {dd:+.4f} | {trades} | {rejected} |".format(
                ...
                rejected=len(payload["rejected_top1_by_window"][label]),
            )
        )
    aggregate = payload["delta_metrics"]["aggregate"]
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Opening-Range Non-Tech Orderly Quality Gate",
            "",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Opening-range non-Tech orderly quality gate",

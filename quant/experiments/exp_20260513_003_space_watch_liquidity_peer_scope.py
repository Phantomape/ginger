...
apply only to specific Space peer-momentum states instead of every watch-tier
signal. This is a scope refinement for an existing production-visible
risk-allocation helper, not a scalar retune, candidate-pool expansion, LLM
ranking change, or live-slot change.
"""
...
STEM = "space_watch_liquidity_peer_scope"
WATCH_LIQUIDITY_RISK_SCALAR = 1.10
PEER_SCOPE_VARIANTS = {
    "watch_liquidity_peer_leader_only": ("leader",),
    "watch_liquidity_peer_nonleader_only": ("nonleader",),
}
...
def _install_space_policy(
    ...
    allowed = {str(item).lower() for item in peer_scope}
    ...
    def size_wrapper(signals, portfolio_value, risk_pct=None):
        ...
        for signal in sized:
            ...
            peer_state = str(signal.get("space_peer_momentum_state") or "missing").lower()
            ...
            if ticker in target_tickers and sizing:
                ...
                if peer_state in allowed:
                    _scale_sizing(
                        sizing,
                        WATCH_LIQUIDITY_RISK_SCALAR,
                        portfolio_value,
                        "space_watch_liquidity_peer_scope",
                    )
                    ...
                    watch_adjustments.append(
                        {
                            ...
                            "marker": "space_watch_liquidity_peer_scope",
                    ...
                    signal = {
                        **signal,
                        ...
                        "space_watch_liquidity_peer_scope": list(peer_scope),
                        "space_watch_liquidity_peer_scope_eligible": True,
                    }
                else:
                    ...
                    watch_out_of_scope.append(
                        {
                            "ticker": ticker,
                            "strategy": signal.get("strategy"),
                            "marker": "space_watch_liquidity_peer_scope_skipped",
                            "space_liquidity_tier": tiers.get(ticker),
                            "space_peer_momentum_state": peer_state,
                            "peer_scope": list(peer_scope),
    ...
    try:
        ...
        for label, window in WINDOWS.items():
            ...
            by_window[label] = {
                ...
                "space_watch_liquidity_peer_scope_adjustment": (
                    _adjustment_summary(watch_adjustments[before_watch:])
                ),
                "space_watch_liquidity_peer_scope_skipped": (
                ...
                "space_watch_liquidity_peer_scope_signal_counts": dict(
...
def _gate(variant: dict[str, Any], before: dict[str, Any], core: dict[str, Any]) -> dict[str, Any]:
    ...
    adjusted_count = sum(
        row["space_watch_liquidity_peer_scope_adjustment"]["adjusted_signal_count"]
        for row in variant["by_window"].values()
    )
    skipped_count = sum(
        row["space_watch_liquidity_peer_scope_skipped"]["adjusted_signal_count"]
        for row in variant["by_window"].values()
    )
    passed = (
    ...
    return {
        ...
        "space_watch_liquidity_peer_scope_adjusted_signal_count": adjusted_count,
        "space_watch_liquidity_peer_scope_skipped_signal_count": skipped_count,
    }
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Space watch-liquidity peer scope",
        "",
        f"- Decision: `{payload['decision']}`",
        (
            "- Single variable: peer-momentum scope for the accepted watch-liquidity "
            "1.10x top-up."
        ),
        ...
        "",
        (
            "| Variant | Peer scope | Gate | dEV | dPnL | Improved windows | "
    ...
    for name, variant in payload["variants"].items():
        ...
        scope = ",".join(variant["space_watch_liquidity_peer_scope"])
        lines.append(
            ...
            f"{gate['space_watch_liquidity_peer_scope_adjusted_signal_count']} | "
            f"{gate['space_watch_liquidity_peer_scope_skipped_signal_count']} |"
        )
    lines.extend(
        [
            "",
...
def run() -> dict[str, Any]:
    ...
    before = _run_accepted_exp112_variant(
        "accepted_exp112_watch_liquidity_all_peer_states",
        WATCH_LIQUIDITY_RISK_SCALAR,
        liquidity_gate,
        company_release_gate,
        financing_gate,
    ...
    variants = {}
    for name, scope in PEER_SCOPE_VARIANTS.items():
    ...
    decision = (
        "accepted_default_off_space_watch_liquidity_peer_scope"
        if accepted
        else "rejected_space_watch_liquidity_peer_scope"
    )
    interpretation = (
        "Restricting the watch-liquidity top-up by peer momentum state improved the accepted "
        ...
        if accepted
            ...
            "Restricting the accepted watch-liquidity top-up by peer momentum state did not "
            "clear the three-window gate versus exp-20260512-112. Keep the current "
            "all-peer-state watch-liquidity helper and do not retry nearby peer-scope "
    ...
    payload = {
        ...
        "changed_variable": "space_watch_liquidity_peer_scope",
        "single_causal_variable": (
            "peer momentum state scope for the accepted 1.10x watch-liquidity risk top-up"
        ),
        "hypothesis": (
            "The accepted Space watch-liquidity scalar may be carried mainly by "
            "signals that lead the official Space peer basket. Since peer-nonleader "
            "breakouts already needed zero extra risk while broad peer-leader top-ups "
            "were underpowered, limiting the existing watch-liquidity 1.10x top-up "
            "by peer momentum state may improve replacement value without changing "
            "the candidate pool, scalar value, ranking, exits, LLM/news, or live slots."
        ),
        "gate_questions": {
            "1_alpha_hypothesis": (
                "risk allocation: keep the accepted watch-liquidity scalar at 1.10x "
                "but test its peer-momentum state scope."
            ),
            "2_history_check": {
                "exp-20260512-112": (
                ...
                "exp-20260513-002": (
                    "Rejected hard customer-win pool pruning; this keeps the official "
                    "pool intact and only tests one accepted helper's scope."
                ),
                "exp-20260512-013": (
                ...
                "exp-20260512-009": (
                    "Rejected broad peer-leader risk top-up; this tests peer state "
                    "only inside the accepted watch-liquidity helper."
                ),
                "exp-20260512-951": (
                    "Rejected strategy-scope split for watch liquidity; this is a peer-state "
                    "test for one accepted helper, not a broad cap."
                ),
            },
            "3_single_causal_variable": (
                "space_watch_liquidity_peer_scope. Candidate pool, scalar value, "
            ...
            "5_reproducibility": (
                ...
                "quant\\experiments\\exp_20260513_003_space_watch_liquidity_peer_scope.py"
            ),
        },
        "parameters": {
            ...
            "tested_peer_scopes": {
                name: list(scope) for name, scope in PEER_SCOPE_VARIANTS.items()
            },
        ...
        "gate1": {
            ...
            "known_bias": (
                ...
                "from a 2026-05-10 research universe. Peer-state and liquidity-tier "
                "metadata are production-observable in the shared Space sleeve, but "
                "any accepted change must remain default-off until forward evidence "
                "matures."
            ),
        },
        "gate2": {
        ...
        "gate3": {
            ...
            "peer_scope_refinement_added": True,
        ...
        "next_evidence_needed": (
            "If rejected, do not retry nearby peer-scope splits for the accepted "
        ...
        "related_files": [
            "quant/experiments/exp_20260513_003_space_watch_liquidity_peer_scope.py",
            "data/experiments/exp-20260513-003/space_watch_liquidity_peer_scope.json",
            "docs/experiments/logs/exp-20260513-003.json",
            "docs/experiments/tickets/exp-20260513-003.json",
            "docs/experiments/artifacts/exp-20260513-003_space_watch_liquidity_peer_scope.md",
        ...
        "why_not_other_changes": (
            ...
            "This tests one production-visible peer-scope variable for an accepted helper."
        ),
    }
    return payload

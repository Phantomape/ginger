...
This alpha search converts the old observed-only sector-leadership trend
shadow source into the current three-window fixed-notional paper replay.

The single variable is an additive default-off paper sleeve that admits at most
one candidate per signal day from the sector-leadership relative-strength
shadow source, enters at the next available open, and exits after ten trading
days at the close.
...
import exp_20260426_sector_leadership_trend_shadow as shadow  # noqa: E402
...
EXPERIMENT_ID = "exp-20260525-916"
STEM = "sector_leadership_top1_fixed_notional_sleeve"
TRIAL_FAMILY = "sector_leadership_trend_continuation_default_off_paper_sleeve"
CHANGED_VARIABLE = "sector_leadership_top1_next_open_10d_fixed_notional_sleeve_v1"
...
def _candidate_rows_for_window(
    ...
    sector_state = shadow._sector_daily_returns(snapshot, universe, dates)
    candidates: list[dict[str, Any]] = []
    for ticker in sorted(set(universe).intersection(snapshot)):
        if ticker in shadow.EXCLUDED_TICKERS:
            continue
        candidates.extend(shadow._candidate_rows(snapshot, ticker, dates, sector_state))
    candidates.sort(
        key=lambda row: (
            row["date"],
            row["sector_rank"],
...
def _target_trade_summary(
    ...
    by_sector_pnl: Counter[str] = Counter()
    by_window_pnl = {}
    ...
    return {
        ...
        "by_sector_pnl": {
            sector: round(pnl, 2) for sector, pnl in sorted(by_sector_pnl.items())
        },
...
def _build_payload() -> dict[str, Any]:
    ...
    decision = (
        "promising_replay_only_sector_leadership_top1_fixed_notional_sleeve"
        if gate4_passed
        else "rejected_sector_leadership_top1_fixed_notional_sleeve"
    )
    ...
    return {
        ...
        "decision": decision,
        "hypothesis": (
            "A production-visible sector-leadership relative-strength trend "
            "source may add candidate-pool alpha. It should prefer stocks in "
            "the top three 20-day sector-median leaders that also beat both "
            "their sector median and SPY, without adding arbitrary noisy "
            "tickers."
        ),
        "change_type": "sector_leadership_default_off_paper_sleeve",
        ...
        "parameters": {
            ...
            "shadow_entry_filters": {
                "lookback_days": shadow.LOOKBACK_DAYS,
                "top_sector_count": shadow.TOP_SECTOR_COUNT,
                "min_sector_median_return": shadow.MIN_SECTOR_MEDIAN_RETURN,
                "min_stock_return": shadow.MIN_STOCK_RETURN,
                "min_stock_vs_sector": shadow.MIN_STOCK_VS_SECTOR,
                "min_stock_vs_spy": shadow.MIN_STOCK_VS_SPY,
                "close_above_50sma": True,
                "close_above_200sma": True,
                "min_candidate_day_dollar_volume": shadow.MIN_DOLLAR_VOLUME,
            },
            "selection_rank": [
                "signal_date",
                "sector_rank asc",
                "stock_vs_sector_20d desc",
                "stock_vs_spy_20d desc",
                "dollar_volume desc",
                "ticker asc",
            ],
            "locked_variables": [
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "entry / candidate_pool: sector-leadership relative-strength "
                "trend-continuation may surface a production-visible free-OHLCV "
                "candidate source."
            ),
            "2_history_check": {
                "exp-20260426-049": (
                    "Observed-only sector-leadership shadow audit; no current "
                    "three-window next-open fixed-notional before/after EV replay."
                ),
                "exp-20260525-011/901": (
                    "Opening-range paper sleeve variants were tested recently; "
                    "this uses cross-sectional sector leadership instead of "
                    "same-stock opening-range/macro confirmation."
                ),
                "recent_candidate_pools": (
                    "Space, AI optical/compute, consumer platform, and broad-market "
                    "feed pools are recent anti-repeat families; this stays within "
                    "the existing core universe and uses sector state, not theme membership."
                ),
            },
            ...
            "5_reproducibility": (
                ...
                "exp_20260525_916_sector_leadership_top1_fixed_notional_sleeve.py"
            ),
        },
        "gate1": {
        ...
        "gate2": {
            ...
            "note": (
                ...
                "next-open/exit prices available to replay. It does not ask LLM "
                "or production to infer hidden fields."
            ),
        },
        "gate3": {
        ...
        "why_not_other_changes": (
            ...
            "coverage remains sparse; skipped AI/compute/consumer/Space fixed "
            "membership retreads and broad-market feed gates due fresh anti-repeat "
            "evidence. This uses a different free-OHLCV cross-sectional sector "
            "leadership source."
        ),
        "interpretation": (
            "The sector-leadership top-1 paper sleeve cleared Gate 4 as a replay-only "
            "lead, but no production/shared policy was promoted."
            if gate4_passed
            else (
                "The sector-leadership top-1 paper sleeve did not clear Gate 4. Do "
                "not promote or retry nearby sector-leadership filters without a "
                "new production-visible discriminator or broader forward sample."
            )
        ),
        "rejection_reason": None if gate4_passed else "; ".join(failed),
        "next_evidence_needed": (
            "If revisited, use a materially new discriminator such as event/source "
            "confirmation, sector-breadth persistence, or forward paper rows; do "
            "not just retune the same sector-rank/return thresholds on this frozen sample."
        ),
        "related_files": [
...
def _build_report(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Sector-Leadership Top-1 Fixed-Notional Sleeve",
            "",
...
def _experiment_log_entry(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "change_summary": (
            "Replay-only default-off sector-leadership top-1 fixed-notional "
            "paper sleeve using next-open entry and 10-trading-day exit."
        ),
        "change_type": payload["change_type"],
        "mechanism_family": "sector_leadership_free_ohlcv_candidate_pool",
        "trial_family": payload["trial_family"],
        "trial_variant_id": "top1_next_open_10d_fixed_notional",
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Sector-leadership top-1 fixed-notional sleeve",
    ...
    _append_jsonl_once(EXPERIMENT_LOG, _experiment_log_entry(payload))

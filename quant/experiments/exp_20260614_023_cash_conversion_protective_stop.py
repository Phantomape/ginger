"""exp-20260614-023: cash-conversion quality protective-stop envelope.

Alpha search. The single decision hypothesis is a drawdown-aware exit envelope
for the positively predictive exp-20260614-020 annual SEC accruals /
cash-conversion quality source: keep the exact quality, price-confirmation,
top-1/day, and same-ticker cooldown definition fixed, but add one fixed
daily-close 7% protective stop. If a paper position closes at or below 93% of
its entry fill after entry, it exits at the next available open with stop-side
sell slippage. Otherwise it keeps the same 10-trading-day close exit.
...
No production code, live/default orders, core ranking, core sizing, core exits,
LLM/news path, or watchlist behavior is changed during the initial replay. A
positive result must be reproduced through a shared historical/daily helper
before acceptance. No JavaScript is used.
"""
...
from fill_model import SLIPPAGE_BPS_STOP, apply_slippage  # noqa: E402
from fundamental_growth_rs_paper_sleeve import load_companyfacts_rows  # noqa: E402


EXPERIMENT_ID = "exp-20260614-023"
STEM = "cash_conversion_protective_stop"
TRIAL_FAMILY = "cash_conversion_quality_protective_stop_candidate_pool"
TRIAL_VARIANT_ID = "annual_cash_conversion_quality_daily_close_7pct_stop_next_open_v1"
CHANGED_VARIABLE = "cash_conversion_quality_daily_close_7pct_protective_stop_v1"
RULE_VERSION = CHANGED_VARIABLE
OWNER = "alpha-search-automation"

OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260614_023_{STEM}.json"
...
PROTECTIVE_STOP_LOSS_PCT = 0.07
PROTECTIVE_STOP_TRIGGER = 1.0 - PROTECTIVE_STOP_LOSS_PCT
...
PREDICTION = {
    ...
    "main_failure_modes": [
        "old_thin_stop_whipsaw",
        "stop_exit_cuts_winners",
        "accepted_comparator_not_beaten",
        "sample_or_drawdown_guard_fail",
    ],
    "confidence_reason": (
        "Prior annual cash-conversion source was positive in all three windows "
        "but failed on drawdown; max-active-one deployment still regressed "
        "old_thin, so a fixed stop tests a distinct tail-risk envelope. Main "
        "risk is that losses are gap/path driven and stop exits cut rebounds."
    ),
    "recorded_at": "2026-06-14T17:55:50+00:00",
}
...
PRODUCTION_IMPACT = {
    ...
    "alters_sizing": False,
    "alters_exits": False,
    "paper_exit_envelope_changed": True,
    "uses_llm": False,
    ...
    "execution_envelope": {
        ...
        "protective_stop_trigger": "daily close <= entry_fill * 0.93",
        "liquidity_source": "price >= $10 and ADV20 >= $50M from PIT OHLCV",
        "order_semantics": (
            "observe-only next-session-open paper entry; stop trigger is known "
            "only after the daily close and exits at the next available open; "
            "no broker order"
        ),
        ...
        "failure_handling": (
            ...
            "missing next-open stop exit or missing 10d exit rejects the paper "
            "candidate"
        ),
    },
    "parity_note": (
        ...
        "gate, liquid SPY-relative confirmation, daily-close stop trigger, "
        "next-open stop exit, scheduled 10-day exit, costs, cooldown, and "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "exit / risk envelope: exp-20260614-020 showed annual cash-conversion "
        "quality is directionally strong but overdeployed into drawdown; a "
        "fixed daily-close 7% protective stop may keep the edge while cutting "
        "tail losses."
    ),
    "2_history_check": {
        ...
        "exp-20260614-020": (
            ...
            "daily deployment. This run keeps the field and candidate selection "
            "fixed and changes only the paper exit envelope."
        ),
        "exp-20260614-021": (
            "The low-deployment max-active-one retry reduced exposure but still "
            "regressed old_thin and failed drawdown. This run is not another "
            "deployment cap; it tests fixed daily-close tail control."
        ),
    },
    ...
    "4_acceptance_standard": (
        ...
        "drawdown drift <=0.5pp, concentration pass, and the stopped source "
        "must beat accepted compression and distribution comparators; any "
        "retained positive must be moved to shared daily/backtest parity first."
    ),
    "5_reproducibility": (
        ".venv\\Scripts\\python.exe -B quant\\experiments\\"
        "exp_20260614_023_cash_conversion_protective_stop.py"
    ),
}
...
def _paper_trade_with_protective_stop(
    snapshot: dict[str, list[dict[str, Any]]],
    candidate: dict[str, Any],
) -> dict[str, Any] | None:
    trade = framework.sleeve._paper_trade_from_candidate(snapshot, candidate)
    if trade is None:
        return None
    ...
    rows = framework.shadow._series(snapshot, ticker)
    idx = framework.shadow._row_index(rows).get(signal_date)
    if idx is None:
        return None
    entry_idx = idx + 1
    scheduled_exit_idx = idx + HOLD_DAYS
    if entry_idx >= len(rows) or scheduled_exit_idx >= len(rows):
        return None

    entry_price = _float_or_none(trade.get("entry_price"))
    if not entry_price or entry_price <= 0:
        return None
    stop_level = entry_price * PROTECTIVE_STOP_TRIGGER
    trade["protective_stop_loss_pct"] = PROTECTIVE_STOP_LOSS_PCT
    trade["protective_stop_level"] = _round(stop_level, 4)
    trade["stop_triggered"] = False
    trade["exit_reason"] = "scheduled_10d_close"
    trade["scheduled_exit_date"] = trade.get("exit_date")
    trade["scheduled_exit_price"] = trade.get("exit_price")
    trade["scheduled_exit_raw_close"] = trade.get("exit_raw_close")

    for close_idx in range(entry_idx, scheduled_exit_idx):
        close_raw = framework.shadow._value(rows[close_idx], "Close")
        if close_raw is None or close_raw > stop_level:
            continue
        exit_idx = close_idx + 1
        if exit_idx >= len(rows):
            return None
        exit_raw_open = framework.shadow._value(rows[exit_idx], "Open")
        if not exit_raw_open:
            return None
        exit_price = apply_slippage(exit_raw_open, SLIPPAGE_BPS_STOP, "sell")
        pnl_pct_net = (exit_price / entry_price) - 1.0 - ROUND_TRIP_COST_PCT
        pnl = BASE_NOTIONAL_USD * pnl_pct_net
        return {
            **trade,
            "exit_date": framework.shadow._date(rows[exit_idx]),
            "exit_raw_open": _round(exit_raw_open, 4),
            "exit_price": _round(exit_price, 4),
            "exit_reason": "daily_close_protective_stop_next_open",
            "stop_triggered": True,
            "stop_trigger_date": framework.shadow._date(rows[close_idx]),
            "stop_trigger_close": _round(close_raw, 4),
            "hold_days": exit_idx - idx,
            "scheduled_hold_days": HOLD_DAYS,
...
def _select_protective_stop_paper_trades(
    *,
    snapshot: dict[str, list[dict[str, Any]]],
    candidates: list[dict[str, Any]],
    ...
    for row in candidates:
        ...
        trade = _paper_trade_with_protective_stop(snapshot, row)
        if trade is None:
...
def _stop_exit_summary(
    ...
    for label, trades in target_trades_by_window.items():
        stop_trades = [row for row in trades if row.get("stop_triggered")]
        scheduled_trades = [row for row in trades if not row.get("stop_triggered")]
        total_trades += len(trades)
        total_stop_trades += len(stop_trades)
        window_stop_pnl = sum(float(row.get("pnl") or 0.0) for row in stop_trades)
        window_scheduled_pnl = sum(float(row.get("pnl") or 0.0) for row in scheduled_trades)
        stop_pnl += window_stop_pnl
        scheduled_pnl += window_scheduled_pnl
        by_window[label] = {
            "trade_count": len(trades),
            "stop_triggered_count": len(stop_trades),
            "scheduled_exit_count": len(scheduled_trades),
            "stop_trigger_rate": _round(len(stop_trades) / len(trades), 6) if trades else None,
            "stop_pnl": _round(window_stop_pnl, 2),
            "scheduled_pnl": _round(window_scheduled_pnl, 2),
        }
    return {
        "total_trade_count": total_trades,
        "stop_triggered_count": total_stop_trades,
        "scheduled_exit_count": total_trades - total_stop_trades,
        "stop_trigger_rate": _round(total_stop_trades / total_trades, 6) if total_trades else None,
        "stop_pnl": _round(stop_pnl, 2),
        "scheduled_pnl": _round(scheduled_pnl, 2),
        "by_window": by_window,
    }
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_cash_conversion_protective_stop"
        if gate["passed"]
        else "rejected_cash_conversion_protective_stop_candidate_pool"
    )
    return gate
...
def _build_payload() -> dict[str, Any]:
    ...
    for label, cfg in framework.WINDOWS.items():
        ...
        selected_trades, filtered_candidates = _select_protective_stop_paper_trades(
            snapshot=snapshot,
    ...
    stop_exit_summary = _stop_exit_summary(target_trades_by_window)
    gate4 = _gate4(
        aggregate=aggregate,
        target_summary=target_summary,
        before_metrics=before_metrics,
    ...
    return {
        ...
        "hypothesis": (
            ...
            "fixed, a daily-close 7% protective stop with next-open paper exit "
            "may preserve the all-window edge while reducing the drawdown drift "
            "that rejected the full 10-day deployment."
        ),
        "change_type": "default_off_paper_exit_envelope",
        "changed_variable": CHANGED_VARIABLE,
        "trial_family": TRIAL_FAMILY,
        "trial_variant_id": TRIAL_VARIANT_ID,
        "mechanism_family": "production_visible_free_sec_companyfacts_quality_candidate_pool",
        "new_evidence_type": "drawdown_aware_exit_envelope_for_sec_cash_conversion_quality",
        ...
        "backtest_protocol": {
            ...
            "execution_model": (
                ...
                "the next available open with existing entry slippage. After "
                "entry, a daily close at or below 93% of entry fill exits at "
                "the next available open with stop-side sell slippage; otherwise "
        ...
        "parameters": {
            ...
            "same_ticker_cooldown_days": SAME_TICKER_COOLDOWN_DAYS,
            "protective_stop_loss_pct": PROTECTIVE_STOP_LOSS_PCT,
            "protective_stop_trigger": PROTECTIVE_STOP_TRIGGER,
        ...
        "stop_exit_summary": stop_exit_summary,
        ...
        "interpretation": (
            "The fixed-stop annual cash-conversion quality envelope cleared "
            "Gate 4 as a replay-only/default-off lead, but no production surface "
            "was promoted yet."
            if gate4["passed"]
            else (
                "The fixed-stop annual cash-conversion quality envelope kept "
                "the original exp-20260614-020 source and selection fixed but "
                "was still rejected on drawdown drift. It is a failed paper "
                "exit-envelope hypothesis, not a reason to retune cash-conversion "
                "or price-confirmation thresholds on the frozen windows."
                if drawdown_only_failure
                else (
                    "The fixed-stop annual cash-conversion quality envelope "
                    "did not clear Gate 4 (failed: "
                    + (", ".join(gate4["failed_reasons"]) or "none")
                    + "). Do not promote or retune this stopped bundle on "
        ...
        "post_run_reflection": {
            "why_result_happened": (
                (
                    "Rejected on drawdown drift only. The fixed-stop envelope "
                    "kept the cash-conversion definition fixed and added a daily "
                    "close stop, but aggregate EV {:+.4f} / PnL ${:+,.2f} still "
                    "did not satisfy the 0.5pp drawdown guardrail; max drawdown "
                    "drift was +{:.4f}. The source remains directionally "
                    "interesting, but this exit envelope did not make the "
                    "historical tail risk acceptable."
                ).format(
                ...
                if drawdown_only_failure
                    ...
                    "Rejected. The fixed-stop envelope on the annual "
                    "cash-conversion quality source did not clear Gate 4 versus "
                    "accepted comparators (failed: {}). The stop may have cut "
                    "rebounds, or old_thin tail behavior may be gap/path driven "
                    "rather than solvable by a daily-close trigger."
            ...
            "new_evidence_required": (
                "Need a materially different exit/deployment envelope, forward "
                ...
                "cash-flow when reported). Do not retry this stopped bundle with "
                "threshold, stop-level, hold, cooldown, or notional sweeps."
                if drawdown_only_failure
                else (
                    ...
                    "rows before revisiting cash-conversion quality. Do not retry "
                    "by sweeping the protective stop level on the same windows."
                )
            ),
        },
        "related_files": [
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Cash-Conversion Protective Stop",
            "",
            ...
            *rows,
            ...
            "- Stop-triggered exits: `{}` ({})".format(
                payload["stop_exit_summary"]["stop_triggered_count"],
                payload["stop_exit_summary"]["stop_trigger_rate"],
            ),
...
def _build_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "accepted_distribution_reference_comparator": DISTRIBUTION_COMPARATOR,
        "gate4": payload["gate4"],
        "stop_exit_summary": payload["stop_exit_summary"],
        "windows": [
            {
                "label": label,

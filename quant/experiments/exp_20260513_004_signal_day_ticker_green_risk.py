"""exp-20260513-004: signal-day own-green-candle risk allocation.
...
EXPERIMENT_ID = "exp-20260513-004"
...
def _run_window(label: str, scalar: float | None) -> dict[str, Any]:
    ...
    original_green_multiplier = getattr(
        portfolio_engine,
    ...
    global ADJUSTMENTS
    ADJUSTMENTS = []
    if original_green_multiplier is not None:
        portfolio_engine.SIGNAL_DAY_TICKER_GREEN_RISK_MULTIPLIER = 1.0
    ...
    try:
        ...
        if original_green_multiplier is not None:
            portfolio_engine.SIGNAL_DAY_TICKER_GREEN_RISK_MULTIPLIER = original_green_multiplier
...
def _build_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            "",
            "Production impact: promoted through shared feature/risk/sizing policy with backtester attribution and parity tests.",
        ]
    )
    return "\n".join(lines)
...
def run() -> dict[str, Any]:
    ...
    decision = (
        "accepted_shared_policy_signal_day_ticker_green_risk"
        if best_variant["gate"]["passed"]
        else "rejected_signal_day_ticker_green_risk"
    )
    interpretation = (
        "Own green-candle signal-day risk allocation cleared the three-window gate and is promoted through shared production/backtest policy."
        if best_variant["gate"]["passed"]
    ...
    return {
        ...
        "change_type": "risk_allocation_shared_policy",
        ...
        "gate_questions": {
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_004_signal_day_ticker_green_risk.py",
        },
        "backtest_protocol": {
        ...
        "related_files": [
            "quant/experiments/exp_20260513_004_signal_day_ticker_green_risk.py",
            "data/experiments/exp-20260513-004/signal_day_ticker_green_risk.json",
            "docs/experiments/logs/exp-20260513-004.json",
            "docs/experiments/tickets/exp-20260513-004.json",
            "docs/experiments/artifacts/exp-20260513-004_signal_day_ticker_green_risk.md",

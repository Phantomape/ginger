...
This run changes one causal variable: a default-off paper notional support
scalar for already-selected broad-market candidates whose composite score is
...
EXPERIMENT_ID = "exp-20260520-009"
EXPERIMENT_SLUG = "broad_market_absolute_score_support_notional"
...
SCORE_SWEEP: OrderedDict[str, dict[str, float]] = OrderedDict(
    [("baseline_no_absolute_score_support", {"score_min": 0.60, "scalar": 1.00})]
)
for threshold in (0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00):
    for scalar in (1.025, 1.05, 1.075, 1.10, 1.15):
        ...
        SCORE_SWEEP[f"score_gte_{threshold_text}_scalar_{scalar_text}"] = {
            "score_min": threshold,
            "scalar": scalar,
        }
...
def _variant_trades(
    *,
    ...
    for row in base_rows:
        ...
        if trade is None:
            continue
        score = float(row["feature"]["score"])
        support_applied = score >= float(score_min) and float(scalar) != 1.0
        if support_applied:
        ...
        trade["window"] = label
        trade["absolute_score_support_score_min"] = score_min
        trade["absolute_score_support_notional_scalar"] = scalar
        trade["absolute_score_support_notional_multiplier"] = scalar if support_applied else 1.0
        trade["absolute_score_support_applied"] = support_applied
        trades.append(trade)
    return trades
        ...
        rows.append(
            {
                ...
                "rank": trade["rank"],
                "score": trade["score"],
                "absolute_score_support_applied": trade.get("absolute_score_support_applied"),
                "absolute_score_support_notional_multiplier": trade.get(
                    "absolute_score_support_notional_multiplier"
                ),
...
def _window_sleeve_summary(
    ...
    adjusted = [row for row in trades if row.get("absolute_score_support_applied")]
    base["absolute_score_adjusted_trade_count"] = len(adjusted)
    base["absolute_score_adjusted_pnl"] = round(
...
def _variant_payload(
    *,
    ...
    adjusted = [row for row in all_trades if row.get("absolute_score_support_applied")]
    ...
    gate4_passed = bool(
        variant_name != "baseline_no_absolute_score_support"
...
def _choose_selected(variants: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pool = passing or [
        row for row in variants if row["variant_name"] != "baseline_no_absolute_score_support"
    ]
    return sorted(
        pool,
        key=lambda row: (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Broad-Market Absolute-Score Support Notional",
        "",
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for name, values in SCORE_SWEEP.items()
    ]
    identity = variants[0]
    identity_control = {
        "passed": (
        ...
        "interpretation": (
            "The current local broad-market replay inputs do not reproduce the "
            "accepted exp-20260520-004 artifact exactly. This blocks promotion "
            "even if a support scalar had passed the field-specific gate."
        ),
    }
    ...
    decision = (
        "accepted_default_off_broad_market_absolute_score_support_notional"
        if accepted
        else "rejected_broad_market_absolute_score_support_notional"
    )
    ...
    shared_adapter_parity = {
        ...
        "reason": (
            "Gate 4 failed; no shared broad-market policy or production adapter change is retained."
            if not accepted
            else "Gate 4 passed; shared helper would need promotion before merge."
        ),
        "selected_score_min": selected["score_min"],
        "selected_absolute_score_scalar": selected["scalar"],
    }
    production_impact = {
    ...
    payload: dict[str, Any] = {
        ...
        "hypothesis": (
            ...
            "high absolute composite score may carry standalone quality beyond "
            "relative rank; a bounded support notional scalar can improve "
            "allocation after the accepted rank, low-extension, high-volatility, "
            "and trend-persistence stack."
        ),
        "alpha_hypothesis": {
            "category": "candidate_pool / capital allocation",
            "playbook_alignment": (
                "Tests a new production-visible quality field on the default-off "
                "broad-market paper sleeve while avoiding LLM soft-ranking, live "
                "core expansion, and nearby price/ret20/ret60/near-high/volume/"
                "ret5 threshold retunes."
            ),
        },
        "history_check": {
            "nearby_experiments": [
                ...
                "exp-20260520-005 rejected decision-day close-location crowding haircut",
            ],
            "anti_repeat": (
                ...
                "price floor, hold, slots, and universe are fixed; only an "
                "absolute composite-score support field changes."
            ),
            "selection_note": (
                "The score is decision-time OHLCV-derived and already produced by "
                "the shared broad-market feature builder; this run tests whether "
                "absolute score should route more paper notional after selection."
            ),
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "broad_market_absolute_score_support_notional_scalar",
        "single_causal_variable": "absolute composite-score support scalar for fixed broad-market selected candidates",
        "component": "quant/experiments/exp_20260520_009_broad_market_absolute_score_support_notional.py",
        "parameters": {
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
            "control_experiment_id": CONTROL_EXPERIMENT_ID,
            "selected_variant": selected["variant_name"],
            "selected_score_min": selected["score_min"],
            "selected_absolute_score_support_scalar": selected["scalar"],
            "sweep": SCORE_SWEEP,
            "profile_config": {
        ...
        "backtest_protocol": (
            ...
            "state; after state adds an absolute-score paper-notional support field."
        ),
        "gate1": {
            "passed": True,
            "baseline_experiment_id": BASELINE_EXPERIMENT_ID,
        ...
        "protocol_answers": {
            "1_alpha_hypothesis": "candidate_pool/capital allocation: high absolute composite score may deserve bounded paper notional support after selection.",
            "2_past_similar_experiments": "Broad-market price-floor, rank-notional, low-extension ret5, high-volatility, trend-persistence, and strong-close crowding were tested; absolute composite-score support was not previously formalized for this sleeve.",
            "3_single_variable": "Only score_min/scalar changes inside one notional-support field; eligibility, rank profile, low-extension, high-volatility, trend-persistence, hold, slots, and universe controls stay fixed.",
        ...
        "interpretation": (
            "Absolute composite score has positive aggregate replacement value "
            "but does not clear the broad-market paper gate: the best variant "
            "regresses one window and worsens max drawdown beyond the guardrail. "
            "The broad-market sleeve should collect forward outcomes instead of "
            "adding another frozen-window support scalar."
        ),
        "rejection_reason": None
        if accepted
        else (
            "Best absolute-score support variant had positive aggregate EV/PnL "
            "but failed Gate 4 on one EV/PnL-regressed window and max-drawdown "
            "drift; the no-support identity control also drifted versus the "
            "accepted exp-20260520-004 artifact, so no policy can be promoted."
        ),
        "next_evidence_needed": (
            "Collect forward broad-market paper outcomes with absolute-score "
            "metadata; do not retry adjacent score-support thresholds on the "
            "frozen sample without new forward evidence or a distinct quality field."
        ),
        ...
        "known_risks": [
            ...
            "The score field is already part of ranking, so support scalars can easily become hidden rank leverage.",
        ],
        "related_files": {

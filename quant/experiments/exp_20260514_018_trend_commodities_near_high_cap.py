"""exp-20260514-018: trend commodities near-high position cap.

Tests one production-visible capital-allocation variable on the accepted core:
the already-accepted `trend_long + Commodities + near 52-week high` sleeve may
be under-allocated because the raw risk boost often hits the default 40% single
position cap. This tests only the sleeve-specific cap, not entries, exits,
ranking, universe, LLM/news behavior, or the raw commodity risk multiplier.
"""
...
EXPERIMENT_ID = "exp-20260514-018"
EXPERIMENT_SLUG = "trend_commodities_near_high_cap"
CAP_KEY = "trend_commodities_near_high_max_position_pct_applied"
CAP_SWEEP = [0.45, 0.50]
MAX_DRAWDOWN_WORSE_GUARDRAIL = 0.005

CURRENT_MAX_POSITION_PCT = 0.40
...
def _resize_with_cap(
    ...
    old_cap_shares = max(
        1,
        int(math.floor(portfolio_value * base.portfolio_engine.MAX_POSITION_PCT / entry)),
    )
    new_cap_shares = max(
        1,
    ...
    out = dict(sizing)
    out["trend_commodities_near_high_cap_baseline_shares"] = old_shares
    out["trend_commodities_near_high_cap_raw_shares"] = raw_shares
    out["trend_commodities_near_high_cap_old_cap_shares"] = old_cap_shares
    out["trend_commodities_near_high_cap_new_cap_shares"] = new_cap_shares
    out["trend_commodities_near_high_cap_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "raw_shares": adjusted_sizing.get(
                                "trend_commodities_near_high_cap_raw_shares"
                            ),
                            "old_cap_shares": adjusted_sizing.get(
                                "trend_commodities_near_high_cap_old_cap_shares"
                            ),
                            "new_cap_shares": adjusted_sizing.get(
                                "trend_commodities_near_high_cap_new_cap_shares"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload["sweep_results"]:
        ...
        sweep_rows.append(
            "| {cap:.2f} | {passed} | {dev:+.4f} | ${dpnl:+,.2f} | {dd:+.4f} | {adj} |".format(
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Trend Commodities Near-High Cap",
            "",
            ...
            *sweep_rows,
            "",
            "Production impact: shadow experiment only unless promoted into shared `portfolio_engine.py`. A positive promotion must set the cap in shared sizing before both backtest and production paths call `size_signals`.",
        ]
    )
...
def run() -> dict[str, Any]:
    ...
    sweep_results = [
        _candidate_payload(cap, before_runs)
    ...
    decision = (
        ...
        else "rejected_trend_commodities_near_high_cap"
    )
    interpretation = (
        "The existing near-high commodity trend sleeve remained cap-bound and the selected sleeve-specific cap improved the canonical three-window stack without EV regression."
        if detailed_selected["passed"]
        else "The sleeve-specific cap did not clear the canonical three-window gate; do not promote a commodity cap change without a new production-visible discriminator."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted trend_long Commodities near-52-week-high sleeve is a convex winner cluster but is often capped at the default 40% single-position limit. Raising only that sleeve's cap should unlock winner convexity without adding noisy tickers or changing entries."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "trend_commodities_near_high_max_position_pct",
        "single_causal_variable": (
            "max_position_pct for trend_long Commodities signals with pct_from_52w_high >= -0.03"
        ),
        "parameters": {
            ...
            "qualifier": {
                "strategy": "trend_long",
                "sector": "Commodities",
                "pct_from_52w_high_min": base.portfolio_engine.TREND_COMMODITIES_NEAR_HIGH_MAX_PULLBACK,
            },
            "locked_variables": [
                "core universe",
                "candidate pool",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "capital allocation: unlock cap room only for an accepted, production-visible commodity trend near-high winner sleeve",
            "2_history_check": {
                "exp-20260508-003": "accepted 1.5x commodity near-high raw risk; 2.0x raw scalar was directionally positive but cap-bound and not promoted",
                "exp-20260502-006": "commodity breakout leader cap failed drawdown guardrail; this run is trend-only and existing near-high sleeve-only",
                "exp-20260501-004": "gold-only cap tuning on an older stack improved windows but was not accepted; this run tests the current accepted sleeve after later core allocation changes",
                "not_repeated": "does not alter raw commodity multiplier, gold target, Space, VSAT, dual-RS, or LLM soft-ranking",
            },
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_018_trend_commodities_near_high_cap.py",
        },
        "backtest_protocol": {
        ...
        "llm_metrics": {
            "used_llm": False,
            "blocker_relation": (
                "LLM soft-ranking remains candidate-replay/data limited, so this run deliberately tested a deterministic capital-allocation alpha."
            ),
        },
        "production_impact": {
        ...
        if detailed_selected["passed"]
        else "Use forward cap-room attribution or a new production-visible commodity state before retrying commodity allocation.",
        "related_files": [
            "quant/experiments/exp_20260514_018_trend_commodities_near_high_cap.py",
            "data/experiments/exp-20260514-018/trend_commodities_near_high_cap.json",
            "docs/experiments/logs/exp-20260514-018.json",
            "docs/experiments/tickets/exp-20260514-018.json",
            "docs/experiments/artifacts/exp-20260514-018_trend_commodities_near_high_cap.md",

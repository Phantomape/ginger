"""exp-20260514-019: RS60 top-quintile position cap.

Tests one production-visible capital-allocation variable on the accepted core:
the already-accepted RS60 top-quintile stock sleeve may still be under-allocated
because the post-sizing share top-up can hit the default 40% single-position
cap. This tests only the sleeve-specific cap, not entries, exits, ranking,
universe, LLM/news behavior, or the accepted RS60 share multiplier.
"""
...
EXPERIMENT_ID = "exp-20260514-019"
EXPERIMENT_SLUG = "rs60_top_quintile_cap"
CAP_KEY = "rs60_top_quintile_max_position_pct_applied"
CAP_SWEEP = [0.45, 0.50]
MAX_DRAWDOWN_WORSE_GUARDRAIL = 0.005

CURRENT_RS60_MAX_POSITION_PCT = 0.40
...
def _is_target_sleeve(sig: dict[str, Any], sizing: dict[str, Any]) -> bool:
    return bool(
        ...
        and sig.get("sector")
        not in base.risk_engine.RS60_TOP_QUINTILE_EXCLUDED_SECTORS
...
def _resize_with_cap(
    ...
    new_cap_shares = max(
        1,
        int(math.floor(portfolio_value * CURRENT_RS60_MAX_POSITION_PCT / entry)),
    )
    ...
    out = dict(sizing)
    out["rs60_top_quintile_cap_baseline_shares"] = old_shares
    out["rs60_top_quintile_cap_raw_shares"] = raw_shares
    out["rs60_top_quintile_cap_old_cap_shares"] = old_cap_shares
    out["rs60_top_quintile_cap_new_cap_shares"] = new_cap_shares
    out["rs60_top_quintile_cap_new_shares"] = new_shares
    ...
    out["max_position_pct_applied"] = CURRENT_RS60_MAX_POSITION_PCT
    out[CAP_KEY] = CURRENT_RS60_MAX_POSITION_PCT
    return out
...
def _run_window(label: str, max_position_pct: float | None) -> dict[str, Any]:
    ...
    global CURRENT_RS60_MAX_POSITION_PCT
    previous_cap = CURRENT_RS60_MAX_POSITION_PCT
    base.ADJUSTMENTS = []

    if max_position_pct is not None:
        CURRENT_RS60_MAX_POSITION_PCT = max_position_pct
    ...
    try:
        ...
        CURRENT_RS60_MAX_POSITION_PCT = previous_cap
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} RS60 Top-Quintile Cap",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        else "rejected_rs60_top_quintile_cap"
    )
    interpretation = (
        "The accepted RS60 top-quintile stock sleeve remained cap-bound and the selected sleeve-specific cap improved the canonical three-window stack without EV regression."
        if detailed_selected["passed"]
        else "The RS60 top-quintile cap did not clear the canonical three-window gate; keep the accepted 1.15x top-up but do not promote extra cap room."
    )
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted RS60 top-quintile stock sleeve identifies momentum winners, but its post-sizing top-up can still be clipped by the default 40% single-position cap. Raising only that sleeve cap should unlock convexity without adding noisy tickers or changing entries."
        ),
        "change_type": "capital_allocation_shadow",
        "changed_variable": "rs60_top_quintile_max_position_pct",
        "single_causal_variable": (
            "max_position_pct for existing rs60_top_quintile_state stock signals"
        ),
        "parameters": {
            ...
            "qualifier": {
                ...
                "excluded_sectors": list(
                    base.risk_engine.RS60_TOP_QUINTILE_EXCLUDED_SECTORS
                ),
            },
            "locked_variables": [
                "core universe",
                ...
                "accepted RS60 1.15x share top-up",
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "capital allocation: unlock cap room only for the accepted production-visible RS60 top-quintile stock sleeve",
            "2_history_check": {
                "exp-20260513-030": "accepted RS60 top-quintile 1.15x cap-aware post-sizing top-up",
                "exp-20260514-014": "dual RS persistent leader top-up was rejected; this run does not change the RS60 multiplier or require RS20 overlap",
                "search_result": "no prior rs60_top_quintile max-position cap experiment was found in docs/data/quant before this run",
                "not_repeated": "does not tune RS60 fraction, RS60 scalar, dual-RS scalar, entry priority, filters, Space, or LLM soft-ranking",
            },
            ...
            "5_reproducibility": "Run .venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_019_rs60_top_quintile_cap.py",
        },
        "backtest_protocol": {
        ...
        if detailed_selected["passed"]
        else "Use new forward cap-room attribution or a different production-visible RS state before retrying RS60 cap expansion.",
        "related_files": [
            "quant/experiments/exp_20260514_019_rs60_top_quintile_cap.py",
            "data/experiments/exp-20260514-019/rs60_top_quintile_cap.json",
            "docs/experiments/logs/exp-20260514-019.json",
            "docs/experiments/tickets/exp-20260514-019.json",
            "docs/experiments/artifacts/exp-20260514-019_rs60_top_quintile_cap.md",

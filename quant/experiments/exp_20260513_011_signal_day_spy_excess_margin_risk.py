"""exp-20260513-011: signal-day ticker-vs-SPY excess-margin risk.

Alpha search. The immediately prior ticker-vs-SPY signal-day outperformance
scout was profitable in all three windows at 1.10x, but failed Gate 4 because
mid_weak max drawdown worsened by more than the guardrail. This experiment
changes one causal variable: the minimum same-day ticker-minus-SPY excess
return required for the same fixed 1.10x cap-aware post-sizing top-up.

No entry filter, ranking, exit, target, universe, LLM/news, portfolio heat, or
production behavior changes here. A positive scout must be promoted through
shared policy before it can affect production.
"""
...
EXPERIMENT_ID = "exp-20260513-011"
EXPERIMENT_SLUG = "signal_day_spy_excess_margin_risk"
MULTIPLIER_KEY = "signal_day_spy_excess_margin_risk_multiplier_applied"
RISK_MULTIPLIER = 1.10
EXCESS_MARGINS = [0.0, 0.01, 0.015, 0.02, 0.025, 0.03, 0.04, 0.05]

CURRENT_EXCESS_MARGIN = 0.0
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        spy_ret = (features_dict.get("SPY") or {}).get(
            "signal_day_ticker_open_close_return_pct"
        )
        for sig in enriched:
            ticker_ret = sig.get("signal_day_ticker_open_close_return_pct")
            excess = None
            eligible = False
            if isinstance(ticker_ret, (int, float)) and isinstance(spy_ret, (int, float)):
                excess = round(float(ticker_ret) - float(spy_ret), 6)
                eligible = excess >= CURRENT_EXCESS_MARGIN
            sig["signal_day_spy_open_close_return_pct"] = spy_ret
            sig["signal_day_ticker_minus_spy_open_close_return_pct"] = excess
            sig["signal_day_spy_excess_margin_eligible"] = eligible
            sig["signal_day_spy_excess_margin_threshold"] = CURRENT_EXCESS_MARGIN
        return enriched
...
def _scale_sizing(
    ...
    desired_shares = max(shares, int(math.floor(shares * RISK_MULTIPLIER)))
    ...
    out = dict(sizing)
    out["signal_day_spy_excess_margin_baseline_shares"] = shares
    out["signal_day_spy_excess_margin_desired_shares"] = desired_shares
    out["signal_day_spy_excess_margin_cap_shares"] = cap_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if (
                sig.get("signal_day_spy_excess_margin_eligible") is True
                and sig.get("strategy") in {"trend_long", "breakout_long"}
                and sizing.get("shares_to_buy")
            ):
                adjusted = _scale_sizing(sig, sizing, portfolio_value)
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "spy_open_close_return_pct": sig.get(
                                "signal_day_spy_open_close_return_pct"
                            ),
                            "ticker_minus_spy_open_close_return_pct": sig.get(
                                "signal_day_ticker_minus_spy_open_close_return_pct"
                            ),
                            "excess_margin_threshold": CURRENT_EXCESS_MARGIN,
                            "baseline_shares": sizing.get("shares_to_buy"),
                            "new_shares": adjusted.get("shares_to_buy"),
                            "scalar": RISK_MULTIPLIER,
...
def _candidate_payload(
    ...
    global CURRENT_EXCESS_MARGIN
    CURRENT_EXCESS_MARGIN = excess_margin
...
def _sweep_summary(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {
            "excess_margin": row["excess_margin"],
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload["sweep_summary"]:
        sweep_rows.append(
            "| {margin:.3%} | {mult:.2f} | {passed} | {dev:+.4f} | ${dpnl:+,.2f} | {improved} | {regressed} | {adj} | {dd:+.4f} |".format(
                margin=row["excess_margin"],
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Signal-Day SPY Excess-Margin Risk",
            "",
            ...
            *sweep_rows,
            "",
            f"Selected excess margin: `{payload['parameters']['selected_excess_margin']:.3%}`.",
            "",
...
def run() -> dict[str, Any]:
    ...
    candidates = [
        _candidate_payload(excess_margin, before_runs)
        for excess_margin in EXCESS_MARGINS
    ]
    ...
    decision = (
        ...
        if passed
        else "rejected_signal_day_spy_excess_margin_risk"
    )
    interpretation = (
        "A stricter signal-day ticker-vs-SPY excess margin cleared Gate 4 and should be promoted through shared policy before production use."
        if passed
        else "Tightening the signal-day ticker-vs-SPY excess margin did not clear the canonical three-window gate; do not promote this relative signal-day top-up on frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The prior ticker-vs-SPY same-day top-up failed only on drawdown; requiring a stronger positive ticker-minus-SPY signal-day excess may keep idiosyncratic follow-through while avoiding the fragile low-margin cases."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "signal_day_ticker_minus_spy_excess_margin_threshold",
        "single_causal_variable": (
            "minimum signal-day ticker open-to-close return minus SPY open-to-close return required for a fixed 1.10x top-up"
        ),
        "parameters": {
            "state_definition": (
                "signal_day_ticker_open_close_return_pct - signal_day_spy_open_close_return_pct >= selected_excess_margin"
            ),
            "fixed_risk_multiplier": RISK_MULTIPLIER,
            "excess_margin_sweep": EXCESS_MARGINS,
            "selected_excess_margin": selected["excess_margin"],
            "locked_variables": [
                "core universe",
                "entry filters",
                "candidate ranking",
                "stop and target logic",
                "all existing sizing multipliers including accepted own-green 1.05x",
                "portfolio heat",
                "LLM/news replay",
                "pilot/event sleeves",
            ],
        ...
        "gate_questions": {
            "1_alpha_hypothesis": "core risk allocation using stronger production-knowable signal-day ticker-vs-SPY excess return",
            "2_history_check": {
                "exp-20260513-007": "own green-candle 1.05x is accepted and remains locked.",
                "exp-20260513-009": "upper-quartile close-location top-up failed from old_thin/drawdown fragility.",
                "exp-20260513-010": "zero-margin ticker-vs-SPY 1.10x improved all windows but failed max-drawdown guardrail; this run changes only the excess-margin threshold.",
                "exp-20260512-106/107": "sector-proxy tape scalars failed or were underpowered; this uses ticker-vs-SPY, not sector tape.",
            },
            "3_single_causal_variable": "signal_day_ticker_minus_spy_excess_margin_threshold, with fixed 1.10x top-up",
            "4_acceptance_standard": "docs/backtesting.md three fixed windows; aggregate EV/PnL positive, at least two EV-improved windows, no EV-regressed window, survival >= 5%, max drawdown drift <= 0.5 pp.",
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_011_signal_day_spy_excess_margin_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        "gate2": {
            ...
            "runtime_fields": [
                "feature_layer signal_day_ticker_open_close_return_pct for each ticker",
                "feature_layer signal_day_ticker_open_close_return_pct for SPY",
                "risk_engine signal_day_ticker_minus_spy_open_close_return_pct",
        ...
        if passed
        else "Use forward signal-day relative-strength attribution or a different production-visible state before retrying.",
        "related_files": [
            "quant/experiments/exp_20260513_011_signal_day_spy_excess_margin_risk.py",
            "data/experiments/exp-20260513-011/signal_day_spy_excess_margin_risk.json",
            "docs/experiments/logs/exp-20260513-011.json",
            "docs/experiments/tickets/exp-20260513-011.json",
            "docs/experiments/artifacts/exp-20260513-011_signal_day_spy_excess_margin_risk.md",
            "docs/experiment_log.jsonl",
        ],
        "why_not_other_changes": (
            "LLM soft-ranking remains data-limited; SEC semantic fields are missing; Space frozen-sample profile scalars and noisy candidate additions have been over-mined or rejected; own-candle close-location and strong-volume neighbors were rejected. This run tests one benchmark-relative threshold variable without changing entries or expanding the pool."
        ),
    }
    return payload
...
def persist(payload: dict[str, Any]) -> None:
    ...
    ticket = {
        ...
        "selected_excess_margin": payload["parameters"]["selected_excess_margin"],
        ...
        json.dumps(
            {
                ...
                "selected_excess_margin": result["parameters"]["selected_excess_margin"],
                "fixed_risk_multiplier": result["parameters"]["fixed_risk_multiplier"],
            },
            indent=2,
            sort_keys=True,
        )

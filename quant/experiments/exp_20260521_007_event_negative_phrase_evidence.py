"""exp-20260521-007: SEC negative phrase evidence event scout.

Alpha search, replay-only. Tests one production-visible event text field on
top of the accepted exp-20260521-006 default-off event overlay adapter:
whether already-selected SEC negative-reaction rows with negative phrase
evidence deserve extra paper notional.
...
import exp_20260521_005_event_governance_source_quality as governance_scout


EXPERIMENT_ID = "exp-20260521-007"
EXPERIMENT_SLUG = "event_negative_phrase_evidence"

REPO_ROOT = governance_scout.REPO_ROOT
...
VARIANTS: "OrderedDict[str, dict[str, Any]]" = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
                "description": "Accepted exp-20260521-006 event adapter with rotation, front-rank, broad-breadth, and governance-source quality scalars fixed.",
                "negative_phrase_evidence_scalar": 1.0,
            },
        ),
        (
            "negative_phrase_evidence_125",
            {
                "description": "Multiply accepted event paper notional by 1.25x when source is sec_negative_reaction and negative_phrase_hits >= 3.",
                "negative_phrase_evidence_scalar": 1.25,
            },
        ),
        (
            "negative_phrase_evidence_150",
            {
                "description": "Multiply accepted event paper notional by 1.50x when source is sec_negative_reaction and negative_phrase_hits >= 3.",
                "negative_phrase_evidence_scalar": 1.50,
            },
        ),
        (
            "negative_phrase_evidence_175",
            {
                "description": "Multiply accepted event paper notional by 1.75x when source is sec_negative_reaction and negative_phrase_hits >= 3.",
                "negative_phrase_evidence_scalar": 1.75,
            },
        ),
        (
            "negative_phrase_evidence_200",
            {
                "description": "Multiply accepted event paper notional by 2.00x when source is sec_negative_reaction and negative_phrase_hits >= 3.",
                "negative_phrase_evidence_scalar": 2.00,
            },
        ),
    ]
)
...
def _parent():
    return governance_scout._parent()
...
def _configure_modules() -> None:
    governance_scout._configure_modules()
...
def _accepted_event_scalar_after_exp006(trade: dict[str, Any]) -> float:
    scalar = governance_scout._accepted_event_scalar(trade)
    if governance_scout.current._is_broad_breadth_event(trade):
        scalar *= 1.25
    if str(trade.get("source") or "") == "sec_governance_procedural":
        scalar *= 2.0
    return scalar
...
def _is_target_negative_phrase_evidence(trade: dict[str, Any]) -> bool:
    return (
        str(trade.get("source") or "") == TARGET_SOURCE
        and _hit_count(trade.get("negative_phrase_hits"))
        >= TARGET_NEGATIVE_PHRASE_HITS_MIN
    )
...
def _scaled_trade(
    ...
    target = _is_target_negative_phrase_evidence(trade)
    phrase_scalar = (
        float(variant["negative_phrase_evidence_scalar"]) if target else 1.0
    )
    scalar = accepted_scalar * phrase_scalar
    ...
    return {
        **trade,
        ...
        "negative_phrase_evidence_target": target,
        "negative_phrase_evidence_scalar": round(phrase_scalar, 4),
...
def _selection_summary(rows_by_window: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    targets = [row for row in all_rows if row.get("negative_phrase_evidence_target")]
    for label, rows in rows_by_window.items():
        window_targets = [
            row for row in rows if row.get("negative_phrase_evidence_target")
        ]
    ...
    return {
        ...
        "target_negative_phrase_hits_min": TARGET_NEGATIVE_PHRASE_HITS_MIN,
        ...
        "target_text_event_types": sorted(
            {str(row.get("text_event_type") or "") for row in targets}
        ),
...
def _choose_best(gates: dict[str, dict[str, Any]]) -> str:
    ...
    return max(
        ...
        key=lambda name: (
            ...
            -VARIANTS[name]["negative_phrase_evidence_scalar"],
        ),
    )
...
def build_payload() -> dict[str, Any]:
    ...
    decision = (
        "accepted_default_off_event_negative_phrase_evidence"
        if accepted
        else "rejected_event_negative_phrase_evidence"
    )
    ...
    return {
        ...
        "status": decision,
        "decision": decision,
        "change_type": "event_text_quality_allocation_replay",
        "mechanism_family": "external_event_satellite_overlay_allocation",
        "trial_family": "sec_negative_reaction_text_evidence",
        "trial_variant_id": "sec_negative_phrase_hits_gte_3_notional",
        "changed_variable": "event_sec_negative_phrase_evidence_paper_notional_scalar",
        "prior_trial_count": 4,
        ...
        "new_evidence_type": "new_accepted_event_adapter_baseline",
        "hypothesis": (
            "Inside the accepted default-off event overlay, already-selected "
            "SEC negative-reaction rows with replay-visible negative phrase "
            "evidence may deserve more paper notional because the market may "
            "have overreacted to text that is already embedded in the first "
            "reaction."
        ),
        "alpha_hypothesis": {
            "category": "capital allocation / event text quality",
            "entry_exit_ranking_or_allocation": "capital allocation",
            "playbook_alignment": (
                "Uses a deterministic production-visible event text field in "
                "the event replacement-value lane, while avoiding LLM soft "
                "ranking, broad-market retunes, state-surface scalars, source "
                "capacity expansion, and governance-source retuning."
            ),
        },
        "single_causal_variable": (
            "paper-notional scalar for event overlay rows with source "
            "sec_negative_reaction and negative_phrase_hits >= 3"
        ),
        "parameters": {
            "variants": VARIANTS,
            ...
            "target_source": TARGET_SOURCE,
            "target_negative_phrase_hits_min": TARGET_NEGATIVE_PHRASE_HITS_MIN,
            "selected_negative_phrase_evidence_scalar": VARIANTS[best_variant][
                "negative_phrase_evidence_scalar"
            ],
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "Selected SEC negative-reaction rows with negative phrase "
                "evidence may carry overreaction/replacement value; this is "
                "event text scoring plus capital allocation."
            ),
            "2_history_check": (
                "SEC negative source pruning/text severity were rejected in "
                "exp-20260507-012/015 and negative source tilt was rejected in "
                "exp-20260516-030. This run is high multiple-testing risk and "
                "is not retainable unless all three windows and sample guards pass."
            ),
            "3_single_causal_variable": (
                "Only the paper-notional scalar for the fixed "
                "negative_phrase_hits >= 3 SEC negative-reaction cohort changes."
            ),
            ...
            "5_reproducibility": (
                ...
                "exp_20260521_007_event_negative_phrase_evidence.py"
            ),
        },
        "historical_experiment_check": {
            "exp-20260507-012": "Rejected event source pruning; full event packet was stronger.",
            "exp-20260507-015": "Rejected simple SEC negative text-severity gating on the frozen sample.",
            "exp-20260516-030": "Rejected source-specific SEC negative-reaction tilt.",
            "exp-20260521-001": "Accepted broad-breadth event context.",
        ...
        "gate2": {
            ...
            "passed": bool(
                ...
                >= 10
                and (
                    selection_by_variant[BASELINE_VARIANT].get(
        ...
        "llm_metrics": {
            ...
            "blocker_relation": (
                ...
                "deterministic SEC event text fields only."
            ),
        },
        "decision_rationale": (
            "Accepted as default-off only."
            if accepted
            else "Rejected. The best variant raised aggregate EV/PnL but failed the no-regressed-window and concentration/sample guard."
        ),
        "rejection_reason": rejection_reason,
        "next_action": (
            "Do not retry simple SEC negative phrase-hit notional scalars on "
            "the frozen sample. Continue event work only with forward "
            "replacement-value evidence or a materially different event quality field."
        ),
        "why_not_other_attractive_points": (
            ...
            "rules; skipped governance-source retuning because exp-20260521-006 "
            "already accepted it."
        ),
        "risk_of_change": (
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {EXPERIMENT_ID} Event Negative Phrase Evidence",
        "",
        f"Decision: `{payload['decision']}`",
        "",
        (
            "Alpha search, replay-only. Tests whether selected SEC negative-"
            "reaction event rows with negative_phrase_hits >= 3 deserve extra "
            "paper notional on top of the accepted event governance-source adapter."
        ),
        "",
        "## Gate 4 Result",
        "",
...
def _compact_log(payload: dict[str, Any]) -> dict[str, Any]:
    def compact_metrics_by_window(rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
        fields = [
    ...
    def compact_windows(rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
        return OrderedDict(
            (
                label,
                {
    ...
    def compact_variant_gates(rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
        return OrderedDict(
            (
                name,
                {
    ...
    def concise_selection(selection: dict[str, Any]) -> dict[str, Any]:
        return {
            "target_source": selection["target_source"],
            "target_negative_phrase_hits_min": selection[
                "target_negative_phrase_hits_min"
            ],
    ...
    compact_parameters = {
        ...
        "target_source": payload["parameters"]["target_source"],
        "target_negative_phrase_hits_min": payload["parameters"][
            "target_negative_phrase_hits_min"
        ],
        "selected_negative_phrase_evidence_scalar": payload["parameters"][
            "selected_negative_phrase_evidence_scalar"
        ],
        "variant_scalars": {
            name: row["negative_phrase_evidence_scalar"]
    ...
    variant_gates = compact_variant_gates(
    ...
    compact = {
        ...
        "backtest_protocol": {
            ...
            "windows": compact_windows(payload["backtest_protocol"]["windows"]),
        ...
        "before_metrics": {
            BASELINE_VARIANT: compact_metrics_by_window(
                payload["before_metrics"][BASELINE_VARIANT]
            )
        },
        "after_metrics": {
            best: compact_metrics_by_window(payload["after_metrics"][best])
        },
        "delta_metrics": {
        ...
        "selection": concise_selection(payload["selection"][best]),
        "source_coverage": {
            "sec_negative_price_ready_candidates": payload["source_coverage"].get(
                "sec_negative_price_ready_candidates"
            ),
            "form4_price_ready_candidates": payload["source_coverage"].get(
                "form4_price_ready_candidates"
            ),
            "source_skipped_counts": payload["source_coverage"].get(
...
def persist(payload: dict[str, Any]) -> None:
    ...
    parent._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "Event negative phrase evidence",
            "status": payload["status"],
            "decision": payload["decision"],

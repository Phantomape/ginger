"""exp-20260617-013: deferred-tax valuation allowance release scout.

Replay-only alpha search. The single decision hypothesis is a PIT free SEC
Companyfacts candidate source: production-universe names whose deferred tax
asset valuation allowance burden is falling versus revenue may have improving
auditor-reviewed confidence in future taxable income. Liquid SPY-relative
leadership then tests whether price is starting to recognize that quality
before a next-open, 10-trading-day default-off paper continuation trade.
...
EXPERIMENT_ID = "exp-20260617-013"
STEM = "deferred_tax_valuation_allowance_release"
TRIAL_FAMILY = "deferred_tax_valuation_allowance_release_candidate_pool"
TRIAL_VARIANT_ID = "deferred_tax_valuation_allowance_release_top1_next_open_10d_v1"
CHANGED_VARIABLE = (
    "raw_sec_companyfacts_deferred_tax_valuation_allowance_release_candidate_source_v1"
)
...
OUT_JSON = OUT_DIR / f"exp_20260617_013_{STEM}.json"
...
MAX_ALLOWANCE_FACT_AGE_DAYS = 430
MIN_CURRENT_REVENUE = 250_000_000.0
MIN_CURRENT_ALLOWANCE = 10_000_000.0
MIN_PRIOR_ALLOWANCE = 15_000_000.0
MAX_CURRENT_ALLOWANCE_TO_REVENUE = 0.50
MIN_ALLOWANCE_RATIO_RELEASE = 0.005
MIN_ALLOWANCE_RELEASE_PCT = 0.15
MIN_REVENUE_GROWTH = -0.05
MIN_PRETAX_MARGIN = -0.08
MIN_PRETAX_MARGIN_IMPROVEMENT = -0.02
MIN_PERIOD_GAP_DAYS = 250
MAX_PERIOD_GAP_DAYS = 460

VALUATION_ALLOWANCE_TAGS = ("DeferredTaxAssetsValuationAllowance",)
REVENUE_TAGS = (
...
PREDICTION = {
    ...
    "main_failure_modes": [
        ...
        "valuation_allowance_release_not_incremental",
    ],
    "confidence_reason": (
        "Mechanism: a falling deferred-tax-asset valuation allowance is an "
        "auditor-reviewed accounting-quality field that can signal higher "
        "confidence in future taxable income. Related tax-burden and raw "
        "Companyfacts quality scouts were mostly rejected on old_thin/drawdown, "
        "so probability is low; this is materially different from effective tax "
        "rate, debt, impairment, lease, working-capital, and SBC threshold "
        "retunes."
    ),
    "recorded_at": "2026-06-17T11:06:05+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC valuation allowance, annual revenue, or pretax "
            "income facts, missing prior comparison point, stale facts, missing "
    ...
    "parity_note": (
        ...
        "SEC valuation allowance tag, filed-date PIT annual revenue/pretax "
        "context, liquid SPY-relative confirmation, cooldown, next-open paper "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts deferred-tax-asset valuation "
        "allowance burden falling versus revenue may identify companies where "
        "future taxable income confidence is improving; liquid SPY-relative "
        "leadership then tests whether the market is beginning to price that "
        "accounting quality before a 10-trading-day continuation leg."
    ),
    "2_history_check": {
        "exp-20260616-005": (
            "Rejected effective tax-burden relief. This run does not retune tax "
            "expense or effective tax-rate thresholds; it tests the deferred tax "
            "asset valuation allowance balance-sheet field."
        ),
        "exp-20260617-008": (
            "Rejected impairment overhang relief. This run tests future taxable "
            "income confidence, not goodwill/asset impairment overhang."
        ),
        "exp-20260616-029": (
            "Rejected principal debt burden relief. This run avoids financing "
            "liability burden and tests a tax asset valuation reserve."
        ),
        "exp-20260617-012": (
            "Rejected SBC grant-value backlog relief. This run is not SBC or "
    ...
    "5_reproducibility": (
        ...
        "exp_20260617_013_deferred_tax_valuation_allowance_release.py"
    ),
}
...
def _load_raw_companyfacts_index() -> tuple[dict[str, dict[str, list[dict[str, Any]]]], dict[str, Any]]:
    ...
    for ticker, cik in ticker_ciks.items():
        ...
        allowance_facts = _raw_instant_facts(usgaap, VALUATION_ALLOWANCE_TAGS)
        ...
        if not allowance_facts:
            stats["tickers_missing_raw_deferred_tax_valuation_allowance"] += 1
            continue
        ...
        stats["tickers_with_raw_allowance_revenue_pretax"] += 1
        stats["raw_deferred_tax_valuation_allowance_fact_count"] += len(allowance_facts)
    ...
    summary = {
        ...
        "valuation_allowance_tags": list(VALUATION_ALLOWANCE_TAGS),
...
def _allowance_release_observation(
    ticker: str,
    asof: str,
    facts: dict[str, list[dict[str, Any]]],
) -> dict[str, Any] | None:
    current = _latest_allowance_fact(facts["allowance"], asof=asof)
    if current is None:
        return None
    if base._days_between(asof, current["filed"]) > MAX_ALLOWANCE_FACT_AGE_DAYS:
        return None
    prior = _latest_allowance_fact(
        facts["allowance"], asof=asof, before_end=current["end"]
    )
    if prior is None:
        return None
    ...
    current_revenue = _latest_annual_fact_at_or_before(
        facts["revenue"], asof=asof, end_lte=current["end"]
    )
    prior_revenue = _latest_annual_fact_at_or_before(
        facts["revenue"], asof=asof, end_lte=prior["end"]
    )
    current_pretax = _latest_annual_fact_at_or_before(
        facts["pretax_income"], asof=asof, end_lte=current["end"]
    )
    prior_pretax = _latest_annual_fact_at_or_before(
        facts["pretax_income"], asof=asof, end_lte=prior["end"]
    )
    if (
        current_revenue is None
        or prior_revenue is None
        or current_pretax is None
        or prior_pretax is None
    ):
        return None

    current_allowance = abs(float(current["value"]))
    prior_allowance = abs(float(prior["value"]))
    ...
    if (
        current_allowance < MIN_CURRENT_ALLOWANCE
        or prior_allowance < MIN_PRIOR_ALLOWANCE
        or current_revenue_value < MIN_CURRENT_REVENUE
        or prior_revenue_value <= 0.0
    ):
        return None

    current_ratio = current_allowance / current_revenue_value
    prior_ratio = prior_allowance / prior_revenue_value
    ratio_release = prior_ratio - current_ratio
    release_pct = (prior_allowance - current_allowance) / prior_allowance
    revenue_growth = current_revenue_value / prior_revenue_value - 1.0
    current_pretax_margin = current_pretax_value / current_revenue_value
    prior_pretax_margin = prior_pretax_value / prior_revenue_value
    pretax_margin_improvement = current_pretax_margin - prior_pretax_margin

    if current_ratio > MAX_CURRENT_ALLOWANCE_TO_REVENUE:
        return None
    if ratio_release < MIN_ALLOWANCE_RATIO_RELEASE:
        return None
    if release_pct < MIN_ALLOWANCE_RELEASE_PCT:
        return None
    if revenue_growth < MIN_REVENUE_GROWTH:
        return None
    if current_pretax_margin < MIN_PRETAX_MARGIN:
        return None
    if pretax_margin_improvement < MIN_PRETAX_MARGIN_IMPROVEMENT:
        return None
    ...
    return {
        ...
        "current_allowance_filed": current["filed"],
        "prior_allowance_filed": prior["filed"],
        "current_allowance_tag": current["tag"],
        "current_allowance_value": _round(current_allowance, 2),
        "prior_allowance_value": _round(prior_allowance, 2),
        ...
        "current_allowance_to_revenue": _round(current_ratio, 6),
        "prior_allowance_to_revenue": _round(prior_ratio, 6),
        "allowance_ratio_release": _round(ratio_release, 6),
        "allowance_release_pct": _round(release_pct, 6),
        "revenue_growth": _round(revenue_growth, 6),
        "current_pretax_margin": _round(current_pretax_margin, 6),
        "prior_pretax_margin": _round(prior_pretax_margin, 6),
        "pretax_margin_improvement": _round(pretax_margin_improvement, 6),
    ...
    *,
    ...
    for signal_date in window_dates:
        for ticker in eligible:
            ...
            quality = _allowance_release_observation(ticker, signal_date, quality_index[ticker])
            if quality is None:
                scan["failed_allowance_release_gate"] += 1
                continue
            ...
            score = (
                1.25 * min(float(quality["allowance_release_pct"] or 0.0), 1.0)
                + 2.0 * min(float(quality["allowance_ratio_release"] or 0.0), 0.12)
                + 0.45 * max(float(quality["pretax_margin_improvement"] or 0.0), -0.05)
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "RAW_SEC_DEFERRED_TAX_VALUATION_ALLOWANCE_RELEASE_PAPER",
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["quality_allowance_release_pct"] or 0.0),
            -float(row["quality_allowance_ratio_release"] or 0.0),
    ...
    return rows, {
        **dict(scan),
        "rule_version": RULE_VERSION,
        "max_allowance_fact_age_days": MAX_ALLOWANCE_FACT_AGE_DAYS,
        "min_current_revenue": MIN_CURRENT_REVENUE,
        "min_current_allowance": MIN_CURRENT_ALLOWANCE,
        "min_prior_allowance": MIN_PRIOR_ALLOWANCE,
        "max_current_allowance_to_revenue": MAX_CURRENT_ALLOWANCE_TO_REVENUE,
        "min_allowance_ratio_release": MIN_ALLOWANCE_RATIO_RELEASE,
        "min_allowance_release_pct": MIN_ALLOWANCE_RELEASE_PCT,
        "min_revenue_growth": MIN_REVENUE_GROWTH,
        "min_pretax_margin": MIN_PRETAX_MARGIN,
        "min_pretax_margin_improvement": MIN_PRETAX_MARGIN_IMPROVEMENT,
    }
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_deferred_tax_valuation_allowance_release"
        if gate["passed"]
        else "rejected_deferred_tax_valuation_allowance_release_candidate_pool"
    )
    return gate
...
def _configure_base() -> None:
    ...
    base.MAX_ANNUAL_FACT_AGE_DAYS = MAX_ALLOWANCE_FACT_AGE_DAYS
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The deferred-tax valuation allowance release source cleared the "
            "numeric three-window replay screen, but remains only a replay lead "
            "because no shared daily/backtest helper was promoted."
        )
    else:
        interpretation = (
            "The deferred-tax valuation allowance release source did not clear "
    ...
    payload.update(
        {
            ...
            "mechanism_family": "production_visible_free_sec_companyfacts_tax_asset_quality_candidate_pool",
            "new_evidence_type": "raw_sec_companyfacts_deferred_tax_asset_valuation_allowance_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "DeferredTaxAssetsValuationAllowance is read as a raw SEC Companyfacts "
        "balance-sheet INSTANT fact and known only by filed date (<= signal "
        "date). Current and prior allowance/revenue ratios use annual revenue "
        "and pretax context filed by the signal date and period-ended no later "
        "than the allowance period. Price confirmation uses only signal-date "
    ...
    payload["gate2"]["runtime_fields"] = [
        "raw SEC companyfacts DeferredTaxAssetsValuationAllowance instant facts",
    ...
    payload["next_evidence_needed"] = (
        "A retry needs materially different PIT tax-asset evidence such as "
        "valuation allowance release paired with analyst revision breadth, "
        "cash-tax normalization, deferred-tax-asset composition, or closed "
        "forward replacement-value rows. Do not sweep allowance tag lists, "
        "ratio/release thresholds, pretax/revenue floors, annual fact freshness, "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping allowance tag lists, allowance/revenue "
            "thresholds, release percentage, pretax/revenue floors, fact "
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Deferred Tax Valuation Allowance Release",
            "",

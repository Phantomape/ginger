...
Read-only alpha search. This refresh tests whether newly matured forward
replacement-value rows since exp-20260622-013 make any default-off paper
sleeve/source family activation- or watchlist-ready.

No strategy, ranking, sizing, exit, order, LLM, paper ledger, or live behavior
is changed.

Reproduce:
    .venv/Scripts/python.exe -B quant/experiments/exp_20260626_014_forward_replacement_activation_readiness_20260626.py
"""
...
EXPERIMENT_ID = "exp-20260626-014"
LANE = "alpha_search"
OWNER = "alpha-explore"
STEM = "forward_replacement_activation_readiness_20260626"
RUNNER = f"quant/experiments/exp_20260626_014_{STEM}.py"
...
PRIOR_ARTIFACT = (
    ...
    / "experiments"
    / "exp-20260622-013"
    / "exp_20260622_013_forward_replacement_activation_readiness_20260622.json"
)
OUT_DIR = REPO_ROOT / "data" / "experiments" / EXPERIMENT_ID
OUT_JSON = OUT_DIR / f"exp_20260626_014_{STEM}.json"
...
HYPOTHESIS = (
    "Updated enriched forward replacement-value rows through 2026-06-25 may "
    "reveal whether any accepted default-off sleeve/source family is now "
    "activation- or watchlist-ready without retuning the envelope."
)
CHANGED_VARIABLE = "updated_forward_replacement_activation_readiness_20260626_v1"
MECHANISM_FAMILY = "production_visible_default_off_paper_adapter_for_candidate_pool_alpha"
TRIAL_FAMILY = "default_off_forward_replacement_value_activation_readiness"
TRIAL_VARIANT_ID = "current_artifact_20260626_update_v1"
NEARBY_PRIOR_EXPERIMENTS = ["exp-20260622-013"]
...
PRIOR_EXPERIMENT_DATE = "2026-06-22"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "exp-20260622-013 had 33 enriched rows and no ready family; current "
        "ledger has 40 rows with seven rows at or after the 2026-06-22 exit "
        "date boundary, a true forward maturation axis but probably still too "
        "thin."
    ),
    "recorded_at": "2026-06-26T13:09:58+00:00",
}
...
PRE_RUN_QUESTIONS = {
    ...
    "2_history_check": {
        "exp-20260622-013": (
            "Rejected after comparator repair: 33 enriched closed rows, no "
            "activation-ready or 20-row watchlist-ready sleeve/source family."
        ),
        "novelty_gate": (
            "Reservation passed without override; new evidence type is newly "
            "closed forward rows through 2026-06-25."
        ),
    },
    "3_single_decision_hypothesis": CHANGED_VARIABLE,
    "4_acceptance_standard": (
        "A sleeve/source family is activation-envelope-ready only if it has "
        "at least 60 closed rows, positive aggregate replacement value versus "
        "cash/SPY/QQQ, win rate versus each comparator at least 50%, max single "
        "positive ticker share at most 50%, and rows spanning at least 20 "
        "calendar days. A 20-row tier is only a watchlist lead, not activation."
    ),
...
PRODUCTION_IMPACT = {
    ...
    "live_ready": False,
    "parity_note": (
        "Read-only forward readiness refresh. It consumes the current "
        "replacement artifact and does not change helper logic, snapshots, "
        "orders, ranking, sizing, exits, LLM/news, or live/default behavior."
    ),
}
...
def _data_freshness(rows: list[dict[str, Any]]) -> dict[str, Any]:
    prior = _load_json(PRIOR_ARTIFACT)
    prior_rows = (
        ((prior.get("gate4") or {}).get("total_forward_rows"))
        or ((prior.get("after_metrics") or {}).get("artifact_rows"))
        or ((prior.get("before_metrics") or {}).get("artifact_rows"))
        or 0
    )
    ...
    exit_ge_prior = [row for row in rows if str(row.get("exit_date") or "") >= PRIOR_EXPERIMENT_DATE]
    exit_after_prior = [row for row in rows if str(row.get("exit_date") or "") > PRIOR_EXPERIMENT_DATE]
    return {
        "prior_artifact": _repo_rel(PRIOR_ARTIFACT),
        "prior_experiment_id": "exp-20260622-013",
        "prior_total_forward_rows": int(prior_rows or 0),
        "current_total_forward_rows": len(rows),
        "row_delta_vs_prior": len(rows) - int(prior_rows or 0),
        ...
        "rows_with_exit_date_ge_prior_experiment_date": len(exit_ge_prior),
        "rows_with_exit_date_after_prior_experiment_date": len(exit_after_prior),
        "newer_exit_rows": [
            {
                "ticker": row.get("ticker"),
                "sleeve_key": row.get("sleeve_key"),
            ...
            for row in sorted(exit_ge_prior, key=lambda item: (str(item.get("exit_date")), str(item.get("ticker"))))
        ],
    }
...
def _overall_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary["rows_by_source"] = dict(Counter(_source_key(row) for row in rows))
    return summary
...
def _gate4(
    ...
    if freshness["row_delta_vs_prior"] <= 0:
        failed.append("no_new_closed_forward_rows_since_prior_audit")

    return {
        "passed": not failed,
        "decision": (
            "accepted_activation_envelope_candidate_after_forward_maturation"
            if not failed
            else "observed_only_rejected_no_forward_activation_ready_20260626_update"
        ),
...
def _calibration(gate4: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "surprise_note": (
            "Low surprise. The forward ledger gained rows, but the largest "
            "sleeve is still below the 20-row watchlist tier and far below the "
            "60-row activation tier."
            if failures
            else "The refreshed forward artifact produced an activation-envelope candidate."
        ),
    }
...
def _build_payload() -> dict[str, Any]:
    ...
    freshness = _data_freshness(rows)
    sleeve_summaries = _summaries_by(rows, "sleeve_key")
    source_summaries = _source_summaries(rows)
    gate4 = _gate4(rows, sleeve_summaries, source_summaries, artifact_audit, freshness)
    status = "accepted_paper_pending_activation_envelope" if gate4["passed"] else "observed_only_rejected"
    ...
    return {
        ...
        "change_summary": (
            "Refresh the forward replacement-value activation readiness audit "
            "using newly closed rows through 2026-06-25."
        ),
        ...
        "causal_components": [
            "forward replacement ledger audit",
            "source-family readiness gates",
            "new closed-row maturity delta",
        ...
        "gate2": {
            "passed": artifact_audit["missing_comparator_bar_rows"] == 0 and bool(rows),
        ...
        "before_metrics": {
            ...
            "artifact_rows": freshness["prior_total_forward_rows"],
            "activation_ready_groups": 0,
            "watchlist_ready_groups": 0,
        },
        "after_metrics": {
            "aggregate_expected_value_score": baseline["aggregate_expected_value_score"],
            "aggregate_total_pnl": baseline["aggregate_total_pnl"],
        ...
        "delta_metrics": {
            ...
            "artifact_rows": freshness["row_delta_vs_prior"],
            "activation_ready_groups": len(gate4["activation_candidates"]),
            "watchlist_ready_groups": len(gate4["watchlist_candidates"]),
        },
        ...
        "post_run_reflection": {
            "why_result_happened": (
                "The current forward artifact has 40 enriched rows, up from 33 "
                "in exp-20260622-013. The extra maturity did not change the "
                "readiness conclusion: low_deployment_etf is still only 17 rows, "
                "fundamental_growth_rs is 7 rows, and no sleeve/source family "
                "has 20 diversified rows, let alone 60 activation rows."
            ),
            "forbidden_near_neighbor_retry": (
                "Do not rerun forward activation readiness, relax the 20/60-row "
                "minimums, or retune low_deployment/state_surface/fundamental "
                "thresholds until materially more closed replacement-value rows "
                "arrive. A useful retry needs new rows, not another adjacent "
                "condition on the same 40-row surface."
            ),
            "new_evidence_required": (
                "At least 20 closed rows for one diversified helper/source "
                "family to justify a watchlist lead, and at least 60 closed rows "
                "with positive replacement value versus cash, SPY, and QQQ for "
                "an activation-envelope experiment."
            ),
        },
        "next_retry_requires": [
            "materially more enriched closed forward rows",
            "one diversified sleeve/source family with at least 20 closed rows for watchlist status",
            "no relaxation of 20-row watchlist or 60-row activation gates",
        ],
...
def _build_log_record(payload: dict[str, Any]) -> dict[str, Any]:
    freshness = payload["gate4"]["freshness"]
    return {
        "experiment_id": EXPERIMENT_ID,
        ...
        "date_range": {
            "start": freshness["entry_date_min"],
            "end": freshness["exit_date_max"],
        },
        ...
        "gate4": payload["gate4"],
        "decision_basis": (
            "Observed-only rejected because the forward artifact gained rows "
            "but still has no 20-row watchlist-ready sleeve/source family and "
            "no 60-row activation-ready family."
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID}: forward replacement activation readiness refresh",
            "",
            f"- Status: `{payload['status']}`",
            f"- Decision: `{payload['decision']}`",
            "- Production orders changed: no",
            "- Trade enabled: false",
            f"- Rows: `{freshness['current_total_forward_rows']}` current vs `{freshness['prior_total_forward_rows']}` prior",
            f"- Rows with exit_date >= 2026-06-22: `{freshness['rows_with_exit_date_ge_prior_experiment_date']}`",
            "",
            "## Hypothesis",
            "",
            payload["hypothesis"],
            ...
            *rows,
            ...
            "- Watchlist candidates: `{}`".format(len(payload["gate4"]["watchlist_candidates"])),
            "",
            "## Boundary",
            "",
        ...
        json.dumps(
            {
                ...
                "row_delta_vs_prior": payload["gate4"]["freshness"]["row_delta_vs_prior"],
                "exit_ge_2026_06_22": payload["gate4"]["freshness"][

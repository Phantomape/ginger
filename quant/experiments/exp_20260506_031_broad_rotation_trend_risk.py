...
Alpha search. Test one capital-allocation variable surfaced by the
meta-allocation state map: `trend_long` trades were positive in all three
canonical windows when IWM's 20-day return beat SPY's by more than 2pp. This
script keeps entries, exits, ranking, universe, LLM/news replay, and all
existing cohort rules fixed, then applies a trend-only risk multiplier on those
broad-rotation candidate days.
"""
...
EXPERIMENT_ID = "exp-20260506-031"
STEM = "broad_rotation_trend_risk"
...
CUSTOM_MULTIPLIER_KEY = "broad_rotation_trend_risk_multiplier_applied"
...
VARIANTS = OrderedDict(
    [
        ("broad_rotation_trend_1_25x", {"risk_multiplier": 1.25}),
        ("broad_rotation_trend_1_50x", {"risk_multiplier": 1.50}),
        ("broad_rotation_trend_2_00x", {"risk_multiplier": 2.00}),
    ]
)

_state: dict[str, Any] = {
    "trend_signals_seen": 0,
    "broad_rotation_trend_signals_seen": 0,
    "signals_resized": 0,
...
def _patch_size_signals(variant: dict[str, float] | None):
    ...
    def patched(signals, portfolio_value, risk_pct=None):
        ...
        for sig in sized:
            ...
            _state["broad_rotation_trend_signals_seen"] += 1
            ...
            preserved["broad_rotation_trend_original_risk_pct"] = original_risk_pct
            preserved["broad_rotation_trend_original_shares"] = sizing.get(
                "shares_to_buy"
            )
...
def _metrics(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "broad_rotation_trend_signals_seen": _state[
            "broad_rotation_trend_signals_seen"
        ],
...
def _markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID}: Broad-Rotation Trend Risk",
        "",
...
def main() -> int:
    ...
    if any_pass:
        interpretation = (
            ...
            "market-state feature and shared sizing policy used by both run.py and the backtester."
        )
    else:
        interpretation = (
            "Broad-rotation trend risk expansion did not pass the three-window "
            ...
            f"{best['aggregate']['total_pnl_delta_sum']}, but the stronger variants "
            "paid for the PnL with worse mid_weak drawdown and lower Sharpe. This "
            "does not justify a new state-aware trend sizing branch."
        )
    ...
    payload: dict[str, Any] = {
        ...
        "timestamp": timestamp,
        "lane": "alpha_search",
        "change_type": "capital_allocation_broad_rotation_trend_risk",
        "alpha_hypothesis_category": "capital_allocation_meta_routing",
        "hypothesis": (
            "If small-cap breadth leadership confirms broad market participation, "
            "then increasing risk only for existing trend_long candidates on days "
            "when IWM 20-day return exceeds SPY by more than 2pp should improve "
            "expected value without adding new tickers or filters."
        ),
        ...
        "history_guardrails": {
            ...
            "why_not_simple_repeat": (
                "This tests trend_long broad-rotation allocation from the "
                "exp-20260506-024 state map. It does not touch breakout_long, "
                "SPY-relative leader sizing, candidate ranking, or static universe growth."
            ),
        },
        "parameters": {
            "broad_rotation_definition": (
                "IWM 20-day return minus SPY 20-day return greater than 0.02"
            ),
            "broad_rotation_iwm_minus_spy_20d_min": (
                BROAD_ROTATION_IWM_MINUS_SPY_20D_MIN
            ),
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, compute broad-rotation state in shared production/backtest "
                "enrichment and consume it from shared sizing."
            ),
        },
        "ticket": {
            "experiment_id": EXPERIMENT_ID,
            "title": "Broad-rotation trend risk allocation",
            "decision": decision,
            "best_variant": best_variant_name,
            "next_action": (
        ...
        "related_files": [
            ...
            "quant/experiments/exp_20260506_031_broad_rotation_trend_risk.py",
        ],
    }
    ...
    payload["experiment_log_entry"] = {
        ...
        "next_retry_requires": [
            "Do not retry nearby broad-rotation trend-only multipliers at the same 2pp IWM-vs-SPY threshold.",
            "A valid retry needs a new discriminator that keeps the PnL lift without the mid_weak drawdown/Sharpe damage.",
            "If promoted later, implement the broad-rotation field and sizing branch through shared production/backtest policy.",
        ],

"""exp-20260514-046: fresh RS20 non-RS60 leader risk allocation.

Tests one production-visible allocation variable on the accepted core stack:
already-qualified trend/breakout signals that are entry-state RS20 leaders but
not RS60 top-quintile leaders may represent fresher rotations than persistent
momentum names. This is not an entry filter, ranking change, exit change,
target change, candidate-pool expansion, LLM/news change, Space sleeve change,
or portfolio heat change.
"""
...
EXPERIMENT_ID = "exp-20260514-046"
EXPERIMENT_SLUG = "fresh_rs20_non_rs60_risk"
STATE_KEY = "fresh_rs20_non_rs60_leader_state"
MULTIPLIER_KEY = "fresh_rs20_non_rs60_risk_multiplier_applied"
    ...
    def wrapped(
        ...
        for sig in enriched:
            sig[STATE_KEY] = bool(
                ...
                and sig.get("rs60_top_quintile_state") is False
            )
        return enriched
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["fresh_rs20_non_rs60_baseline_shares"] = shares
    out["fresh_rs20_non_rs60_desired_shares"] = desired_shares
    out["fresh_rs20_non_rs60_cap_shares"] = cap_shares
    out["fresh_rs20_non_rs60_new_shares"] = new_shares
                    ...
                    base.ADJUSTMENTS.append(
                        {
                            ...
                            "cap_shares": adjusted_sizing.get(
                                "fresh_rs20_non_rs60_cap_shares"
                            ),
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    for label in base.WINDOWS:
        ...
        window_rows.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {ddd:+.4f} | {dworst:+.4f} | {dtail:+.4f} | {surv:.4f} | {adj} |".format(
                label=label,
                ...
                dworst=delta.get("worst_trade_pct", 0.0),
                dtail=delta.get("tail_loss_share", 0.0),
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} Fresh RS20 Non-RS60 Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    decision = (
        ...
        if passed
        else "rejected_fresh_rs20_non_rs60_risk"
    )
    interpretation = (
        "The fresh RS20/non-RS60 rotation state cleared the canonical three-window gate and should be promoted only through shared production/backtest sizing policy."
        if passed
        else "The fresh RS20/non-RS60 rotation state did not clear Gate 4 as an additional standalone sizing top-up after the accepted RS20 layer."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "The accepted RS20 allocation state may contain a fresher rotation sub-sleeve: "
            "signals that are already 20-day entry-state leaders but not yet RS60 "
            "top-quintile leaders may be under-allocated because they capture earlier "
            "rotation before the broader 60-day momentum state is confirmed."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "fresh_rs20_non_rs60_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for signals where rs20_entry_state_leader is true and rs60_top_quintile_state is false"
        ),
        "parameters": {
            "state_definition": {
                "strategies": ["trend_long", "breakout_long"],
                "requires_rs20_entry_state_leader": True,
                "requires_rs60_top_quintile_state": False,
                "applied_after_existing_sizing_layers": True,
            },
        ...
        "historical_experiment_check": {
            "similar_prior_results": {
                "exp-20260510-010": (
                    "RS20 entry-state leadership was promoted as a broad cap-aware top-up; this run does not retune that scalar and tests only the non-RS60 fresh-rotation sub-sleeve."
                ),
                "exp-20260513-030": (
                    "RS60 top-quintile momentum was promoted as a broad top-up; this run explicitly excludes that state."
                ),
                "exp-20260510-014": (
                    "RS60 non-RS20 was rejected; this run tests the opposite early-rotation state, not medium-term leadership without entry-state strength."
                ),
                "exp-20260514-014": (
                    "Dual RS persistent leader top-up was rejected; this run excludes persistent RS60 leaders instead of stacking on them."
                ),
                "exp-20260514-045": (
                    "Triple RS20/RS60/green cap expansion failed; this run is not a cap-room retry and removes RS60 persistence."
                ),
                "exp-20260514-012": (
                    "Broad non-Financials sector-relative leadership regressed late_strong; this run uses cross-horizon ticker strength, not sector-relative strength."
                ),
            },
            "why_not_llm_or_space": (
                "LLM soft-ranking, SEC/Form4 semantic overlays, and Space sleeve refinements are currently forward-sample or field limited; this run uses deterministic replayable fields already present in production and backtest paths."
            ),
        },
        "gate_questions": {
            "1_alpha_hypothesis": (
                "capital/risk allocation: fresh RS20 leaders that are not RS60 top-quintile may monetize earlier rotation before persistent momentum becomes crowded."
            ),
            "2_history_check": (
                "Opposite and nearby RS-family tests were checked: RS60 non-RS20, dual RS persistent, triple RS20/RS60/green cap, broad RS20 cap, and broad non-Financials sector-relative leadership. This run changes the segmentation state only."
            ),
            "3_single_causal_variable": (
                "fresh_rs20_non_rs60_risk_multiplier sweep only."
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260514_046_fresh_rs20_non_rs60_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        "production_impact": {
            ...
            "promotion_requirement": (
                "If accepted, implement the multiplier in shared portfolio_engine.py and add the attribution key for both production and backtest paths."
            ),
        },
        ...
        if passed
            ...
            "Do not retry RS20/RS60 interaction top-ups without new forward attribution or a materially different production-visible discriminator."
        ),
        "related_files": [

...
Tests one allocation variable on top of accepted exp-20260516-023: whether
source-diverse official Space trend signals with both customer and government
catalysts deserve an incremental scalar when the production registry event
guard profile contains financing or dilution sensitivity.
...
import exp_20260516_023_space_dual_catalyst_near_perfect_trend_risk as prev


LOGGER = logging.getLogger(__name__)

# exp023 imports exp019 as `prev`; expose the same accepted-stack base module
# under the exp023 module so this experiment can mirror the accepted patch order.
prev.base = prev.prev.base

EXPERIMENT_ID = "exp-20260516-024"
STEM = "space_dual_catalyst_financing_profile_trend_risk"
BEFORE_EXPERIMENT_ID = "exp-20260516-023"
BEFORE_STEM = "space_dual_catalyst_near_perfect_trend_risk"
...
ACCEPTED_DUAL_CATALYST_NEAR_PERFECT_TREND_SCALAR = 1.0125
ACCEPTED_DUAL_CATALYST_IWM_LEADER_TREND_SCALAR = 1.0125
ACCEPTED_DUAL_CATALYST_SAME_THEME_WINNER_TREND_SCALAR = 1.0125
...
def _is_financing_dual_signal(
    signal: dict[str, Any],
    profile: dict[str, Any] | None,
    financing_tickers: set[str],
) -> bool:
    ticker = str(signal.get("ticker") or "").upper()
    return (
        str(signal.get("strategy") or "") == TARGET_STRATEGY
        and ticker in financing_tickers
    ...
    *,
    ...
    def patched_scale_and_record(
        *,
        ...
        if prev._is_near_perfect_dual_signal(signal, profile):
            prev.base._apply_scale(
                signal=signal,
                sizing=sizing,
        ...
        if not _is_financing_dual_signal(signal, profile, financing_tickers):
            return
        prev.base._apply_scale(
            signal=signal,
            sizing=sizing,
            scalar=financing_scalar,
            portfolio_value=portfolio_value,
            marker=MARKER,
            counts=counts,
    ...
    try:
        variant = prev.base.exp021._run_exp051_stack_variant(
            f"{STEM}_{str(financing_scalar).replace('.', '_')}",
    ...
    variant["parameters"] = {
        **variant["parameters"],
        ...
        "space_dual_catalyst_financing_profile_trend_scalar": financing_scalar,
        "target_strategy": TARGET_STRATEGY,
        "target_profile_gate": "financing_gate",
        "target_tickers": sorted(financing_tickers),
    }
    summarizer = prev.base.exp041.source_diversity_exp._adjustment_summary
    ...
    variant["dual_catalyst_financing_profile_counts"] = {
        key: value for key, value in sorted(counts.items()) if MARKER in key
    }
    variant["dual_catalyst_financing_profile_counts_by_window"] = by_window_counts
    variant["dual_catalyst_financing_profile_adjustment_summary"] = summarizer(
        dual_financing_adjustments
    )
    variant["dual_catalyst_financing_profile_adjustment_sample"] = (
        dual_financing_adjustments[:25]
    )
    return variant
...
def _gate_variant(variant: dict[str, Any], before: dict[str, Any]) -> dict[str, Any]:
    ...
    counts = variant.get("dual_catalyst_financing_profile_counts") or {}
    by_window_counts = (
        variant.get("dual_catalyst_financing_profile_counts_by_window") or {}
    )
    ...
    scalar = float(
        variant["parameters"]["space_dual_catalyst_financing_profile_trend_scalar"]
    )
    passed = bool(
        scalar != 1.0
        and changed_count > 0
    ...
    return {
        ...
        "eligible_dual_financing_signal_count": eligible_count,
        "changed_dual_financing_signal_count": changed_count,
...
def _experiment_record(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    selected_scalar = best["parameters"][
        "space_dual_catalyst_financing_profile_trend_scalar"
    ]
    return {
        ...
        "changed_variable": "space_dual_catalyst_financing_profile_trend_scalar",
        "parameters": {
        ...
        "gate_answers": {
            ...
            "2_prior_similar_experiments": [
                "exp-20260512-041 accepted broad Space financing/dilution event-guard profile risk.",
                "exp-20260516-014/015/019/023 accepted dual-catalyst, IWM-leader, same-theme-winner, and near-perfect Space trend allocation states.",
                "exp-20260516-017/018 rejected peer-state refinements with zero incremental EV/PnL.",
                "No prior run isolated financing/dilution registry evidence inside the accepted dual-catalyst sleeve after exp-20260516-023.",
            ],
            "3_single_causal_variable": (
                "Only the incremental dual-catalyst financing-profile trend "
            ...
            "5_reproducibility": (
                ...
                "exp_20260516_024_space_dual_catalyst_financing_profile_trend_risk.py"
            ),
        },
        "gate_results": gate,
        ...
        if promoted
            ...
            "Gate 4 failed: the dual-catalyst financing-profile scalar did not "
            "satisfy the fixed three-window EV/PnL, drawdown, and cohort guards."
        ),
        "next_evidence_needed": None
        if promoted
        else (
            "Do not retry nearby dual-catalyst financing-profile scalars on "
        ...
        "why_not_other_changes": (
            ...
            "zero incremental EV/PnL, exact perfect-TQS dual-catalyst rows were "
            "sample-thin in one window, and stronger near-perfect/same-theme "
            "scalars already hit drawdown. This keeps the fixed Space pool and "
            "tests one production-visible registry event-guard interaction."
        ),
        "related_files": [
            "quant/experiments/exp_20260516_024_space_dual_catalyst_financing_profile_trend_risk.py",
            "data/experiments/exp-20260516-024/space_dual_catalyst_financing_profile_trend_risk.json",
            "docs/experiments/logs/exp-20260516-024.json",
            "docs/experiments/tickets/exp-20260516-024.json",
            "docs/experiments/artifacts/exp-20260516-024_space_dual_catalyst_financing_profile_trend_risk.md",
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    ...
    promoted = payload["decision"] == "accept"
    lines = [
        f"# {EXPERIMENT_ID} Space dual-catalyst financing-profile trend risk",
        "",
        "## Hypothesis",
        payload["hypothesis"],
        "",
        ...
        (
            "`space_dual_catalyst_financing_profile_trend_scalar` on top of "
        ...
        f"- financing profile field check passed: `{payload['financing_field_check']['passed']}`",
        f"- target tickers: `{payload['financing_field_check'].get('target_tickers')}`",
        "",
    ...
    lines.extend(
        [
            "",
            "## Best Variant",
            f"- scalar: `{best['parameters']['space_dual_catalyst_financing_profile_trend_scalar']}`",
            f"- eligible signals: `{gate['eligible_dual_financing_signal_count']}`",
            f"- changed signals: `{gate['changed_dual_financing_signal_count']}`",
...
def _ticket(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            "Dual-catalyst financing-profile Space scalar "
            f"{best['parameters']['space_dual_catalyst_financing_profile_trend_scalar']} "
            f"changed {gate['changed_dual_financing_signal_count']} "
...
def _financing_field_check(gates: dict[str, Any]) -> dict[str, Any]:
    financing_gate = gates["financing_gate"]
    source_profiles = gates["source_diversity_gate"]["profiles"]
    financing_tickers = set(financing_gate.get("target_tickers") or [])
    target_tickers = [
        ticker
        for ticker, profile in source_profiles.items()
        if ticker in financing_tickers
        and prev.base.exp014._is_dual_catalyst_profile(profile)
    ]
    return {
        "passed": bool(financing_gate.get("passed")) and bool(target_tickers),
        "financing_gate_passed": bool(financing_gate.get("passed")),
        "required_signal_fields": [
            "strategy",
            "ticker",
            "space_source_diversity_profile.event_fields",
            "registry event_guard_profile",
        ],
        "target_profile_gate": "financing_gate",
        "target_tickers": sorted(target_tickers),
        "all_financing_tickers": sorted(financing_tickers),
    }
...
def run() -> dict[str, Any]:
    ...
    financing_field_check = _financing_field_check(gates)
    ...
    if not financing_field_check["passed"]:
        raise RuntimeError(
            f"Financing profile field check failed: {financing_field_check}"
        )

    variants = [
        _run_variant(financing_scalar=scalar, gates=gates) for scalar in SCALARS
    ]
    before = next(
        variant
    ...
    payload = {
        ...
        "financing_field_check": financing_field_check,
        "variants": variants,
        ...
        "hypothesis": (
            ...
            "signals may still be mis-sized when the production registry "
            "event_guard_profile contains financing or dilution sensitivity; "
            "the customer-plus-government catalyst stack may change the payoff "
            "distribution of that already accepted Space financing state."
        ),
        "changed_variable": "space_dual_catalyst_financing_profile_trend_scalar",
    }
            ...
            _safe(
                {
                    ...
                    "eligible_signals": gate[
                        "eligible_dual_financing_signal_count"
                    ],
                    "changed_signals": gate[
                        "changed_dual_financing_signal_count"
                    ],

...
queue-rank 2 candidates with own volume confirmation receive a bounded
...
import exp_20260519_020_state_surface_rank4_volume_confirmation_notional as prev


EXPERIMENT_ID = "exp-20260519-021"
EXPERIMENT_SLUG = "state_surface_rank2_volume_confirmation_notional"
...
RULE_VERSION = "state_surface_rank2_volume_confirmation_notional_v1"

DOC_HELPERS = prev.DOC_HELPERS
CORE_HELPERS = prev.CORE_HELPERS
CONCENTRATION_HELPERS = prev.CONCENTRATION_HELPERS

RANK2_VOLUME_CONFIRMATION_VARIANTS: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
        ...
        (
            "rank2_volume_ge_100_scalar_110",
            {
                "volume_min": 1.00,
                "scalar": 1.10,
                "target_queue_rank": 2,
                "aggression_order": 1,
                "description": "rank 2 volume ratio >= 1.00 receives 10% support",
            },
        ),
        (
            "rank2_volume_ge_100_scalar_125",
            {
                "volume_min": 1.00,
                "scalar": 1.25,
                "target_queue_rank": 2,
                "aggression_order": 2,
                "description": "rank 2 volume ratio >= 1.00 receives 25% support",
            },
        ),
        (
            "rank2_volume_ge_100_scalar_150",
            {
                "volume_min": 1.00,
                "scalar": 1.50,
                "target_queue_rank": 2,
                "aggression_order": 3,
                "description": "rank 2 volume ratio >= 1.00 receives 50% support",
            },
        ),
        (
            "rank2_volume_ge_110_scalar_110",
            {
                "volume_min": 1.10,
                "scalar": 1.10,
                "target_queue_rank": 2,
                "aggression_order": 4,
                "description": "rank 2 volume ratio >= 1.10 receives 10% support",
            },
        ),
        (
            "rank2_volume_ge_110_scalar_125",
            {
                "volume_min": 1.10,
                "scalar": 1.25,
                "target_queue_rank": 2,
                "aggression_order": 5,
                "description": "rank 2 volume ratio >= 1.10 receives 25% support",
            },
        ),
        (
            "rank2_volume_ge_110_scalar_150",
            {
                "volume_min": 1.10,
                "scalar": 1.50,
                "target_queue_rank": 2,
                "aggression_order": 6,
                "description": "rank 2 volume ratio >= 1.10 receives 50% support",
            },
        ),
        (
            "rank2_volume_ge_120_scalar_125",
            {
                "volume_min": 1.20,
                "scalar": 1.25,
                "target_queue_rank": 2,
                "aggression_order": 7,
                "description": "rank 2 volume ratio >= 1.20 receives 25% support",
            },
        ),
        (
            "rank2_volume_ge_120_scalar_150",
            {
                "volume_min": 1.20,
                "scalar": 1.50,
                "target_queue_rank": 2,
                "aggression_order": 8,
                "description": "rank 2 volume ratio >= 1.20 receives 50% support",
            },
        ),
    ]
)
...
def _apply_rank2_volume_confirmation(
    ...
    *,
    ...
    target_rank = int(variant.get("target_queue_rank") or 2)
    ...
    for trade in trades:
        ...
        row["rank2_volume_confirmation_variant"] = variant_name
        row["rank2_volume_confirmation_rule_version"] = RULE_VERSION
        row["rank2_volume_confirmation_volume_ratio_20"] = volume_ratio
        row["rank2_volume_confirmation_target_queue_rank"] = target_rank
        row["rank2_volume_confirmation_min"] = threshold
        row["rank2_volume_confirmation_configured_scalar"] = scalar
        row["rank2_volume_confirmation_scalar"] = scalar if applies else None
        row["rank2_volume_confirmation_applied"] = bool(applies)
        row["rank2_volume_confirmation_profile_name"] = (
        ...
        row["rank2_volume_confirmation_base_multiplier"] = _float(
            row.get("rank_notional_multiplier")
        )
        row["rank_notional_rank2_volume_confirmation_rule_version"] = RULE_VERSION
        ...
        if applies:
            ...
            row["rank2_volume_confirmation_base_notional"] = base_notional
        ...
        rows.append(
            {
                ...
                "rank2_volume_confirmation_min": trade.get(
                    "rank2_volume_confirmation_min"
                ),
                "rank2_volume_confirmation_scalar": trade.get(
                    "rank2_volume_confirmation_scalar"
                ),
                "rank2_volume_confirmation_profile_name": trade.get(
...
def _variant_payload(
    *,
    ...
    selected = _apply_rank2_volume_confirmation(
        baseline_trades,
    ...
    for label in WINDOWS:
        ...
        applied = [
            row for row in adjusted if row.get("rank2_volume_confirmation_applied")
        ]
        surface_sleeve[label] = {
            "selected_trade_count": len(adjusted),
            "rank2_volume_confirmation_adjusted_trade_count": len(applied),
            "rank2_volume_confirmation_adjusted_pnl": round(
    ...
    applied_all = [
        row for row in selected if row.get("rank2_volume_confirmation_applied")
    ]
    applied_windows = {str(row.get("window")) for row in applied_all if row.get("window")}
    return {
        "variant_name": variant_name,
        "variant_type": "rank2_volume_confirmation_notional_profile",
        ...
        "rank2_volume_confirmation_adjusted_trade_count": len(applied_all),
        "rank2_volume_confirmation_adjusted_windows": sorted(applied_windows),
...
def _gate4_for_variant(
    *,
    ...
    adjusted_guard_passed = (
        variant["rank2_volume_confirmation_adjusted_trade_count"]
        >= MIN_ADJUSTED_TRADES
        and len(variant["rank2_volume_confirmation_adjusted_windows"])
    ...
    return {
        ...
        "rank2_volume_confirmation_adjusted_trade_count": variant[
            "rank2_volume_confirmation_adjusted_trade_count"
        ],
        "rank2_volume_confirmation_adjusted_windows": variant[
            "rank2_volume_confirmation_adjusted_windows"
        ],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rank-2 Volume Confirmation Notional",
        "",
    ...
    for label in WINDOWS:
        ...
        lines.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {bdd:.2%} | {add:.2%} | {trades} |".format(
                ...
                trades=sleeve["rank2_volume_confirmation_adjusted_trade_count"],
            )
        )
    lines.extend(
        [
...
def build_payload() -> dict[str, Any]:
    ...
    baseline_trades = prev._accepted_rank3_volume_trades(
        core_results=core_results,
        prices=prices,
    )
    ...
    variants = [
        ...
        for variant_name, variant in RANK2_VOLUME_CONFIRMATION_VARIANTS.items()
    ]
    sweep_summary = []
    ...
    decision = (
        "accepted_default_off_state_surface_rank2_volume_confirmation_notional"
        if passed
        else "rejected_state_surface_rank2_volume_confirmation_notional"
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": "Rank-2 state-surface candidates with volume confirmation are high-participation continuation alternatives and deserve bounded default-off paper-notional support.",
        "alpha_hypothesis": {
            "category": "capital allocation",
            "entry_exit_ranking_or_allocation": "default-off paper allocation",
            "playbook_alignment": "Continues state-surface maturation with an OHLCV-confirmed rank-depth field while avoiding rank4 no-sample, LLM soft-ranking, and candidate-pool expansion.",
        },
        "change_type": "default_off_paper_allocation",
        "changed_variable": "rank2_volume_confirmation_notional_profile",
        ...
        "parameters": {
            ...
            "profile_priority": "applies after accepted rank-3 volume confirmation; applies only to queue_rank == 2",
            "locked_variables": [
                "core entries",
                "core exits",
                "core sizing",
        ...
        "history_check": {
            "exp-20260519-015": "Accepted rank-3 volume confirmation; this experiment freezes it and tests rank-2 own volume confirmation only.",
            "exp-20260519-020": "Rejected rank-4 volume because closed paper entries had no rank-4 sample; this switches to rank 2 where coverage exists.",
            "anti_repeat": "Not a rank3 threshold retry, rank4 no-sample retry, sector-diversity retry, core-misfit expansion, AI-infra cached pool, or LLM soft-ranking experiment.",
        },
        "llm_metrics": {
            "used_llm": False,
        ...
        "interpretation": (
            "Rank-2 volume confirmation improved the default-off state-surface paper overlay in all three windows while staying paper-only and production-visible."
            if passed
            else "Rank-2 volume confirmation did not clear Gate 4; keep the accepted rank3-volume stack unchanged."
        ),
        ...
        "next_evidence_needed": (
            ...
            if passed
            else "Do not retry nearby rank2 volume profiles without forward evidence or a distinct quality field."
        ),
        "protocol_answers": {
            "1_alpha_hypothesis": "capital allocation: queue_rank == 2 candidates with volume_ratio_20 confirmation indicate high participation and deserve bounded paper-notional support.",
            "2_history_check": "Prior accepted work covered rank2 near-high and rank3 volume confirmation; rank4 volume had no closed-sample coverage, so this tests a distinct rank2 OHLCV confirmation field with three-window coverage.",
            "3_single_causal_variable": "rank2_volume_confirmation_notional_profile",
            "4_acceptance_standard": "docs/backtesting.md three fixed windows; positive aggregate EV/PnL, all three windows EV-positive, zero EV-regressed windows, adjusted trades >=6 across all 3 windows, max DD drift <=0.5pp, single-ticker positive share <=50%.",
...
def main() -> None:
    ...
    DOC_HELPERS._write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rank-2 volume confirmation notional",
            "status": payload["status"],
            "decision": payload["decision"],
            "artifact": DOC_HELPERS._repo_rel(OUT_JSON),
            "summary": (
                f"Rank-2 volume profile {payload['parameters']['best_variant']} "

"""exp-20260513-030: RS60 top-quintile core risk allocation.
...
return is in the top quintile of the non-ETF/non-commodity stock universe.
...
EXPERIMENT_ID = "exp-20260513-030"
EXPERIMENT_SLUG = "rs60_top_quintile_risk"
MULTIPLIER_KEY = "rs60_top_quintile_risk_multiplier_applied"
...
def _momentum_60d_pct(ohlcv_data: Any) -> float | None:
    ...
    try:
        ...
        current = float(close.iloc[-1].item() if hasattr(close.iloc[-1], "item") else close.iloc[-1])
        prior = float(close.iloc[-61].item() if hasattr(close.iloc[-61], "item") else close.iloc[-61])
    except Exception:
        return None
    if prior <= 0:
        return None
...
def _rs60_top_cutoff(features_dict: dict[str, dict[str, Any]]) -> float | None:
    values = []
...
def _make_enrich_wrapper(
    ...
    def wrapped(
        ...
        cutoff = _rs60_top_cutoff(features_dict)
        for sig in enriched:
            ...
            sig["rs60_top_quintile_cutoff"] = cutoff
            sig["rs60_top_quintile_state"] = bool(eligible)
        return enriched
...
def _scale_sizing(
    ...
    out = dict(sizing)
    out["rs60_top_quintile_baseline_shares"] = shares
    out["rs60_top_quintile_desired_shares"] = desired_shares
    out["rs60_top_quintile_cap_shares"] = cap_shares
    out["rs60_top_quintile_new_shares"] = new_shares
...
def _make_size_wrapper(
    ...
    def wrapped(
        ...
        for sig in sized:
            ...
            if sig.get("rs60_top_quintile_state") and sizing.get("shares_to_buy"):
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} RS60 Top-Quintile Risk",
            "",
...
def run() -> dict[str, Any]:
    ...
    original_shared_multiplier = getattr(
        base.portfolio_engine,
        "RS60_TOP_QUINTILE_RISK_MULTIPLIER",
        None,
    )
    if original_shared_multiplier is not None:
        base.portfolio_engine.RS60_TOP_QUINTILE_RISK_MULTIPLIER = 1.0
    ...
    try:
        ...
        if original_shared_multiplier is not None:
            base.portfolio_engine.RS60_TOP_QUINTILE_RISK_MULTIPLIER = (
                original_shared_multiplier
            )
    passed = selected["passed"]
    decision = (
        "accepted_shared_policy_implemented"
        if passed
        else "rejected_rs60_top_quintile_risk"
    )
    interpretation = (
        "RS60 top-quintile stock signals cleared the canonical three-window gate and are implemented through shared production/backtest policy."
        if passed
        else "RS60 top-quintile stock risk top-up did not clear the canonical three-window gate; do not promote this state variable on the frozen windows."
    )
    ...
    payload = {
        ...
        "decision": decision,
        "hypothesis": (
            "Cross-sectional research found 60-day momentum was the strongest standalone EOD ranking feature. Existing A/B stock signals in the same-day RS60 top quintile may deserve a small cap-aware risk top-up without changing entries or ranking."
        ),
        "change_type": "risk_allocation_shadow",
        "changed_variable": "rs60_top_quintile_risk_multiplier",
        "single_causal_variable": (
            "cap-aware post-sizing risk top-up for trend/breakout stock signals whose PIT 60d return is in the same-day top quintile"
        ),
        "parameters": {
            "state_definition": {
                "strategies": ["trend_long", "breakout_long"],
                "excluded_sectors": sorted(EXCLUDED_SECTORS),
                "momentum_60d_pct": "top 20% of feature-complete non-ETF/non-commodity stocks on that signal day",
            },
        ...
        "gate_questions": {
            "1_alpha_hypothesis": (
                "core risk allocation using a longer-horizon relative-strength state supported by exp-20260503-008 cross-sectional research"
            ),
            "2_history_check": {
                "exp-20260503-008": (
                    "observed 60-day momentum and pullback-RS cross-sectional promise, but no production slot-aware integration."
                ),
                "exp-20260510-012": (
                    "accepted 20-day SPY-relative entry-state top-up; this tests a different 60-day cross-sectional top-quintile state."
                ),
                "exp-20260513-013/018/023": (
                    "recent short-horizon momentum and quality stacks failed; this avoids 10d acceleration/deceleration and high-TQS stacking."
                ),
                "exp-20260513-027": (
                    "own-green slot priority failed; this does not alter ranking or replacement order."
                ),
                "llm_soft_ranking": (
            ...
            "3_single_causal_variable": (
                "rs60_top_quintile_risk_multiplier with fixed top-quintile definition from prior cross-sectional research"
            ),
            ...
            "5_reproducibility": (
                ".venv\\Scripts\\python.exe quant\\experiments\\exp_20260513_030_rs60_top_quintile_risk.py"
            ),
        },
        "backtest_protocol": {
        ...
        if passed
            ...
            "Use forward RS60 attribution or a different production-visible long-horizon state before retrying core momentum overlays."
        ),
        "related_files": [
            "quant/experiments/exp_20260513_030_rs60_top_quintile_risk.py",

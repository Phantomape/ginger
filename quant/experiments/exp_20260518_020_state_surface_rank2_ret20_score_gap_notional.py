"""exp-20260518-020: state-surface rank-2 ret20 plus score-gap notional.

Alpha search. Tests one production-visible default-off paper-sleeve variable:
when the accepted rotation state-surface queue's rank-2 candidate leads rank 1
on 20-day excess return versus SPY, and rank 1 still has a material composite
score lead, shift more paper notional toward rank 2. Core entries, exits,
ranking, candidate eligibility, hold days, active capacity, LLM/news, and
...
EXPERIMENT_ID = "exp-20260518-020"
EXPERIMENT_SLUG = "state_surface_rank2_ret20_score_gap_notional"
...
RANK2_SCORE_GAP_VARIANTS: OrderedDict[str, dict[str, Any]] = OrderedDict(
    [
        (
            BASELINE_VARIANT,
            {
                ...
                "score_gap_min": None,
                "aggression_order": 0,
                "description": "current accepted rank-2 ret20 lead profile",
            },
        ),
        (
            "rank2_ret20_lead_score_gap_ge_020_mild",
            {
                "profile": [1.20, 1.65, 1.10, 0.675, 0.35],
                "rank2_ret20_lead_min": ACCEPTED_RANK2_RET20_LEAD_MIN,
                "score_gap_min": 0.20,
                "aggression_order": 1,
                "description": "mild rank-2 lift when rank-1 score lead remains at least 0.20",
            },
        ),
        (
            "rank2_ret20_lead_score_gap_ge_030_mild",
            {
                "profile": [1.20, 1.65, 1.10, 0.675, 0.35],
                "rank2_ret20_lead_min": ACCEPTED_RANK2_RET20_LEAD_MIN,
                "score_gap_min": 0.30,
                "aggression_order": 2,
                "description": "mild rank-2 lift when rank-1 score lead remains at least 0.30",
            },
        ),
        (
            "rank2_ret20_lead_score_gap_ge_030_balanced",
            {
                "profile": [1.10, 1.75, 1.10, 0.675, 0.35],
                "rank2_ret20_lead_min": ACCEPTED_RANK2_RET20_LEAD_MIN,
                "score_gap_min": 0.30,
                "aggression_order": 3,
                "description": "balanced rank-2 lift when score and ret20 disagree",
            },
        ),
        (
            "rank2_ret20_lead_score_gap_ge_030_strong",
            {
                "profile": [1.00, 1.85, 1.10, 0.675, 0.35],
                "rank2_ret20_lead_min": ACCEPTED_RANK2_RET20_LEAD_MIN,
                "score_gap_min": 0.30,
                "aggression_order": 4,
                "description": "strong rank-2 lift when score and ret20 disagree",
            },
        ),
        (
            "rank2_ret20_lead_score_gap_ge_050_balanced",
            {
                "profile": [1.10, 1.75, 1.10, 0.675, 0.35],
                "rank2_ret20_lead_min": ACCEPTED_RANK2_RET20_LEAD_MIN,
                "score_gap_min": 0.50,
                "aggression_order": 5,
                "description": "balanced rank-2 lift only when rank-1 score lead is large",
            },
        ),
    ]
)
...
def _baseline_profile_for_trade(
    ...
    if lead is not None and lead >= ACCEPTED_RANK2_RET20_LEAD_MIN:
...
def _profile_for_trade(
    *,
    ...
    min_gap = _float(variant.get("score_gap_min"))
    if (
        lead is not None
        ...
        gap_tag = str(min_gap).replace(".", "p")
        lead_tag = str(min_lead).replace(".", "p")
        return (
            f"rank2_ret20_lead_ge_{lead_tag}_score_gap_ge_{gap_tag}",
...
def _apply_score_gap_profile(
    ...
    *,
    ...
    for trade in trades:
        ...
        profile_name, profile, score_gap_profile_applied = _profile_for_trade(
        ...
        row["rank2_ret20_score_gap_rank_notional_variant"] = variant_name
        row["rank2_ret20_score_gap_profile_name"] = profile_name
        row["rank2_ret20_score_gap_profile_applied"] = score_gap_profile_applied
        row["regime"] = regime
        ...
        rows.append(
            {
                ...
                "rank2_ret20_score_gap_profile_name": trade.get(
                    "rank2_ret20_score_gap_profile_name"
                ),
                "rank2_ret20_score_gap_profile_applied": trade.get(
                    "rank2_ret20_score_gap_profile_applied"
                ),
...
def _notional_by_profile(trades: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for trade in trades:
        key = str(trade.get("rank2_ret20_score_gap_profile_name") or "unknown")
...
def _variant_payload(
    *,
    ...
    for label, window in WINDOWS.items():
        ...
        adjusted = _apply_score_gap_profile(
            selected,
            variant_name=variant_name,
            variant=variant,
            prices=prices,
        ...
        adjusted_trades = [
            ...
            if trade.get("rank2_ret20_score_gap_profile_applied")
        ]
        surface_sleeve[label] = {
            ...
            "rank2_ret20_score_gap_adjusted_trade_count": len(adjusted_trades),
            "rank2_ret20_score_gap_adjusted_pnl": round(
            ...
            "notional_by_rank2_score_gap_profile": _notional_by_profile(adjusted),
    ...
    return {
        "variant_name": variant_name,
        "variant_type": "rank2_ret20_score_gap_rank_notional_profile",
        ...
        "rank2_ret20_score_gap_adjusted_trade_count": len(adjusted_all),
        "rank2_ret20_score_gap_adjusted_windows": sorted(adjusted_windows),
...
def _gate4_for_variant(
    *,
    ...
    adjusted_guard_passed = (
        variant["rank2_ret20_score_gap_adjusted_trade_count"] >= MIN_ADJUSTED_TRADES
        and len(variant["rank2_ret20_score_gap_adjusted_windows"]) >= MIN_ADJUSTED_WINDOWS
    )
    ...
    return {
        ...
        "rank2_ret20_score_gap_adjusted_trade_count": variant[
            "rank2_ret20_score_gap_adjusted_trade_count"
        ],
        "rank2_ret20_score_gap_adjusted_windows": variant[
            "rank2_ret20_score_gap_adjusted_windows"
        ],
...
def _artifact_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# {EXPERIMENT_ID} State-Surface Rank-2 Ret20 Score-Gap Notional",
        "",
    ...
    for row in payload["sweep_summary"]:
        ...
        lines.append(
            "| {variant} | {passed} | {gap} | {profile} | {ev:+.4f} | ${pnl:+,.2f} | {wi} | {wr} | {adj} | {dd:+.4%} | {share} |".format(
                ...
                adj=row["gate4"]["rank2_ret20_score_gap_adjusted_trade_count"],
    ...
    for label in WINDOWS:
        ...
        lines.append(
            "| {label} | {bev:.4f} | {aev:.4f} | {dev:+.4f} | ${bpnl:,.2f} | ${apnl:,.2f} | ${dpnl:+,.2f} | {bdd:.2%} | {add:.2%} | {trades} |".format(
                ...
                trades=sleeve["rank2_ret20_score_gap_adjusted_trade_count"],
            )
        )
    lines.extend(
        [
...
def build_payload() -> dict[str, Any]:
    ...
    variants = [
        ...
        for variant_name, variant in RANK2_SCORE_GAP_VARIANTS.items()
    ]
    ...
    if best_summary["gate4"]["passed"]:
        ...
        status = "accepted"
        interpretation = (
            "Rank-2 ret20 leadership is strongest when it appears despite a "
            "material rank-1 composite-score lead. In that score/ret20 disagreement "
            "state, shifting more bounded paper notional to rank 2 improves fixed "
            "state-surface replacement value without changing candidate eligibility, "
            "ranking, hold days, or live/default orders."
        )
    else:
        decision = "rejected_state_surface_rank2_ret20_score_gap_notional"
        status = "rejected"
        interpretation = (
            "No tested score-gap-conditioned rank-2 ret20 paper-notional profile "
            "improved the accepted rank-2 ret20 baseline across the fixed-window gate."
        )
    ...
    return {
        ...
        "decision": decision,
        "lane": "alpha_search",
        "change_type": "state_surface_rank2_ret20_score_gap_notional",
        "changed_variable": "rank2_ret20_score_gap_rank_notional_profile",
        "change_summary": (
            "Sweep a score-gap-conditioned rank-2 ret20-leadership paper-notional "
            "profile for the accepted default-off rotation state-surface sleeve."
        ),
        "component": "quant/experiments",
        "mechanism_family": "state_aware_candidate_pool_allocation",
        "hypothesis": (
            "When rank 2 leads rank 1 on 20-day excess return versus SPY but rank 1 "
            "still keeps a material composite-score lead, the queue is exposing a "
            "clean score/ret20 disagreement. A bounded shift from rank 1 to rank 2 "
            "should improve default-off replacement value."
        ),
        "alpha_hypothesis": {
        ...
        "history_check": {
            ...
            "exp-20260518-018": "Accepted rank-2 ret20 leadership profile.",
            "anti_repeat_boundary": (
                "This is not a nearby standalone rank-2 ret20 scalar retry. The new "
                "single discriminator is the joint production-visible score/ret20 "
                "disagreement: rank-2 ret20 leadership plus rank-1 composite score gap."
            ),
        },
        "protocol_answers": {
            "1_alpha_hypothesis": (
                "capital allocation: use score-gap-conditioned rank-2 ret20 "
                "leadership as a production-visible rank-quality field before "
                "shifting default-off paper notional toward rank 2."
            ),
            "2_history_check": (
                "Score compression and rank-2 ret20 leadership are logged. No prior "
                "current-stack run isolated the score-gap-conditioned subset."
            ),
            "3_single_causal_variable": "rank2_ret20_score_gap_rank_notional_profile",
            "4_acceptance_standard": (
                "docs/backtesting.md three fixed windows; best non-control profile "
                "must improve aggregate EV/PnL versus accepted rank-2 ret20 notional, "
            ...
            "5_reproducibility": (
                ...
                "exp_20260518_020_state_surface_rank2_ret20_score_gap_notional.py"
            ),
        },
        "parameters": {
            "single_causal_variable": "rank2_ret20_score_gap_rank_notional_profile",
            ...
            "accepted_rank2_ret20_lead_profile": ACCEPTED_RANK2_RET20_LEAD_PROFILE,
            "accepted_rank2_ret20_lead_min": ACCEPTED_RANK2_RET20_LEAD_MIN,
            "variants": RANK2_SCORE_GAP_VARIANTS,
        ...
        "production_impact": {
            ...
            "production_impact": (
                ...
                "score-gap-conditioned rank-2 ret20 leadership. The same "
                "state_surface_sleeve.py path is used by production; live/default "
        ...
        "next_evidence_needed": (
            "Promote the accepted score-gap-conditioned rank-2 ret20 profile in shared "
            "state_surface_sleeve.py, add parity coverage, and continue forward closed "
            "replacement-value observation."
            if best_summary["gate4"]["passed"]
            else "Keep the accepted rank-2 ret20 profile; next state-surface alpha needs a different production-visible discriminator or forward replacement-value evidence."
        ),
...
def persist(payload: dict[str, Any]) -> None:
    ...
    _write_json(
        ...
        {
            "experiment_id": EXPERIMENT_ID,
            "title": "State-surface rank-2 ret20 score-gap notional",

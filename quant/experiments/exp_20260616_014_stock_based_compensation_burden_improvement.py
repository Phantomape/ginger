"""exp-20260616-014: stock-based compensation burden improvement scout.

Replay-only alpha search. The single decision hypothesis is a PIT free SEC
Companyfacts candidate source: production-universe names whose annual
stock-based compensation burden is falling versus revenue, while revenue and
gross profit remain positive, may be growth-quality leaders when liquid
...
EXPERIMENT_ID = "exp-20260616-014"
STEM = "stock_based_compensation_burden_improvement"
TRIAL_FAMILY = "stock_based_compensation_burden_improvement_candidate_pool"
TRIAL_VARIANT_ID = "sbc_burden_improvement_top1_next_open_10d_v1"
CHANGED_VARIABLE = "stock_based_compensation_burden_improvement_candidate_source_v1"
...
OUT_JSON = OUT_DIR / f"exp_20260616_014_{STEM}.json"
...
PREDICTION = {
    ...
    "confidence_reason": (
        "SBC burden improvement is a distinct shareholder-dilution quality "
        "field versus recent R&D, SG&A, tax, interest, and buyback scouts; "
        "however raw Companyfacts alpha has been saturated and likely fails "
        "on window or comparator robustness."
    ),
    "recorded_at": "2026-06-16T12:04:14+00:00",
}
    ...
    **base.PRODUCTION_IMPACT,
    ...
    "execution_envelope": {
        **base.PRODUCTION_IMPACT["execution_envelope"],
        "failure_handling": (
            "missing raw SEC annual SBC, revenue, or gross-profit facts, "
            "missing prior comparison period, stale facts, missing CIK "
    ...
    "parity_note": (
        ...
        "SEC tag mapping, filed-date PIT SBC/revenue improvement gate, liquid "
...
PRE_RUN_QUESTIONS = {
    "1_alpha_hypothesis": (
        "candidate_pool: raw SEC Companyfacts annual stock-based compensation "
        "burden falling versus revenue, with positive revenue/gross-profit "
        "context and liquid SPY-relative leadership, may identify growth-quality "
        "candidates whose shareholder dilution cost is improving before a "
        "10-trading-day continuation leg."
    ),
    "2_history_check": {
        "exp-20260616-010": (
            "Rejected SGA/operating-expense leverage. This run tests dilution "
            "quality through SBC burden, not broad cost-line leverage."
        ),
        "exp-20260616-009": (
            "Rejected free-cash-flow covered buyback. This run tests reduced "
            "compensation dilution burden, not cash-funded repurchases."
        ),
        "exp-20260616-003/004/005": (
            "Rejected R&D, interest, and tax burden raw Companyfacts scouts. "
            "This run uses a distinct shareholder-cost quality field."
        ),
        "exp-20260615-016": (
            "Rejected operating leverage acceleration. This run tests one "
            "specific dilution-cost input rather than operating-income outcome."
        ),
        "exp-20260614-029": (
            "Rejected diluted-share-count contraction. This run is not a share "
            "count threshold; it uses explicit SBC burden versus revenue."
        ),
    },
    ...
    "5_reproducibility": (
        ...
        "exp_20260616_014_stock_based_compensation_burden_improvement.py"
    ),
}
...
def _latest_sbc_fact(
    ...
    *,
    ...
    return max(
        candidates,
        key=lambda row: (row["end"], row["filed"], -_sbc_tag_priority(str(row.get("tag") or ""))),
    )
    ...
    *,
            ...
            candidates.append(
                {
                    "date": signal_date,
                    "ticker": ticker,
                    "source": "SBC_BURDEN_IMPROVEMENT_PAPER",
                    ...
                    "uses_llm": False,
                    "trade_enabled": False,
                    **{f"sbc_{k}": v for k, v in observation.items()},
                    **confirm,
                }
            )
    ...
    rows.sort(
        key=lambda row: (
            ...
            -float(row["sbc_sbc_ratio_improvement"] or 0.0),
            -float(row["sbc_revenue_growth"] or 0.0),
...
def _gate4(
    *,
    ...
    gate["decision"] = (
        "positive_replay_lead_not_promoted_stock_based_compensation_burden_improvement"
        if gate["passed"]
        else "rejected_stock_based_compensation_burden_improvement_candidate_pool"
    )
    return gate
...
def _postprocess_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate4["passed"]:
        interpretation = (
            "The SBC burden-improvement source cleared the numeric three-window "
            "replay screen, but remains only a replay lead because no shared "
            "daily/backtest helper was promoted."
        )
    else:
        interpretation = (
            "The SBC burden-improvement source did not clear Gate 4 "
    ...
    payload.update(
        {
            ...
            "new_evidence_type": "raw_sec_companyfacts_stock_based_compensation_burden_pit_field",
    ...
    payload["backtest_protocol"]["execution_model"] = (
        "Annual stock-based compensation, revenue, and gross-profit facts are "
        "read from raw SEC Companyfacts tags and are known only by filed date "
        "(<= signal date). The current SBC/revenue ratio is compared with the "
        "prior annual period using the same SBC tag. Price confirmation uses "
    ...
    payload["next_evidence_needed"] = (
        ...
        "as per-share SBC burden net of buybacks, grant-value normalization, "
        "or closed forward replacement-value rows. Do not sweep SBC tags, ratio "
        "thresholds, annual fact freshness, price guards, top-N, hold, cooldown, "
    ...
    payload["post_run_reflection"] = {
        ...
        "forbidden_near_neighbor_retry": (
            "Do not retry by sweeping SBC tag lists, SBC/revenue thresholds, "
            "revenue or gross-profit floors, annual fact freshness, RS/close/"
...
def _build_card(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"# {EXPERIMENT_ID} SBC Burden Improvement",
            "",

...
SPACE_CATALYST_FORWARD_HYPOTHESIS = {
    ...
    "space_source_diversity_peer_nonleader_trend_definition": (
        "source-diverse official non-attention evidence on trend_long signals "
        "while Space peer momentum state is nonleader"
    ),
    ...
    "space_source_diversity_dual_catalyst_iwm_leader_trend_experiment_id": (
        "exp-20260516-015"
    ),
    "space_source_diversity_dual_catalyst_iwm_leader_trend_definition": (
        "source-diverse official non-attention evidence on trend_long signals "
        "with both customer_win and government_space_contract event fields "
        "while IWM 20d momentum is above SPY 20d momentum"
    ),
    "space_source_diversity_dual_catalyst_iwm_leader_trend_state": (
        "smallcap_leader"
    ),
    "space_source_diversity_dual_catalyst_iwm_leader_trend_risk_scalar": (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_IWM_LEADER_TREND_RISK_SCALAR
    ),
    "space_source_diversity_dual_catalyst_same_theme_winner_trend_experiment_id": (
        "exp-20260516-019"
    ),
    "space_source_diversity_dual_catalyst_same_theme_winner_trend_definition": (
        "source-diverse official non-attention trend_long signals with both "
        "customer_win and government_space_contract event fields while closed "
        "defense-budget government_space_contract rows are cash- and "
        "same-theme replacement-positive"
    ),
    "space_source_diversity_dual_catalyst_same_theme_winner_trend_risk_scalar": (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_SAME_THEME_WINNER_TREND_RISK_SCALAR
    ),
    "space_source_diversity_dual_catalyst_near_perfect_trend_experiment_id": (
        "exp-20260516-023"
    ),
    "space_source_diversity_dual_catalyst_near_perfect_trend_definition": (
        "source-diverse official non-attention trend_long signals with both "
        "customer_win and government_space_contract event fields while TQS is "
        "near-perfect but not perfect"
    ),
    "space_source_diversity_dual_catalyst_near_perfect_trend_score_floor": (
        SPACE_CATALYST_NEAR_PERFECT_TQS_SCORE_FLOOR
    ),
    "space_source_diversity_dual_catalyst_near_perfect_trend_score_ceiling": (
        SPACE_CATALYST_NEAR_PERFECT_TQS_SCORE_CEILING
    ),
    ...
    "space_source_diversity_dual_catalyst_financing_profile_trend_definition": (
        "source-diverse official non-attention trend_long signals with both "
        "customer_win and government_space_contract event fields while the "
        "Space registry event-guard profile is financing/dilution sensitive"
    ),
    ...
    "space_source_diversity_dual_catalyst_benchmark_breadth_trend_definition": (
        "source-diverse official non-attention trend_long signals with both "
        "customer_win and government_space_contract event fields while closed "
        "event-state rows are cash-, SPY-, QQQ-, UFO-, and ARKX-positive"
    ),
...
def _observation_slot_row(
    *,
    ...
    source_diversity_dual_catalyst_iwm_leader_trend_bucket = (
        source_diversity_dual_catalyst_trend_bucket
        and (iwm_relative_momentum_state or {}).get("state") == "smallcap_leader"
    )
    source_diversity_dual_catalyst_iwm_leader_trend_risk_scalar = (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_IWM_LEADER_TREND_RISK_SCALAR
        if source_diversity_dual_catalyst_iwm_leader_trend_bucket
        else 1.0
    )
    source_diversity_dual_catalyst_same_theme_winner_trend_bucket = (
        source_diversity_dual_catalyst_trend_bucket
        and _is_space_defense_budget_same_theme_winner_profile(
            forward_replacement_profile
        )
    )
    source_diversity_dual_catalyst_same_theme_winner_trend_risk_scalar = (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_SAME_THEME_WINNER_TREND_RISK_SCALAR
        if source_diversity_dual_catalyst_same_theme_winner_trend_bucket
        else 1.0
    )
    source_diversity_dual_catalyst_near_perfect_trend_bucket = (
        source_diversity_dual_catalyst_trend_bucket
        and _is_space_near_perfect_tqs(signal.get("trade_quality_score"))
    )
    source_diversity_dual_catalyst_near_perfect_trend_risk_scalar = (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_NEAR_PERFECT_TREND_RISK_SCALAR
        if source_diversity_dual_catalyst_near_perfect_trend_bucket
        else 1.0
    )
    source_diversity_dual_catalyst_financing_profile_trend_bucket = (
        source_diversity_dual_catalyst_trend_bucket
        and financing_dilution_profile_bucket
    )
    source_diversity_dual_catalyst_financing_profile_trend_risk_scalar = (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_FINANCING_PROFILE_TREND_RISK_SCALAR
        if source_diversity_dual_catalyst_financing_profile_trend_bucket
        else 1.0
    )
    source_diversity_dual_catalyst_benchmark_breadth_trend_bucket = (
        source_diversity_dual_catalyst_trend_bucket
        and _is_space_benchmark_breadth_profile(forward_replacement_profile)
    )
    source_diversity_dual_catalyst_benchmark_breadth_trend_risk_scalar = (
        SPACE_CATALYST_SOURCE_DIVERSITY_DUAL_CATALYST_BENCHMARK_BREADTH_TREND_RISK_SCALAR
        if source_diversity_dual_catalyst_benchmark_breadth_trend_bucket
        else 1.0
    )
    ...
    return {
        ...
        "space_source_diversity_dual_catalyst_same_theme_winner_profile": (
            forward_replacement_profile
            if source_diversity_dual_catalyst_same_theme_winner_trend_bucket
            else None
        ),
        "space_source_diversity_dual_catalyst_financing_profile": (
            event_guard_profile
            if source_diversity_dual_catalyst_financing_profile_trend_bucket
            else None
        ),
        "space_source_diversity_dual_catalyst_benchmark_breadth_profile": (
            forward_replacement_profile
            if source_diversity_dual_catalyst_benchmark_breadth_trend_bucket
            else None
        ),
        ...
        "space_source_diversity_dual_catalyst_iwm_leader_trend_risk_scalar": _round(
            source_diversity_dual_catalyst_iwm_leader_trend_risk_scalar,
            6,
        ),
        ...
        "space_source_diversity_dual_catalyst_same_theme_winner_trend_risk_scalar": _round(
            source_diversity_dual_catalyst_same_theme_winner_trend_risk_scalar,
            6,
        ),
        ...
        "space_source_diversity_dual_catalyst_near_perfect_trend_risk_scalar": _round(
            source_diversity_dual_catalyst_near_perfect_trend_risk_scalar,
            6,
        ),
        ...
        "space_source_diversity_dual_catalyst_financing_profile_trend_risk_scalar": _round(
            source_diversity_dual_catalyst_financing_profile_trend_risk_scalar,
            6,
        ),
        ...
        "space_source_diversity_dual_catalyst_benchmark_breadth_trend_risk_scalar": _round(
            source_diversity_dual_catalyst_benchmark_breadth_trend_risk_scalar,
            6,
        ),

"""exp-20260426-041 opening-range continuation shadow audit.

Observed-only alpha discovery. This script does not modify production strategy
logic. With daily OHLCV only, "opening range" is approximated as: a strong
prior-day move is not materially faded at the next open, and the candidate day
closes above the prior high with positive relative strength versus SPY.
"""
...
EXPERIMENT_ID = "exp-20260426-041"
OUT_JSON = os.path.join(
    REPO_ROOT,
    "docs",
    "experiments",
    "artifacts",
    "exp_20260426_041_opening_range_continuation_shadow.json",
)
...
MIN_PRIOR_DAY_RETURN = 0.02
MIN_PRIOR_DAY_RS_VS_SPY = 0.0
MIN_OPEN_VS_PRIOR_CLOSE = -0.005
...
def _candidate_rows(snapshot: dict[str, list[dict]], ticker: str, dates: list[str]) -> list[dict]:
    ...
    for date in dates:
        ...
        prior_ret = _return_from_closes(rows, idx - 2, idx - 1) if idx >= 2 else None
        spy_prior_ret = _return_from_closes(spy_rows, spy_prev_idx - 1, spy_prev_idx) if spy_prev_idx >= 1 else None
        candidate_ret = _return_from_closes(rows, idx - 1, idx)
        spy_candidate_ret = _return_from_closes(spy_rows, spy_idx - 1, spy_idx)
        if None in (prior_ret, spy_prior_ret, candidate_ret, spy_candidate_ret):
            continue
        ...
        if not prev_close or prev_high is None or cur_open is None or cur_close is None:
            continue
        dollar_volume = (cur_close * cur_volume) if cur_volume is not None else None
        if dollar_volume is None or dollar_volume < MIN_DOLLAR_VOLUME:
            continue
        open_vs_prior_close = (cur_open / prev_close) - 1.0
        prior_rs = prior_ret - spy_prior_ret
        candidate_rs = candidate_ret - spy_candidate_ret
        if prior_ret < MIN_PRIOR_DAY_RETURN:
            continue
        if prior_rs <= MIN_PRIOR_DAY_RS_VS_SPY:
            continue
        if open_vs_prior_close < MIN_OPEN_VS_PRIOR_CLOSE:
            continue
        if cur_close <= cur_open or cur_close <= prev_high:
            continue
        if candidate_rs <= MIN_CANDIDATE_RS_VS_SPY:
            continue
        forwards = {
        ...
        out.append({
            ...
            "prior_day_spy_return": round(spy_prior_ret, 6),
            "prior_day_rs_vs_spy": round(prior_rs, 6),
            "open_vs_prior_close": round(open_vs_prior_close, 6),
...
def main() -> int:
    ...
    payload = {
        ...
        "hypothesis": (
            "Opening-range continuation after a strong prior-day move may surface "
            "replayable continuation candidates with useful forward returns and "
            "limited same-day A/B overlap."
        ),
        "single_causal_variable": "opening range continuation shadow entry definition",
        "shadow_entry_definition": {
            "data_granularity": "daily OHLCV approximation; no intraday bars available",
            "filters": {
                "prior_day_close_to_close_return_min": MIN_PRIOR_DAY_RETURN,
                "prior_day_rs_vs_spy_min": MIN_PRIOR_DAY_RS_VS_SPY,
                "candidate_open_vs_prior_close_min": MIN_OPEN_VS_PRIOR_CLOSE,
                "candidate_close_must_be_above_open": True,
                "candidate_close_must_be_above_prior_high": True,
        ...
        "production_promotion_reason": (
            "Shadow-only source audit. Requires a real strategy replay with position "
            "sizing, slot competition, and exit rules before any production change."
        ),
    }

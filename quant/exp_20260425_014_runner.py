"""exp-20260425-014 residual breakout pocket: Technology 26-40 DTE.

Hypothesis:
    After the accepted A+B residual allocation stack, Technology breakouts
    26-40 trading days before earnings are a remaining event-proximity leak.
    The audited fixed-window trades mapped this to mid/old losses while the
    dominant late-strong window had no qualifying exposure.
...
VARIANTS = OrderedDict([
    ...
    ("breakout_tech_dte2640_025", {"multiplier": 0.25}),
    ("breakout_tech_dte2640_000", {"multiplier": 0.0}),
])

OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260425_014_results.json")
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    original_multiplier = pe_module.BREAKOUT_TECH_DTE_RISK_MULTIPLIER
    pe_module.BREAKOUT_TECH_DTE_RISK_MULTIPLIER = 1.0
    try:
        sized = _original_size_signals(signals, portfolio_value, risk_pct=risk_pct)
    finally:
        pe_module.BREAKOUT_TECH_DTE_RISK_MULTIPLIER = original_multiplier

    for sig in sized:
        if sig.get("strategy") != "breakout_long" or sig.get("sector") != "Technology":
            continue

        dte = sig.get("days_to_earnings")
        if dte is None or not (26 <= dte <= 40):
            continue
        ...
        new_sizing["breakout_tech_dte2640_multiplier_applied"] = multiplier
...
def _run_window(
    ...
    return {
        ...
        "breakout_tech_dte2640_multiplier": variant_cfg["multiplier"],
        "eligible_breakout_tech_dte2640_signals": _state["eligible_signals"],
        "adjusted_breakout_tech_dte2640_signals": _state["adjusted_signals"],
...
def main() -> int:
        ...
        for label, cfg in WINDOWS.items():
            for variant_name, variant_cfg in VARIANTS.items():
                ...
                print(
                    ...
                    f"eligible={summary['eligible_breakout_tech_dte2640_signals']} "
                    f"adjusted={summary['adjusted_breakout_tech_dte2640_signals']}"
                )
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule": {
                    "name": "breakout_tech_dte2640_de_risk",
                    "definition": (
                        "apply a sleeve-specific risk multiplier only when "
                        "strategy == breakout_long, sector == Technology, "
                        "and 26 <= days_to_earnings <= 40"
                    ),
                    "goal": (
                        "test whether residual Technology breakout drag is "
                        "concentrated in the 26-40 trading-day pre-earnings pocket"
                    ),
                },
                "variants": VARIANTS,
                "results": results,

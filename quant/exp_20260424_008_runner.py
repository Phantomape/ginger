"""exp-20260424-008 residual breakout pocket: Industrials 3-4% gap risk.

Hypothesis:
    The remaining breakout leak is not all `breakout_long | Industrials`.
    The drag is concentrated in the narrower risk-shape pocket

        strategy == breakout_long
        sector == Industrials
        0.03 <= gap_vulnerability_pct < 0.04

    Zeroing only that cohort should preserve the late-strong winner that sits
    outside the band while improving the weaker windows.
"""
...
VARIANTS = OrderedDict([
    ...
    ("bk_ind_gap34_025", {"multiplier": 0.25}),
    ("bk_ind_gap34_000", {"multiplier": 0.0}),
])

OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260424_008_results.json")
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    ...
    for sig in sized:
        ...
        new_sizing["breakout_industrials_gap34_multiplier_applied"] = multiplier
...
def _run_window(universe: list[str], label: str, cfg: dict, variant_name: str, variant_cfg: dict) -> dict:
    ...
    return {
        ...
        "breakout_industrials_gap34_multiplier": variant_cfg["multiplier"],
        "eligible_breakout_industrials_gap34_signals": _state["eligible_signals"],
        "adjusted_breakout_industrials_gap34_signals": _state["adjusted_signals"],
...
def main() -> int:
        ...
        for label, cfg in WINDOWS.items():
            for variant_name, variant_cfg in VARIANTS.items():
                ...
                print(
                    ...
                    f"eligible={summary['eligible_breakout_industrials_gap34_signals']} "
                    f"adjusted={summary['adjusted_breakout_industrials_gap34_signals']}"
                )
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule": {
                    "name": "breakout_industrials_gap34_de_risk",
                    "definition": (
                        "apply a sleeve-specific risk multiplier only when "
                        "strategy == breakout_long, sector == Industrials, "
                        "and 0.03 <= gap_vulnerability_pct < 0.04"
                    ),
                    "goal": (
                        "test whether the residual breakout Industrials leak is "
                        "the 3-4% gap-vulnerability pocket rather than the whole sector"
                    ),
                },
                "variants": VARIANTS,
                "results": results,

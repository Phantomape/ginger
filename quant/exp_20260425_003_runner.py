"""exp-20260425-003 residual breakout pocket: Financials with 8-14 DTE.

Hypothesis:
    After the accepted A+B stack plus the recent residual breakout/trend cohort
    rules, the remaining breakout Financials drag is not the whole sector. The
    repeated losers cluster in the narrower event-proximity pocket

        strategy == breakout_long
        sector == Financials
        8 <= days_to_earnings <= 14

    De-risking only that pocket should keep the dominant late-strong tape
    intact while improving weaker windows.
"""
...
VARIANTS = OrderedDict([
    ...
    ("breakout_fin_dte814_025", {"multiplier": 0.25}),
    ("breakout_fin_dte814_000", {"multiplier": 0.0}),
])

OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260425_003_results.json")
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    ...
    for sig in sized:
        if sig.get("strategy") != "breakout_long" or sig.get("sector") != "Financials":
            continue

        days_to_earnings = sig.get("days_to_earnings")
        if days_to_earnings is None or not (8 <= days_to_earnings <= 14):
            continue
        ...
        new_sizing["breakout_financials_dte814_multiplier_applied"] = multiplier
...
def _run_window(
    ...
    return {
        ...
        "breakout_financials_dte814_multiplier": variant_cfg["multiplier"],
        "eligible_breakout_financials_dte814_signals": _state["eligible_signals"],
        "adjusted_breakout_financials_dte814_signals": _state["adjusted_signals"],
...
def main() -> int:
        ...
        for label, cfg in WINDOWS.items():
            for variant_name, variant_cfg in VARIANTS.items():
                ...
                print(
                    ...
                    f"eligible={summary['eligible_breakout_financials_dte814_signals']} "
                    f"adjusted={summary['adjusted_breakout_financials_dte814_signals']}"
                )
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule": {
                    "name": "breakout_financials_dte814_de_risk",
                    "definition": (
                        "apply a sleeve-specific risk multiplier only when "
                        "strategy == breakout_long, sector == Financials, "
                        "and 8 <= days_to_earnings <= 14"
                    ),
                    "goal": (
                        "test whether the residual breakout Financials drag "
                        "is concentrated in the 8-14 DTE pocket"
                    ),
                },
                "variants": VARIANTS,
                "results": results,

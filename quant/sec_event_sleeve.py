"""Default-off SEC governance/procedural paper event sleeve.

This module advances the accepted_requires_followup SEC governance/procedural
event alpha from a static replay into a production-visible paper ledger. It
does not emit orders, rank core candidates, size positions, or consume core
A/B slots.
"""
...
SLEEVE_NAME = "SEC_GOVERNANCE_EVENT_SLEEVE_PAPER"
...
def empty_sec_event_sleeve_state() -> dict[str, Any]:
    return {
...
def load_sec_event_sleeve_state(path: Path | str = DEFAULT_STATE_PATH) -> dict[str, Any]:
    state_path = Path(path)
    if not state_path.exists():
        return empty_sec_event_sleeve_state()
...
def save_sec_event_sleeve_state(
    state: dict[str, Any],
...
def append_sec_event_sleeve_snapshot(
...
def build_sec_event_sleeve_snapshot(
    *,
    ...
    if persist:
        save_sec_event_sleeve_state(working_state, state_path)
        append_sec_event_sleeve_snapshot(snapshot, snapshot_log_path)
    return snapshot


def empty_sec_event_sleeve_snapshot(as_of: str, reason: str) -> dict[str, Any]:
    return {
        "sleeve": SLEEVE_NAME,
...
def _decision_id(candidate: dict[str, Any]) -> str:
    ...
    return f"{SLEEVE_NAME}:{GOVERNANCE_RULE_VERSION}:{usable}:{ticker}:{accession}:{target_cell}"
...
def _production_impact() -> dict[str, Any]:
    return {
        ...
        "alters_sizing": False,
        "alters_orders": False,
        "scope": "default_off_sec_governance_paper_event_sleeve",
    }

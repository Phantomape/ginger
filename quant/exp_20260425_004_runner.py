"""exp-20260425-004 residual breakout pocket: Communication Services 3-4% gap.

Hypothesis:
    After the accepted A+B stack plus the earlier Communication Services
    near-high haircut, the remaining breakout drag in that sleeve is not
    another broad sector cut. The residual losers cluster in the narrower
    risk-shape pocket
    ...
    De-risking only that pocket should preserve the dominant late-strong tape
    while improving the weaker windows.
"""
...
VARIANTS = OrderedDict([
    ...
    ("breakout_comms_gap34_025", {"multiplier": 0.25}),
    ("breakout_comms_gap34_000", {"multiplier": 0.0}),
])

OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260425_004_results.json")
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    ...
    for sig in sized:
        if (
            ...
            or sig.get("sector") != "Communication Services"
        ):
            continue

        gap = sig.get("gap_vulnerability_pct")
        if gap is None or not (0.03 <= gap < 0.04):
            continue
        ...
        new_sizing["breakout_comms_gap34_multiplier_applied"] = multiplier
...
def _run_window(
    ...
    return {
        ...
        "breakout_comms_gap34_multiplier": variant_cfg["multiplier"],
        "eligible_breakout_comms_gap34_signals": _state["eligible_signals"],
        "adjusted_breakout_comms_gap34_signals": _state["adjusted_signals"],
...
def main() -> int:
        ...
        for label, cfg in WINDOWS.items():
            for variant_name, variant_cfg in VARIANTS.items():
                ...
                print(
                    ...
                    f"eligible={summary['eligible_breakout_comms_gap34_signals']} "
                    f"adjusted={summary['adjusted_breakout_comms_gap34_signals']}"
                )
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule": {
                    "name": "breakout_comms_gap34_de_risk",
                    "definition": (
                        "apply a sleeve-specific risk multiplier only when "
                        "strategy == breakout_long, sector == Communication Services, "
                        "and 0.03 <= gap_vulnerability_pct < 0.04"
                    ),
                    "goal": (
                        "test whether the residual breakout Communication Services "
                        "drag is concentrated in the 3%-4% gap-vulnerability pocket"
                    ),
                },
                "variants": VARIANTS,
                "results": results,

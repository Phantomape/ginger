"""exp-20260426-045 volatility-contraction breakout shadow audit.

Observed-only alpha discovery. This script does not modify production strategy
logic. With daily OHLCV, the shadow entry approximates a volatility contraction
breakout as: recent true-range compression versus the prior longer lookback,
price above the 50-day average, and a close above the prior 20-day high with
...
def _true_range_pct(rows: list[dict], idx: int) -> float | None:
    if idx <= 0 or idx >= len(rows):
        return None
    high = _value(rows[idx], "High")
    low = _value(rows[idx], "Low")
    prev_close = _value(rows[idx - 1], "Close")
...
def _candidate_rows(snapshot: dict[str, list[dict]], ticker: str, dates: list[str]) -> list[dict]:
    ...
    min_history = max(LONG_ATR_DAYS, MA_DAYS, BREAKOUT_LOOKBACK_DAYS) + 2
    ...
    for date in dates:
        ...
        if cur_close is None or cur_high is None or cur_volume is None:
            continue
        ...
        prior_highs = [
            _value(row, "High")
            for row in rows[idx - BREAKOUT_LOOKBACK_DAYS:idx]
        ]
        prior_high_values = [value for value in prior_highs if value is not None]
        if len(prior_high_values) < BREAKOUT_LOOKBACK_DAYS:
            continue
        prior_breakout_high = max(prior_high_values)
        if cur_close <= prior_breakout_high:
            continue
        ...
        if moving_average is None or cur_close <= moving_average:
            continue

        short_atr = _avg([
            _true_range_pct(rows, tr_idx)
            for tr_idx in range(idx - SHORT_ATR_DAYS, idx)
        ])
        long_atr = _avg([
            _true_range_pct(rows, tr_idx)
            for tr_idx in range(idx - LONG_ATR_DAYS, idx - SHORT_ATR_DAYS)
        ])
        if short_atr is None or long_atr is None or long_atr <= 0:
            continue
        atr_ratio = short_atr / long_atr
        if atr_ratio > MAX_SHORT_TO_LONG_ATR_RATIO:
            continue
        ...
        out.append({
            ...
            "breakout_above_prior_20d_high_pct": round((cur_close / prior_breakout_high) - 1.0, 6),
            "pct_above_50d_ma": round((cur_close / moving_average) - 1.0, 6),
...
def main() -> int:
    ...
    payload = {
        ...
        "hypothesis": (
            "A volatility-contraction breakout shadow entry may surface "
            "institutional accumulation candidates with useful forward returns "
            "and limited same-day A/B overlap."
        ),
        "single_causal_variable": "volatility contraction breakout shadow entry pattern",
        "shadow_entry_definition": {
            "data_granularity": "daily OHLCV approximation; no intraday bars used",
            "filters": {
                "short_atr_days": SHORT_ATR_DAYS,
                "long_atr_days": LONG_ATR_DAYS,
                "max_short_to_long_atr_ratio": MAX_SHORT_TO_LONG_ATR_RATIO,
                "breakout_close_above_prior_n_day_high": BREAKOUT_LOOKBACK_DAYS,
                "close_above_n_day_moving_average": MA_DAYS,
        ...
        "production_promotion_reason": (
            "Shadow-only source audit. Requires a production-path replay with "
            "sizing, slot competition, and exits before any production change."
        ),
    }

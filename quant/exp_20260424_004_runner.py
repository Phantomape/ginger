"""exp-20260424-004 candidate-day OMDB -> trend boost runner.

Tests a lightweight healthy-tape sleeve-routing idea using a candidate-day state:
    offense_minus_defense_breadth >= 0.15

Action:
    modestly boost `trend_long` sizing on those dates only.

This keeps entries, exits, ranking, and all accepted A+B cohort rules unchanged.
"""
...
OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260424_004_results.json")
...
def _compute_omdb_boost_days(cfg: dict) -> tuple[set[str], list[str]]:
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    ...
    for sig in signals:
        ...
        if sig2.get("strategy") == "trend_long":
            sig2["_omdb_trend_boost_multiplier"] = trend_multiplier
            boosted += 1
        patched.append(sig2)

    sized = _original_size_signals(patched, portfolio_value, risk_pct=risk_pct)
    for sig in sized:
        mult = sig.pop("_omdb_trend_boost_multiplier", None)
        if mult is None:
            continue
        ...
        if new_sizing:
            ...
            new_sizing["omdb_trend_boost_multiplier_applied"] = mult
...
def _run_window(universe: list[str], label: str, cfg: dict, trend_multiplier: float) -> dict:
    boost_days, sim_dates = _compute_omdb_boost_days(cfg)
...
def main() -> int:
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule": {
                    "name": "candidate_day_omdb_trend_boost",

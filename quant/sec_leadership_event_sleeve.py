"""Default-off SEC leadership-change paper event sleeve.

This module turns the observe-only SEC leadership-change forward queue into a
...
try:
    ...
    from sec_event_queue import LEADERSHIP_RULE_VERSION, PRIMARY_HORIZON_TRADING_DAYS
except ImportError:  # pragma: no cover - package-style imports in tests
    from quant.constants import ROUND_TRIP_COST_PCT
    from quant.price_asof_guard import filter_prices_for_asof
    from quant.sec_event_queue import LEADERSHIP_RULE_VERSION, PRIMARY_HORIZON_TRADING_DAYS


SLEEVE_NAME = "SEC_LEADERSHIP_CHANGE_EVENT_SLEEVE_PAPER"
STATE_SCHEMA_VERSION = 1
DEFAULT_EVENT_NOTIONAL_USD = 10_000.0
DEFAULT_MAX_POSITIONS = 1
DEFAULT_STATE_PATH = data_artifact_path("sec_leadership_event_sleeve_paper_state")
DEFAULT_SNAPSHOT_LOG_PATH = data_artifact_path("sec_leadership_event_sleeve_paper_snapshots")
...
def empty_sec_leadership_event_sleeve_state() -> dict[str, Any]:
    return {
...
def load_sec_leadership_event_sleeve_state(
    path: Path | str = DEFAULT_STATE_PATH,
) -> dict[str, Any]:
    state_path = Path(path)
    if not state_path.exists():
        return empty_sec_leadership_event_sleeve_state()
...
def save_sec_leadership_event_sleeve_state(
    state: dict[str, Any],
    path: Path | str = DEFAULT_STATE_PATH,
) -> None:
    state_path = Path(path)
...
def append_sec_leadership_event_sleeve_snapshot(
...
def build_sec_leadership_event_sleeve_snapshot(
    *,
    sec_leadership_event_queue: dict[str, Any] | None,
    as_of: str,
    ...
    working_state = deepcopy(
        state if state is not None else load_sec_leadership_event_sleeve_state(state_path)
    )
    ...
    if persist:
        save_sec_leadership_event_sleeve_state(working_state, state_path)
        append_sec_leadership_event_sleeve_snapshot(snapshot, snapshot_log_path)
    return snapshot


def empty_sec_leadership_event_sleeve_snapshot(as_of: str, reason: str) -> dict[str, Any]:
    return {
        "sleeve": SLEEVE_NAME,
...
def _pending_sort_key(entry: dict[str, Any]) -> tuple[str, str, float, str]:
...
def _candidate_sort_key(candidate: dict[str, Any]) -> tuple[str, float, str]:
    return (
        str(candidate.get("target_cell") or ""),
        float(candidate.get("reaction_excess_return") or 0.0),
...
def _decision_id(candidate: dict[str, Any]) -> str:
    ...
    target_cell = str(candidate.get("target_cell") or "unknown_cell")
    return (
        f"{SLEEVE_NAME}:{LEADERSHIP_RULE_VERSION}:"
        f"{usable}:{ticker}:{accession}:{target_cell}"
    )
...
def _production_impact() -> dict[str, Any]:
    return {
        ...
        "alters_sizing": False,
        "alters_orders": False,
        "scope": "default_off_sec_leadership_change_paper_event_sleeve",
    }

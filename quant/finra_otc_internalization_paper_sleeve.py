"""Default-off FINRA weekly non-ATS OTC internalization-retreat paper sleeve.

Shared helper for exp-20260706-018. It materializes the FINRA OTC-transparency
weekly NON-ATS (wholesaler internalization) symbol-level summary
(``api.finra.org`` group ``otcMarket`` name ``weeklySummary``,
``summaryTypeCode=OTC_W_SMBL``) as a versioned local archive, and exposes ONE
candidate rule used by BOTH the historical replay and the daily default-off
snapshot:

    signal day T admits ticker X when a new non-ATS OTC week row for X was
    first published on the session T (``prev_session < initialPublishedDate
    <= T``), the week's wholesaler-internalization share of consolidated tape
    volume RETREATS below the mean share of the prior 4 published weeks
    (share_retreat_ratio < 1: retail participation washout / quiet
    institutional accumulation), and the standard liquidity guards pass
    (close >= $10, 20d avg dollar volume >= $50M, 20d SPY-relative return
    non-negative); candidates rank ASCENDING by share_retreat_ratio (deepest
    retreat first), top-1 per day, next-open paper entry, 10-trading-day
    close exit.

This is the inverse gate direction from the rejected ATS dark-share RISE
sleeve (exp-20260703-016): a different venue population (wholesaler
internalization, predominantly retail flow, vs ATS dark pools) and a
retreat-admission shape instead of a rise-admission shape.

PIT boundary: FINRA publishes each week's non-ATS OTC file with a fixed
multi-week lag (~21-22 days observed for NMS Tier 1) and reports
``initialPublishedDate`` per row; the sleeve never uses a row before the
first session on/after that date, so historical replay and daily forward use
share the same honest availability boundary.
...
SLEEVE_NAME = "FINRA_OTC_INTERNALIZATION_RETREAT_PAPER"
RULE_VERSION = "finra_otc_internalization_retreat_top1_v1"
SOURCE_RULE_VERSION = "finra_otc_transparency_weekly_nonats_archive_v1"
...
DEFAULT_ROWS_PATH = data_artifact_path("finra_otc_weekly_rows")
DEFAULT_MANIFEST_PATH = data_artifact_path("finra_otc_weekly_manifest")
DEFAULT_STATE_PATH = data_artifact_path("finra_otc_internalization_paper_state")
DEFAULT_SNAPSHOT_LOG_PATH = data_artifact_path("finra_otc_internalization_paper_snapshots")
...
DEFAULT_CONFIG = {
    ...
    "trailing_weeks": 4,
    "max_share_retreat_ratio": 1.0,
    "min_weekly_volume_days": 3,
    "max_valid_otc_share": 1.0,
    "daily_entry_slots": 1,
...
def _otc_row_key(row: dict[str, Any]) -> tuple[str, str]:
    return (str(row.get("ticker") or "").upper(), str(row.get("week_start_date") or ""))


def normalise_otc_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows or []:
        ...
        otc_shares = _float_or_none(row.get("otc_share_quantity"))
        if not ticker or not week_start or not published or otc_shares is None:
            continue
        if otc_shares <= 0:
            continue
        ...
        out.append(
            {
                ...
                "otc_share_quantity": otc_shares,
                "otc_trade_count": _float_or_none(row.get("otc_trade_count")),
                "otc_notional": _float_or_none(row.get("otc_notional")),
                "tier": str(row.get("tier") or ""),
                "fetched_at": row.get("fetched_at"),
                "source": row.get("source") or "finra_otc_transparency_weekly_nonats",
            }
        )
    out.sort(key=_otc_row_key)
    return out


def load_finra_otc_weekly_rows(
    ...
    try:
        ...
        return []
    return normalise_otc_rows(rows)


def save_finra_otc_weekly_archive(
    *,
    rows: list[dict[str, Any]],
    ...
    manifest = {
        ...
        "source": "FINRA OTC transparency weekly NON-ATS (wholesaler internalization) symbol-level summary",
        "endpoint": FINRA_API_URL,
        "summary_type_code": "OTC_W_SMBL",
...
def fetch_finra_otc_weekly_rows(
    *,
    ...
    """Fetch weekly non-ATS OTC symbol rows for ``tickers`` from the FINRA query API.
                    ...
                    rows.append(
                        {
                            ...
                            "otc_share_quantity": record.get("totalWeeklyShareQuantity"),
                            "otc_trade_count": record.get("totalWeeklyTradeCount"),
                            "otc_notional": record.get("totalNotionalSum"),
                            "tier": record.get("tierIdentifier"),
                            "fetched_at": fetched_at,
                            "source": "finra_otc_transparency_weekly_nonats",
                        }
                    )
    ...
    return normalise_otc_rows(rows), fetch_log


def refresh_finra_otc_weekly_archive(
    *,
    ...
    """Merge newly published non-ATS OTC rows into the archive when it is stale.
    ...
    """
    fetch = fetch_fn or fetch_finra_otc_weekly_rows
    existing = normalise_otc_rows(existing_rows or [])
    ...
    merged = {_otc_row_key(row): row for row in existing}
    added = 0
    for row in new_rows:
        key = _otc_row_key(row)
        if key not in merged:
            added += 1
            merged[key] = row
        # Never overwrite an earlier-fetched row: the first observation is the
        # most PIT-faithful one we have.
    rows = normalise_otc_rows(list(merged.values()))
    if save:
        save_finra_otc_weekly_archive(
...
def otc_rows_by_ticker(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    """Index rows as ticker -> chronological (by week_start_date) list."""
    out: dict[str, list[dict[str, Any]]] = {}
    for row in normalise_otc_rows(rows):
...
def build_finra_otc_internalization_candidates(
    *,
    rows_by_ticker: dict[str, list[dict[str, Any]]],
    otc_by_ticker: dict[str, list[dict[str, Any]]],
    tickers: list[str],
    as_of: str,
    ...
    for ticker in tickers:
        ...
        if weekly_volume is None:
            ...
            continue
        otc_share = float(signal_row["otc_share_quantity"]) / weekly_volume
        if not (0.0 < otc_share <= float(config["max_valid_otc_share"])):
            _inc(rejects, "otc_share_out_of_range")
            continue

        prior_shares: list[float] = []
        for prior in reversed(ticker_otc):
            ...
            if prior_volume is None:
                continue
            prior_share = float(prior["otc_share_quantity"]) / prior_volume
            if not (0.0 < prior_share <= float(config["max_valid_otc_share"])):
                continue
        ...
        if baseline_share <= 0:
            ...
            continue
        share_retreat_ratio = otc_share / baseline_share
        if share_retreat_ratio >= float(config["max_share_retreat_ratio"]):
            _inc(rejects, "otc_share_not_retreating")
            continue
        ...
        candidates.append(
            {
                ...
                "date": as_of,
                "signal_date": as_of,
                "strategy": "finra_otc_internalization_retreat_candidate_pool",
                "rule_version": RULE_VERSION,
                "source_rule_version": SOURCE_RULE_VERSION,
                "score": round(share_retreat_ratio, 10),
                "share_retreat_ratio": round(share_retreat_ratio, 10),
                "otc_share": round(otc_share, 8),
                "baseline_share": round(baseline_share, 8),
                "prior_week_count": len(prior_shares),
                "week_start_date": week_start,
                "published_date": signal_row["published_date"],
                "otc_share_quantity": signal_row["otc_share_quantity"],
...
def replay_finra_otc_internalization_paper_trades(
    *,
    ...
    otc_index = otc_rows_by_ticker(otc_rows)
    universe = sorted(
        tickers
            ...
            trades.append(
                {
                    ...
                    "share_retreat_ratio": candidate["share_retreat_ratio"],
                    "otc_share": candidate["otc_share"],
                    ...
                    "rank_policy": (
                        "top1 per signal date by LOWEST share_retreat_ratio, then "
                        "lowest otc_share, then ticker"
                    ),
                    "trade_enabled": False,
                }
            )
...
def empty_finra_otc_internalization_paper_state() -> dict[str, Any]:
    return {
        "schema_version": STATE_SCHEMA_VERSION,
        "sleeve": SLEEVE_NAME,
        "updated_at": None,
...
def load_finra_otc_internalization_paper_state(
    path: Path | str = DEFAULT_STATE_PATH,
) -> dict[str, Any]:
    state_path = Path(path)
    if not state_path.exists():
        return empty_finra_otc_internalization_paper_state()
    try:
...
def save_finra_otc_internalization_paper_state(
    state: dict[str, Any],
    path: Path | str = DEFAULT_STATE_PATH,
) -> None:
...
def append_finra_otc_internalization_paper_snapshot(
...
def empty_finra_otc_internalization_paper_sleeve_snapshot(
    ...
    return {
        ...
        "unrealized_pnl": 0.0,
        "data_source": {"status": reason, "otc_row_count": 0},
...
def build_finra_otc_internalization_paper_sleeve_snapshot(
    *,
    as_of: str,
    ohlcv_by_ticker: dict[str, Any] | None = None,
    candidate_universe: dict[str, Any] | list[str] | None = None,
    otc_rows: list[dict[str, Any]] | None = None,
    ...
    if not is_us_equity_session(as_of_date):
        return empty_finra_otc_internalization_paper_sleeve_snapshot(
    ...
    if not rows_by_ticker:
        return empty_finra_otc_internalization_paper_sleeve_snapshot(as_of_date, "missing_ohlcv")
    if _index_on_date(rows_by_ticker.get("SPY") or [], as_of_date) is None:
        return empty_finra_otc_internalization_paper_sleeve_snapshot(as_of_date, "missing_spy_asof")
    ...
    otc_source_status = "provided"
    fetch_log: list[dict[str, Any]] = []
    if otc_rows is None:
        otc_rows = load_finra_otc_weekly_rows()
        otc_source_status = "local_archive" if otc_rows else "missing_local_archive"
        if cfg.get("allow_network_fetch", True):
            otc_rows, otc_source_status, fetch_log = refresh_finra_otc_weekly_archive(
                existing_rows=otc_rows,
                tickers=candidate_tickers,
                as_of=as_of_date,
                max_staleness_days=int(cfg["max_archive_staleness_days"]),
            )
    otc_rows = normalise_otc_rows(otc_rows or [])
    if not otc_rows:
        ...
        otc_source_status = f"{otc_source_status}_empty"

    working_state = deepcopy(
        state if state is not None else load_finra_otc_internalization_paper_state(state_path)
    )
    ...
    candidates, reject_counts = build_finra_otc_internalization_candidates(
        rows_by_ticker=rows_by_ticker,
        otc_by_ticker=otc_rows_by_ticker(otc_rows),
    ...
    snapshot = {
        ...
        "data_source": {
            "status": "ok",
            "otc_status": otc_source_status,
            "otc_row_count": len(otc_rows),
            "otc_fetch_log_count": len(fetch_log),
    ...
    if persist:
        save_finra_otc_internalization_paper_state(working_state, state_path)
        append_finra_otc_internalization_paper_snapshot(snapshot, snapshot_log_path)
    return snapshot


def prep_and_build_finra_otc_internalization_paper_sleeve_snapshot(
    *,
    as_of: str,
    ohlcv_dict: dict,
    spy_ohlcv=None,
    ...
    return build_finra_otc_internalization_paper_sleeve_snapshot(
        as_of=as_of,
...
def _production_impact() -> dict[str, Any]:
    return {
        ...
        "alters_exits": False,
        "alters_orders": False,
        "parity_rule": "shared_finra_otc_internalization_paper_adapter_v1",
    }

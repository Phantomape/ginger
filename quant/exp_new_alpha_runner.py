"""exp-20260427-003 post-news continuation shadow audit.

Observed-only alpha discovery. This script does not modify production strategy
logic. The shadow entry tests whether archived clean-news items with positive
...
EXPERIMENT_ID = "exp-20260427-003"
OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260427_002_post_news_continuation_shadow.json")
...
MAX_NEWS_LOOKBACK_CALENDAR_DAYS = 1
...
def _prior_trading_date(dates: list[str], idx: int, lookback: int) -> set[str]:
...
def _candidate_rows(
    ...
    for date_pos, date in enumerate(dates):
        ...
        if idx is None or spy_idx is None or idx < 1 or spy_idx < 1:
            continue
        allowed_news_dates = _prior_trading_date(dates, date_pos, MAX_NEWS_LOOKBACK_CALENDAR_DAYS)
        matched_news = [
            item
...
def main() -> int:
    ...
    payload = {
        ...
        "hypothesis": (
            "Post-news continuation after a fresh positive clean-news event may "
            "surface replayable candidates with useful forward returns and "
        ...
        "shadow_entry_definition": {
            ...
            "filters": {
                "news_lookback_trading_days_including_event_day": MAX_NEWS_LOOKBACK_CALENDAR_DAYS + 1,
        ...
        "production_promotion_reason": (
            "Shadow-only source audit. Current archive coverage must show stable "
            "multi-window edge before any production-path replay or strategy-file change."
        ),
    }

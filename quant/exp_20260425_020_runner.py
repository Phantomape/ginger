"""exp-20260425-020 residual A+B sector probe.

Runtime-only alpha search after the accepted residual sizing stack.

Hypothesis:
    The remaining post-stack leakage may be concentrated in trend sleeves that
    still show repeat stop-outs in the trade log, rather than in another
    Technology/Energy/DTE-adjacent pocket. This runner screens a small set of
    fresh, coarse residual cohorts to decide whether any deserve a single-rule
...
VARIANTS = OrderedDict([
    ...
    (
        "trend_comms_025",
        {"rules": [{"strategy": "trend_long", "sector": "Communication Services", "multiplier": 0.25}]},
    ),
    (
        "trend_comms_000",
        {"rules": [{"strategy": "trend_long", "sector": "Communication Services", "multiplier": 0.0}]},
    ),
    (
        "trend_consumer_025",
        {"rules": [{"strategy": "trend_long", "sector": "Consumer Discretionary", "multiplier": 0.25}]},
    ),
    (
        "trend_consumer_000",
        {"rules": [{"strategy": "trend_long", "sector": "Consumer Discretionary", "multiplier": 0.0}]},
    ),
    (
        "trend_comms_or_consumer_025",
        {
            "rules": [
                {"strategy": "trend_long", "sector": "Communication Services", "multiplier": 0.25},
                {"strategy": "trend_long", "sector": "Consumer Discretionary", "multiplier": 0.25},
            ]
        },
    ),
])

OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260425_020_results.json")
...
def _matching_rule(sig: dict) -> dict | None:
    for rule in _state["rules"]:
        if sig.get("strategy") != rule["strategy"]:
            continue
        if sig.get("sector") != rule["sector"]:
            continue
        return rule
    return None
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    ...
    for sig in sized:
        ...
        new_sizing["exp_20260425_020_multiplier_applied"] = multiplier
...
def _run_window(universe: list[str], label: str, cfg: dict, variant_name: str, variant_cfg: dict) -> dict:
    _state["rules"] = variant_cfg["rules"]
...
def main() -> int:
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "hypothesis": (
                    "remaining A+B leakage may be concentrated in fresh "
                    "trend sector residual cohorts visible in the post-stack "
                    "trade log"
                ),
                "variants": VARIANTS,
                "results": results,

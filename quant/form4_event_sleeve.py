"""Default-off Form 4 paper event sleeve.

This module turns the observe-only Form 4 forward queue into a paper ledger
that can accumulate forward outcomes in production runs. It deliberately does
not emit orders, change core signals, or compete for core A/B slots.
"""
...
def empty_form4_event_sleeve_state() -> dict[str, Any]:
    return {
...
def load_form4_event_sleeve_state(
    path: Path | str = DEFAULT_STATE_PATH,
) -> dict[str, Any]:
    state_path = Path(path)
    if not state_path.exists():
        return empty_form4_event_sleeve_state()
...
def save_form4_event_sleeve_state(
    state: dict[str, Any],
    path: Path | str = DEFAULT_STATE_PATH,
) -> None:
    state_path = Path(path)
...
def append_form4_event_sleeve_snapshot(
...
def build_form4_event_sleeve_snapshot(
    *,
    form4_event_queue: dict[str, Any] | None,
    as_of: str,
    ...
    """Advance the paper sleeve one production day and return an audit snapshot."""
    cfg = {**DEFAULT_CONFIG, **(config or {})}
    cfg["enabled"] = False
    cfg["trade_enabled"] = False

    working_state = deepcopy(
        state if state is not None else load_form4_event_sleeve_state(state_path)
    )
    ...
    if persist:
        save_form4_event_sleeve_state(working_state, state_path)
        append_form4_event_sleeve_snapshot(snapshot, snapshot_log_path)
    return snapshot


def empty_form4_event_sleeve_snapshot(as_of: str, reason: str) -> dict[str, Any]:
    return {
        "sleeve": SLEEVE_NAME,
    ...
    *,
    ...
    pending = sorted(
        ...
        key=lambda row: (
            ...
            -float((row.get("candidate") or {}).get("total_purchase_value") or 0.0),
    ...
    for entry in pending:
        ...
        if position["last_price"] is not None:
            _mark_unrealized(position, position["last_price"], cost)
...
def _candidate_sort_key(candidate: dict[str, Any]) -> tuple[float, str]:
    return (
        -float(candidate.get("total_purchase_value") or 0.0),
...
def _decision_id(candidate: dict[str, Any]) -> str:
    ...
    return f"{SLEEVE_NAME}:{RULE_VERSION}:{usable}:{ticker}"
...
def _production_impact() -> dict[str, Any]:
    return {
        ...
        "alters_sizing": False,
        "alters_orders": False,
        "scope": "default_off_form4_paper_event_sleeve",
    }

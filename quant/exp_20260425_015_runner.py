"""exp-20260425-015 residual trend pocket: Technology 44-64 DTE.

Hypothesis:
    After the accepted A+B residual allocation stack, Technology trend entries
    44-64 trading days before earnings are a remaining event-proximity leak.
    This is not the rejected Technology trend 59-69 DTE + gap probe: the new
    sleeve excludes the 69-DTE winner that broke that variant and uses no gap
    threshold.
...
VARIANTS = OrderedDict([
    ...
    ("trend_tech_dte4464_025", {"multiplier": 0.25}),
    ("trend_tech_dte4464_000", {"multiplier": 0.0}),
])

OUT_JSON = os.path.join(REPO_ROOT, "data", "exp_20260425_015_results.json")
...
def _patched_size_signals(signals, portfolio_value, risk_pct=None):
    original_multiplier = pe_module.TREND_TECH_DTE_RISK_MULTIPLIER
    pe_module.TREND_TECH_DTE_RISK_MULTIPLIER = 1.0
    try:
        sized = _original_size_signals(signals, portfolio_value, risk_pct=risk_pct)
    finally:
        pe_module.TREND_TECH_DTE_RISK_MULTIPLIER = original_multiplier
    ...
    for sig in sized:
        ...
        if dte is None or not (44 <= dte <= 64):
            continue
        ...
        new_sizing["trend_tech_dte4464_multiplier_applied"] = multiplier
...
def _run_window(
    ...
    return {
        ...
        "trend_tech_dte4464_multiplier": variant_cfg["multiplier"],
        "eligible_trend_tech_dte4464_signals": _state["eligible_signals"],
        "adjusted_trend_tech_dte4464_signals": _state["adjusted_signals"],
...
def main() -> int:
        ...
        for label, cfg in WINDOWS.items():
            for variant_name, variant_cfg in VARIANTS.items():
                ...
                print(
                    ...
                    f"eligible={summary['eligible_trend_tech_dte4464_signals']} "
                    f"adjusted={summary['adjusted_trend_tech_dte4464_signals']}"
                )
    ...
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(
            {
                "rule": {
                    "name": "trend_tech_dte4464_de_risk",
                    "definition": (
                        "apply a sleeve-specific risk multiplier only when "
                        "strategy == trend_long, sector == Technology, "
                        "and 44 <= days_to_earnings <= 64"
                    ),
                    "goal": (
                        "test whether residual Technology trend drag is "
                        "concentrated in the 44-64 trading-day pre-earnings pocket"
                    ),
                },
                "variants": VARIANTS,
                "results": results,

...
EXPERIMENT_ID = "exp-20260511-038"
...
MANUFACTURING_INFRA_TICKERS = ("RDW",)
...
VARIANTS = {
    "accepted_all_official_trend": {
        "description": "accepted exp-20260511-032 scope: all official Space trend_long signals get 5 ATR target",
        "target_scope_tickers": OFFICIAL_SPACE_TICKERS,
    },
    "exclude_data_vendor_trend": {
        "description": "exclude PL/BKSY data-vendor trend_long signals from the 5 ATR target helper",
        "target_scope_tickers": tuple(
            ticker for ticker in OFFICIAL_SPACE_TICKERS
            if ticker not in DATA_VENDOR_TICKERS
        ),
    },
    "launch_connectivity_only": {
        "description": "apply 5 ATR target helper only to RKLB/ASTS launch-connectivity trend_long signals",
        "target_scope_tickers": LAUNCH_CONNECTIVITY_TICKERS,
    },
    "launch_connectivity_plus_rdw": {
        "description": "apply 5 ATR target helper to RKLB/ASTS plus RDW manufacturing-infra trend_long signals",
        "target_scope_tickers": LAUNCH_CONNECTIVITY_TICKERS + MANUFACTURING_INFRA_TICKERS,
    },
}
...
def _delta(after: dict, before: dict) -> dict:
    ...
    for key in keys:
        ...
        if isinstance(a, (int, float)) and isinstance(b, (int, float)):
            out[key] = _round(a - b, 4 if key != "total_pnl" else 2)
    return out
...
def _space_trade_attribution(result: dict) -> dict:
    ...
    for trade in trades:
        ...
        for bucket, key in ((by_ticker, ticker), (by_strategy, strategy)):
    ...
    return {
        ...
        "by_ticker": {
            ...
            for key, row in sorted(by_ticker.items())
        },
        "by_strategy": {
            key: {**row, "pnl": _round(row["pnl"], 2)}
            for key, row in sorted(by_strategy.items())
        },
    }


def _install_space_policy(target_scope_tickers: tuple[str, ...]):
    original_enrich = risk_engine.enrich_signals
    original_size = portfolio_engine.size_signals
    target_scope = {ticker.upper() for ticker in target_scope_tickers}
    ...
    def enrich_wrapper(signals, features_dict, atr_target_mult=None):
        ...
        for signal in enriched:
            ...
            if ticker in target_scope and strategy == "trend_long" and atr:
                updated = risk_engine._retarget_signal_with_atr_mult(
                    signal,
                    atr,
                    ACCEPTED_SPACE_TREND_TARGET_ATR_MULT,
                )
                updated["space_trend_target_scope"] = "bucket_scope_experiment"
        ...
        for signal in sized:
            ...
            if ticker in OFFICIAL_SPACE_TICKERS and sizing:
                ...
                sizing["space_extra_risk_scalar_applied"] = _round(
                    scalar / BASE_SPACE_RISK_SCALAR, 6
                )
...
def _run_variant(name: str, target_scope_tickers: tuple[str, ...]) -> dict:
    core_universe = get_universe()
    universe = sorted(set(core_universe) | set(OFFICIAL_SPACE_TICKERS))
    original_enrich, original_size = _install_space_policy(target_scope_tickers)
    try:
        by_window = {}
    ...
    return {
        "variant": name,
        "target_scope_tickers": sorted(target_scope_tickers),
        "by_window": by_window,
...
def main() -> int:
    ...
    for name, spec in VARIANTS.items():
        ...
        variants[name] = _run_variant(name, spec["target_scope_tickers"])

    before = variants["accepted_all_official_trend"]
    ...
    candidates = [
        ...
        if name != "accepted_all_official_trend"
    ]
    best_variant = max(
        candidates,
        key=lambda variant: (
    ...
    decision = (
        "accepted_default_off_space_trend_target_scope"
        if best_variant["gate"]["passed"]
        else "rejected_space_trend_target_scope"
    )
    ...
    artifact = {
        ...
        "lane": "alpha_search",
        "hypothesis": (
            "Exit alpha: the accepted 5 ATR Space trend target may belong only "
            "to higher-convexity official-catalyst buckets, while data-vendor "
            "trend signals may be better left on the shared regime target."
        ),
        "changed_variable": "space_official_trend_target_bucket_scope",
        "single_causal_variable": (
            "which official Space economic buckets receive the 5 ATR trend target helper"
        ),
        "backtest_protocol": (
        ...
        "parameters": {
            ...
            "space_trend_target_atr_mult": ACCEPTED_SPACE_TREND_TARGET_ATR_MULT,
            "official_space_tickers": list(OFFICIAL_SPACE_TICKERS),
            "data_vendor_tickers": list(DATA_VENDOR_TICKERS),
            "variants": {
                name: {
                    "description": spec["description"],
                    "target_scope_tickers": list(spec["target_scope_tickers"]),
                }
                for name, spec in VARIANTS.items()
            },
        },
        ...
        "known_risks": [
            ...
            "Bucket-scope evidence still needs forward replacement-value confirmation before trade-enabled promotion.",
        ],
    }

    artifact_path = out_dir / "space_trend_target_bucket_scope.json"

...
EXPERIMENT_ID = "exp-20260511-106"
...
LUNAR_MANUFACTURING_TICKERS = ("LUNR", "RDW")
...
VARIANTS = {
    "accepted_exp105_stack": {
        "description": (
            "accepted exp-20260511-105 semantics: RKLB/ASTS trend_long uses "
            "7 ATR, other official Space trend_long signals use 5 ATR"
        ),
        "lunar_manufacturing_trend_target_atr_mult": 5.0,
    },
    "lunar_manufacturing_6_0": {
        "description": (
            "only LUNR/RDW lunar-manufacturing trend_long signals use a 6 ATR "
            "target; RKLB/ASTS remain at accepted 7 ATR and all other official "
            "Space trend signals remain at 5 ATR"
        ),
        "lunar_manufacturing_trend_target_atr_mult": 6.0,
    },
    "lunar_manufacturing_7_0": {
        "description": (
            "only LUNR/RDW lunar-manufacturing trend_long signals use a 7 ATR "
            "target; RKLB/ASTS remain at accepted 7 ATR and all other official "
            "Space trend signals remain at 5 ATR"
        ),
        "lunar_manufacturing_trend_target_atr_mult": 7.0,
    },
}
...
def _install_space_policy(lunar_manufacturing_target_mult: float):
    ...
    def enrich_wrapper(signals, features_dict, atr_target_mult=None):
        ...
        for signal in enriched:
            ...
            if ticker in OFFICIAL_SPACE_TICKERS and strategy == "trend_long" and atr:
                ...
                elif ticker in LUNAR_MANUFACTURING_TICKERS:
                    target_mult = lunar_manufacturing_target_mult
                ...
                updated["space_trend_target_scope"] = (
                    "lunar_manufacturing_target_experiment"
                )
...
def _run_variant(name: str, target_mult: float) -> dict:
    ...
    return {
        "variant": name,
        "lunar_manufacturing_trend_target_atr_mult": target_mult,
        "by_window": by_window,
...
def _ticket(payload: dict) -> dict:
    ...
    return {
        ...
        "title": "Space lunar/manufacturing trend target",
...
def _artifact_markdown(payload: dict) -> str:
    lines = [
        f"# {EXPERIMENT_ID} Space Lunar/Manufacturing Trend Target",
        "",
...
def build_payload() -> dict:
    ...
    for name, spec in VARIANTS.items():
        ...
        variants[name] = _run_variant(
            name,
            spec["lunar_manufacturing_trend_target_atr_mult"],
        )
    ...
    decision = (
        "accepted_default_off_lunar_manufacturing_trend_target_extension"
        if accepted
        else "rejected_lunar_manufacturing_trend_target_extension"
    )
    decision_rationale = (
        "LUNR/RDW trend target extension improved the accepted Space stack in "
        "at least two canonical windows without EV regression or unacceptable "
        "drawdown/survival damage."
        if accepted
        else (
            "LUNR/RDW trend target extension did not beat the accepted exp-105 "
            "Space stack under the three-window gate. Keep non-launch official "
            "Space trend signals at the accepted 5 ATR target."
        )
    )
    ...
    return {
        ...
        "single_causal_variable": (
            "target ATR multiple for LUNR/RDW lunar-manufacturing trend_long "
            "signals inside the default-off official Space sleeve"
        ),
        "hypothesis": (
            "RDW/LUNR lunar and space-manufacturing trend winners may deserve "
            "a wider target than the accepted 5 ATR all-non-launch Space trend "
            "target, while the accepted RKLB/ASTS 7 ATR launch-connectivity "
            "target remains fixed."
        ),
        "gate_questions": {
            "alpha_hypothesis": (
                "Bucket-specific lifecycle convexity for lunar/manufacturing "
                "Space trend_long entries."
            ),
            "prior_similar_experiments": [
                "exp-20260511-032 accepted 5 ATR for all official Space trend signals.",
                "exp-20260511-037 rejected wider targets for official Space breakouts.",
                "exp-20260511-038 rejected narrowing the 5 ATR Space trend target scope.",
                "exp-20260511-105 accepted 7 ATR only for RKLB/ASTS launch-connectivity trend signals.",
            ],
            "single_causal_variable": (
                "Only LUNR/RDW trend target ATR multiple changes; all other "
                "Space target/risk semantics are locked."
            ),
            "acceptance_standard": (
            ...
            "reproducibility": (
                ...
                "LUNR/RDW target variants across the three docs/backtesting.md "
                "fixed snapshots."
            ),
        },
        "backtest_protocol": (
        ...
        "parameters": {
            ...
            "tested_lunar_manufacturing_trend_target_atr_mult": [5.0, 6.0, 7.0],
            "official_space_tickers": list(OFFICIAL_SPACE_TICKERS),
            "lunar_manufacturing_tickers": list(LUNAR_MANUFACTURING_TICKERS),
            "locked_variables": [
        ...
        "next_evidence_needed": (
            "If rejected, do not retry nearby LUNR/RDW trend target widths on "
            "the same frozen snapshots; future work needs forward replacement "
            "value by catalyst bucket or a genuinely new official event-quality field."
        ),
        "related_files": [
...
def persist(payload: dict) -> None:
    ...
    md_path = (
        ...
        / "experiments"
        / "artifacts"
        / f"{EXPERIMENT_ID}_space_lunar_manufacturing_trend_target.md"
    )
            ...
            _safe(
                {
                    ...
                    "artifact": (
                        "data/experiments/exp-20260511-106/"
                        "space_lunar_manufacturing_trend_target.json"
                    ),
                }
            ),
            indent=2,

...
- ``docs/current_state_snapshot.md``: compact exact-state entrypoint.
...
DEFAULT_CURRENT_STATE_SNAPSHOT = REPO_ROOT / "docs" / "current_state_snapshot.md"
DEFAULT_LESSONS_DIR = REPO_ROOT / "docs" / "lessons"
DEFAULT_CONTEXT_LINE_BUDGET = 420
DEFAULT_STATE_LINE_BUDGET = 220
...
def accepted_records(records):
    return [record for record in records if _is_accepted_decision(record)]


def production_visible_records(records):
    out = []
    for record in records:
        text = json.dumps(record, ensure_ascii=False).lower()
        if "default_off" in text or "paper" in text or "production" in text:
            out.append(record)
    return out


def render_current_state_snapshot(model, state_line_budget=DEFAULT_STATE_LINE_BUDGET):
    records = model["records"]
    accepted = [record_summary(record) for record in accepted_records(records)]
    production_visible = [
        record_summary(record)
        for record in production_visible_records(accepted_records(records))
    ]
    recent_accepts = accepted[-12:]
    recent_visible = production_visible[-10:]
    report = model["report"]
    ...
    lines = [
        ...
        f"- Accepted strategy records: `{len(accepted)}`",
    ...
    lines.extend(render_experiment_rows(recent_accepts) if recent_accepts else [
        "- No accepted strategy records found in loaded logs."
    ])
    lines.extend(["", "## Recent Production-Visible Or Default-Off Accepted Records", ""])
    ...
    lines.extend([
        ...
        "## Line Budget",
        "",
        f"- Target maximum lines: `{state_line_budget}`",
    ...
    if actual_lines > state_line_budget:
        raise ValueError(
            f"current state snapshot has {actual_lines} lines, above budget {state_line_budget}"
        )
    return text
...
def write_alpha_memory(
    ...
    current_state_snapshot=DEFAULT_CURRENT_STATE_SNAPSHOT,
    lessons_dir=DEFAULT_LESSONS_DIR,
    lesson_count=12,
    recent_count=12,
    context_line_budget=DEFAULT_CONTEXT_LINE_BUDGET,
    state_line_budget=DEFAULT_STATE_LINE_BUDGET,
):
    ...
    current_state_snapshot = Path(current_state_snapshot)
    lessons_dir = Path(lessons_dir)
    context_pack.parent.mkdir(parents=True, exist_ok=True)
    current_state_snapshot.parent.mkdir(parents=True, exist_ok=True)
    ...
    state_text = render_current_state_snapshot(
        model,
        state_line_budget=state_line_budget,
    )
    current_state_snapshot.write_text(state_text, encoding="utf-8")
...
def main(argv=None):
    ...
    parser.add_argument("--current-state-snapshot", default=str(DEFAULT_CURRENT_STATE_SNAPSHOT))
    ...
    parser.add_argument("--state-line-budget", type=int, default=DEFAULT_STATE_LINE_BUDGET)
    parser.add_argument(
        "--git-ref",
        default=None,
        help=(
    ...
    if args.git_ref:
        with tempfile.TemporaryDirectory(prefix="alpha-memory-git-ref-") as tmp:
            ...
            result = write_alpha_memory(
                ...
                current_state_snapshot=Path(args.current_state_snapshot),
                ...
                state_line_budget=args.state_line_budget,
            )
        ...
        json.dumps(
            {
                ...
                "current_state_snapshot": str(result["current_state_snapshot"]),

...
def build_log_draft(experiment, judgement, before_path, after_path):
    return {
        "experiment_id": experiment.get("experiment_id"),
        "timestamp": utc_now_iso(),
        "status": judgement["decision"],
        "hypothesis": experiment.get("hypothesis"),
        "change_summary": "Generated by scripts/judge_experiment.py; fill in code changes before appending.",
        ...
        "rejection_reason": (
            None if judgement["decision"] == "accepted"
            else "No AGENTS.md Gate 4 acceptance condition was met."
        ),
        "next_retry_requires": [],
        "related_files": [_repo_relative(before_path), _repo_relative(after_path)],
        "notes": "; ".join(judgement.get("acceptance_reasons") or []),
    }


def update_result(registry, experiment_id, judgement, before_path, after_path):
    exp = get_experiment(registry, experiment_id)
    if not exp:
        raise ValueError(f"unknown experiment_id: {experiment_id}")
    exp["status"] = judgement["decision"]
